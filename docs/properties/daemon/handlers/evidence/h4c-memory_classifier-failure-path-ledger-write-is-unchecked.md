...
Superseding note: the `dream_task_commands` table and `record_dream_task_command` this file quotes were replaced by the MemoryClassifier receipt ledger (`crates/memory-store/src/memory_classifier_ledger.rs`). The failure path now calls `complete_memory_classifier_receipt` with a failed terminal kind and fails the request closed (`memory_classifier_ledger_failed`, `memory_classifier_ledger_fenced`) when that write does not land. The trail below records the defect as it stood before that change.

## Discovery trigger

`handle_memory_classifier_run_task` writes its ledger row twice, on two different exits, and
...
9627            return invalid_params_error("memory_classifier.run_task requires command_id");
9628        };
9629        if command_id.trim().is_empty() || command_id.len() > 256 {
9630            return invalid_params_error("memory_classifier.run_task command_id must be 1-256 bytes");
9631        }
```
...
9624            return invalid_params_error(format!("unknown memory_classifier task {task:?}"));
9625        }
```
...
9985                "ok": false,
9986                "code": "memory_classifier_run_failed",
...
9996                code: "memory_classifier_run_failed".to_string(),
...
10036                code: "memory_classifier_ledger_failed".to_string(),
...
`memory_classifier_ledger_failed` at `:10036`, which the failure path does not have: it
returns `memory_classifier_run_failed` at `:9996` whether or not the ledger write worked, so
...
9824                    code: "memory_classifier_ledger_failed".to_string(),
...
1. A `memory_classifier.run_task` arrives with `command_id = "dream-3"`, a valid
...
5. `:9995-9998` returns `memory_classifier_run_failed` with `last_error` as the message. The
   caller cannot tell that the ledger write also failed, because the success path's
   distinct code `memory_classifier_ledger_failed` (`:10036`) is not used here.
...
success path a failed write yields `memory_classifier_ledger_failed` (`:10036`), a distinct
code that tells the caller the run happened but was not recorded. On the failure
path a failed write yields `memory_classifier_run_failed` (`:9996`), the same code as a
...
Dependency: whether the caller retries at all on `memory_classifier_run_failed`. The handler
...
- A route with a `memories` authority in `MODULE` state at a known generation, so
  the gate at `:9638-9698` passes. The four existing `memory_classifier_run_task_*` tests
  (`:25872`, `:25899`, `:25931`, `:25977`) plus the `memory_classifier_classify_outcome`
  helper at `:25798` already build this.
- A classify producer whose every model fails, so `output.is_none()` at `:9983`
  holds. `memory_classifier_run_task_requires_a_positive_timeout_ms` (`:25977`) suggests the
  fixture can control producer behaviour.
- A store fault on `record_dream_task_command`.
- Oracle: after any terminal `memory_classifier.run_task` response, assert a ledger row exists
...
- Coverage form for the preconditions, per METHOD.md: assert independently that the
  response code was `memory_classifier_run_failed` and that the classify run reached the
...
- Findings: this file documents deliberate discards when they are deliberate.
  ...
  a pass". The memory_classifier success path devotes five lines to why a purge failure cannot
