...
| Delta turns and cache hits | Invalidated by #829: the delta channel is retired and [`expand_transform_tail_delta`][expand] is deleted. Historically, the plugin's delta protocol produced a projection cache hit and a reattached prefix on steady turns. | A replayed unflagged synthetic pair needs a prior bust pass and a harness replay. |
...
| Post-commit interleaving and abort | The `#[cfg(test)]` [`between_transform_and_prepare`][hook] runs between the transform and `prepare_history_summarizer_fire`. | The hook is test-only; the Emergency95 awaits at `:8263`, `:8289`, and `:8315` are the only production windows. |
...
| Config and schedule edits | Tier files and the override can be rewritten between passes; two project roots can be bound; `Local` zone and DST are environment inputs. | The memory_classifier schedule defaults to `None`, so W6 is explicit-config-only. |
...
| [W13][w13] | A `ScriptedHost` project with a valid cron in `MODULE` authority and a `ManualClock` advanced past the instant; or a user tier with `memory_classifier_review_user_memories_schedule` set and a bound `MODULE` route. | The project, schedule, `now_ms`, and finite instant recorded at the `next_due` call. |
...
[expand]: ../../../../crates/daemon/src/lib.rs#L4214-L4301
[hook]: ../../../../crates/daemon/src/lib.rs#L8343-L8351
...
[synthetic-delta-witness]: ../../../../crates/daemon/src/lib.rs#L24080-L24361
...
[w5]: catalog.md#history_summarizer-firing-input-is-preserved-by-cheaper-construction
