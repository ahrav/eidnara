...
| Payload bytes or tuple rewritten | A raw connection altering `payloads.bytes` or `occurrences.tuple`. | `packing.required.corrupt_required_presented` | packing-required-phase-precedes-optional-work |
...
| Token limit one below the required cost | `ByteEstimator` with `token_limit = total - 1`. | `packing.required.over_budget_required_presented` | packing-required-phase-precedes-optional-work, packing-required-bytes-are-charged-untruncated |
| `EvalBudget` expired or cancelled before the phase | `EvalBudget::new` with a past deadline; `EvalBudget::cancel`. | `packing.required.budget_exhausted_before_optional` | packing-required-phase-precedes-optional-work |
| Malformed floating budget | `ClaudeTokens::from_budget` with NaN, negative, fractional, infinite, or absent values; the legacy trim with the same values. | `packing.budget.malformed_budget_presented` | packing-budget-is-an-integer-never-clamped |
