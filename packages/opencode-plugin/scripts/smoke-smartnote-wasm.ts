...
const entry = join(here, "../src/features/context/smart-notes/sandbox-runner.ts");
const outDir = mkdtempSync(join(tmpdir(), "eidnara-smartnote-wasm-smoke-"));
...
try {
    ...
    check(
        "runCompiledSmartNoteCheck is exported from bundle",
        typeof mod.runCompiledSmartNoteCheck === "function",
    );
