...
- **Primary evidence:** `ensure_fence_table` (`crates/storage/src/lib.rs:697-704`) and `run_migrations` (`crates/storage/src/lib.rs:807-815`) emit the DDL; `is_infrastructure_table` (`crates/storage/src/lib.rs:540-543`) and `require_no_shadow` (`crates/storage/src/lib.rs:424-444`) name the same two tables.
- **Existing evidence:** `fence_and_version_tables_keep_their_ddl` (`crates/storage/src/lib.rs:2263`) reads `sqlite_schema` after open and one migration and compares the recorded SQL text for both tables and the single fence row; `legacy_database_without_fence_table_uses_zero_floor` (`crates/storage/src/lib.rs:2085`) shows what a missing table does.
