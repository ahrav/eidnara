...
```rust
// crates/shm-transport/src/backend/ring.rs:1175
pub fn release(&self, identity: ReleaseIdentity) -> Result<(), LeaseError>
```

```rust
// crates/shm-transport/src/backend/ring.rs:1769
...
- `crates/shm-transport/src/lease.rs:201-207` — the receiver's own `Drop` then
...
- Existing checks, corrected: the identity-validation ladder is
  `crates/shm-transport/tests/ring.rs:152-175`; lines `:177-180` are the
...
1. **No non-test caller retains the identity `commit` returns.** Every non-test
   `commit` call site discards it: `crates/host-runtime/src/ring_transport.rs:615` and
   `:628` (`reservation.commit(body_len).map_err(|_| ())?;`),
   `packages/shm-native/src/lib.rs:929-932` and `:1043-1046`
...
2. **The only non-test direct `Ring::release` call is a receiver's own.** A search
   ...
   `packages/shm-native/src/lib.rs:327-331`,
...
- Sources examined: `ring.rs:1175-1247` (`release`), `:1670-1677`
  ...
  incarnation); `crates/shm-transport/src/lease.rs:159-207`
  (`ReceiveLease::release`, `release_once`, `Drop`);
  `packages/shm-native/src/lib.rs:75-76`, `:314-334`, `:1160-1230`;
  `crates/host-runtime/src/ring_transport.rs:492-571`, `:651-738`; a
