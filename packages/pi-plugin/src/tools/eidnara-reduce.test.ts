...
async function callTool(backends: PiRustToolBackends, params: Record<string, unknown>) {
    const tool = createEidnaraReduceTool({ rustToolBackends: backends });
...
describe("Pi eidnara_reduce tool", () => {
    ...
    it("forwards the raw drop string, the session, a stable command id, and the abort signal", async () => {
        const { calls, reduce } = recordingReduce();
        const tool = createEidnaraReduceTool({ rustToolBackends: { reduce } });
    ...
    it("maps a backend rejection to the failure wording", async () => {
        ...
        expect(rejected.text).toBe(
            "Error: Failed to queue eidnara_reduce operations. tag 9 is protected",
        );
        ...
        expect(thrown.text).toBe(
            "Error: Failed to queue eidnara_reduce operations. module unavailable",
        );
    });
