...
pub const MODEL_EXECUTION_MAX_OUTPUT_TOKENS_ENV: &str = "EIDNARA_MODEL_EXECUTION_MAX_OUTPUT_TOKENS";
pub const MODEL_EXECUTION_TEMPERATURE_ENV: &str = "EIDNARA_MODEL_EXECUTION_TEMPERATURE";

/// Pi materializes the hook per run in an owner-only temporary file and loads it last.
/// explicit extension.
pub const PI_MODEL_EXECUTION_EXTENSION_BYTES: &[u8] =
    include_bytes!("../../assets/pi-model_execution-extension.mjs");

/// Pi materializes the hook in each run's private directory under this name.
pub const PI_MODEL_EXECUTION_EXTENSION_FILE: &str = "pi-model_execution-extension.mjs";
...
async fn run_pi(
    ...
    let hook_write = {
        let mut write = std::pin::pin!(dir.write_private_async(
            PI_MODEL_EXECUTION_EXTENSION_FILE.to_owned(),
            PI_MODEL_EXECUTION_EXTENSION_BYTES.to_vec(),
        ));
