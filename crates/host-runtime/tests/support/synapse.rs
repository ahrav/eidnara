...
pub fn sha256_hex(text: &str) -> String {
    host_runtime::synapse::protocol::sha256_hex(text.as_bytes())
}
...
pub fn request_key(lane: &LaneInfo, items: &[(String, String)]) -> String {
    let batch_items: Vec<host_runtime::synapse::jobs::BatchItem> = items
        .iter()
        .map(|(id, text)| host_runtime::synapse::jobs::BatchItem {
            id: id.clone(),
            content_sha256: sha256_hex(text),
            text: text.clone(),
        })
        .collect();
    host_runtime::synapse::protocol::canonical_request_key(lane, &batch_items)
}
