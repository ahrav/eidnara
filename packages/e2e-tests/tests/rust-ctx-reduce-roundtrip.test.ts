/**
 * A ctx_reduce drop completes when the next cache-busting pass consumes its ledger entry; a
...
describe.skipIf(!rustPrereqs.ok)("rust invariant: ctx_reduce round-trip", () => {
    ...
    it("consumes an agent ctx_reduce drop on the next producer-backed bust", async () => {
