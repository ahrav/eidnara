...
use host_runtime::local_embeddings::jobs::{AdmitOutcome, BatchItem, JobTable, PollOutcome};
use support::local_embeddings::{
    BUDGET, DeterministicEngine, LocalEmbeddingsHost, batch_params, call, constraints, items,
    open_local_embeddings_route, ready_component, request_key, test_lane,
};
...
async fn failed_jobs_report_their_stored_error() {
    let engine = DeterministicEngine::new();
    engine.fail_next(
        host_runtime::local_embeddings::inference::InferenceError::Invariant(
