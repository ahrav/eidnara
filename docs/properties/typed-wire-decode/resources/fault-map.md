...
| `typed-wire-resources-held-pool` | fired | `a_pool_with_room_for_the_prefix_only_refuses_before_the_large_string_is_unescaped` and the drained-pool probes. |
| `typed-wire-resources-after-holder-release` | unfired | No test releases the holder and retries the same body. |
