...
/// `EIDNARA_BROCA_CHILD_ENV` makes a Broca child exit plugin initialization before migration, database access, hooks, timers, or RPC startup.
/// `packages/plugin/src/index.ts`.
pub const EIDNARA_BROCA_CHILD_ENV: &str = "EIDNARA_BROCA_CHILD";
...
async fn run_opencode(
    ...
    child_env.push((OsString::from(EIDNARA_BROCA_CHILD_ENV), OsString::from("1")));
