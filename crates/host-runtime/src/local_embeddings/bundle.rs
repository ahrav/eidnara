...
use super::{LocalEmbeddingsLimits, jobs};
...
impl std::fmt::Display for BundleError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "invalid local_embeddings bundle: {}", self.0)
    }
}
...
pub fn load_bundle(
    ...
    if let Some(expected) = expected_manifest_sha256
        && sha256_hex(&manifest_bytes) != expected
    {
        return Err(err(
...
pub(crate) fn max_queued_input_bytes(limits: &LocalEmbeddingsLimits) -> Option<u64> {
...
fn max_retained_job_bytes(limits: &LocalEmbeddingsLimits) -> Option<u64> {
...
pub(crate) fn max_retained_input_bytes(limits: &LocalEmbeddingsLimits) -> Option<u64> {
...
/// `LocalEmbeddingsLimits` validation is a pure function of the configured limits.
/// A `LocalEmbeddingsLimits` violation fails initialization as an operator configuration error.
/// Manifest dimensions or recommended rows that exceed configured limits degrade the lane.
pub(crate) fn validate_limits(limits: &LocalEmbeddingsLimits) -> Result<(), BundleError> {
...
pub fn canonical_fingerprint(manifest: &BundleManifest) -> String {
    ...
    let mut lines = String::from("eidnara-local-embeddings-fingerprint-v1");
...
mod tests {
    ...
    fn maximum_batch_result_must_fit_retention() {
        ...
        let mut limits = LocalEmbeddingsLimits::default();
    ...
    fn zero_job_and_retention_counts_are_rejected() {
        for (mutate, fragment) in [
            (
                (|limits: &mut LocalEmbeddingsLimits| limits.max_retained_jobs = 0)
                    as fn(&mut LocalEmbeddingsLimits),
                "retained job count",
            ),
        ...
        assert!(validate_limits(&LocalEmbeddingsLimits::default()).is_ok());
    }
