...
`crates/host-runtime/src/setup_socket.rs`, `activate_server`:
...
The value and the teardown, in `crates/host-runtime/src/connection.rs`:
...
- `crates/host-runtime/src/config.rs:227` ships `transport_setup_deadline: Duration::
...
The peer already exists. `crates/host-runtime/tests/shm_failure_modes.rs:44-58` is the
...
forever. It runs against a real host in CI, at `the source repository `ci.yml` workflow:133`.
