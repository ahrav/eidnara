...
a local `cargo test`. Invalidated in part: `tag_baseline_warm_hydration_50k` is
deleted with the tag baseline cache, leaving `apply_once_stage_timings_large_fixture`
and `full_module_pass_timing_fixture` as the two ignored tests. Zero `#[ignore]` in the other six 4e files.
