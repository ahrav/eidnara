...
/// Descriptor slots and lease bound of `HOST_TEST_RING_PROFILE`.
pub const HOST_TEST_RING_DEPTH: usize = 8;

/// Geometry named by `HOST_TEST_RING_PROFILE`, so a peer or harness that echoes that id exercises the depth and topology the host actually creates.
...
mod tests {
    ...
    /// it and the depth rather than reading the constants it is checking.
    #[test]
