...
- A word-wide path in `copy_out`/`copy_in` for aligned spans, if `to_vec` or `write` shows up in a profile; it needs the peer to also use word-wide accesses on the same bytes.
...
- KTD4. **`LeaseSpan::new` stays `unsafe fn`; every span access is one atomic byte.** `new` gains a `# Safety` section naming two obligations: mapped and readable for `'lease`, and no `&[u8]` or `&mut [u8]` formed over the range by this process while a `LeaseSpan` exists. `read_byte`, `checksum`, and the two copy routines (`copy_out` from the mapping into a `&mut [u8]`, `copy_in` from a `&[u8]` into the mapping; together they replace `volatile_copy`) use `AtomicU8::from_ptr(p)` only on the shared-memory side, because `from_ptr` requires read-and-write validity that a slice-derived pointer cannot promise; the ordinary-memory side uses plain slice access. No word-wide path: Miri reports a byte-wide store racing a word-wide load as a mixed-size data race, and the same rule would force the peer to promise its access width. One width everywhere removes that promise. Measured on this host: the old `[u8; 8]` volatile copy ran at 1.8 GB/s, per-byte relaxed atomics at 3.4 to 4.9 GB/s, so the change is faster, not slower; a word-wide path (30 GB/s) is recorded as follow-up work if a profile ever asks for it. The remaining assumption, recorded in the evidence file, is that the foreign process writes with whole-byte stores, which every store instruction does. Rationale: it turns a documented gap into a Miri-checked property.
...
- **Approach:**
  1. Change `LeaseSpan::read_byte` and `checksum` to load through `AtomicU8::from_ptr(..).load(Relaxed)`; replace `volatile_copy` with `copy_out(*const u8, &mut [u8])` and `copy_in(&[u8], *mut u8)`, per-byte atomics on the mapping side only (KTD4). Update `# Safety` on `LeaseSpan::new` to two obligations.
...
- **Patterns to follow:** the existing volatile-copy alignment test, rewritten as `copy_in_then_copy_out_round_trips_at_every_alignment_and_length`.
- **Test scenarios:**
  ...
  - `copy_in_then_copy_out_round_trips_at_every_alignment_and_length` passes for every alignment of source and shared destination.
