...
Verification. `verify_ort_library` (`src/local_embeddings/inference.rs:105-161`)
...
`src/local_embeddings/bundle.rs:641-664`), bounds the length by
...
`Backend::load` once per activation (`src/local_embeddings/mod.rs:1035-1036`).
...
`EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY` (`tests/local_embeddings_bundle.rs:29-41`,
`tests/local_embeddings_roundtrip.rs:27-38`), which CI does not set.
...
1. A load-path assertion with the real runtime: after `Backend::load`
   ...
   on-disk path. This needs `EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY`.
...
- Sources examined: `src/local_embeddings/inference.rs:71-95`, `:105-161`, `:343-384`.
...
- Sources examined: `src/local_embeddings/inference.rs:85-91`; `Cargo.toml:21`
