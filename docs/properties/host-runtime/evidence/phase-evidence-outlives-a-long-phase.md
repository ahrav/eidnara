...
- The freshness window is **60 seconds**, `Duration::from_secs(60)`, set in
  ...
  `crates/host-runtime/src/lifecycle.rs:770-776`, value at `:773`. The struct is
  `:765-768`.
- It is not configurable. The sole production construction is
  `ProbeFreshness::default()` at
  `crates/daemon/src/bin/eidnara-host.rs:402`, inside `probe()`. There is no
  CLI flag, no `HostTiming` field, and no environment override; the only other
  constructions in the tree are in `crates/host-runtime/tests/lifecycle.rs` and
  `crates/daemon/tests/lifecycle_cli.rs`.
...
- The flip is terminal for the CLI. `settle_probe`
  (`eidnara-host.rs:408-418`) re-probes only while the state is `Starting` or
...
- Sources examined: `config.rs:200-232` (`HostTiming` fields and defaults),
  ...
  `generation.rs:412-450` and `:657-664`; `eidnara_host/serve.rs:1-8` and
  `:544-548`.
