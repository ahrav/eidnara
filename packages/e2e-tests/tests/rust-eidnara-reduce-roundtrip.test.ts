/**
 * A eidnara_reduce drop completes when the next cache-busting pass consumes its ledger entry; a
 * history head may replace its `[dropped §N§]` sentinel. `session.status` exposes the ledger
 * as `pending_drop_count`.
 */
...
describe.skipIf(!rustPrereqs.ok)("rust invariant: eidnara_reduce round-trip", () => {
    ...
    it("consumes an agent eidnara_reduce drop on the next producer-backed bust", async () => {
        ...
        for (let i = 1; i <= 3; i += 1) {
            ...
            await h.sendPrompt(sessionId, `eidnara_reduce turn ${i}: ${h.ballast(1_500)}`);
        }
        await Bun.sleep(500);
        ...
        h.mock.addMatcher((body) => {
            ...
            const name = (
                tools.find((t) =>
                    /eidnara_reduce/.test(String((t as { name?: unknown })?.name ?? "")),
        ...
        await h.sendPrompt(sessionId, `eidnara_reduce turn 4: reduce tag ${dropTag}`);
        const queued = (await h.host.primaryStatus(
            sessionId,
            h.env.workdir,
            "session.status",
        )) as ModuleStatus;
        ...
        expect(
            ...
            "queued eidnara_reduce drop must be pending before the bust",
        ).toBeGreaterThan(0);
        ...
        for (let i = 5; i <= 10; i += 1) {
            ...
            await h.sendPrompt(sessionId, `eidnara_reduce pressure turn ${i}: ${h.ballast(2_500)}`);
            await Bun.sleep(200);
            if (i === 5) {
        ...
        const finalStatus = (await h.host.primaryStatus(
            sessionId,
            h.env.workdir,
            "session.status",
        )) as ModuleStatus;
        expect(finalStatus.pending_drop_count ?? -1).toBe(0);
        expect(finalStatus.history_segment_count ?? 0).toBeGreaterThan(0);
        expect(await h.host.backendRequestCount()).toBeGreaterThan(0);
