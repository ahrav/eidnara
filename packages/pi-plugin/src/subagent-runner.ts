...
} from "@eidnara/opencode/shared/harness-provider-map";
 ...
 * Subagents without the extension run without Eidnara tools.
 */
...
export const EIDNARA_PI_SUBAGENT_ENV = "EIDNARA_PI_SUBAGENT";
...
function normalizedOmpProfile(): string | undefined {
    ...
    return raw && raw !== "default" && /^[a-z0-9][a-z0-9._-]{0,63}$/.test(raw) ? raw : undefined;
}
...
function getHostAgentSettingsDir(): string {
    ...
    const configRoot = join(homedir(), process.env.PI_CONFIG_DIR?.trim() || ".omp");
...
export class PiSubagentRunner implements SubagentRunner {
    ...
    private async runOnce(
        ...
        if (options.systemPrompt.length > 0) {
            try {
                systemPromptTempDir = mkdtempSync(join(tmpdir(), "eidnara-pi-prompt-"));
            ...
            try {
                child = this.spawnImpl(
                    ...
                    [...this.invocation.prefixArgs, ...this.extraArgs, ...args],
                    {
                        ...
                        // `EIDNARA_PI_SUBAGENT_ENV` must not replace `process.env`.
                        env: {
                            ...process.env,
                            [EIDNARA_PI_SUBAGENT_ENV]: "1",
                        },
...
function isSilentNoAssistantFailure(result: SubagentRunResult): result is FailedRunResult {
    return (
        !result.ok && result.reason === "no_assistant" && result.meta?.sawProtocolOutput === false
    );
}
...
export function buildArgs(
    ...
    const args: string[] = [
        ...
        // Known Eidnara children receive an explicit `--tools` allowlist.
    ];
    ...
    if (typeof options.model === "string" && options.model.length > 0) {
        ...
        args.push("--model", opts?.modelRef ?? resolveModelRefForHost(options.model));
    }
