...
2. **`wire.rs` is not 4e.** The map assigns `src/wire.rs` (1,279) to
   ...
   where a 4e property depends on it, at `wire.rs:440-451`.
...
| 19 | "SQLite probes and row hydration always happen after the snapshot has been copied out." (`transform.rs:7545`) | The tag baseline cache lock is not held across store I/O. | **Invalidated**. The tag baseline cache and `load_cached_tags` are deleted; `load_window_tags` (`transform.rs:7253`) reads the window's tag rows with no process-global lock, checked by `window_tag_read_keeps_every_session_relative_tag_decision` (`:24155`) and `every_pass_read_is_bounded_independent_of_history_size` (`transform_read_bound.rs:440`) |
...
6. **`memory-render-golden.json` is an orphan fixture.** It is generated by
   `crates/context-core/testdata/gen-golden.ts:339` and holds 6 cases
...
- **The only report of a release-build content removal is a stderr line.**
  ...
  `daemon: duplicate_tool_use_id session={} id={} message_index={} block_index={} action=drop_later`.
...
- **CI builds the debug profile.** `ci.yml:169` is
  `cargo build -p daemon --bin eidnara-host`, with no `--release`, so the
  artifact CI produces selects the panicking arm. Whether the *distributed*
  `eidnara-host` is a release build is unresolved and needs the release pipeline;
...
1. **Enumerate.** All `#[test]` and `#[tokio::test]` attributes. Re-counted
   ...
   **5** in `mod nudge_formula_tests` (`:9629`, extent `:9629-9783`, attributes
   ...
   (`../../transform/existing-checks.md:55-62`).
...
(`../../transform/existing-checks.md:68-74`): 210 whole-pass drivers,
...
assigns to 4e (`../../transform/existing-checks.md:60`): `:9662`
...
**Table-driven fixture inventory**, re-derived from `crates/daemon/testdata/`
at `HEAD`:
...
| `decay-store-differential.json` | 4 | `decay_render.rs:710` | `crates/daemon/testdata/gen-decay-store-differential.ts` |
...
1. **The only `daemon` test invocation in any workflow is
   `cargo test -p daemon --test lifecycle_cli`.** It is `ci.yml:168` at
   ...
   `--lib` target, so no in-crate `daemon` test is compiled.
2. **The other `daemon` step is build-only:**
   `cargo build -p daemon --bin eidnara-host`, `ci.yml:165` at `76cd6f41` and
   `:169` at `HEAD`. No `--release`, so the CI artifact is a debug build. This is
   the fact the debug-versus-release section turns on.
3. **The only other `daemon` mention in any workflow is a comment**,
   `ci.yml:361`.
...
(`../../facade/existing-checks.md:163-227`). 4e's pattern is different
...
`packages/plugin/src/features/eidnara/memory/constants.ts` and parses three
...
`wire.rs:440-451`: a singleton call's `arc_id` is the call block's own
...
the invariant; they assume it. `wire.rs` is 4f scope, so a 4f change can
...
5. **`memory-render-golden.json` is generated, committed and unread.** 6
   ...
   `crates/context-core/testdata/gen-golden.ts:339`, with no Rust consumer at `HEAD`.
...
- Which build profile does the distributed `eidnara-host` use? `ci.yml:169` builds
...
- Does the `tail_hygiene.rs:364` disjointness assumption need a local guard? It
  currently depends on `wire.rs:440-451`, which is 4f scope, and the
