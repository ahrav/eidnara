...
use support::local_embeddings::{
    BUDGET, DeterministicEngine, LocalEmbeddingsHost, batch_params, call, constraints, items,
    open_local_embeddings_route, ready_component, request_key, send_call, sha256_hex, test_lane,
};

use host_runtime::local_embeddings::{LocalEmbeddingsLimits, protocol};

const TY_ERROR: u8 = 5;

async fn spawn_query(
    host: &LocalEmbeddingsHost,
    lane: &host_runtime::local_embeddings::LaneInfo,
    text: &str,
    deadline_ms: u64,
) -> tokio::task::JoinHandle<support::raw_client::RawFrame> {
    let mut client = host.client().await;
    let (channel, epoch) = open_local_embeddings_route(&mut client).await;
...
fn waiter_limits(max_waiting_queries: usize) -> LocalEmbeddingsLimits {
...
async fn bounded_query_waiters_are_fifo_and_reject_bound_plus_one() {
    ...
    let host = LocalEmbeddingsHost::start(ready_component(engine.clone(), limits)).await;
...
async fn expired_waiter_releases_its_slot_without_engine_work() {
    ...
    let (channel, epoch) = open_local_embeddings_route(&mut expired_client).await;
...
async fn mixed_batch_and_query_waiters_share_fifo_cpu_without_starvation() {
    ...
    let (batch_channel, batch_epoch) = open_local_embeddings_route(&mut batch_client).await;
    let batch = call(
        &mut batch_client,
        batch_channel,
        batch_epoch,
...
async fn shutdown_cancels_waiters_but_drains_started_query() {
    ...
    let (channel, epoch) = open_local_embeddings_route(&mut waiting_client).await;
...
async fn route_loss_drops_queued_query_without_engine_work_and_releases_slot() {
    ...
    let (channel, epoch) = open_local_embeddings_route(&mut lost).await;
    let mut params = constraints(&lane);
...
fn unservable_startup_limits_are_refused_before_publication() {
    let engine = DeterministicEngine::new();
    host_runtime::local_embeddings::LocalEmbeddingsComponent::ready_with_engine(
        test_lane(),
        engine.clone(),
    ...
    for (limits, fragment) in cases {
        let error =
            match host_runtime::local_embeddings::LocalEmbeddingsComponent::ready_with_engine(
                test_lane(),
                engine.clone(),
                limits,
            ) {
...
async fn boundary_waiters_with_maximal_texts_are_all_admitted() {
    ...
    let host =
        LocalEmbeddingsHost::start(ready_component(engine.clone(), waiter_limits(BOUNDARY))).await;
    let (clock_running, clock_task) = keep_paused_clock_manual();
    let lane = test_lane();
...
async fn models_list_returns_exactly_the_certified_entry() {
    ...
    let host = LocalEmbeddingsHost::start(ready_component(engine, limits)).await;
    let mut client = host.client().await;
    let (channel, epoch) = open_local_embeddings_route(&mut client).await;
...
async fn query_overload_preserves_tool_provider_capacity() {
    ...
    let host = LocalEmbeddingsHost::start_with(ready_component(engine.clone(), limits), |config| {
    ...
    let (first_channel, first_epoch) = open_local_embeddings_route(&mut first_client).await;
    ...
    let (overload_channel, overload_epoch) =
        open_local_embeddings_route(&mut overload_client).await;
    ...
    let (tool_channel, tool_epoch) = tool_client
        .route_open("context", "/workspace/project", "opencode", "tool")
        .await
...
async fn maximum_page_with_escaped_ids_fits_its_output_reservation() {
    ...
    let host =
        LocalEmbeddingsHost::start(ready_component(engine, LocalEmbeddingsLimits::default())).await;
    let mut client = host.client().await;
    let (channel, epoch) = open_local_embeddings_route(&mut client).await;
    let frame = call(
        &mut client,
