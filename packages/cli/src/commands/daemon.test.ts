...
describe("daemon command contract", () => {
    ...
    test.each([
        ...
        h.dependencies.env = {
            ...
            EIDNARA_TEST_DATA_DIR: "relative",
            HOME: "relative",
        };
        h.dependencies.createPolicy = (env) => new HostLifecyclePolicy({ env });
    ...
    test("redacts lifecycle roots and secret-shaped native version text", async () => {
        ...
        expect(output).toContain("payload <data-root>/eidnara/bin");
    });
    ...
    test("redaction failures fall back per value instead of rejecting", async () => {
        ...
        expect(output.versions.daemon).toBe("eidnara-host/0.1.0");
    });
