...
export const CONTEXT_RESEARCHER_SYSTEM_PROMPT = `You are ContextResearcher, a focused memory-retrieval subagent for an AI coding assistant.

Your job is to search the project's durable memories and return a concise augmentation for the user's prompt.

Rules:
- Use eidnara_search(query="...") to look up relevant project memories before answering. It searches memories only, not session facts or conversation history.
...
export function isEmptyContextResearcherResult(text: string): boolean {
    const trimmed = text
        .trim()
        .toLowerCase()
        .replace(/[.!]+$/, "");
