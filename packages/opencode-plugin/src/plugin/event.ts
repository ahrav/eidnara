export function createEventHandler(args: {
    ...
    return async (input): Promise<void> => {
        // The `finally` block runs disposal cleanup even when `args.eidnara?.event` rejects.
        // The original rejection still propagates; cleanup failures never do.
        try {
            await args.eidnara?.event?.(input);
        } finally {
            if (args.onInstanceDisposed && input.event?.type === "server.instance.disposed") {
                const directory = (input.event as { properties?: { directory?: string } })
