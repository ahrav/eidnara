...
| tokenizer-pattern-is-upstream-with-ecmascript-whitespace | T1 as pattern drift: an edit to `assets/claude.pat` or to `ecmascript_whitespace!` that moves piece boundaries | Partial - `pattern_is_upstream_with_ecmascript_whitespace` detects asset and `concat!` edits but expands the macro on both sides, so it cannot see a macro edit; `whitespace_class_matches_ecmascript_not_unicode_white_space` pins 16 of 25 members (U+FEFF in, U+0085 out) and leaves U+2001 through U+2009 unasserted; the literal-set check in item 4 below is the missing case |
...
| tokenizer-encoding-is-deterministic-across-calls-and-threads | Concurrent first callers racing the `OnceLock` initialisation; repeated calls on one input | Partial - `deterministic_across_calls` constructs the repeated case; `deterministic_across_threads` constructs concurrency after initialisation but not a reliable cold start (siblings in the same binary initialise `TOKENIZER` first) and never encodes an over-cap input, so the pattern `OnceLock` is unraced; an isolated cold process with an over-cap input is the missing case |
