...
**Review operations (protocol 3).** `review.list` and `review.read` are local reads of the bound project's completed Curator review outcomes and the proposals they select. Neither contacts a model, mutates a receipt, or accepts anything; acceptance is a later protocol. They do not depend on the receipt limit set: a route bound to a root the binding does not accept answers the kernel routes' `invalid`/`project_mismatch` state and an unready Kernel its kernel state, as every kernel route does; an uninstalled Memory Store, or a root whose memories authority is not MODULE, answers `{"kind":"terminal","terminal":"disabled"}`. A closed activation gate does not disable them, because an outcome already recorded is readable whether or not new disclosure is admitted. Bodies are parsed strictly after the envelope keys are removed; any other unknown field is `invalid_params`.

`review.list` carries an optional `after` (a causal identity; the page starts after it) and an optional `limit` (1–64; absent, zero, or larger is clamped into that range). It answers `{"kind":"page","items":[...],"next": string | null}`, the completed receipts in causal-identity order, `next` the last identity when the page is full and `null` otherwise; a page exactly filling the limit yields a `next` whose follow-up may be empty. A receipt still in progress is not an outcome and is not listed. Each item is `{"causal_identity": hex64, "generation": u64, "outcome": "complete" | "abstained" | "failed" | "cancelled" | "unknown" | "expired", "selected": bool}` with `"reason"` present only under `abstained`, one of `owner_sensitive`, `wrong_scope`, `secret`, `expectation_changed`, `undisclosed_citation`, `partial_disclosure`, `model_declined`, `budget_exhausted`. An item lists from the receipt row alone; no Kernel row is read and no payload, evidence, or project text appears. A store that cannot be read answers `{"kind":"terminal","terminal":"store_unavailable"}`.

`review.read` carries `causal_identity` (hex64; any other spelling is `invalid_params`) and answers the proposal its completed receipt selects. The proposal is read under the Kernel project digest the Curator worker stages the authority project's jobs under (its newest bound root), so every root bound to the project reads the same proposal: `{"kind":"proposal","causal_identity","reference":{"database_incarnation_id","candidate_id","payload_digest"},"proposal": <staged proposal>,"review_expires_at": i64}`. The proposal is the Kernel's staged `ReviewProposal` as serialized: `action`, `target`, optional `new_text`, `support`, `contradictions`, `limitations`, `uncertainty`, `manifest`, `policy_dependencies`. `review_expires_at` is the live review hold's expiry and the only source of the review deadline. Otherwise the answer is `{"kind":"terminal","terminal": t}` with `t` one of `not_selected` (no job, no receipt, a receipt in progress, or a terminal that selected nothing, including every abstention), `incarnation_mismatch` (the receipt binds another Kernel or Memory Store incarnation than the live pair), `selection_mismatch`, `kernel_refused`, `review_expired` (the review hold ended; selection never refreshes a deadline), `dependency_refused` (a disclosed input no longer passes the Kernel for a local reader), or `store_unavailable`. A staged row the Kernel classifies `Secret` is `dependency_refused`, not decoded.
