...
- `crates/shm-transport/src/backend/ring.rs:980-1048` `reserve_until`. The loop
...
- Three distinct conditions all surface as `Exhausted`, so all three are retried:
  ...
  (`crates/shm-transport/src/arena.rs:103-110`).
...
- Existing check, partial: `two_process_zero_copy_exchange_uses_authenticated_grant`
  (`crates/shm-transport/tests/ring.rs:551-592`). The parent fills the arena with
...
4. `reserve_until` keeps returning to the doorbell until the deadline and reports
   ...
   (`crates/host-runtime/src/ring_transport.rs:479-483`), the endpoint thread joins,
...
- Sources examined: `crates/shm-transport/src/backend/ring.rs:384-467`,
  `:905-978`, `:980-1048`, `:1229-1241`, `:1418-1432`, `:1470-1565`;
  `crates/shm-transport/src/arena.rs:89-112`;
  `crates/host-runtime/src/ring_transport.rs:276`, `:479-483`, `:560-630`;
  `crates/shm-transport/tests/ring.rs:128-209`, `:551-625`.
