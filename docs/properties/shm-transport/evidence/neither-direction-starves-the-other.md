...
- **First starvation path, outbound blocks inbound: superseded at HEAD.** The
  ...
  thread on `capacity_ready` (`:1075-1078`); its host caller is the test peer
  `RingClientEndpoint::send_bounded` (`ring_transport.rs:1412`), not the endpoint.
...
- Sources examined: `crates/host-runtime/src/ring_transport.rs:636-894`
  ...
  `:1403-1435` (`RingClientEndpoint::send_bounded`);
