...
`crates/shm-transport/src/arena.rs:4-6`), the control-region prefix is
...
`packages/shm-native/tests/mechanism.ts:110` is `16_384n`. It is **correct for
...
- `crates/shm-transport/fuzz/corpus/provider_grant/valid` — the golden grant
  fixture, pinned as a hex literal at `crates/shm-transport/tests/ring.rs:513-514`.
...
`profile != PROFILE` (`packages/shm-native/src/lib.rs:504-506`) and nothing
...
- Sources examined: `packages/shm-native/tests/mechanism.ts:64-135` for the
  fixture and its `loadRawAddon` path; `packages/shm-native/src/lib.rs:640-722`
  for `create_test_pair`; `crates/shm-transport/src/profile.rs:698-720` for
  `ring_profile`; `git log -1 --format=%B daf6e244`;
  `crates/shm-transport/tests/ring.rs:500-527` for the golden fixture.
...
- Missing evidence: any document or plan stating which geometry
  `host-test-ring-v1` denotes. `docs/shm-transport.md:99-104` tabulates
...
- Correction recorded while verifying: the assertion message at
  `crates/shm-transport/tests/ring.rs:518-519` instructs a human to "update the
  copy of this hex in
