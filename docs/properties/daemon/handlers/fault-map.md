...
`crates/daemon/src/lib.rs` is byte-identical to `76cd6f41` across that span so
every `lib.rs` line below holds at both commits, and the one CI step that matters
moved: `cargo test -p daemon --test lifecycle_cli` is `ci.yml:168` at
`76cd6f41` and `:172` at `HEAD`. Both are cited.

Current disposition: the mural artifact path is removed, and
`h4c-transform-writes-two-side-effects-before-its-fenced-commit` is invalidated
in [the catalog](catalog.md#h4c-transform-writes-two-side-effects-before-its-fenced-commit).
Its construction row, `transform_wrote_the_project_mural_before_the_pass`
marker, and ranking item 7 below are withdrawn, not current harness work.
Their source-catalog descriptions remain for traceability.
...
`Handler` (`direct_host.rs:67`, `direct_host.rs:149`, `host_adapter.rs:102`).
...
`execute_tag_sql_for_test` (`crates/memory-store/src/lib.rs:6431-6440`) was described
...
`daemon` already enables that feature for its tests
(`crates/daemon/Cargo.toml:66` `[dev-dependencies]`, `:71`
`memory-store = { workspace = true, features = ["test-support"] }`), and already calls
...
- `bind_authority_route` upserts `authority_route_bindings`
  (`memory-store:5124-5132`).
- `record_recomp_command` inserts `recomp_commands` (`:6816-6822`).
- `record_dream_task_command` inserts `dream_task_commands` (`:6945-6951`).
- `commit_state_import` inserts `state_imports` (`:7180-7190`).
...
and `:6114`, so a blanket trigger on `recomp_commands` fails whichever runs
...
between the cheap history_summarizer read and the fenced state-sync transaction. Note what
...
`memory-store/src/lib.rs:5500-5505` returns `row_version: None` for any session with
...
| --- | --- | --- |
| H0 test execution in CI | Any workflow job that builds and runs `daemon --lib`, or that runs the `direct_host` or `host_adapter` integration binaries | **No.** Verified across all five files in `.github/workflows/`. The only `daemon` test invocation is `cargo test -p daemon --test lifecycle_cli` (`ci.yml:168` at `76cd6f41`, `:172` at `HEAD`), which selects one integration binary and does not build `--lib`. The step above it is build-only. There is no `--lib`, no `nextest -p daemon`, and no `--workspace` test job. `scripts/test-rust.sh` (`cargo nextest run --workspace`) is wired into root `package.json` and no workflow calls it. `EIDNARA_E2E_MODE: ts` (`ci.yml:714` (source-catalog line, not present at HEAD)) keeps Rust out of the host e2e lanes deliberately, stated at `ci.yml:719-721` (source-catalog line, not present at HEAD). This costs a workflow change and no new infrastructure |
| H1 repeat delivery with a caller identity | The same logical request delivered twice carrying the same `command_id`, `import_id`, `transform_page_id`, or `expected_shadow_seq`, so the handler's own dedup path is exercised | **Yes, and it needs no fault.** Two sequential calls. `management_binding` (`:5892-5933`) requires only `v` and a `session_id` plus a matching binding, and the identities are plain request fields: `command_id` capped 1..=128 bytes for recomp (`:6008`) and 1..=256 for memory_classifier (`:9626-9631`), `import_id` capped 1..=128 (`:5639`, const `:651`). `:27182` already sends an identical `todo_state.set` twice, so the pattern is established in the suite |
...
| H8 an unwind during an applying phase | A panic, or a dropped future, at the terminal `await` while a session's phase is `Applying` | **Partial, and the two halves differ.** The consequence is cheap to construct: a test can set the phase to `Applying` by hand, exactly as `:18730` fabricates a `Collecting` phase, and then assert that later pages return `in_progress` (`:1242-1254`, surfaced at `:9501-9503`). Reaching it by a *real* unwind is not available. There is no injectable panic in `handle_transform_unpaged_value`, and the file's only production panic is `:3661` on a store-open `JoinError`, a different path. The cancellation half is unresolved rather than unavailable: `handle` (`:11963-11996`) awaits inline, so whether `host-runtime` can drop a dispatch future is Part 2a's fact. The structural side is fully verified: ten `impl Drop` blocks exist in this file (`:328`, `:497`, `:1875`, `:3063`, `:3083`, `:3097`, `:3121`, `:3174`, `:3210`, `:11919`) and none covers a staging phase, which is released by a plain statement at `:9554` |
...
`packages/plugin/scripts/drive-preseed.ts:48` (source-catalog path, not present at HEAD), so its two records are constructible
...
`memory_classifier-dispatched-attempt-always-settles-through-the-receipt`, the successor
to the invalidated MemoryClassifier record, whose row stays for traceability.
...
| --- | --- | --- |
| h4c-recomp-reset-precedes-its-ledger-row | A session with `has_history_segments` true or a nonempty `boundary_id`, so `never_minted` is false at `:6058-6059` and the early `nothing_to_do` return at `:6060-6074` is not taken. Then a fault on the second `record_recomp_command` at `:6114` **only**, not the first at `:6060` (H3) | **Yes, revised from No.** A `BEFORE INSERT` trigger with `RAISE(ABORT, ...)` on `recomp_commands` (`memory-store:6816-6822`), installed via `execute_tag_sql_for_test`, fails that write; `RAISE(ABORT)` is not swallowed by the statement's `INSERT OR IGNORE`. Precision is free here rather than difficult: the two call sites are on mutually exclusive branches, `nothing_to_do` at `:6060` versus the reset path, so a blanket trigger on the table hits only the call the record targets on a reset-path request. The recomp latch from `try_claim_recomp_session` (`:6030`) releases on the way out because `_guard` drops, so the retry is admitted and the second reset commits against a freshly loaded `row_version` |
...
| h4c-memory_classifier-failure-path-ledger-write-is-unchecked | For the main window, a classify run that exhausts its models so `output.is_none()` at `:9983`, plus a store fault on `record_dream_task_command` at `:9989` (H3). The authority gate at `:9684-9698` must pass first. For the duplicate-guard half, two **concurrent** deliveries so the in-flight guard at `:9796-9810` returns `memory_classifier_run_failed` at `:9803` | **Yes, revised from Partial.** All three halves are now constructible. The model-exhaustion state is already reachable: the fixture at `:25806-25810` poisons the route model chain to prove the classify loop ignores it. The unchecked write is inducible by an aborting trigger on `dream_task_commands` (`memory-store:6945-6951`); `RAISE(ABORT)` is not swallowed by the `INSERT OR IGNORE`. The code collision is observable from the three `memory_classifier_run_failed` sites at `:9804`, `:9968` and `:9996`, in two of which no ledger row exists **by design**, so a caller cannot tell whether one does. One correction to the earlier row, which called the duplicate-guard half "pure H1": it is not, and two sequential calls cannot reach it. `inflight_dream_commands` is inserted at `:9802` and `DreamCommandGuard` (`:9811-9814`) removes the key on return, so a second delivery that begins after the first returns never sees the key. That half needs two overlapping in-flight calls |
| memory_classifier-dispatched-attempt-always-settles-through-the-receipt | A classify run whose authority gate passes and whose model chain is exhausted, plus a store fault on `complete_memory_classifier_receipt` (a `BEFORE UPDATE OF state ON memory_classifier_receipts` trigger raising `ABORT`) (H3); for the attempt window the same trigger on `memory_classifier_attempts` `UPDATE OF terminal_kind`; for the restart windows a request dropped between `start` and `complete_memory_classifier_receipt` with the scripted producer answering `status` as `Missing`, `Terminal`, or `Active`, or never answering under a shared `timeout_ms`; for takeover a receipt left `in_progress` with no current-generation attempt other than `not_sent` rows; for cleanup fencing a generation change inside `on_start` or `on_await_output`; for the budget the durable attempt count at the budget, or one below it with a chain whose first model fails (H1 for every replay half) | **Yes.** Every window is driven by a named test in the catalog record: the receipt and attempt faults through `execute_tag_sql_for_test` triggers, the restart windows by dropping the request future while `await_output` is blocked, the stalled probe by `block_status`, takeover by a `NotSent` row, fencing by moving the receipt inside the producer hooks, and the budget by filling `memory_classifier_attempts`. Replaces the invalidated `h4c-memory_classifier-failure-path-ledger-write-is-unchecked` row above, whose unchecked write no longer exists. |
| h4c-transform-writes-two-side-effects-before-its-fenced-commit (invalidated; historical construction) | `serializer_profile == OpencodeAiSdk` and a request carrying a mural so `host_mural_artifact` returns `Some` at `:8209` and `upsert_project_mural_artifact` commits at `:8210-8215`. Then any `TransformError` from `run_transform`, reaching `reject_transform` at `:8330-8337` from `:8338-8340`. Optionally a due side-channel row so the drain at `:8252` has work | **Withdrawn. Historical assessment: yes.** No injected fault: the pass engine's own rejections are reachable from a crafted request, and `transform_failed` has exactly one site (`:8334`), so the rejection is unambiguous to observe. The mural is content-keyed by `content_hash` (`:8213`), so the double-apply is benign; the durable consequence is that a rejected pass's artifact becomes the project's inherited mural via `cc_mural_input` (`:8224`) |
| h4c-side-channel-drain-result-is-discarded-by-the-caller | A due history_summarizer side-channel row plus a delivery failure, so the store's per-row counters at `memory-store/src/lib.rs:9572-9581` report `failed > 0` while `:8252` binds the whole result to `let _` | **Yes, and the seam already exists and is already used.** `fail_next_history_summarizer_side_channel_for_test` (`memory-store/src/lib.rs:5249`) is called at `lib.rs:30041` by `status_diagnostics_surface_pending_history_summarizer_side_channel_failure` (`:30037`). That test proves the operator surface works, which bounds this record to a per-pass observability gap rather than silent loss |
...
| stagelc-applying-phase-has-no-unwind-guard | A panic inside `handle_transform_unpaged_value`, or the dispatch future dropped at the `await` at `:9528-9536`, while a session's phase is `Applying` (set at `:1298-1304`, released at `:9554`). Then a further page request for the same session, which should succeed and instead returns `in_progress` | **Partial.** The consequence is cheap: fabricate an `Applying` phase directly, as `:18730` fabricates a `Collecting` one, then assert `InProgress` (`:1242-1254`, surfaced `:9501-9503`). Reaching it by a real unwind is not available: no injectable panic on that path, and the file's only production panic is `:3661` on a different one. The cancellation half is unresolved and needs a Part 2a fact about whether `host-runtime` drops a dispatch future; `handle` (`:11963-11996`) awaits inline. The structural claim is fully verified: ten `impl Drop` blocks and none for a staging phase, against an idiom the code states itself at `:479-480` |
...
`host-runtime` can drop a dispatch future (Part 2a territory), and, newly identified
...
| `guidance_session_loaded_with_no_row_version` | `loaded.row_version` was `None` at `:7746`, so the function returned before the commit | Legal: `memory-store/src/lib.rs:5500-5505` produces it for any uncommitted session. The precondition of the silent-success record, not the violation |
...
| `side_channel_drain_reported_a_nonzero_failed_count` | The store's per-row counters (`memory-store/src/lib.rs:9572-9581`) computed `failed > 0` for the drain at `:8252` | Legal; the seam at `memory-store/src/lib.rs:5249` exists for it and `:30041` already uses it |
...
| `memory_classifier_ledger_read_found_no_row_before_a_producer_ran` | The read at `:9819-9828` returned no row and a producer was then constructed at `:9848` | Legal on every first delivery. The precondition of the second-billable-run hazard the code names at `:9816-9818`, stated without asserting a second run |
| `memory_classifier_chain_exhausted_without_an_output` | `output.is_none()` at `:9983`, so the failure-path write at `:9989` was reached | Legal; the classify ladder can exhaust its models, and `:25806-25810` already poisons a model chain |
| `memory_classifier_returned_run_failed_from_the_duplicate_guard` | The in-flight guard at `:9796-9814` returned `memory_classifier_run_failed` at `:9804` | Legal and correct: no ledger row exists on that path **by design**. It is the independent precondition of the error-code collision |
| `todo_state_set_store_returned_the_noop_variant` | The store returned `TodoStateSetOutcome::Noop` (`memory-store/src/lib.rs:2738-2741`), whose predicate at `:6737-6740` requires both `owner_message_id` and `state_hash` to match | Legal and is a genuine content-keyed no-op. The precondition of the collapsed-response record, asserted without claiming a double-apply |
...
| `side_channel_drain_attempted_more_than_it_succeeded` | The store's per-drain counters (`memory-store/src/lib.rs:9572` `attempted`, `:9575` `succeeded`) disagreed for one pass | Legal: a failing delivery produces it, and the seam at `memory-store/src/lib.rs:5249` exists for exactly that. It is the precondition of the discarded-result record stated as a fact about the drain rather than about the surface |
| `memory_classifier_duplicate_arrived_while_the_first_was_in_flight` | A second delivery found its key already present in `inflight_dream_commands` at `:9802` and returned at `:9803-9809` | Legal and deliberate, documented at `:9786-9789`. Recording it separately is what marks the response path that correctly leaves **no** ledger row, which is the precondition the memory_classifier record's scoped check depends on |
...
- `stagelc-a-restart-is-observed-with-staged-state-present` **complies.** The
  ...
  alternatives, `shutdown` returning or a fresh `Handler` observed with zero
  ...
  (`../transform/fault-map.md:204-215`): a marker that a legal-but-narrow
...
- Do not pair `always(!second_billable_run)` with `sometimes(second_billable_run)`.
  ...
  `memory_classifier_ledger_read_found_no_row_before_a_producer_ran` and
  `memory_classifier_chain_exhausted_without_an_output` instead: two independent
...
1. **H0, running the existing tests in CI, including the two integration binaries
   ...
   **zero** new records and **protects 72 existing checks**: the 69 claim-bearing
   ...
   reach 4c through a real `Handler` (`direct_host.rs:67`, `direct_host.rs:149`,
   ...
   `ci.yml:719-721` (source-catalog line, not present at HEAD) says Rust is absent from the e2e lanes because private
   `../commons` and `../hostonscious` path-deps are not provisioned, and
...
4. **H1, repeat delivery with an identity.** Also two sequential calls, and it sits
   ...
   `h4c-memory_classifier-failure-path-ledger-write-is-unchecked` by observing that
   `memory_classifier_run_failed` is returned at `:9804`, `:9968` and `:9996` with no way for
...
7. **Withdrawn: crafted transform requests reaching a rejection.** Historical
   ranking: more test-authoring work
...
9. **H8's honest half, a real unwind at the terminal await.** Needs either an
   ...
   whether `host-runtime` drops a dispatch future. It unblocks zero records beyond the
...
10. **Named store-side write failpoints, which are a clarity improvement rather
    ...
    `execute_tag_sql_for_test` (`memory-store:6431-6440`) runs an arbitrary SQL batch, is
    enabled for this crate's tests (`crates/daemon/Cargo.toml:66-72`), and is
    ...
    `h4c-memory_classifier-failure-path-ledger-write-is-unchecked`. What remains worth doing,
    ...
    following the shape of `fail_next_history_summarizer_side_channel_for_test`
    (`memory-store/src/lib.rs:5249`, consumed at `lib.rs:30041`), would say what they
    mean. They would land in `crates/memory-store`, which appears in **no** workflow, so
...
- Whether `memory_classifier_run_failed` should be split. Two of its three sites correctly
  leave no ledger row **by design**, and the success path already distinguishes
  its failure with `memory_classifier_ledger_failed` at `:10036`.
...
- Whether the absence of any 4c entry from `docs/AUDIT-KNOWN-ISSUES.md` (source-catalog path, not present at HEAD) is
...
- Whether the TypeScript memory_classifier lanes and the Rust `handle_memory_classifier_run_task`
  implement the same contract, which would make the CI-gated TypeScript suite a
  parallel-implementation gate as 4a found for the history_summarizer validator.
