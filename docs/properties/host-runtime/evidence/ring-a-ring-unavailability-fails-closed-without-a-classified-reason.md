...
degrades silently. `docs/shm-transport.md:7` is explicit about the
...
(`crates/host-runtime/src/ring_transport.rs:217-303`).**
...
An operator running `eidnara daemon doctor` gets a healthy report from a
...
- Sources examined: `ring_transport.rs:103-112` (the unit struct and its fixed
  `Display`), the five producer sites, `docs/shm-transport.md:53-59` (the
  five doctor classes), `packages/plugin/src/shared/host-client/types.ts:69-73`
