...
The hook body, `crates/host-runtime/src/panic_boundary.rs:41-47`:
...
**The daemon's stderr is a log file.** `crates/daemon/src/bin/eidnara-host.rs:424-428` resolves
`daemon_log_path()` to `coordination_dir_path(None)?.join("daemon.log")`, and
`crates/daemon/src/bin/eidnara_host/spawn.rs` opens it (`:40-53`) and redirects both standard
...
- Sources examined: `crates/daemon/src/bin/eidnara-host.rs:424-428`, `:733-737`;
  `crates/daemon/src/bin/eidnara_host/spawn.rs:37-53`, `:136-151`, `:249-250`.
