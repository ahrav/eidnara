...
| `AccessShape` identical-boundaries rule | point the writer's `copy_in` at `base..base+64` while the span reads `base+3..base+65` | `Undefined Behavior: Race condition detected between (1) 1-byte atomic load ... and (2) 8-byte atomic store`; overlapping ranges with different boundaries can assign different widths to shared bytes, so the enforced contract is that concurrent accesses are disjoint or cover the identical range |
