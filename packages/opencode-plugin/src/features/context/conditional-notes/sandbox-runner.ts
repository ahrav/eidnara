...
// Deferring the import to the first conditional-note evaluation splits the variant
...
import {
    ...
    conditionalNoteAbortError,
    isConditionalNoteNetworkError,
} from "./types";
 ...
 *
 * Dynamic imports load the QuickJS variant and runtime on the first conditional-note check.
 * Dynamic imports defer parsing the large QuickJS modules until a conditional-note check runs.
 */
 ...
 */
export interface RunCompiledConditionalNoteCheckOptions {
    compiledCheck: string;
    capabilities?: ConditionalNoteCapabilityApi;
...
export interface RunCompiledConditionalNoteCheckSuccess {
    ok: true;
    result: ConditionalNoteCheckResult;
}

export interface RunCompiledConditionalNoteCheckFailure {
    ok: false;
    cancelled: false;
    error: string;
    network: boolean;
...
export interface RunCompiledConditionalNoteCheckCancelled {
    ok: false;
    cancelled: true;
    error: string;
    network: false;
...
export type RunCompiledConditionalNoteCheckResult =
    | RunCompiledConditionalNoteCheckSuccess
    | RunCompiledConditionalNoteCheckFailure
    | RunCompiledConditionalNoteCheckCancelled;
...
function resolveCapabilitiesForRun(
    options: RunCompiledConditionalNoteCheckOptions,
    signal: AbortSignal,
): ConditionalNoteCapabilityApi {
    ...
    throw new Error("conditional-note check requires capabilities");
}

function runAbortReason(signal: AbortSignal): unknown {
    return signal.reason ?? new Error("conditional-note check aborted");
}
...
export async function runCompiledConditionalNoteCheck(
    options: RunCompiledConditionalNoteCheckOptions,
): Promise<RunCompiledConditionalNoteCheckResult> {
    ...
    const run = withSandboxLock(() => {
        acquired = true;
        return runCompiledConditionalNoteCheckLocked(options);
    });
    ...
    // stays serialized.
    return new Promise<RunCompiledConditionalNoteCheckResult>((resolve, reject) => {
        const onAbort = () => {
...
async function runCompiledConditionalNoteCheckLocked(
    options: RunCompiledConditionalNoteCheckOptions,
): Promise<RunCompiledConditionalNoteCheckResult> {
    if (options.signal?.aborted) return cancelledResult(options.signal.reason);
    ...
    const timer = setTimeout(() => {
        ...
        controller.abort(new Error("conditional-note check timed out"));
    }, timeoutMs);
    ...
    try {
        ...
        try {
            ...
            if (controller.signal.aborted) throw conditionalNoteAbortError(controller.signal);
            ...
            if (
                ...
                if (!(reason instanceof ConditionalNoteNetworkError)) {
        ...
        const network =
            ...
            (hostCalls?.rejected === true && isConditionalNoteNetworkError(hostCalls.reason));
...
function cancelledResult(reason: unknown): RunCompiledConditionalNoteCheckCancelled {
    return {
        ok: false,
        cancelled: true,
...
function installCapabilityObject(
    ...
    cap: ConditionalNoteCapabilityApi,
    signal: AbortSignal,
): HostCallLedger {
...
function raceWithAbort<T>(
    promise: Promise<T>,
    signal: AbortSignal,
    abortReason: (signal: AbortSignal) => unknown = conditionalNoteAbortError,
): Promise<T> {
...
}
{
  ...
  const poison = (name) => frozen(function () {
    throw new TypeError(name + " is nondeterministic and disabled in conditional-note checks");
  });
...
JSON.stringify(__wireResult);`;
    const evalResult = await context.evalCodeAsync(wrapped, "conditional-note-check.js", {
        type: "global",
    });
