...
Issue 316 moves MemoryClassifier scheduling into the daemon: cron schedules are read
...
`memory_classifier-dispatched-attempt-always-settles-through-the-receipt` covers what a
...
`crates/daemon/src/memory_classifier_scheduler.rs` unless another path is given;
...
- `MemoryClassifierScheduler::tick` reads the clock once for due-ness, asks the
...
- `run` logs every `Skipped`, `Retained`, and `Deferred` event to
  ...
  are not leased.
- `run_slot` leases before it runs: `acquire_memory_classifier_task` with acquisition
  ...
  `complete_memory_classifier_task` both retain the slot with its claim live. The next
...
- The command id the run is dispatched under is derived from the returned
  ...
  runs the predecessor's slot and `run_memory_classifier_task` finds the interrupted
  receipt under that command id.
- The registration generation is allocated on first use from
  `next_memory_classifier_scheduler_generation` (`crates/memory-store/src/lib.rs`),
...
- `SchedulerBridge::scheduled_projects` (`crates/daemon/src/lib.rs`)
  ...
  `Module`. That helper is the same one `run_memory_classifier_task` uses, so a store
...
- `SchedulerBridge::run_task` (`crates/daemon/src/lib.rs`) returns `NotRunnable` when
  ...
  (`MemoryClassifierRuntime::classify_inputs`), and
  otherwise calls `run_memory_classifier_task` under `SCHEDULER_LEDGER_SESSION`
  ...
  project the lease is on. `MemoryClassifierRuntime::new` sets its input builder to
  ...
  `authority_lookup_failed`, `memory_classifier_ledger_failed`, and `kernel_unavailable`
...
- A slot identifies one receipt, not necessarily one model call. The original
  `run_memory_classifier_task` chain may try several models. The receipt property prevents
...
- `run_memory_classifier_task` resolves the route again at its authority gate
...
A scheduler leases slot `t`, `run_memory_classifier_task` writes the `IN_PROGRESS`
...
`run_memory_classifier_task` for `t`, and the receipt's recovery path settles or resumes
...
- Recovery by rebind needs acquisition before the predecessor's lease expires,
  `MEMORY_CLASSIFIER_TASK_LEASE_MS` (20 min, `crates/memory-store/src/lib.rs`)
...
`memory_classifier_scheduled_run_writes_one_receipt_and_a_restart_adds_no_attempt` in
...
`MemoryStore::fail_next_memory_classifier_task_acquire_for_test`, which makes one
`acquire_memory_classifier_task` return a backend error before it touches the ledger;
`fail_next_memory_classifier_task_complete_for_test` does the same for
`complete_memory_classifier_task`, and `fail_next_authority_route_read_for_test` armed
...
`MemoryClassifierHarness::start(&producer).await` in `crates/daemon/src/lib.rs`. The main
...
The 50 MemoryClassifier tests and 50 further runs of those 50 tests at default
...
- Sources examined: `run_slot`, `acquire_task_lease` slot recovery,
  `collect_ledgers_tx`, `resume_memory_classifier_receipt`.
...
- Sources examined: `SchedulerBridge::scheduled_projects`,
  `MemoryClassifierScheduler::tick`, `due_projects`, `run`, and the wire route's
  authority gate in `run_memory_classifier_task`.
...
- Conclusion: resolved; covered by
  ...
  `memory_classifier_scheduler_bridge_reports_a_project_once_across_its_roots`.
...
- Sources examined: `SchedulerBridge::run_task`, `run_memory_classifier_task`'s
...
- Findings: `bind_authority_route` upserts the route's project, so a root can
  ...
  the run under itself. `MemoryClassifierRunRequest::leased_project` now names the
  leased project and the gate refuses a mismatch before any receipt is written.
- Missing evidence: none.
- Conclusion: resolved; covered by
  `memory_classifier_scheduler_bridge_refuses_a_root_that_moved_to_another_project`.
