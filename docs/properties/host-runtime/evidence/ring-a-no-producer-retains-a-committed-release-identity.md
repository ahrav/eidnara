...
(`crates/shm-transport/src/backend/ring.rs:1769`). The identity is minted at
...
| Site | Form | Identity |
| --- | --- | --- |
| `crates/host-runtime/src/ring_transport.rs:615` | `reservation.commit(body_len).map_err(\|_\| ())?;` | discarded at the `?` |
| `crates/host-runtime/src/ring_transport.rs:628` | `reservation.commit(body_len).map_err(\|_\| ())?;` | discarded at the `?` |
| `crates/host-runtime/src/ring_transport.rs:696` | `.commit(body.len()).map_err(\|_\| RingClientError)?;` | discarded at the `?` |
| `crates/host-runtime/src/ring_transport.rs:935` | `reservation.commit(1).unwrap();` | dropped as a statement value |
| `crates/host-runtime/src/ring_transport.rs:985` | `reservation.commit(body.len()).unwrap();` | dropped |
| `crates/host-runtime/src/ring_transport.rs:1022` | `reservation.commit(1).unwrap();` | dropped |
| `crates/host-runtime/tests/support/raw_client.rs:698` | `reservation.commit(body.len()).is_err()` | dropped |
| `crates/host-runtime/tests/support/raw_client.rs:743` | `let _ = reservation.commit(0);` | explicitly discarded |
| `crates/host-runtime/tests/support/raw_client.rs:799` | `.commit(body.len()).map_err(..)?;` | discarded at the `?` |
...
`crates/host-runtime/src/frame_channel/contract_tests.rs:567` and `:600` call
...
1. A source-level check in the same family as the `mandatory-ring-architecture`
   grep gate (`ci.yml:41-58`): assert that no `host-runtime` line binds the result of
...
- Missing evidence: out-of-tree embedders. `Ring` and `ProducerReservation` are
  `pub` in `shm-transport`, and `host-runtime` re-exports nothing that would
  hand a reservation to an embedder, but `shm-transport` is itself a
...
- Findings: nothing in the tree uses it, and nothing documents a use.
  `packages/shm-native/src/lib.rs:330` releases a consumer identity, so even
