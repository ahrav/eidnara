...
} from "@eidnara/opencode/hooks/context/channel2-cycle";
import { decideChannel1, evaluateChannel2 } from "@eidnara/opencode/hooks/context/ctx-reduce-nudge";
import { createTestDb } from "./__tests__/test-utils";
...
function textMessage(role: "user" | "assistant", text: string): Record<string, unknown> {
...
function toolArc(
    ...
    const messages = [
        {
            role: "assistant",
            content: [{ type: "toolCall", id: callId, name: toolName, arguments: input }],
        },
        {
            role: "toolResult",
            toolCallId: callId,
...
describe("Pi rendered-tail hygiene walk", () => {
    ...
    it("uses rendered sentinels, not durable tag status, to exclude a whole tool arc", () => {
        const live = toolArc("owner", "call", "read", { path: "large" }, "result ".repeat(5_000));
        const dropped = toolArc("owner", "call", "read", { path: "large" }, "[dropped §1§]");
    ...
    it("keeps the recency reserve and newest three ctx_reduce exemplars out of U", () => {
        ...
        expect(
            measured.parts.filter((part) => part.tagNumber === 1).every((part) => part.uTokens > 0),
        ).toBe(true);
        expect(
            measured.parts
    ...
    it("handles empty, all-synthetic, untagged, and all-protected degenerates", () => {
        expect(measurePiTailHygiene({ messages: [], tags: [], protectedTags: 20 })).toMatchObject({
            u: 0,
            t: 0,
        });
...
describe("Pi baseline persistence and defer deltas", () => {
    it("is deterministic across bust/defer and adds every typed append", () => {
        ...
        expect(effectivePiTailHygiene(unchanged)).toEqual(effectivePiTailHygiene(bust));
    ...
    it("advances the protection boundary additively without changing generation", () => {
        ...
        const beforeTags = [tag(1, "old:p0", "message"), tag(2, "recent:p0", "message")];
...
function adaptFixtureToPi(fixture: HygieneFixture): {
    ...
    for (const message of fixture.messages) {
        for (const block of message.blocks) {
            ...
            if (block.type === "tool_result") {
                ...
                const text = block.type === "text" ? block.unit.repeat(block.repeat) : block.url;
    ...
    const tags = fixture.tags.map((fixtureTag) => {
        ...
        const legacyCallId = !block && fixtureTag.kind === "tool" ? fixtureTag.block_id : null;
        const callId =
            block?.type === "tool_call" || block?.type === "tool_result" ? block.id : legacyCallId;
        const isTextFile = block?.type === "file" && !block.mime.startsWith("image/");
        const messageId =
...
describe("TS/Pi/module differential hygiene corpus", () => {
    const golden = JSON.parse(
        readFileSync(
            join(import.meta.dir, "../../../crates/daemon/testdata/nudge-hygiene-golden.json"),
            "utf8",
        ),
    ...
    it("keeps Pi as the third leg across the full shared corpus", () => {
        ...
        for (const fixture of golden.cases) {
            ...
            for (const [label, actual, expected] of [
                ...
                expect(Math.abs(actual - expected), `${fixture.id} ${label}`).toBeLessThanOrEqual(
                    tolerance,
                );
            }
