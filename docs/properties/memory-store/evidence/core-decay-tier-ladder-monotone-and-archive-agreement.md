...
`crates/context-core/src/decay.rs:12-13` states the model's invariants as fact:
...
- `crates/context-core/src/decay.rs:69` — `h = (H50 * f) / p` is decreasing in `p`,
  so `z` is non-decreasing in `p`.
- `crates/context-core/src/decay.rs:79-89` — the ladder is monotone in `z` by
...
- `crates/context-core/src/decay.rs:165-173` walks `idx` 1..=400 with
  `importance = 50` and `pressure = 1.0`. One slice.
- `crates/context-core/src/decay.rs:176-183` compares `importance` 10 against 90 at
  three indices with `pressure = 1.0`. Two points per index, not a sweep.
- `crates/context-core/src/decay.rs:186-192` compares `pressure` 0.5 against 4.0 at
  three indices with `importance = 50`. Two points per index.
- `crates/context-core/src/decay.rs:201-206` asserts
...
3. Assert the agreement clause with `anchor_overlap` swept over
   ...
   at `crates/context-core/src/decay.rs:102` is part of the contract:
...
- Sources examined: `crates/context-core/src/decay.rs:36` (the `Z4` doc, "P4→P5
  ...
  `crates/daemon/src/decay_render.rs:291-296`.
...
- Sources examined: `crates/context-core/src/decay.rs:201-206`.
