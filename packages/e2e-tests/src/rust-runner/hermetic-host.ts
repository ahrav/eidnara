...
interface RustE2ePidFile {
    createdAtMs: number;
    pids: Array<{ pid: number; role: "host-runtime"; executable: string }>;
}
...
export class HermeticHostStack {
    ...
    private statusClient: HostClient | null = null;
    private statusClientPromise: Promise<HostClient> | null = null;
    private stdout = "";
    private stderr = "";
    ...
    private async startHost(): Promise<void> {
        ...
        const fixtureConfigRoot = join(this.fixtureConfigDir, "eidnara");
        const fixtureConfigPath = join(fixtureConfigRoot, "eidnara.jsonc");
        ...
        const timeout = new Promise<never>((_, rejectTimeout) => {
            timeoutHandle = setTimeout(
                () => rejectTimeout(new Error("direct host fixture readiness timed out")),
                this.startTimeoutMs,
            );
        });
        try {
...
export const __hermeticHostTest = {
    FixtureControlClient,
    diagnostics(stack: HermeticHostStack): {
        stdout: string;
        stderr: string;
        retainedLog: string;
    } {
