...
- `crates/shm-transport/src/backend/ring.rs:1081-1091` — the claim happens first:
...
- `ring.rs:1452-1468` `lease_span` — its four failure modes: two
  ...
  `LeaseSpan::new`'s null-pointer check at `crates/shm-transport/src/lease.rs:21-29`.
- `crates/shm-transport/src/lease.rs:111-117` — `ReceiveLease::new`'s only
  rejection: `span_count` outside `1..=2`, `spans[0]` none, `span_count == 1` with
  `spans[1]` some, or `span_count == 2` with `spans[1]` none.
- `crates/shm-transport/src/descriptor.rs:183-283` `validate` — the constraints
...
6. Consequence: on the host this surfaces as
   ...
   (`crates/host-runtime/src/ring_transport.rs:498-499`), which ends the generation
...
- Sources examined: `ring.rs:1055-1135` (`try_receive` in full), `:1452-1468`
  (`lease_span`), `crates/shm-transport/src/lease.rs:21-29` (`LeaseSpan::new`)
  and `:102-117` (`ReceiveLease::new`),
  `crates/shm-transport/src/descriptor.rs:183-283` (`validate` in full) and
  `:14` (`MAX_SPANS`), `crates/shm-transport/src/arena.rs:4`
  (`MAX_FRAME_BYTES`).
