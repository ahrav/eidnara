...
1. Lens A's `## What persists across a reconnect` cites
   `docs/host-wire-protocol.md:12` for the client `Recovering` state. `:12`
...
2. Lens A's leads L1 and O17 cite `docs/host-wire-protocol.md:746` for the
...
3. Lens A's lead L2 cites `docs/host-wire-protocol.md:62` for `NotSent`
...
1. **The three send-outcome spellings are duplicated in TypeScript as string
   ...
   (`host-client/client.ts:323`, `:331`). Two definitions, no cross-check.
...
**`crates/host-runtime/tests/client.rs` holds 6 tests in 243 lines, and it is named in
...
| --- | --- | --- |
| `ci.yml:132` | `cargo nextest run -p host-runtime --test client`, step "Mandatory ring client suite" | Linux, job `shm-crash-recovery` (`:111`) |
| `ci.yml:178-179` | `cargo nextest run -p host-runtime \` / `--test client --test lifecycle` (one wrapped command) | `if: runner.os == 'Linux'`, job `shm-source-build` (`:137`) |
| `ci.yml:187` | `cargo nextest run -p host-runtime --test client --test lifecycle`, step "Fixed-ring contracts (macOS)" | `if: runner.os == 'macOS'`, same job |
...
crates/host-runtime/src/client.rs` returns **0**: there is no code fence, `text`
fence, or `compile_fail` block anywhere in the file's 3,998 lines.

This matters because `cargo test -p host-runtime --doc` **does** run in CI
...
| host-runtime release contract drift gate | `bun run release:contract:check` | `:87`, `:109` | pins the four byte-coupled forms of the release contract, including "the `include!`-able Rust constants that feed production epoch values" (comment at `:105-108`). Verified not to reach here: `grep -rn 'include!' crates/host-runtime/src/` returns **zero** hits, so no constant in `client.rs` is under this gate |
