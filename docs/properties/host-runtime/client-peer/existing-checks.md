...
`crates/host-runtime/src/client.rs` (3,998 lines), the integration binary dedicated to
...
| `crates/host-runtime/tests/client.rs` | whole file | 243 | **6** | **Yes**, three times |

The reason the 40 do not run is structural rather than an omission. Every
`-p host-runtime` invocation in `ci.yml` carries a `--test <name>` filter, which
selects one integration binary and never builds the lib target. Re-verified at
`HEAD`: the 13 `host-runtime` hits are `:87`, `:132`, `:133`, `:134`, `:168`, `:169`,
...
`grep -c '```' crates/host-runtime/src/client.rs` returns **0**: there is no code
fence, `text` fence, or `compile_fail` block anywhere in the file's 3,998 lines.
That matters because `cargo test -p host-runtime --doc` **does** run, at `ci.yml:190`
...
is the six integration tests in `crates/host-runtime/tests/client.rs`, plus the two
...
**`crates/host-runtime/tests/client.rs` holds 6 tests in 243 lines, and the binary is
...
| --- | --- | --- |
| `ci.yml:119` | `cargo nextest run -p host-runtime --test client`, step "Mandatory ring client suite" (`:117`) | Linux, job `shm-crash-recovery` (`:92`) |
| `ci.yml:168-169` | `cargo nextest run -p host-runtime \` / `--test client --test lifecycle`, one wrapped command | Linux (`runs-on: ubuntu-latest`), job `shm-source-build` (`:131`) |
...
| --- | --- | --- | --- |
| Plugin shared-memory contracts (Bun) | `bun test packages/plugin/src/shared/host-client` | `:211` | tests `McHostClient`, the *TypeScript* managed client. It is a sibling peer implementation of the same normative contract, not this Rust file. Its `errors.ts:12` and `:38` are the duplicate of the send-outcome spellings recorded below |
| host-runtime client interop (Bun + Node 24) | `bun run --cwd packages/plugin test:host-client:node` | `:461`, job `:442-443` | resolves to `bun scripts/run-host-client-node.ts smoke` (`packages/plugin/package.json:43`). Drives the TypeScript client against a real host, so it exercises the *host*, not `host_runtime::Client` |
| Plugin shared-memory lifetime (Node 24) | `bun run --cwd packages/plugin test:context-shm:node` | `:214` | native lease and channel lifetime across the runtime boundary; 2b scope |
| host-runtime release contract drift gate | `bun run release:contract:check` | `:87`, `:109` | pins the byte-coupled forms of the release contract, including the `include!`-able Rust constants that feed production epoch values. Verified not to reach here: `grep -rn 'include!' crates/host-runtime/src/` returns **zero** hits, so no constant in `client.rs` is under this gate |
...
`docs/host-wire-protocol.md:745`'s shared-pool sentence still binds the
...
Doc references without a file are `docs/host-wire-protocol.md`.
...
1. **The three send-outcome spellings are duplicated in TypeScript as string
   ...
   `terminal`; `packages/plugin/src/shared/host-client/errors.ts:12` restates
   them as a `readonly string[]` and `:38` as a union type, and the TypeScript
   retry policy branches on them (`host-client/client.ts:323`, `:331`).
...
6. **Historical: the polling-era bridge used a bare 50-microsecond literal.** At
   ...
   constant (`crates/host-runtime/tests/support/process_resources.rs:75`).
...
- `crates/daemon` was not read beyond confirming the three production
