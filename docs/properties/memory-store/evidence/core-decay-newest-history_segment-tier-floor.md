...
`newest_history_segment_is_tier_1` (`crates/context-core/src/decay.rs:154-162`), whose
...
- `crates/context-core/src/decay.rs:65` — `let a = (history_segment_index.max(1) - 1) as f64;`
  For `history_segment_index` of 0 or 1 this is exactly `0.0`.
...
- Measured in the same scratch program: `history_budget = 5e-324` and
  ...
  history_segment slice. `history_budget` of NaN or `+inf` both yield `0.1`.
...
evaluated for every history_segment. HistorySegments at index 2 and beyond get
`z = +inf` and archive, which is the sane degenerate answer. The single newest
history_segment, at index 1, gets `z = NaN` and is rendered at tier 4 instead of
tier 1.
...
`should_archive` disagree for that history_segment: a consumer reading `tier`
