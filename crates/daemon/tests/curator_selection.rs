...
use memory_store::curator_jobs::{
    CausalInputs, CuratorJobOutcome, MAX_SELECTION_REFERENCES, ProducerBinding, ReviewTarget,
};
...
fn intent(key: &str) -> CommitIntent {
    CommitIntent {
        producer: "curator-selection-test".to_string(),
...
impl Fixture {
    fn open() -> Self {
        ...
        let ledger = MemoryStore::open(&MemoryStore::test_descriptor(
            ledger_dir.path(),
            "eidnara-curator-selection-test",
        ))
        .unwrap();
        Self {
    ...
    fn targets(page: &memory_store::curator_jobs::FrozenSelectionPage) -> Vec<String> {
        page.references
            .iter()
...
fn targets_with_a_job_and_targets_the_remote_fold_denies_are_passed_over() {
    ...
    fixture
        .ledger
        .reserve_curator_job(PROJECT, &producer, &page.references[0], fixture.now)
        .unwrap();
    let mut versioned = page.references[1].clone();
    versioned.policy_versions = BTreeMap::from([("egress".to_string(), "v2".to_string())]);
    fixture
        .ledger
        .reserve_curator_job(PROJECT, &producer, &versioned, fixture.now)
        .unwrap();
