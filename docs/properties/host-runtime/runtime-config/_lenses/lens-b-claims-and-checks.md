...
| 2 | CLI or config-file exposure of these knobs belongs to the source daemon-lifecycle work, not this crate | `config.rs:5-6` | **Nothing, and nothing should.** A negative claim; recorded because it is what makes doc comments the only contract |
...
### L1. `HostInit::host_capabilities` is inert, and it is the one field the redaction impl does not redact

`config.rs:250` declares `pub host_capabilities: Vec<String>` with no doc
comment. A repo-wide search returns **six** occurrences and nothing else:

| Site | Role |
| --- | --- |
| `crates/host-runtime/src/config.rs:250` | the declaration |
| `crates/host-runtime/src/config.rs:262` | the `Debug` render |
| `crates/daemon/src/bin/eidnara_host/serve.rs:487` | write, `Vec::new()` |
| `crates/daemon/examples/direct_host_fixture.rs:577` | write, `Vec::new()` |
| `crates/daemon/tests/host_adapter.rs:29` | write, `Vec::new()` |
| `crates/daemon/tests/host_adapter.rs:88` | write, `Vec::new()` |
...
`:133`, `:134-135`, `:178-179`, `:187`. The remaining `host-runtime` hits are `:87`,
`:168-169`, `:190`, `:211`, `:361`, `:442`, `:461`, and none is an unfiltered
`cargo nextest run -p host-runtime` or a `--lib` run.
...
This matters because `ci.yml:190` runs `cargo test -p host-runtime --doc` under the
...
`:481-483` gives the reason as the eidnara sibling stubs.
