...
- `crates/shm-transport/src/lease.rs:69-75` — `checksum`. Line 71 is
...
- `crates/shm-transport/src/lease.rs:70` — the SAFETY comment justifies it
  with "R19 forbids peer writes before release", a contract term, not a
  mechanism.
- `docs/shm-transport.md:116` — the contract term the comment leans on,
...
- `crates/shm-transport/src/lease.rs:57-66` — `copy_to` uses
  ...
  undefined behaviour.
- `crates/shm-transport/src/lease.rs:48-54` — `read_byte` uses `read_volatile`
  at `:53`. Same property.
- `crates/shm-transport/src/lease.rs:178-196` — `to_vec`, the path the host
  actually uses, is built entirely on `copy_to`, so the aggregate body read is
  sound by the same argument.
- `crates/shm-transport/src/lease.rs:38-40` — `pub const fn as_mut_ptr(self)
...
- `packages/shm-native/src/lib.rs:741` and `:817` — the same helper on the two
...
- Findings: exactly one call site exists in the entire tree —
  `crates/shm-transport/benches/hardware_envelope.rs:406`,
