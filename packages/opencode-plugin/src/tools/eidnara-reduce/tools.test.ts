...
import { createEidnaraReduceTools } from "./tools";
...
describe("createEidnaraReduceTools", () => {
    describe("eidnara_reduce", () => {
        it("requires the drop parameter", async () => {
            const { calls, reduce } = recordingReduce();
            const tools = createEidnaraReduceTools({ rustToolBackends: { reduce } });

            const result = await tools.eidnara_reduce.execute({}, toolContext());
        ...
        it("accepts reduced compatibility fields alongside a real drop", async () => {
            ...
            const result = await tools.eidnara_reduce.execute(
                {
                    drop: "3",
                    reduced: true,
        ...
        it("uses the raw drop string and stable call id in rust mode", async () => {
            const { calls, reduce } = recordingReduce();
            const rustTools = createEidnaraReduceTools({ rustToolBackends: { reduce } });
            const rustContext = {
                sessionID: "ses-1",
            ...
            const rustResult = await rustTools.eidnara_reduce.execute({ drop: "3-5" }, rustContext);
            ...
            await rustTools.eidnara_reduce.execute({ drop: "3-5" }, rustContext);
            expect(calls[1]?.commandId).toBe(calls[0]?.commandId);

            await rustTools.eidnara_reduce.execute({ drop: "3-5" }, {
                sessionID: "ses-1",
                directory: "/repo/project",
                callID: "call-2",
            } as never);
        ...
        it("returns the existing failure wording when the rust module rejects a drop", async () => {
            const tools = createEidnaraReduceTools({
                rustToolBackends: {
                    reduce: async () => {
            ...
            await expect(
                tools.eidnara_reduce.execute({ drop: "3-5" }, toolContext()),
            ).resolves.toBe("Error: Failed to queue eidnara_reduce operations. module unavailable");
        });
        ...
        it("reports a deleted session without claiming the module rejected the drop", async () => {
            ...
            await expect(
                ...
                "Error: Session was deleted before eidnara_reduce could run; nothing was queued.",
            );
        });
        ...
        it("returns terminal success wording when all requested tags are already queued", async () => {
            ...
            const result = await tools.eidnara_reduce.execute({ drop: "10-12" }, toolContext());
        ...
        it("acknowledges only the tags the daemon accepted when some were unknown", async () => {
            ...
            const result = await tools.eidnara_reduce.execute({ drop: "1,99" }, toolContext());
        ...
        it("names already-queued tags separately so only newly queued targets read as queued", async () => {
            ...
            const result = await tools.eidnara_reduce.execute({ drop: "1, 2, 99" }, toolContext());
        ...
        it("derives the stable command id from every supported tool-call id alias", async () => {
            ...
            for (const alias of aliases) {
                await tools.eidnara_reduce.execute({ drop: "1" }, {
                    sessionID: "ses-1",
                    directory: "/repo/project",
                    [alias]: ` call-${alias} `,
                } as never);
        ...
        it("refuses a drop when the host supplies no tool-call id instead of minting one", async () => {
            ...
            const result = await tools.eidnara_reduce.execute({ drop: "1" }, {
                sessionID: "ses-1",
                directory: "/repo/project",
            } as never);

            expect(result).toBe(
                "Error: eidnara_reduce requires a stable tool-call identity from the host; nothing was queued.",
            );
            expect(calls).toHaveLength(0);
        });

        it("returns the unavailable error when no reduce backend is registered", async () => {
            const tools = createEidnaraReduceTools({ rustToolBackends: {} });

            const result = await tools.eidnara_reduce.execute({ drop: "3" }, toolContext());

            expect(result).toBe(
                "Error: Failed to queue eidnara_reduce operations. The daemon backend is unavailable.",
            );
        });
    });
});
