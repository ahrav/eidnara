...
- **Approach:**
  ...
  4. Add a test where a spawned thread stores through `copy_in` into an `AtomicU64` array while the main thread constructs a `LeaseSpan` over the same range and calls `read_byte`, `checksum`, `copy_to`; assert no panic and that each observed byte is one of the two written values. The writer must use the same `AccessShape` as the reader; a byte-wide writer is a mixed-size race Miri reports.
