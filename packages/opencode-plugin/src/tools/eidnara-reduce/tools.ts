...
import type { EidnaraReduceArgs } from "./types";

export { EIDNARA_REDUCE_LIGHT_DESCRIPTION } from "../light-descriptions";

export interface EidnaraReduceToolDeps {
...
const eidnaraReduceArgsShape = {
    drop: tool.schema
        .string()
        .optional()
        .describe("Tag IDs to drop entirely. Ranges: '3-5', '1,2,9'"),
};
// Accepts fields outside eidnaraReduceArgsShape.
const eidnaraReduceArgsSchema = tool.schema.object(eidnaraReduceArgsShape).passthrough();

function createEidnaraReduceTool(deps: EidnaraReduceToolDeps): ToolDefinition {
    return tool({
        description: EIDNARA_REDUCE_DESCRIPTION,
        args: eidnaraReduceArgsShape,
        async execute(rawArgs: EidnaraReduceArgs, toolContext) {
            const parsedArgs = eidnaraReduceArgsSchema.safeParse(rawArgs);
            let args = (parsedArgs.success ? parsedArgs.data : rawArgs) as EidnaraReduceArgs;
            ...
            if (!callId) {
                return "Error: eidnara_reduce requires a stable tool-call identity from the host; nothing was queued.";
            }

            const rustReduce = deps.rustToolBackends.reduce;
            if (!rustReduce) {
                return "Error: Failed to queue eidnara_reduce operations. The daemon backend is unavailable.";
            }
            try {
                ...
                if (record === null || record.ok !== true) {
                    ...
                    return `Error: Failed to queue eidnara_reduce operations. ${message}`;
                }
                ...
                if (isRustToolSessionDeletedError(error)) {
                    return "Error: Session was deleted before eidnara_reduce could run; nothing was queued.";
                }
                return `Error: Failed to queue eidnara_reduce operations. ${getErrorMessage(error)}`;
            }
        },
    });
}

export function createEidnaraReduceTools(
    deps: EidnaraReduceToolDeps,
): Record<string, ToolDefinition> {
    return {
        eidnara_reduce: createEidnaraReduceTool(deps),
    };
}
