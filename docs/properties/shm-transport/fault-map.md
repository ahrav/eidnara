...
| raw-native-attach-rejects-hostile-descriptors-without-effects | A descriptor object built without the wrapper: non-object, unsafe numeric field, malformed or aliased grant text, accessor or proxy | **Yes** - four raw-descriptor suites in `tests/mechanism.ts` (`:804`, `:827`, `:849`, `:879`) with the leak counters checked around each call |
