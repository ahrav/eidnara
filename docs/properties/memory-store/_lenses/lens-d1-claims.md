...
Scope: `crates/memory-store`, `crates/context-core`, `crates/tokenizer`, plus the
out-of-repo `storage` dependency resolved by root `Cargo.toml:16` to
`../commons/crates/storage`. Citations to that crate are marked
`storage:NNN` and resolve to
`/local/home/ahrav/scratch/commons/crates/storage/src/lib.rs` (789
lines).
...
Second, the task cites `crates/memory-store/Cargo.toml:16` as the path resolution
for `storage`. That line is `storage-types = { workspace = true
}`. The member manifest declares `storage = { workspace = true }` at
`crates/memory-store/Cargo.toml:15`, and the path lives in the workspace root at
`Cargo.toml:16`. The rest of the lead is confirmed.

Production code in `crates/memory-store/src/lib.rs` ends at line 13930. Lines 13932
...
- C1: "Application connections verify: foreign keys enabled, WAL activation,
  ...
  Exact check: `reachable` on the verifier call site from `MemoryStore::open`, then
  ...
  Production call site NOT FOUND. `MemoryStore::open` (`lib.rs:4816-4905`) does not
  call it; the only callers are `crates/memory-store/tests/sqlite_runtime.rs:183,
...
- C2: "declared synchronous mode"
  ...
  `PRAGMA synchronous` on a `MemoryStore::open` connection equals a value a setter
  wrote. Implementing code: NOT FOUND. `grep -rn synchronous
  storage/src/` exits 1, no match. Nothing in `crates/memory-store/src`
...
- C4: "Durability + concurrency pragmas: WAL for concurrent readers, a busy
  ...
  [`storage:285-286`] → implied property: the comment enumerates the
  ...
  "durability". Implementing code: `storage:287` `journal_mode = WAL`,
...
- C5: "one immutable `format_marker` row"
  ...
  `format_marker` object appears in `MIGRATIONS` (`lib.rs:432-1312`) and no
...
- C6: "`PRAGMA application_id = 0x4549444E` (`EIDN`)" and "`PRAGMA user_version
  ...
  a supported store. Implementing code: NOT FOUND for `store.db`. `MemoryStore::open`
...
- C8: "The old migration version ranges remain relevant only to historical
  ...
  open. Exact check: `always` — open succeeds only when the recorded `memory`
...
- C10: "A pristine family is bootstrapped under `BEGIN IMMEDIATE`... Concurrent
  ...
  `TransactionBehavior::Immediate` (`storage:191`), but the migration
  runner uses `conn.transaction()` at `storage:366`, which is DEFERRED,
  and the version-table creation and current-version read at
  `storage:341-357` precede any transaction. Reachability:
...
- C11: "each in its own transaction together with its version record, so a
  ...
  [`storage:329-332`] → implied property: migration statements and the
  ...
  and absence implies none does. Implementing code: `storage:366`
...
- C12: "writes go through `storage`'s epoch-fenced transaction (rejects
...
- C14: "Session cleanup removes session-owned runtime state but preserves claim
  ...
  check: `always` — after `delete_session`, every `claim_intents` row in
...
- C15: "Operation receipts remain for the lifetime of the database incarnation
  ...
  `claim_mirror_receipts` (`lib.rs:1298-1309`), keyed
...
- C16: "Reset is a separate Doctor operation, never a startup branch."
  ...
  entry point from `MemoryStore::open`. Implementing code: `MemoryStore::open`
...
- C18: "DETERMINISM is the load-bearing property... The vocab is VENDORED and
  ...
  [`crates/tokenizer/src/lib.rs:13-17`] → implied property: `estimate_tokens`
  ...
  `crates/tokenizer/src/lib.rs:37`; single `OnceLock` build at `:47-67`;
  `tiktoken-rs = "=0.11.0"` exactly pinned at
  `crates/tokenizer/Cargo.toml:19`. Reachability: default-production. One
...
- C19: "Embedded at build time so there is no runtime file read or network fetch
  ...
  [`crates/tokenizer/src/lib.rs:34-36`] → implied property: tokenization
...
- C20: "Tier demotion is byte-deterministic at render time, driven by
  ...
  `crates/context-core/src/decay.rs:99-104` `should_archive`,
  `:109-123` `rendered_tier`, `:129-140` `compute_budget_pressure`; all take
  values and return values with no I/O, and the crate sets
  `#![forbid(unsafe_code)]` at `crates/context-core/src/lib.rs:9`. Reachability:
  default-production.
...
- C21: "the cache-stability invariant this layer must satisfy is INTRA-module
  ...
  invariant" [`crates/context-core/src/decay.rs:15-19`] → implied property: the
  ...
  `crates/context-core/src/decay.rs:249-253` loads `decay-golden.json` and is the
...
- C22: "the model's invariants (age/importance monotonicity, finite demotion
  ...
  [`crates/context-core/src/decay.rs:10-13`] → implied property: five named
  ...
  `crates/context-core/src/decay.rs:159` (finite demotion at importance 100),
...
- C23: "Monotonic absolute ordinal — strictly increasing across the lineage,
  ...
  [`crates/context-core/src/lib.rs:28-30`] → implied property: for any two items in
  ...
  `FlatBlock` (`crates/context-core/src/lib.rs:23-37`); `context-core` never constructs an
  item and cannot check it. Enforcement is the implementor's duty in
  `daemon`, outside this scope. Reachability: default-production obligation
...
- C24: "Synthetic items are stripped before boundary/coverage/tail computation —
  ...
  [`crates/context-core/src/lib.rs:31-34`] → implied property: no boundary,
  ...
  `false` default at `crates/context-core/src/lib.rs:35-37`. The stripping itself is
  NOT FOUND in `context-core`. A defaulted-to-`false` predicate means an implementor
...
- C25: "This is a faithful port of the council-validated model. The
  ...
  [`crates/context-core/src/decay.rs:9-11`] → implied property: eleven named
  ...
  `crates/context-core/src/decay.rs:23`, `D = 25.0` at `:25`, `G = 2.0` at `:27`,
...
- C26: "Both runtimes are proven against the golden corpus
  ...
  [`crates/context-core/src/claim_operation.rs:3-4`] → implied property: the Rust
  ...
  are at `crates/context-core/src/claim_operation.rs:6-18` (doc) with
...
- C27: "This is independent from request/result encoding versions so transport
  ...
  [`crates/context-core/src/claim_operation.rs:29-31`] → implied property: a
  ...
  `crates/context-core/src/claim_operation.rs:27`,
...
though `foreign_keys = ON` is set (`storage:291`), so the pragma has
...
- S4: `PRIMARY KEY (producer, operation_key)` on `claim_intents`
...
- S5: `CHECK ((state = 'staged' AND result_json IS NULL) OR (state <> 'staged'
  AND result_json IS NOT NULL))` on `claim_intents` [`lib.rs:1231-1234`] —
...
- S6: `request_encoding_version INTEGER NOT NULL CHECK
  (request_encoding_version = 1)` on `claim_intents` [`lib.rs:1222`] —
  ...
  `CLAIM_REQUEST_ENCODING_VERSION = 1` at
  `crates/context-core/src/claim_operation.rs:27`. Note the compatibility direction:
  ...
  `vector_version = 1` on `claim_mirror_state` (`lib.rs:1252-1253`).

- S7: `state TEXT NOT NULL CHECK (state IN ('staged', 'context-committed',
  'acknowledged', 'terminal-rejected'))` on `claim_intents`
...
- S11: `UNIQUE (database_incarnation_id, revision_locator)` on
  `claim_mirror_claims` [`lib.rs:1290`] — encodes: a revision locator
...
- S12: `FOREIGN KEY (database_incarnation_id, project_id) REFERENCES
  claim_mirror_projects(database_incarnation_id, project_id) ON DELETE
  ...
  place `foreign_keys = ON` (`storage:291`) changes behaviour, which
  makes that pragma's blast radius exactly one edge.

- S13: `id INTEGER PRIMARY KEY CHECK (id = 1)` on
  `claim_intent_controls` [`lib.rs:1241`] and `claim_mirror_state`
...
- S14: `last_effect_id INTEGER NOT NULL CHECK (last_effect_id >=
  ...
  on `claim_mirror_receipts` [`lib.rs:1303-1305`] — encodes: a receipt covers
...
- S17: `public_claim_id TEXT NOT NULL CHECK (length(public_claim_id) = 36)`
  ...
  `crates/context-core/src/claim_operation.rs:41` means the app form is a prefixed
...
- S20: `WITHOUT ROWID` on `claim_mirror_projects`, `claim_mirror_claims`,
  and `claim_mirror_receipts` [`lib.rs:1270`, `1295`, `1309`] — encodes a
...
1. `docs/migration-version-lanes.md:50` requires a "declared synchronous mode"
   and `storage:285-286` heads its pragma block "Durability +
   concurrency pragmas". No code in either crate sets `PRAGMA synchronous`
   (`grep -rn synchronous storage/src/` exits 1). The pragma that
...
2. `docs/migration-version-lanes.md:47-50` states that "Application connections
   ...
   production caller; `MemoryStore::open` (`lib.rs:4816-4905`) does not call it. No
...
3. `docs/migration-version-lanes.md:9-17` defines format identity by
   `application_id`, `user_version`, an immutable `format_marker` row, a
   ...
   `schema_version` (`storage:351-357`, read at
   `lib.rs:5348-5358`), and no `format_marker` object exists in `MIGRATIONS`
...
4. `docs/migration-version-lanes.md:82-83` says old version ranges are not
   ...
   (`storage:363-365`). An older binary therefore opens a
...
5. `docs/migration-version-lanes.md:30-34` says a pristine family is
   ...
   (`storage:366`), with the version-table creation and current-version
   read outside any transaction (`storage:341-357`). `IMMEDIATE` is used
   on the write path (`storage:191`) but not on the path the sentence
   describes.
...
7. `crates/tokenizer/src/lib.rs:15-17` claims `fancy-regex` is
...
8. `crates/context-core/src/lib.rs:31-34` says synthetic items "must never
   masquerade as the real boundary", and the `synthetic()` trait method defaults
   to `false` (`crates/context-core/src/lib.rs:35-37`). A forgotten override produces
...
9. `docs/migration-version-lanes.md:79-80` says operation receipts are removed
   only with whole-family reset. `claim_mirror_receipts`
...
- Append stability and O(H) render cost, two of the five decay invariants named
  at `crates/context-core/src/decay.rs:10-13`, have no located check while the other
...
- Six tables constrain `length(database_incarnation_id) = 32` (S8) and only four
  ...
  `claim_mirror.rs:253`). Which insert paths into `claim_mirror_claims`
  (`lib.rs:1273`) and `claim_mirror_receipts` (`lib.rs:1301`) reach SQL
...
- Does any caller reach `delete_session` (`lib.rs:5432`) with a `session_id`
  ...
  stored as a `cache_state` row keyed
...
- Is the tokenizer's cross-machine determinism claim (C18) meant to survive this
  ...
  (`crates/tokenizer/Cargo.toml:5`) and the pin is `Cargo.lock`? If yes,
