...
This directory records accepted incident identity and history. It does not
decide release weights or fix product defects.
...
1. Record source provenance in `source-inventory.json`. Account for each stable
   ...
   unsupported wording into a reproduction.
...
3. Rust is the only harness. Every executable variant declares
   `applicability.harness: "rust"` with no omitted harnesses.
...
6. Append a fingerprint-bound baseline event to `adjudications.jsonl`. Green
   wrappers enter `mode-manifest.json`.
7. Run incident unit tests, manifest validation, the report command, and every
   mutation replay bound to a changed verifier.
...
- applicability;
- green versus adjudication-only lane;
...
`tests/incident-pool-green.test.ts`. Adjudication-only variants record reviewed
provenance and never execute.
...
`expected_green`, `regression`, and `unscored` compare the reviewed baseline.
