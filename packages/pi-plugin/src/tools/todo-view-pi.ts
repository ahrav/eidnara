...
const WIDGET_KEY = "eidnara-todos";
...
export function rememberTodowriteToolCallTodos(
    ...
    while (cachedTodowriteTodosByToolCallId.size > MAX_TODOWRITE_RENDER_CACHE_ENTRIES) {
...
function getCachedTodowriteToolCallTodos(toolCallId: string | undefined): TodoItem[] | null {
...
function formatTodoLine(todo: TodoItem, theme: Theme, options: { showId?: boolean } = {}): string {
...
export function renderTodowriteCall(
    ...
    const todos =
        getCachedTodowriteToolCallTodos(context?.toolCallId) ?? parseTodos(args.todos) ?? [];
    const active = activeTitleCount(todos);
    const activeColor: Parameters<Theme["fg"]>[0] = active > 0 ? "warning" : "success";
...
export function renderTodowriteResult(
    ...
    const detailsTodos = (result.details as { todos?: unknown } | undefined)?.todos;
    ...
    return lineComponent((width) => {
        ...
        const lines = visible.map((todo) =>
            truncateToWidth(formatTodoLine(todo, theme, { showId: true }), width, "…"),
        );
        if (hiddenCount > 0) {
...
export function registerTodosCommand(pi: Pick<ExtensionAPI, "registerCommand">): void {
    pi.registerCommand(TODOS_COMMAND_NAME, {
        description: "Show the current Eidnara todo list",
...
export class TodoOverlay {
    ...
    private pruneCompletedDisplayState(todos: readonly TodoItem[]): void {
        ...
        for (const taskId of this.hiddenCompletedTaskIds) {
            if (!completedKeys.has(taskId)) this.hiddenCompletedTaskIds.delete(taskId);
        }
    }
    ...
    private renderWidget(theme: Theme, width: number): string[] {
        ...
        const headingColor: Parameters<Theme["fg"]>[0] = hasInProgress ? "accent" : "dim";
        ...
        if (hiddenCount > 0) {
            lines.push(
                truncate(`${theme.fg("dim", "└─")} ${theme.fg("dim", `+${hiddenCount} more`)}`),
            );
        }
