//! Host process lifecycle: acquires the instance, publishes the connection file, accepts
//! setup connections, and drives orderly or fatal shutdown.
...
impl<H: HostHandler> HostShared<H> {
...
fn retain_lock_until_drained<H: HostHandler>(shared: Arc<HostShared<H>>, guard: InstanceGuard) {
...
impl<H: HostHandler> PrePublicationCleanup<H> {
...
impl<H: HostHandler> Drop for PrePublicationCleanup<H> {
    fn drop(&mut self) {
...
struct AbandonGuard<H: HostHandler> {
    inner: Option<(Arc<HostShared<H>>, InstanceGuard)>,
}

impl<H: HostHandler> AbandonGuard<H> {
    fn guard_mut(&mut self) -> &mut InstanceGuard {
...
impl<H: HostHandler> Drop for AbandonGuard<H> {
    fn drop(&mut self) {
        let Some((shared, mut guard)) = self.inner.take() else {
            return;
        };
        shared.shutdown.cancel();
        for generation in shared
            .connections
            .lock()
            ...
            .values()
        {
            ...
            generation.shutdown_complete.cancel();
...
pub async fn run<H: HostHandler>(
    handler: H,
    config: HostConfig,
...
#[doc(hidden)]
pub async fn run_with_publish_hook<H: HostHandler>(
    handler: H,
...
fn spawn_activation_task<H: HostHandler>(shared: &Arc<HostShared<H>>) {
...
/// it (plan KTD10).
async fn accept_loop<H: HostHandler>(shared: &Arc<HostShared<H>>, listener: UnixListener) {
    loop {
...
async fn shutdown_sequence<H: HostHandler>(
...
async fn send_connection_goodbyes(generations: &[Arc<GenerationCore>], deadline: Instant) {
    ...
    for generation in generations {
        ...
        sends.spawn(async move { send_connection_goodbye(generation, deadline).await });
    }
...
async fn run_handler_shutdown<H: HostHandler>(shared: &Arc<HostShared<H>>) -> bool {
