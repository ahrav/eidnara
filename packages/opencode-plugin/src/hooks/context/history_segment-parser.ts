export interface ParsedHistorySegment {
    startMessage: number;
    endMessage: number;
    title: string;
    /** content stores the P1 mirror in v2 and the flat history_segment body in v1. */
    content: string;
    /** p1 through p4 store v2 paraphrase tiers; they are undefined for v1/flat history_segments, and p4 may be "" for a self-closing tier. */
    p1?: string;
    p2?: string;
    p3?: string;
    p4?: string;
    /** importance stores the v2 decay-rate signal and is undefined for v1/flat history_segments. */
    importance?: number;
    /** episodeType stores comma-separated v2 activity types and is undefined for v1/flat history_segments. */
    episodeType?: string;
}
...
export interface ParsedEvent {
    kind: string;
    /** atHistorySegment stores the 1-based history_segment index from `at_history_segment="N"` and is null when the attribute is absent or invalid. */
    atHistorySegment: number | null;
...
export interface ParsedPrimerCandidate {
    ...
    originHistorySegmentIndex?: number;
}

export interface ParsedHistorySegmentOutput {
...
const HISTORY_SEGMENT_REGEX = /<history_segment\s+([^>]*?)\s*>(.*?)<\/history_segment>/gs;
...
// A primer uses `<primer at_history_segment="N">question</primer>`, where N is the origin history_segment's start ordinal.
// Legacy primer bullets (`*`, `-`, or `1.`) fall back to the chunk span at emission.
const PRIMER_ELEMENT_REGEX = /<primer\s+at_history_segment="(\d+)"\s*>(.*?)<\/primer>/gs;
const PRIMER_ITEM_REGEX = /^\s*(?:\*|-|\d+\.)\s*(.+)$/gm;

// Only elements inside `<events>` are events.
// Children of `<events>` are parsed kind-agnostically; a child with `at_history_segment` is an event.
// Scoping to the `<events>` block prevents fact and history_segment tags from being parsed as events.
...
const EVENT_ELEMENT_REGEX = /<([a-z_]+)\s+at_history_segment="(\d+)"\s*>(.*?)<\/\1>/gs;
 ...
 * If the close tag is absent, the next opening tier tag or history_segment end bounds the body.
 *
 * Returns:
...
export function parseHistorySegmentOutput(text: string): ParsedHistorySegmentOutput {
    const history_segments: ParsedHistorySegment[] = [];
    const facts: ParsedFact[] = [];

    for (const match of text.matchAll(HISTORY_SEGMENT_REGEX)) {
        const attrs = match[1];
        const inner = match[2];
        ...
        // A v2 tiered history_segment contains at least `<p1>`.
    ...
    // The parser scopes category extraction to `<facts>` because category tags can occur in event fields and history_segment bodies.
    // Without `<facts>`, the parser removes `<events>` and `<history_segment>` blocks before extracting categories.
    ...
    // Without `<facts>`, the parser strips `<events>` and `<history_segment>` blocks before fallback extraction to exclude category-shaped tags in their prose or attributes.
    const factsScope = factsBlockMatch
        ? factsBlockMatch[1]
        : text
              .replace(ALL_EVENTS_BLOCKS_REGEX, "")
              .replace(/<history_segment\s+[^>]*?\s*>.*?<\/history_segment>/gs, "");
    ...
    if (primerMatch) {
        ...
        // Element-form candidates use `<primer at_history_segment="N">…</primer>`, where `N` is the origin ordinal.
        ...
        for (const el of block.matchAll(PRIMER_ELEMENT_REGEX)) {
            ...
            if (question) {
                primerCandidates.push({
                    question,
                    originHistorySegmentIndex: Number.parseInt(el[1], 10),
                });
            }
        }
    ...
    history_segments.sort((a, b) => a.startMessage - b.startMessage);

    return { history_segments, facts, events, unprocessedFrom, userObservations, primerCandidates };
}
...
function parseEvents(text: string): ParsedEvent[] {
    ...
    for (const elMatch of block.matchAll(EVENT_ELEMENT_REGEX)) {
        ...
        const atHistorySegment = Number.isNaN(atRaw) ? null : atRaw;
        ...
        events.push({ kind, atHistorySegment, fields });
    }
    return events;
}
