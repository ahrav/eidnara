...
retains the ten-pair AB/BA comparison against the then-current harness tip
`2a415271`, whose allocation ledgers equal the `c1dafa76` capture that the
`073f578e` baseline regeneration replaced. That paired record predates the
`073f578e` harness correction and has not been rerun on it.
