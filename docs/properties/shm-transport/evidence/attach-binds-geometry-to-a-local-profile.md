...
- `crates/shm-transport/src/backend/ring.rs:598-616` — `Ring::attach(fd:
  ...
  (`:607`).
- `crates/shm-transport/src/backend/ring.rs:544-558` — the contrast.
...
- `crates/shm-transport/src/backend/ring.rs:461-478` — `checked_layout`, the
...
- `crates/shm-transport/src/backend/ring.rs:604-605` — the mapping size comes
...
- Every `Ring::attach` call site in the tree passes `(fd, grant, scheduling)` and
  ...
  `packages/shm-native/src/lib.rs:250` (the addon's `attach_ring`, which no
  ...
  and `crates/shm-transport/tests/ring.rs:437`, `:459`, `:631`.
- `packages/shm-native/src/lib.rs:503-506` — the addon does check a profile,
  but only as a string: `profile != PROFILE` rejects. `PROFILE` is
  `"host-test-ring-v1"` (`:27`). A name match is not a geometry match.
- `crates/host-runtime/src/ring_transport.rs:822-827` —
...
- `packages/plugin/src/shared/host-client/shm-grant.ts:161-171` — the only
...
- Sources examined: `crates/shm-transport/src/backend/ring.rs:598-616` and
  ...
  `packages/shm-native/src/lib.rs:240-252` and, at `9c1eb4d1`,
  ...
  `packages/plugin/src/shared/host-client/shm-grant.ts:146-174` for the
...
- Findings: the claim holds. Six call sites, none passing a profile. The two
  ...
  (`packages/shm-native/src/lib.rs:633-650`) is the one path that does use a
