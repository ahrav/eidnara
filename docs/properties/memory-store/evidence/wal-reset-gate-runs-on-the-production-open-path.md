...
`docs/migration-version-lanes.md:41-44` (source-catalog path, not present at HEAD) makes a two-clause claim: the TypeScript
...
`MemoryStore::open` found no probe.

## Evidence trail

The claim:

- `docs/migration-version-lanes.md:41-44` (source-catalog path, not present at HEAD):
...
- `crates/memory-store/src/sqlite_runtime.rs:43-48` (source-catalog path, not present at HEAD)
...
- `crates/memory-store/src/lib.rs:4816-4905` `MemoryStore::open`, read in full. Its
...
(`storage:279-282`) and before the real file is touched at
`storage:284`. Placing it first means an unsafe engine never opens the
...
2. Call `MemoryStore::open` on a temp-dir descriptor.
...
- Conclusion: resolved on the mechanics, needs human input on the policy. If the
  ...
  `docs/migration-version-lanes.md:43` (source-catalog path, not present at HEAD) should be marked pending.
...
- Sources examined: content search for the shared vocabulary constants across
  `packages/`. `packages/cli/src/lib/database-access.ts:31` (source-catalog path, not present at HEAD) imports
  `KERNEL_APPLICATION_ID` and `:262` compares against it;
