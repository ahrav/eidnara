...
found that `NativeChannel.attach` (`packages/shm-native/index.ts:739-742`)
...
- `packages/shm-native/index.ts:739-742`:
...
- `packages/shm-native/src/lib.rs:634` is `pub fn attach(env: &Env, descriptor:
  Unknown<'_>) -> Result<u32>`. `:639-640` reject a non-object with
  `descriptor_error()`; `:643` casts to `Object` with the same error; `:646`
  returns "shared-memory profile is unavailable" when the profile is absent;
  later field reads map to `descriptor_error()` (`:691`, `:701`).
- `DESCRIPTOR_ERROR: &str = "invalid shared-memory descriptor"` at `lib.rs:31`;
  `fn descriptor_error() -> Error` at `:150`.
- `tests/mechanism.ts:790` is `expectRejectedWithoutEffects(descriptor, pattern =
...
- Suites: "rejects non-object and structurally hostile arguments" (`:836`);
  "rejects every unsafe numeric representation before narrowing" (`:859`, with
  `hostileFds = [-1, -0, 2 ** 31, 3.5, NaN, "10"]` at `:862`); "rejects
  malformed, non-ASCII, and aliased grant text" (`:881`); "accessor objects and
  proxies get one bounded redacted error" (`:911`, asserting the message equals
  `invalid shared-memory descriptor` at `:928`, with a flipping `Proxy` at
  `:935`).
...
- Sources examined: `packages/shm-native/index.ts:739-742` and a search of
...
- Sources examined: the invalidated record's Impact; `lib.rs:634-701` field
  reads.
...
- Sources examined: `tests/mechanism.ts:806`, `:829`, `:851`, `:881`, `:912`,
  `:918`, `:922`, `:928`; `packages/shm-native/src/lib.rs:646`, `:276`, `:283`.
- Findings: the four shape suites expect `invalid shared-memory descriptor`;
  ...
  (`tests/mechanism.ts:918`, emitted at `lib.rs:646`) and the unresolvable-descriptor suite expects
  `shared-memory attachment failed` (`tests/mechanism.ts:928`, emitted at `lib.rs:276` and
...
- Findings:
  ...
  - line 25, `tests/mechanism.ts:389` now `tests/mechanism.ts:790`: At HEAD the helper takes the addon as its first parameter: `expectRejectedWithoutEffects(addon, descriptor, pattern = DESCRIPTOR_ERROR)`.
