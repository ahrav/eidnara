...
`docs/shm-transport.md` presents quarantined charge accounting as a live
...
(`crates/host-runtime/src/ring_transport.rs:168-173`). The question is whether the
...
`Admission::quarantine` has no `host-runtime` caller.
...
`crates/shm-transport/tests/contract.rs:368` and `:479`. There is no
`host-runtime` caller. There is no caller in `packages/shm-native` either.
...
`packages/shm-native/src/lib.rs:259-263`). That hazard lives on the native
...
The cheap screen is a static assertion: no `host-runtime` path calls
...
2. Have the peer publish a frame, then corrupt the shared descriptor for a later
   ...
   `validate` call at `ring.rs:1095` fails. `crates/shm-transport/tests/ring.rs`
...
- Findings: the host's own mappings are dropped before the release, so from the
  ...
  lives in `packages/shm-native/src/lib.rs`. I did not read that file in this
  pass, so I cannot say whether the native side retains its own accounting.
- Missing evidence: whether `shm-native` performs an equivalent retention on
  its side, which would make the host's release correct and complete.
- Conclusion: unresolved, needs a read of `packages/shm-native/src/lib.rs`
