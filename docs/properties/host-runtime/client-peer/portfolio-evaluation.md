...
`crates/host-runtime/tests/client.rs`, 0 doctests, 24 integration binaries of which 8
...
whose three commands are `cargo nextest run -p host-runtime --test client`,
`cargo test -p host-runtime --test shm_failure_modes -- --test-threads=1` (unfiltered,
so `:218` runs), and `cargo test -p host-runtime --test shm_soak
