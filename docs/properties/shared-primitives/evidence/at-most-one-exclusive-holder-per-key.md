...
- **Primary evidence:** headline contract at `crates/lease/src/lib.rs:2-6`; `FileLeaseStore::lease_path` derives one per-key path and `FileLeaseStore::acquire` calls `File::try_lock`, mapping `TryLockError::WouldBlock` to `Held` and other lock errors to `Io`. PostgreSQL derives one advisory key in `advisory_key` and, in `open_postgres`, treats a false `pg_try_advisory_lock` result as `Held` (`commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:67-72,355-389`).
