...
- **Primary evidence:** `lease_key` includes the database file name (`crates/storage/src/lib.rs:60-72`); `open_sqlite` derives the root from the database parent (`:565-578`); `protect_file` refuses a database path that is not a regular file, which rejects file-symlink aliases (`:581-587`).
