...
1. `RING_PROFILE` is the string `"host-test-ring-v1"`
   ...
   nothing about whether the ring runs. `docs/shm-transport.md:11`
...
a named OS thread `host-shm-endpoint` (`:254-256`), and blocks on the
...
ordering matches `docs/shm-transport.md:49`, "Joined endpoint teardown
...
it to its own OS thread `host-ring-client`. Teardown is thread exit, which
...
3. **Publication gate.** `publish_one` (`:536`) calls
   ...
   `docs/host-wire-protocol.md:60`, "`not_sent`: sender proves the request
...
`crates/shm-transport/tests/ring.rs:164-187`.

O3. `admission.release()` (`:291`) is unconditional and sits after the
`catch_unwind` at `:279-290`. `Admission::quarantine`
(`crates/shm-transport/src/profile.rs:566`) has no caller in `host-runtime` at
all; its only in-tree callers are `crates/shm-transport/tests/contract.rs:368`
and `:479`.
...
accounting mutex (`profile.rs:501-505`). `docs/shm-transport.md:53-59`
...
`docs/shm-transport.md:66` phrase "completed attachment and activation
...
`ReceiveLease::Drop` (`crates/shm-transport/src/lease.rs:215-221`) calls
...
`shm_transport::backend::ring::ProducerReservation` (`:14`) instead.
...
`prepare`, so the suite exercises production code, but no `host-runtime` inline unit
test runs in CI (re-scope CI section, `ci.yml` `-p host-runtime` invocations all
carry `--test <name>`).
...
(`lib.rs:8`) currently forecloses that inside `host-runtime`.
...
(`profile.rs:566`) is never entered from any `host-runtime` call path.
...
Existing check: `crates/shm-transport/tests/ring.rs:256`
...
`packages/plugin/src/shared/host-client/types.ts:69-73` and
...
Impact: `docs/shm-transport.md:53-59` promises the operator a
five-class terminal taxonomy from `eidnara daemon doctor`. The host can
...
- Is the five-class taxonomy the *client's* contract only, with the host
  ...
  to `eidnara daemon doctor`, which is host-side.
  (needs human input)
...
`host-runtime` inline tests do not run in CI. Status unaudited.
...
`docs/shm-transport.md:19` - "A receiver validates the descriptor and
header before exposing a scoped lease." `docs/host-wire-protocol.md:294` -
...
`docs/shm-transport.md:21` - "Active and quarantined charges are
...
`Admission::quarantine` (`profile.rs:566`) has no `host-runtime` caller, so the
quarantined figure is structurally zero. See
`ring-a-host-never-quarantines-an-admission-charge`.

L3. **Doctor terminal taxonomy.**
`docs/shm-transport.md:53-59` - `eidnara daemon doctor` "reports
...
(`packages/plugin/src/shared/host-client/types.ts:69-73`). See
`ring-a-host-doctor-emits-one-of-five-declared-terminal-classes`.

L4. **Two counts that cannot diverge.**
`docs/shm-transport.md:66` - "completed attachment and activation
...
lifecycle diagram at `docs/shm-transport.md:26-36` has a distinct
...
`docs/host-wire-protocol.md:321` - the host "MUST NOT buffer the oversize
...
`docs/host-wire-protocol.md:298` - "Writers MUST verify header `len` equals
body length".
...
- Was host-side quarantine accounting deliberately dropped with
  ...
  `host-runtime` drives it. (needs human input)
...
- Is the doctor's five-class terminal taxonomy the client's contract only, with
  the host intended to expose counters? `docs/shm-transport.md:53`
  attributes it to `eidnara daemon doctor`, which is host-side.
  (needs human input)
- What does `bun run check:shm-architecture` (`ci.yml:58`) actually assert? It
  ...
  `scripts/check-shm-architecture.test.ts`; the re-scope raised the same
