/**
 *
 * Tests can assert persisted state through eidnara's `context.db`.
 *
 * Usage:
 *
 * ```ts
 * const h = await TestHarness.create({ eidnaraConfig: { execute_threshold_percentage: 40 } });
...
export class TestHarness {
    ...
    readonly hostStack: SpawnedOpencode["hostStack"];
    ...
    private constructor(
        ...
        this.client = client;
        this.hostStack = opencode.hostStack;
    }
    ...
    /**
     * `status` accepts Eidnara `TagStatus` values `"active"` and `"dropped"`.
     */
    ...
    contextDbPath(): string {
        // The shared eidnara path lets OpenCode and Pi share
