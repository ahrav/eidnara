...
Every claim-bearing check for the history_summarizer subsystem:
`crates/daemon/src/history_summarizer.rs`, `history_summarizer_producer.rs`,
`history_summarizer_chunk.rs`, `history_summarizer_validate.rs`, the `lib.rs` history_summarizer and wrapup
...
| --- | --- | --- |
| `history_summarizer.rs` | 51 | **No** |
| `history_summarizer_validate.rs` (`:1383-1848`) | 19 | **No** |
| `history_summarizer_chunk.rs` | 19 | **No** |
| `history_summarizer_producer.rs` | 18 | **No** |
| `lib.rs`, history_summarizer and wrapup | 34 | **No** |
| **Total in-crate** | **141** | **No** |
| `crates/memory-store/src/lib.rs`, publish transaction (`:16625-18336`) | 7 | **No** |
| `crates/daemon/tests/` mentioning the history_summarizer | **0** | n/a |
...
**Zero integration tests in `crates/daemon/tests/` mention the history_summarizer.** A
case-insensitive search for `history_summarizer` across all seven files there and both
...
`model_execution_roundtrip` 2, `direct_host` 6, `host_adapter` 4, `lifecycle_cli` 12,
`prepared_output` 10, `release_contract_conformance` 3), and not one names this
subsystem. Every check on the history_summarizer is an in-crate unit test compiled into
...
## The TypeScript "history_summarizer-eval" gates test a different implementation

This section is separate because these gates are the **only** history_summarizer-adjacent
...
catalogs. Reading them as coverage of the Rust history_summarizer would be the single
easiest mistake to make about this subsystem.

The gate is one CI job, `history_summarizer-eval-contracts`, at `HEAD` `ci.yml:407-432` (source-catalog line, not present at HEAD)
...
| --- | --- | --- | --- |
| HistorySummarizer eval unit contracts | `ci.yml:426` (source-catalog line, not present at HEAD) | `bun run test:history_summarizer-eval-unit` | Six unit test files under `src/history_summarizer-eval/`: `contract.test.ts`, `dev-corpus.test.ts`, `mutations.test.ts`, `payload.test.ts`, `promote.test.ts`, `scorer.test.ts`. `runner.test.ts` is excluded as harness-booting (`run-test-selection.ts:59-70`, `:84`). |
| Freeze lint over the dev corpus | `ci.yml:429` (source-catalog line, not present at HEAD) | `run-history_summarizer-eval.ts --lint` | Scenario-schema and freeze conformance of the `history_summarizer-eval/dev` corpus. Corpus hygiene, not module behaviour. |
| Invalid-state mutation battery | `ci.yml:432` (source-catalog line, not present at HEAD) | `run-history_summarizer-eval.ts --mutations` | Seven mutation classes with pinned outcomes (`mutations.ts:33-56`): `speculation-promoted` and `rejected-proposal-active` must score `FAIL:false-authoritative`; `wrong-category` and `dropped-gold-fact` must score `FAIL:recall`; `near-miss-perturbation` either; `structural-overlap` must land at `validation-rejected`; `probe-wrong-answer` must fail probe comparison. The battery fails on a **stage** mismatch, not only on a PASS (`mutations.ts:5-11`). |

**These gates call no Cargo target.** Verified by reading the scorer's imports.
`scorer.ts:20-26` imports `validateHistorySummarizerOutput`,
...
`packages/plugin/src/hooks/eidnara/history_segment-runner-validation.ts` (source-catalog path, not present at HEAD), and
...
`history_summarizer_validate.rs`, no `history_summarizer_producer.rs`.
...
only: `daemon`'s Rust history_summarizer producer does not promote claims, so these
...
`history_summarizer-eval/README.md` says the frozen corpus's crafted-wrong outputs "are
...
`history_summarizer-eval.yml` is the live lane. Its README records it as
...
(`history_summarizer_validate.rs:1384`), driven by a checked-in
...
| `lib.rs`, history_summarizer and wrapup | **34** | within the test module opening at `:15993` | `:16445` `history_summarizer_trigger_token_reuse_matches_retokenized_production_shape` / `:30037` `status_diagnostics_surface_pending_history_summarizer_side_channel_failure` |

### `history_summarizer.rs`, 51 tests, six clusters
...
### `history_summarizer_validate.rs`, 19 tests
...
`history_segment_end_must_be_anchorable`); side-channel suppression (`:1774`
...
### `history_summarizer_chunk.rs`, 19 tests
...
### `history_summarizer_producer.rs`, 18 tests
...
### `lib.rs`, 34 history_summarizer and wrapup tests
...
(`:16445`, `:16518`, `:16720`), and shutdown of spawned history_summarizer work
(`:16932`, `:17007`, `:17017`).
...
| Line | Test |
| --- | --- |
| `:16625` | `history_summarizer_publish_failure_counter_accumulates_and_success_state_resets` |
...
the history_summarizer path, so every check in this part is a hand-written fixture case or
...
- `history_summarizer.rs:1492`, `RestartAction::ReattachProducer { .. } => unreachable!()`.
  ...
  `history_summarizer.rs` (`:2903`, `:2964`, `:3045`, `:3160`, `:3213`, `:3802`, `:4552`,
...
`history_summarizer_chunk.rs`; every one sampled is a static regex compilation inside a
`OnceLock::get_or_init` (`history_summarizer_validate.rs:1159-1195`,
`history_summarizer_chunk.rs:1133-1168`). None depends on runtime data. Zero unwraps in
...
- `lib.rs:7065-7069`: the wrapup drain reached the keep watermark unless a failure
  ...
  "history_segments never shrink, so a failure-free exit has reached the keep
...
- **The seven store-side publish gates** (`memory-store:9373-9455`): row-version CAS
  ...
  (`:9427-9434`), history_segment-set generation re-read inside the transaction
  (`:9436-9455`), and the range-overlap backstop in `append_history_segments_tx`
  (`:12634-12652`).
- **The 22 rejecting validation checks** enumerated by lens B
  (`history_summarizer_validate.rs:267-1081`), plus one reject that fires before the gate
  is entered: `history_summarizer.rs:1666-1671` refuses an output the producer marked
  `length_capped`.
- **The nine silent parse-time drops** (`history_summarizer_validate.rs:289-296`,
  `:297-303`, `:309`/`:333-347`, `:364-366`, `:372`, `:391`/`:410`/`:425`,
  `:828-830`, `:837-839`/`:844`), which are gate holes with a `continue` in them
  rather than checks.
- **The fingerprint re-check at the commit point** (`history_summarizer.rs:448-460`).
- **The four differentiated abandon arms and their cooldown policy**
  (`history_summarizer.rs:533-593`).
- **The closed `run.status` wire vocabulary** and **the send-replay fence**
  (`history_summarizer_producer.rs:929`, `:949`, fenced at `:999-1000`).
- **The cancellation-proof predicate** on the output branch
  (`history_summarizer.rs:1401`), which has no counterpart on the start-failure branch
  (`:1290-1329`).
- **The substance floor and its two bypasses**, and the chunk budget and
  truncation bounds (`history_summarizer_chunk.rs`).
- **The wrapup per-round progress check** (`lib.rs:6977-6989`) and the deadline
  re-check before each round (`:6831-6834`).
- **Restart-load phase mapping** (`history_summarizer.rs:648-653`) and **the side-channel
  anchor filter** (`history_summarizer_validate.rs:1086-1098`).

## Test support and fault-injection seams

**Producer doubles: available and heavily used.** The producer is a trait, and
`history_summarizer_producer.rs`'s 18 tests plus `history_summarizer.rs`'s wired-path tests are
...
land a fault between the history_segment append (`memory-store:9457-9471`) and the
...
**Clock and deadline seams for the history_summarizer: two exist, and both are already
...
- **The failure backoff is expirable, and the gate reads an injected `now`.** The
  ...
  (`lib.rs:5042-5047`), and `now` arrives through `HistorySummarizerPrepareContext` into
  `prepare_history_summarizer_fire` (`:4808-4821`) rather than being read inside the gate.
  The test helper `expire_history_summarizer_backoff` (`:29784-29791`) commits
...
1. **The quietest thing in this scope is the whole subsystem.** 141 in-crate
   ...
   and no integration test in `crates/daemon/tests/` mentions the history_summarizer.
...
3. **Ten of the validation gate's 22 rejecting checks have no test at any level,
   ...
   untested checks are: nested `<output>` tags (check 2); history_segment endpoint
...
4. **The publish transaction's atomicity is untested, and half of it is
   untestable.** Seven tests reach `publish_history_summarizer_chunk`, none asserts that all
...
6. **The user-facing failure signal has no producer and its counter has no
   ...
   it forward unchanged (`history_summarizer.rs:358`) and the only increments are in
   ...
   `buildHistorySummarizerFailureNotice` (`history_segment-runner-validation.ts:210`) are in
   `history_segment-runner-incremental.ts`, the TypeScript runner. The only tests near
...
7. **The start-failure fallback branch has no test and no cancellation proof.**
   All three cancellation-proof tests (`history_summarizer.rs:3389`, `:3444`, `:3498`)
   ...
   no test, while `HistorySummarizerSendOutcome::OutcomeUnknown`
   (`history_summarizer_producer.rs:78-82`) exists precisely to describe a start that may
...
8. **The three consumer deadline budgets are cross-language constants with one
   ...
   (`history_summarizer.rs:962`, `Duration::from_secs(3_800)`) is duplicated by hand as
   `MAX_WRAPUP_REQUEST_BUDGET_MS = 3_800_000` at `module-transport.ts:72-73`.
   `MAX_EMERGENCY_REQUEST_BUDGET` (`history_summarizer.rs:983`, 1500 s) appears to have no
...
9. **Two documented configuration keys have no implementation and no test that
   would notice.** `history_summarizer.two_pass` (`CONFIGURATION.md:454` (source-catalog path, not present at HEAD)) and
   `history_summarizer_timeout_ms` (`CONFIGURATION.md:170` (source-catalog path, not present at HEAD)) have no identifier anywhere in
...
10. **`history_summarizer_chunk.rs`'s two golden fixtures are the only guard on chunk
...
11. **The subsystem's only two panic sites have no named test.**
    `history_summarizer.rs:1492` and `history_summarizer_validate.rs:929` are each reachable only
    through a state the author believed impossible, which is exactly the class a
    fresh test should attack.

12. **`docs/AUDIT-KNOWN-ISSUES.md` (source-catalog path, not present at HEAD) contains no history_summarizer publish, validation, or
    producer entry.** The file mentions the history_summarizer only in passing. None of the
...
- **Two claims in the register are unverified rather than confirmed.** The
  `run.status` production mapping (`history_summarizer_producer.rs`) has a test asserting
  ...
  `collect_user_memory_candidates` (`history_summarizer.rs:421`) was traced into the
