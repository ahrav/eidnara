...
| `validation_refuses_a_kernel_of_another_incarnation` | `crates/daemon/tests/claim_sources.rs` | a batch classified against one kernel is refused `IncarnationMismatch` by a kernel of another incarnation before any verdict, so a restore between classification and handoff cannot judge displaced candidates | unaudited |
| `exhausted_validation_stops_before_candidate_preparation` | `crates/retrieval/src/claims.rs` | a cancelled or expired `EvalBudget` refuses `Eligibility(Deadline)` before any candidate is prepared, including one with an invalid object id | unaudited |
| `validation_bounds_only_current_submissions_and_preserves_denied_duplicates` | `crates/retrieval/src/claims.rs` | exactly `MAX_ELIGIBILITY_CANDIDATES` Current candidates are accepted and one more is `InvalidInput`; an empty or over-long object id and a malformed digest are `InvalidInput`; non-Current duplicates past the bound are all denied by state, positionally, with the object in the rejected and Unknown sets | unaudited |
| `cancellation_after_judgment_refuses_empty_and_non_current_results` | `crates/retrieval/src/claims.rs` | a budget cancelled after the kernel read refuses `Eligibility(Deadline)` for an empty and a non-Current slice instead of returning the judged result | unaudited |
