...
- [`truncate_history_summarizer_input_if_needed`][trunc] returns the input when
  ...
  [prompt assembly][trunc-call] to build the history_segment prompt.
- The [differential header][diff-header] says token counts are not monotonic
  ...
  `0..320`).
- [`history_summarizer_chunk_golden_fixture_matches_builder`][t-golden] pins
  every truncation case from `testdata/history_summarizer-chunk-golden.json` and asserts
...
at a probe, picks a different `best` and the history_summarizer receives different
prompt bytes.

## Timing windows and dependencies

The fingerprint window is a restart between `HistorySummarizerDurableState` being
...
- Findings: The tests pin byte identity with the frozen reference, and the
  ...
  history_summarizer prompt bytes for the same input and would fail every one of
...
- All-feature focused runs pass: 140 history_summarizer tests (one manual benchmark
...
- `differential_goldens.rs` and `history_summarizer-chunk-golden.json` compare byte
...
- Findings: `normalize_synthetic_todo_ingress` recognizes reserved task-list
  ...
  `prepare_history_summarizer_fire`, which uses those distinct observers for boundary
...
```sh
cargo test -p daemon --locked --lib unflagged_synthetic_delta_prepares_history_summarizer_and_native_output -- --nocapture
```
...
```sh
cargo test -p daemon --locked --lib history_summarizer_boundary_construction_matches_owned_reference -- --nocapture && cargo test -p daemon --locked --lib chunk_fingerprint_uses_id_kind_and_byte_length
```
...
```sh
cargo test -p daemon --locked --lib history_summarizer_boundary_construction_matches_owned_reference && cargo test -p daemon --locked --lib chunk_fingerprint_uses_id_kind_and_byte_length && cargo test -p daemon --locked --lib unflagged_synthetic_delta_prepares_history_summarizer_and_native_output
```
...
- Eight selected unit tests pass with `--all-features`: the construction
  ...
  drop target, and production-shape trigger reuse. All three history_summarizer
...
- `cargo clippy --workspace --all-targets --all-features --locked -- -D warnings`
  ...
  passes for `crates/daemon/src/lib.rs` and `crates/daemon/src/history_summarizer.rs`.
...
[owned-item]: ../../../../../crates/daemon/src/history_summarizer_chunk.rs#L30-L36
[as-item]: ../../../../../crates/daemon/src/history_summarizer_chunk.rs#L40-L46
[tok-import]: ../../../../../crates/daemon/src/history_summarizer_chunk.rs#L15
...
[fp-call]: ../../../../../crates/daemon/src/history_summarizer_chunk.rs#L697-L702
[trunc]: ../../../../../crates/daemon/src/history_summarizer_chunk.rs#L744-L777
[t-golden]: ../../../../../crates/daemon/src/history_summarizer_chunk.rs#L1773-L1859
[t-marker]: ../../../../../crates/daemon/src/history_summarizer_chunk.rs#L1744-L1757
...
[diff-prod]: ../../../../../crates/daemon/tests/history_summarizer_truncate_differential.rs#L100-L113
[diff-exact]: ../../../../../crates/daemon/tests/history_summarizer_truncate_differential.rs#L115-L129
[diff-small]: ../../../../../crates/daemon/tests/history_summarizer_truncate_differential.rs#L131-L140
