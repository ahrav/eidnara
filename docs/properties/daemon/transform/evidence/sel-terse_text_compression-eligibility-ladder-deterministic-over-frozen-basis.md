...
TerseTextCompression is the one sub-pass whose eligibility depends on data the same pass can
...
(`:4435-4438`). So terse_text_compression runs only on a pass that is already rewriting the
...
both tool drops and terse_text_compression compression together").
...
```
if tag_number > protected_cutoff || row.source_bytes.len() < req.terse_text_compression_min_chars {
    return None;
}
```

So the newest `protected_tags` tag positions are excluded, and blocks shorter than
`terse_text_compression_min_chars` bytes are excluded. Defaults: `protected_tags` 20
(`:893-895`), `terse_text_compression_min_chars` 500 (`:120`, factory at `:889-891`).
...
`new_terse_text_compression_units` with `age_basis_tag` 1; `:25752-25760` covers the protected
...
- Sources examined: `:6355-6358`, `:6400-6435` (`surviving_terse_text_compression_units`),
  `:6385-6398` (`prune_covered_terse_text_compression_units`).
- Findings: `surviving_terse_text_compression_units` keeps any existing `cav:*` unit whose target
  ...
  `prune_covered_terse_text_compression_units` removes units only when the target left the tail
  (`:6390-6397`).
...
- Conclusion: resolved with answer, by construction: a deeper unit is retained.
  ...
  `CONFIGURATION.md:742` (source-catalog path, not present at HEAD) ("tier assignments are persisted in `tags.terse_text_compression_depth`
