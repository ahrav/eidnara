...
import {
    ...
    compileConditionalNoteCheck,
    isValidConditionalNoteCron,
...
async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
    const dir = await mkdtemp(path.join(tmpdir(), "eidnara-note-condition-compiler-"));
    try {
        return await fn(dir);
    } finally {
...
function compileArgs(
    ...
    overrides: Partial<Parameters<typeof compileConditionalNoteCheck>[0]> = {},
) {
    return {
        client,
...
describe("compileConditionalNoteCheck", () => {
    test("returns before creating a session when the deadline has already passed", async () => {
        ...
        const result = await compileConditionalNoteCheck(
        ...
        expect(result).toEqual({
            ok: false,
            cancelled: false,
            error: "conditional-note compile deadline expired",
        });
    ...
    test("reports a dry-run failure when no fallback model is configured", async () => {
        ...
        const result = await compileConditionalNoteCheck(compileArgs(client));
    ...
    test("accepts a check whose declared host is unreachable, with the dry run pending", async () => {
        const httpGet = mock(async () => {
            throw new ConditionalNoteNetworkError("CONDITIONAL_NOTE_NETWORK: connect ECONNREFUSED");
        });
        const client = createCompilerClient([
            compilerOutput(
    ...
    test("does not accept a guest-thrown error that merely carries the network marker", async () => {
        const client = createCompilerClient([
            compilerOutput(
                `function check(cap) { throw new Error("CONDITIONAL_NOTE_NETWORK: ConditionalNoteNetworkError pretend"); }`,
            ),
        ]);
    ...
    test("waives the dry run when a later declared URL's failure escapes after an earlier one was caught", async () => {
        const httpGet = mock(async (url: string) => {
            throw new ConditionalNoteNetworkError(
                `CONDITIONAL_NOTE_NETWORK: connect ECONNREFUSED ${url}`,
            );
        });
        const client = createCompilerClient([
            compilerOutput(
    ...
    test("returns cancelled without creating a session when the signal is already aborted", async () => {
        ...
        expect(result).toEqual({
            ok: false,
            cancelled: true,
            error: "conditional-note compile cancelled",
        });
        expect(client.session.create).not.toHaveBeenCalled();
    });
    ...
    test("passes the compile signal to session creation and transcript fetches", async () => {
        ...
        await compileConditionalNoteCheck(compileArgs(client));
    ...
    test("deletes the compiler child session unless keep_subagents is set", async () => {
        ...
        await compileConditionalNoteCheck(compileArgs(deleted));
        ...
        await compileConditionalNoteCheck(compileArgs(kept));
...
describe("bindDeclaredRequests", () => {
    ...
    test("stores a fetch failure and rethrows it from the guest call", async () => {
        const unreachable = new ConditionalNoteNetworkError(
            "CONDITIONAL_NOTE_NETWORK: connect ECONNREFUSED",
        );
...
describe("conditional-note compiler runtime boundary", () => {
    test("blocks .envrc reads at runtime", async () => {
        await withTempDir(async (dir) => {
            ...
            const result = await runCompiledConditionalNoteCheck({
                ...
                capabilities: createConditionalNoteCapabilities({
                    projectRoot: dir,
                    signal: new AbortController().signal,
                }),
            });
...
describe("conditional-note compiler output bounds", () => {
    ...
    test("accepts the daemon's five-field numeric cron grammar and rejects the rest", () => {
        for (const cron of [
            ...
            expect(isValidConditionalNoteCron(cron)).toBe(true);
        ...
        for (const cron of [
            ...
            expect(isValidConditionalNoteCron(cron)).toBe(false);
