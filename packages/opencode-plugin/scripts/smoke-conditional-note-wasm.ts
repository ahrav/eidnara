// This smoke test verifies that the bundled conditional-note sandbox loads QuickJS.
//
...
const entry = join(here, "../src/features/context/conditional-notes/sandbox-runner.ts");
const outDir = mkdtempSync(join(tmpdir(), "eidnara-conditional-note-wasm-smoke-"));
...
try {
    ...
    check(
        "runCompiledConditionalNoteCheck is exported from bundle",
        typeof mod.runCompiledConditionalNoteCheck === "function",
    );
    ...
    const res = await mod.runCompiledConditionalNoteCheck({
    ...
    failures++;
    console.log(
        `FAIL  bundle-path smoke threw — ${error instanceof Error ? error.message : String(error)}`,
    );
} finally {
...
console.log("\nAll conditional-note wasm bundle-path smoke checks passed.");
