...
use daemon::memory_reviewer::steps::Step;
use kernel::source_identity::OccurrenceClass;
use kernel::{
    ArtifactDeletionIdentity, ArtifactDeletionKind, ArtifactDeletionRequest, ArtifactDestination,
    CommitIntent, Dimension, DomainSpec, EvidenceReference, ExtractedFact, KernelStore,
    ManifestReference, MemoryReviewerHoldBinding, MemoryReviewerHoldError, MemoryReviewerHoldKind,
    MemoryReviewerHoldRefusal, PolicyDependencies, ProposalAction, ProposalTarget, ProviderEgress,
    REVIEW_EXPIRY_MAX_MS, ReviewBinding, ReviewOwner, ReviewPayload, ReviewProposal,
    ReviewQuestionTemplate, ReviewReadRefusal, ReviewStagedReference, ReviewStagingSpec,
    ReviewSubject, ScopeSpec, ScopeTermSpec, Sensitivity, SourceDependency, SourceDescriptorDetail,
    SourceSpan, StagingTerminalState, Uncertainty, provisional_result_identity,
};
...
fn intent(key: &str) -> CommitIntent {
    CommitIntent {
        producer: "memory_reviewer-settlement-test".to_string(),
...
impl Fixture {
    fn open() -> Self {
        ...
        let ledger = MemoryStore::open(&MemoryStore::test_descriptor(
            ledger_dir.path(),
            "eidnara-memory_reviewer-settlement-test",
        ))
        .unwrap();
        let registration = {
        ...
        let MemoryReviewerBeginOutcome::Begun(_) = fixture
            .ledger
    ...
    fn memstore_incarnation(&self) -> String {
        self.ledger.memory_reviewer_store_incarnation().unwrap()
    }
    ...
    fn hold_binding(&self, generation: u64) -> MemoryReviewerHoldBinding {
...
fn the_staged_dependencies_are_the_brokers_union_including_uncited_inputs_and_ancestry() {
    ...
    for _ in 0..2 {
        broker.ledger.record_disclosure(
            &alias,
            context_core::memory_reviewer_policy_union::PolicyUnionMember {
...
fn unknown_partial_and_declined_runs_complete_without_content() {
    ...
    assert_eq!(
        page.outcomes[0].terminal,
        MemoryReviewerReceiptTerminal::Unknown
    );
    assert!(!page.outcomes[0].selected);
...
fn a_fenced_content_free_completion_releases_the_recovered_review_hold() {
    ...
    let binding = fixture.review_binding();
    let clock = move || later - MEMORY_REVIEWER_TASK_LEASE_MS + 5;
    let mut settlement = fixture.settlement(&binding, &fixture.claim, &clock);
...
fn a_cancelled_or_expired_receipt_records_that_whatever_the_run_produced() {
    ...
    fixture
        .ledger
        .cancel_memory_reviewer_receipt(PROJECT, &fixture.identity, fixture.now + 4)
        .unwrap();
    assert_eq!(
        fixture
            .settle(
    ...
    let broker = fixture.broker(1);
    let binding = fixture.review_binding();
    let late = fixture.now + MEMORY_REVIEWER_RUN_DEADLINE_MS;
    let mut renewed_at = fixture.now;
    while renewed_at + MEMORY_REVIEWER_TASK_LEASE_MS <= late {
        renewed_at += MEMORY_REVIEWER_TASK_LEASE_MS - 5;
        fixture
            .ledger
...
fn a_resumed_claim_adopts_the_durable_result_without_the_broker_or_a_new_request() {
    ...
    let settled = fixture
        .settlement(&fixture.review_binding(), &successor, &clock)
        .adopt(&fixture.resumed_broker(2))
        .unwrap();
...
fn a_proposal_without_a_completed_marker_at_its_generation_is_not_staged() {
    let fixture = Fixture::open();
    let broker = fixture.broker(1);
    fixture.attempt(1, Some(MemoryReviewerAttemptTerminal::Failed));
    let evidence = fixture.evidence_id();
    assert_eq!(
        fixture
            .settle(
