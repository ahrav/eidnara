...
//! `EIDNARA_BROCA_FIXTURE_MODE` routes `main` to fixture behavior.
...
use host_runtime::broca::opencode::{
    EIDNARA_BROCA_CHILD_ENV, OPENCODE_BROCA_AGENT, OpenCodeBackend, OpenCodeRuntime,
};
use host_runtime::broca::pi::{
    BROCA_MAX_OUTPUT_TOKENS_ENV, BROCA_TEMPERATURE_ENV, EIDNARA_PI_SUBAGENT_ENV,
    PI_BROCA_EXTENSION_BYTES, PI_BROCA_EXTENSION_FILE, PiBackend, PiRuntimeDescriptor,
    pi_model_ref,
};
...
use host_runtime::broca::supervisor::{SessionKey, Supervisor};
...
const FIXTURE_MODE_ENV: &str = "EIDNARA_BROCA_FIXTURE_MODE";
...
fn main() {
    ...
    if std::env::var_os(EIDNARA_BROCA_CHILD_ENV).is_some()
...
fn failed(terminal: &BackendTerminal) -> &host_runtime::broca::backend::BackendError {
    match terminal {
...
fn opencode_argv_env_stdin_contract() {
    ...
    assert_eq!(
        env.get(EIDNARA_BROCA_CHILD_ENV).map(String::as_str),
        Some("1")
    );
    assert_eq!(
...
fn opencode_hostile_project_untouched() {
    ...
    let config: serde_json::Value =
        serde_json::from_str(env.get("OPENCODE_CONFIG_CONTENT").expect("config"))
            .expect("config parses");
    assert!(
        config["agent"][OPENCODE_BROCA_AGENT]
            .get("prompt")
            .is_none()
    );
}
...
fn opencode_oversized_inline_config_rejected_before_spawn() {
    ...
    let oversized_system = "s".repeat(host_runtime::broca::config::MAX_OPENCODE_CONFIG_BYTES + 1);
    let request = request(
        setup.project.path(),
        Harness::OpenCode,
        "anthropic/claude-test",
...
fn env_snapshot_admission_charges_per_entry_overhead() {
    ...
    let string_bytes: usize = flood
        ...
        .sum();
    assert!(
        string_bytes < host_runtime::broca::config::MAX_ENV_SNAPSHOT_BYTES,
