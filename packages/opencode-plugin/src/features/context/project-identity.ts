...
/**
 * `resolveProjectIdentityStrict()` returns `git:<root-commit-sha>` or throws `ProjectIdentityError`; it never falls back to a directory hash.
...
function reuseLastKnownGitIdentity(canonical: string): string | undefined {
    ...
    if (!transientGitIdentityReuseLoggedDirectories.has(canonical)) {
        ...
        log(
            `[eidnara] git identity resolution is temporarily unavailable for ${canonical}; reusing the last successful project identity${sourceNote} to avoid splitting project-scoped memory`,
        );
    }
...
function recordDubiousOwnershipFallback(canonical: string): void {
    ...
    log(`[eidnara] ${formatDubiousOwnershipWarning(canonical)}`);
}
