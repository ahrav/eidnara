...
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:1444-1453`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:1455-1466`) proves sibling files in one directory do not contend; `symlinked_database_paths_contend_or_are_refused_never_aliased` (`:958-1010`) proves a directory-symlink alias contends and a file-symlink alias is refused. No test covers descriptor disagreement or hardlink aliases; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced DDL, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1650-1670`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:1672-1738`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:1740-1788`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:1790-1835`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`primitives@89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`primitives@89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`primitives@89abb40`) and `a_regressed_positive_epoch_fails_closed` (`primitives@89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`primitives@89abb40`), `a_suppressed_epoch_increment_is_rejected` (`primitives@89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`primitives@89abb40`) protect the lease row and its issuance; `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:1837-1929`) refuses DML, triggers, indexes, views, and renames onto the `fence` and `format_marker` tables; and `a_fenced_callback_cannot_rewrite_the_format_marker` (`:1931-1987`) refuses an update of, or a rename onto, the marker row. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/store-schema-identity-matches-the-baseline.md). `ExpectedIdentity::for_baseline` (`crates/storage/src/lib.rs:761-778`), `classify` (`:780-830`), and `apply` (`:832-846`) implement the contract inside `open_sqlite` (`:565-661`); `fresh_file_matches_the_baseline_inventory` (`:1254-1318`) pins the fixture, and `a_file_with_foreign_objects_is_refused_without_mutation` (`:1379-1405`), `a_consumer_baseline_is_applied_once_and_verified_on_reopen` (`:1320-1360`), `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched` (`:1362-1377`), and `negative_database_fence_fails_closed` (`:2033-2061`) cover the refusals.
Existing check: `fresh_file_matches_the_baseline_inventory`, `a_file_with_foreign_objects_is_refused_without_mutation`, `a_consumer_baseline_is_applied_once_and_verified_on_reopen`, `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched`, `negative_database_fence_fails_closed`; audited at U2.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:378-383`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:456-484`); `GuardedConn` (`crates/storage/src/lib.rs:219-331`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:1614-1637`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:1740-1788`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:1989-2002`) pin the three denials.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:448-453`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1650-1670`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:548-563`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:1639-1648`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:1672-1738`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:565-661`) creates the database owner-only and calls `protect_file` on all three paths before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:1070-1120`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:1522-1550`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:170-216`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:2004-2031`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:1597-1612`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:936-939`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:897-904`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:2033-2061`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:2141-2157`) cover both directions.
