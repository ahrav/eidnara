...
Both run at `crates/daemon/src/history_summarizer_validate.rs:493-504`, between parse
(`:486`) and mapping (`:506`), and before `validate_parsed_history_segments` (`:514`).

`heal_history_segment_gaps` (`:899-932`) closes an interior gap:
...
for i in 1..history_segments.len() {
    ...
    if fully_inside_tool_only {
        history_segments[i - 1].end_message = *omitted_present.last().expect("...");
    }
}
```
...
`heal_terminal_completed_tool_arc` (`:864-897`) extends the last history_segment
...
`map_parsed_history_segments_to_chunk` (`:934-981`) runs AFTER healing and resolves the
end ordinal to a block id:

```rust
let end_line = chunk.lines.iter().find(|line| line.ordinal == history_segment.end_message);
...
...
and `:36-37` explains why it matters: "A history_segment must end on an anchorable block
...
- The healed end must be anchorable (`:958-963`), covered by
  `history_segment_end_must_be_anchorable` (`:1665`).
...
2. That absorbing such a gap into the PRECEDING history_segment is semantically right
   rather than absorbing it into the following one, or leaving it uncovered.
   `:927` chose the preceding history_segment; no comment explains the choice.

## Failure scenario

Chunk 300..=340. `history_summarizer_chunk` marks 311..=318 as tool-only (a long file-read
result plus its invocation). The model emits:

```
<history_segment start="300" end="310" title="Reproducing the bug" ...><p1>...</p1></history_segment>
<history_segment start="319" end="340" title="Fixing the parser" ...><p1>...</p1></history_segment>
```

The model deliberately skipped 311..=318 as noise, which is reasonable behaviour.

`heal_history_segment_gaps` computes `gap_start = 311`, `gap_end = 318`, finds all
eight omitted ordinals inside a tool-only range, and sets the FIRST history_segment's
`end_message = 318` (`:927`).

Mapping then stamps `end_message_id` from chunk line 318, which is a block inside
the tool result.

Published state: a history_segment titled "Reproducing the bug", whose body describes
...
- The boundary identity is the fragile part. Downstream code keys on
  `end_message_id`: `to_stored_history_segment` looks up
  `boundary_dates.get(&c.end_message_id)` (`history_summarizer.rs:49-50`), so a healed
  boundary can silently get no date if the tool-result block has no date entry.
  `history_summarizer_status_summary` and coverage comparisons elsewhere key on the same id.
...
`validate_history_summarizer_output` from `HistorySummarizerChunk` (`:58-64`), built by
`history_summarizer_chunk.rs`. If they are derived purely from module-side classification
...
2. The two-history_segment document above.
3. Assert healing happened: `validated.history_segments[0].end_message == 318`. That
   part passes today.
...
5. Assert the boundary identity moved:
   `validated.history_segments[0].end_message_id == "msg-318"`. This makes the identity
...
- Sources examined: `history_summarizer_validate.rs:58-64` (the two fields and their doc
  comments), `history_summarizer_chunk.rs:23-25` (the imports, which include `MessageRange`),
  `history_summarizer_chunk.rs:1329` and `:1419-1428` (test call sites that build a chunk and
...
- Findings: `build_tool_arcs` derives arcs from decoded message block structure
  ...
  conversation data rather than history_summarizer-model output. So the history_summarizer producer
  ...
  itself lives in `history_summarizer_chunk.rs` and was not located precisely in this pass.
- Missing evidence: the exact construction site of `tool_only_ranges` inside
  `history_summarizer_chunk.rs`, and whether it depends on anything a model produced in an
  earlier run (a prior history_segment, for instance).
...
### Q: Why does gap healing extend the PRECEDING history_segment rather than the following one?

- Sources examined: `history_summarizer_validate.rs:904-931` (the loop and its indexing),
  `:923-925` (the only comment), `:1039-1051` (the contiguity check that consumes
  the result), `:1051` (`expected_start = next_present_after(&chunk_ordinals, history_segment.end_message)`).
- Findings: The choice is mechanically necessary given how contiguity is checked.
  `expected_start` advances from the PREVIOUS history_segment's `end_message`, so
  extending the preceding history_segment is what makes the following history_segment's
  start match `expected`. Extending the following history_segment's start backward would
