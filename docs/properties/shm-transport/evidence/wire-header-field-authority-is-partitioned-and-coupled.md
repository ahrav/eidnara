...
`crates/host-runtime/src/wire.rs:319-357`.
...
(`crates/host-runtime/src/ring_transport.rs:474-485`). The TCP path that duplicated
...
per-transport copy no longer exists in `crates/host-runtime/src`.
...
2. **Two equal constants in two crates.** `MAX_FRAME_BYTES` is 64 MiB
   (`crates/shm-transport/src/arena.rs:4`) and bounds `body_len`
...
(`crates/shm-transport/src/harness.rs:29-100`) already feeds 21 arbitrary
...
`host-runtime` depends on `shm-transport` (`crates/host-runtime/Cargo.toml:25`) and not
the reverse, and only `shm-transport` has a `fuzz` directory. The check
...
of the two 64 MiB constants is a one-line static assertion in `host-runtime`.
