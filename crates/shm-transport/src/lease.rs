...
impl<'lease> LeaseSpan<'lease> {
    ...
    /// Copies the whole span into `destination`, which must have exactly the span's length.
    ...
    /// Wrapping byte sum over the span, used by tests to detect payload mutation.
...
impl<'lease> ReceiveLease<'lease> {
    ...
    /// Declared body length across all segments.
    pub const fn len(&self) -> usize {
        self.body_len
    }
    ...
    /// Identity the release callback receives.
    pub const fn identity(&self) -> ReleaseIdentity {
        self.identity
    }

    /// Returns the frame to the producer. Dropping the lease releases it too; calling
    /// `release` surfaces the callback's error instead of discarding it.
    pub fn release(mut self) -> Result<(), LeaseError> {
        self.release_once()
    }

    /// Copies every segment, in order, into one owned buffer of `len` bytes.
