...
The socket path is fully predictable, `${dataDir}/eidnara/run/setup.sock`, and
...
- Managed Rust: `crates/host-runtime/src/client.rs:346-350`,
...
- Native: `packages/shm-native/src/setup.rs:106`,
  `UnixStream::connect(path)?`, likewise unchecked.
- Test support does the same: `crates/host-runtime/tests/support/raw_client.rs:879`.

So impersonation is defeated by mutual authentication alone. The peer's three
checks, all before it emits anything secret:

`crates/host-runtime/src/auth.rs:303-367`:
...
`packages/shm-native/src/setup.rs:174-220` is the same sequence with the three
...
(`packages/shm-native/src/setup.rs:200-205`) and nothing keeps the two lists in
step.
...
- Findings: the shared helper is driven by two callers, covering the proof
  ...
  that test was not located in `crates/host-runtime/src/auth.rs`, so it lives
  elsewhere.
...
- Sources examined: `auth.rs:26-29`, `:314-323`,
  `packages/shm-native/src/setup.rs:183-191`.
