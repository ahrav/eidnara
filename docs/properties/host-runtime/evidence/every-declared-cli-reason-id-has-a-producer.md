...
`remediation_for` (`crates/daemon/src/bin/eidnara-host.rs:86-118`) is pinned to
...
`release/host-release.json:71`, 31 entries. Surveying `crates/` for a Rust site
...
come from `instance_failure` (`eidnara-host.rs:330-342`), with
...
(`packages/plugin/src/shared/host-lifecycle/paths.ts:382-438`) returns
...
`exchange_dirs` (`crates/host-runtime/src/generation.rs:1195-1198`) maps every
...
`generation_failure` maps at `eidnara-host.rs:347` to
`("stopped", "native_payload_invalid")`. Its remediation, per both the contract
and `remediation_for` at `:93`, is **`reinstall_eidnara`**. A user whose
...
rename, is told to reinstall Eidnara. The precedence table makes the
...
- Sources examined: `release/host-release.json:71` onward, all 31 entries;
  a per-id grep of `crates/` for each; `eidnara-host.rs:86-118`, `:330-350`,
  `:1753-1785`; `generation.rs:1195-1205`, `:1216-1242`, `:876-912`;
  `packages/plugin/src/shared/host-lifecycle/paths.ts:178-196`, `:355-438`;
...
- Findings: 17 ids have Rust producers, 14 do not. `unsupported_filesystem` has a
  ...
  and therefore on `reinstall_eidnara`.
- Missing evidence: the other 13 ids with no Rust producer were surveyed only
  ...
  `scripts/generate-host-release-manifest.ts:325-329`, and the generated
  `release/generated/host-release-contract.rs`. I did not complete that sweep,
