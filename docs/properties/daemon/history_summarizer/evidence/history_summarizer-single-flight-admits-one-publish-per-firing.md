...
- `:4556-4581` `try_claim_live_history_summarizer_session` takes the
  `live_history_summarizer_sessions` mutex, returns `Busy` with a completion `Notify` if an
...
- `:3106` `LiveHistorySummarizerCompletionWait`, and `:4543-4554`
  `live_history_summarizer_completion_wait`, are how a waiter joins rather than racing.
...
- `:255-275` the fired state, with `firing_seq: current.firing_seq.saturating_add(1)`
  at `:257`.
- `:1013-1035` `history_summarizer_producer_session_id` folds `firing_seq` into the
...
- `:9398-9407` the predicate: `firing_seq`, `producer_run_id`,
  ...
  `history_segment_set_generation` must all match the durable row.
- `:9436-9455` re-reads the history_segment-set generation inside the transaction and
  rejects a mismatch, so a publish that landed between this run's assembly and
  this run's commit retires this run.
- `:9489` resets `meta.history_summarizer` to idle at the same `firing_seq`, and `:9491`
...
fresh read. `history_summarizer.rs:1706-1707` captures it, `:1719` passes it, and the
...
`crates/daemon/src/history_summarizer.rs:531-594` differentiates four outcomes:
...
- `HistorySegmentOverlap` does the same (`:548-559`), described as "the storage
...
or overlapping range and returns `HistorySegmentOverlap` rather than writing, so the
...
to carry different validated history_segment sets, which requires two different
...
holds a stale view. That is what the history_segment-set generation re-read at
...
The window is between the `Publishing` persist (`history_summarizer.rs:1707`) and the
...
1. Drive one firing to `Publishing` with a producer double, capture the predicate
   ...
   `StateMismatch` or `CasConflict`, and that `count(history_segments)` increased
   by exactly the history_segment count once.
2. Two firings on one session id, forced by constructing two `Handler`s over one
   ...
   than the publish.
3. `history_summarizer.rs:3011` `concurrent_lineages_reattach_and_publish_in_isolated_sessions`
