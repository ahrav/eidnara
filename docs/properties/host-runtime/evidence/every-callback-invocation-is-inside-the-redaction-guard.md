...
`crates/host-runtime/src/panic_boundary.rs` is 66 lines and contains the whole mechanism.
...
`REDACTED_DIAGNOSTIC` is the fixed string at `:7`, `"eidnara-host handler callback panicked (details
...
`crates/host-runtime/src/`, excluding comment lines, returns **27** call sites - 18 `redact_sync` and 9
...
`HostHandler` or provider trait object and assert each is lexically inside `redact_sync` or
`redact`.
...
- Sources examined: `grep -rn "panic_boundary::redact_sync(\|panic_boundary::redact("` over
  `crates/host-runtime/src/`, with comment-only lines filtered.
...
- Sources examined: all 66 lines of `crates/host-runtime/src/panic_boundary.rs`; `grep -n "cfg(test)\|mod
  tests"` on that file; `crates/host-runtime/tests/dispatch.rs:598-663`.
