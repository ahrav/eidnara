...
| --- | --- | --- |
| **F0** test execution in CI | A workflow job that builds and runs the checks a record's oracle would live in | **No, and this is the weakest position of the three sub-parts.** *In-crate: no.* All 11 execute in no job, because every `-p host-runtime` invocation carries a `--test <name>` filter and never builds the lib target; the 13 `host-runtime` hits are `:87`, `:132`, `:133`, `:134`, `:168`, `:169`, `:178`, `:187`, `:190`, `:211`, `:361`, `:442`, `:461`, and `:168-169` are `cargo build`. *Integration: no.* CI names `client`, `lifecycle`, `shm_failure_modes`, and `shm_soak`; it names none of `local_embeddings_bundle`, `harness_closure`, `ipc_budget_topology`, or `activation`. *Doctests: none exist.* The five files contain zero doc fences, yet `ci.yml:190` runs `cargo test -p host-runtime --doc` and `config.rs`, `harness_closure.rs`, and `lib.rs` are all `pub mod` (`lib.rs:14`, `:17`, `:18`), so a doctest added to any 2f file **would** run. That is the one CI lane this sub-part could reach and does not use. The single exception is transitive: `tests/lifecycle.rs` is CI-named and reaches `runtime.rs` because `run` is how any host starts, but it is Part 2a scope and tests lifecycle records rather than the configuration contract |
...
unresolved question in `daemon`, not a weakness in the code reading.
...
1. **A `compile_fail` or ordinary doctest in `config.rs`.** `config.rs` is
   `pub mod` (`lib.rs:14`), `ci.yml:190` runs `cargo test -p host-runtime --doc`, and
...
2. **`F0`, running the existing suites in CI.** A workflow change: add an
   unfiltered `-p host-runtime` invocation, or name `local_embeddings_bundle`,
...
11. **A classified startup-unavailability reason.** Last, because it is a
    ...
    ultimately reports to an operator - must be answered by an `daemon` pass
