...
- `crates/kernel/tests/kernel_claim_facts.rs`: `bounds_apply_before_decoding_and_malformed_required_fields_fail_explicitly` sets an 8-byte bound and asserts `Oversized` with an identity-only record whose `operation == None`.
...
Write a real record, read with a bound smaller than its payload, assert `Unknown(Oversized)` and a causal record preserving the row identity with `operation == None`.
