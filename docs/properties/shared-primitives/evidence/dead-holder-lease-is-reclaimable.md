...
- **Primary evidence:** the file-backend contract says the kernel releases the advisory lock on process death (`crates/lease/src/lib.rs:4-5`). PostgreSQL instead holds a session advisory lock, which the server releases when the connection drops (the shared-crate source repository at `89abb40`); its `Drop` impl also attempts `pg_advisory_unlock` before closing the connection (source at `89abb40`).
- **Existing evidence:** `shared_lease_across_processes_blocks_exclusive` lets the child exit normally once the parent closes its stdin; every in-process file release runs `Drop`. `superseded_writer_is_rejected_after_reopen` shows PostgreSQL reopening only after a normal drop (source at `89abb40`).
