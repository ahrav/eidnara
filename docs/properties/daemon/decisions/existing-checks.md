...
| --- | --- | --- | --- |
| `selection.rs` | 38 | `selection.rs:1467`, `selection.rs:3248` | unaudited |
| `boundary.rs` | 29 | `boundary.rs:1961`, `boundary.rs:2813` | unaudited |
| `config.rs` | 43 | `config.rs:1089`, `config.rs:2185` | unaudited |
| `codec/opencode.rs` | 17 | `codec/opencode.rs:1377`, `codec/opencode.rs:2135` | unaudited |
...
Total: 182 file-local tests. This excludes tests in `lib.rs`, `transform.rs`, and
...
| --- | --- | --- |
| `direct_host.rs`, `transform_canonical_memory.rs`, and the moved `../src/transform_meta_bound.rs` unit test | JSON string `"render_config"` (`direct_host.rs:96`, `direct_host.rs:162`, `../src/transform_meta_bound.rs:31`, `transform_canonical_memory.rs:127`) | A config-identity field on the request, not `config.rs`. Out of scope |
...
| --- | --- | --- | --- | --- |
| `selection.rs` | `mod tests` at `selection.rs:1309` | 38 | `selection.rs:1467`-`selection.rs:3248` | `natural_bust_drains_a_single_command_remainder` (`selection.rs:1467`); `selection_golden_matches_ts_selectors` (`selection.rs:1621`), the TypeScript selector golden; age reclaim, force band, emergency, and supersession batching from `age_reclaim_requires_both_scheduler_pressure_and_a_ride` (`selection.rs:1745`) through `supersession_floor_tracks_active_tag_window_on_thirty_arc_fixture` (`selection.rs:2381`); `edit_marker_region_hint_caps_utf16_and_backs_off_split_surrogate` (`selection.rs:2429`) and `edit_marker_region_hint_only_trusts_a_sentinel_on_a_hint_sized_value` (`selection.rs:2446`); the `provider_executed_arc_never_targeted` (`selection.rs:2466`), `frozen_arc_blocks_never_re_emitted` (`selection.rs:2496`), and `dynamic_block_protection_filters_automatic_and_agent_drop_decisions` (`selection.rs:2520`) filters; reasoning eligibility, cross-turn call ids, and the skeleton window from `reasoning_only_assistant_makes_the_whole_tool_arc_ineligible` (`selection.rs:2544`) through `skeleton_window_keeps_call_shell_older_full_drops` (`selection.rs:2716`); `drop_wins_over_edit_marker` (`selection.rs:2745`); `payload_purity_independent_of_pressure` (`selection.rs:2791`); agent drops, ride opportunities, and carriers from `agent_drop_ids_reduce_directly` (`selection.rs:2899`) through `pass_through_carriers_are_never_reduction_targets` (`selection.rs:2980`); the duplicate-safe-tool family from `duplicate_safe_tools_keep_the_newest_same_owner_full_drop` (`selection.rs:3002`) through `duplicate_full_drop_skeletonizes_for_reasoning_adjacency` (`selection.rs:3199`); `defer_pass_produces_nothing` (`selection.rs:3248`) |
...
| `codec/opencode.rs` | `mod tests` at `codec/opencode.rs:1286` | 17 | `codec/opencode.rs:1377`-`codec/opencode.rs:2135` | Fresh-part completeness, adjacency deletion, native-extras survival, polarity round trip, native time, wire reachability, and reasoning exemptions from `every_fresh_tool_part_from_folded_transform_fixture_has_complete_state` (`codec/opencode.rs:1377`) through `hard_epoch_fold_with_head_todo_coalesces_unmatched_reduced_tool_arc` (`codec/opencode.rs:1938`), including `compaction_is_extracted_as_boundary_not_content` (`codec/opencode.rs:1818`); `whole_array_tool_use_guard_rejects_both_id_collision_directions` (`codec/opencode.rs:2037`), debug-only behind `#[cfg(debug_assertions)]` (`codec/opencode.rs:2026`); `incremental_sidecar_carries_pins_across_three_generations` (`codec/opencode.rs:2071`); `text_before_latest_reasoning_uses_typed_wire_projection` (`codec/opencode.rs:2135`) |
