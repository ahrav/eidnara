...
| Parent/N1 exclusions | Out of Scope keeps canonical CRUD/maintenance/disposition/transitional removal and general MemoryClassifier elsewhere. TypeScript adapters/glue/configuration/presentation/confirmation/QuickJS/mural stay outside. No migration, predecessor compatibility, or Stage 1 re-verification work is added. |
| Rejected complexity | C1-C6 and Out of Scope exclude cross-database atomicity, post-load-truncated unbounded snapshots, second tokenizer authority, new LocalEmbeddings wire method, duplicate queue/store/scheduler/lease plane, and deleted-claim hardening. |
