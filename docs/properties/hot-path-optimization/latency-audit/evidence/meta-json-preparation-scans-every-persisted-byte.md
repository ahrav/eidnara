...
`block_identities`. The receipts of the documents one commit scans share an
owner named by the commit's row version, which the rows they wrote record in
`scan_version`; the owner is retired once no stored row names it, so the
receipts never outnumber the stored rows. The store test now plants the `meta` secrets in
...
`memory_classifier_ledger.rs`, and 22 in `production_redaction.rs`, the three above
among them. `cargo test -p daemon --locked` passed 1021; the two
`memory_classifier_run_task_bounds_*` tests fail under full-suite load on the base
