...
`RuntimeDir` (`crates/shm-transport/src/backend/ring.rs:291-377`) reads like a
...
builds `root.join("shm-{suffix}")` (`:326`), calls `mkdir` with `0o700`
...
(`:1753-1788`) calls `shm_open` with the name `/shm-<32 hex>`
...
`crates/shm-transport/tests/ring.rs` (for example `:67`, `:150`, `:257`) runs
