...
- `:6889-6897` `current_end = store.max_history_segment_end_ordinal(&session_id)`,
...
- `:6977-6989` the progress check: `after_end = max_history_segment_end_ordinal(...)`,
...
- `:5488` `let wait = history_summarizer::wrapup_round_wait_budget().min(remaining);` so the
...
- `:5558-5574` `await_wrapup_history_summarizer_completion` applies the same
...
2. A fence rejection, which abandons **without** arming the failure cooldown
   (`history_summarizer.rs:533-547`). The next round's backoff check at `lib.rs:6872-6887`
...
set to `MAX_WRAPUP_REQUEST_BUDGET` verbatim (`history_summarizer.rs:952-961`), so it would
...
- `max_history_segment_end_ordinal` must be the right progress signal. It reads the
  store, so it observes a committed publish and nothing else, which is the correct
  granularity.

## What a test must construct

1. The progress break: a producer double that publishes a fold whose history_segment
   ends at or before `current_end`. Assert the loop breaks with the
   "history_summarizer completed without advancing the history_segment boundary" reason and that
...
- Sources examined: `crates/daemon/src/history_summarizer.rs:533-547` and `:1786-1801`
...
- Findings: `SnapshotStale` is a `break`, not a `continue`, so a fence rejection
  ends the drain in the current code. The comment at `history_summarizer.rs:534-537`
