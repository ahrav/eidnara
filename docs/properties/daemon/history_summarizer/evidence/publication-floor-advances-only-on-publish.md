...
- `:1776` field declaration on `HistorySummarizerPublishRequest`.
...
`abandon_with_detail` (`history_summarizer.rs:348-361`) constructs a
`HistorySummarizerDurableState` from `HistorySummarizerDurableState::default()`. That type lives at
`meta.history_summarizer`; the floor lives at `meta.publication_floor_ordinal`, a sibling
field on `ModuleMeta` (`memory-store:2395`). `persist_history_summarizer_state`
(`history_summarizer.rs:391-403`) clones the loaded meta and replaces only
`meta.history_summarizer` (`:397-398`), so the floor is carried through unchanged. The same
holds for `abandon_history_summarizer_run_if_matching_with_publish_failure`
(`history_summarizer.rs:1786-1819` wrapping the store call) and
`record_history_summarizer_publish_failure_if_matching` (`:588-591`).
...
- `history_summarizer.rs:2360` `assert_eq!(loaded.meta.publication_floor_ordinal, None)`
  after `selected_range_identity_drift_during_await_rejects_without_cooldown`.
- `history_summarizer.rs:3006` the same assertion after
  `reattach_equal_length_identity_drift_rejects_before_publish`.
- `history_summarizer.rs:2309` and `:2404` assert `Some(4)` after publishes, including
  `tail_identity_extension_during_await_still_publishes`.
- `history_summarizer.rs:4025` and `:4050` assert floor values across the
...
- **False negative.** If a publish could commit without advancing the floor, the
  ...
  own reasoning (`history_summarizer.rs:983-997`) is that forwarding a raw array risks
...
derivation at `history_summarizer_chunk.rs:629-642` prevents. Worth asserting rather than
assuming.
...
2. Negative behaviour: drive a firing to each abandon path (fence rejection, CAS
   ...
   unchanged while the row version increased. `history_summarizer.rs:2360` and `:3006`
...
argument that the abandon paths rebuild `meta.history_summarizer` and cannot reach a sibling
