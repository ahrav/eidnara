...
Attention focus: what the history_summarizer pipeline writes, in what order, under what
compare-and-swap, and what can be lost or double-applied. Content validation is
a sibling lens's territory; this pass treats `validate_history_summarizer_output` as an
...
dde0c051..HEAD -- crates/daemon/src/` is empty, so every `daemon` line
reference in
...
`crates/memory-store/src/lib.rs` (the publish transaction itself) and
`../commons/crates/storage/src/lib.rs` (the transaction wrapper, which
lives outside this repository).

## Pipeline map

The pipeline has five durable phases and six durable writes. The phases are
named in the module header (`history_summarizer.rs:1-6`): `idle -> firing ->
...
`Handler::handle_transform_unpaged_value`. Two arms can start a firing. The
...
`/history_summarizer/module_model`, `/history_summarizer/model`, and their fallback arrays
(`config.rs:390-425`). `prepare_history_summarizer_fire` returns `no_models` when it is
empty (`lib.rs:5021-5031`). This is why every record in this lens is labelled
`explicit-config-only`.

**2. Assembly (no durable write).** `assemble_history_summarizer_firing`
(`history_summarizer_chunk.rs:611-790`) reads one snapshot of stored history_segments, the
revert epoch, and the history_segment-set generation (`:624-627`), picks the chunk
...
- `chunk_fingerprint`, ordered `id:kind:byte-length` pieces joined with `|`
  (`history_summarizer.rs:151-160`), computed at `history_summarizer_chunk.rs:751-756`.
- `raw_chunk_messages`, the original wire messages of the chunk range serialized
  to JSON (`history_summarizer_chunk.rs:717-727`). This is the user's real conversation,
  captured before the model has run.

**3. Fire (durable write 1).** `fire` refuses unless the phase is exactly `Idle`
(`history_summarizer.rs:251-253`) and otherwise returns a state with `firing_seq + 1`,
the chunk range, the fingerprint, the identities, the expected revert epoch, and
the history_segment-set generation (`:255-275`). `run_history_summarizer_firing` re-verifies
...
id, and harness (`history_summarizer.rs:278-297`), persisted at `:1332-1338`. The
...
(`history_summarizer_producer.rs:848-878`), so draining is replay-safe against the
producer. A timeout falls back to a re-drain (`history_summarizer.rs:1340-1368`).

**6. Validating (durable write 3).** `publish_output_from_awaiting` transitions
to `Validating` and persists before it validates (`history_summarizer.rs:1663-1664`),
...
use (`history_summarizer.rs:1706-1707`). The comment at `:1709-1713` states why the row
...
`publish_validated_chunk` (`history_summarizer.rs:444-595`) re-checks the fingerprint
(`:448-460`), projects the validated history_segments, events, primers, and optional
...
`publish_history_summarizer_chunk` (`memory-store/src/lib.rs:9351-9546`) does everything else
...
(`memory-store/src/lib.rs:9508-9517`). Failures stay queued for a later transform.

## Observations

### The publish transaction and its gates

- `memory-store/src/lib.rs:9360` opens the transaction via
  ...
  (`../commons/crates/storage/src/lib.rs:185-232`). The closure's `Ok`
  return is what causes `tx.commit()` at `:230`.
- Gate 1, row-version CAS: `memory-store/src/lib.rs:9373-9382` compares the current
...
- Gate 3, predicate identity: `:9398-9407` requires `firing_seq`,
  ...
  `history_segment_set_generation` all to match.
...
- Gate 6, history_segment-set generation, re-read inside the transaction:
  `:9436-9455` recomputes `(MAX(sequence), COUNT(*))` and rejects a mismatch.
- Gate 7, range overlap, the storage backstop: `append_history_segments_tx`
...
1. `append_history_segments_tx` inserts the model-generated history_segment rows
   (`:9457-9471`).
2. `insert_chunk_transcripts_tx` inserts the condensed transcript and the
   deflated original wire messages, keyed to the same sequences (`:9472-9481`,
   implementation at `:12671-12716`).
3. `enqueue_history_summarizer_side_channels_tx` queues events, primers, and user
   observations (`:9482`).
4. `meta.publication_floor_ordinal` is raised, never lowered, by a `max`
   (`:9484-9488`).
5. `meta.history_summarizer` is reset to idle at the same `firing_seq` (`:9489`).
6. `UPDATE cache_state SET row_version = current + 1, meta = ...` with
   `WHERE row_version = current` (`:9491-9500`).

### Preservation of the original

- `raw_chunk_messages` is always `Some` on this path (`history_summarizer.rs:524-525`),
  and its field doc calls it "Original wire messages for exact durable
  full-message and verbose recovery" (`history_summarizer.rs:425-426`).
- The condensed transcript is dropped when it compresses past
  `MAX_CHUNK_TRANSCRIPT_COMPRESSED_BYTES` (256 KiB, `memory-store:405`, applied at
...
- The only paths that delete `chunk_transcripts` are whole-session teardown
  (`:8894`, `:8960`) and the suffix revert at `:9106`, which deletes transcripts
  and history_segments for the same reverted suffix inside one transaction
  (`:9105-9111`).

### Single flight, races, and fences

- In-process claim: `try_claim_live_history_summarizer_session` (`lib.rs:4556-4581`)
  inserts a per-session entry and hands back a guard whose `Drop` removes it and
  notifies waiters. Both `prepare_history_summarizer_fire` (`lib.rs:5146-5163`) and
  `prepare_wrapup_fire` (`lib.rs:5286-5291`) go through it.
- Durable claim: `fire` returns `Busy` on any non-idle phase
  (`history_summarizer.rs:251-253`). `prepare_wrapup_fire` additionally refuses when the
...
- Cross-process: the store is opened under a single-writer lease acquired before
  the file is opened (`../commons/crates/storage/src/lib.rs:265-277`),
...
- Losing-race handling is differentiated. A fence rejection abandons **without**
  ...
  (`history_summarizer.rs:533-547`, helper at `:1786-1801`). A history_segment overlap does
...
- `handle_restart_load` (`history_summarizer.rs:620-655`) maps `Idle` to `Done`,
...
- A reattach whose `producer.status` call fails does **not** abandon; the comment
  at `history_summarizer.rs:1499-1504` says abandoning would authorize a second billable
...
- Chunks are ordered by construction, not by an applied-in-order check. The next
  ...
  (`history_summarizer_chunk.rs:629-642`), the store assigns sequences from
  `MAX(sequence) + 1` at write time (`memory-store:12618`, `:12654-12656`), and
  ...
  (`history_summarizer_validate.rs:625-635`) and refuses an empty history_segment set
  (`:487-491`).
- `unprocessed_from` is `last_new_end + 1` where `last_new_end` is the end of the
  last **kept** history_segment after discard-last healing
  (`history_summarizer_validate.rs:638`), and it becomes the publication floor
  (`history_summarizer.rs:1725`).
- The wrapup drain has no round cap by design (`lib.rs:6831-6834`,
  `history_summarizer.rs:952-961`); its ceiling is `MAX_WRAPUP_REQUEST_BUDGET` (3800 s,
  `history_summarizer.rs:962`) with a per-round bound of `wrapup_round_wait_budget`
  (600 s, `:966`). Each round demands observable progress: if
  `max_history_segment_end_ordinal` did not advance, the loop breaks with a
...
- Output-failure fallback demands typed proof that the failed attempt stopped:
  ...
  (`history_summarizer.rs:1401`), where only `Ok(())` counts as proof, with the reasoning
  spelled out at `:1226-1240`.
- Start-failure fallback demands no such proof. The branch at
  `history_summarizer.rs:1290-1329` persists an abandon and, if `decision.try_next_model`,
  ...
  (`:1052-1143`) never inspects the send outcome. `HistorySummarizerSendOutcome` has an
  `OutcomeUnknown` variant (`history_summarizer_producer.rs:78-82`), so a start that
  actually reached ModelExecution and began a run can be followed by a second start.

## Commit point and irreversibility

**The commit point is the transaction opened at
`crates/memory-store/src/lib.rs:9360` and committed by `tx.commit()` at
`../commons/crates/storage/src/lib.rs:230`.** Inside the repository the
last operation before that commit is the row-version bump at
`crates/memory-store/src/lib.rs:9496-9500`. Everything the substitution depends on
lands in that one transaction: the model-generated history_segment rows
...
1. **The substitution is additive, not destructive.** The transaction writes no
   ...
   (`memory-store:9345-9350`), matching `history_summarizer.rs:441-443`. Nothing on the
   publish path deletes or overwrites a source message.
2. **The user's original conversation is recoverable.** The original wire messages
   for the folded range are serialized at assembly time
   (`history_summarizer_chunk.rs:717-727`), carried through the publish request
   (`history_summarizer.rs:525`), and inserted in the same transaction as the summary
   (`memory-store:9472-9481`). Session-level eviction is designed never to reclaim
   them (`:12748-12756`). Two independent copies therefore survive a publish:
   `chunk_transcripts.raw_messages_deflate`, and the harness's own session
   file, which this module reads but never writes.
3. **The reversal is a suffix revert, not an undo.** `:9105-9111` drops
   history_segments and their transcripts above a kept sequence inside one
...
point (`lib.rs:5021-5031`, `lib.rs:5228-5231`, `history_summarizer.rs:1249-1251`). Each
...
Status: active
Exercised: partial — `memory-store/src/lib.rs:16984` `publish_history_summarizer_chunk_persists_transcript_inside_cas` and `:17017` `publish_history_summarizer_chunk_cas_conflict_leaves_no_transcript_row` cover the transcript half of the atom; no test asserts all six writes land or none do, and no CI job runs either (`ci.yml:171-172` is the only `daemon` test invocation and it runs one integration binary).
Guarantee: The history_segment rows, the deflated original messages, the queued side channels, the raised publication floor, and the history_summarizer phase reset either all become durable together or none of them do.
Check: `always` — after any outcome of `publish_history_summarizer_chunk`, the observed store satisfies: `count(history_segments appended by this predicate) > 0` if and only if `publication_floor_ordinal >= validated.unprocessed_from` and `meta.history_summarizer.state == Idle` at the published `firing_seq` and a `chunk_transcripts` row exists for each appended sequence. Semantics are `always` because it constrains every reachable post-publish state, not one code point.
Fault/timing angle: Process kill or SQLite failure between any two of the six writes at `memory-store:9457-9500`. The window is the interior of one `Immediate` transaction, so the property is the claim that the wrapper's commit boundary really is the only visible boundary.
...
Confidence: high — [evidence](../evidence/publish-transaction-is-the-single-commit-point.md). Read the whole closure at `memory-store:9360-9505` and the wrapper at `../commons/crates/storage/src/lib.rs:185-232`; confirmed the single `tx.commit()` at `:230` and that every early return inside the closure is a value, not a commit.
Existing check: `memory-store/src/lib.rs:16984`, `:17017`, `:18221` (`publish_history_summarizer_chunk_fails_loud_from_non_publish_state`). Status `unaudited`.
Impact: A partial commit that appended history_segments without raising the floor, or raised the floor without appending, would leave a range that is neither summarized nor eligible for boundary placement.
Open questions:
...
Reachability: explicit-config-only
Status: active
Exercised: partial — `memory-store/src/lib.rs:16984` asserts a transcript row exists after a publish and `:17017` asserts none exists after a CAS conflict; neither asserts that `raw_messages_deflate` round-trips to the exact pre-publish messages. Not run in CI.
Guarantee: Every history_segment appended by a publish has, in the same transaction, a durable deflated copy of the original wire messages for the range it replaces.
Check: `always` — for every appended history_segment sequence `s`, a `chunk_transcripts` row exists with `history_segment_seq = s` and non-null `raw_messages_deflate` that inflates to the exact JSON serialized at `history_summarizer_chunk.rs:717-727`. `always` because this is the invariant that makes the substitution recoverable, and it must hold in every post-publish state.
...
Required faults and enabling state: A configured model chain and one accepted publish. To attack it: a chunk whose compressed raw payload is large, and a chunk whose condensed transcript exceeds 256 KiB so the transcript is dropped (`memory-store:12682-12686`) while raw must survive.
Confidence: high — [evidence](../evidence/publish-preserves-raw-chunk-messages-atomically.md). Traced `raw_chunk_messages` from `history_summarizer_chunk.rs:717-727` through `history_summarizer.rs:525` and `memory-store:9472-9481` into `:12687-12713`; confirmed no size filter on the raw payload and that a compression error aborts the publish.
Existing check: `memory-store/src/lib.rs:16984`, `:17039`-region transcript-cap tests. Status `unaudited`.
...
- `insert_chunk_transcripts_tx` returns early when both payloads are absent (`memory-store:12691-12693`) and when the history_segment list is empty (`:12679-12681`). The history_summarizer path cannot reach either, but nothing in the store's own signature prevents another caller from publishing history_segments with no recoverable original. Unresolved, needs a decision on whether the store should enforce it.
...
- Is unbounded raw retention the intended contract, or is a separate raw budget missing? The comment at `memory-store:12749-12750` reads as deliberate. Needs a product decision on long-session storage. (needs human input)

### history_summarizer-single-flight-admits-one-publish-per-firing

Type: safety
Reachability: explicit-config-only
Status: active
Exercised: partial — `history_summarizer.rs:4243` `pure_state_machine_happy_path_and_single_flight` covers the pure `fire`/`Busy` transition and `:3011` `concurrent_lineages_reattach_and_publish_in_isolated_sessions` covers two lineages; no test drives two concurrent publishes against one session id. Not run in CI.
...
Existing check: `history_summarizer.rs:4243`, `:4314` `fingerprint_mismatch_at_publish_abandons_and_releases_single_flight`, `:4451` `history_segment_generation_fence_releases_overlapped_publish_to_idle`. Status `unaudited`.
...
Reachability: explicit-config-only
Status: active
Exercised: partial — `history_summarizer.rs:2323` `selected_range_identity_drift_during_await_rejects_without_cooldown` and `:2942` `reattach_equal_length_identity_drift_rejects_before_publish` cover the reject; `:2369` `tail_identity_extension_during_await_still_publishes` covers the permitted extension. Not run in CI.
...
Fault/timing angle: The whole model-run window, which is minutes. A harness can edit, retract, or re-stamp a message while the producer runs. The fingerprint alone would not catch a same-length content edit; the module header says so explicitly (`history_summarizer.rs:141-143`).
...
Confidence: high — [evidence](../evidence/publish-fence-rejects-selected-content-drift.md). Read the fence at `memory-store:9413-9425` and confirmed the empty-vector rejection is separate from and prior to the per-mid comparison, with the reasoning at `:9409-9412`.
Existing check: `history_summarizer.rs:2323`, `:2369`, `:2942`, `:3776` `reattach_fingerprint_mismatch_recovers_to_idle_and_releases_routes`. Status `unaudited`.
...
Reachability: explicit-config-only
Status: active
Exercised: not yet — `memory-store/src/lib.rs:18221` `publish_history_summarizer_chunk_fails_loud_from_non_publish_state` proves some phases are refused, but no test pins which phases are admitted, and no test asserts a committed publish was preceded by a `Validating` transition.
...
Check: `always` — at the instant of commit, `meta.history_summarizer.state == Publishing`. `always` rather than `unreachable`, because the thing to forbid is a state at the commit point, not the execution of a code location: the guard at `memory-store:9389-9396` is one expression covering both admitted phases and cannot be marked as a forbidden location.
...
Required faults and enabling state: A configured model chain plus a caller that reaches `publish_history_summarizer_chunk` from `AwaitingProducer`. `publish_output_from_awaiting` always transitions first (`history_summarizer.rs:1706-1707`), so today the coverage check is on the preconditions: the store's phase guard admits two phases, and the module has more than one publish route (`history_summarizer.rs:527-530`, plus two fence implementations at `lib.rs:3296` and `:3332`).
Confidence: medium — [evidence](../evidence/publish-admits-awaiting-producer-phase-at-commit.md). The widening is verified by reading `memory-store:9389-9396`. What is not established is whether any current caller exercises it; I found no in-repo caller that does, and I could not rule out an external consumer of the public `memory-store` API.
Existing check: `memory-store/src/lib.rs:18221`. Status `unaudited`.
Impact: The documented five-phase machine (`history_summarizer.rs:1-6`) and the "fail-closed before any database write" claim (`history_summarizer_validate.rs:1-9`) both rest on validation preceding publish. The commit point does not check that. A future or external caller that skips `validation_ok` publishes unvalidated model text through the same gates.
Open questions:
...
Reachability: explicit-config-only
Status: active
Exercised: partial — `history_summarizer.rs:4596` `restart_mid_awaiting_exposes_reattach_ids` and `:4647` `restart_mid_publishing_with_committed_tx_detects_idle` cover the two load outcomes with a simulated restart; neither uses a real process kill. Not run in CI.
Guarentee placeholder removed.
Guarantee: A crash at any point before the publish transaction commits leaves no history_segment rows, no transcript rows, and no raised floor, and the next load makes the session refire-eligible rather than stuck.
Check: `always-or-unreached` — if a restart observes `state in {Firing, Validating, Publishing}`, then no history_segment appended by that `firing_seq` exists and the load transitions the row to `Idle` with a backoff; if it observes `Idle`, the publish either committed or never fired. `always-or-unreached` because a crash in this window is an optional event that a campaign may not produce, and the property must hold whenever it does.
Fault/timing angle: Four distinct windows: between the fire persist and the producer start, between the start and the output, between the `Validating` persist and the `Publishing` persist, and between the `Publishing` persist and the transaction commit. The last is the dangerous one and is the one `handle_restart_load` reasons about at `history_summarizer.rs:616-619`.
...
Confidence: high — [evidence](../evidence/crash-before-publish-commit-refires-without-partial-state.md). Verified that `Publishing` is a separate transaction from the publish (`history_summarizer.rs:1707` versus `memory-store:9360`), that the publish CAS uses the version that transition wrote (`history_summarizer.rs:1719`), and that `handle_restart_load` abandons all three pre-commit phases (`:648-653`).
Existing check: `history_summarizer.rs:4596`, `:4647`, `:3776`. Status `unaudited`.
...
- `handle_restart_load` for `AwaitingProducer` with missing producer ids abandons (`history_summarizer.rs:630-639`), but `MissingProducerIds` is also a `publish_predicate` error (`:377-381`). Whether the two paths agree on what a partially written `AwaitingProducer` row means is unresolved, needs a targeted test.
...
Reachability: explicit-config-only
Status: active
Exercised: not yet — `history_summarizer.rs:3389` `unconfirmed_cancellation_stops_the_fallback_chain` and `:3444` `uncertain_cancel_send_outcomes_stop_the_fallback_chain` cover the symmetric protection on the output path. No test covers a start failure carrying `OutcomeUnknown`.
...
Confidence: high — [evidence](../evidence/uncertain-producer-start-authorizes-a-second-billable-run.md). Read `history_summarizer.rs:1290-1329` and confirmed there is no `cancel`, no outcome inspection, and no `cancellation_confirmed_stopped` call on that branch, while `:1401` requires exactly that proof on the output branch. Confirmed `decide_producer_failure` (`:1052-1143`) never reads the send outcome, and neither does `heuristic_decision` (`history_summarizer_producer.rs:412-433`).
Existing check: `history_summarizer.rs:3389`, `:3444`, `:3498` `a_terminal_cancel_error_never_authorizes_fallback`. All cover the output branch only. Status `unaudited`.
Impact: Duplicate spend and a duplicate provider run, not a duplicate publish. The orphaned run's output is never drained because its id is unknown, and the second attempt fires under a new `firing_seq` (`history_summarizer.rs:257`) with its own producer session id (`:1013-1035`), so only one publish can commit. The cost is money and provider load, plus a live run the module cannot cancel.
Open questions:
...
Reachability: explicit-config-only
Status: active
Exercised: partial — `history_summarizer.rs:2309` and `:4199` assert the floor equals a specific value after a publish, and `:2360` and `:3006` assert it stays `None` after a rejection. No test relates the floor to the appended history_segment ends generally. Not run in CI.
Guarantee: After any publish, the publication floor is at most one past the highest appended history_segment end, and it never decreases.
Check: `always` — after every committed publish, `meta.publication_floor_ordinal <= MAX(end_message) + 1` over all history_segments for the session, and `floor_after >= floor_before`. `always` because it constrains every post-publish state.
Fault/timing angle: None for monotonicity, which is structural via the `max` at `memory-store:9484-9488`. The upper bound depends on validation producing `unprocessed_from = last_new_end + 1` after discard-last healing (`history_summarizer_validate.rs:638`), so the interesting case is a healed batch where the last emitted history_segment was popped (`:539-556`).
Required faults and enabling state: A configured model chain and an accepted publish whose last history_segment was discarded by boundary healing, which needs at least two history_segments and a lookahead distance within `BOUNDARY_HEALING_SLACK` (`history_summarizer_validate.rs:19`, applied at `:554`).
Confidence: high — [evidence](../evidence/publication-floor-never-outruns-appended-coverage.md). Traced the floor from `history_summarizer_validate.rs:638` through `history_summarizer.rs:1725` and `:523` to `memory-store:9484-9488`, and confirmed the empty-history_segment rejection at `history_summarizer_validate.rs:487-491` and the forward-progress check at `:625-635` together prevent a floor advance with no appended coverage.
Existing check: `history_summarizer.rs:2309`, `:2360`, `:3006`, `:4199`. Status `unaudited`.
Impact: A floor past the covered range would make messages in the gap ineligible as boundary candidates (`boundary.rs:1417-1426`) and ineligible for tool-arc fencing (`:1339-1357`) while no history_segment summarizes them. They would still be served verbatim, so this is a boundary-quality defect rather than data loss.
Open questions: None.
...
Reachability: explicit-config-only
Status: active
Exercised: partial — the emergency re-run at `lib.rs:8490-8501` depends on this claim and `history_summarizer.rs:2360` and `:3006` show a rejected run leaves the floor untouched. No test enumerates the write sites. Not run in CI.
...
Confidence: high — [evidence](../evidence/publication-floor-advances-only-on-publish.md). Grepped every `publication_floor_ordinal` occurrence in `memory-store/src/lib.rs`; the only production assignment is `:9484-9488`, and the abandon path rebuilds only `HistorySummarizerDurableState` (`history_summarizer.rs:353-360`), which lives under `meta.history_summarizer` and cannot touch the sibling field. The comment at `lib.rs:8484-8486` states the intent and the reason row-version advancement is not used instead.
Existing check: `history_summarizer.rs:2360`, `:3006`. Status `unaudited`.
...
Guarantee: A wrapup drain either advances the maximum history_segment end ordinal on every counted round or stops with a retryable failure, and it terminates within the request budget.
Check: `always` on the per-round progress condition plus a bounded liveness check on termination: after `MAX_WRAPUP_REQUEST_BUDGET` (3800 s, `history_summarizer.rs:962`) the handler has returned a response, and for every counted round `after_end > before_end`. The bound is stated in the unit the code bounds, a wall-clock budget re-checked before each round (`lib.rs:6831-6834`), because there is deliberately no round-count cap.
Fault/timing angle: The loop has no round cap by design, so its only ceiling is the deadline. A producer that publishes an empty-progress fold, or a repeated fence rejection with no cooldown (`history_summarizer.rs:533-547`), are the two shapes that could spin.
...
- Can a fence rejection with no cooldown (`history_summarizer.rs:533-547`) recur every round until the budget expires, or does the snapshot generation stabilize? Unresolved, needs a test that holds the transform-snapshot generation moving while a wrapup drains.
...
1. **The documented five-phase machine is not enforced at the commit point.**
   `history_summarizer.rs:1-6` names `validating` as a phase the pipeline passes through,
   and `history_summarizer_validate.rs:1-9` says validation resolves everything "before
   any database write is possible". The publish transaction admits
   `HistorySummarizerPhase::AwaitingProducer` as well as `Publishing`
   (`memory-store:9389-9396`), so the commit point does not require that a
   validation ever happened. Captured as
   `publish-admits-awaiting-producer-phase-at-commit`.

2. **The store, not validation, assigns the history_segment sequence.**
   `to_stored_history_segment` copies the validated sequence onto the row shape
   (`history_summarizer.rs:38-45`) and its doc says "publication stamps the row"
   (`:35-37`). `append_history_segments_tx` ignores that field and assigns
   `MAX(sequence) + 1 + index` at write time (`memory-store:12618`, `:12654-12656`,
   `insert_history_segment_tx` signature at `:12352-12357`). The validated sequence
   is not dead, though: `to_store_event` uses it to resolve a side-channel
   event's `history_segment_id` (`history_summarizer.rs:87-88`), as do the primer and
   user-observation projections (`:102-104`, `:128-130`). So an event's
   `history_segment_id` is derived from `validate_options.sequence_offset` while the
   ...
   suffix revert (`memory-store:9110`), but the coupling is undocumented and the two
   call sites derive the offset differently: assembly uses
   `MAX(sequence) + 1` (`history_summarizer_chunk.rs:758-763`) and reattach uses
   `prior_history_segments.len() + 1` (`lib.rs:4761`). Not raised to a record
   because I could not construct a non-contiguous state; recorded here so the
   synthesis pass can decide.

3. **`HistorySummarizerReattachRequest::publication_floor_ordinal` is written and never
   read.** `lib.rs:4769` sets it to `range.to_ordinal`, and the field is declared
   at `history_summarizer.rs:931`, but `reattach_history_summarizer_producer` builds its
   `PublishOutputRequest` without it (`history_summarizer.rs:1592-1610`) and the floor
...
4. **"A publish never mutates cached render state" is stated twice and holds, but
   only for the store transaction.** `history_summarizer.rs:441-443` and
   `memory-store:9345-9350` both make the claim, and the transaction does leave
...
6. **The scope map's CI line reference has drifted.**
   ...
   daemon --test lifecycle_cli`. At this `HEAD` that step is
   `.github/workflows/ci.yml:171-172`. The substance is unchanged: it is still
   the only `daemon` test invocation in the workflow set, so none of the 51
   test functions in `history_summarizer.rs` and none of the `memory-store` publish tests run
   in CI.
...
- The commit point lives outside this repository. `tx.commit()` is at
  `../commons/crates/storage/src/lib.rs:230`, in a sibling checkout that
...
- Is there a fault-injection seam inside the publish transaction? The
  ...
  history_segment append and the row-version bump. Every atomicity record here is
...
- Can two publishes commit for one session across two processes? The single-writer
  ...
  (`../commons/crates/storage/src/lib.rs:265-277`) and every transaction
...
- The reattach path passes `in_emergency: false` and
  `force_keep_last_history_segment: false` (`lib.rs:4762`, `:4767`) regardless of the
