...
import { CLASSIFY_SYSTEM_PROMPT } from "../../features/context/dreamer/classify-prompt";
import { MAP_MEMORIES_SYSTEM_PROMPT } from "../../features/context/dreamer/map-memories-prompt";
import {
...
} from "../../features/context/dreamer/task-prompts";
import { VERIFY_SYSTEM_PROMPT } from "../../features/context/dreamer/verify-prompt";
...
describe("system-prompt-hash v2 system prompt contents", () => {
    it("keeps project docs, user profile, and key files out of the system prompt", async () => {
        ...
        expect(joined).toContain("## Eidnara");
...
describe("single system-entry serialization (issue #311)", () => {
    it("produces one system message through the real OpenAI-compatible converter", async () => {
        ...
        expect(system).toHaveLength(1);
        expect(system[0]).toContain("## Eidnara");
    ...
    it("documents Anthropic's unavoidable two-block to one-block byte transition", async () => {
        ...
        expect(guidance).toContain("## Eidnara");
...
/**
 * Eidnara does not inject guidance into OpenCode's title, summary, or compaction agents.
 */
describe("system-prompt-hash skips OpenCode internal hidden agents (issue #52)", () => {
    ...
    it("skips ALL injection for the title agent (signature from title.txt)", async () => {
        ...
        // The handler does not append `## Eidnara`, `<project-docs>`, `<user-profile>`, or `<key-files>` to the system prompt.
        // The handler preserves the system array exactly as OpenCode passed it.
        expect(system).toHaveLength(1);
        expect(system[0]).toBe(TITLE_PROMPT_HEAD);
        expect(system.join("\n")).not.toContain("## Eidnara");
    });
...
/**
 * The handler must not inject guidance into Eidnara's historian, dreamer, sidekick, or migration children.
...
describe("system-prompt-hash skips Eidnara internal child agents", () => {
    ...
    // All dreamer task prompts contain "for the memory system".
    // The dreamer signature must match each task-prompt opener to exclude dreamer children before the title flag is available.
    const DREAMER_BASE_HEAD = "You are a background maintenance agent for the memory system,";
    const CURATE_HEAD = "You are a memory-pool curator for the memory system.";
    const MAINTAIN_DOCS_HEAD = "You are a documentation maintainer for the memory system.";
    const REVIEW_USER_HEAD = "You are a user-profile reviewer for the memory system.";
    const PRIMER_HEAD = "You are a read-only code investigator for the memory system.";
    ...
    for (const [label, head] of [
        ...
        it(`skips ALL injection for the ${label} agent (prompt signature)`, async () => {
            ...
            const { handler } = buildHandler();
            const system = [head];
            await handler({ sessionID: `ses-eidnara-${label}` }, { system });
            expect(system).toHaveLength(1);
    ...
    it("detects every dedicated Eidnara child prompt constant", () => {
        const prompts = [
        ...
        for (const [label, prompt] of prompts) {
            expect(isEidnaraInternalAgent(prompt), label).toBe(true);
        }
    });
 ...
 * The internal-child skip runs before subagent handling, so internal Eidnara children receive no block.
 * branch).
 */
describe("system-prompt-hash subagent self-management (Unit B)", () => {
    ...
    it("injects NO block for a subagent without callable ctx_reduce (no primary-role leak)", async () => {
        ...
        // If the tool allow-list excludes `ctx_reduce`, the subagent receives no Eidnara block.
        // guidance).
        ...
        const joined = system.join("\n");
        expect(joined).not.toContain("## Eidnara");
        expect(joined).not.toContain("long-term partner");
        expect(joined).not.toContain("ctx_memory");
    });
    ...
    it("ORDER INVARIANT: an internal Eidnara child that is ALSO marked subagent still skips entirely", async () => {
...
/**
 * Agents opted out of system-prompt injection must not receive Eidnara tool guidance.
 * Tool opt-outs suppress Eidnara guidance because that guidance requires unavailable tools.
 */
describe("system-prompt-hash honors per-agent opt-out (issue #53)", () => {
    ...
    it("does NOT skip when skip signatures don't match the prompt", async () => {
        ...
        // Eidnara injection appends to the existing system entry instead of adding a second entry.
        expect(system).toHaveLength(1);
        expect(system[0]).toContain("## Eidnara");
    });
...
describe("OpenCode prompt-surface guidance epochs", () => {
    ...
    it("emits one hash fold when a preset/model boundary selects authored light", async () => {
        ...
        const firstHash = getOrCreateSessionMeta(openDatabase(), sessionID).systemPromptHash;
        expect(first).toContain("## Eidnara");
        historyRefreshSessions.clear();
        systemPromptRefreshSessions.clear();
        pendingMaterializationSessions.clear();
