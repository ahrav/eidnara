//! MemoryReviewer review jobs and frozen Memory Classifier selections.
//!
...
pub const MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_PROJECT: usize = 64;
pub const MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_HOST: usize = 256;
...
pub const MAX_MEMORY_REVIEWER_STARTING_REFERENCES: usize = 8;
...
pub const MAX_MEMORY_REVIEWER_JOB_INPUT_BYTES: usize = 8 * 1024;
...
pub const MEMORY_REVIEWER_JOB_ALLOWANCE_BYTES: u64 = 32 * 1024;
...
/// Reference-only job input: the subject, at most eight starting references, and the fixed question template. Serialized size is bounded by [`MAX_MEMORY_REVIEWER_JOB_INPUT_BYTES`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct MemoryReviewerJobInput {
...
pub enum MemoryReviewerJobOutcome {
    Expired,
    Nonadmitted,
    Failed,
    Unknown,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MemoryReviewerJobState {
    Reserved,
    Ready(MemoryReviewerJobInput),
    Terminal(MemoryReviewerJobOutcome),
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MemoryReviewerJob {
    pub project: String,
    ...
    pub state: MemoryReviewerJobState,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MemoryReviewerHeadroom {
...
pub enum MemoryReviewerJobRefusal {
    #[error("the request is malformed or over a bound")]
    InvalidRequest,
    #[error("the project already has {MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_PROJECT} pending jobs")]
    ProjectCapacity,
    #[error("the host already has {MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_HOST} pending jobs")]
    HostCapacity,
...
pub enum MemoryReviewerJobError {
    #[error(transparent)]
    Refused(#[from] MemoryReviewerJobRefusal),
    #[error(transparent)]
    Store(#[from] MemoryStoreError),
}

impl From<rusqlite::Error> for MemoryReviewerJobError {
...
pub(crate) fn refuse(refusal: MemoryReviewerJobRefusal) -> rusqlite::Error {
...
pub(crate) fn refusal_of(error: &rusqlite::Error) -> Option<MemoryReviewerJobRefusal> {
    match error {
        rusqlite::Error::ToSqlConversionFailure(inner) => {
            inner.downcast_ref::<MemoryReviewerJobRefusal>().copied()
        }
        _ => None,
    }
}
...
pub(crate) fn check_project(project: &str) -> Result<(), MemoryStoreError> {
    check_identity(project).map_err(|_| {
        MemoryStoreError::Serde("memory_reviewer project must be 1 to 256 bytes".to_string())
    })
}

fn check_identity(value: &str) -> Result<(), MemoryReviewerJobRefusal> {
    if value.is_empty() || value.len() > MAX_IDENTITY_BYTES {
        return Err(MemoryReviewerJobRefusal::InvalidRequest);
    }
    Ok(())
}
...
impl ReviewTarget {
    fn validate(&self) -> Result<(), MemoryReviewerJobRefusal> {
        match self {
            Self::StagedSubject {
                kernel_incarnation,
                candidate_id,
    ...
    fn encode(&self) -> Result<String, MemoryReviewerJobRefusal> {
        let text =
            serde_json::to_string(self).map_err(|_| MemoryReviewerJobRefusal::InvalidRequest)?;
...
impl CausalInputs {
    ...
    fn normalized(mut self) -> Result<Self, MemoryReviewerJobRefusal> {
    ...
    fn fingerprint(&self) -> Result<String, MemoryReviewerJobRefusal> {
        #[derive(Serialize)]
        ...
        .map_err(|_| MemoryReviewerJobRefusal::InvalidRequest)?;
        Ok(digest_hex(&bytes))
    }

    /// The causal review identity: target digest joined with the fingerprint.
    pub fn causal_identity(&self) -> Result<String, MemoryReviewerJobRefusal> {
...
impl MemoryReviewerJobInput {
    ...
    fn encode(&self) -> Result<String, MemoryReviewerJobRefusal> {
        ...
        check_identity(&self.question_template)?;
        if self.starting_references.len() > MAX_MEMORY_REVIEWER_STARTING_REFERENCES {
            return Err(MemoryReviewerJobRefusal::InvalidRequest);
        }
        ...
        let text =
            serde_json::to_string(self).map_err(|_| MemoryReviewerJobRefusal::InvalidRequest)?;
        if text.len() > MAX_MEMORY_REVIEWER_JOB_INPUT_BYTES {
            return Err(MemoryReviewerJobRefusal::InvalidRequest);
        }
        Ok(text)
    }
...
impl ProducerBinding {
    fn validate(&self) -> Result<i64, MemoryReviewerJobRefusal> {
        if self.producer.is_empty() || self.producer.len() > 64 {
            return Err(MemoryReviewerJobRefusal::InvalidRequest);
        }
        check_identity(&self.firing_id)?;
        i64::try_from(self.ordinal).map_err(|_| MemoryReviewerJobRefusal::InvalidRequest)
    }
}
...
fn job_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<MemoryReviewerJob> {
    ...
    let state = match (state.as_str(), input_json, outcome) {
        ("reserved", None, None) => MemoryReviewerJobState::Reserved,
        ("ready", Some(input), None) => MemoryReviewerJobState::Ready(
            serde_json::from_str(&input).map_err(|_| undecodable(8, "input_json"))?,
        ),
        ("terminal", _, Some(outcome)) => MemoryReviewerJobState::Terminal(
            MemoryReviewerJobOutcome::parse(&outcome).ok_or_else(|| undecodable(9, "outcome"))?,
        ),
        _ => return Err(undecodable(7, "state")),
    };
    Ok(MemoryReviewerJob {
        project: row.get(0)?,
...
pub(crate) fn load_memory_reviewer_job(
    conn: &GuardedConn<'_>,
    project: &str,
    causal_identity: &str,
) -> rusqlite::Result<Option<MemoryReviewerJob>> {
    conn.query_row(
        &format!(
            "SELECT {JOB_COLUMNS} FROM memory_reviewer_jobs WHERE project = ?1 AND causal_identity = ?2"
        ),
...
pub(crate) fn metadata_bytes(
    ...
    let jobs = scalar(
        conn,
        project,
        "SELECT COALESCE(SUM(receipt_charge_bytes + allowance_bytes), 0) FROM memory_reviewer_jobs
         WHERE project = ?1",
        "SELECT COALESCE(SUM(receipt_charge_bytes + allowance_bytes), 0) FROM memory_reviewer_jobs",
    )?;
    let pages = scalar(
        ...
        "SELECT COALESCE(SUM(receipt_charge_bytes + allowance_bytes), 0)
         FROM memory_reviewer_frozen_selections WHERE project = ?1",
        "SELECT COALESCE(SUM(receipt_charge_bytes + allowance_bytes), 0)
         FROM memory_reviewer_frozen_selections",
    )?;
...
fn pending_jobs(conn: &GuardedConn<'_>, project: Option<&str>) -> rusqlite::Result<usize> {
    let count = scalar(
        conn,
        project,
        "SELECT COUNT(*) FROM memory_reviewer_jobs
         WHERE project = ?1 AND state IN ('reserved', 'ready')",
        "SELECT COUNT(*) FROM memory_reviewer_jobs WHERE state IN ('reserved', 'ready')",
    )?;
...
fn frozen_pages(conn: &GuardedConn<'_>, project: Option<&str>) -> rusqlite::Result<usize> {
    let count = scalar(
        conn,
        project,
        "SELECT COUNT(*) FROM memory_reviewer_frozen_selections
...
fn check_quota(
    ...
    if !fits(
        ...
        MAX_MEMORY_REVIEWER_METADATA_BYTES_PER_HOST,
    ) {
        return Err(refuse(MemoryReviewerJobRefusal::MetadataQuota));
    }
    Ok(())
}

/// Reserves one job for `inputs` inside the caller's fenced transaction, or returns the row the same causal identity already has. The queue deadline is `now_ms` plus [`MEMORY_REVIEWER_QUEUE_LIFETIME_MS`] and never moves afterwards.
pub fn reserve_memory_reviewer_job_in_tx(
    conn: &GuardedConn<'_>,
    project: &str,
    producer: &ProducerBinding,
    inputs: &CausalInputs,
    ...
    if let Some(existing) = load_memory_reviewer_job(conn, project, &causal_identity)? {
...
fn check_reservation_headroom(
    ...
    if pending_jobs(conn, Some(project))?.saturating_add(count)
        > MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_PROJECT
    {
        return Err(refuse(MemoryReviewerJobRefusal::ProjectCapacity));
    }
    if pending_jobs(conn, None)?.saturating_add(count) > MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_HOST {
        return Err(refuse(MemoryReviewerJobRefusal::HostCapacity));
    }
    check_quota(
        conn,
        project,
...
fn insert_reservation_in_tx(
    ...
    causal_identity: &str,
    now_ms: i64,
) -> rusqlite::Result<MemoryReviewerJob> {
    let ordinal = producer.validate().map_err(refuse)?;
    let deadline = now_ms
        .checked_add(MEMORY_REVIEWER_QUEUE_LIFETIME_MS)
        .ok_or_else(|| refuse(MemoryReviewerJobRefusal::InvalidRequest))?;
    conn.execute(
        "INSERT INTO memory_reviewer_jobs (
        ...
        params![
            ...
            deadline,
            i64::try_from(MEMORY_REVIEWER_JOB_ALLOWANCE_BYTES).unwrap_or(i64::MAX),
            i64::try_from(MEMORY_REVIEWER_RECEIPT_CHARGE_BYTES).unwrap_or(i64::MAX),
            now_ms,
        ],
    )?;
    load_memory_reviewer_job(conn, project, causal_identity)?
        .ok_or_else(|| refuse(MemoryReviewerJobRefusal::Missing))
}
...
pub fn activate_memory_reviewer_job_in_tx(
    conn: &GuardedConn<'_>,
    project: &str,
    causal_identity: &str,
    producer: &ProducerBinding,
    ...
    match job.state {
        ...
        MemoryReviewerJobState::Ready(_) => {
            return Err(refuse(MemoryReviewerJobRefusal::NotReserved));
        }
        MemoryReviewerJobState::Terminal(_) => {
            return Err(refuse(MemoryReviewerJobRefusal::Terminal));
        }
    }
    if job.producer != *producer {
        return Err(refuse(MemoryReviewerJobRefusal::ProducerMismatch));
    }
    if job.queue_deadline_ms <= now_ms {
        return Err(refuse(MemoryReviewerJobRefusal::Expired));
    }
    if input.subject != job.target || input.question_template != job.question_template {
        return Err(refuse(MemoryReviewerJobRefusal::InvalidRequest));
    }
    conn.execute(
        "UPDATE memory_reviewer_jobs SET state = 'ready', input_json = ?3, updated_at_ms = ?4
...
pub fn rebind_reserved_memory_reviewer_job_in_tx(
    conn: &GuardedConn<'_>,
    project: &str,
    causal_identity: &str,
    producer: &ProducerBinding,
    ...
    conn.execute(
        "UPDATE memory_reviewer_jobs SET firing_id = ?3, updated_at_ms = ?4
         WHERE project = ?1 AND causal_identity = ?2 AND state = 'reserved'",
...
pub fn finish_memory_reviewer_job_in_tx(
    conn: &GuardedConn<'_>,
    project: &str,
    causal_identity: &str,
    outcome: MemoryReviewerJobOutcome,
    now_ms: i64,
) -> rusqlite::Result<MemoryReviewerJob> {
    ...
    if changed == 0 {
        return Err(refuse(match job.state {
            MemoryReviewerJobState::Terminal(_) => MemoryReviewerJobRefusal::Terminal,
            _ => MemoryReviewerJobRefusal::Expired,
        }));
    }
    Ok(job)
}
...
pub fn expire_reserved_memory_reviewer_job_in_tx(
    conn: &GuardedConn<'_>,
    project: &str,
    causal_identity: &str,
    now_ms: i64,
    ...
    if changed == 0 {
        return Err(refuse(match job.state {
            MemoryReviewerJobState::Terminal(_) => MemoryReviewerJobRefusal::Terminal,
            MemoryReviewerJobState::Ready(_) => MemoryReviewerJobRefusal::NotReserved,
            MemoryReviewerJobState::Reserved => MemoryReviewerJobRefusal::InvalidRequest,
        }));
    }
    Ok(job)
}

/// One fenced write behind [`MemoryStore::settle_memory_reviewer_reservation`]: the reservation and its retained publication go first, and the job it names closes in the same transaction, so no pass can observe a `Publishing` state without its reservation while the job is still open. A job that is no longer `Reserved` was closed by the sweep or activated by a publication that also cleared this reservation, so it is left alone.
pub(crate) fn settle_memory_reviewer_reservation(
    store: &MemoryStore,
    session_id: &str,
    expected: &crate::MemoryReviewerReservation,
    project: &str,
    now_ms: i64,
) -> Result<bool, MemoryReviewerJobError> {
    store.memory_reviewer_transaction(
        project,
        "settle",
        |_| Ok(()),
        |conn| {
            if !crate::clear_memory_reviewer_reservation_tx(conn, session_id, expected)? {
                return Ok(WriteDisposition::Replay(false));
            }
            let reserved = load_memory_reviewer_job(conn, project, &expected.causal_identity)?
                .is_some_and(|job| job.state == MemoryReviewerJobState::Reserved);
            if reserved {
                if expected.queue_deadline_ms <= now_ms {
                    expire_reserved_memory_reviewer_job_in_tx(
                        conn,
                        project,
                    ...
                    finish_memory_reviewer_job_in_tx(
                        conn,
                        project,
                        &expected.causal_identity,
...
pub fn freeze_selection_in_tx(
    ...
    let page_json = serde_json::to_string(&page)
        .map_err(|_| refuse(MemoryReviewerJobRefusal::InvalidRequest))?;
    if page_json.len() > MAX_FROZEN_PAGE_BYTES {
        return Err(refuse(MemoryReviewerJobRefusal::PageTooLarge));
    }
        ...
        return match &existing.page {
            Some(frozen) if existing.state == FrozenSelectionState::Frozen => {
                if *frozen == page {
                    ...
                    Err(refuse(MemoryReviewerJobRefusal::InvalidRequest))
                }
            }
            _ => Err(refuse(MemoryReviewerJobRefusal::Terminal)),
        };
    }
    if frozen_pages(conn, Some(project))? >= MAX_FROZEN_SELECTIONS_PER_PROJECT {
        return Err(refuse(MemoryReviewerJobRefusal::ProjectSelectionCapacity));
    }
    if frozen_pages(conn, None)? >= MAX_FROZEN_SELECTIONS_PER_HOST {
        return Err(refuse(MemoryReviewerJobRefusal::HostSelectionCapacity));
    }
    check_quota(
        conn,
        project,
...
fn retire_frozen_page_in_tx(
    ...
    conn.execute(
        "UPDATE memory_reviewer_frozen_selections
...
fn frozen_page_owner_key(slot_id: &str, selection_attempt: &str) -> String {
    active_scan_owner_key(&["memory_reviewer", "freeze", slot_id, selection_attempt])
}
...
fn release_frozen_page_scans(
    ...
    retire_active_scan_domain_owner(
        ...
        "project",
        project,
        DurableWriteFamily::MemoryReviewerJobs.owner_kind(),
...
pub enum EnqueueOutcome {
    ...
    Deferred(MemoryReviewerJobRefusal),
...
pub fn advance_selection_cursor_in_tx(
    ...
    conn.execute(
        "INSERT INTO memory_reviewer_selection_cursors (project, slot_id, cursor, updated_at_ms)
         VALUES (?1, ?2, ?3, ?4)
...
pub fn enqueue_frozen_selection_in_tx(
    ...
    for inputs in &page.references {
        let inputs = inputs.clone().normalized().map_err(refuse)?;
        let causal_identity = inputs.causal_identity().map_err(refuse)?;
        if load_memory_reviewer_job(conn, project, &causal_identity)?.is_some()
            || fresh
                .iter()
            ...
            check_reservation_headroom(conn, project, fresh.len(), FROZEN_SELECTION_ALLOWANCE_BYTES)
    {
        return match refusal_of(&error) {
            Some(
                reason @ (MemoryReviewerJobRefusal::ProjectCapacity
    ...
    for (inputs, causal_identity) in &fresh {
        insert_reservation_in_tx(conn, project, producer, inputs, causal_identity, now_ms)?;
        activate_memory_reviewer_job_in_tx(
            conn,
            project,
            causal_identity,
            producer,
...
fn load_selection(
    ...
    conn.query_row(
        "SELECT page_json, state, selection_deadline_ms, created_at_ms FROM memory_reviewer_frozen_selections
...
/// The prepared write every MemoryReviewer family operation runs under: the project is bounded and scanned as an existing identity, and the scan audit is owned by `owner_parts`.
pub(crate) fn memory_reviewer_write(
    project: &str,
    owner_parts: &[&str],
) -> Result<PreparedWrite, MemoryStoreError> {
    check_project(project)?;
    let mut write = PreparedWrite::new(DurableWriteFamily::MemoryReviewerJobs);
...
pub(crate) fn store_incarnation_in_tx(conn: &GuardedConn<'_>) -> rusqlite::Result<String> {
    conn.query_row(
        "SELECT database_incarnation_id FROM memory_reviewer_store_identity WHERE id = 0",
        [],
        |row| row.get(0),
    )
}
...
impl MemoryStore {
    ...
    pub fn memory_reviewer_store_incarnation(&self) -> Result<String, MemoryStoreError> {
        self.inner
    ...
    pub(crate) fn ensure_memory_reviewer_store_identity(
        &self,
        now_ms: i64,
    ) -> Result<(), MemoryStoreError> {
        let present: bool = self.inner.with_conn(|conn| {
            conn.query_row(
                "SELECT EXISTS(SELECT 1 FROM memory_reviewer_store_identity WHERE id = 0)",
                [],
                |row| row.get(0),
            )
        })?;
        ...
        self.inner.with_conn_fenced(|conn| {
            conn.execute(
                "INSERT INTO memory_reviewer_store_identity (id, database_incarnation_id, created_at_ms)
                 SELECT 0, lower(hex(randomblob(16))), ?1
                 WHERE NOT EXISTS (SELECT 1 FROM memory_reviewer_store_identity WHERE id = 0)",
                [now_ms],
            )?;
            Ok(())
        })?;
    ...
    fn memory_reviewer_transaction<T>(
        &self,
        project: &str,
        operation: &str,
    ...
    ) -> Result<T, MemoryReviewerJobError> {
        let mut write = memory_reviewer_write(project, &["memory_reviewer", operation])?;
        prepare(&mut write)?;
        ...
        match (result, refusal.get()) {
            (Err(_), Some(refusal)) => Err(MemoryReviewerJobError::Refused(refusal)),
            (Err(error), None) => Err(MemoryReviewerJobError::Store(error)),
            (Ok(value), _) => Ok(value),
        }
    }

    pub fn reserve_memory_reviewer_job(
        &self,
        project: &str,
        producer: &ProducerBinding,
        inputs: &CausalInputs,
        now_ms: i64,
    ) -> Result<ReserveOutcome, MemoryReviewerJobError> {
        self.memory_reviewer_transaction(
            project,
            "reserve",
            |write| {
            ...
            |conn| {
                let outcome =
                    reserve_memory_reviewer_job_in_tx(conn, project, producer, inputs, now_ms)?;
                Ok(match outcome {
    ...
    pub fn activate_memory_reviewer_job(
        &self,
        project: &str,
        causal_identity: &str,
        producer: &ProducerBinding,
        input: &MemoryReviewerJobInput,
        now_ms: i64,
    ) -> Result<MemoryReviewerJob, MemoryReviewerJobError> {
        self.memory_reviewer_transaction(
            project,
            "activate",
            |write| {
                let MemoryReviewerJobInput {
                    subject,
                    starting_references,
                    question_template,
                } = input;
    ...
    pub fn rebind_reserved_memory_reviewer_job(
        &self,
        project: &str,
        causal_identity: &str,
        producer: &ProducerBinding,
        ...
        self.memory_reviewer_transaction(
            ...
            |conn| {
                rebind_reserved_memory_reviewer_job_in_tx(
                    conn,
                    project,
                    causal_identity,
                    producer,
    ...
    pub fn finish_memory_reviewer_job(
        &self,
        project: &str,
        causal_identity: &str,
        outcome: MemoryReviewerJobOutcome,
        ...
        if matches!(
            outcome,
            MemoryReviewerJobOutcome::Completed | MemoryReviewerJobOutcome::Abstained
        ) {
            return Err(MemoryReviewerJobError::Refused(
        ...
        self.memory_reviewer_transaction(
            ...
            |conn| {
                let job =
                    finish_memory_reviewer_job_in_tx(conn, project, causal_identity, outcome, now_ms)?;
                ...
                let job_id: i64 = conn.query_row(
                    "SELECT job_id FROM memory_reviewer_jobs WHERE project = ?1 AND causal_identity = ?2",
                    params![project, causal_identity],
                    |row| row.get(0),
                )?;
                crate::task_lease::fence_task_claims_tx(
                    conn,
                    &crate::memory_reviewer_ledger::MEMORY_REVIEWER_REVIEW_TASK,
                    project,
                    job_id,
                    "stale",
                    now_ms,
    ...
    pub fn expire_memory_reviewer_work(
        &self,
        now_ms: i64,
    ) -> Result<(usize, usize), MemoryReviewerJobError> {
        ...
        write
            .execute(&self.inner, |coordinated| {
                    ...
                    "UPDATE memory_reviewer_receipts
                        SET state = 'complete', updated_at_ms = ?1,
                            terminal_kind = CASE
                              ...
                              WHEN EXISTS(
                                SELECT 1 FROM memory_reviewer_attempts a
                                 WHERE a.project = memory_reviewer_receipts.project
                        ...
                        SET state = 'terminal', allowance_bytes = 0, input_json = NULL, updated_at_ms = ?1,
                            outcome = COALESCE((
                                SELECT CASE r.terminal_kind
                                         ...
                                         ELSE 'expired' END
                                  FROM memory_reviewer_receipts r
                                 WHERE r.project = memory_reviewer_jobs.project
                                   AND r.causal_identity = memory_reviewer_jobs.causal_identity), 'expired')
                      WHERE state IN ('reserved', 'ready')
                        AND (queue_deadline_ms <= ?1
                             OR EXISTS(SELECT 1 FROM memory_reviewer_receipts r
                                        WHERE r.project = memory_reviewer_jobs.project
                                          AND r.causal_identity = memory_reviewer_jobs.causal_identity
                                          AND r.state = 'complete'))
                        AND NOT EXISTS(SELECT 1 FROM memory_reviewer_receipts r
                                        WHERE r.project = memory_reviewer_jobs.project
                                          AND r.causal_identity = memory_reviewer_jobs.causal_identity
                ...
                let expiring_pages: Vec<(String, String, String)> = coordinated
                    .tx()
                    .prepare(
                        "SELECT project, slot_id, selection_attempt FROM memory_reviewer_frozen_selections
                ...
                coordinated.tx().execute(
                    ...
                    params![
                        ...
                        crate::memory_reviewer_ledger::MEMORY_REVIEWER_REVIEW_TASK.task_kind
                    ],
                )?;
                Ok(if jobs + pages == 0 {
            ...
            .map_err(MemoryReviewerJobError::Store)
    }
    ...
    pub fn freeze_selection(
        ...
        page: &FrozenSelectionPage,
        now_ms: i64,
    ) -> Result<FrozenSelection, MemoryReviewerJobError> {
        self.memory_reviewer_transaction(
            project,
            "freeze",
            |write| {
    ...
    pub fn complete_frozen_selection(
        ...
        if state == FrozenSelectionState::Enqueued {
            return Err(MemoryReviewerJobRefusal::InvalidRequest.into());
        }
        self.memory_reviewer_transaction(
            project,
            "complete-selection",
    ...
    pub fn enqueue_frozen_selection(
        ...
        producer: &ProducerBinding,
        now_ms: i64,
    ) -> Result<EnqueueOutcome, MemoryReviewerJobError> {
        ...
        let outcome = self.complete_task_lease(
            ...
            |coordinated, _claim| {
                ...
                let code = match &enqueue {
                    EnqueueOutcome::Enqueued { .. } => "memory_reviewer_selection_enqueued",
                    EnqueueOutcome::Expired => "memory_reviewer_selection_expired",
                ...
                Ok(crate::task_lease::LeaseCompletion::Applied {
                    response_json: serde_json::json!({"ok": code == "memory_reviewer_selection_enqueued", "code": code})
                        .to_string(),
                })
            },
        );
        match (outcome, applied.take()) {
            ...
            (Ok(crate::LeaseCompleteOutcome::Conflict { kind }), _) => {
                Err(MemoryReviewerJobError::Refused(if kind == "invalid" {
                    MemoryReviewerJobRefusal::InvalidRequest
                } else {
                    MemoryReviewerJobRefusal::SlotConflict
                }))
            }
            (Err(_), Some(deferred @ EnqueueOutcome::Deferred(_))) => Ok(deferred),
            (Err(error), _) => Err(MemoryReviewerJobError::Store(error)),
            (Ok(crate::LeaseCompleteOutcome::Applied { .. }), None) => Err(
                MemoryReviewerJobError::Refused(MemoryReviewerJobRefusal::InvalidRequest),
            ),
        }
    }
        ...
        self.inner
            .with_conn(|conn| {
                let key: Option<(String, String)> = conn
                    .query_row(
                        "SELECT slot_id, selection_attempt FROM memory_reviewer_frozen_selections
    ...
    pub fn selection_cursor(
        ...
        self.inner
            .with_conn(|conn| {
                conn.query_row(
                    "SELECT cursor FROM memory_reviewer_selection_cursors WHERE project = ?1 AND slot_id = ?2",
    ...
    pub fn lookup_memory_reviewer_job(
        &self,
        project: &str,
        causal_identity: &str,
    ) -> Result<Option<MemoryReviewerJob>, MemoryStoreError> {
        check_project(project)?;
        self.inner
            .with_conn(|conn| load_memory_reviewer_job(conn, project, causal_identity))
            .map_err(Into::into)
    }
    ...
    pub fn ready_memory_reviewer_jobs(
        &self,
        project: &str,
        limit: usize,
        now_ms: i64,
    ) -> Result<Vec<MemoryReviewerJob>, MemoryStoreError> {
        check_project(project)?;
        let limit =
            i64::try_from(limit.min(MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_PROJECT)).unwrap_or(0);
        self.inner
            .with_conn(|conn| {
                let mut statement = conn.prepare_cached(&format!(
                    "SELECT {JOB_COLUMNS} FROM memory_reviewer_jobs
    ...
    pub fn memory_reviewer_headroom(
        &self,
        project: &str,
    ) -> Result<MemoryReviewerHeadroom, MemoryStoreError> {
        check_project(project)?;
        self.inner
            .with_conn(|conn| {
                let project_bytes = metadata_bytes(conn, Some(project))?;
                let host_bytes = metadata_bytes(conn, None)?;
                Ok(MemoryReviewerHeadroom {
                    ...
                    project_metadata_remaining: MAX_MEMORY_REVIEWER_METADATA_BYTES_PER_PROJECT
                        .saturating_sub(project_bytes),
                    host_metadata_remaining: MAX_MEMORY_REVIEWER_METADATA_BYTES_PER_HOST
