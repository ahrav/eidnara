...
import {
    type ConditionalNoteResolver,
    createConditionalNoteRequestAgent,
    createPinnedLookup,
    guardedConditionalNoteHttpGet,
    requestValidatedAddress,
    validateConditionalNoteHttpUrl,
} from "./ssrf-guard";
import { ConditionalNoteNetworkError } from "./types";

const signal = new AbortController().signal;

function resolver(rows: Array<{ address: string; family: 4 | 6 }>): ConditionalNoteResolver {
    return { lookup: async () => rows };
}

describe("conditional-note SSRF guard", () => {
    test("requires https", async () => {
        await expect(
            validateConditionalNoteHttpUrl("http://example.com", { signal }),
        ).rejects.toThrow(/https/i);
        await expect(
            validateConditionalNoteHttpUrl("file:///etc/passwd", { signal }),
        ).rejects.toThrow();
    });
    ...
    test("blocks alternate IPv4 loopback encodings canonicalized by URL", async () => {
        for (const host of ["127.1", "0177.0.0.1", "0x7f.0.0.1", "2130706433"]) {
            await expect(
                validateConditionalNoteHttpUrl(`https://${host}/`, { signal }),
    ...
    test("blocks private, link-local, metadata, CGNAT, multicast, and documentation IPv4", async () => {
        for (const address of [
            ...
            await expect(
                validateConditionalNoteHttpUrl(`https://${address}/`, { signal }),
            ).rejects.toThrow(/non-global|internal/i);
        }
    });
    ...
    test("rejects every IPv6 DNS answer before address classification", async () => {
        for (const address of [
            ...
            await expect(
                validateConditionalNoteHttpUrl("https://ipv6-only.example.test/", {
                    signal,
                    resolver: resolver([{ address, family: 6 }]),
                }),
            ).rejects.toBeInstanceOf(ConditionalNoteNetworkError);
        }
    });

    test("allows a dual-stack host and pins only its public IPv4 answer", async () => {
        const validated = await validateConditionalNoteHttpUrl("https://dual-stack.example.test/", {
            signal,
            resolver: resolver([
    ...
    test("rejects DNS answers with any private IPv4 address", async () => {
        await expect(
            validateConditionalNoteHttpUrl("https://example.test/", {
                signal,
                resolver: resolver([
                    { address: "93.184.216.34", family: 4 },
    ...
    test("allows public IPv4 DNS answers and preserves all validated candidates", async () => {
        const validated = await validateConditionalNoteHttpUrl("https://example.test/path", {
            signal,
            resolver: resolver([
                { address: "93.184.216.34", family: 4 },
    ...
    test("stops after a terminal per-target failure", async () => {
        ...
        const error = await guardedConditionalNoteHttpGet("https://example.test/", {
            signal,
            resolver: resolver([
                { address: "93.184.216.34", family: 4 },
                { address: "1.1.1.1", family: 4 },
        ...
        expect(error).toBeInstanceOf(ConditionalNoteNetworkError);
        expect((error as ConditionalNoteNetworkError).terminal).toBe(true);
    ...
    test("advances to the next address after a connection-level failure", async () => {
        ...
        const response = await guardedConditionalNoteHttpGet("https://example.test/", {
            signal,
            resolver: resolver([
                { address: "93.184.216.34", family: 4 },
                { address: "1.1.1.1", family: 4 },
...
describe("guarded HTTPS request agent", () => {
    test("does not use a pre-seeded keep-alive global agent", async () => {
        ...
        try {
            const dedicated = createConditionalNoteRequestAgent();
...
describe("guarded HTTPS request lifetime", () => {
    ...
    test("enforces a wall-clock deadline against a server that keeps the socket active", async () => {
        ...
        try {
            ...
            expect(error).toBeInstanceOf(ConditionalNoteNetworkError);
            expect((error as ConditionalNoteNetworkError).message).toMatch(/timed out/);
            expect((error as ConditionalNoteNetworkError).terminal).toBe(false);
    ...
    test("tries the next validated address after the first one stalls", async () => {
        const server = await listenDripping();
        try {
            const error = await guardedConditionalNoteHttpGet(
                `https://example.test:${server.port}/`,
                {
                    signal,
                    timeoutMs: 150,
                    resolver: resolver([
...
describe("default resolver abort listener hygiene", () => {
    test("does not accumulate abort listeners across lookups sharing one signal", async () => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            await validateConditionalNoteHttpUrl("https://localhost/", {
                signal: controller.signal,
            }).catch(() => undefined);
        }
        expect(getEventListeners(controller.signal, "abort")).toHaveLength(0);
