...
//! `ModelExecutionLimits::default` mirrors these constants one-to-one.
//! Production uses `ModelExecutionLimits` defaults; tests may lower limits through `ModelExecutionLimits`.
...
/// ModelExecution derives route-identity headroom from `MAX_BOUND_ROUTES` and enforces that limit at bind time.
/// ModelExecution enforces `MAX_BOUND_ROUTES` even when the host configures a larger `max_routes` value.
/// Raising the host's `max_routes` cannot increase ModelExecution's retained route identities beyond the declared reservation.
...
/// Ingress sizing around ModelExecution must use `DECLARED_RETAINED_RESIDENT_BYTES`, not `MAX_RETAINED_BYTES` alone.
...
/// The paired reservations prevent saturated ModelExecution work from consuming a general admission slot.
...
/// Bounding the probe keeps stalled sends from holding every reserved handler slot while cancel, delete, and status requests wait behind them.
...
/// `ModelExecutionLimits::default()` uses the fixed product-contract capacities.
///
/// `ModelExecutionLimits` lets deterministic tests exercise eviction, overflow, and saturation with smaller capacities.
#[derive(Debug, Clone)]
...
impl Default for ModelExecutionLimits {
    fn default() -> Self {
        Self {
