...
const CLI_PACKAGE_NAME = "@eidnara/cli";
...
async function checkEmbeddingConfig(
    ...
    if (!existsSync(eidnaraConfigPath)) {
        return checkLocalEmbeddingRuntimeForDoctor();
    }

    let rawText: string;
    try {
        rawText = readFileSync(eidnaraConfigPath, "utf-8");
    } catch {
        log.warn("Could not read eidnara.jsonc for embedding check");
        return { issues: 1 };
    }

    const substituted = substituteConfigVariables({
        text: rawText,
        configPath: eidnaraConfigPath,
    });
    ...
    try {
        ...
        log.warn(
            `Embedding check skipped — could not parse eidnara.jsonc: ${error instanceof Error ? error.message : String(error)}`,
        );
        return { issues: 1 };
    }
...
export async function runDoctor(
    ...
    const authorityDbPath = join(getEidnaraStorageDir(), "context.db");
    ...
    if (!npmLatest) {
        log.info(`Eidnara CLI v${selfVersion}; npm latest check unavailable`);
    } else if (compareVersions(selfVersion, npmLatest) < 0) {
        warn(`Eidnara CLI v${selfVersion} is older than npm latest v${npmLatest}`);
    } else {
        pass(`Eidnara CLI v${selfVersion} is current (npm latest v${npmLatest})`);
    }
    ...
    if (existsSync(paths.eidnaraConfig)) {
        try {
            const mcRaw = readFileSync(paths.eidnaraConfig, "utf-8");
            ...
            if (mcChanged) {
                writeFileAtomic(paths.eidnaraConfig, `${stringify(mcConfig, null, 2)}\n`);
            }
        } catch {
            log.warn("Could not migrate deprecated config keys in eidnara.jsonc");
        }
    }
    ...
    const embeddingCheck = await checkEmbeddingConfig(paths.eidnaraConfig);
