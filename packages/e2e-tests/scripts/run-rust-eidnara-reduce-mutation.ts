...
const source = resolve(e2eRoot, "tests/rust-eidnara-reduce-roundtrip.test.ts");
const command =
    "EIDNARA_E2E_MODE=rust bun test --timeout 600000 --max-concurrency=1 tests/rust-eidnara-reduce-roundtrip.test.ts";
...
if (occurrences !== 1) {
    throw new Error(
        `EIDNARA_REDUCE_UNKNOWN_TARGET: expected one mutation target, found ${occurrences}`,
    );
}
...
if (observedFailure.exit_status === 0) {
    throw new Error(
        "EIDNARA_REDUCE_UNKNOWN_TARGET: mutation did not redden the queued-ledger assertion",
    );
}
...
if (
    !observedFailure.output.includes(
        "error: queued eidnara_reduce drop must be pending before the bust",
    )
) {
    throw new Error(
        "EIDNARA_REDUCE_UNKNOWN_TARGET: failure did not reach the queued-ledger assertion",
    );
}
if (revertedRerun.exit_status !== 0) {
    throw new Error("EIDNARA_REDUCE_UNKNOWN_TARGET: reverted eidnara_reduce drill did not pass");
}
...
const record = {
    ...
    mutations: [
        {
            name: "EIDNARA_REDUCE_UNKNOWN_TARGET",
            applied_diff: {
...
writeFileSync(
    resolve(e2eRoot, "mutations/rust-eidnara-reduce-roundtrip.json"),
    `${JSON.stringify(record, null, 2)}\n`,
);
console.log("wrote mutations/rust-eidnara-reduce-roundtrip.json");
