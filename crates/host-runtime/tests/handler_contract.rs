//! `Handler` must use `host-runtime` types rather than the private predecessor SDK.
...
use support::{BindPolicy, CompositeTestHost, Event, LINKED_MODULE_ID, TestHost, mode_body};
...
fn limit_defaults_are_finite_and_validated() {
    ...
    assert!(
        defaults.max_resident_bytes >= host_runtime::config::MIN_RESIDENT_BYTES,
...
async fn a_composite_sizes_the_resident_cap_from_its_own_declarations() {
    ...
    assert_eq!(
        ...
        host_runtime::config::MIN_RESIDENT_BYTES + u64::from(host_runtime::MAX_FRAME_BODY_LEN),
