    ...
    return (
        ...
        >
            ...
            <box flexDirection="row" justifyContent="space-between" width="100%">
                {compactionOff() ? (
                    ...
                    >
                        ...
                        {formatThresholdPercent(s().executeThreshold)}%
            ...
            <box flexDirection="column">
                ...
                <text fg={t().textMuted}>
                    * Conversation includes Reasoning; hygiene excludes it
                </text>
            ...
            {!compactionOff() &&
                ...
                (() => {
                    ...
                    const verb =
                        p.kind === "upgrade" ? "Upgrade" : p.kind === "embed" ? "Embed" : "Recomp";
                    return (
                        <box marginTop={1} width="100%" flexDirection="column">
                            ...
                            {(() => {
                                if (p.phase === "recomp") {
                                    const frac =
                                        p.totalMessages > 0
                                            ? p.processedMessages / p.totalMessages
                                            : 0;
                                    const width = 24;
                                    const filled = Math.round(
                                        Math.max(0, Math.min(1, frac)) * width,
                                    );
                                    const bar =
                                        p.totalMessages > 0
                                ...
                                if (p.phase === "migration")
                                    return (
                                        <R
                                            t={t()}
                                            l="Status"
                                            v={p.note ?? "Migrating memories ⟳"}
                                            fg={t().warning}
                                        />
                                    );
                                if (p.phase === "done")
                                ...
                                if (p.phase === "skipped")
                                    return (
                                        <R
                                            t={t()}
                                            l="Status"
                                            v={p.message ?? `${verb} stopped early`}
                                            fg={t().textMuted}
                                        />
                                    );
                                return (
                                    <R
                                        t={t()}
                                        l="Status"
                                        v={`✗ ${verb} failed${p.message ? `: ${p.message}` : ""}`}
                                        fg={t().error}
                                    />
                                );
                            })()}
                    ...
                    <>
                        <box flexDirection="column" flexGrow={1} flexBasis={0}>
                            ...
                            <R
                                t={t()}
                                l="Active"
                                v={`${s().activeTags} (~${fmtBytes(s().activeBytes)})`}
                            />
                            ...
                            <R
                                t={t()}
                                l="Drops"
                                v={String(s().pendingOpsCount)}
                                fg={s().pendingOpsCount > 0 ? t().warning : t().textMuted}
                            />
                            <box marginTop={1}>
                                <text fg={t().text}>
                                    <b>Cache TTL</b>
                                ...
                                v={
                                    s().cacheExpired
                                        ...
                                        : s().cacheNeverExpires
                                          ...
                                          : `${Math.round(s().cacheRemainingMs / 1000)}s`
                                }
                                fg={s().cacheExpired ? t().warning : t().textMuted}
                            />
                            <R
                                t={t()}
                                l="Auto-execute"
                                v={
                                    s().cacheExpired
                                        ? "yes (expired)"
                                        : s().cacheNeverExpires
                                          ? `at ≥${formatThresholdPercent(s().executeThreshold)}%`
                                          : `at TTL or ≥${formatThresholdPercent(s().executeThreshold)}%`
                                }
                                fg={t().textMuted}
                            />
                            <box marginTop={1}>
                        ...
                        <box flexDirection="column" flexGrow={1} flexBasis={0}>
                            ...
                            <R
                                t={t()}
                                l="Execute threshold"
                                v={`${formatThresholdPercent(s().executeThreshold)}%${s().executeThresholdClamped ? "*" : ""}`}
                            />
                            <R
                                t={t()}
                                l="Last reduce anchor"
                                v={`${fmt(s().lastNudgeTokens)} tok`}
                            />
                            <box marginTop={1}>
                                <text fg={t().text}>
                                    <b>Context Details</b>
                            ...
                            <R
                                t={t()}
                                l="Protected tags"
                                v={String(s().protectedTagCount)}
                                fg={t().textMuted}
                            />
                            <R
                                t={t()}
                                l="Subagent"
                                v={s().isSubagent ? "yes" : "no"}
                                fg={t().textMuted}
                            />
                            <box marginTop={1}>
                                <text fg={t().text}>
                            ...
                            {typeof s().boundaryPresent === "boolean" && (
                                <R
                                    t={t()}
                                    l="Boundary"
                                    v={s().boundaryPresent ? "present" : "absent"}
                                />
                            )}
                            {s().coverageOrdinal !== undefined && (
                                <R
                                    ...
                                    l="Coverage ordinal"
                                    v={
                                        s().coverageOrdinal == null
                                            ? "none"
                                            : String(s().coverageOrdinal)
                                    }
                                />
                            )}
                            {typeof s().boundaryPresent === "boolean" && (
                            ...
                            <R
                                t={t()}
                                l="History block"
                                v={`~${fmt(s().historyBlockTokens)} tok`}
                            />
            ...
            <box marginTop={1} width="100%">
                ...
                <R
                    ...
                    fg={
                        (s().loggerDiagnostics?.swallowedWriteCount ?? 0) > 0
                            ? t().error
                            : t().textMuted
                    }
                />
...
function showResultDialog(api: TuiPluginApi, title: string, message: string): Promise<boolean> {
    return new Promise((resolve) => {
        api.ui.dialog.replace(
            () => <api.ui.DialogAlert title={title} message={message} onConfirm={() => {}} />,
            () => resolve(true),
        );
    });
}
