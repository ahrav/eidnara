import type { TagEntry } from "@eidnara/opencode/features/context/types";
import { estimateImageTokensFromDataUrl } from "@eidnara/opencode/hooks/context/image-token-estimate";
import {
    contentSignature,
    memoizedTokens,
    partHash,
    ...
    sameMeasuredPrefix,
    TAG_PREFIX,
} from "@eidnara/opencode/hooks/context/tail-hygiene-memo";
import {
...
function parseVisibleTag(
    ...
    return Number.isSafeInteger(tagNumber) ? tagsByNumber.get(tagNumber) : undefined;
}

function isDropSentinel(content: string): boolean {
    const head = content.trimStart().replace(TAG_PREFIX, "").trimStart().toLowerCase();
...
function imageContentAndTokens(part: Record<string, unknown>): {
    ...
    const content = data.startsWith("data:") ? data : `data:${mimeType};base64,${data}`;
    return {
        content,
...
function messageIdForTag(tag: TagEntry): string | null {
    ...
    const contentDerived = tag.messageId.indexOf(":eidnara-text-v1:");
...
function neighborhoodConsistent(input: {
    ...
    for (let index = input.messageIndex + 1; index < input.messageCount; index += 1) {
        const bound = input.boundsByMessageIndex.get(index);
    ...
    for (let messageIndex = 0; messageIndex < input.messages.length; messageIndex += 1) {
        ...
        if (role === "assistant" && Array.isArray(raw.content)) {
            for (let partIndex = 0; partIndex < raw.content.length; partIndex += 1) {
                ...
                const ownerId = messageIds[messageIndex] ?? `pi-message:${messageIndex}`;
                const arc: PiToolArc = {
                    ...
                    sentinel:
                        ...
                        typeof part.arguments.__eidnara_dropped__ === "string",
                    parts: [part],
                };
                arcs.push(arc);
        ...
        if (typeof raw.toolCallId !== "string" || raw.toolCallId.length === 0) continue;
        const pending = pendingByCall.get(raw.toolCallId);
        const matched = pending?.pop();
        const ownerId =
            matched?.ownerId ?? messageIds[messageIndex] ?? `pi-message:${messageIndex}`;
...
function attributeTags(
    ...
    for (const tag of input.tags) {
        ...
        const messageIndex = ownerId === null ? undefined : messageIndexById.get(ownerId);
    ...
    for (const arc of arcs) {
        ...
        const byOwner = orphanCandidates.get(arc.callId) ?? new Map<string, PiToolArc[]>();
...
function finalizeParts(
    ...
    for (const part of drafts) {
        if (part.kind !== "excluded" && part.tag) visibleTags.set(part.tag.tagNumber, part.tag);
    }
    ...
    const parts = drafts.map((draft): TailHygienePartMeasurement => {
        const protectedPart = draft.tag ? protectedNumbers.has(draft.tag.tagNumber) : false;
        const tokens = Math.max(0, draft.tokens);
        const uTokens = draft.tag && !protectedPart && draft.kind !== "excluded" ? tokens : 0;
        t += tokens;
        u += uTokens;
        return {
            key: draft.key,
...
export function measurePiTailHygiene(input: PiTailHygieneWalkInput): TailHygieneMeasurement {
    ...
    for (let messageIndex = 0; messageIndex < input.messages.length; messageIndex += 1) {
        ...
        if (typeof raw.content === "string") {
            ...
            const tag =
                parseVisibleTag(content, tagsByNumber) ?? messageTags.get(`${messageKey}:p0`);
        ...
        if (!Array.isArray(raw.content)) {
            drafts.push(excludedDraft(`${messageKey}\0excluded-content`, raw.content));
            continue;
        }
        let textOrdinal = 0;
        for (let partIndex = 0; partIndex < raw.content.length; partIndex += 1) {
            ...
            if (part.type === "image") {
                ...
                const tag = arc?.tag ?? messageTags.get(`${messageKey}:file${partIndex}`);
...
export function measurePiToolResultDelta(content: readonly unknown[]): number {
    ...
    for (const part of content) {
        ...
        if (part.type === "text" && typeof part.text === "string") {
            ...
            if (text && !isDropSentinel(text)) tokens += memoizedTokens("toolOutput", text);
...
export function assertPiTailHygieneContentUnchanged(
    ...
    if (current.contentSignature !== input.expectedSignature) {
        throw new Error("Pi tail hygiene walk was not the last byte-affecting operation");
    }
}
