...
use crate::control::{CODE_INVALID_CONTROL_REQUEST, ControlAction, parse_control};
use crate::dispatch::{
...
use crate::wire::{FrameId, encode_owned_frame, pure_header_flags};
...
pub async fn run_connection<H: HostHandler>(
    ...
    let generation = new_generation(&shared, root.clone(), read_cancel.clone(), sender);
    let peer_gen = Arc::clone(&generation);
    ...
    serve_generation(&shared, generation, receiver, io_task).await;
...
fn new_generation<H: HostHandler>(
    shared: &Arc<HostShared<H>>,
...
async fn serve_generation<H: HostHandler>(
    shared: &Arc<HostShared<H>>,
    generation: Arc<GenerationCore>,
    channel: ShmReceiver,
    mut io_task: AbortOnDropHandle<()>,
) {
    // Retained past `generation`: the writer must be told to stop even when a
    // handler still holds a sender clone through a retained RequestCtx.
    let writer_finish = generation.writer.clone();
    ...
    let read_task = generation
        .read_tasks
        .track_future(read_loop(shared, &generation, channel));
    {
        ...
        if shared.draining.load(Ordering::SeqCst) || shared.shutdown.is_cancelled() {
            discard_unregistered_generation(&generation);
            return;
        }
        connections.insert(generation.id, Arc::clone(&generation));
    }
    if let Some(policy) = shared.liveness.clone() {
        ...
        let stop = generation.read_cancel.child_token();
        let gen_ping = Arc::clone(&generation);
        shared.spawn_tracked(generation.read_tasks.track_future(liveness_loop(
            gen_ping,
            policy,
            stop.clone(),
        )));
    ...
    let begun_closes = shared.registry.begin_close_generation(generation.id);
    generation.read_tasks.close();
    generation.read_tasks.wait().await;
    if shared.draining.load(Ordering::SeqCst) {
        generation.shutdown_complete.cancelled().await;
    }

    close_generation(shared, &generation, begun_closes).await;
...
fn discard_unregistered_generation(generation: &GenerationCore) {
    generation.read_cancel.cancel();
    generation.token.cancel();
    generation.writer.discard();
}
    ...
    loop {
        ...
        match event {
            InboundEvent::Rejected(RejectedFrame { corr }) => {
                ...
                let Ok(reject_permit) = generation.busy_rejects.clone().try_acquire_owned() else {
                    generation.token.cancel();
                    generation.writer.discard();
                    return ReadExit::Peer;
                };
            ...
            InboundEvent::Frame(frame) => {
                ...
                match header.ty {
                    ...
                    FrameType::Cancel => {
                        ...
                        handle_cancel(generation, (header.channel, header.epoch, header.corr));
                    }
                    FrameType::Pong => {
                        ...
                        let now = Instant::now();
                        let mut pings = generation.pings.lock().expect("pings lock");
                    ...
                    FrameType::Goodbye => {
                        ...
                        let decision = shared.registry.begin_close_owned(handle, generation.id);
...
async fn handle_control<H: HostHandler>(
    ...
    match action {
        ...
        ControlAction::RouteOpen { target, identity } => {
            ...
            let shared_task = Arc::clone(shared);
            let gen_task = Arc::clone(generation);
            shared.spawn_lifecycle(generation.read_tasks.track_future(async move {
                let _pending_permit = pending_permit;
...
async fn emit_catalog_response(
    ...
    let frame = reserve_catalog_frame(budget, generation, deadline, id, body).await?;
    generation
        .writer
...
async fn liveness_loop(
    ...
    loop {
        ...
        let wake_at = {
            let pings = generation.pings.lock().expect("pings lock");
            pings
                .values()
        ...
        next_ping_at = now + policy.ping_interval;
        let corr = generation.next_ping_corr.fetch_add(1, Ordering::SeqCst);
        let flags = pure_header_flags();
        // `generation.pings` must contain the correlation before the Ping reaches the wire; otherwise the read loop drops a fast Pong as unmatched.
        generation.pings.lock().expect("pings lock").insert(
            corr,
            PingProbe {
                flags: flags.0,
                sent: Instant::now(),
        ...
        let gen_probe = Arc::clone(&generation);
        ...
        let send = generation.writer.send(crate::frame_channel::OutboundFrame {
            bytes,
            tail: Vec::new(),
            direct: None,
...
mod tests {
    ...
    async fn shutdown_registration_rejection_leaves_no_graceful_drain_work() {
        ...
        assert!(generation.read_cancel.is_cancelled());
        assert!(generation.token.is_cancelled());
        assert!(queue.discard.is_cancelled());
        generation.read_tasks.close();
        tokio::time::timeout(Duration::from_millis(10), generation.read_tasks.wait())
            .await
