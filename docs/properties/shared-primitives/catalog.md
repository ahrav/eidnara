...
and `cache-stability`. The record contract is [`../METHOD.md`](../METHOD.md).
...
- The PostgreSQL backend in the shared-crate source repository at `89abb40` is not carried.
  ...
  this tree. No citation names a path or line range in the source.
...
Confidence: high - [evidence](evidence/distinct-lease-keys-do-not-alias.md). High that the separator is rejected in every field (`LeaseKey::identity` (`crates/lease/src/lib.rs:617-643`)); high that collision handling is absent; low on practical targeted-FNV cost.
Existing check: `distinct_identity_axes_do_not_conflict` covers distinct scope, module, and backend axes; `separator_in_a_key_field_fails_closed_instead_of_aliasing` (`crates/lease/src/lib.rs:2168-2191`) covers the separator in each field; status **unaudited**.
...
- Windows acquisition rejects a reparse point on the lease file and on every component of the lease directory's path, and denies delete sharing on held lease handles, but does not evaluate directory ACLs and has no Unix owner-only mode semantics. The public `protect_file` remains a no-op on Windows. `(needs human input)`
...
- Which deployment actors can replace lease or SQLite/WAL/SHM paths during hardening? Lease acquisition retains its descriptor, refuses a lease directory that is group- or world-writable or owned by another user, refuses an ancestor that another principal could rename in unless it is sticky, refuses a symlink on the path that another user owns or that sits in a directory others can write to, and checks after locking (on Unix by inode, on Windows by volume and file index) that the path still names the locked inode; a principal with rename rights in the directory or a non-sticky ancestor is outside the contract. Public `protect_file` intentionally avoids opening caller-owned files.
...
Existing check: `superseded_writer_is_fenced_out_after_handover` (`crates/storage/src/lib.rs:4167-4223`), and `superseded_writer_is_rejected_after_reopen` (source at `89abb40`); both use synthetic stale stores and remain **unaudited**.
...
Status: active
Exercised: partial - sibling databases in one directory, symlink aliases, and hardlink aliases are covered; same-database descriptor disagreement is not.
...
Fault/timing angle: The lease key includes the database file name and the root is the database parent, so sibling files with equal descriptors get distinct leases. Every open refuses a symlink anywhere in the database path (`SQLITE_OPEN_NOFOLLOW` plus `O_NOFOLLOW` at creation), so neither a directory alias nor a file alias reaches the bytes. A database file with more than one name is refused before any open, so a hardlink alias cannot reach the bytes either. One database opened with differing module or namespace values still splits into independent locks.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:73-100,913-1090`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:3194-3203`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:3205-3216`) proves sibling files in one directory do not contend; `symlinked_database_paths_are_refused_never_aliased` (`:2694-2761`) proves both a directory-symlink alias and a file-symlink alias are refused, with and without a live holder. `a_hard_linked_database_is_refused` (`:1714-1741`) proves a second name for the database inode is refused through either name. No test covers descriptor disagreement; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:913-1090,1595-1618`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return` (`crates/storage/src/lib.rs:2949-2964`), observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:2733-2769`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Type: safety
Reachability: default-production - applies to the SQLite backend; the PostgreSQL backend in the shared-crate source repository at `89abb40` is not carried.
Status: active
...
Existing check: backend tests cover fenced callbacks, fenced DDL, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:3400-3420`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:3422-3488`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:3490-3538`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:3540-3585`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (source at `89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (source at `89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (source at `89abb40`) and `a_regressed_positive_epoch_fails_closed` (source at `89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (source at `89abb40`), `a_suppressed_epoch_increment_is_rejected` (source at `89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (source at `89abb40`) protect the lease row and its issuance; `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:3587-3690`) refuses DML, triggers, indexes, views, and renames onto the `fence` and `format_marker` tables; and `a_fenced_callback_cannot_rewrite_the_format_marker` (`:3692-3748`) refuses an update of, or a rename onto, the marker row. The [durable consumer inventory](durable-consumer-inventory.md) records the consumer reads; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/defer-pass-replays-frozen-bytes-verbatim.md). `step_defer` (`crates/cache-stability/src/lib.rs:221-247`) never reads `rendered_units` and mutates no frozen unit; the run-boundary filter in `CoreState::step` (`crates/cache-stability/src/lib.rs:182-219`) is the only path that removes units on a defer. `defer_does_not_mutate_frozen_bytes_or_render` (`crates/cache-stability/src/lib.rs:369-390`), `defer_boundary_absent_keeps_bytes_and_sets_reconcile_pending` (`crates/cache-stability/src/lib.rs:431-449`), and `run_started_keeps_lineage_resets_episode` (`crates/cache-stability/src/lib.rs:692-714`) pin the three arms, and every golden vector compares `cached_prefix_bytes()` after each pass.
...
Confidence: high - [evidence](evidence/never-minted-boundary-is-not-reconcile-pending.md). The guard is one expression in `step_defer` (`crates/cache-stability/src/lib.rs:221-247`); `defer_on_never_minted_boundary_is_stable_not_reconcile_pending` (`crates/cache-stability/src/lib.rs:392-429`) covers both the vacuous and the non-vacuous arm.
Existing check: `defer_on_never_minted_boundary_is_stable_not_reconcile_pending`, `defer_boundary_absent_keeps_bytes_and_sets_reconcile_pending` (`crates/cache-stability/src/lib.rs:431-449`); audited at U2.
...
Status: active
Exercised: yes - a coverage-extending `Soft` after a revert, a coverage-extending `Soft` at a live anchor, and a `Soft` that is the first pass to see the anchor gone.
Guarantee: A `Soft` pass advances `boundary_id` only when the prior anchor is present and no reconcile is pending; it never clears `reconcile_pending`, and when the anchor is absent it sets `reconcile_pending` itself, so the loss is reported on the pass that finds it rather than on a later defer.
...
Confidence: high - [evidence](evidence/anchor-holds-while-reconcile-pending.md). The guard in `step_soft` (`crates/cache-stability/src/lib.rs:249-292`) is a let chain over both conditions; `soft_does_not_advance_anchor_while_reconcile_pending` (`crates/cache-stability/src/lib.rs:649-690`) and `coverage_extending_soft_advances_anchor_keeps_m0_frozen` (`crates/cache-stability/src/lib.rs:575-647`) pin both outcomes.
Existing check: `soft_does_not_advance_anchor_while_reconcile_pending`, `coverage_extending_soft_advances_anchor_keeps_m0_frozen`, `soft_with_an_absent_anchor_marks_reconcile_pending`; audited at U2.
...
Confidence: high - [evidence](evidence/frozen-unit-order-is-preserved.md). `apply_units` (`crates/cache-stability/src/lib.rs:325-337`) is a find-or-push loop; `soft_replaces_by_key_keeps_slot_appends_new` (`crates/cache-stability/src/lib.rs:538-573`) asserts the resulting key order and the untouched first unit.
...
Confidence: high - [evidence](evidence/episode-units-reset-at-run-boundary.md). `CoreState::step` (`crates/cache-stability/src/lib.rs:182-219`) retains by `durability_class` before dispatching any action; `run_started_keeps_lineage_resets_episode` (`crates/cache-stability/src/lib.rs:692-714`) and `cross_episode_lineage_reproduces_byte_identical` (`crates/cache-stability/tests/golden_vectors.rs:274-322`) cover both classes.
...
Confidence: high - [evidence](evidence/store-schema-identity-matches-the-baseline.md). `ExpectedIdentity::for_baseline` (`crates/storage/src/lib.rs:1335-1401`), `classify` (`:1462-1512`), and `apply` (`:1514-1546`) implement the contract inside `open_sqlite` (`:913-1018`); `fresh_file_matches_the_baseline_inventory` (`:3004-3068`) pins the fixture, and `a_file_with_foreign_objects_is_refused_without_mutation` (`:3129-3155`), `a_consumer_baseline_is_applied_once_and_verified_on_reopen` (`:3070-3110`), `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched` (`:3112-3127`), and `negative_database_fence_fails_closed` (`:4137-4165`) cover the refusals.
Existing check: `fresh_file_matches_the_baseline_inventory`, `a_file_with_foreign_objects_is_refused_without_mutation`, `a_consumer_baseline_is_applied_once_and_verified_on_reopen`, `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched`, `a_baseline_that_hooks_an_infrastructure_table_is_rejected`, `a_baseline_that_creates_temporary_objects_is_rejected`, `a_baseline_that_attaches_or_redefines_infrastructure_is_rejected`, `fenced_callbacks_cannot_change_the_schema_so_the_store_stays_reopenable`, `negative_database_fence_fails_closed`; audited at U2.
Impact: A foreign or half-formed file is written to as if it were this store, a renamed table or a deleted `fence` row makes the floor read zero and reissues a superseded epoch, or a baseline-planted `fence` row fences every real writer out.
Open questions: None.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:601-612`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:789-831`); `GuardedConn` (`crates/storage/src/lib.rs:332-511`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:3364-3387`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:3490-3538`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:3750-3770`) pin the three denials; the last pins `VACUUM` refused by the read transaction (`cannot VACUUM from within a transaction`) and `PRAGMA incremental_vacuum` denied by the authorizer.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:694-699`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:3400-3420`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:896-911`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:3389-3398`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:3422-3488`) observe the pinned values. Power loss itself is not injected.
...
Fault/timing angle: The database file is created owner-only before SQLite opens it and pre-existing files are narrowed after the identity check and before WAL setup and the fence write, so a refused foreign file keeps its mode; WAL and SHM files created later inherit the database mode.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:913-1018`) creates the database owner-only and calls `protect_file` on all three paths after `classify` accepts the file and before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:2819-2869`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:3272-3300`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:261-311`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:4108-4135`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:3347-3362`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:1659-1662`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:1620-1627`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:4137-4165`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:4245-4261`) cover both directions.
