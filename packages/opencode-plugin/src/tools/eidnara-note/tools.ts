...
import { EIDNARA_NOTE_DESCRIPTION } from "./constants";
import type { EidnaraNoteArgs } from "./types";
...
function noteAuthorityRefusal(
    ...
    return `Error: ${readiness} ${verb} REFUSED and ${outcome}; RESEND the same eidnara_note call (${preserved.join(", ")}) after authority is ready.${content}`;
}
...
const eidnaraNoteArgsSchema = tool.schema.object(eidnaraNoteArgsShape).passthrough();

function createEidnaraNoteTool(deps: EidnaraNoteToolDeps): ToolDefinition {
    return tool({
        description: EIDNARA_NOTE_DESCRIPTION,
        args: eidnaraNoteArgsShape,
        async execute(rawArgs: EidnaraNoteArgs, toolContext) {
            const parsedArgs = eidnaraNoteArgsSchema.safeParse(rawArgs);
            let args = (parsedArgs.success ? parsedArgs.data : rawArgs) as EidnaraNoteArgs;
            ...
            if (action !== "read" && !callId) {
                ...
                return `Error: eidnara_note ${action} requires a stable tool-call identity from the host; the note was not ${outcome}.`;
            }
            ...
            try {
                ...
                if (text === null) {
                    return "Error: Rust module returned an invalid eidnara_note response.";
                }
                ...
                if (isRustToolSessionDeletedError(error)) {
                    ...
                    return `Error: Session was deleted before eidnara_note could run; ${outcome}.`;
                }
                if (isRustAuthorityDrainingError(error)) {
                    return noteAuthorityRefusal(args, action);
                }
                return `Error: Rust module eidnara_note failed. ${error instanceof Error ? error.message : String(error)}`;
            }
        },
    });
}

export function createEidnaraNoteTools(deps: EidnaraNoteToolDeps): Record<string, ToolDefinition> {
    return {
        eidnara_note: createEidnaraNoteTool(deps),
    };
}
