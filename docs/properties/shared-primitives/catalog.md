...
Confidence: high - [evidence](evidence/distinct-lease-keys-do-not-alias.md). High that the separator is rejected in every field (`LeaseKey::identity` (`crates/lease/src/lib.rs:300-326`)); high that collision handling is absent; low on practical targeted-FNV cost.
Existing check: `distinct_identity_axes_do_not_conflict` covers distinct scope, module, and backend axes; `separator_in_a_key_field_fails_closed_instead_of_aliasing` (`crates/lease/src/lib.rs:1655-1678`) covers the separator in each field; status **unaudited**.
...
Existing check: `superseded_writer_is_fenced_out_after_handover` (`crates/storage/src/lib.rs:3144-3200`), and `superseded_writer_is_rejected_after_reopen`, `primitives@89abb40`; both use synthetic stale stores and remain **unaudited**.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:66-78,676-816`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:2514-2523`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:2525-2536`) proves sibling files in one directory do not contend; `symlinked_database_paths_are_refused_never_aliased` (`:2025-2081`) proves both a directory-symlink alias and a file-symlink alias are refused, with and without a live holder. `a_hard_linked_database_is_refused` (`:1436-1463`) proves a second name for the database inode is refused through either name. No test covers descriptor disagreement; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:676-816,1319-1342`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return` (`crates/storage/src/lib.rs:2269-2284`), observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:2287-2323`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced DDL, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:2720-2740`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:2742-2808`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:2810-2858`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:2860-2905`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`primitives@89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`primitives@89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`primitives@89abb40`) and `a_regressed_positive_epoch_fails_closed` (`primitives@89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`primitives@89abb40`), `a_suppressed_epoch_increment_is_rejected` (`primitives@89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`primitives@89abb40`) protect the lease row and its issuance; `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:2907-3010`) refuses DML, triggers, indexes, views, and renames onto the `fence` and `format_marker` tables; and `a_fenced_callback_cannot_rewrite_the_format_marker` (`:3012-3068`) refuses an update of, or a rename onto, the marker row. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/store-schema-identity-matches-the-baseline.md). `ExpectedIdentity::for_baseline` (`crates/storage/src/lib.rs:1066-1132`), `classify` (`:1185-1235`), and `apply` (`:1237-1269`) implement the contract inside `open_sqlite` (`:675-815`); `fresh_file_matches_the_baseline_inventory` (`:2324-2388`) pins the fixture, and `a_file_with_foreign_objects_is_refused_without_mutation` (`:2449-2475`), `a_consumer_baseline_is_applied_once_and_verified_on_reopen` (`:2390-2430`), `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched` (`:2432-2447`), and `negative_database_fence_fails_closed` (`:3114-3142`) cover the refusals.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:403-415`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:535-594`); `GuardedConn` (`crates/storage/src/lib.rs:227-339`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:2684-2707`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:2810-2858`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:3070-3083`) pin the three denials.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:488-493`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:2720-2740`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:658-673`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:2709-2718`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:2742-2808`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:675-815`) creates the database owner-only and calls `protect_file` on all three paths after `classify` accepts the file and before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:2139-2189`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:2592-2620`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:174-224`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:3085-3112`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:2667-2682`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:1382-1385`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:1343-1350`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:3114-3142`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:3222-3238`) cover both directions.
