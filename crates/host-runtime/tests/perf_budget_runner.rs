...
use evidence::{HistogramConfig, counterbalanced_schedule};
...
fn script_text() -> String {
    std::fs::read_to_string(crate_root().join("scripts/perf-host.sh")).unwrap()
}
...
fn perf_script_parses() {
    let script = crate_root().join("scripts/perf-host.sh");
