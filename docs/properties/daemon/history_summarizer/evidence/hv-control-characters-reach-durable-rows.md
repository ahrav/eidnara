...
The only text transform in `crates/daemon/src/history_summarizer_validate.rs:1-1304` is
...
```
rg -n "sanitiz|is_control|\\\\u\\{2028\\}|escape" crates/daemon/src/history_summarizer_validate.rs
# only the unescape_xml lines above
```

`validate_parsed_history_segments` (`:983-1084`) reads `p1` for blankness only.
...
fn sanitize_history_segment_title(title: &str) -> String {
    // HistorySummarizer-authored titles are untrusted: controls and Unicode line/paragraph
...
Called at `decay_render.rs:134` from `history_segment_heading`. Note the comment
explicitly names history_summarizer-authored titles as untrusted, so the trust boundary IS
recognised in this codebase, just located at the renderer.

For BODIES, `decay_render.rs:235` and `:245`:

```rust
let body = guard_history_segment_body(&escape_xml_content(&legacy_body_for_tier(flat, tier)));
...
guard_history_segment_body(&escape_xml_content(&body))
```

`escape_xml_content` (`:80-84`) escapes `&`, `<`, `>` and nothing else.
`guard_history_segment_body` (`:138-147`) does one thing:
...
the sharpest consequence. `render_session_ref_history_segment`
(`history_summarizer_prompt.rs:218-265`) escapes every field it renders: `escape_xml_attr`
...
The producer emits a history_segment whose body contains a Unicode line separator and
an ANSI escape introducer:

```
<history_segment start="1" end="9" title="Refactor" ...><p1>Done.\u{2028}## Injected heading\u{1b}[2J</p1></history_segment>
```
...
history_segment publishes.

Store: `to_stored_history_segment` (`history_summarizer.rs:52-56`) clones the body verbatim
into `StoredHistorySegment.p1`. The durable row now holds U+2028 and an ESC byte.

Render into m0: `escape_xml_content` leaves both alone (neither is `&`, `<`, or
`>`). `guard_history_segment_body` looks for `"\n## "`; the injected heading is
...
prevents that; nothing in the history_summarizer path is.
...
lens: which surfaces read `StoredHistorySegment.p1` and `.content`, and whether each
...
2. A history_segment whose `p1` contains `'\u{2028}'`, `'\r'`, and `'\u{1b}'`, and
...
- Sources examined: `decay_render.rs:104-107` (the comment naming history_summarizer titles
  untrusted), `decay_render.rs:138-141` (the body guard's comment, which justifies
  itself only in terms of history_segment-boundary ambiguity), `history_summarizer_validate.rs:1-9`
  (the module's four stated concerns, none of them content), `history_summarizer_prompt.rs:21`
  (`HISTORY_SUMMARIZER_TRANSCRIPT_GUARD`, an injection defence at the prompt layer).
- Findings: The codebase clearly recognises the history_summarizer as untrusted and places
...
- Missing evidence: whether a design note anywhere states the sanitise-at-render
  policy. The scope map's resolved open question establishes there is no history_summarizer
  specification outside `history_summarizer*.rs`, so no such note exists to consult.
...
### Q: Can a stored control character or markup sequence corrupt a later history_summarizer run?

- Sources examined: `history_summarizer_prompt.rs:96-108` (both escape functions),
  `:218-265` (`render_session_ref_history_segment` and every escape call site),
  `:267-278` (`render_session_references_block`), `:292-300`
  (`build_reference_blocks_from_stored`).
- Findings: Every history_segment field rendered into the next run's prompt is
  escaped, with `&` escaped first in both functions, so the escaping is correct.
  A stored `</output>` or `<history_segment>` is neutralised. Control characters are
