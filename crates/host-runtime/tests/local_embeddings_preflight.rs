...
use host_runtime::local_embeddings::bundle::load_bundle;
use host_runtime::local_embeddings::embed_tokens::UntruncatedTokenizer;
...
use host_runtime::local_embeddings::{
    ...
    InferenceFailureKind, LaneInfo, LaneUnavailableState, LocalEmbeddingsComponent,
    LocalEmbeddingsLimits, LocalEmbeddingsStatus,
};
use support::local_embeddings::{BUDGET, DeterministicEngine, ready_component, test_lane};
...
fn the_verified_bundle_counts_its_corpus_in_process_without_truncation() {
    let bundle = load_bundle(
        &fixture_dir("local_embeddings-tiny"),
...
fn exact_limit_input_reaches_inference_unchanged_and_one_over_makes_no_call() {
    ...
    let component = ready_component(Arc::clone(&engine), LocalEmbeddingsLimits::default());
    let LocalEmbeddingsStatus::Ready(lane) = component.status() else {
        panic!("ready lane");
    };
...
fn byte_overflow_missing_identity_empty_input_and_lane_failures_have_exact_dispositions() {
    ...
    .unwrap();
    let tiny = LocalEmbeddingsComponent::ready_with_engine(
        test_lane(),
    ...
    let not_initialized = LocalEmbeddingsComponent::new(None);
    ...
    let unsupported = LocalEmbeddingsComponent::unsupported("local_embeddings_unsupported");
    ...
    let other_reason = LocalEmbeddingsComponent::unsupported("some other reason");
    assert_eq!(
    ...
    let component = ready_component(Arc::clone(&deterministic), LocalEmbeddingsLimits::default());
    ...
    *deterministic.fail_next_count.lock().unwrap() =
    ...
    let LocalEmbeddingsStatus::Disabled { reason } = component.status() else {
...
fn a_held_inference_permit_is_reported_as_busy_not_as_an_artifact_fault() {
    ...
    assert_eq!(
        ...
        Err(DenseUnavailable::LaneBusy {
            retry_after_ms: LocalEmbeddingsLimits::default().query_retry_after_ms,
        })
    );
    assert!(matches!(
        component.status(),
...
fn an_admitted_input_embeds_only_under_the_lane_that_admitted_it() {
    ...
    for other_lane in [other_fingerprint, other_epoch] {
        ...
        let other = LocalEmbeddingsComponent::ready_with_engine(
            other_lane,
...
fn an_admission_reused_across_same_identity_lanes_respects_the_receiving_byte_cap() {
    ...
    let wide = ready_component(Arc::clone(&wide_engine), LocalEmbeddingsLimits::default());
    let LocalEmbeddingsStatus::Ready(wide_lane) = wide.status() else {
        panic!("ready lane");
    };
    let admitted = wide
