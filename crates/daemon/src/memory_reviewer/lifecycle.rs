...
use memory_store::{MemoryReviewerStatusFacts, MemoryStore};
use serde::Serialize;
...
/// Whether the deployment owner's activation record admits MemoryReviewer disclosure, from a closed set: `open`, or the closed reason's kind.
...
/// The `memory_reviewer` block under the context component's metrics.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct MemoryReviewerHealthBlock {
    pub memory_reviewer_state: MemoryReviewerState,
    ...
    #[serde(flatten)]
    pub facts: Option<MemoryReviewerStatusFacts>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum MemoryReviewerState {
    Starting,
    Ready,
    Unavailable,
}

impl MemoryReviewerHealthBlock {
    pub fn to_json(&self) -> Value {
        serde_json::to_value(self).expect("memory_reviewer health block serializes")
    }

    fn starting() -> Self {
        Self {
            memory_reviewer_state: MemoryReviewerState::Starting,
    ...
    fn unavailable(sampled_at_ms: Option<i64>) -> Self {
        Self {
            memory_reviewer_state: MemoryReviewerState::Unavailable,
            activation_state: ActivationStateText(ActivationState::Closed("unknown")),
            sampled_at_ms,
            swept_jobs: 0,
            swept_selections: 0,
...
struct Published {
    block: MemoryReviewerHealthBlock,
    stale_at: Instant,
}
...
impl MemoryReviewerStatus {
    ...
    pub fn reported(&self) -> MemoryReviewerHealthBlock {
        let now = Instant::now();
        let published = self.snapshot.load();
        let mut block = if published.block.memory_reviewer_state == MemoryReviewerState::Ready
            && now >= published.stale_at
        {
            MemoryReviewerHealthBlock::unavailable(published.block.sampled_at_ms)
        } else {
    ...
    fn publish(&self, block: MemoryReviewerHealthBlock, sampled: Instant) {
...
pub struct Pass {
    pub block: MemoryReviewerHealthBlock,
...
pub fn sweep_and_sample(
    ...
    let unavailable = |advanced: bool| Pass {
        block: MemoryReviewerHealthBlock::unavailable(last_sampled_at_ms),
        advanced,
        healthy: false,
    };
    if cancelled() {
    ...
    let (swept_jobs, swept_selections) = match store.expire_memory_reviewer_work(now_ms) {
        Ok((jobs, selections)) => (jobs as u64, selections as u64),
        Err(error) => {
            eprintln!("daemon: memory_reviewer expiry sweep failed: {error}");
    ...
    Some(match store.memory_reviewer_status_facts() {
        Ok(facts) => Pass {
            block: MemoryReviewerHealthBlock {
    ...
    loop {
        ...
        let pass = tokio::select! {
            ...
            joined = &mut worker => match joined {
                ...
                Ok(None) => return,
                Err(error) => {
                    eprintln!("daemon: memory_reviewer sampler worker failed: {error}");
                    Pass {
                        block: MemoryReviewerHealthBlock::unavailable(last_sampled_at_ms),
                        advanced: false,
                        healthy: false,
...
mod tests {
    ...
    use memory_store::memory_reviewer_jobs::{
        CausalInputs, MEMORY_REVIEWER_QUEUE_LIFETIME_MS, MemoryReviewerJobOutcome,
        MemoryReviewerJobState, ProducerBinding, ReviewTarget,
    };
    ...
    fn stage_review_input(
        ...
        let binding = crate::memory_reviewer::handoff::review_binding(
            &"a".repeat(64),
            "memory",
            "ses",
            1,
    ...
    /// Seeds one lapsed MemoryReviewer capture whose evidence a foreign observation cites, so the sweep can retire only its observation and must keep the evidence.
    fn seed_retained_capture(kernel_store: &kernel::KernelStore, lapses_at: i64) {
        ...
        let handle = kernel_store
            .ingest_artifact(kernel::ArtifactIngestRequest {
                ...
                retention_class: kernel::MEMORY_REVIEWER_CAPTURE_RETENTION_CLASS.to_string(),
    ...
    fn retiring_a_retained_capture_counts_as_advancing() {
        ...
        let store = MemoryStore::open(&MemoryStore::test_descriptor(
            store_dir.path(),
            "eidnara-memory_reviewer-lifecycle-test",
        ))
        .unwrap();
    ...
    fn the_sweep_closes_expired_work_and_the_sample_counts_the_rest() {
        ...
        store
            .reserve_memory_reviewer_job(
                ...
                &producer(1),
                &inputs(1),
                t0 - MEMORY_REVIEWER_QUEUE_LIFETIME_MS - 1,
            )
            .unwrap();
        let live = match store
            .reserve_memory_reviewer_job("git:p", &producer(2), &inputs(2), t0)
            .unwrap()
        {
        ...
        let ready = match store
            .reserve_memory_reviewer_job("git:p", &producer(3), &inputs(3), t0)
            .unwrap()
        {
            memory_store::memory_reviewer_jobs::ReserveOutcome::Reserved(job) => job,
            other => panic!("{other:?}"),
        ...
        store
            .activate_memory_reviewer_job(
                ...
                &producer(3),
                &memory_store::memory_reviewer_jobs::MemoryReviewerJobInput {
        ...
        assert_eq!(block.memory_reviewer_state, MemoryReviewerState::Ready);
        ...
        assert_eq!(
            ...
            3 * memory_store::memory_reviewer_jobs::MEMORY_REVIEWER_RECEIPT_CHARGE_BYTES
        );
        assert_eq!(
            facts.allowance_bytes,
            2 * memory_store::memory_reviewer_jobs::MEMORY_REVIEWER_JOB_ALLOWANCE_BYTES,
        ...
        assert_eq!(
            store
                .lookup_memory_reviewer_job("git:p", &live.causal_identity)
                .unwrap()
                .unwrap()
                .state,
        ...
        assert_eq!(
            store
                .lookup_memory_reviewer_job(
                    "git:p",
                    &store
                        .reserve_memory_reviewer_job("git:p", &producer(1), &inputs(1), t0)
                        .map(|outcome| match outcome {
                            memory_store::memory_reviewer_jobs::ReserveOutcome::Existing(job) =>
                                job.causal_identity,
                            other => panic!("{other:?}"),
                        })
                        .unwrap()
    ...
    fn a_closed_reason_changes_once_per_distinct_reason_and_an_open_gate_clears_it() {
        let status = MemoryReviewerStatus::default();
    ...
    fn a_stale_ready_block_reads_as_unavailable() {
        let status = MemoryReviewerStatus::default();
        assert_eq!(
            status.reported().memory_reviewer_state,
            MemoryReviewerState::Starting
        );
        status.publish(
            MemoryReviewerHealthBlock {
                memory_reviewer_state: MemoryReviewerState::Ready,
                activation_state: ActivationStateText(ActivationState::Open),
                sampled_at_ms: Some(7),
                swept_jobs: 0,
                swept_selections: 0,
                facts: Some(MemoryReviewerStatusFacts::default()),
            },
            Instant::now(),
        );
        assert_eq!(
            status.reported().memory_reviewer_state,
            MemoryReviewerState::Ready
        );
        status.expire_for_test();
        let stale = status.reported();
        assert_eq!(
            stale.memory_reviewer_state,
        ...
        let json = MemoryReviewerHealthBlock {
            memory_reviewer_state: MemoryReviewerState::Ready,
            activation_state: ActivationStateText(ActivationState::Open),
            sampled_at_ms: Some(1),
            swept_jobs: 2,
            swept_selections: 0,
            facts: Some(MemoryReviewerStatusFacts {
                jobs_ready: 3,
                ..MemoryReviewerStatusFacts::default()
            }),
        }
        .to_json();
        assert_eq!(json["memory_reviewer_state"], "ready");
    ...
    fn a_failed_step_reads_as_unavailable_and_keeps_the_last_sample_time() {
        ...
        let store = MemoryStore::open(&MemoryStore::test_descriptor(
            store_dir.path(),
            "eidnara-memory_reviewer-lifecycle-failure-test",
        ))
        .unwrap();
        let kernel_dir = tempfile::tempdir().unwrap();
        let kernel_store = kernel::KernelStore::open(kernel_dir.path()).unwrap();
    ...
    fn a_cancelled_pass_stops_before_its_next_step() {
        ...
        let store = MemoryStore::open(&MemoryStore::test_descriptor(
            store_dir.path(),
            "eidnara-memory_reviewer-lifecycle-cancel-test",
        ))
        .unwrap();
        let kernel_dir = tempfile::tempdir().unwrap();
        let kernel_store = kernel::KernelStore::open(kernel_dir.path()).unwrap();
