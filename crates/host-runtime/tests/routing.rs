...
use support::{BindPolicy, Event, LINKED_MODULE_ID, MODULE_VERSION, TestHost};
...
async fn unroutable_targets_are_classified_distinctly() {
    ...
    let frame = control_once(
        ...
        serde_json::json!({
            "op": "route.open",
            "target": {"kind": "management_surface", "module_id": "context"},
            "identity": {"project_root": ROOT, "harness": "opencode", "session": "s"}
        }),
    )
    .await;
    ...
    let frame = control_once(
        ...
        serde_json::json!({
            "op": "route.open",
            "target": {"kind": "tool_provider", "module_id": "context-next"},
            "identity": {"project_root": ROOT, "harness": "opencode", "session": "s"}
        }),
    )
    .await;
