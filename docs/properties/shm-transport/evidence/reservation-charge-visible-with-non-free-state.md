...
- `crates/shm-transport/src/backend/ring.rs:664-736` is `try_reserve`. The
...
- `ring.rs:991-995` computes `bytes.free = self.grant.arena_bytes.checked_sub(
  ...
  `:938-987`.
- `crates/shm-transport/src/arena.rs:204-216` is `ArenaCounts::conserves`. It
...
- Existing checks: `crates/shm-transport/tests/ring.rs:131-135`, `:197-200`,
...
(`crates/shm-transport/tests/ring.rs:581`), is lockstep with a sleep. The
...
- Findings: `Ring::conservation()` is called from exactly one non-test place,
  ...
  `crates/shm-transport/tests/ring.rs:265`
