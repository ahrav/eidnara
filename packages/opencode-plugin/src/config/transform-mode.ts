...
export interface ResolveTransformModeArgs {
    ...
    /** The user tier supplies a trusted explicit daemon. */
    userTierHasExplicitDaemon: boolean;
...
export const RUST_REQUIRES_USER_CONSENT_WARNING =
    "rust mode requires user-level consent (user-tier transform_mode or an explicit daemon configuration); running ts.";
...
export function resolveTransformMode(args: ResolveTransformModeArgs): {
    ...
    // User-tier Rust selection or a trusted explicit daemon grants consent.
    if (
        args.configured === "rust" &&
        !args.userTierConfiguredRust &&
        !args.userTierHasExplicitDaemon
    ) {
        return {
            mode: "ts",
