...
- The repaired defect is `0fe5eba1`, "fix(host-runtime): freeze admission at the
...
- Before the repair, the three gates read `draining` alone. The diff replaces each
  with a two-term read. Current sites: `crates/host-runtime/src/connection.rs:285`
  (generation registration, comment at `:281-284`),
  `crates/host-runtime/src/dispatch.rs:821` (routed dispatch, comment at `:817-820`),
...
- A companion commit `86913952`, "fix(host-runtime): report server_busy when the
...
- The authoritative fence is the registry flag, not the atomic:
  `crates/host-runtime/src/routing.rs:209-211` is `freeze_admission`, setting
...
- **The freeze is the caller's duty, unenforced by the type.**
  `crates/host-runtime/src/lifecycle.rs:1274-1283` is `CommitOnAck::new`, taking only a
...
- Sources examined: `runtime.rs:78-88`, `:419-430`, `:910-911`, `:996`,
  ...
  `shared.shutdown.cancel()` site in `crates/host-runtime/src/`.
