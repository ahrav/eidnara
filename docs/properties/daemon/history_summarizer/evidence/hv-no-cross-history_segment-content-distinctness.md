...
`validate_parsed_history_segments`
(`crates/daemon/src/history_summarizer_validate.rs:983-1084`) is the only per-set
validator. It iterates with `for (index, history_segment) in history_segments.iter().enumerate()`
at `:997` and inside the loop touches exactly these fields:

- `history_segment.p1` — presence and non-blankness, `:1000-1008`.
- `history_segment.start_message`, `history_segment.end_message` — ordering `:1009-1014`,
...
- `map_parsed_history_segments_to_chunk` (`:934-981`) iterates and clones each field
  into a `ValidatedHistorySegment` (`:964-978`). No comparison.
- `parse_history_segment_output` (`:264-446`) collects history_segments into a `Vec` and
  sorts by `start_message` at `:436`. A sort by start alone; no dedup, and no
  `dedup_by` anywhere in the file.
- `validate_history_summarizer_output` (`:450-641`) after validation only pops the last
  element (`:555`) and filters side channels. No content inspection.
- The publish projection `to_stored_history_segment` (`history_summarizer.rs:38-67`) clones
  per history_segment with no set-level view.
...
`:313-316` already duplicates content WITHIN a history_segment, backfilling `p2` from
`p1` and `p3` from `p2`-or-`p1`. So the codebase treats intra-history_segment tier
duplication as normal, which means an inter-history_segment duplicate does not look
...
emits four correctly-ranged history_segments with the same body:

```
<output><history_segments>
<history_segment start="100" end="109" title="Session work" ...><p1>The user worked on the codebase.</p1></history_segment>
<history_segment start="110" end="119" title="Session work" ...><p1>The user worked on the codebase.</p1></history_segment>
<history_segment start="120" end="129" title="Session work" ...><p1>The user worked on the codebase.</p1></history_segment>
<history_segment start="130" end="139" title="Session work" ...><p1>The user worked on the codebase.</p1></history_segment>
</history_segments><meta><unprocessed_from>140</unprocessed_from></meta></output>
```
...
Discard-last DOES fire here, because `history_segments.len() >= 2` (`:539`) and the
lookahead distance is `139 - 139 = 0`, which is at most
`BOUNDARY_HEALING_SLACK = 2` (`:19`, `:554`). So the fourth is popped and three
identical history_segments publish, with `discarded_last = true`, which additionally
...
history_summarizer run is shown these rows as calibration examples
(`history_summarizer_prompt.rs:267-278`), so the repetition becomes the model's own
reference for what good output looks like.

The raw text survives in `raw_chunk_messages` (`history_summarizer.rs:1727`), so the
...
(`history_summarizer_prompt.rs:267-278`) shows the last `SESSION_REF_WINDOW = 6`
history_segments (`:15`) to the next run. Three identical rows occupy half that
...
2. The four-history_segment document above.
3. Assert the current behaviour is `Ok`, then assert the property directly:
   collect `(title, p1, p2, p3, p4)` from `validated.history_segments` into a set and
   assert its length equals `validated.history_segments.len()`. Today that assertion
...
a legitimate model MAY emit two history_segments with identical `p3` (the densest
...
- Sources examined: `history_summarizer_validate.rs:313-317` (the tier backfill),
  `history_summarizer_prompt.rs:218-265` (`render_session_ref_history_segment`, which shows the
  model the expected shape), `testdata/validate-golden.json` (all 16 cases),
  `history_summarizer_validate.rs:1367-1375` (the `xml` test helper, which derives every
  body from the history_segment title).
- Findings: Every fixture in the tree gives each history_segment a distinct body
  derived from its title. The prompt's reference block shows per-history_segment
  distinct bodies. Nothing in the contract contemplates identical bodies for
  distinct ranges. The one legitimate collapse is intra-history_segment
...
- Sources examined: `memory-store/src/lib.rs:12246-12290` (the history_segment upsert and
...
- Findings: The upsert's conflict target is the row key, and `importance` and
  ...
  the SAME key. Three history_segments with distinct sequences and distinct ranges are
...
- Missing evidence: the exact `ON CONFLICT` target column list was read at
  ...
  `to_stored_history_segment`'s `sequence: c.sequence as i64` (`history_summarizer.rs:41`)
