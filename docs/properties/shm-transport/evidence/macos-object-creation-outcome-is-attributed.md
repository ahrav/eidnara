...
`docs/shm-transport.md:121` states that on macOS "hosted `Ring::create`
...
`create_macos_shm` (`crates/shm-transport/src/backend/ring.rs:1748-1783`) has
...
The name is computed at `:1755-1764`: `String::from("/shm-")` folded over 16
...
it off. The comparable `RuntimeDir` leaf is `shm-<32 hex>` = 39 characters
(`:326`), but that is a filesystem path, which is not subject to the POSIX
shared-memory name limit.

No CI job on macOS executes this function. `the source repository `ci.yml` workflow:132` builds
the matrix `[ubuntu-latest, macos-latest]`. The Linux step (`:159-166`) runs
`cargo nextest run -p shm-native -p shm-transport` with no target filter.
The macOS step (`:169-176`) runs `cargo nextest run -p shm-transport --test
contract --test fuzz_corpus`, then `-p host-runtime --test shm_soak`, then one
`host-runtime` lib test. `tests/contract.rs` references `BackendId::Ring` only as an
enum value (`:366`, `:449`) and never constructs a `Ring`; `tests/fuzz_corpus.rs`
uses only `shm_transport::harness` (`:11`). `tests/shm_soak.rs` gates its ring
...
`the source repository `ci.yml` workflow` at HEAD `bdf72f46a` contains only `ubuntu-latest`
jobs. The same merge deleted the Darwin npm packages
(`packages/host-darwin-*`, removed in `55f47ac64`). See the dated
...
(former `crates/host-runtime/src/shm_provider.rs:276-278`) and `prepare` (former `:288-290`) both
...
- Sources examined: `ring.rs:1748-1783` line by line; `ring.rs:215-245` for the
  ...
  a different variant; `docs/shm-transport.md:110-125`;
  `docs/evidence/shm-traceability-v1.json:48`, `:157`, `:195`;
  `the source repository `ci.yml` workflow:126-177`; `tests/contract.rs` and
  `tests/fuzz_corpus.rs` import lists; `crates/host-runtime/tests/shm_soak.rs` cfg
  gates; former `crates/host-runtime/src/shm_provider.rs:275-290`, former `:827-848`; the diff of
...
- Sources examined: `packages/` listing at HEAD `bdf72f46a` (no
  `host-darwin-*` directories remain); `the source repository `ci.yml` workflow` at HEAD
  ...
  `crates/shm-transport/src/backend/ring.rs:1-2`, `:311-312`, `:2109-2115`,
  `:2176-2219`; `docs/shm-transport.md:5`, `:83`.
- Findings: the Darwin packages and macOS CI jobs are gone. `ring.rs:1-2` now
  raises `compile_error!("shm-transport ring backend supports Linux only")`
  ...
  `docs/shm-transport.md:121`, macOS `Ring::create` returns
...
- Checked: `rg -n 'create_macos_shm|shm_open|shm_unlink|target_os = "macos"' crates/shm-transport/src` returns nothing. `Mapping::create` (`crates/shm-transport/src/backend/ring.rs:458-459`) calls `create_linux_memfd` only. `validate_object` (`ring.rs:2833-2846`) computes `type_valid` from `S_IFREG` on every build. `compile_error!("shm-transport ring backend supports Linux only")` sits at `ring.rs:18-19`. `docs/shm-transport.md:5-7` and `:96` state Linux x64 glibc only, with no runtime selector or alternate backend.
- Conclusion: no. The code location the `reachable` check named does not exist here, so the check has nothing to enter. The record is marked `invalidated` in the catalog; the source-tree line references in the trail above resolve against the host repo at `bdf72f46a` and `e447c927`, not this tree.
