//! LocalEmbeddings is an optional, certified CPU-only local embedding component behind the `local_embeddings/management_surface` target.
//!
//! Missing configuration, invalid bundles, incompatible ONNX Runtime, and failed certification disable only LocalEmbeddings.
//! Artifact faults keep LocalEmbeddings's catalog identity published, make binds reject with `artifact_invalid`, and make internal health report degraded.
...
pub const LOCAL_EMBEDDINGS_MODULE_ID: &str = "local_embeddings";

/// Only trusted startup configuration sets LocalEmbeddings's finite lane capacities.
/// Requests cannot select `LocalEmbeddingsLimits`; only trusted startup configuration provides them.
#[derive(Debug, Clone)]
pub struct LocalEmbeddingsLimits {
...
impl Default for LocalEmbeddingsLimits {
    fn default() -> Self {
...
/// A component-level failure omits LocalEmbeddings from the deployment.
#[derive(Debug, Clone)]
pub struct LocalEmbeddingsConfig {
    ...
    pub limits: LocalEmbeddingsLimits,
}
...
#[derive(Debug, Clone)]
pub enum LocalEmbeddingsStatus {
    Ready(LaneInfo),
    Starting,
...
struct LocalEmbeddingsInner {
    config: Option<LocalEmbeddingsConfig>,
...
impl LocalEmbeddingsComponent {
    pub fn new(config: Option<LocalEmbeddingsConfig>) -> Self {
        let limits = config
            .as_ref()
    ...
    pub fn status(&self) -> LocalEmbeddingsStatus {
        match &*self.inner.lock_state() {
            LaneState::Ready(lane) => LocalEmbeddingsStatus::Ready(lane.lane.clone()),
            LaneState::Starting => LocalEmbeddingsStatus::Starting,
            LaneState::Disabled { reason } => LocalEmbeddingsStatus::Disabled {
                reason: reason.clone(),
            },
            LaneState::Failing { reason } => LocalEmbeddingsStatus::Failing {
                reason: reason.clone(),
            },
            LaneState::ShutDown => LocalEmbeddingsStatus::Disabled {
...
fn mark_failing(inner: &LocalEmbeddingsInner, mut reason: String) {
...
fn mark_disabled(inner: &LocalEmbeddingsInner, mut reason: String) {
    protocol::bound_diagnostic(&mut reason);
    let mut state = inner.lock_state();
...
pub enum SubmitOutcome {
    /// The job is queued, running, or already complete; poll it with [`LocalEmbeddingsComponent::poll_admitted`].
...
fn checked_local_input_capacity(limits: &LocalEmbeddingsLimits) -> Option<u64> {
...
fn constructor_local_input_capacity(limits: &LocalEmbeddingsLimits) -> u64 {
    checked_local_input_capacity(limits).unwrap_or(0)
}

fn declared_local_input_capacity(limits: &LocalEmbeddingsLimits) -> u64 {
...
fn lane_failure_reason(inner: &LocalEmbeddingsInner) -> Option<String> {
...
/// The disabled reason the host passes to `LocalEmbeddingsComponent::unsupported` on a platform with no lane.
/// Both the `local_embeddings_state` health metric and `LaneUnavailableState` classify a disabled lane by this reason, so the two projections cannot disagree.
const UNSUPPORTED_REASON: &str = "local_embeddings_unsupported";
...
impl CompositeComponent for LocalEmbeddingsComponent {
    fn manifest(&self) -> ManifestSnapshot {
        ManifestSnapshot {
            module_id: LOCAL_EMBEDDINGS_MODULE_ID.to_owned(),
    ...
    async fn bind(&self, _route: RouteHandle, _identity: RouteIdentity) -> BindOutcome {
        match self.status() {
            LocalEmbeddingsStatus::Ready(_) => BindOutcome::Accept,
            LocalEmbeddingsStatus::Starting => BindOutcome::Reject {
                code: "module_reloading".to_owned(),
                message: STARTING_REASON.to_owned(),
            },
            LocalEmbeddingsStatus::Disabled { .. } | LocalEmbeddingsStatus::Failing { .. } => {
                BindOutcome::Reject {
                    code: "artifact_invalid".to_owned(),
                    message: "the local_embeddings model bundle is unavailable".to_owned(),
                }
            }
        }
    }
    ...
    async fn handle(&self, ctx: RequestCtx) -> RequestOutcome {
        ...
        let Some(lane) = self.ready_lane() else {
            return app_error(
                "artifact_invalid",
                "the local_embeddings lane is unavailable",
            );
        };
    ...
    async fn health(&self) -> HealthReport {
        match self.status() {
            LocalEmbeddingsStatus::Ready(_) => HealthReport {
                status: HealthStatus::Ok,
                detail: None,
                metrics: Some(serde_json::json!({"local_embeddings_state": "ready"})),
            },
            LocalEmbeddingsStatus::Starting => HealthReport {
                status: HealthStatus::Degraded,
                detail: Some(STARTING_REASON.to_owned()),
                metrics: Some(serde_json::json!({"local_embeddings_state": "starting"})),
            },
            LocalEmbeddingsStatus::Disabled { reason } => HealthReport {
                status: HealthStatus::Degraded,
                metrics: Some(serde_json::json!({
                    "local_embeddings_state": if is_unsupported_reason(&reason) {
                        "unsupported"
                    } else {
                        "degraded"
                    }
            ...
            LocalEmbeddingsStatus::Failing { reason } => HealthReport {
                status: HealthStatus::Failing,
                detail: Some(reason),
                metrics: Some(serde_json::json!({"local_embeddings_state": "degraded"})),
            },
        }
    }
...
impl SecondaryComponent for LocalEmbeddingsComponent {
    async fn initialize(&self) -> Result<(), InitError> {
            ...
            && let Err(error) = bundle::validate_limits(&config.limits)
        {
            return Err(InitError(format!(
                "local_embeddings limits are invalid: {}",
                error.0
            )));
        }
    ...
    async fn activate(&self) -> Result<(), InitError> {
        ...
        match loaded {
            ...
            Err(join_error) => Err(InitError(format!(
                "local_embeddings activation task failed: {join_error}"
            ))),
        }
    }
}
...
mod tests {
    ...
    fn the_declared_hold_bound_is_the_query_permit_count() {
        for max_waiting_queries in [0usize, 1, 2, 5] {
            let limits = LocalEmbeddingsLimits {
                max_waiting_queries,
    ...
    fn local_job_inputs_are_declared_as_retained_resident_memory() {
        ...
        let expected = limits
            ...
            .unwrap();
        let component =
            LocalEmbeddingsComponent::ready_with_engine(lane(), Arc::new(NoopEngine(1.0)), limits)
                .expect("limits validate");
    ...
    async fn overflowing_unvalidated_limits_fail_initialization_without_panicking() {
        ...
        let component = LocalEmbeddingsComponent::new(Some(LocalEmbeddingsConfig {
    ...
    fn an_unready_lane_reports_its_reason_without_panicking() {
        let component = LocalEmbeddingsComponent::unsupported("local_embeddings_unsupported");
        match component.embed_blocking(&["x"]) {
            Err(InferenceError::Artifact(reason)) => {
                assert_eq!(reason, "local_embeddings_unsupported")
            }
            other => panic!("expected an artifact error, got {other:?}"),
        }
        let component = LocalEmbeddingsComponent::new(None);
        match component.embed_blocking(&["x"]) {
    ...
    async fn shutdown_disables_the_lane_for_late_callers() {
        ...
        match component.status() {
            LocalEmbeddingsStatus::Disabled { reason } => assert_eq!(reason, SHUT_DOWN_REASON),
    ...
    fn frozen_preflight_settles_count_failures_and_panics() {
        for error in [
            ...
            match (kind, component.status()) {
                ...
                (InferenceFailureKind::Artifact, LocalEmbeddingsStatus::Disabled { reason }) => {
                    assert_eq!(reason, "token counting declared the artifact unusable");
                }
                (InferenceFailureKind::Invariant, LocalEmbeddingsStatus::Failing { reason }) => {
        ...
        let LocalEmbeddingsStatus::Failing { reason } = component.status() else {
    ...
    async fn a_count_failure_that_settles_after_shutdown_preserves_the_terminal_state() {
        ...
        match component.status() {
            LocalEmbeddingsStatus::Disabled { reason } => assert_eq!(
    ...
    async fn a_count_invariant_escalates_runtime_disablement() {
        for artifact_source in ["inference", "count"] {
            ...
            assert!(
                matches!(component.status(), LocalEmbeddingsStatus::Failing { .. }),
