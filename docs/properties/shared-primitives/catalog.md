...
- Source catalog: `commons@89abb409b8c71b03146eedb5bf64cd964f2a92c0`,
  `docs/property-catalogs/cortexkit-lease/` (29 records covering the lease
  crate and the SQLite fence it guards). Those records enter with their source
...
- The PostgreSQL backend (`cortexkit-store-postgres`) is not migrated. Records
  that cited its checks keep the SQLite half; citations of the form
  `commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:NNN` point into
  the archived source repository and are provenance, not destination checks.
- Line citations name destination files at the wave's destination commit.
...
Existing check: `distinct_identity_axes_do_not_conflict` covers distinct scope, module, and backend axes; `separator_in_a_key_field_fails_closed_instead_of_aliasing` (`crates/lease/src/lib.rs:1368-1388`) covers the separator in each field; status **unaudited**.
...
Existing check: `superseded_writer_is_fenced_out_after_handover`, `crates/storage/src/lib.rs:2111-2149`, and `superseded_writer_is_rejected_after_reopen`, `commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:1144-1175`; both use synthetic stale stores and remain **unaudited**.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:60-72,559-616`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:1297-1306`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:1308-1321`) proves sibling files in one directory do not contend; `symlinked_database_paths_contend_or_are_refused_never_aliased` (`:883-936`) proves a directory-symlink alias contends and a file-symlink alias is refused. No test covers descriptor disagreement or hardlink aliases; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:559-616,699-714`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return`, `crates/storage/src/lib.rs:1163-1175`, observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:1182-1218`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Type: safety
Reachability: default-production - applies to the SQLite backend; the PostgreSQL backend is dropped.
Status: active
...
Existing check: backend tests cover fenced callbacks, fenced migrations, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1688-1708`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:1711-1775`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:1778-1831`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:1834-1890`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:647-689`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`:773-832`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`:835-879`) and `a_regressed_positive_epoch_fails_closed` (`:882-924`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`:927-971`), `a_suppressed_epoch_increment_is_rejected` (`:974-991`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`:994-1018`) protect the lease row and its issuance; and `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:1893-2001`) refuses DML, triggers, indexes, views, and renames onto the fence table. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Guarantee: Applying rendered units replaces a unit with the same key in its existing slot and appends new keys in input order, so the cached-prefix byte order is the frozen-set order.
...
Status: active
Exercised: yes - the fixture is byte-identical to `commons@89abb40` and all eleven vectors, the schema-3 empty wire format, and the cross-episode lineage vector pass in the destination workspace.
Guarantee: `tests/golden/cache-stability-golden-vectors.json` keeps the exact bytes captured at the pinned source commit, and every vector in it reproduces: after each pass the executed action, `cached_prefix_bytes()`, `boundary_id`, `reconcile_pending`, and the pending-change count equal the fixture.
Check: `always` - `always(sha256(fixture) == source_blob_sha256)` at receipt verification, and `always(observed_after_pass == expected_after_pass)` for every pass of every vector.
Fault/timing angle: The fixture is the cross-harness contract; a regenerated fixture would pass its own test, so byte identity to the pinned source blob is checked outside the test.
Required faults and enabling state: A verbatim receipt entry with the source blob hash, plus the test run against the destination crate.
Confidence: high - [evidence](evidence/cache-stability-golden-vectors-are-byte-stable.md). Receipt verification recomputes the destination hash and compares it to `git cat-file` of the source blob; `all_golden_vectors_pass` (`crates/cache-stability/tests/golden_vectors.rs:160-166`) drives every vector through `CoreState::step`. The action-equality assertion is near-tautological because the harness feeds the expected action as `proposed`; the byte, boundary, flag, and queue assertions are independent.
Existing check: `golden_fixture_is_schema_v3_with_eleven_vectors` (`crates/cache-stability/tests/golden_vectors.rs:97`), `core_state_schema_v3_empty_wire_format_is_stable` (`crates/cache-stability/tests/golden_vectors.rs:108`), `all_golden_vectors_pass`, `cross_episode_lineage_reproduces_byte_identical`; audited at U2 with the tautology noted.
...
Status: active
Exercised: yes - the fixture is byte-identical to `commons@89abb40`; `postgres_database_name`, `sqlite_store_path`, and descriptor reserialization reproduce all eight vectors in the destination workspace.
Guarantee: `tests/golden/storage_vectors.json` keeps the exact bytes captured at the pinned source commit, and for every vector `postgres_database_name(id)`, `sqlite_store_path(data_home, id)`, and the reserialized `sqlite_descriptor` equal the fixture.
...
Type: safety
Reachability: default-production - every `open_sqlite` and every migration touches both tables.
Status: active
Exercised: yes - a fresh store after open and one migration, read back through `sqlite_schema`.
Guarantee: The tables `cortexkit_fence` and `cortexkit_schema_version` keep the names and column definitions that existing databases carry, and `cortexkit_fence` holds exactly one row with `id = 0`.
...
Confidence: high - [evidence](evidence/fence-tables-keep-their-durable-identity.md). `ensure_fence_table` (`crates/storage/src/lib.rs:668-675`) and `run_migrations` (`crates/storage/src/lib.rs:778-786`) emit the DDL; `fence_and_version_tables_keep_their_ddl` (`crates/storage/src/lib.rs:2234`) compares the recorded SQL text and the fence row. Both names are `frozen-durable` in `migration/registry.json`.
...
Type: safety
Reachability: default-production - every store runs its migration chain on open.
Status: active
Exercised: yes - reopen with the same chain, a later version on top of an earlier one, and two independent namespaces in one database.
Guarantee: Within a namespace, each migration version applies at most once, versions apply in ascending order, and the version record commits in the same transaction as the migration SQL; namespaces do not affect each other.
...
Confidence: high - [evidence](evidence/migrations-apply-once-per-namespace.md). `run_migrations` (`crates/storage/src/lib.rs:767-863`) reads `MAX(version)` per namespace, sorts, skips `<= current`, and commits SQL and record together; `migrations_seed_once_across_reopen` (`crates/storage/src/lib.rs:1221`), `later_migration_applies_on_top_of_earlier` (`crates/storage/src/lib.rs:1547`), and `independent_namespace_chains_in_one_database` (`crates/storage/src/lib.rs:1576-1607`) cover the three clauses.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:358-362`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:446-464`); `GuardedConn` (`crates/storage/src/lib.rs:227-339`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:1651`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:1778`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:2004`) pin the three denials.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:433-438`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1688`) exercises the unwind.
...
Type: safety
Reachability: default-production - every fenced write and migration re-pins the journal settings.
Status: active
Exercised: yes - open, a read callback that is denied lowering `synchronous`, and a fenced write plus a migration that re-pin `FULL` and WAL after the maintenance path changed them.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:519-531`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:1677`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:1711`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:581-587`) creates the database owner-only and calls `protect_file` on all three paths before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:1029`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:1449-1480`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:184-206`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:2020`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:1633`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:756-758`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:717-723`) use checked conversions; `legacy_negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:2081`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:2208`) cover both directions.
...
| `migrations-apply-once-per-namespace`, `protected-transactions-pin-fence-durability` | `/testing:crash-consistency-and-failpoint-testing` for the crash windows the records name but do not inject |
