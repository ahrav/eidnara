...
describe("source inventory contract", () => {
    ...
    it("rejects malformed ids, digests, and empty rationale", () => {
        expect(() => parseSourceInventory(inventory([claim("Claim One" as never)]))).toThrow(
            /static lowercase id/,
        );
        expect(() =>
...
describe("incident catalog contract", () => {
    ...
    it("rejects invalid lane/applicability combinations", () => {
        ...
        expect(() =>
            parseIncidentCatalog(
                catalog(
                    [adjudicationOnlyVariant("var-note-only"), variant("var-red-one")].map(
                        (entry) =>
                            entry.id === "var-note-only"
                                ? {
                                      ...entry,
                                      applicability: variant("var-red-one").applicability,
                                  }
                                : entry,
                    ) as IncidentVariant[],
                ),
    ...
    it("rejects dynamic check labels that could carry fixture data", () => {
        for (const bad of ["check-${fixture}", "Check One", "check-", "assert stale row"]) {
            expect(() =>
                parseIncidentCatalog(
                    catalog([variant("var-red-one", { normative_checks: [bad] })]),
                ),
    ...
    it("rejects omitting the declared harness", () => {
        expect(() =>
            parseIncidentCatalog(
                catalog([
                    variant("var-red-one", {
                        applicability: {
                            harness: "rust",
                            omitted: [{ harness: "rust", reason: "nope" }],
                        },
                    }),
                ]),
            ),
    ...
    it("rejects self and unknown blocked_by dependencies", () => {
        expect(() =>
            parseIncidentCatalog(
                catalog([variant("var-red-one", { blocked_by: ["var-red-one"] })]),
            ),
    ...
    it("rejects duplicate values in contract arrays", () => {
        ...
        expect(() =>
            parseIncidentCatalog(
                catalog([
                    variant("var-red-one", {
                        normative_checks: ["check-durable-state", "check-durable-state"],
                    }),
                ]),
            ),
        ).toThrow(/must not contain duplicates/);
...
describe("adjudication event contract", () => {
    ...
    it("rejects unknown fields", () => {
        expect(() =>
            parseAdjudicationEvent({ ...(event() as Record<string, unknown>), extra: 1 }, "e"),
        ).toThrow(/must contain exactly/);
    });
    ...
    it("rejects malformed sequence numbers, ids, and empty rationale", () => {
        ...
        expect(() => parseAdjudicationEvent(event({ identity: "mystery" }), "e")).toThrow(
            /identity/,
        );
...
describe("emergency redaction contract", () => {
    ...
    it("rejects unknown fields", () => {
        expect(() =>
            parseEmergencyRedaction({ ...(redaction() as Record<string, unknown>), why: "" }, "r"),
        ).toThrow(/must contain exactly/);
    });
    ...
    it("rejects a missing review reference, bad scope, or bad data class", () => {
        ...
        expect(() =>
            parseEmergencyRedaction(redaction({ scope: "everything" as never }), "r"),
