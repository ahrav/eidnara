...
/// Provider spelling is preserved because `HistorySummarizerProducer` matches the wire string rather than a normalized finish-reason class.
...
/// The producer/consumer error-class contract mirrors `ERROR_CLASS_WIRE_SET` in `daemon`'s history_summarizer producer.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ErrorClass {
    Transient,
    Permanent,
