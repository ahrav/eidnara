...
One attention focus: `crates/daemon/src/history_summarizer_validate.rs` treated as the
...
The gate is two public functions plus their helpers. `parse_history_segment_output`
(`history_summarizer_validate.rs:264-446`) turns text into structures.
`validate_history_summarizer_output` (`:450-641`) is the admission decision. The single
production caller is `publish_output_from_awaiting` (`history_summarizer.rs:1641-1704`),
which calls it at `:1673-1678`.

Two different dispositions matter, and the module does not name the difference:
a **reject** returns `Err` and no history_segment is written; a **drop** silently
...
| 4 | `:463-467` | Already-persisted ranges that are inverted or overlapping, via `validate_stored_history_segments` (`:647-676`). Also our own state. |
...
| 7 | `:487-491` | Zero usable history_segments after parsing: "HistorySummarizer returned no usable history_segments." |
| 8 | `:506-512` -> `:949-957` | A history_segment whose `start` or `end` ordinal is not present as a chunk line. |
| 9 | `:958-963` | A history_segment whose end line is not `anchorable` or whose `message_id` is empty, so publication cannot mint an unanchorable coverage boundary. |
| 10 | `:514-524` -> `:1000-1008` | A history_segment whose `p1` is absent, or present but whitespace-only. |
...
| 15 | `:1033-1038` | A history_segment that starts after chunk coverage has already been consumed. |
...
| 22 | `:565-570` | No forward progress: the last surviving history_segment does not end at or beyond the first ordinal after the prior stored end. |

**22 rejecting checks.** One further reject fires before the gate is entered:
`history_summarizer.rs:1666-1671` refuses an output the producer marked `length_capped`,
so a truncated document never reaches `parse_history_segment_output`.

### Silent drops (parse-time, not rejects)

| Line | What is discarded, keeping the rest |
| --- | --- |
| `:289-296` | A `<history_segment>` with a non-numeric or absent `start=` or `end=`. |
| `:297-303` | A `<history_segment>` with an absent or empty `title=`. |
| `:309`, `:333-347` | A `<history_segment>` with neither a non-empty tier body nor non-empty flat content. |
...
These drops are mostly benign because checks 16 to 20 turn a dropped history_segment
...
- `heal_history_segment_gaps` (`:899-932`) extends the previous history_segment's
...
- `heal_terminal_completed_tool_arc` (`:864-897`) extends the last
  history_segment's `end_message` forward to close a completed arc and rewrites
  `unprocessed_from` (`:891-896`).
- Discard-last (`:539-558`) pops the final history_segment when lookahead distance
  is at most `BOUNDARY_HEALING_SLACK = 2` (`:19`), gated on `history_segments.len()
  >= 2`.
...
publish projection `history_summarizer.rs:38-67` and the render consumer
...
2. **No cross-history_segment content distinctness.** Nothing compares one
   history_segment's `title`/`p1`..`p4` with another's. Ranges must be contiguous
...
3. **No binding of the output text to the chunk's identity.** The output is tied
   ...
   `history_summarizer_prompt.rs` and `history_summarizer_validate.rs` returns nothing. The chunk
   fingerprint at `history_summarizer.rs:1449-1461` binds the chunk to the firing, not
   the model's text to the chunk.
4. **No token or byte cap on the accepted document.** The producer-side
   `length_capped` flag (`history_summarizer.rs:1666`) refuses a *truncated* document;
...
7. **No `episode_type` allowlist.** Any attribute string is accepted verbatim
   (`:305`) and stored (`history_summarizer.rs:58`).
...
10. **No lookahead protection for a single-history_segment output.** Discard-last
    requires `history_segments.len() >= 2` at `:539`.
11. **No re-validation of content after a heal mutates a range.** Both heal
    functions change `end_message`, and `map_parsed_history_segments_to_chunk`
...
15. **The state machine looks at nothing.** `output_received`
    (`history_summarizer.rs:299-307`) takes the text as `_output_text` and discards it.
16. **`ValidatedChunk` is not a proof-carrying token.** All fields are `pub`
    ...
    `pub` in a `pub mod` (`history_summarizer.rs:444`, `lib.rs:19`), so the type system
...
- `history_summarizer.rs:1673-1678` is the only production call of
  `validate_history_summarizer_output`. Confirmed by `rg`: every other call is inside
  `#[cfg(test)]` (`history_summarizer_validate.rs:1402` etc.,
  `history_summarizer_chunk.rs:1419`).
- `history_summarizer.rs:1663-1664` moves durable state to `Validating` and persists it
  BEFORE validating. `history_summarizer.rs:1680-1704` handles the rejection: it
  computes a backoff (`:1683-1687`), persists
  `abandon_with_detail(..., Some(format!("validate rejected: {err}{cap_hint}")))`
  (`:1693-1701`), and returns `HistorySummarizerDriveError::Validation(err)`.
- **Rejection is fail-closed on the history_segment path.** No arm of
  `publish_output_from_awaiting` reaches `publish_validated_chunk`
  (`:1714`) without a successful `validate_history_summarizer_output`. There is no
  best-effort, partial-accept, or force branch. The original conversation is
  preserved because nothing is written to the history_segment table.
- **The gate is not, however, side-effect free for the caller.** Two durable
  `persist_history_summarizer_state` writes happen around a rejection: the `Validating`
  ...
  true of the *history_segment* write and false of the *phase* write. See
  contract-vs-code lead 1.
- **Rejection is observable, but weakly.** The reason lands in
  `HistorySummarizerDurableState.last_failure` and surfaces through
  `history_summarizer_status_summary` (`lib.rs:15464-15478`) and `HistorySummarizerDiagnostics`
...
- **A validation rejection does not count as a publish failure.**
  ...
  (`history_summarizer.rs:358`). The only increments are in `memory-store`
...
- **Retry within one firing is bounded by the model chain.**
  `history_summarizer.rs:1440-1450` treats `HistorySummarizerDriveError::Validation` as
...
- **Retry across firings is bounded only by time.** The sole gate is
  ...
  reason `backoff` and declines. `HISTORY_SUMMARIZER_FAILURE_BACKOFF_MS` is 60_000
  (`history_summarizer.rs:30`) and the completion-time backoff preserves the configured
  cooldown (`history_summarizer.rs:1145-1154`) rather than escalating it. There is no
...
- The raw material is retained. `publish_validated_chunk` stores
  `chunk_transcript` and `raw_chunk_messages` (`history_summarizer.rs:1726-1727`,
...
- Feeding prior history_segments back into the next run's prompt IS escaped
  (`history_summarizer_prompt.rs:218-265`), and the transcript carries an explicit
  injection guard string (`history_summarizer_prompt.rs:21`). So the prompt feedback loop
...
- `map_parsed_history_segments_to_chunk` (`:506`) runs BEFORE
  `validate_parsed_history_segments` (`:514`), so endpoint mapping and sequence
...
Guarantee: A published history_segment set is bound to the specific chunk it summarizes by something the model must have read, not only by integer ordinals that any output covering the same span satisfies.
...
Required faults and enabling state: A configured history_summarizer model chain; a producer that returns a document whose history_segment ranges are contiguous over `chunk.start_index..=chunk.end_index` and whose `<unprocessed_from>` is `chunk_end + 1`, with bodies describing unrelated content.
...
Existing check: none. The chunk fingerprint at `history_summarizer.rs:1449-1461` and `:444-455` binds chunk to firing, not text to chunk.
...
- Is a chunk-derived echo requirement compatible with the byte-identical TypeScript oracle the golden test pins (`history_summarizer_validate.rs:1384`)? (needs human input)
...
Reachability: default-production
Status: active
Exercised: not yet — the 19 inline tests all use bodies generated from the history_segment title (`:1367-1375`), never a degenerate one.
Guarantee: The publish path can be reached with a history_segment body that carries essentially no information about the ordinals it replaces.
Check: `reachable` — the `to_stored_history_segment` call at `history_summarizer.rs:1738` (via `publish_validated_chunk`, `history_summarizer.rs:471-475`) is reached with `p1.trim().chars().count() == 1` and `end_message - start_message` at least 100. Semantics are `reachable` because the finding is that this code location is attainable, not that a forbidden state is entered.
Fault/timing angle: None. No interleaving needed.
Required faults and enabling state: A configured history_summarizer model chain and a producer returning one well-formed history_segment with a one-character `p1`, a non-empty `title`, and a matching `<unprocessed_from>`.
...
Impact: A long stretch of real conversation is served as a near-empty summary. Compounded by hv-single-history_segment-skips-lookahead-discard, because a one-history_segment output also skips the discard-last protection.
Open questions:

- Does the project want a span-relative floor, or is body adequacy deliberately delegated to the history_summarizer-eval scorer lane (`ci.yml:415-440`)? (needs human input)
...
Guarantee: Two history_segments published in one run that cover disjoint ordinal ranges do not carry byte-identical bodies.
Check: `always` — for every accepted `ValidatedChunk`, no two elements of `history_segments` share the same `(title, p1, p2, p3, p4)` tuple. `always` because it is a property of each admitted set, evaluated at every publish.
Fault/timing angle: None.
Required faults and enabling state: A producer returning N contiguous history_segments whose ranges partition the chunk and whose bodies are copies of one another.
Confidence: high — [evidence](../evidence/hv-no-cross-history_segment-content-distinctness.md). `validate_parsed_history_segments` (`:983-1084`) iterates history_segments and compares only `p1` presence and ordinals; no cross-element content comparison exists anywhere in `:1-1304`.
Existing check: none.
...
Guarantee: A published history_segment's stored `importance` lies in the documented 1..=100 band that the decay curve consumes, or the output is rejected.
Check: `always` — for every `StoredHistorySegment` produced by `to_stored_history_segment`, `1 <= importance <= 100`. `always` because the invariant must hold for every published row.
Fault/timing angle: None.
Required faults and enabling state: A producer emitting `importance="4294967296"` or any value above `i32::MAX` on an otherwise valid history_segment.
Confidence: high — [evidence](../evidence/hv-importance-unbounded-then-truncating-cast.md). Parsed as unbounded `\d+` at `:1195`, captured to `u64` at `:306` via `capture_u64` (`:1106-1110`), narrowed by `as i32` at `history_summarizer.rs:57`. Confirmed no clamp in `memory-store` (schema default only, `memory-store/src/lib.rs:455`; insert at `:12288` casts back to `i64`). The only clamp is at render, `decay_render.rs:269-272`.
...
Impact: A history_segment the model marked maximally important is stored with a wrapped value and rendered as least important, so it decays to the densest tier first. Silent, and the stored row is wrong for any consumer that does not clamp.
Open questions:

- Are there stored-history_segment consumers besides `decay_render.rs` that read `importance` without clamping? Unresolved, needs a sweep of `memory-store` readers in a Part 3 or 4d pass.
...
Guarantee: Model-authored text admitted by the gate cannot carry control characters or line/paragraph separators into durable history_segment rows.
Check: `always` — for every accepted history_segment, no character of `title`, `content`, or `p1`..`p4` satisfies `char::is_control()` or is `\u{2028}`/`\u{2029}`. `always` because it must hold for each admitted history_segment.
Fault/timing angle: None.
Required faults and enabling state: A producer emitting a history_segment whose `p1` body or `title` attribute contains `\u{2028}`, `\r`, or an ANSI escape introducer.
...
Existing check: `decay_render.rs:104-121` for titles at render time, marked in its own comment as "HistorySummarizer-authored titles are untrusted". Bodies have no equivalent.
...
Reachability: default-production
Status: active
Exercised: not yet — `history_summarizer_prompt.rs:430-431` tests the escape direction only; nothing tests the round trip.
...
Confidence: high — [evidence](../evidence/hv-unescape-xml-double-decodes-entities.md). `unescape_xml` (`:1148-1154`) replaces `&amp;` -> `&` FIRST, then `&lt;` -> `<`, so `&amp;lt;` becomes `&lt;` becomes `<`. The counterpart `escape_xml_content` (`decay_render.rs:80-84`, `history_summarizer_prompt.rs:104-108`) escapes `&` first and is therefore correct; only the inverse is wrong.
Existing check: `history_summarizer_prompt.rs:427-432` asserts the escape functions, never the inverse.
...
Reachability: default-production
Status: active
Exercised: partial — `discard_last_progress_guard_boundary_k1_vs_k2` (`:1633`) covers the k1/k2 lookahead boundary with two history_segments; no test covers the one-history_segment case.
Guarantee: A history_segment whose terminal boundary was chosen with less than `BOUNDARY_HEALING_SLACK` ordinals of lookahead is withheld for re-derivation, regardless of how many history_segments the run produced.
Check: `always` — whenever `chunk.end_index - last.end_message <= BOUNDARY_HEALING_SLACK`, `in_emergency` is false, and `force_keep_last_history_segment` is false, the accepted set does not contain that last history_segment. `always` because the protection is claimed on every non-emergency publish.
...
Required faults and enabling state: A chunk whose content yields a single history_segment ending at or within two ordinals of `chunk.end_index`, with `in_emergency` and `force_keep_last_history_segment` both false.
Confidence: high — [evidence](../evidence/hv-single-history_segment-skips-lookahead-discard.md). The guard at `:539` requires `history_segments.len() >= 2`; with one history_segment the whole block `:539-558` is skipped and `discarded_last` stays false. `BOUNDARY_HEALING_SLACK = 2` at `:19`, applied at `:554`.
Existing check: `:539-558` for two or more history_segments; tests at `:1633`, `:1730`, `:1748`, `:1849`.
...
- Is the `>= 2` guard intentional (popping the only history_segment would fail the `:565-570` forward-progress check and reject the run) or accidental? The interaction suggests intentional; no comment says so. (needs human input)
...
Reachability: default-production
Status: active
Exercised: partial — `zero_side_channel_anchor_is_suppressed` (`:1774`) and `events beyond persisted history_segment count are filtered` (golden case) cover the filtering; nothing asserts the drop is reported.
Guarantee: A side-channel item whose declared anchor does not name a persisted history_segment is either rejected with the run or reported, never silently discarded.
Check: `always` — for every input side-channel item with `origin_history_segment_index` or `at_history_segment` outside `1..=persisted_count`, the run either returns `Err` or records a counted drop. `always` because it must hold for each admitted output.
Fault/timing angle: None.
Required faults and enabling state: A producer emitting a fact, event, or primer anchored to a history_segment index above the count that survives discard-last.
...
Check: `always` — poll for a bounded window of `N` firing opportunities after the last configuration change; after `N` consecutive validation rejections, either `history_summarizer.failure_backoff_at_ms` has escalated beyond `HISTORY_SUMMARIZER_FAILURE_BACKOFF_MS` or `publish_health_degraded` is true. Stated in attempts, not in an unbounded "eventually", per the liveness rules.
Fault/timing angle: The window is the 60-second backoff at `history_summarizer.rs:30`, re-evaluated at `lib.rs:5042-5047`. Each expiry admits one more firing, each costing a full model chain of live calls.
...
Confidence: high — [evidence](../evidence/hv-validation-rejection-retry-has-no-attempt-bound.md). Traced the whole rejection path: `history_summarizer.rs:1680-1703` abandons with a backoff; `abandon_with_detail` (`:352-361`) copies `consecutive_publish_failures` unchanged; the only increments are in `memory-store/src/lib.rs:9264-9268` and `:9323-9326`, which this path does not reach; `completion_failure_backoff_at_ms` (`history_summarizer.rs:1145-1154`) preserves rather than escalates the cooldown; the intra-firing fallback at `history_summarizer.rs:1440-1450` bounds attempts per firing only.
...
Reachability: default-production
Status: active
Exercised: partial — `tierless_history_segments_reject_while_p1_only_output_keeps_soft_fallbacks` (`:1463`) proves the gate rejects tierless output; nothing asserts the `legacy: 1` arm is never taken.
Guarantee: The legacy-row arm of the publish projection never executes, because validation rejects every tierless history_segment before publish.
Check: `unreachable` — the `1` arm of the `legacy` expression at `history_summarizer.rs:65` must never be evaluated during a publish. Semantics are `unreachable` because this is a specific code location the code's own comment says cannot be reached, not a state without a detection point.
Fault/timing angle: None.
...
Confidence: high — [evidence](../evidence/hv-tierless-stored-row-arm-must-stay-unreachable.md). `history_summarizer.rs:60-62` states the claim: "Strict validation makes tierless output unreachable, but derive legacy from P1 so a future bypass cannot falsely mark a flat row as v2." Verified the gate's side: `:1000-1008` rejects an absent or blank `p1`, and `ValidatedHistorySegment.p1` is a clone of the parsed value (`:972`), so the projection sees the validated field.
Existing check: `history_summarizer.rs:63-67` is itself the defensive derivation; `:1463` covers the gate side.
...
Reachability: default-production
Status: active
Exercised: partial — four tests call `publish_validated_chunk` directly with hand-built input (`history_summarizer.rs:4173`, `:4328`, `:4414`, `:4495`), demonstrating the bypass is trivially constructible, but no test asserts the production invariant.
Guarantee: Every set of history_segments that reaches the durable publish transaction was produced by `validate_history_summarizer_output` from the same output text and the same pinned chunk.
Check: `always` — every execution of `publish_validated_chunk` (`history_summarizer.rs:444`) is preceded in the same call chain by a successful `validate_history_summarizer_output` over the text that produced its `validated` argument. `always` because the obligation applies to each publish; the forbidden state has no single detection point, so per the coverage rules this is `always(...)`, not `unreachable`.
...
Confidence: high — [evidence](../evidence/hv-publish-accepts-unvalidated-validated-chunk.md). `ValidatedChunk` at `:226-238` has all-`pub` fields and derives `Default`; `history_summarizer.rs:444` is `pub fn` inside `pub mod history_summarizer` (`lib.rs:19`), and `history_summarizer_validate` is also `pub` (`lib.rs:23`). Confirmed by `rg` that the only production constructions of a `ValidatedChunk` are `history_summarizer_validate.rs:628` (the gate's own return) and test code at `history_summarizer.rs:4476`.
Existing check: convention only. Both production paths (`history_summarizer.rs:1419`, `:1592`) funnel through `publish_output_from_awaiting`, which validates at `:1673`.
Impact: A future publish route, or an external crate depending on `daemon`, can write model text into durable history_segments with zero validation. This is the mechanism that would make hv-tierless-stored-row-arm-must-stay-unreachable fire.
Open questions:
...
Guarantee: When healing widens a history_segment's ordinal range, the widened range still contains only ordinals whose content the history_segment's body is entitled to claim.
Check: `always` — for every accepted history_segment whose `end_message` differs from the parsed value, every newly covered present ordinal lies inside `chunk.tool_only_ranges` or inside a `chunk.completed_tool_arcs` entry that the terminal heal closed. `always` because it must hold for each healed history_segment in each publish.
Fault/timing angle: The window is between parse and mapping: `heal_history_segment_gaps` (`:493-497`) and `heal_terminal_completed_tool_arc` (`:498-504`) mutate ranges at `:493-504`, before mapping at `:506` and before validation at `:514`.
Required faults and enabling state: A chunk with `tool_only_ranges` or `completed_tool_arcs` populated by `history_summarizer_chunk.rs`, and a model output that leaves a gap inside one of them.
Confidence: high — [evidence](../evidence/hv-heal-extends-range-without-revalidating-content.md). `heal_history_segment_gaps` sets `history_segments[i - 1].end_message` at `:927-930` and touches no content field; `heal_terminal_completed_tool_arc` sets `last.end_message` at `:889` and rewrites `unprocessed_from` at `:891-896`. `map_parsed_history_segments_to_chunk` then stamps a NEW `end_message_id` from the healed ordinal (`:945-948`, `:969`), so the durable boundary identity changes too. The healed end is still checked for anchorability at `:958-963`, and a non-tool-only gap still rejects at `:1046-1049`.
...
Impact: The stored history_segment asserts coverage of raw messages its summary never described, and its `end_message_id` names a block the model never saw as its boundary. The module's own comment at `:923-925` accepts this for tool-only noise; the record exists so the premise ("Production replay showed contiguous narrative coverage") is a claim under test rather than a settled fact.
Open questions:

- Are `tool_only_ranges` and `completed_tool_arcs` themselves derived only from module-side classification, with no model influence? `history_summarizer_chunk.rs` builds them, so they appear trustworthy, but the derivation was not audited in this pass. Unresolved, needs the chunk-construction lens.
...
- **19 tests, count confirmed.** `grep -c "#\[test\]"` over
  `history_summarizer_validate.rs` restricted to lines after the test-module opener at
...
- **CI status confirmed: none of them run.** The only `daemon` test
  ...
  `claude-code-review.yml`, and `history_summarizer-eval.yml` name no Rust `daemon`
  test target. `history_summarizer-eval.yml` is `workflow_dispatch`/`schedule` only and
  runs a Bun harness; its per-PR deterministic counterpart is the
  `history_summarizer-eval-contracts` job at `ci.yml:415-440`, which runs
  `bun run test:history_summarizer-eval-unit` and two `run-history_summarizer-eval.ts` modes.
...
| 4 stored ranges | `stored_history_segment_validation_is_basis_agnostic_and_allows_sparse_gaps` (`:1531`) |
...
| 7 no usable history_segments | golden "malformed xml", "empty output" |
| 8 endpoint maps to a line | **no test** |
| 9 end line anchorable | `history_segment_end_must_be_anchorable` (`:1665`) |
| 10 `p1` required | `tierless_history_segments_reject...` (`:1463`), `mismatched_tier_close...` (`:1483`) |
...
1. **"before any database write is possible" versus two durable writes around a
   rejection.** `history_summarizer_validate.rs:6-9` states the module keeps
   ...
   phase at `history_summarizer.rs:1664` before validating and persists the abandon at
   `history_summarizer.rs:1693-1701` after rejecting. The claim holds for the
   *history_segment* write and not for the *phase* write. The phase writes are
   deliberate (the comment at `history_summarizer.rs:1709-1713` explains why a pre-check
...
2. **"re-emit with all four tiers" versus a p1-only requirement.** The error
   ...
   gate accepts a p1-only history_segment and then reports a message demanding four
   tiers. `tierless_history_segments_reject_while_p1_only_output_keeps_soft_fallbacks`
...
3. **"callers surface them in repair prompts" with no repair prompt.**
   ...
   and telemetry." `rg -n "repair"` over `history_summarizer_validate.rs`,
   `history_summarizer.rs`, `history_summarizer_prompt.rs`, and `lib.rs` finds no repair-prompt
...
4. **"Strict validation makes tierless output unreachable" is a claim, not a
   proof.** `history_summarizer.rs:60-62`. It is true of the two current call paths and
   is not enforced by any type or assertion; see
   hv-publish-accepts-unvalidated-validated-chunk.
5. **The doc-comment purity claim is accurate.** `:5-7` says the functions are
   "deliberately pure. They receive the raw history_summarizer text plus caller-provided
   chunk/store metadata". Verified: `history_summarizer_validate.rs` imports only
...
6. **Scope-map correction.** The scope map's Part 4a rationale says the
   history_summarizer publish is "gated solely by in-crate validation" whose "19 tests
...
- The golden file records TypeScript's error strings for malformed envelopes
  (`"HistorySummarizer returned no usable history_segments."` for the empty input) while
...
- Is `history_summarizer_chunk.rs`'s derivation of `tool_only_ranges` and
...
- `handle_session_status_value` exposes no validation-rejection signal at all
  ...
  `history_summarizer_status_summary`, which `last_no_fire` masks
