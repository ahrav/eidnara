...
lie outside those crates: `CONFIGURATION.md:165` (source-catalog path, not present at HEAD) and `:167`, the sibling
`../commons/crates/storage/src/lib.rs:249-281` (source-catalog path, not present at HEAD), and the pinned
...
1. **Whether cross-process selection impurity is a defect at all, or deliberate
   ...
   (`../commons/crates/storage/src/lib.rs:249-281` (source-catalog path, not present at HEAD)). The doc comment there
...
2. **The TypeScript-suite observation now appears in all three artifacts without
   ...
   which documented byte-identity claims (`CONFIGURATION.md:659` (source-catalog path, not present at HEAD), `:716`, `:763`)
