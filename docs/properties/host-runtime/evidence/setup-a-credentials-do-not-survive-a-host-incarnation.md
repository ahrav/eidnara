...
Credential minting, `crates/host-runtime/src/instance.rs:232-280`, inside
...
Peer-side refusal, `crates/host-runtime/src/auth.rs:303-367`, in order:
...
`packages/shm-native/src/setup.rs:200-205` folds all three into one `if` before
the `write_message` at `:206-219`.

The wire doc states the intended behaviour at
`docs/host-wire-protocol.md:218`: "Mixed-generation key, daemon-ID, and
...
`crates/host-runtime/src/client.rs:346` takes `info` by value and never re-reads the
...
`crates/host-runtime/tests/host_roundtrip.rs:186` constructs a `ConnectionInfo` with a
...
- Sources examined: the comment at `auth.rs:385-403`, which says the always-false
  ...
  `crates/host-runtime/tests/` for `handshake`, `max_handshakes` and
  `unauthenticated`.
...
- Missing evidence: a repository-wide search including `packages/` and any
  bootstrap crate outside `crates/host-runtime/tests/` was not run in this pass.
...
- Sources examined: `auth.rs:246-252`, `:268-274`, `:336-338`,
  `packages/shm-native/src/setup.rs:200-205`.
