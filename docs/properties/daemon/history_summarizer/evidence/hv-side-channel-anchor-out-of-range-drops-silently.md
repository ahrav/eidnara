...
The bound, `crates/daemon/src/history_summarizer_validate.rs:1086-1098`:
...
This is a genuine anti-fabrication check: an anchor must name a history_segment that
actually persisted in THIS run, 1-based. `source_history_segment`
(`history_summarizer.rs:69-78`) relies on it, doing
`origin.and_then(|i| i.checked_sub(1)).and_then(|i| history_segments.get(i as usize))`,
...
any logging or metric facility: `history_summarizer_validate.rs` imports only `BTreeMap`,
...
- `force_keep_last_history_segment` suppresses facts, primers, and observations
  entirely via the `!options.force_keep_last_history_segment &&` conjunct
...
- user observations: `user_memory_collection_enabled` defaults FALSE
  ...
  `collect_user_memory_candidates` is set (`history_summarizer.rs:1724`, documented at
  `:422-423`). So the observation channel is off by default and the practical
  exposure is facts, events, and primers.

## Failure scenario

Chunk 60..=99. The model emits four history_segments and five facts, anchoring two of
them to history_segment 4:

```
<facts><PROJECT_RULES>
* [origin_history_segment="4"] The deploy script requires the vault token to be exported first.
* [origin_history_segment="2"] Tests run with --test-threads=1.
</PROJECT_RULES></facts>
```

The anchor prefix is parsed by `split_anchor_prefix` (`:1136-1146`) via
`side_channel_anchor_regex` (`:1295-1303`), which accepts
`[origin_history_segment="4"]`.

Now discard-last fires: four history_segments, lookahead distance 0, so `:555` pops
...
That case is documented (`:585-588`: "A discarded lookahead history_segment
...
from "the run discarded its last history_segment", which today produce identical
...
beyond persisted history_segment count are filtered". Neither asserts observability.

To construct the property:

1. Chunk `60..=99`, four contiguous history_segments, `in_emergency: true` so
...
- Sources examined: `history_summarizer_validate.rs:600-612` (the expression),
  ...
  `:231` (`ValidatedChunk.primer_candidates`, no doc comment), `history_summarizer.rs:96-121`
  (`to_store_primer` area, reached via `source_history_segment`),
  `history_summarizer_prompt.rs` (searched for a primer count constant; `SEED_FLOOR = 4`
...
- Findings: The cap is real, silent, and undocumented. No constant names it, so it
  is not a tunable. The `Vec` type on both `ParsedHistorySegmentOutput.primer_candidates`
...
- Sources examined: all of `history_summarizer_validate.rs:264-1098`, looking for any check
...
- Findings: Three checks compare model output against a known set:
  ...
  persisted history_segment). The first two are ordinal-existence checks that also
