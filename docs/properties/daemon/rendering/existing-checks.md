...
`terse_text_compression.rs` (651), `memory_render.rs` (538), `classify.rs` (490) and
...
decisions, surface strips, the m0/m1 splice, the tail-hygiene metric, terse_text_compression
...
`transform.rs:9927`, `:9972`, `:10154` and 17 sites in `terse_text_compression.rs` (`:180`,
...
4. **The classify byte and chain caps are mirrored by comment only.**
   `packages/plugin/src/features/eidnara/memory_classifier/classify.ts:52` (source-catalog path, not present at HEAD) says
...
- **`terse_text_compression.rs`'s frozen fixture is the seam.** `terse_text_compression-golden.json` is
  generated from `terse_text_compression.ts` and replayed at `:628`, so the oracle is a
...
the metric's `red:` arm (`:508`), terse_text_compression arm (`:526`, via `terse_text_compression_content`
`:422-429`) and absent strip arm are all unreachable from the cross-language
corpus. One case is *named* `terse_text_compression-rendered-not-original-weight`
(`crates/daemon/gen/gen-nudge-hygiene-golden.ts:106-112` (source-catalog path, not present at HEAD)), but it encodes the
terse_text_compression text as literal block content rather than as a `cav:` frozen unit, so it
...
3. **`terse_text_compression.rs` is the closest thing 4e has to a zero-test file: 651 lines, one
   ...
   `packages/plugin/src/hooks/eidnara/terse_text_compression.ts` (source-catalog path, not present at HEAD) can change and only a
   regeneration would notice. No workflow regenerates it. And terse_text_compression output
   feeds the hygiene metric through `terse_text_compression_content`
