...
import type { UnifiedSearchResult } from "@eidnara/opencode/features/context/search";
import * as searchModule from "@eidnara/opencode/features/context/search";
...
import { createTestDb, fakeContext, fakeKernelResolver } from "../__tests__/test-utils";
...
describe("createCtxSearchTool", () => {
    it("rejects an over-cap query with the native isError envelope before any work (AE1)", async () => {
        ...
        try {
            ...
            expect(byteResult.content[0]?.text).toStartWith("Error: query is too large:");
            ...
            expect(atomResult.content[0]?.text).toStartWith("Error: query is too complex:");
    ...
    it("falls through to local search when no object id matches (Pi parity)", async () => {
        ...
        try {
            ...
            expect(result.content[0]?.text ?? "").toContain("Fallback text search hit.");
            expect(calls).toBe(1);
        } finally {
            spy.mockRestore();
            closeQuietly(db);
