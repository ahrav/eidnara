...
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:1493-1502`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:1504-1515`) proves sibling files in one directory do not contend; `symlinked_database_paths_are_refused_never_aliased` (`:1005-1061`) proves both a directory-symlink alias and a file-symlink alias are refused, with and without a live holder. No test covers descriptor disagreement or hardlink aliases; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced DDL, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1699-1719`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:1721-1787`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:1789-1837`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:1839-1884`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`primitives@89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`primitives@89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`primitives@89abb40`) and `a_regressed_positive_epoch_fails_closed` (`primitives@89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`primitives@89abb40`), `a_suppressed_epoch_increment_is_rejected` (`primitives@89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`primitives@89abb40`) protect the lease row and its issuance; `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:1886-1978`) refuses DML, triggers, indexes, views, and renames onto the `fence` and `format_marker` tables; and `a_fenced_callback_cannot_rewrite_the_format_marker` (`:1980-2036`) refuses an update of, or a rename onto, the marker row. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/hard-bust-drains-deferred-work.md). `step_hard` (`crates/cache-stability/src/lib.rs:254-281`) appends `pending_changes` into the rendered set before `apply_units` and clears `reconcile_pending` only when it minted or the anchor is present; `hard_drains_pending_changes_into_the_bust` (`crates/cache-stability/src/lib.rs:430-463`) asserts the drain, the mint, and the cleared flag, and `hard_without_mint_on_absent_boundary_keeps_reconcile_pending` (`crates/cache-stability/src/lib.rs:694-729`) asserts the flag stays set when nothing reanchors.
...
Confidence: high - [evidence](evidence/never-minted-boundary-is-not-reconcile-pending.md). The guard is one expression in `step_defer` (`crates/cache-stability/src/lib.rs:191-217`); `defer_on_never_minted_boundary_is_stable_not_reconcile_pending` (`crates/cache-stability/src/lib.rs:348-381`) covers both the vacuous and the non-vacuous arm.
Existing check: `defer_on_never_minted_boundary_is_stable_not_reconcile_pending`, `defer_boundary_absent_keeps_bytes_and_sets_reconcile_pending` (`crates/cache-stability/src/lib.rs:383-399`); audited at U2.
...
Confidence: high - [evidence](evidence/frozen-unit-order-is-preserved.md). `apply_units` (`crates/cache-stability/src/lib.rs:283-295`) is a find-or-push loop; `soft_replaces_by_key_keeps_slot_appends_new` (`crates/cache-stability/src/lib.rs:465-500`) asserts the resulting key order and the untouched first unit.
...
Confidence: high - [evidence](evidence/episode-units-reset-at-run-boundary.md). `step_defer` (`crates/cache-stability/src/lib.rs:191-217`) retains by `durability_class`; `run_started_keeps_lineage_resets_episode` (`crates/cache-stability/src/lib.rs:611-633`) and `cross_episode_lineage_reproduces_byte_identical` (`crates/cache-stability/tests/golden_vectors.rs:274-320`) cover both classes.
...
Confidence: high - [evidence](evidence/store-schema-identity-matches-the-baseline.md). `ExpectedIdentity::for_baseline` (`crates/storage/src/lib.rs:780-797`), `classify` (`:799-849`), and `apply` (`:851-865`) implement the contract inside `open_sqlite` (`:573-677`); `fresh_file_matches_the_baseline_inventory` (`:1303-1367`) pins the fixture, and `a_file_with_foreign_objects_is_refused_without_mutation` (`:1428-1454`), `a_consumer_baseline_is_applied_once_and_verified_on_reopen` (`:1369-1409`), `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched` (`:1411-1426`), and `negative_database_fence_fails_closed` (`:2082-2110`) cover the refusals.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:379-387`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:464-492`); `GuardedConn` (`crates/storage/src/lib.rs:219-331`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:1663-1686`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:1789-1837`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:2038-2051`) pin the three denials.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:456-461`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1699-1719`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:556-571`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:1688-1697`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:1721-1787`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:573-677`) creates the database owner-only and calls `protect_file` on all three paths before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:1119-1169`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:1571-1599`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:170-216`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:2053-2080`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:1646-1661`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:955-958`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:916-923`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:2082-2110`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:2190-2206`) cover both directions.
