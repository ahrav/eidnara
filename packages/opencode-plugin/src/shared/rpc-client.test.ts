...
function makeTempDir(): string {
    const dir = mkdtempSync(join(tmpdir(), "eidnara-rpc-client-"));
    tempDirs.push(dir);
    return dir;
}
...
describe("EidnaraRpcClient", () => {
    ...
    test("same-process servers keep distinct port files during overlap", async () => {
        ...
        const first = new EidnaraRpcServer(storageDir, directory);
        const second = new EidnaraRpcServer(storageDir, directory);
        await first.start();
