...
- **Failure scenario:** a regenerated fixture passes its own test as long as it keeps schema 3 and eleven vectors; only the diff to the tracked fixture shows a changed contract.
- **Timing window:** none.
- **Instrumentation:** no automated instrumentation. The tests prove that the in-tree core reproduces the fixture; nothing compares the fixture bytes to an in-code expectation. A regenerated fixture is caught only when a reviewer reads its diff.
