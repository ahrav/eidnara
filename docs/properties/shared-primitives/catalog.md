...
Existing check: `superseded_writer_is_fenced_out_after_handover` (`crates/storage/src/lib.rs:3591-3647`), and `superseded_writer_is_rejected_after_reopen` (source at `89abb40`); both use synthetic stale stores and remain **unaudited**.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:73-100,728-904`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:2961-2970`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:2972-2983`) proves sibling files in one directory do not contend; `symlinked_database_paths_are_refused_never_aliased` (`:2461-2528`) proves both a directory-symlink alias and a file-symlink alias are refused, with and without a live holder. `a_hard_linked_database_is_refused` (`:1527-1554`) proves a second name for the database inode is refused through either name. No test covers descriptor disagreement; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:728-904,1409-1432`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return` (`crates/storage/src/lib.rs:2716-2731`), observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:2733-2769`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced DDL, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:3167-3187`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:3189-3255`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:3257-3305`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:3307-3352`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (source at `89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (source at `89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (source at `89abb40`) and `a_regressed_positive_epoch_fails_closed` (source at `89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (source at `89abb40`), `a_suppressed_epoch_increment_is_rejected` (source at `89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (source at `89abb40`) protect the lease row and its issuance; `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:3354-3457`) refuses DML, triggers, indexes, views, and renames onto the `fence` and `format_marker` tables; and `a_fenced_callback_cannot_rewrite_the_format_marker` (`:3459-3515`) refuses an update of, or a rename onto, the marker row. The [durable consumer inventory](durable-consumer-inventory.md) records the consumer reads; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/store-schema-identity-matches-the-baseline.md). `ExpectedIdentity::for_baseline` (`crates/storage/src/lib.rs:1148-1214`), `classify` (`:1275-1325`), and `apply` (`:1327-1359`) implement the contract inside `open_sqlite` (`:727-831`); `fresh_file_matches_the_baseline_inventory` (`:2771-2835`) pins the fixture, and `a_file_with_foreign_objects_is_refused_without_mutation` (`:2896-2922`), `a_consumer_baseline_is_applied_once_and_verified_on_reopen` (`:2837-2877`), `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched` (`:2879-2894`), and `negative_database_fence_fails_closed` (`:3561-3589`) cover the refusals.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:533-544`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:721-763`); `GuardedConn` (`crates/storage/src/lib.rs:283-448`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:3249-3273`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:3375-3425`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:3635-3656`) pin the three denials; the last pins `VACUUM` refused by the read transaction (`cannot VACUUM from within a transaction`) and `PRAGMA incremental_vacuum` denied by the authorizer.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:539-544`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:3167-3187`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:710-725`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:3156-3165`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:3189-3255`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:727-831`) creates the database owner-only and calls `protect_file` on all three paths after `classify` accepts the file and before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:2586-2636`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:3039-3067`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:196-246`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:3532-3559`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:3114-3129`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:1472-1475`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:1433-1440`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:3561-3589`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:3669-3685`) cover both directions.
