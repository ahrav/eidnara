...
| --- | --- | --- |
| H0 test execution in CI | Any workflow job that builds and runs `daemon --lib`, or that runs the `direct_host` or `host_adapter` integration binaries | **No.** Verified across all five files in `.github/workflows/`. The only `daemon` test invocation is `cargo test -p daemon --test lifecycle_cli` (`ci.yml:168` at `76cd6f41`, `:172` at `HEAD`), which selects one integration binary and does not build `--lib`. The step above it is build-only. There is no `--lib`, no `nextest -p daemon`, and no `--workspace` test job. `scripts/test-rust.sh` (`cargo nextest run --workspace`) is wired into root `package.json` and no workflow calls it. `EIDNARA_E2E_MODE: ts` (`ci.yml:714`) keeps Rust out of the host e2e lanes deliberately, stated at `ci.yml:719-721`. This costs a workflow change and no new infrastructure |
