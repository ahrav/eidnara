...
async function probeStatus(connectionFile: string): Promise<WakePlaneStatus> {
    try {
        ...
        // Connection and catalog failures return `unknown` so standalone conditional notes remain on.
        return "unknown";
    }
}
 ...
 * BEFORE the first Rust or LocalEmbeddings demand, and the managed start that follows
 ...
 * Only an affirmative catalog capability disables standalone conditional notes.
 * An unreachable daemon or a catalog without `wake.create` leaves standalone conditional notes enabled.
 */
