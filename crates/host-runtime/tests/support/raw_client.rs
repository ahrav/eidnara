...
//! The oracle derives proof computation from literal values in `docs/host-wire-protocol.md`.
//! The oracle must never call `host-runtime`'s encoders or proof helpers.
...
> {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            let endpoint =
                host_runtime::ring_transport::RingClientEndpoint::attach_with_descriptors(
                    &descriptor,
                    descriptors,
                );
...
fn publish_raw(
    endpoint: &host_runtime::ring_transport::RingClientEndpoint,
    bytes: &[u8],
    ...
    while offset < bytes.len() {
        ...
        let mut reservation = loop {
            match endpoint.to_host.try_reserve(body.len(), header) {
                ...
                Err(shm_transport::backend::ring::ProducerError::Exhausted)
