...
import { scheduleClearAndReindex } from "../../features/context/message-index-async";
import { detectOverflow, type OverflowDetection } from "../../features/context/overflow-detection";
import {
...
} from "../../features/context/transform-decision-log";
import type { ContextUsage, SessionMeta } from "../../features/context/types";
import { captureWindowReport } from "../../features/context/window-report-ledger";
...
export interface EventHandlerDeps {
    ...
    tagger: Tagger;
    // `db` is non-null because the hook disables Eidnara when `openDatabase()` fails.
        ...
        if (input.event.type === "session.error") {
            ...
            try {
                ...
                if (deps.compactionOff) {
                    // Compaction-off: never arm Eidnara emergency recovery — the
                    ...
                    if (
                        ...
                        if (contextLimit < totalInputTokens && !sessionMeta.cacheAlertSent) {
                            ...
                            const delivery = await sendIgnoredMessage(
                                deps.client,
                                info.sessionID,
                                `⚠️ Eidnara: OpenCode reports a context limit of ${formatTokens(contextLimit)} tokens for ${info.providerID}/${info.modelID} but you've successfully sent ${formatTokens(safeTokens)} tokens in this session — the cached limit looks wrong. Restart OpenCode if you suspect this is incorrect.`,
