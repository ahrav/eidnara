...
| foreign-slot-state-on-reserve-is-a-fault-not-backpressure | F2 on the next reusable slot with depth not exhausted, and on a published slot before receive | **Yes** - `foreign_slot_state_on_reserve_is_a_fault_not_backpressure` (`ring.rs:3960`) and `impossible_slot_state_quarantines_the_receiver` (`:4277`) force each state in-process and assert `InvalidSharedState` plus quarantine; no cross-process writer |
