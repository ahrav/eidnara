...
import { getCompartmentsByEndMessageId } from "../../features/context/compartment-storage";
import {
...
import { getTagNumberByMessageId, updateTagStatus } from "../../features/context/storage-tags";
...
function validatePendingTarget(
    ...
    if (compartments.length > 1) {
        ...
        log(
            `[eidnara][${sessionId}] WARNING: ${compartments.length} compartments share endMessageId=${pending.endMessageId} — schema invariant violated; treating as stale`,
        );
        return "compartment-removed";
    }
...
/**
 *
 * OpenCode's `/fork` copies parent eidnara compaction-marker rows without copying persisted marker state.
 ...
 *
 * Reconciliation treats only markers with `eidnara` summary lineage as fork-orphans.
...
export function reconcileForkOrphanedCompactionMarkers(
    ...
    try {
        ...
        if (removed > 0) {
            log(
                `[eidnara][${sessionId}] compaction-marker hygiene: removed ${removed} fork-orphaned marker(s); filterCompacted will now stop at this session's own marker`,
            );
        }
...
export function checkCompactionMarkerConsistency(db: Database): void {
    ...
    try {
        ...
        log(
            `[eidnara] compaction-marker consistency check skipped: ${error instanceof Error ? error.message : String(error)}`,
        );
        return;
    }
    ...
    try {
        ...
        if (reconciledCount > 0) {
            log(
                `[eidnara] compaction-marker consistency: reconciled ${reconciledCount} session(s) with orphaned marker state at startup`,
            );
        }
    } catch (error) {
        log(
            `[eidnara] compaction-marker consistency check failed: ${error instanceof Error ? error.message : String(error)}`,
        );
    } finally {
        try {
