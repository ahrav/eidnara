...
Existing check: `superseded_writer_is_fenced_out_after_handover` (`crates/storage/src/lib.rs:4154-4210`), and `superseded_writer_is_rejected_after_reopen` (source at `89abb40`); both use synthetic stale stores and remain **unaudited**.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:73-100,909-1086`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:3190-3199`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:3201-3212`) proves sibling files in one directory do not contend; `symlinked_database_paths_are_refused_never_aliased` (`:2690-2757`) proves both a directory-symlink alias and a file-symlink alias are refused, with and without a live holder. `a_hard_linked_database_is_refused` (`:1710-1737`) proves a second name for the database inode is refused through either name. No test covers descriptor disagreement; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:909-1086,1591-1614`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return` (`crates/storage/src/lib.rs:2945-2960`), observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:2733-2769`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced DDL, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:3396-3416`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:3418-3484`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:3486-3534`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:3536-3581`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (source at `89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (source at `89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (source at `89abb40`) and `a_regressed_positive_epoch_fails_closed` (source at `89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (source at `89abb40`), `a_suppressed_epoch_increment_is_rejected` (source at `89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (source at `89abb40`) protect the lease row and its issuance; `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:3583-3686`) refuses DML, triggers, indexes, views, and renames onto the `fence` and `format_marker` tables; and `a_fenced_callback_cannot_rewrite_the_format_marker` (`:3688-3744`) refuses an update of, or a rename onto, the marker row. The [durable consumer inventory](durable-consumer-inventory.md) records the consumer reads; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/store-schema-identity-matches-the-baseline.md). `ExpectedIdentity::for_baseline` (`crates/storage/src/lib.rs:1331-1397`), `classify` (`:1458-1508`), and `apply` (`:1510-1542`) implement the contract inside `open_sqlite` (`:909-1014`); `fresh_file_matches_the_baseline_inventory` (`:3000-3064`) pins the fixture, and `a_file_with_foreign_objects_is_refused_without_mutation` (`:3125-3151`), `a_consumer_baseline_is_applied_once_and_verified_on_reopen` (`:3066-3106`), `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched` (`:3108-3123`), and `negative_database_fence_fails_closed` (`:4124-4152`) cover the refusals.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:597-608`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:785-827`); `GuardedConn` (`crates/storage/src/lib.rs:332-507`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:3360-3383`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:3486-3534`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:3746-3766`) pin the three denials; the last pins `VACUUM` refused by the read transaction (`cannot VACUUM from within a transaction`) and `PRAGMA incremental_vacuum` denied by the authorizer.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:690-695`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:3396-3416`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:892-907`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:3385-3394`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:3418-3484`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:909-1014`) creates the database owner-only and calls `protect_file` on all three paths after `classify` accepts the file and before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:2815-2865`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:3268-3296`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:261-311`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:4095-4122`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:3343-3358`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:1655-1658`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:1616-1623`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:4124-4152`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:4232-4248`) cover both directions.
