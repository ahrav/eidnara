...
/// `EIDNARA_MODEL_EXECUTION_CHILD_ENV` makes a ModelExecution child exit plugin initialization before migration, database access, hooks, timers, or RPC startup.
/// `packages/plugin/src/index.ts`.
pub const EIDNARA_MODEL_EXECUTION_CHILD_ENV: &str = "EIDNARA_MODEL_EXECUTION_CHILD";

/// `OPENCODE_MODEL_EXECUTION_AGENT` names the inline-config agent selected by `--agent`.
pub const OPENCODE_MODEL_EXECUTION_AGENT: &str = "model_execution";
...
async fn run_opencode(
    ...
    let args = vec![
        ...
        OsString::from("--agent"),
        OsString::from(OPENCODE_MODEL_EXECUTION_AGENT),
    ...
    child_env.push((
        OsString::from(EIDNARA_MODEL_EXECUTION_CHILD_ENV),
        OsString::from("1"),
    ));
...
mod tests {
    ...
    fn inline_config_neutralizes_opencode_substitution_tokens() {
        ...
        assert_eq!(
            decoded["agent"][OPENCODE_MODEL_EXECUTION_AGENT]["prompt"],
