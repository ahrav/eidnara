//! These tests validate bundle handling and certified-inference conformance for the local_embeddings-tiny fixture.
//! local_embeddings-tiny fixture.
//!
//! Native-inference tests require an ONNX Runtime shared library at `EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY`.
//! Those tests are `#[ignore]`d so a default `cargo test` reports them as ignored rather than passed.
//! Run them with `cargo test -p host-runtime --test local_embeddings_bundle -- --ignored` after setting the variable.
...
use host_runtime::local_embeddings::{
    LocalEmbeddingsComponent, LocalEmbeddingsConfig, LocalEmbeddingsLimits, LocalEmbeddingsStatus,
};
use host_runtime::{CompositeComponent, HostError, SecondaryComponent, StaticComposite};
use sha2::{Digest, Sha256};

use support::local_embeddings::{DeterministicEngine, EchoPrimary, test_lane};

const ORT_LIBRARY_ENV: &str = "EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY";

fn fixture_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/local_embeddings-tiny")
}
...
#[tokio::test]
async fn host_only_platform_reports_exact_local_embeddings_unsupported_state() {
    ...
    assert!(matches!(
        component.status(),
        LocalEmbeddingsStatus::Disabled { reason } if reason == "local_embeddings_unsupported"
    ));
...
fn config_for(dir: &Path, ort: &(PathBuf, String)) -> LocalEmbeddingsConfig {
...
async fn initialize(config: LocalEmbeddingsConfig) -> LocalEmbeddingsComponent {
    let component = LocalEmbeddingsComponent::new(Some(config));
    component
        .initialize()
        .await
...
fn disabled_reason(component: &LocalEmbeddingsComponent) -> String {
...
async fn expect_limits_fail_startup(
    mutate: impl FnOnce(&mut LocalEmbeddingsLimits),
    ...
    assert!(
        matches!(component.status(), LocalEmbeddingsStatus::Disabled { .. }),
...
async fn waiting_query_memory_bound_rejects_both_construction_paths() {
    ...
    let accepted = LocalEmbeddingsLimits {
        max_waiting_queries: BOUNDARY,
        ..LocalEmbeddingsLimits::default()
    };
    host_runtime::local_embeddings::bundle::load_bundle(&fixture_dir(), &accepted, None)
        .expect("the boundary configuration loads through the bundle path");
    LocalEmbeddingsComponent::ready_with_engine(test_lane(), DeterministicEngine::new(), accepted)
    ...
    let error = match LocalEmbeddingsComponent::ready_with_engine(
        test_lane(),
...
fn edit_certified_manifest(dir: &Path, edit: impl FnOnce(&mut serde_json::Value)) {
    edit_manifest(dir, |value| {
        edit(value);
        let manifest: host_runtime::local_embeddings::bundle::BundleManifest =
            serde_json::from_value(value.clone()).expect("edited manifest");
        value["fingerprint"] =
            host_runtime::local_embeddings::bundle::canonical_fingerprint(&manifest).into();
    });
}
...
fn bundles_require_bpe_dropout_to_be_disabled() {
    for dropout in [None, Some(0.0), Some(0.001), Some(0.5), Some(1.0)] {
        ...
        let result = host_runtime::local_embeddings::bundle::load_bundle(
            dir.path(),
            &LocalEmbeddingsLimits::default(),
            None,
        );
...
// ---------------------------------------------------------------------------
// Pre-ORT rejection paths disable LocalEmbeddings before native model construction and do not require an ONNX Runtime library.
// ---------------------------------------------------------------------------
...
async fn unconfigured_component_is_disabled_not_fatal() {
    ...
    let ready = LocalEmbeddingsComponent::ready_with_engine(
        test_lane(),
        DeterministicEngine::new(),
...
fn the_committed_fixture_carries_its_canonical_fingerprint() {
    ...
    let bundle = host_runtime::local_embeddings::bundle::load_bundle(&fixture_dir(), &limits, None)
    ...
    assert_eq!(
        ...
        host_runtime::local_embeddings::bundle::canonical_fingerprint(&bundle.manifest),
        "regenerate the fixture with generate-local_embeddings-tiny.py"
    );
    ...
    assert_eq!(
        bundle.manifest.fingerprint,
        "3e126b1b83f890470fd871a3b65a9d6cc7ad4924b986e703b3f30795f0da9afc",
...
fn a_bundle_manifest_outside_the_committed_digest_does_not_load() {
    ...
    host_runtime::local_embeddings::bundle::load_bundle(&fixture_dir(), &limits, Some(&committed))
    ...
    let Err(error) =
        host_runtime::local_embeddings::bundle::load_bundle(&fixture_dir(), &limits, Some(&other))
    else {
...
async fn limits_incoherent_with_the_manifest_disable_the_lane_before_ort() {
    type IncoherentLimit = (fn(&mut LocalEmbeddingsLimits), [&'static str; 2]);
...
#[tokio::test]
#[ignore = "requires EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY; run with --ignored"]
async fn certified_bundle_loads_and_serves_expected_vectors() {
    ...
    let limits = host_runtime::local_embeddings::EmbeddingInputLimits::of_lane(&lane);
    ...
    assert_eq!(
        ...
        Err(
            host_runtime::local_embeddings::DenseUnavailable::TokenOverflow {
                tokens: host_runtime::local_embeddings::embed_tokens::EmbedTokens::new(10),
                max_tokens: host_runtime::local_embeddings::embed_tokens::EmbedTokens::new(8),
            }
        )
    );
...
#[tokio::test]
#[ignore = "requires EIDNARA_LOCAL_EMBEDDINGS_PRODUCTION_BUNDLE and EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY; run with --ignored"]
async fn production_bundle_from_environment_certifies_offline() {
    let bundle_dir = std::env::var_os("EIDNARA_LOCAL_EMBEDDINGS_PRODUCTION_BUNDLE")
        .map(PathBuf::from)
        .expect(
            "EIDNARA_LOCAL_EMBEDDINGS_PRODUCTION_BUNDLE must name a certified production bundle",
        );
    let ort = ort_library();
    ...
    if let Some(path) = std::env::var_os("EIDNARA_LOCAL_EMBEDDINGS_CAPTURE_CORPUS") {
...
fn a_dropped_activate_keeps_shutdown_waiting_for_the_blocking_load() {
    single_blocking_thread_runtime().block_on(async {
        ...
        copy_fixture_to(dir.path());
        let component =
            LocalEmbeddingsComponent::new(Some(config_for(dir.path(), &pre_ort_identity())));
        component.initialize().await.expect("initialize");
        ...
        tokio::time::timeout(Duration::from_secs(10), component.shutdown())
            ...
            .expect("local_embeddings shutdown returns cleanly");
        ...
        assert!(matches!(
            component.status(),
            LocalEmbeddingsStatus::Disabled { reason } if reason.contains("shut down")
        ));
    });
}
...
fn probe_composite()
-> StaticComposite<EchoPrimary, LocalEmbeddingsComponent, support::StubComponent> {
    StaticComposite::new(
        EchoPrimary,
        LocalEmbeddingsComponent::new(None),
...
fn an_abandoned_activation_holds_the_instance_lock_until_the_blocking_load_stops() {
    single_blocking_thread_runtime().block_on(async {
        ...
        let local_embeddings =
            LocalEmbeddingsComponent::new(Some(config_for(bundle_dir.path(), &pre_ort_identity())));
