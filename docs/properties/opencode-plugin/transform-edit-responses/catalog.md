...
`5def3c71`), executed on 2026-09-13 from `packages/opencode-plugin` with Node 24.18.0 first
on PATH: `bun test src/hooks/context/ src/plugin/messages-transform.test.ts
src/shared/host-client/client.test.ts` reports 1122 pass, 0 fail, 34 files
...
captured source after each ordinal prime, before the wire is built, before a
series restart, before a full-sync retry rebuilds or re-serializes its body,
and at publication; the directory await, the permission await, and each
transport page are ownership fences (`assertCurrentPass()`). The daemon's candidate is not inspected before
...
- Capture: membership and recursive content snapshots, with rechecks after
  ...
  before a full-sync retry rebuilds or re-serializes its body, and at
  publication; ownership fences at the directory await, the permission await,
  and each transport page.
...
Check: `always` - After the first await, source-dependent work uses only `captured.members` (`rust-mode-transform.ts:982`); `recheckCapture` (`:984-992`) runs after each ordinal prime (`:1172`, phase `ordinal:<detail>`), before the wire is built (`:1215`, `wire-build`), before a series restart (`:1353`, `series-restart`), before a full-sync retry rebuilds the wire (`:1395`, `retry-wire-build`) or re-serializes an unchanged body (`:1421`, `full-retry`), and at publication (`:1471`, `publish`). The directory await (`:1049`) and the permission await (`:1090`) are followed by `assertCurrentPass()` only (`:1087`, `:1096`) because every message read happens synchronously before the first await (`:1010-1047`); each transport page is preceded by `assertCurrentPass()` only (`:1299`) because page bodies are frozen text (`:1298`). A failed recheck permits no publication and NACKs every reported delivery, without claiming to undo an earlier dispatch; `state.forceFullWire = true` before the first send (`:1363`) makes the next pass resend the full history.
...
Existing check: `rust-mode-transform.test.ts:2732` "permits no further dispatch or publication when an accessor is installed during preflight" (marker: `started.promise` races the pass on the directory read, then `expect(getter).not.toHaveBeenCalled()` and `expect(calls).toHaveLength(0)`; the change is caught at the `ordinal:attempt=first` recheck); `:2770` "rejects publication when a message is edited in place while the transform response is pending" (marker: `expect(calls).toHaveLength(1)` before the edit; `["transform", "transform.nack"]` after); `:2367` "rejects mutation|supersession|clear|invalidation at the persisted ordinal yield with shared|distinct arrays" (8 cases; marker: `expect(pageSizes).toEqual([MODULE_ORDINAL_PAGE_SIZE])` and `expect(calls).toHaveLength(0)`); `:2454` "preserves a host member|append|rebind|metadata during transport with shared|distinct source" (8 cases; marker: `expect(calls).toHaveLength(1)` before the change; `output.messages` is the current array after and `["transform", "transform.nack"]`); `:2499` "rejects nested accessor|toJSON|proxy installed at source-await|pre-apply without invoking it" (6 cases; markers: `directoryReached` and `expect(methods).toHaveLength(0)`, or `transportProcessedBeforeInstall`); `:2594` "does not dispatch a need_full_sync retry after mutation|supersession|clear|invalidation of the valid first send" (4 cases; marker: `expect(bodies).toHaveLength(2)` with `tail_delta` defined before the fault; `forceFullWire` equals `fault !== "clear"` after); `:2660` "declines before publication and NACKs known deliveries when the source changes between pages" (marker: `bodies.length > 1`, the last body has `transform_page_complete` true, `expect(hook).not.toHaveBeenCalled()`, `output.messages[0]` is `member`, no ACK, and the NACK list equals `delivered`); `:2010` "stops a series restart after reconnect|attempt-mismatch when mutation|accessor|invalidation lands first" (6 cases; marker: one page-zero body and `transform_page_index` 1 pending; the `series-restart` recheck refuses the restart for every fault, so one page-zero body remains, `getterCalls` 0, NACK `["page-zero"]`); `:803` "forces a full send after a dispatched delta pass is source-declined" (marker: `bodies[2].tail_delta` defined and `live[0]` replaced by the fake before the response; `forceFullWire` true, `consecutiveFailures` 0, then `bodies[3]` has no `tail_delta`); `hook.test.ts:360` "rejects source mutation during hook directory lookup before direct transform" (marker: `client.session.get` called once before the getter install; `fake.calls` 0 after).
...
Check: `always` - `stagedMemo` is a charged copy (`rust-mode-transform.ts:1006-1009`, `:1028-1031`); `state.ordinals = stagedMemo` (`:1481`) runs only after `replaceHostArrayContents` (`:1479`); a declined, superseded, cleared, invalidated, or failed pass leaves `state.ordinals` equal to its value at pass start, except that clear and invalidation reset it by their own contract (`:823-829`, `:1541`).
...
Check: `always` - `deliveries.attempted` collects IDs per page response (`rust-mode-transform.ts:1321`), `deliveries.applied` is set only in the synchronous publication block (`:1486`), and `deliverTransformNotes` (`:749-777`) sends NACK for attempted-not-applied and ACK for applied, after the lease release (`:1529-1530`); an ACK error is logged and leaves the published candidate unchanged.
...
Check: `always` - `admit` runs in `run` before any await (`rust-mode-transform.ts:1517-1525`); the capture charge (`:979`), the wire-projection charge (`WIRE_PROJECTION_FACTOR` 4 at `:712`, `:999-1005`), and the memo-copy charge (`:1006-1009`) are reserved before the first await at `:1049`; the walk charges `TAPE_SLOT_BYTES` per retained slot, two bytes per UTF-16 unit of strings and symbol descriptions, the declared array length, and one slot per descriptor read (`transform-capture.ts:9`, `:151-182`, `:247-253`, `:296-297`); candidate slots are charged at `CANDIDATE_SLOT_BYTES` (8, `:729`) inside `buildNativeCandidate` (`:605`, `:620`, `:641`, called with `lease.reserve` at `:1460`), and that slot charge is the only bound on candidate length because `hostArrayReplacementRejection` (`transform-capture.ts:458-474`) takes no length and applies no cap; the lease releases in `.finally` before `deliverTransformNotes` (`:1545-1546`); counts and charges stay within limits and equal a reference model.
...
Confidence: medium - [evidence](evidence/owner-admission-covers-live-captures.md). Every named witness was read and ran green; the byte-pressure test asserts the exact held charge composition, and the model-based test fixes its seed in the failure message. `TransformCaptureAdmission` (`transform-capture.ts:498-571`) and `ReferenceableWalk` (`:141`) no longer validate injected limits; callers pass safe integers. Successful publication transfers `captured.snapshots` (as `rawContentSnapshots`) and the candidate array to the 64-session `wireCaches` owner, and the promoted memo to `state.ordinals` in `states`; `wireCaches` is count-bounded, not byte-bounded, while `states` is a plain `Map` (`:788`) that only `clearSession` deletes from (`:1541`), so ordinal memos of undeleted sessions outlive wire-cache eviction. The optional-output byte budget is TE25 in #538; the `states` bound is a recorded gap, not part of that budget.
Existing check: `rust-mode-transform.test.ts:1953` "admits a full pass over 1,000 realistic messages under the default budget with room for a second session" (marker: `jsonBytes > 2 MiB`; `peakCharge > jsonBytes`, `peakCharge < admission.remainingBytes / 2`, `chargedBytes` 0 after); `:2923` "declines on byte pressure alone while the aggregate charge stays within the budget" (marker: `expect(admission.chargedBytes).toBe(heldCharge)` where `heldCharge` sums capture, 4x wire bytes, annotation, and memo charges; `smallInspection.estimatedBytes > remainingBytes`); `:2874` "keeps every pass within the global count limit and declines without queueing" (marker: `activePasses` 2, `calls` 2, decline log `pass_count`); `:3025` "releases capture admission before the ACK so a paused ACK does not block the next pass" (marker: `ackStarted.promise` race, then `activePasses` 0 and `remainingBytes` 64 MiB); `:3086` "releases rejected capture state before a paused NACK and keeps delivery IDs separate"; `:3170` "shares default admission across factories and cancels the earlier session owner" (marker: `defaultTransformCaptureAdmission.chargedBytes` equals `heldBytes` after the decline); `:2367` (8 cases; `heldBytes > MODULE_ORDINAL_PAGE_SIZE * ORDINAL_ENTRY_RETAINED_BYTES` at the scan yield); `transform-capture.test.ts:889` "keeps counts and charges equal to a reference model across generated lease sequences" (200 seeded sequences of 40 steps; marker: `[activePasses, chargedBytes, detail]` equals the model each step); `:971` "shares the default 64-pass and 64 MiB owner across independent borrowers"; `:998` "rejects invalid charges, isolates accounting, and cannot release a replacement lease"; `:1020` "holds one lease per session, declines a newer call, and cancels the holder"; `:1036` "declines beyond the global pass count without queueing"; `:1046` "charges bytes against one aggregate budget and releases them exactly once"; `:1089` "requires reservation before capture and refuses released or cancelled leases"; `:1166` "accepts the exact byte charge and rejects one byte less"; `hook.test.ts:301` "logs a byte-budget decline at warn before preflight" (marker: `sessionLog.warn` receives `pass declined: capture_bytes`; `client.session.get` not called; `fake.calls` 0); `:396` "admits before the actual hook directory await and never restores a rebound output" (marker: `get` called once, second call declines `session_busy`).
...
Check: `always` - `requestCancel` only aborts the controller (`transform-capture.ts:553-555`, `:568-570`); `release` is idempotent and is the only path that subtracts the charge and frees the session slot (`:556-562`); the transform calls it in `.finally` after `execute` settles (`rust-mode-transform.ts:1529`); cancellation alone leaves the charge held.
...
Reachability: explicit-config-only
Status: active
Exercised: yes - A first `transform` call that throws records exactly one call and no resend; a thrown full retry after `need_full_sync` records no further send; a reconnect or attempt mismatch after an invalidation stops the bounded series restart at the fence, and after a mutation or accessor the `series-restart` recheck refuses the restart before it is sent; positive controls show one restart on attempt mismatch or reconnect alone.
...
Check: `always` - exactly one `transform` call is recorded for a pass whose first call throws a transport error; `sendTransformSeries` rethrows generic errors (`rust-mode-transform.ts:1311-1315`) and only classified attempt-mismatch or reconnect results restart, once (`:1337-1361`).
...
Existing check: `rust-mode-transform.test.ts:3255` "does not resend after an outcome-unknown transport failure and recovers on the next attempt" (marker: `expect(calls).toHaveLength(1)` and `failureCount` 1 after the throw); `:2335` "retains full-sync recovery after a failed retry until a full request publishes" (marker: `bodies` 3 after the thrown full retry; `forceFullWire` true); `:2010` "stops a series restart after reconnect|attempt-mismatch when mutation|accessor|invalidation lands first" (6 cases; marker: one page-zero body before the restart result and still one after it, `getterCalls` 0, no ACK, `failureCount` 0); positive controls `:594` "restarts a paged transform series after a thrown-code|returned-code|message attempt mismatch" (3 cases; two series starts) and `:640` "restarts a paged transform series after a mid-series reconnect".
...
Check: `sometimes` - a declined or failed session's next `run` records a `transform` call after `activePasses` returns to zero or the blocking charge is released; the bound is one attempt after settlement, not an unbounded eventually. Declines do not call `markFailure` (`rust-mode-transform.ts:1501-1510`), so no failure count accrues against the recovering session.
...
Confidence: low - [evidence](evidence/inbound-baseline-independent-of-output-base.md). `buildWireCache` derives `rawContentSnapshots` and fingerprints from `captured.snapshots` and the submitted messages (`rust-mode-transform.ts:263-288`, `:1222-1231`) and `nativeOutput` is attached separately at `:1480`; `state.forceFullWire = true` at `:1363` precedes the first send; that is source evidence, not an equality oracle.
Existing check: `rust-mode-transform.test.ts:3025` "releases capture admission before the ACK so a paused ACK does not block the next pass" (two passes; no delta/full comparison); `:1344` "keeps applied note output when a newer pass starts during the ack" (marker: third body's `tail_delta.native_replace_from` 1); `:1622` "in-place mutation of an older message forces a full send instead of a delta" (input-side guard, not an output-base comparison); `:803` "forces a full send after a dispatched delta pass is source-declined" (marker: `bodies[2].tail_delta` defined, `forceFullWire` true after the decline, `bodies[3]` has no `tail_delta` and carries `native_messages` equal to the new input); `:2594` (4 cases; `forceFullWire` equals `fault !== "clear"`).
