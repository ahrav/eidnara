...
describe("createEidnaraNoteTools", () => {
    ...
    it("routes notes only when the notes domain reports module authority", async () => {
        ...
        const moduleTools = createEidnaraNoteTools({
            resolveProjectPath,
            rustToolBackends: {
        ...
        const preparingTools = createEidnaraNoteTools({
            resolveProjectPath,
            rustToolBackends: {
        ...
        const moduleResult = await moduleTools.eidnara_note.execute(
            { action: "write", content: "module owned note" },
            toolContext(),
        );
        const preparingResult = await preparingTools.eidnara_note.execute(
        ...
        expect(preparingResult).toBe(
            "Error: Rust notes authority is not ready. Write REFUSED and NOT saved; RESEND the same eidnara_note call (action=write) after authority is ready.\nContent to resend:\nnot yet",
        );
    });

    it("refuses when the notes domain reports TS authority", async () => {
        const { requests, note } = recordingNote();
        const tools = createEidnaraNoteTools({
            resolveProjectPath,
            rustToolBackends: {
        ...
        const result = await tools.eidnara_note.execute(
    ...
    it("reports a failed authority probe", async () => {
        ...
        const result = await tools.eidnara_note.execute({ action: "read" }, toolContext());
    ...
    it("returns an error when the project identity cannot be resolved", async () => {
        ...
        expect(result).toBe(
            "Error: Could not resolve project identity for eidnara_note preflight checks.",
        );
        expect(requests).toHaveLength(0);
    });
    ...
    it("reports a deleted session without claiming the module returned an invalid response", async () => {
        ...
        expect(result).toBe(
            "Error: Session was deleted before eidnara_note could run; nothing was read.",
        );
    });
    ...
    it("preserves update arguments in the transition refusal so a retry stays an update", async () => {
        ...
        expect(requests).toHaveLength(0);
        expect(result).toBe(
            'Error: Rust notes authority is not ready. Update REFUSED and NOT applied; RESEND the same eidnara_note call (action=update, note_id=7, surface_condition="when release v2 exists") after authority is ready.\nContent to resend:\nedited body',
        );
    ...
    it("names the dismissed note in a transition refusal", async () => {
        ...
        expect(result).toBe(
            "Error: Rust notes authority is not ready. Dismiss REFUSED and NOT applied; RESEND the same eidnara_note call (action=dismiss, note_id=3) after authority is ready.\nContent to resend:\nsuperseded by #9",
        );
    });
    ...
    it("refuses a conditioned update before reaching the backend when the wake plane is present", async () => {
        ...
        const withContent = await tools.eidnara_note.execute(
            {
                action: "update",
                note_id: 4,
                content: "new body",
        ...
        const conditionOnly = await tools.eidnara_note.execute(
    ...
    it("forwards a conditioned update to the module backend when the wake plane is absent", async () => {
        ...
        await tools.eidnara_note.execute(
            { action: "update", note_id: 4, surface_condition: "when release exists" },
            toolContext(),
        );
    ...
    it("passes the unchanged condition to the module backend when the wake plane is absent", async () => {
        ...
        const { requests, note } = recordingNote("Created conditional note #1.");
        const tools = createEidnaraNoteTools({
            resolveProjectPath,
            rustToolBackends: {
                authorityState: async () => "MODULE",
        ...
        expect(result).toContain("Created conditional note #1");
    });

    it("compiles a fenced MODULE-authority conditional note before the facade write", async () => {
        const { requests, note } = recordingNote({
            content: [{ type: "text", text: "Created conditional note #1." }],
        });
        const tools = createEidnaraNoteTools({
            resolveProjectPath,
            rustToolBackends: {
        ...
        expect(result).toBe(`Created conditional note #1.${conditionCompileReplySuffix(expected)}`);
        expect(result).toContain("Observation compile refused: fenced path");
    });
    ...
    it("refuses a mutation when the host supplies no tool-call id, before any backend call", async () => {
        ...
        const write = await tools.eidnara_note.execute(
            { action: "write", content: "would duplicate on redelivery" },
            noCallId,
        );
        const update = await tools.eidnara_note.execute(
            { action: "update", note_id: 3, content: "changed" },
            noCallId,
        );
        const dismiss = await tools.eidnara_note.execute(
            { action: "dismiss", note_id: 3 },
            noCallId,
        );

        expect(write).toBe(
            "Error: eidnara_note write requires a stable tool-call identity from the host; the note was not written.",
        );
        expect(update).toContain("eidnara_note update requires a stable tool-call identity");
    ...
    it("serves a read without a tool-call id and sends no command id", async () => {
        ...
        const result = await tools.eidnara_note.execute({ action: "read" }, {
            sessionID: "ses-note",
    ...
    it("floors fractional pagination before forwarding so the daemon selects the requested page", async () => {
        ...
        await tools.eidnara_note.execute(
            { action: "read", limit: 2.9, offset: 3.1 },
            toolContext(),
        );
        await tools.eidnara_note.execute({ action: "read", limit: 0, offset: -4 }, toolContext());
    ...
    it("bounds an over-long tool-call id so the daemon accepts it as the command id", async () => {
        ...
        await tools.eidnara_note.execute({ action: "write", content: "bounded id" }, {
            sessionID: "ses-note",
            directory: "/workspace/project-a",
            callID: longCallId,
        } as never);
    ...
    it("defaults to read (not write) when content is an empty string and no action is given", async () => {
        ...
        const { requests, note } = recordingNote(
            "## Notes\n\nNo session notes or conditional notes.",
        );
        const tools = createEidnaraNoteTools({
            resolveProjectPath,
            rustToolBackends: { authorityState: async () => "MODULE", note },
...
describe("eidnara_note raw argument fallback", () => {
    it("returns a tool error for a non-string content instead of throwing", async () => {
        ...
        const conditionResult = await tools.eidnara_note.execute(
...
describe("eidnara_note conditional-note compile root", () => {
    it("resolves a relative file condition against the repository root, not the nested working directory", async () => {
        const root = realpathSync(mkdtempSync(join(tmpdir(), "eidnara-note-root-")));
        try {
