...
`ReceiveLease::wire_header` (`crates/shm-transport/src/lease.rs:163-165`)
hands out 21 bytes with no qualification. But the transport's `validate` reads
only two of those bytes' worth of meaning
(`crates/shm-transport/src/descriptor.rs:289-297`): the little-endian `len`
...
One receive is `receive_one` (`crates/host-runtime/src/ring_transport.rs:455-534`,
...
3. `:473` `validate_inbound_header(header)?`
   (`crates/host-runtime/src/frame_channel.rs:58-76`).
...
(`crates/host-runtime/src/tcp_frame_channel.rs:188`, `:196`, `:198` at `9c1eb4d1`) is
gone: `ed487e11` deleted `tcp_frame_channel.rs`, so `receive_one` is now the only
consumer read path in `crates/host-runtime/src`.
...
(`packages/shm-native/src/lib.rs:980`) with no native validation at all, and
the decode happens in TypeScript at
`packages/plugin/src/shared/host-client/shm-frame-channel.ts:305` through
...
allocation. `crates/host-runtime/tests/shm_failure_modes.rs:195`
