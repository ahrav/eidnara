...
- `packages/shm-native/src/lib.rs:859-935` `produce` — reserves with the caller's wire header (`:883-890`),
...
- `packages/shm-native/src/lib.rs:1042-1045` — the same ordering on the two-phase `commit_reservation` entry
  point: `before_publish.call(())?` then `reservation.commit(written as usize)`.
- `packages/plugin/src/shared/host-client/shm-frame-channel.ts:289-321` `publishFrame` —
...
- `crates/host-runtime/src/ring_transport.rs:560-602` `publish_one` — the host's ordering is the opposite: the publish
...
- `crates/shm-transport/src/backend/ring.rs:1769-1811` `commit` — five failure branches, all aborting the
...
- Existing check, **corrected and re-anchored at post-#131 HEAD**: the catalog cites
  `packages/shm-native/tests/runtime.ts:108-127`.
...
- Sources examined: `packages/plugin/src/shared/host-client/shm-frame-channel.ts:289-321`;
  `packages/shm-native/src/lib.rs:859-935` and `:1017-1046`; `crates/host-runtime/src/ring_transport.rs:560-602`;
  `crates/shm-transport/src/backend/ring.rs:1769-1811`, `:1577-1627`;
  `packages/shm-native/tests/runtime.ts:109-128`.
