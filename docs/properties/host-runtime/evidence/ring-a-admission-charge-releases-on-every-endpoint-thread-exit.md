...
Part 1's two custody records rested on `crates/host-runtime/src/provider_recovery.rs`,
...
`crates/host-runtime/src/ring_transport.rs:223`. On failure it increments
...
`Admission::release` (`crates/shm-transport/src/profile.rs:512-515`) consumes
...
`docs/shm-transport.md:49`, "Joined endpoint teardown returns its
...
last has no `host-runtime` caller at all (see
...
`crates/shm-transport/tests/contract.rs:472` covers `Admission::release` at
...
- Missing evidence: whether the removal of a recovery driver was deliberate.
  `docs/shm-transport.md:49` still describes recovery-shaped behaviour
  ("Native aliases whose detachment fails keep their channel and mapping alive
  until cleanup succeeds"), and nothing in `host-runtime` implements that retention.
