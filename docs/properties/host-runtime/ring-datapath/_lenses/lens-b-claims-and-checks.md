...
| 4 of 24 integration binaries named | confirmed; `ls crates/host-runtime/tests/*.rs` returns 24 |
| every `-p host-runtime` run carries a `--test` filter | confirmed; the 13 `host-runtime` hits are `:87`, `:132`, `:133`, `:134`, `:168`, `:169`, `:178`, `:187`, `:190`, `:211`, `:361`, `:442`, `:461`, and none is an unfiltered or `--lib` run |
| clippy deliberately absent | confirmed; the reason comment is at `:481-483` and the `check-rust` job (`:463`) runs only `cargo fmt --check` (`:485`) and `cargo check -p context-core --no-default-features` (`:492`). The re-scope's `:481-493` is the enclosing block |
...
functions, but `cargo test -p host-runtime --doc` (`:190`) *does* build and run the
...
- **`Admission::quarantine` has zero `host-runtime` callers.** Confirmed. The function
  is `crates/shm-transport/src/profile.rs:568` (with the private
  `AdmissionController::quarantine` at `:522`). A repository-wide grep for
  `quarantine` under `crates/host-runtime/` returns only: the unrelated
...
`docs/host-wire-protocol.md:28` ("Production application transport is the
...
`docs/host-wire-protocol.md:563` ("No setup or runtime failure changes
...
The *producing* half is **NOT FOUND**: no `host-runtime` call site reaches
...
lists all five, and `host-lifecycle/policy.ts:869`, `:871` maps two into
...
(`shm-transport/src/profile.rs:180-203`) doubles descriptors and leases for
...
`MIN_ARENA_BYTES` is `64 * 1024 * 1024` (`shm-transport/src/arena.rs:4-6`),
...
teardown is orderly connection close"), `docs/shm-transport.md:49`
...
`packages/plugin/src/shared/host-client/shared-memory-failure.ts:10-30`. Worse
...
draws (`docs/shm-transport.md:26-36`, `Attached --> Active: activation
...
an absence of looking.** `docs/shm-transport.md` was audited line by
...
`docs/host-wire-protocol.md:583-585` explicitly states "There is no provider
...
one did, since `scripts/check-shm-architecture.ts:34` matches those names in
...
1. **The architecture gate is a text grep, and it is the only thing keeping the
   deleted transports deleted.** `scripts/check-shm-architecture.ts` reads
   ...
   (`:48`) and never walks `crates/host-runtime/tests/`, so a test could reintroduce
   ...
   gate also pins the continued existence of `packages/shm-native/index.ts`
   and the five source roots.
2. **`RING_PROFILE` is a string literal shared across three languages by copy.**
   `ring_transport.rs:31` is `"host-test-ring-v1"`;
   `packages/shm-native/src/lib.rs:27` restates it as `PROFILE`;
   `packages/shm-native/index.ts:8` restates it as
   `QUALIFIED_TEST_PROFILE`. Three definitions, no cross-check. The value is the
   release identity `docs/shm-transport.md:11` fixes.
3. **`HEADER_LEN` and `MAX_FRAME_BODY_LEN` are re-declared in TypeScript as
   ...
   `docs/host-wire-protocol.md` §6 as its authority in prose. No test parses
...
5. **`DESCRIPTOR_DEPTH = 8` doubles into the documented 16 through a derivation
   ...
   `shm-transport/src/profile.rs:185-188`, which multiplies by 2. The doc
   states the post-doubling number (`docs/shm-transport.md:77`).
   Nothing in `host-runtime` asserts the derived charge vector, so a change to the
...
6. **The `ReadClose` enum carries `#[allow(dead_code)]`, which suppresses the
   ...
   `crates/host-runtime/src`: `Io` (`frame_channel.rs:44-45`, consumed at
...
`cargo test -p host-runtime --doc` (`ci.yml:190`, step "Rust lease non-escape").
...
(`host_runtime::run_with_publish_hook`), against the production hook installer
...
| --- | --- | --- | --- |
| Plugin shared-memory contracts | `bun test packages/plugin/src/shared/host-client` | `:211` | `protocol.test.ts` (32 tests) pins the 21-byte header, frozen-prefix decode order and the 64 MiB cap against literals in `protocol.ts:14`, `:18`. `shm-frame-channel.test.ts` (15) covers the nine terminal-class mappings at `:49-57` |
| Architecture audit | `bun test scripts/check-shm-architecture.test.ts` and `bun run check:shm-architecture` | `:55`, `:58` | the deleted-transport grep gate of claim C1 |
...
| Native behaviour | `bun run --cwd packages/shm-native test:bun` and `test:node` | `:196`, `:203` | the peer half's mechanism gate, under `EIDNARA_SHM_NATIVE_CLAIMED_TARGET: "1"` |
...
`packages/shm-native/shm_native.node` and asserts its absence. Verified:
...
`packages/shm-native/index.ts:189-191` rejects a non-release addon with
`NativeStartupError("debug_build")`, so the shipped host and the shipped addon do
not share a profile assumption. Which profile the distributed `host-runtime` binary
...
1. **The whole semantic contract suite runs nowhere but a developer's laptop.**
   ...
   `host-runtime` lib test target. This is the quietest thing in the sub-part not
...
3. **Nothing publishes a maximum-size frame through a real ring.** The 64 MiB
   ...
   (`ring_transport.rs:48`, `shm-transport/src/arena.rs:4-6`), so one in-flight
...
8. **`macos-15-intel` builds this code and runs none of its tests.** The
   ...
   `host-runtime` lib target on any platform. Recorded because the platform contract
   (`docs/shm-transport.md:85`) makes macOS ring behaviour the one
   explicitly unresolved release risk, and the checks that would exercise it are
   the unexecuted in-crate ones.

## Open questions

- Does the distributed `host-runtime` artifact ship debug or release? It decides
  ...
  (`packages/shm-native/index.ts:189-191`), so the two halves of one
...
- Is the ring `default-production` or `explicit-config-only`? Carried forward
  ...
  `"host-test-ring-v1"` (`:31`) is what
  `docs/shm-transport.md:11` names as the *release* identity. A
