...
import { EIDNARA_REDUCE_DESCRIPTION } from "@eidnara/opencode/tools/eidnara-reduce/constants";
...
type EidnaraReduceParams = Static<typeof ParamsSchema>;
...
export function createEidnaraReduceTool(
    ...
    return {
        ...
        async execute(toolCallId, params: EidnaraReduceParams, signal, _onUpdate, ctx) {
            ...
            try {
                ...
                if (record === null || record.ok !== true) {
                    ...
                    return err(`Error: Failed to queue eidnara_reduce operations. ${message}`);
                }
                ...
                return err(
                    `Error: Failed to queue eidnara_reduce operations. ${getErrorMessage(error)}`,
                );
            }
        },
    };
