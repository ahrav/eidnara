...
describe("doctor OpenCode plugin cache", () => {
    it("clears stale @latest cache when cached plugin is older than npm latest", async () => {
        const cacheRoot = makeTempDir("eidnara-opencode-cache-");
...
describe("doctor OpenCode helper logic", () => {
    it("treats dist-tags like @next and @beta as pinned plugin entries", () => {
        expect(isPinnedOpenCodePluginSpecifier("@eidnara/opencode@next")).toBe(true);
        expect(isPinnedOpenCodePluginSpecifier("@eidnara/opencode@beta")).toBe(true);
        expect(isPinnedOpenCodePluginSpecifier("@eidnara/opencode@0.29.1")).toBe(true);
        expect(isPinnedOpenCodePluginSpecifier("@eidnara/opencode")).toBe(false);
        expect(isPinnedOpenCodePluginSpecifier("@eidnara/opencode@latest")).toBe(false);
    });

    it("honors NPM_CONFIG_USERCONFIG before HOME for npmrc release-age warnings", () => {
        const root = makeTempDir("eidnara-npmrc-");
