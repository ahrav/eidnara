//! This module provides a deterministic scriptable backend and a real-loopback composite host for ModelExecution tests.
//! The composite host uses a real loopback with a `ModelExecutionComponent` tertiary.
...
use host_runtime::model_execution::{MODEL_EXECUTION_MODULE_ID, ModelExecutionComponent};
...
/// Real-loopback host whose tertiary is the supplied ModelExecution component, with
/// the fixed direct-profile catalog shape (context, local_embeddings, model_execution).
///
/// The host requires Linux because `support::local_embeddings::EchoPrimary` is Linux-gated and LocalEmbeddings ships only for `linux-x64-gnu`.
#[cfg(target_os = "linux")]
pub async fn start_model_execution_host(
    component: ModelExecutionComponent,
) -> super::CompositeTestHost {
    let composite = StaticComposite::new(
        super::local_embeddings::EchoPrimary,
        super::StubComponent::new("local_embeddings", "management_surface"),
        component,
    )
...
pub async fn open_model_execution_route(
    ...
    client
        ...
        .await
        .expect("model_execution route binds")
}
