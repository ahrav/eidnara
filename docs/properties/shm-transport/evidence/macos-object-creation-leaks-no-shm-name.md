...
`crates/shm-transport/src/backend/ring.rs:1748-1783` runs, in order:
...
`crates/shm-transport/src`, at `:1779`. The contrast is `RuntimeDir`, which
...
No CI job executes any of this. The macOS step (`the source repository `ci.yml` workflow:169-176`)
...
- Sources examined: `git show a5568707 -- crates/shm-transport/src/backend/ring.rs`
  ...
  `the source repository `ci.yml` workflow:158-175`.
...
- Sources examined: `ring.rs:2176-2219` at HEAD `bdf72f46a` (`getrandom`
  ...
  site, `:2209`); `packages/` listing at HEAD; `the source repository `ci.yml` workflow` at
  HEAD; `git log --diff-filter=D` on the Darwin package manifests
  (`55f47ac64`, merged via `5d638e3e8`); `docs/shm-transport.md:5`,
  `:83`.
- Findings: `099a314d5` rewrote the body — the record's earlier "byte-identical
  ...
  compile-errors on non-Linux targets. `docs/shm-transport.md:5`
...
- Conclusion: no. There is no `shm_open`/`shm_unlink` window in this tree, and `memfd_create` objects are anonymous. The record is marked `invalidated` in the catalog; the source-tree line references in the trail above resolve against the host repo at `bdf72f46a` and `e447c927`, not this tree.
