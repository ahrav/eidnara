...
caller. `eidnara_reduce` is the first of the two, and the answer is that assembly
cannot mask it because assembly never sees it. The masking happens in the
handler's own choice of text and `isError` value.

## Evidence trail

`crates/daemon/src/lib.rs:10482-10588`, `handle_eidnara_reduce_facade`.
...
`handle_eidnara_reduce_facade` records that an acknowledgement was issued, so the
...
- `eidnara_reduce` does not consult `memory_enabled`, unlike `eidnara_memory`
  (`:10608-10610`), so the `config.rs:124` default is irrelevant to this path.
- The plugin's `eidnara-reduce-availability.ts:82` and `reclaim-protection.ts:15`
...
2. Call `eidnara_reduce` with a `drop` range that has at least one queueable tag.
...
- Sources examined: `lib.rs:10585-10587`, the comment and the response;
  ...
  `packages/plugin/src/hooks/eidnara/eidnara-reduce-availability.ts:82` (source-catalog path, not present at HEAD) and
