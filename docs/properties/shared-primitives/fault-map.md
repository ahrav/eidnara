...
| A lease directory reached through the running user's own final symlink | `at-most-one-exclusive-holder-per-key`; accepted, since the component check owns the link and the ownership and mode checks apply to the directory it names (`a_lease_directory_behind_the_running_users_final_symlink_is_accepted`) |
| A permissive umask when the lease root does not yet exist | `at-most-one-exclusive-holder-per-key`; `create_lease_root` creates the root and its missing ancestors `0700` regardless of the umask, so the acquisition is not refused by its own directory check (`a_fresh_lease_root_is_owner_only_under_a_permissive_umask`) |
| A rename or delete of a Windows lease file while a holder's handle is open | `at-most-one-exclusive-holder-per-key`; the lease handle is opened without `FILE_SHARE_DELETE`, so the operation fails with a sharing violation for the guard's lifetime; compile-checked for `x86_64-pc-windows-gnu`, not exercised |
| An opener superseded between inspection and claim | `stale-writer-write-is-rejected`; on an initialized file the fence row is read before `journal_mode = WAL` is pinned and an epoch at or below it is refused, so the opener leaves the journal mode as it found it (`a_stale_epoch_is_refused_before_the_journal_mode_is_pinned`) |
| A descriptor field carrying U+001F, the lease-key separator | `distinct-lease-keys-do-not-alias`; refused as `InvalidInput` before any lease or database file exists instead of reaching the panicking key encoder (`a_descriptor_field_holding_the_lease_separator_is_refused_without_panicking`) |
| A temp object left under a main-schema name before a callback runs | `protected-write-set-is-fence-complete`; `CallbackScope::install` refuses the scope while `temp.sqlite_schema` holds a name from the main schema, so no fenced write commits into the shadow and no read answers from it (`a_preexisting_temp_shadow_refuses_callbacks_until_it_is_dropped`) |
| A live store's file read while its owner checkpoints | `store-schema-identity-matches-the-baseline`; the inspection runs under a shared lease, which the owner's exclusive lease refuses, so the opener returns `Lease(Held)` instead of classifying a half-written copy (`a_live_holder_is_reported_before_the_file_is_inspected`) |
| A symbolic link or junction on a component of a Windows lease directory path | `at-most-one-exclusive-holder-per-key`; `require_no_reparse_point_components` refuses the directory, since a holder's handle pins the final file and not the entry an ancestor link resolves through; compile-checked for `x86_64-pc-windows-gnu`, not exercised |
