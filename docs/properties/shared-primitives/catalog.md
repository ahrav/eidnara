...
Confidence: high - [evidence](evidence/distinct-lease-keys-do-not-alias.md). High that the separator is rejected in every field (`LeaseKey::identity` (`crates/lease/src/lib.rs:168-194`)); high that collision handling is absent; low on practical targeted-FNV cost.
...
Existing check: `superseded_writer_is_fenced_out_after_handover` (`crates/storage/src/lib.rs:2499-2555`), and `superseded_writer_is_rejected_after_reopen`, `primitives@89abb40`; both use synthetic stale stores and remain **unaudited**.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:62-74,617-722`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:1869-1878`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:1880-1891`) proves sibling files in one directory do not contend; `symlinked_database_paths_are_refused_never_aliased` (`:1381-1437`) proves both a directory-symlink alias and a file-symlink alias are refused, with and without a live holder. `a_hard_linked_database_is_refused` (`:1152-1179`) proves a second name for the database inode is refused through either name. No test covers descriptor disagreement; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:617-722,1046-1061`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return` (`crates/storage/src/lib.rs:1625-1640`), observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:1642-1677`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced DDL, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:2075-2095`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:2097-2163`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:2165-2213`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:2215-2260`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`primitives@89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`primitives@89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`primitives@89abb40`) and `a_regressed_positive_epoch_fails_closed` (`primitives@89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`primitives@89abb40`), `a_suppressed_epoch_increment_is_rejected` (`primitives@89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`primitives@89abb40`) protect the lease row and its issuance; `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:2262-2365`) refuses DML, triggers, indexes, views, and renames onto the `fence` and `format_marker` tables; and `a_fenced_callback_cannot_rewrite_the_format_marker` (`:2367-2423`) refuses an update of, or a rename onto, the marker row. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/defer-pass-replays-frozen-bytes-verbatim.md). `step_defer` (`crates/cache-stability/src/lib.rs:205-231`) never reads `rendered_units` and mutates no frozen unit; the run-boundary filter in `CoreState::step` (`crates/cache-stability/src/lib.rs:175-203`) is the only path that removes units on a defer. `defer_does_not_mutate_frozen_bytes_or_render` (`crates/cache-stability/src/lib.rs:343-364`), `defer_boundary_absent_keeps_bytes_and_sets_reconcile_pending` (`crates/cache-stability/src/lib.rs:405-423`), and `run_started_keeps_lineage_resets_episode` (`crates/cache-stability/src/lib.rs:666-688`) pin the three arms, and every golden vector compares `cached_prefix_bytes()` after each pass.
...
Confidence: high - [evidence](evidence/hard-bust-drains-deferred-work.md). `step_hard` (`crates/cache-stability/src/lib.rs:268-297`) appends `pending_changes` into the rendered set before `apply_units` and clears `reconcile_pending` only when it minted or the anchor is present; `hard_drains_pending_changes_into_the_bust` (`crates/cache-stability/src/lib.rs:477-510`) asserts the drain, the mint, and the cleared flag, and `hard_without_mint_on_absent_boundary_keeps_reconcile_pending` (`crates/cache-stability/src/lib.rs:749-788`) asserts the flag stays set when nothing reanchors.
...
Confidence: high - [evidence](evidence/never-minted-boundary-is-not-reconcile-pending.md). The guard is one expression in `step_defer` (`crates/cache-stability/src/lib.rs:205-231`); `defer_on_never_minted_boundary_is_stable_not_reconcile_pending` (`crates/cache-stability/src/lib.rs:366-403`) covers both the vacuous and the non-vacuous arm.
Existing check: `defer_on_never_minted_boundary_is_stable_not_reconcile_pending`, `defer_boundary_absent_keeps_bytes_and_sets_reconcile_pending` (`crates/cache-stability/src/lib.rs:405-423`); audited at U2.
...
Confidence: high - [evidence](evidence/anchor-holds-while-reconcile-pending.md). The guard in `step_soft` (`crates/cache-stability/src/lib.rs:233-266`) is a let chain over both conditions; `soft_does_not_advance_anchor_while_reconcile_pending` (`crates/cache-stability/src/lib.rs:623-664`) and `coverage_extending_soft_advances_anchor_keeps_m0_frozen` (`crates/cache-stability/src/lib.rs:549-621`) pin both outcomes.
...
Confidence: high - [evidence](evidence/frozen-unit-order-is-preserved.md). `apply_units` (`crates/cache-stability/src/lib.rs:299-311`) is a find-or-push loop; `soft_replaces_by_key_keeps_slot_appends_new` (`crates/cache-stability/src/lib.rs:512-547`) asserts the resulting key order and the untouched first unit.
...
Confidence: high - [evidence](evidence/episode-units-reset-at-run-boundary.md). `CoreState::step` (`crates/cache-stability/src/lib.rs:175-203`) retains by `durability_class` before dispatching any action; `run_started_keeps_lineage_resets_episode` (`crates/cache-stability/src/lib.rs:666-688`) and `cross_episode_lineage_reproduces_byte_identical` (`crates/cache-stability/tests/golden_vectors.rs:274-322`) cover both classes.
...
Confidence: high - [evidence](evidence/store-schema-identity-matches-the-baseline.md). `ExpectedIdentity::for_baseline` (`crates/storage/src/lib.rs:860-926`), `classify` (`:928-978`), and `apply` (`:980-1012`) implement the contract inside `open_sqlite` (`:617-722`); `fresh_file_matches_the_baseline_inventory` (`:1679-1743`) pins the fixture, and `a_file_with_foreign_objects_is_refused_without_mutation` (`:1804-1830`), `a_consumer_baseline_is_applied_once_and_verified_on_reopen` (`:1745-1785`), `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched` (`:1787-1802`), and `negative_database_fence_fails_closed` (`:2469-2497`) cover the refusals.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:379-387`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:492-536`); `GuardedConn` (`crates/storage/src/lib.rs:219-331`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:2039-2062`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:2165-2213`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:2425-2438`) pin the three denials.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:456-461`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:2075-2095`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:600-615`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:2064-2073`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:2097-2163`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:617-722`) creates the database owner-only and calls `protect_file` on all three paths after `classify` accepts the file and before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:1495-1545`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:1947-1975`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:170-216`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:2440-2467`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:2022-2037`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:1102-1105`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:1063-1070`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:2469-2497`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:2577-2593`) cover both directions.
