...
Context-application protocol: 2
...
The context application protocol rides on the daemon's routed request envelope (`method`, `v: 1`, `session_id`, `project_root`) and adds three method literals, one closed outcome vocabulary, one capability gate, and one accounting-profile binding. The literals below change only by an edit to this section, which replaces the prior text.
