...
import { type ConditionalNoteResolver, guardedConditionalNoteHttpGet } from "./ssrf-guard";
import { ConditionalNoteNetworkError, conditionalNoteAbortError } from "./types";
...
export interface ConditionalNoteCapabilityApi {
...
export type ConditionalNoteCapabilityFactory = (
    signal: AbortSignal,
) => ConditionalNoteCapabilityApi;

export interface ConditionalNoteCapabilitiesOptions {
    projectRoot: string;
    signal: AbortSignal;
...
export function createConditionalNoteCapabilities(
    options: ConditionalNoteCapabilitiesOptions,
): ConditionalNoteCapabilityApi {
    ...
    return {
        ...
        httpGet: (url) =>
            guardedConditionalNoteHttpGet(url, {
...
async function guardedReadFile(
    ...
    const abort = new Promise<never>((_, reject) => {
        onAbort = () => reject(conditionalNoteAbortError(signal));
...
async function closeLateOpenOnAbort(
    ...
    throw conditionalNoteAbortError(signal);
}
    ...
    try {
        ...
        if (signal.aborted || failure.signal === "SIGTERM") {
            throw new ConditionalNoteNetworkError(
                "CONDITIONAL_NOTE_NETWORK: git command timed out or aborted",
            );
        }
        ...
        if (typeof failure.code === "string") {
            throw new ConditionalNoteNetworkError(
                `CONDITIONAL_NOTE_NETWORK: git unavailable: ${failure.code}`,
            );
        }
        const stderr = typeof failure.stderr === "string" ? failure.stderr : "";
        if (await isExpectedGitMiss(projectRoot, stderr)) return "";
        const reason = stderr.split("\n")[0]?.trim() || `exit status ${String(failure.code)}`;
        throw new ConditionalNoteNetworkError(`CONDITIONAL_NOTE_NETWORK: git failed: ${reason}`);
    }
}
