...
import { replaceAllCompartmentState } from "../features/context/compartment-storage";
...
import { FORK_MIGRATION_VERSION_FLOOR } from "../features/context/migrations";
import {
...
import { estimateTokens } from "../hooks/context/read-session-formatting";
...
describe("buildSidebarSnapshot — stale build error state", () => {
    test("surfaces the persisted stale-build failure in the sidebar snapshot", () => {
        ...
        try {
            ...
            ).run(sessionId, "Eidnara: plugin build is older than its database — restart OpenCode");
