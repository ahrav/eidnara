...
- `publish_output_from_awaiting` persists the `Publishing` state, captures its
  ...
  `crates/daemon/src/historian.rs:1624-1647`;
- `publish_validated_chunk` builds `HistorianPublishRequest` and dispatches
  ...
  `MemoryStore::publish_historian_chunk`, at `historian.rs:431-434` and `:500-517`;
- both production fence implementations call the same store method at
  `crates/daemon/src/lib.rs:3245-3268` and `:3280-3305`.

`MemoryStore::publish_historian_chunk` runs one fenced transaction
...
historian tests at `daemon/src/historian.rs:1731-1732`, transform tests at
