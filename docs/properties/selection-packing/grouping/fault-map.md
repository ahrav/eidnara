...
| Reversed, empty, overflowing, past-whole-buffer-sibling, disagreeing, or UTF-8-splitting span | `damaged_span` rows. | `packing.grouping.ungroupable_span_presented` | packing-coverage-is-a-per-identity-partition |
| Cost list 11, 4, 6 against remaining 10; through the daemon entry, rendered costs big, small, medium against small plus medium | `skip_and_continue` and the daemon entry with `byte_profile()`. | `packing.scan.non_fitting_group_before_fitting_groups` | packing-optional-scan-skips-and-continues |
...
| Each optional bound one below the set, or a request count one past the kernel batch cap | `OptionalBounds` fixture values; unpersisted request identifiers past the cap. | `packing.optional.bound_at_limit_plus_one_presented` | packing-optional-bounds-refuse-at-limit-plus-one |
| Optional occurrence duplicated, missing, stale, kernel-excluded, or corrupt | A repeated request, an unpersisted request, a revision-2 request, a tombstoned row, an unseeded object, a payload row whose bytes were altered in the store. | `packing.optional.excluded_optional_presented` | packing-coverage-is-a-per-identity-partition |
