...
`retrieval.confirm` carries `preparation_id`, `forwarded_identity`, `applied_identity` (string or null), and `outcome`. `outcome` is exactly one of `keep`, `append`, `applied_replacement`, `preparation_failure`; `unknown` is a receipt state and is never accepted as an outcome. A confirm whose `applied_identity` equals the recorded forwarded identity completes the receipt; a confirm without an applied identity leaves it `unknown` with the forwarded identity still recorded; a `forwarded_identity` other than the recorded one, an applied identity other than the recorded one, a confirm of a preparation that was never forwarded, or a second acknowledgment with another outcome is `conflict`. For a key of another daemon incarnation the daemon holds no record: a confirm whose `forwarded_identity` and `applied_identity` are equal 64-character lowercase hex identities completes and is recorded; anything else leaves `unknown`. An empty replacement is `applied_replacement`. A daemon receipt alone never yields a complete state: only a confirm carrying the applied identity does.

Receipt state is in memory, bounded by a key count enforced when a key is minted and a retention measured from creation that MUST be at least the longest supported retry path, and cleared by a daemon restart; a limits change keeps the receipts.
