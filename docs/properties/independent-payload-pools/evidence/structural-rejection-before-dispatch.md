...
- `crates/host-runtime/src/ring_transport.rs:1003`

Witness status: partial - `crates/shm-transport/src/backend/ring.rs:2540` covers descriptor and header structure; host header validation stays in `validate_inbound_header` tests in `crates/host-runtime/src/frame_channel.rs`.
