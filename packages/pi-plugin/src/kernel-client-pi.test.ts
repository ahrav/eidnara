...
describe("isolated Pi session token caches", () => {
    it("evicts the least-recently-resolved root's tokens past the project cap", () => {
        ...
        for (let i = 0; i <= MAX_ISOLATED_TOKEN_CACHE_PROJECTS; i += 1) {
            ...
            tokens.rememberTokens(root(i), [{ object_id: OBJECT_ID, known_as_of: i + 1 }], i + 1);
        }
    ...
    it("isolating an already isolated session keeps its accumulated tokens", () => {
        ...
        expect(tokens.knownAsOfFor(root(0))).toBe(7);
    });
});
