...
- **Primary evidence:** `decode_fence_epoch` (`crates/storage/src/lib.rs:1472-1475`) uses `u64::try_from` and maps failure to `FenceCorrupt`; `fence_epoch_sql_value` (`crates/storage/src/lib.rs:1433-1440`) uses `i64::try_from` before any statement runs.
- **Existing evidence:** `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:3561-3589`) writes `-1` without the `CHECK` constraint and asserts the open fails and the row is unchanged; `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:3669-3685`) constructs a store at `i64::MAX + 1` and asserts the fenced write fails with the range message.
