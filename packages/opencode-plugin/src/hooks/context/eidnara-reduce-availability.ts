 ...
 *
 * The resolver keeps the `eidnara_reduce` verdict frozen when live permissions deny the tool because the verdict gates guidance and the system-prompt hash.
 * Changing the eidnara_reduce verdict mid-session would invalidate the provider prefix even though permission changes do not alter the prompt.
...
let eidnaraReduceRegisteredGlobally = true;

/**
 * `resolveEidnaraReduceAvailability*` returns a frozen `callable: false` verdict when `eidnara_reduce` is not registered globally.
 */
export function setEidnaraReduceRegisteredGlobally(registered: boolean): void {
    eidnaraReduceRegisteredGlobally = registered;
}

/**
 * */
export function resetEidnaraReduceRegisteredGloballyForTest(): void {
    eidnaraReduceRegisteredGlobally = true;
}

export type EidnaraReduceAvailabilityVerdict = ToolAvailabilityVerdict;

const EIDNARA_REDUCE_TOOL = "eidnara_reduce";
const TODOWRITE_TOOL = "todowrite";

/**
 * Verdicts are cached independently for each `(tool, session)` pair.
 * The 1,000-entry cap supports at most 500 sessions when both `eidnara_reduce` and `todowrite` have entries.
 */
...
const eidnaraReducePermissionDenyLogged = new BoundedSessionMap<boolean>(1000);
...
export function resolveEidnaraReduceAvailability(
    ...
    return resolveToolAvailability(sessionId, EIDNARA_REDUCE_TOOL);
}

export function clearEidnaraReduceAvailability(sessionId: string): void {
    clearToolAvailability(sessionId, EIDNARA_REDUCE_TOOL);
}
...
export function clearToolPermissionDenied(sessionId: string): void {
    ...
    eidnaraReducePermissionDenyLogged.delete(sessionId);
}

export function hasLoggedEidnaraReducePermissionDeny(sessionId: string): boolean {
    return eidnaraReducePermissionDenyLogged.get(sessionId) === true;
}
