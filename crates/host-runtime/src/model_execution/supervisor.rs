...
struct Inner {
    ...
    /// ModelExecution retains a nonblocking 64 MiB reservation.
...
/// as `local_embeddings::jobs::JobTable::lock_jobs`.
...
impl Supervisor {
    pub fn new(backend: Arc<dyn LlmExecutionBackend>) -> Self {
        Self::with_limits(backend, ModelExecutionLimits::default())
    }
    ...
    /// The `harness_unavailable` subreason every run on `harness` would fail with, without
    /// executing anything; `None` when the harness can run.
