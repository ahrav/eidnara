...
`docs/shm-transport.md:53-59` states that
`eidnara daemon doctor` "reports either a healthy fixed ring or one
...
**The whole producer.** `crates/host-runtime/src/ring_transport.rs:165-179`:
...
(`crates/shm-transport/src/profile.rs:451` post-#131; the file was not fully
...
- `"missing_addon"`, `"identity_mismatch"`, `"peer_death"`,
  ...
  union at `packages/plugin/src/shared/host-client/types.ts:69-73`, and as
  classification outputs in
  `packages/plugin/src/shared/host-client/shared-memory-failure.ts:14-30`,
...
cited (`docs/shm-transport.md:66`, `:26-36`) were not re-checked in
...
- Sources examined: `docs/shm-transport.md:51-73` (the whole Doctor and
  diagnostics section), `ring_transport.rs:142-196`,
  `packages/plugin/src/shared/host-client/types.ts:69-73`,
...
- Missing evidence: whether `eidnara daemon doctor` renders the host's
...
- Sources examined: `ring_transport.rs:165-179` (the `match`), `:190-193` (the
  ...
  `docs/shm-transport.md:59` (`resource_exhaustion` as a terminal class).
