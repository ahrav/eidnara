...
Existing check: `distinct_identity_axes_do_not_conflict` covers distinct scope, module, and backend axes; `separator_in_a_key_field_fails_closed_instead_of_aliasing` (`crates/lease/src/lib.rs:2079-2102`) covers the separator in each field; status **unaudited**.
...
Existing check: `superseded_writer_is_fenced_out_after_handover` (`crates/storage/src/lib.rs:3467-3523`), and `superseded_writer_is_rejected_after_reopen`, `primitives@89abb40`; both use synthetic stale stores and remain **unaudited**.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:73-100,698-867`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:2837-2846`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:2848-2859`) proves sibling files in one directory do not contend; `symlinked_database_paths_are_refused_never_aliased` (`:2348-2404`) proves both a directory-symlink alias and a file-symlink alias are refused, with and without a live holder. `a_hard_linked_database_is_refused` (`:1490-1517`) proves a second name for the database inode is refused through either name. No test covers descriptor disagreement; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:698-867,1372-1395`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return` (`crates/storage/src/lib.rs:2592-2607`), observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:2609-2645`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced DDL, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:3043-3063`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:3065-3131`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:3133-3181`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:3183-3228`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`primitives@89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`primitives@89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`primitives@89abb40`) and `a_regressed_positive_epoch_fails_closed` (`primitives@89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`primitives@89abb40`), `a_suppressed_epoch_increment_is_rejected` (`primitives@89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`primitives@89abb40`) protect the lease row and its issuance; `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:3230-3333`) refuses DML, triggers, indexes, views, and renames onto the `fence` and `format_marker` tables; and `a_fenced_callback_cannot_rewrite_the_format_marker` (`:3335-3391`) refuses an update of, or a rename onto, the marker row. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/store-schema-identity-matches-the-baseline.md). `ExpectedIdentity::for_baseline` (`crates/storage/src/lib.rs:1111-1177`), `classify` (`:1238-1288`), and `apply` (`:1290-1322`) implement the contract inside `open_sqlite` (`:697-794`); `fresh_file_matches_the_baseline_inventory` (`:2647-2711`) pins the fixture, and `a_file_with_foreign_objects_is_refused_without_mutation` (`:2772-2798`), `a_consumer_baseline_is_applied_once_and_verified_on_reopen` (`:2713-2753`), `a_baseline_that_does_not_apply_is_rejected_before_the_file_is_touched` (`:2755-2770`), and `negative_database_fence_fails_closed` (`:3437-3465`) cover the refusals.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:425-437`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:557-616`); `GuardedConn` (`crates/storage/src/lib.rs:249-361`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:3007-3030`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:3133-3181`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:3393-3406`) pin the three denials.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:509-514`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:3043-3063`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:680-695`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:3032-3041`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:3065-3131`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:697-794`) creates the database owner-only and calls `protect_file` on all three paths after `classify` accepts the file and before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:2462-2512`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:2915-2943`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:196-246`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:3408-3435`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:2990-3005`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:1435-1438`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:1396-1403`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:3437-3465`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:3545-3561`) cover both directions.
