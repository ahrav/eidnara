...
`mod.rs:4` states the contract: "Artifact faults keep LocalEmbeddings's catalog
identity published and make binds reject with `artifact_invalid`". LocalEmbeddings is
...
Unconfigured. `initialize` (`src/local_embeddings/mod.rs:993-1013`) sets
...
(`src/local_embeddings/bundle.rs:670`), hash mismatch (`:694`), unlisted entry
(`:731`), and the ORT identity faults in `verify_ort_library`
(`src/local_embeddings/inference.rs:105-129`: bad digest string, missing, not regular,
...
depend on LocalEmbeddings's outcome once LocalEmbeddings returns `Ok`.

Existing checks:

- `unconfigured_component_is_disabled_not_fatal`
  (`tests/local_embeddings_bundle.rs:226-265`): `LocalEmbeddingsComponent::new(None)`;
...
- `wrong_ort_identity_disables_the_lane` (`:708-728`): wrong hash, missing
  ...
  `EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY` is unset (`:709`, `:29-37`). That
...
- `corrupt_bundle_degrades_local_embeddings_and_keeps_context_routable`
  (`tests/local_embeddings_roundtrip.rs:57-117`): a flipped model byte over a real
  loopback host; the catalog lists three modules (`:88`), the LocalEmbeddings route
  open is `artifact_invalid` (`:92-93`), and a request on a `context` route
  echoes (`:96-114`). The `context` primary is `EchoPrimary`
  (`tests/support/local_embeddings.rs:126-170`), a test stub.
...
(`tests/support/local_embeddings.rs:286-311`), so the test's assertion at `:93` is not
racy, but a client that treats `module_reloading` as final would misread a
lane that is about to become `Disabled`.

The `liveness` half of the record (a context request completes while LocalEmbeddings
is disabled) depends on the composite routing by `module_id`
(`src/composite.rs:225-236`), not on anything in the LocalEmbeddings module.
...
2. A CI configuration that provides `EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY`, or a
...
- Sources examined: `src/local_embeddings/mod.rs:1015-1063`; `src/runtime.rs:832-857`;
  `src/local_embeddings/bundle.rs:175-214`; `src/local_embeddings/inference.rs:71-95`,
  `:105-129`.
...
- Sources examined: `tests/local_embeddings_roundtrip.rs:57-117`;
  `tests/support/local_embeddings.rs:126-170`, `:180-240`.
