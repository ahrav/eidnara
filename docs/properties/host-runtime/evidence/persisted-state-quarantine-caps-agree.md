...
`crates/host-runtime/src/lifecycle.rs:64-65`
...
`crates/host-runtime/src/generation.rs:48-49`
...
`crates/daemon/src/bin/eidnara-host.rs:347` maps to `native_payload_invalid`,
whose contract remediation is `reinstall_eidnara`. The fourth use is the
...
- Sources examined: `lifecycle.rs:64-65`, `:257-284`, `:467-491`, `:803-830`;
  ...
  `crates/host-runtime/src/instance.rs:852-865` for `read_all_fd`'s over-cap error;
  `eidnara-host.rs:344-350` for the error-to-reason mapping; every workspace
