...
/// Fixed prefix ahead of every sample body: schema, wire header, identity, body length.
...
/// A sample prefix that passed validation; `body_range` is the only range a decoder may read.
#[derive(Clone, Copy, PartialEq, Eq)]
