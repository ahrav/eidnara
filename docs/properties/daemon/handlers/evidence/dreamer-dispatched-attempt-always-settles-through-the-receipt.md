...
`crates/daemon/src/lib.rs` unless stated. The default dispatch arm at `:12692`
...
- `handle_dreamer_run_task` validates the request, passes the memories
  ...
  (which would dispatch) is refused over budget (`:9339-9586`, `:9909-9918`).
...
- `begin_dreamer_receipt` is the first write (`:9574`). Each attempt row records
  ...
  goes to `resume_dreamer_receipt` (`:9894`).
- `resume_dreamer_receipt` reads `list_dreamer_attempts` and picks the newest
  ...
  attempt: `take_over_undispatched_dreamer_receipt` (`:9913`) atomically
  ...
  receipt `unknown` through `complete_receipt_as_unknown` (`:13703`), leaving
  ...
  the same, as does a failed session bind (`:9933-9991`). An open attempt with
  no handle: settle `unknown`. Settling an open attempt goes through
  `settle_dispatched_attempt_as_unknown` (`:13683`), which writes the attempt
  terminal best-effort and then completes the receipt through the same helper,
  matching `Applied`, `Fenced`, and `Err` separately.
- In the chain loop, `begin_dreamer_attempt` precedes `start` (`:9655-9705`), and
  `record_dreamer_run_handle` follows a successful `start` (`:9717`). A fenced
  ...
  the receipt (`:9723-9734`, `:13714-13723`). The await-terminal write has the
  same no-purge rule when fenced (`:9766-9799`).
  ...
  `cancelled`, and nothing more is read from the run (`:9737-9757`).
  ...
  otherwise the request tries unknown settlement (`:9759-9799`). A generation
  fence prevents that settlement from overwriting the successor's receipt.
- The exhausted-chain write (`:9846`) and the success write (`:9861`) match
  `Applied`, `Fenced`, and `Err`; only `Applied` answers with the receipt's
  recorded response, read back through `read_dream_task_response` (`:13727`).
...
4. A receipt stranded `in_progress` by an aborted attempt insert: assert the
   retry dispatches once at generation 2 (`:27429-27482`). Also reopen a receipt
...
9. Move the receipt to generation 2 in each of the scripted producer's
   ...
   await window performs exactly one (`:27485-27551`).
...
- Sources examined: `classify_request_timeout`, `classify_attempt_timeout`, and
  the await call in the chain loop (`:9491`, `:9737-9744`).
- Findings: the request deadline is clamped to `CLASSIFY_AWAIT_TIMEOUT`, and the
  ...
  write is fenced (`:9748-9799`, `:9819-9834`).
