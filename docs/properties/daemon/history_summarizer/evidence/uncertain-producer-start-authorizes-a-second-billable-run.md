...
tracked separately on any path where a response can be lost. The history_summarizer's
...
`crates/daemon/src/history_summarizer.rs:1369-1413`, the `await_output` failure arm:
...
`crates/daemon/src/history_summarizer.rs:1285-1329`, the `producer.start` failure arm:
...
`crates/daemon/src/history_summarizer.rs:1052-1143` `decide_producer_failure` branches
...
- `:78-82` `enum HistorySummarizerSendOutcome { NotSent, OutcomeUnknown, Terminal }`.
- `:84-92` the `From<SendOutcome>` conversion, so the variant comes from
  `host_runtime`.
- `:94-99` `HistorySummarizerCallFailure` carries `outcome`, `code`, and `message`.
...
`history_summarizer_producer.rs:779-790` gates the start on `stop_requested()` and returns
...
- `run_history_summarizer_firing` returns immediately after the first successful publish
...
2. `start` for model A is sent, ModelExecution receives it and begins a run, and the reply
...
5. Two provider runs execute for one chunk. One publishes; the other completes and
   is never read, or is cancelled by ModelExecution's own lifecycle, or runs to completion
   and bills.
...
(30 s, `history_summarizer_producer.rs:29`). Dependencies:

- `host_runtime`'s `SendOutcome` must be accurate about `OutcomeUnknown`. That is Part
  2b territory.
- ModelExecution's own behaviour on an unacknowledged start: whether it begins the run
...
1. A producer double with two models configured. The first `start` returns
   `HistorySummarizerProducerError::Call(HistorySummarizerCallFailure { outcome: OutcomeUnknown,
...
- Sources examined: `crates/daemon/src/history_summarizer.rs:1285-1329`, `:1369-1413`,
  `:1226-1240`, `:1052-1143`; `crates/daemon/src/history_summarizer_producer.rs:78-99`,
  `:376-433`, `:752-800`; the three cancel-proof tests at `history_summarizer.rs:3389`,
  `:3444`, `:3498`; `history_summarizer.rs:2494`
...
- Missing evidence: whether ModelExecution can start a run before acknowledging the start
