...
- **Primary evidence:** `read_epoch` uses `Read::take(21)`, preallocates 21 bytes, and rejects lengths above the 20-byte decimal maximum (`crates/lease/src/lib.rs:414-443`).
- **Existing evidence:** `invalid_epoch_states_fail_closed` exercises a 21-byte file through ordinary and floor-based acquisition (`crates/lease/src/lib.rs:661-714`).
