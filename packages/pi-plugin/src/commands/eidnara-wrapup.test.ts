...
function wrapupHarness(respond: (call: RecordedCall) => unknown, compactionOff = false) {
    ...
    const run = async (args = "", signal?: AbortSignal) => {
        const command = fake.commands.get("eidnara-wrapup") as {
...
describe("Pi /eidnara-wrapup", () => {
    ...
    it("rejects non-positive or non-numeric keep values without calling the daemon", async () => {
        ...
        expect(calls).toHaveLength(0);
        expect(badEntries.at(-1)?.text).toContain("Usage: `/eidnara-wrapup [messages_to_keep]`");
    });
    ...
    const outcomes: Array<[string, string, CtxStatusEntryData["level"]]> = [
        ...
        [
            "already_in_progress",
            "## Eidnara Wrapup — Skipped\n\n/eidnara-wrapup is already running for this session.",
            "warning",
        ],
        ["retryable", "## Eidnara Wrapup — Partial\n\n", "warning"],
        [
            "failed",
            "## Eidnara Wrapup — Failed\n\nWrapup failed; try /eidnara-wrapup again.",
            "error",
        ],
    ];
