...
every `store.` call inside `apply_once` (`crates/daemon/src/transform.rs:3222-5697`)
...
(`memory-store/src/lib.rs:5531-5546`).
...
3. Drive `transform` with an array crafted to raise each error variant in turn.
   `ReservedId` needs a live block whose flat id starts with `eidnara_`
...
- Sources examined: `transform.rs:2711-3219`, `:3234`, `:3108-3141`,
  ...
  `packages/plugin/src/config/agent-disable.ts:31-35` (source-catalog path, not present at HEAD),
  `packages/cli/src/commands/setup-opencode.ts:45-57` (source-catalog path, not present at HEAD).
- Findings: `apply_additive_only` has exactly one commit (`:3113`) with
  ...
  (`packages/plugin/src/config/agent-disable.ts:34` (source-catalog path, not present at HEAD)), so an absent value means
  enabled, and `resolveCompactionEnabledForWriter`
  (`packages/cli/src/commands/setup-opencode.ts:45-57` (source-catalog path, not present at HEAD)) uses that. Both the
