...
`history_summarizer.rs:60-62` makes an explicit unreachability claim about the publish
...
`crates/daemon/src/history_summarizer.rs:38-67`, inside `to_stored_history_segment`:
...
check at `history_summarizer_validate.rs:1000-1008`:

```rust
match history_segment.p1.as_deref() {
    Some(p1) if !p1.trim().is_empty() => {}
    _ => { return Some(format!("history_segment {} is missing the tiered paraphrase structure ...", index + 1)); }
}
```
...
- `ParsedHistorySegment.p1` is set at `history_summarizer_validate.rs:323`
  (`p1: Some(p1_value)`) or `:340` (`p1: None` for the flat v1 shape).
- `map_parsed_history_segments_to_chunk` copies it at `:972`
  (`p1: history_segment.p1.clone()`), so `ValidatedHistorySegment.p1` (`:212`) is the
  same value.
- `validate_parsed_history_segments` (`:514-524`) runs on `&parsed.history_segments`, the
...
`map_parsed_history_segments_to_chunk` runs at `:506`, BEFORE
`validate_parsed_history_segments` at `:514`. So the `ValidatedHistorySegment` values are
built before the tier check runs. That does not weaken the claim, because
`validate_history_summarizer_output` returns `Err` at `:521-523` and the mapped vector is
...
construction: `parse_history_segment_output:333-347` builds a `ParsedHistorySegment` with
`p1: None` when the history_segment has flat content and no tier. So a tierless
history_segment exists as a value inside the module and is stopped one function later.

### Test coverage of the claim's two halves

- The gate half IS covered:
  `tierless_history_segments_reject_while_p1_only_output_keeps_soft_fallbacks`
  (`history_summarizer_validate.rs:1463`) and
...
- The projection half is NOT covered. Grepped for a `legacy` assertion:
  `to_stored_history_segment` has no direct test, and no test asserts the `1` arm is
...
(`history_summarizer_validate.rs:226-238`) has all-`pub` fields and derives `Default`, and
`publish_validated_chunk` is `pub fn` (`history_summarizer.rs:444`) inside `pub mod
history_summarizer` (`lib.rs:19`). Four existing tests already construct the bypass shape
(`history_summarizer.rs:4173`, `:4328`, `:4414`, `:4495`), and `:4476-4489` builds a
...
reconstructs history_segments from a durable artifact and calls
`publish_validated_chunk` directly. If any reconstructed history_segment has
`p1: None` (a legacy row read back from an older store, say), then:

- `to_stored_history_segment` takes the `1` arm and writes `legacy = 1`.
...
the type system. Verified both paths: `history_summarizer.rs:1419` (the reattach route) and
`history_summarizer.rs:1592` (the drive route), each calling
...
1. Place a constant, globally unique marker inside the `1` arm at
   `history_summarizer.rs:65`. Under the project's assertion conventions a
...
- Pin the gate half more tightly than the existing tests do: assert that for a
  history_segment with `p1: Some("   ")` (whitespace only, not `None`) the gate
...
- Pin the projection: call `to_stored_history_segment` with `p1: None` and assert
...
- Sources examined: `history_summarizer_validate.rs:1000-1001`
  (`Some(p1) if !p1.trim().is_empty()`), `history_summarizer.rs:63`
  (`c.p1.as_deref().is_some_and(|p1| !p1.trim().is_empty())`),
  `history_summarizer_validate.rs:212` and `:972` (the field type and the clone),
...
- Sources examined: all 19 tests in `history_summarizer_validate.rs:1305-1868`; the
  `history_summarizer.rs` test module hits for `legacy` and for
  `to_stored_history_segment`.
- Findings: `history_summarizer_validate.rs:1463` and `:1483` assert the GATE rejects
  tierless output. No test names `legacy` as an assertion target, and
  `to_stored_history_segment` has no direct test. So the claim's second half is
  unasserted.
