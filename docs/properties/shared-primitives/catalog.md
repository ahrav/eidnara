...
Existing check: `superseded_writer_is_fenced_out_after_handover`, `crates/storage/src/lib.rs:2233-2271`, and `superseded_writer_is_rejected_after_reopen`, `primitives@89abb40`; both use synthetic stale stores and remain **unaudited**.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:60-72,600-657`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:1421-1430`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:1432-1445`) proves sibling files in one directory do not contend; `symlinked_database_paths_contend_or_are_refused_never_aliased` (`:925-977`) proves a directory-symlink alias contends and a file-symlink alias is refused. No test covers descriptor disagreement or hardlink aliases; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:600-657,743-758`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return`, `crates/storage/src/lib.rs:1291-1303`, observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:1310-1344`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced migrations, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1811-1832`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:1834-1899`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:1901-1955`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:1957-2014`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`primitives@89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`primitives@89abb40`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`primitives@89abb40`) and `a_regressed_positive_epoch_fails_closed` (`primitives@89abb40`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`primitives@89abb40`), `a_suppressed_epoch_increment_is_rejected` (`primitives@89abb40`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`primitives@89abb40`) protect the lease row and its issuance; and `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:2016-2125`) refuses DML, triggers, indexes, views, and renames onto the fence table. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/hard-bust-drains-deferred-work.md). `step_hard` (`crates/cache-stability/src/lib.rs:250-277`) appends `pending_changes` into the rendered set before `apply_units` and clears `reconcile_pending` only when it minted or the anchor is present; `hard_drains_pending_changes_into_the_bust` (`crates/cache-stability/src/lib.rs:426-459`) asserts the drain, the mint, and the cleared flag, and `hard_without_mint_on_absent_boundary_keeps_reconcile_pending` (`crates/cache-stability/src/lib.rs:690-725`) asserts the flag stays set when nothing reanchors.
Existing check: `hard_drains_pending_changes_into_the_bust`, `hard_prefers_rendered_units_over_deferred_copies_of_the_same_key`, `hard_without_mint_on_absent_boundary_keeps_reconcile_pending`, golden vectors with `queued` units; audited at U2.
...
Confidence: high - [evidence](evidence/never-minted-boundary-is-not-reconcile-pending.md). The guard is one expression in `step_defer` (`crates/cache-stability/src/lib.rs:187-213`); `defer_on_never_minted_boundary_is_stable_not_reconcile_pending` (`crates/cache-stability/src/lib.rs:344-377`) covers both the vacuous and the non-vacuous arm.
Existing check: `defer_on_never_minted_boundary_is_stable_not_reconcile_pending`, `defer_boundary_absent_keeps_bytes_and_sets_reconcile_pending` (`crates/cache-stability/src/lib.rs:379-395`); audited at U2.
...
Confidence: high - [evidence](evidence/anchor-holds-while-reconcile-pending.md). The guard in `step_soft` (`crates/cache-stability/src/lib.rs:215-248`) is a let chain over both conditions; `soft_does_not_advance_anchor_while_reconcile_pending` (`crates/cache-stability/src/lib.rs:568-605`) and `coverage_extending_soft_advances_anchor_keeps_m0_frozen` (`crates/cache-stability/src/lib.rs:498-566`) pin both outcomes.
...
Confidence: high - [evidence](evidence/frozen-unit-order-is-preserved.md). `apply_units` (`crates/cache-stability/src/lib.rs:279-291`) is a find-or-push loop; `soft_replaces_by_key_keeps_slot_appends_new` (`crates/cache-stability/src/lib.rs:461-496`) asserts the resulting key order and the untouched first unit.
...
Guarantee: The tables `eidnara_fence` and `eidnara_schema_version` keep the names and column definitions that existing databases carry, and `eidnara_fence` holds exactly one row with `id = 0`.
Check: `always` - `always(sqlite_schema rows for both tables == pinned DDL text)` after open and `always(count(eidnara_fence) == 1 && fence.id == 0)` after every fence claim.
...
Confidence: high - [evidence](evidence/fence-tables-keep-their-durable-identity.md). `ensure_fence_table` (`crates/storage/src/lib.rs:711-719`) and `run_migrations` (`crates/storage/src/lib.rs:804-907`) emit the DDL; `fence_and_version_tables_keep_their_ddl` (`crates/storage/src/lib.rs:2354-2409`) compares the recorded SQL text and the fence row. Both names are `frozen-durable` in `migration/registry.json`.
...
Confidence: high - [evidence](evidence/migrations-apply-once-per-namespace.md). `run_migrations` (`crates/storage/src/lib.rs:804-907`) reads `MAX(version)` per namespace, sorts, skips `<= current`, and commits SQL and record together; `migrations_seed_once_across_reopen` (`crates/storage/src/lib.rs:1346-1368`), `later_migration_applies_on_top_of_earlier` (`crates/storage/src/lib.rs:1670-1698`), and `independent_namespace_chains_in_one_database` (`crates/storage/src/lib.rs:1700-1731`) cover the three clauses.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:386-391`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:464-492`); `GuardedConn` (`crates/storage/src/lib.rs:227-339`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:1774-1798`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:1901-1955`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:2127-2141`) pin the three denials.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:456-461`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1811-1832`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:557-572`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:1800-1809`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:1834-1899`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:574-654`) creates the database owner-only and calls `protect_file` on all three paths before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:1152-1216`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:1573-1604`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:160-206`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:2143-2177`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:1756-1772`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:799-802`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:760-767`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:2203-2230`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:2329-2345`) cover both directions.
