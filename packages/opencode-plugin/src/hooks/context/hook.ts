...
import {
    ...
    type SidekickConfig,
} from "../../config/schema/eidnara";
import type { ResolvedTransformMode } from "../../config/transform-mode";
import type { createCompactionHandler } from "../../features/context/compaction";
import {
    applyMirrorPage,
...
import { confirmSmartNoteReadOnly } from "../../features/context/dreamer/evaluate-smart-notes";
import { openOpenCodeDb } from "../../features/context/dreamer/open-opencode-db";
import { OpenCodeRetrospectiveRawProvider } from "../../features/context/dreamer/retrospective-raw-provider";
import {
    buildDreamTaskRuntimeConfigs,
    userMemoryCollectionEnabled,
} from "../../features/context/dreamer/task-config";
import { createDreamTaskExecutor } from "../../features/context/dreamer/task-executor";
import {
    runDueTasksForProject,
    runManualDream,
} from "../../features/context/dreamer/task-scheduler";
import {
...
import { createSmartNoteCapabilities } from "../../features/context/smart-notes/capabilities";
import { compileSmartNoteCheck } from "../../features/context/smart-notes/compiler";
import { SmartNoteEvaluatorWorker } from "../../features/context/smart-notes/evaluator-worker";
import { runCompiledSmartNoteCheck } from "../../features/context/smart-notes/sandbox-runner";
...
import type { ContextUsage } from "../../features/context/types";
...
// `createEidnaraHook` owns `lastScheduleCheckMs` so each hook instance tracks its dream schedule independently across projects.
...
function notifyEidnaraDisabled(client: PluginContext["client"], reason: string): void {
    ...
    const message =
        detail.length > 0
            ? `Persistent storage is unavailable, so eidnara is disabled for safety. ${detail}`
            : "Persistent storage is unavailable, so eidnara is disabled for safety.";
    ...
    void c.tui
        ...
        .catch((error) => {
            log("[eidnara] failed to show disabled toast:", error);
        });
}
...
export function createEidnaraHook(deps: EidnaraDeps) {
    ...
    try {
        ...
        if (!opened || !isDatabasePersisted(opened)) {
            ...
            log("[eidnara] disabling feature because persistent storage is unavailable:", reason);
            notifyEidnaraDisabled(deps.client, reason);
        ...
        log("[eidnara] hook failed to open storage; disabling feature:", error);
        notifyEidnaraDisabled(deps.client, reason);
        clearHookInitFailure();
        recordHookInitFailure({
            type: "storage",
    ...
    try {
        ...
        log("[eidnara] startup compaction-marker consistency check failed:", error);
    }
    ...
    try {
        ...
        if (sessionsWithPending.length > 0) {
            ...
            log(
                `[eidnara] rehydrated ${sessionsWithPending.length} session(s) with pending compaction-marker drain at hook init`,
            );
        }
    } catch (error) {
        log("[eidnara] hook init: pending-marker rehydration failed:", error);
    }
    ...
    const dreamerProgressByProject =
        ...
        new Map<string, import("../../features/context/dreamer/task-registry").DreamTaskProgress>();
    ...
    const maybeSendProjectIdentitySessionWarning = (sessionId: string, directory: string): void => {
        ...
        void sendIgnoredMessage(deps.client, sessionId, warning, notificationParams).catch(
            (error) => {
                log(
                    `[eidnara] failed to send project identity warning for ${directory}: ${getErrorMessage(error)}`,
                );
            },
        );
    };
    ...
    const maybeAutoEmbedSession = (sessionId: string): void => {
        ...
        void (async () => {
            ...
            try {
                ...
                log("[eidnara] auto-embed drain failed:", error);
            } finally {
    ...
    const evaluatorTransport = rustModeModuleClient
        ? new HostModuleTransport(deps.config.subc?.connection_file)
        : undefined;
    ...
    const ensureModuleNoteEvaluationBridge = (
        ...
        if (existingBridge) {
            ...
            void existingBridge.ensureRegistered?.().catch((error) => {
                log(`[eidnara] evaluator registration retry failed: ${error}`);
            });
            return;
        }
        ...
        void worker.register().catch((error) => {
            log(`[eidnara] evaluator registration failed: ${error}`);
        });
    };
    ...
    const notifyRustModeParked = (sessionId: string, message: string): void => {
        ...
        void client.tui
            ?.showToast?.({
                body: {
                    title: "Rust Eidnara paused",
                    message,
                    variant: "warning",
                    duration: 8000,
                },
            })
            .catch((error) => log(`[eidnara] rust park toast failed for ${sessionId}:`, error));
    };
