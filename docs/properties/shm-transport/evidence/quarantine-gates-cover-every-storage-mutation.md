...
- A repository-wide grep for `is_quarantined` over `crates/` and
  ...
  `crates/shm-transport/src/backend/ring.rs`: the definition at `:1381` and
...
- `ring.rs:916` shows `try_reserve` calling `reclaim_completed()` after its gate
  ...
  transitively.
- `packages/shm-native/src/lib.rs:272-294` is `cleanup_created_refs`. On a
...
- `quarantine_channel` in the addon
  (`packages/shm-native/src/lib.rs:375-393`) quarantines both directions at
...
2. The peer publishes a structurally invalid frame in the other direction, or
   ...
   `packages/shm-native/src/lib.rs:283` on the detach path.
...
`packages/shm-native/src/lib.rs:283` and `:290`. `Ring` is
...
- Sources examined: `ring.rs:1577-1627` and `:1567-1575` for the absent gates,
  ...
  ordering at `packages/shm-native/src/lib.rs:352-379`.
