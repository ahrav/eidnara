 ...
 * suffix, an optional build suffix), or `null`. A probe's warnings can carry
 * their own version numbers, so a match anywhere in the output is not enough.
 */
export function standaloneVersion(text: string | null, prefix = ""): string | null {
    if (text === null) return null;
    const escapedPrefix = prefix.replace(/[.*+?^${}()|[\]\\/]/g, "\\$&");
    const pattern = new RegExp(
        `^(?:${escapedPrefix})?v?(\\d+\\.\\d+\\.\\d+(?:-[0-9A-Za-z.-]+)?(?:\\+[0-9A-Za-z.-]+)?)$`,
    );
