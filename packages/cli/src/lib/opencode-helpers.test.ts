...
const originalPathExpansionProbe = process.env.EIDNARA_OPENCODE_TEST_PATH;
const tempDirs: string[] = [];

afterEach(() => {
    if (originalComSpec === undefined) delete process.env.ComSpec;
    else process.env.ComSpec = originalComSpec;
    if (originalPathExpansionProbe === undefined) delete process.env.EIDNARA_OPENCODE_TEST_PATH;
    else process.env.EIDNARA_OPENCODE_TEST_PATH = originalPathExpansionProbe;
    for (const dir of tempDirs.splice(0)) rmSync(dir, { recursive: true, force: true });
});

function fakeOpencode(body: string): string {
    const dir = mkdtempSync(join(tmpdir(), "eidnara-oc-bin-"));
    tempDirs.push(dir);
...
function fakeHomeOpencode(body: string): { home: string; bin: string } {
    const home = mkdtempSync(join(tmpdir(), "eidnara-oc-home-"));
    tempDirs.push(home);
...
function fakeOpenCodeCommandShim(): string {
    const dir = mkdtempSync(join(tmpdir(), "eidnara %EIDNARA_OPENCODE_TEST_PATH% & shim "));
    tempDirs.push(dir);
...
describe("OpenCode command execution", () => {
    ...
    it("executes a cmd shim for version and model probes", () => {
        process.env.EIDNARA_OPENCODE_TEST_PATH = "expanded-to-the-wrong-path";
