...
import { seedProjectMemoryClaim } from "@eidnara/opencode/features/context/test-claim-database";
import { createDirectTestDatabase } from "@eidnara/opencode/features/context/test-database";
import { replayCavemanCompression } from "@eidnara/opencode/hooks/context/caveman-cleanup";
import { resetKernelClientsForTest } from "@eidnara/opencode/hooks/context/kernel-transport";
import type { TagTarget } from "@eidnara/opencode/hooks/context/tag-messages";
...
import { __test, handlePiCloneSessionStart, readPiSessionIdFromFile } from "./clone-inheritance";
...
describe("Pi clone state inheritance", () => {
    ...
    it("inherits session notes and facts with fork-prefix anchors remapped", () => {
        ...
        const sourceNoteId = (
            database
                .prepare("SELECT id FROM notes WHERE session_id = ? ORDER BY id LIMIT 1")
                .get("source") as { id: number }
        ).id;
        const sourceFactId = (
            database.prepare("SELECT id FROM session_facts WHERE session_id = ?").get("source") as {
                id: number;
            }
        ).id;
        ...
        const fact = database
            .prepare("SELECT id, category, content FROM session_facts WHERE session_id = ?")
    ...
    it("reads the source id from the previous JSONL header", async () => {
        const directory = await mkdtemp(join(tmpdir(), "eidnara-clone-header-"));
    ...
    it("signals a migrated pending marker only after the transaction commits", async () => {
        ...
        const directory = await mkdtemp(join(tmpdir(), "eidnara-clone-signal-"));
        temporaryDirectories.push(directory);
        const file = join(directory, "source.jsonl");
    ...
    it("U7 scenario 7: clone copies session runtime without cloning claim history or receipt identity", () => {
        ...
        const before = {
            ...
            derivations: database.prepare("SELECT * FROM claim_derivations ORDER BY id").all(),
        };
    ...
    it("gives a forked session no parent token and reads from tip first", async () => {
        ...
        const database = db();
        const directory = await mkdtemp(join(tmpdir(), "eidnara-clone-tokens-"));
        temporaryDirectories.push(directory);
        const file = join(directory, "source.jsonl");
        await writeFile(file, '{"type":"session","id":"source"}\n');
