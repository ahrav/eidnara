...
fingerprint apart. `the_documented_encoder_separates_adjacent_fields` encodes
...
fingerprint. `credential_fingerprint_refuses_empty_and_oversize_values_before_hashing`
asserts that an empty value is refused as `CredentialMissing` and that a
value over `CREDENTIAL_VALUE_CAP_BYTES` is refused as
