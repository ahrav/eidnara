...
(`packages/shm-native/src/setup.rs:174-220`), so the doc comment's stated
...
The re-scope's rule that every `-p host-runtime` run carries a `--test` filter is
confirmed, and it means the 49 in-crate tests across this sub-part's four
`host-runtime` files never execute in CI. But the peer half is in a different crate,
...
**Second difference.** `cargo test -p host-runtime --doc` (`ci.yml:190`) executes the
...
**no source-resident check that CI executes at all** on the `host-runtime` side.
...
- **The native-versus-managed asymmetry has inverted.** Confirmed, and it runs
  ...
  (`packages/shm-native/src/setup.rs:122`), while the *managed Rust* peer's
...
`docs/shm-transport.md:5` ("Application frames never use the setup
...
peer side `proof` at `packages/shm-native/src/setup.rs:222-235`, with the two
...
(`packages/shm-native/src/setup.rs:401-432`), which asserts the same 32-byte
arrays as `docs/host-wire-protocol.md:180` and `:182`, and **does** run in CI
...
(`packages/shm-native/src/setup.rs:187`).
...
`docs/shm-transport.md:9-13` (the three fixed values),
...
(`packages/shm-native/src/setup.rs:115-124`). See lead L1.
...
Implementing code: `packages/shm-native/index.ts:151-187`
...
`packages/plugin/src/shared/host-client/shared-memory-failure.ts:10-30`. For
this sub-part the sharp part is where the matched strings are minted:
`/identity mismatch/i` (`:19`) matches
`packages/shm-native/src/setup.rs:366` ("shared-memory identity mismatch"),
...
(`packages/shm-native/src/setup.rs:359-375`). Nine host-side distinctions
reach the peer as at most three, and then as at most two terminal classes
through L3. `docs/shm-transport.md:73` requires peer-controlled text be
...
Verified: `packages/plugin/src/shared/host-client/auth.test.ts` does not
...
`connection_file.rs` or `packages/shm-native/src/setup.rs` names a deleted
file or mechanism. `docs/shm-transport.md` and
`docs/host-wire-protocol.md` were audited against the deletion set
...
1. **The proof construction is defined three times.** `docs/host-wire-protocol.md:186-190`
   (prose formula), `auth.rs` (`compute_proof`, host), and
   `packages/shm-native/src/setup.rs:222-235` (peer). The two code definitions
...
3. **`MAX_SETUP_MESSAGE_LEN` is 16 KiB in two crates, independently.**
   `setup_socket.rs:24` and `packages/shm-native/src/setup.rs:19`. No
...
4. **`MAX_AUTH_MESSAGE_LEN` is 4,096 in two crates, and the wire document is the
   ...
   `lib.rs:44`) and `packages/shm-native/src/setup.rs:18`, against
   `docs/host-wire-protocol.md:163`.
5. **`RING_DESCRIPTOR_COUNT = 2` is a named constant on the host
   (`setup_socket.rs:25`) and a bare `2` on the peer**
   (`packages/shm-native/src/setup.rs:255`, with `ScmRights(3)` at `:240`).
...
7. **The ring profile string is defined three times.**
   `ring_transport.rs:31`, `packages/shm-native/src/lib.rs:27` (`PROFILE`),
   `packages/shm-native/index.ts:8` (`QUALIFIED_TEST_PROFILE`). The value is
   the release identity `docs/shm-transport.md:11` fixes.
8. **The nine-member `NativeStartupFailureReason` set and the five-member
   ...
   expression.** `packages/shm-native/index.ts:22-31` against
...
peer half do.** The 49 are excluded structurally: every `-p host-runtime` invocation
...
in reverse: `ci.yml:177` runs `cargo nextest run -p shm-native
-p shm-transport` with no filter on Linux, and `:184` runs
`cargo nextest run -p shm-native` with no filter on macOS.
...
| --- | --- | --- | --- |
| Plugin shared-memory contracts | `bun test packages/plugin/src/shared/host-client` | `:211` | `connection-file.test.ts` (27) is the CI-executed twin of claim C13; `credential-fingerprint.test.ts` (3); `deadline.test.ts` (15); `owner.test.ts` (4); `shm-frame-channel.test.ts` (15), including the nine terminal-class mappings at `:49-57` |
| Native behaviour (Bun) | `bun run --cwd packages/shm-native test:bun` | `:196` | `tests/suite.test.ts` imports `capability.ts`, `mechanism.ts`, `runtime.ts`; 8 `test(` calls in `mechanism.ts`. Runs under `EIDNARA_SHM_NATIVE_CLAIMED_TARGET: "1"` (`:197`) |
| Native behaviour (Node 24) | `bun run --cwd packages/shm-native test:node` | `:203` | same suite through `tests/runtime.ts` |
...
| Architecture audit | `bun test scripts/check-shm-architecture.test.ts`, `bun run check:shm-architecture` | `:55`, `:58` | the forbidden-name gate; scans `packages/plugin/src/shared/host-client` and `packages/shm-native/src` among its five roots |
...
`packages/shm-native/src/setup.rs`.

**`.expect(`: 3, in two clusters.**

| Cluster | Sites | Labels |
| --- | --- | --- |
| Infallible slice-to-array | 2 | `setup_socket.rs:400` and `packages/shm-native/src/setup.rs:290`, both `"four-byte prefix"`, each guarded by a preceding `while prefix.len() < 4` loop |
...
(`packages/shm-native/src/setup.rs:359-375`). `NativeStartupError`
(`packages/shm-native/index.ts:34-39`) carries a nine-member closed reason and
...
2. **49 of the 51 in-crate tests run nowhere but a developer's laptop, and the
   ...
   (`packages/shm-native/src/setup.rs:401`) assert the same 64 literal bytes
...
4. **The manifest-and-checksum branch is five of nine failure reasons and CI
   never enters it.** `packages/shm-native/index.ts:151-187` implements
   ...
   verification `docs/shm-transport.md:83` promises "before loading" has
...
8. **`activation.rs` holds the startup-ordering claim and is named in no
   ...
   `docs/host-wire-protocol.md:586-593`, the normative seven-step startup
...
- Should the two `auth.rs` citations (D1, D2) be corrected, or do they signal
  ...
  that its cross-language partner has moved to `shm-native`? This is a
  remediation decision and METHOD rule 6 forbids resolving it here. (needs human
  input)
- Does `packages/shm-native/src/setup.rs` belong to Part 1 or 2c? Carried
...
- Is the polarity difference in lead L1 (`==` rejected natively versus
  ...
  equality and `geometry()` semantics from `shm-transport`, which is Part 1
  scope. Unresolved.
