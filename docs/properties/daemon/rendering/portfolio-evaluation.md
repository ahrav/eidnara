...
state of a correct implementation: a served `<eidnara-search-hint>` block with a
...
| # | Gap | Evidence |
| --- | --- | --- |
| G1 | **`terse_text_compression.rs` compatibility and its hygiene integration have no record.** 651 lines with one test, `differential_golden_matches_typescript_oracle` (`terse_text_compression.rs:626`, inside the `#[cfg(test)]` module whose attribute is `:612` and whose `mod tests` opens at `:613`, both closing at `:650`), which replays `testdata/terse_text_compression-golden.json` and asserts all three levels per case. The fixture holds **42 cases**, counted at `HEAD`. So the unit's whole claim-bearing surface is one cross-language snapshot that no workflow runs, and its integration with the hygiene metric is separately uncovered: `terse_text_compression_content` (`tail_hygiene.rs:422-429`) supplies measured text for a block, and all 12 `nudge-hygiene-golden.json` cases pass `&CoreState::default()` (`:1049-1056`), so the terse_text_compression arm (`:526`) is unreachable from the fixture that exists to stop cross-language drift. `fault-map.md` already asks for a `HYGIENE_MEASURED_A_TAIL_CONTAINING_A_TERSE_TEXT_COMPRESSION_UNIT` marker; no record owns the unit. |
