...
because every `-p host-runtime` invocation carries a `--test <name>` filter. The
...
> `crates/host-runtime/tests/support/shm_process.rs` was deleted by the refactor and
...
> `crates/host-runtime/src/provider_recovery.rs`.
>
...
| --- | --- | --- |
| **R0** test execution in CI | Any workflow job that builds and runs the `host-runtime` **lib** test target | **No.** Verified at `HEAD`: all 13 `host-runtime` hits in `ci.yml` are `:87`, `:132`, `:133`, `:134`, `:168`, `:169`, `:178`, `:187`, `:190`, `:211`, `:361`, `:442`, `:461`; every test invocation carries `--test <name>` and `:168-169` are `cargo build`. 35 in-crate tests execute in no job. The one exception is `cargo test -p host-runtime --doc` (`:190`), which runs the two `compile_fail` doctests at `frame_channel.rs:296-308`. This costs a workflow change and no new infrastructure |
...
with no `host-runtime` caller in production or test
...
| ring-a-segmented-inbound-body-has-no-production-producer | **No fault, and no runtime state either.** `InboundFrame::segmented` (`frame_channel.rs:477`) had zero call sites tree-wide, so `ReceiveBody::Segmented` (`:448`) was unconstructible; the borrowing adapter and the connection engine's contiguous-or-copy decoder that consumed it are absent from HEAD, where `receive_one` hands the `PayloadLease` itself to `InboundFrame::new` (`ring_transport.rs:1032-1034`) and `InboundFrame::into_private` (`frame_channel.rs:107-128`) copies it with `PayloadLease::to_vec` (`crates/shm-transport/src/lease.rs:332-338`). A body straddling an arena wrap point is not a shape the pool transport produces: the pool backend writes each frame into one fixed block (`crates/shm-transport/src/backend/ring.rs:5-9`) and `PayloadLease::body` yields one contiguous `LeaseSpan` (`lease.rs:308-329`), so nothing on the host path can reach the record's subject | **No.** Static absence, not runtime non-vacuity: the record's subject has no producer at HEAD, and the fixed-block pool cannot produce a wrap-around body, so the wrap-around test the evidence file once proposed is historical and arena-backend only. |
...
| `ring_receive_one_dropped_a_lease_without_routing_a_release_error` | A `Cancelled` or `Overloaded` return (`:988`, `:998`, `:1007`) or an `Ok(false)` exit on `read_cancel` or `discard` (`:991`, `:993`) dropped its lease, so `PayloadLease::Drop` (`crates/shm-transport/src/lease.rs:364-370`) handled the release | The independent precondition of the reporting asymmetry, and legal: the drop path is the designed teardown. It does **not** assert that a release failed |
...
1. **R0, running the existing in-crate suite in CI.** A workflow change and
   nothing else: add an `host-runtime` lib target invocation alongside the existing
