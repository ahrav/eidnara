...
pub const MODEL_EXECUTION_MODULE_ID: &str = "model_execution";

pub struct ModelExecutionComponent {
...
impl CompositeComponent for ModelExecutionComponent {
    ...
    fn manifest(&self) -> ManifestSnapshot {
        ManifestSnapshot {
            module_id: MODEL_EXECUTION_MODULE_ID.to_owned(),
    ...
    async fn bind(&self, route: RouteHandle, identity: RouteIdentity) -> BindOutcome {
        ...
        if routes.len() >= config::MAX_BOUND_ROUTES && !routes.contains_key(&route) {
            return BindOutcome::Reject {
                ...
                message: "model_execution route capacity is exhausted".to_owned(),
            };
        }
    ...
    async fn handle(&self, ctx: RequestCtx) -> RequestOutcome {
        let Some(key) = self.key_of_route(ctx.route) else {
            return app_error(
                "internal_error",
                "route is not bound to a model_execution session",
            );
        };
    ...
    async fn health(&self) -> HealthReport {
        ...
        // The probe is bounded well under the host's lifecycle callback deadline so a stalled closure store degrades ModelExecution instead of making the health callback host-fatal.
        let supervisor = Arc::clone(&self.supervisor);
...
impl SecondaryComponent for ModelExecutionComponent {
    async fn initialize(&self) -> Result<(), InitError> {
        ...
        if cfg!(not(target_os = "linux")) {
            return Err(InitError(
                "model_execution requires Linux: crash-ownership records and sweeps \
        ...
        let swept = tokio::task::spawn_blocking(move || {
            subprocess::group_registry::sweep_orphaned_groups(&state_root).map_err(|err| {
                InitError(format!(
                    "model_execution could not sweep crash-orphaned process groups: {err}"
                ))
            })?;
            subprocess::group_registry::sweep_orphaned_run_dirs(&state_root).map_err(|err| {
                InitError(format!(
                    "model_execution could not sweep crash-orphaned run directories: {err}"
                ))
            })?;
            Ok(())
        })
