//! Real-store proofs for MemoryReviewer review jobs and frozen selections: incarnation identity, causal dedup, capacity, frozen-page retention, atomic rollback, expiry, and nonreclaimable receipt charges.

use std::collections::BTreeMap;

use context_core::redaction::RedactionErrorKind;
use memory_store::memory_reviewer_jobs::{
    CausalInputs, EnqueueOutcome, EvidenceAvailability, FROZEN_PAGE_RECEIPT_CHARGE_BYTES,
    FROZEN_SELECTION_ALLOWANCE_BYTES, FrozenSelectionPage, FrozenSelectionState,
    MAX_CAUSAL_POLICY_VERSIONS, MAX_CAUSAL_SIGNALS, MAX_FROZEN_PAGE_BYTES,
    MAX_FROZEN_SELECTIONS_PER_HOST, MAX_MEMORY_REVIEWER_METADATA_BYTES_PER_PROJECT,
    MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_HOST, MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_PROJECT,
    MAX_REQUIRED_EVIDENCE, MAX_SELECTION_REFERENCES, MEMORY_REVIEWER_JOB_ALLOWANCE_BYTES,
    MEMORY_REVIEWER_QUEUE_LIFETIME_MS, MEMORY_REVIEWER_RECEIPT_CHARGE_BYTES,
    MemoryReviewerJobError, MemoryReviewerJobInput, MemoryReviewerJobOutcome,
    MemoryReviewerJobRefusal, MemoryReviewerJobState, ProducerBinding, ReserveOutcome,
    ReviewTarget, activate_memory_reviewer_job_in_tx, advance_selection_cursor_in_tx,
    complete_frozen_selection_in_tx, enqueue_frozen_selection_in_tx, freeze_selection_in_tx,
    reserve_memory_reviewer_job_in_tx,
};
...
fn descriptor(dir: &std::path::Path) -> StorageDescriptor {
    MemoryStore::test_descriptor(dir, "eidnara-memory_reviewer-jobs-test")
}
...
fn input(candidate: &str) -> MemoryReviewerJobInput {
...
fn refusal(error: MemoryReviewerJobError) -> MemoryReviewerJobRefusal {
    match error {
        MemoryReviewerJobError::Refused(refusal) => refusal,
        MemoryReviewerJobError::Store(error) => panic!("store error instead of refusal: {error:?}"),
    }
}

fn reserved(outcome: ReserveOutcome) -> memory_store::memory_reviewer_jobs::MemoryReviewerJob {
    match outcome {
...
fn incarnation_survives_reopen_and_changes_on_replacement() {
    ...
    let store = MemoryStore::open(&descriptor(dir.path())).unwrap();
    assert_eq!(store.memory_reviewer_store_incarnation().unwrap(), first);
    ...
    let replaced: Result<(), _> = store.with_fenced_conn_for_test(|conn| {
        conn.execute(
            "INSERT OR REPLACE INTO memory_reviewer_store_identity (id, database_incarnation_id, created_at_ms)
             VALUES (0, ?1, 1)",
            ["ab".repeat(16)],
        )
        .map(drop)
    ...
    assert_ne!(replaced.memory_reviewer_store_incarnation().unwrap(), first);
}
...
fn identical_causal_inputs_deduplicate_and_changed_evidence_permits_one_new_job() {
    ...
    let job = reserved(
        store
            .reserve_memory_reviewer_job("proj", &producer("f1"), &inputs("cand-1"), NOW)
            .unwrap(),
    );
    assert_eq!(job.state, MemoryReviewerJobState::Reserved);
    assert_eq!(
        job.queue_deadline_ms,
        NOW + MEMORY_REVIEWER_QUEUE_LIFETIME_MS
    );
    assert_eq!(
        job.causal_identity,
    ...
    match store
        .reserve_memory_reviewer_job("proj", &producer("f2"), &reordered, NOW + 5_000)
        .unwrap()
    {
    ...
    store
        ...
        .unwrap();
    match store
        .reserve_memory_reviewer_job("proj", &producer("f3"), &inputs("cand-1"), NOW + 20)
        .unwrap()
    {
    ...
    let renewed = reserved(
        store
            .reserve_memory_reviewer_job("proj", &producer("f4"), &with_evidence, NOW + 40)
            .unwrap(),
    );
    assert_ne!(renewed.causal_identity, job.causal_identity);
    assert_eq!(renewed.target, job.target);
    match store
        .reserve_memory_reviewer_job("proj", &producer("f5"), &with_evidence, NOW + 50)
        .unwrap()
    {
        ReserveOutcome::Existing(existing) => {
    ...
    assert!(matches!(
        store
            .reserve_memory_reviewer_job("proj", &other_firing, &with_evidence, NOW + 60)
            .unwrap(),
    ...
    let unavailable = reserved(
        store
            .reserve_memory_reviewer_job("proj", &producer("f7"), &flipped, NOW + 70)
            .unwrap(),
    );
    ...
    assert_eq!(
        refusal(
            store
                .reserve_memory_reviewer_job("proj", &producer("f8"), &conflicting, NOW + 80)
                .unwrap_err()
        ),
...
fn a_memory_target_reserves_and_activates_with_the_memory_as_subject() {
    ...
    let job = reserved(
        store
            .reserve_memory_reviewer_job("proj", &producer("f1"), &causal, NOW)
            .unwrap(),
    );
    ...
    let ready = store
        ...
        .unwrap();
    assert_eq!(ready.state, MemoryReviewerJobState::Ready(memory_input));
}
...
fn metadata_quota_exhaustion_refuses_new_work_and_deletes_nothing() {
    ...
    store
        ...
        .unwrap();
    let headroom = store.memory_reviewer_headroom("proj").unwrap();
    assert_eq!(
    ...
    assert_eq!(
        refusal(
            store
                .reserve_memory_reviewer_job("proj", &producer("f2"), &inputs("cand-2"), NOW)
                .unwrap_err()
        ),
    ...
    assert_eq!(store.memory_reviewer_headroom("proj").unwrap(), headroom);
    let rows: (i64, i64) = store
        .with_conn_for_test(|conn| {
            conn.query_row(
                "SELECT (SELECT COUNT(*) FROM memory_reviewer_jobs), (SELECT COUNT(*) FROM memory_reviewer_frozen_selections)",
                [],
    ...
    assert_eq!(
        refusal(
            store
                .reserve_memory_reviewer_job("proj", &producer("f3"), &inputs("cand-3"), NOW)
                .unwrap_err()
        ),
        MemoryReviewerJobRefusal::MetadataQuota
    );
...
fn activation_requires_the_reservation_and_takes_no_second_slot() {
    ...
    let before = store.memory_reviewer_headroom("proj").unwrap();
    ...
    let ready = store
        ...
        .unwrap();
    assert_eq!(ready.state, MemoryReviewerJobState::Ready(input("cand-1")));
    assert_eq!(ready.queue_deadline_ms, job.queue_deadline_ms);
    let after = store.memory_reviewer_headroom("proj").unwrap();
    ...
    assert_eq!(
        store
            .ready_memory_reviewer_jobs("proj", 10, NOW + 2)
            .unwrap(),
        vec![ready]
    );
    assert_eq!(
        store
            .ready_memory_reviewer_jobs("proj", 10, job.queue_deadline_ms)
            .unwrap(),
        vec![],
...
fn rebinding_hands_a_reserved_row_to_another_firing_of_the_same_producer() {
    ...
    assert_eq!(
        refusal(
            store
                .rebind_reserved_memory_reviewer_job(
                    "proj",
                    &job.causal_identity,
                    &foreign,
                    NOW + 1
    ...
    assert_eq!(
        ...
        MemoryReviewerJobRefusal::Expired
    );
    let missing = inputs("cand-9").causal_identity().unwrap();
    assert_eq!(
        refusal(
            store
                .rebind_reserved_memory_reviewer_job("proj", &missing, &producer("f2"), NOW + 1)
                .unwrap_err()
        ),
        MemoryReviewerJobRefusal::Missing
    );
    ...
    let rebound = store
        .rebind_reserved_memory_reviewer_job("proj", &job.causal_identity, &recut_producer, NOW + 1)
        .unwrap();
    let rebound_producer = producer("f2");
    assert_eq!(rebound.producer, rebound_producer);
    assert_eq!(rebound.state, MemoryReviewerJobState::Reserved);
    ...
    assert_eq!(store.memory_reviewer_headroom("proj").unwrap(), before);
...
fn activation_composed_with_a_failing_producer_transaction_commits_nothing() {
    ...
    let result: Result<(), _> = store.with_fenced_conn_for_test(|conn| {
        let activated = activate_memory_reviewer_job_in_tx(
            conn,
            "proj",
            &job.causal_identity,
            &producer("f1"),
        ...
        assert!(matches!(activated.state, MemoryReviewerJobState::Ready(_)));
    ...
    let committed = store
        .with_fenced_conn_for_test(|conn| {
            let outcome = reserve_memory_reviewer_job_in_tx(
                conn,
                "proj",
                &producer("f2"),
                &inputs("cand-2"),
        ...
        .unwrap();
    assert!(matches!(committed.state, MemoryReviewerJobState::Ready(_)));
    assert_eq!(
        store
            .ready_memory_reviewer_jobs("proj", 10, NOW)
            .unwrap()
            .len(),
        1
    );
...
fn capacity_counts_reserved_and_ready_and_refusal_writes_nothing() {
    ...
    assert_eq!(
        refusal(
            store
                .reserve_memory_reviewer_job("proj", &producer("f"), &inputs("cand-over"), NOW)
                .unwrap_err()
        ),
    ...
    assert!(
        store
            .lookup_memory_reviewer_job("proj", &inputs("cand-over").causal_identity().unwrap())
            .unwrap()
            .is_none()
    );
    assert_eq!(
        store.memory_reviewer_headroom("proj").unwrap(),
        headroom,
    ...
    assert!(matches!(
        store
            .reserve_memory_reviewer_job("proj", &producer("f"), &inputs("cand-0"), NOW)
            .unwrap(),
        ReserveOutcome::Existing(_)
    ));
    ...
    store
        .finish_memory_reviewer_job("proj", &first, MemoryReviewerJobOutcome::Failed, NOW + 1)
        .unwrap();
    let after = store.memory_reviewer_headroom("proj").unwrap();
    assert_eq!(
        after.pending_jobs,
        MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_PROJECT - 1
    );
    assert_eq!(
    ...
    for project in
        1..MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_HOST / MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_PROJECT
    {
        for index in 0..MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_PROJECT {
            reserved(
                store
    ...
    assert_eq!(
        refusal(
            store
                .reserve_memory_reviewer_job("proj-host", &producer("f"), &inputs("cand-0"), NOW)
                .unwrap_err()
        ),
        MemoryReviewerJobRefusal::HostCapacity
    );
    let host_full = store.memory_reviewer_headroom("proj-host").unwrap();
    assert_eq!(host_full.pending_jobs, 0);
    assert_eq!(
        host_full.host_pending_jobs, MAX_PENDING_MEMORY_REVIEWER_JOBS_PER_HOST,
...
fn expiry_records_terminal_outcomes_without_resurrection_and_receipts_survive_reopen() {
    ...
    let later = reserved(
        store
            .reserve_memory_reviewer_job("proj", &producer("f3"), &inputs("cand-3"), NOW + 1_000)
            .unwrap(),
    );
    ...
    store
        ...
        .unwrap();
    let charged = store.memory_reviewer_headroom("proj").unwrap();

    let deadline = NOW + MEMORY_REVIEWER_QUEUE_LIFETIME_MS;
    assert_eq!(
        store.expire_memory_reviewer_work(deadline - 1).unwrap(),
        (0, 0)
    );
    ...
    assert_eq!(
        store
            .lookup_memory_reviewer_job("proj", &reserved_job.causal_identity)
            .unwrap()
            .unwrap()
            .state,
        MemoryReviewerJobState::Reserved
    ...
    assert!(matches!(
        store
            .lookup_memory_reviewer_job("proj", &ready_job.causal_identity)
            .unwrap()
            .unwrap()
            .state,
        MemoryReviewerJobState::Ready(_)
    ));
    assert_eq!(store.expire_memory_reviewer_work(deadline).unwrap(), (2, 1));
    let ready_input: Option<String> = store
        .with_conn_for_test(|conn| {
            conn.query_row(
                "SELECT input_json FROM memory_reviewer_jobs WHERE causal_identity = ?1",
    ...
    for identity in [&reserved_job.causal_identity, &ready_job.causal_identity] {
        assert_eq!(
            store
                .lookup_memory_reviewer_job("proj", identity)
                .unwrap()
                .unwrap()
                .state,
    ...
    assert_eq!(
        store
            .lookup_memory_reviewer_job("proj", &later.causal_identity)
            .unwrap()
            .unwrap()
            .state,
        MemoryReviewerJobState::Reserved,
    ...
    let expired_page: Option<String> = store
        .with_conn_for_test(|conn| {
            conn.query_row(
                "SELECT page_json FROM memory_reviewer_frozen_selections WHERE slot_id = 'slot-1'",
                [],
                |row| row.get(0),
            )
        })
    ...
    assert_eq!(
        ...
        2 * MEMORY_REVIEWER_JOB_ALLOWANCE_BYTES + FROZEN_SELECTION_ALLOWANCE_BYTES
    );
    assert_eq!(
        after.project_metadata_bytes,
        3 * MEMORY_REVIEWER_RECEIPT_CHARGE_BYTES
    ...
    assert!(matches!(
        store.reserve_memory_reviewer_job("proj", &producer("f9"), &inputs("cand-1"), deadline + 10).unwrap(),
        ReserveOutcome::Existing(job) if job.state == MemoryReviewerJobState::Terminal(MemoryReviewerJobOutcome::Expired)
    ));
}
...
fn frozen_pages_are_bounded_retained_under_deferral_and_enqueued_once() {
    ...
    assert_eq!(
        ...
        MemoryReviewerJobRefusal::ProjectSelectionCapacity,
    ...
    assert_eq!(
        ...
        MemoryReviewerJobRefusal::HostSelectionCapacity
    );
...
fn identities_and_inputs_reject_secrets_and_stay_reference_only() {
    ...
    let secret_detected = |error: MemoryReviewerJobError| {
        assert!(
            matches!(
                error,
                MemoryReviewerJobError::Store(MemoryStoreError::Redaction(
    ...
    secret_detected(
        store
            .reserve_memory_reviewer_job("proj", &producer("f2"), &secret_signal, NOW)
            .unwrap_err(),
    );
    ...
    let raw: Result<(), _> = store.with_fenced_conn_for_test(|conn| {
        reserve_memory_reviewer_job_in_tx(conn, "proj", &secret_producer, &inputs("cand-5"), NOW)
            .map(drop)
    });
    assert!(
        raw.is_err(),
    ...
    let raw: Result<(), _> = store.with_fenced_conn_for_test(|conn| {
        reserve_memory_reviewer_job_in_tx(conn, "proj", &producer("f5"), &inputs(KEYED_SECRET), NOW)
            .map(drop)
    });
    assert!(
        raw.is_err(),
    ...
    let raw: Result<(), _> = store.with_fenced_conn_for_test(|conn| {
        reserve_memory_reviewer_job_in_tx(conn, "proj", &producer("f4"), &secret_signal, NOW)
            .map(drop)
    });
    assert!(
        raw.is_err(),
    ...
    let stored: Vec<String> = store
        .with_conn_for_test(|conn| {
            ...
            let mut jobs = conn.prepare(
                "SELECT project || producer || firing_id || target_json || question_template || COALESCE(input_json, '') || causal_identity FROM memory_reviewer_jobs",
            )?;
            rows.extend(jobs.query_map([], |row| row.get::<_, String>(0))?.collect::<Result<Vec<_>, _>>()?);
            let mut pages = conn.prepare(
                "SELECT project || slot_id || selection_attempt || COALESCE(page_json, '') || COALESCE(next_cursor, '') FROM memory_reviewer_frozen_selections",
            )?;
...
fn a_selection_cursor_carrying_a_secret_is_refused_on_insert_and_upsert() {
    ...
    let stored: Vec<String> = store
        .with_conn_for_test(|conn| {
            let mut rows = conn.prepare("SELECT cursor FROM memory_reviewer_selection_cursors")?;
...
fn a_cursor_for_an_overlong_project_is_refused_before_it_is_written() {
    ...
    let rows: i64 = store
        .with_conn_for_test(|conn| {
            conn.query_row(
                "SELECT COUNT(*) FROM memory_reviewer_selection_cursors",
                [],
                |row| row.get(0),
            )
        })
...
fn a_deferred_enqueue_leaves_the_page_frozen_even_when_its_transaction_commits() {
    ...
    assert_eq!(
        outcome,
        EnqueueOutcome::Deferred(MemoryReviewerJobRefusal::ProjectCapacity)
    );
    let row = store
        .lookup_frozen_selection("proj", "slot-1", "attempt-1")
        .unwrap()
...
fn an_enqueue_at_or_after_the_page_deadline_records_the_failed_slot() {
    ...
    let frozen = store
        .freeze_selection("proj", "slot-1", "attempt-1", &page, NOW)
        .unwrap();
    let at_deadline = NOW + MEMORY_REVIEWER_QUEUE_LIFETIME_MS;
    let outcome = store
        .with_fenced_conn_for_test(|conn| {
    ...
    assert_eq!(
        store
            .lookup_memory_reviewer_job("proj", &inputs("cand-1").causal_identity().unwrap())
            .unwrap(),
        None,
...
fn frozen_page_bound_is_typed_and_admits_pages_the_schema_stores() {
    ...
    let stored_len: i64 = store
        .with_conn_for_test(|conn| {
            conn.query_row(
                "SELECT length(page_json) FROM memory_reviewer_frozen_selections WHERE slot_id = 'slot-1'",
                [],
                |row| row.get(0),
            )
        })
    ...
    assert_eq!(
        ...
        MemoryReviewerJobRefusal::PageTooLarge
    );
    assert!(
        store
...
fn terminal_pages_drop_their_references_and_keep_a_receipt_charge() {
    ...
    let (page_json, cursor): (Option<String>, Option<String>) = store
        .with_conn_for_test(|conn| {
            conn.query_row(
                "SELECT page_json, next_cursor FROM memory_reviewer_frozen_selections WHERE slot_id = 'slot-1'",
                [],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
        })
    ...
    let retained: i64 = store
        .with_conn_for_test(|conn| {
            conn.query_row(
                "SELECT COUNT(*) FROM memory_reviewer_frozen_selections WHERE page_json IS NOT NULL",
                [],
                |row| row.get(0),
            )
        })
...
fn transaction_local_primitives_validate_the_project_identity() {
    ...
    for project in ["", too_long.as_str()] {
        ...
        assert!(
            message.contains(&MemoryReviewerJobRefusal::InvalidRequest.to_string()),
