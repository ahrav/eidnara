...
`docs/evidence/shm-traceability-v1.json` carries `current_verdict:
...
- Of the 32 strings without a fragment (9 distinct), 8 name paths that exist.
  The ninth is a source repository CI run,
...
| --- | --- | --- |
| `crates/shm-transport/tests/ring.rs#lease_limit_rejects_then_recovers_after_release` | `lease_limit_reports_backpressure_then_recovers_after_release` (`tests/ring.rs:279`) | R11 (PASS) |
| `crates/host-runtime/tests/shm_transport.rs#omitted_and_unqualified_profiles_fall_back_without_side_effects` | `omitted_and_unqualified_profiles_fall_back_reasonless_without_side_effects` (`tests/shm_transport.rs:117`) | R3 (PASS), R14 (PASS), AE2 (PASS) |
...
| --- | --- | --- |
| `docs/shm-transport.md#trusted-peer-boundary` | `## Trusted-peer boundary` (line 114) | R19 (PARTIAL), AE11 (BLOCKED) |
| `docs/perf/shm-hardware-envelope.md#evidence-boundary` | `## Evidence boundary` (line 115) | R19 (PARTIAL) |
...
- Sources examined: all 29 distinct fragment pointers resolved mechanically;
  `packages/plugin/src/shared/host-client/shm-frame-channel.test.ts` test
  declarations (lines 203-527);
  `packages/plugin/src/shared/host-client/test-support/frame-channel-contract.ts`
  scenario `name:` fields (lines 180-611); headings of
  `docs/shm-transport.md` and `docs/perf/shm-hardware-envelope.md`.
