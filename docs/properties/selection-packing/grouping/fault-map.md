...
| Empty, overflowing, out-of-parent, disagreeing, or UTF-8-splitting span | `damaged_span` rows. | `packing.grouping.ungroupable_span_presented` | packing-coverage-is-a-per-identity-partition |
...
| Generated selected sets and cost lists, with byte-flip, end-stretch, and empty-span damage | The seeded differential proptest. | `packing.scan.generated_set_compared_to_reference` | packing-groups-carry-only-selected-bytes, packing-optional-scan-skips-and-continues |
...
| Optional occurrence duplicated, missing, stale, or kernel-excluded | A repeated request, an unpersisted request, a revision-2 request, an unseeded object. | `packing.optional.excluded_optional_presented` | packing-coverage-is-a-per-identity-partition |
