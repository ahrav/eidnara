//!
//! The boundary excludes private predecessor SDK types.
...
///
/// ```compile_fail
/// # use host_runtime::RequestOutcome;
...
///
/// ```compile_fail
/// fn no_correlation(ctx: host_runtime::RequestCtx) { let _ = ctx.corr; }
/// ```
///
/// ```compile_fail
/// fn no_socket(ctx: host_runtime::RequestCtx) { let _ = ctx.socket; }
/// ```
///
/// ```compile_fail
/// fn no_credentials(ctx: host_runtime::RequestCtx) { let _ = ctx.credentials; }
/// ```
...
/// calls these methods in the order the wire protocol fixes: `install_connection_key`,
/// `initialize`, `activate`, then `bind`, `handle`, `route_gone`, and `health` until
/// `shutdown`.
pub trait HostHandler: Send + Sync + 'static {
    /// The immutable module manifests the host publishes through its catalog.
