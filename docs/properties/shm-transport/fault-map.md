...
| --- | --- | --- |
| F1 process kill | `SIGKILL` at a chosen point, signal-9 wait status required, observation window anchored to reap | **Partial, and weaker than this map assumed** — `ed487e11` deleted `crates/host-runtime/tests/support/shm_process.rs`, the reusable harness that implemented the full class, along with the provider it drove. What survives at `e447c927` is an inline primitive in one test file: `Victim::kill` at `crates/host-runtime/tests/shm_failure_modes.rs:155-159` sends `SIGKILL`, reaps, and asserts `status.signal() == SIGKILL`. It has no reap-anchored observation window, and its only injection points are the connection roles `setup`, `active`, and `idle` (`setup_active_and_idle_sigkill_each_return_exact_capacity`, `:214-224`). None of the transport-level kill points this map depends on — a receiver holding leases, a producer between reserve and commit, a peer committed without a goodbye — has an implementation any more. |
| F2 hostile peer writes | A peer that writes shared control pages: the quarantine byte, cursors, slot fields, a pending descriptor | **Yes in-process, no cross-process** (revised 2026-09-05) - the source tree had only immutable-decoder fuzz targets; at HEAD an in-process forged-shared-state fixture stores values straight into the shared pages in at least sixteen unit tests in `crates/shm-transport/src/backend/ring.rs`: `forged_consumer_cursors_fail_waits_instead_of_parking` (`:3304`), `forged_active_lease_count_quarantines_on_release` (`:3255`), `rewound_arena_write_quarantines_instead_of_overlapping_a_live_frame` (`:3271`), `rewound_published_cursor_quarantines_even_with_a_freed_slot` (`:3288`), `oversized_active_lease_count_quarantines_on_receive` (`:3827`), `health_check_bounds_do_not_overflow_on_forged_cursors` (`:3667`), `forged_arena_write_quarantines_instead_of_underflowing` (`:3999`), `impossible_slot_state_quarantines_the_receiver` (`:4277`), `forged_reclaim_length_quarantines_the_producer` (`:4291`), `quarantine_survives_peer_clearing_shared_flag` (`:4261`), and the doorbell tests. No second process performs the writes |
...
| F4 true concurrency | Producer and receiver progressing independently, not in lockstep | Partial — `stale_capacity_token_after_a_drain_does_not_deadlock_the_next_park` (`crates/shm-transport/src/backend/ring.rs:4213-4239`) blocks the producer in `reserve_until` on the test thread while a spawned consumer thread signals, sleeps, receives, and releases, which is real in-process producer/consumer concurrency at the mid-block scale; no test constructs the instruction-scale arm race, and the only cross-process test is lockstep with a sleep |
...
| release-failure-is-observable | F3: a release that fails while the surrounding operation is otherwise clean | Partial - `mismatched_release_identity_names_the_field_and_quarantines` does not exist at HEAD; the return path is `PayloadLease::return_once` (`crates/shm-transport/src/lease.rs:346-355`) into `Retained::complete` (`crates/shm-transport/src/backend/retained.rs:584-623`), which reports the doorbell failure to the caller as `WakeFailed`. Every host release is explicit and reports: `InboundFrame::into_private` (`crates/host-runtime/src/frame_channel.rs:126`) and `InboundFrame::release` (`:105-108`) map it to `PrivateCopyError::Transport`; the read loop releases pure-header leases before applying them (`crates/host-runtime/src/connection.rs:414`) and the pre-copy branches of `dispatch_request` retire the generation on failure (`crates/host-runtime/src/dispatch.rs:963`, `:1001`). `into_private_reports_a_failed_return_wake_as_a_transport_error` (`crates/host-runtime/src/ring_transport.rs:3306`) and `a_pure_header_frame_with_a_failed_return_wake_ends_the_read_loop` (`crates/host-runtime/src/runtime/close_tests.rs:380`) drive failing returns; the `PayloadLease` drop path (`crates/shm-transport/src/lease.rs:364-370`) still discards the result, and the dispatch branches are covered by inspection only |
...
| validated-spans-are-disjoint-and-inside-the-arena | Attacker-controlled descriptor fields **and** an arena larger than the minimum | No — the `frame_descriptor` fuzz oracle fixes `arena_bytes` to `MAX_FRAME_BYTES` and asserts only each span's bound and the summed length, never comparing the two accepted ranges, so an overlapping pair that satisfies those assertions passes; fuzzing the fields does not make disjointness non-vacuous until a pairwise-disjointness assertion and a larger-arena input exist |
...
| reclaim-advance-bounded-by-the-producer-reservation | F2 writing a pending slot's descriptor between release and reclaim | **Yes** - `lengthened_released_descriptor_cannot_reclaim_a_live_frame` (`crates/shm-transport/src/backend/ring.rs:3234`) rewrites a released slot's `allocation_len`, triggers reclamation, and asserts the reclaim cursor stops short of the live frame |
...
| Property | Required faults and enabling state | Non-vacuous today |
| --- | --- | --- |
| decoder-totality-over-arbitrary-bytes | None; arbitrary bytes are the enabling state. Needs an exhaustive-length sweep, structured mutation of an accepted seed, and an allocation oracle | No — committed-seed replays only (`tests/fuzz_corpus.rs:76`, `:81`, `:86`); the source tree's ten-length, two-fill sweep is absent from this tree |
...
`crates/host-runtime/tests/support/shm_process.rs`, the harness that implemented it,
...
- The F1 class row, refined from "no implementation" to "partial": a `SIGKILL`
  ...
  `crates/host-runtime/tests/shm_failure_modes.rs:155-159`, but with no reap-anchored
...
| wake-published-during-readiness-callback-is-not-lost | A publication strictly inside an unacknowledged callback (F14); enabling situation `shm_publish_during_readiness_callback` | Partial — constructed at the raw-addon level (`mechanism.ts:527-650`); the `NativeChannel` wrapper and multi-channel variants are not |
...
| quarantine-wakes-a-parked-waiter | A peer parked in `wait_until` or `wait_for_data`, then F4 quarantine on the other side, a quarantine between arm and park, or F1 closing the doorbell end | Partial - `armed_wait_recheck_sees_a_quarantine_that_sent_no_token` and `peer_closing_its_doorbell_quarantines_the_waiting_side` build the arm-window and doorbell-close cases in-process under a 5 s bound; `quarantine_wakes_a_parked_peer` sets `parked` by hand, quarantines, and only then calls `wait_until`, so it consumes an already queued token rather than releasing a thread blocked in the poll; the parked-then-quarantined interleaving is the missing case |
| raw-native-attach-rejects-hostile-descriptors-without-effects | A descriptor object built without the wrapper: non-object, unsafe numeric field, malformed or aliased grant text, accessor or proxy | Partial - four raw-descriptor suites in `tests/mechanism.ts` (`:804`, `:827`, `:849`, `:879`) with the leak counters checked around each call. The shared helper matches the message with the unanchored `/invalid shared-memory descriptor/` regex, so a regression that adds text around the promised single fixed message passes every suite except the accessor case, which alone asserts exact equality; the no-effects half is exact in all four. |
...
| transport-debug-output-redacts-every-sentinel | None; formatting each sentinel-bearing type and error variant | Partial - `debug_and_errors_redact_every_sentinel`, `sample_errors_redact_every_sentinel`, `debug_redacts_profile_admission_and_quarantine_record` format 8 of the 13 sentinel-bearing macro types; `HardwareProfileId`, `ValidatedFrame`, `ValidatedSample`, `ArenaSpan`, and `SpanPlan` are formatted by no test (no shipped formatter reaches the impls today) |
| readiness-redispatch-is-bounded-under-persistent-arm-failure | A watched channel whose ring is quarantined (F1 or any verification failure) and left open by its handler; count `dispatchReadiness` invocations and confirm a macrotask still runs | **Yes** - `a dead peer leaves the reactor instead of redispatching forever` (`packages/shm-native/tests/mechanism.ts:292-345`) closes the peer, counts dispatches against settled handlers, and confirms a 100 ms macrotask still runs; `a handler that throws over an undrained frame does not liveloop the event loop` (`:347-403`) covers the throwing-handler shape |
| each-channel-wake-survives-a-shared-acknowledgement | Two registered channels with a publication to the second inside the first's pending window; enabling situation `shm_second_channel_edge_during_pending_callback` | No - `mechanism.ts:527-650` proves the single-channel case and a later pair on the same reactor (`:627-642`), but only after the first pair is closed |
...
| setup-descriptors-name-distinct-open-files | A setup bundle with two slots naming one open file description, or a slot aliasing the setup socket; a sandbox refusing `kcmp` for the fallback | Partial - `distinct_descriptors_are_accepted_and_a_dup_is_rejected`, `inode_fallback_rejects_the_same_aliases`, and `kcmp_separates_eventfds_that_share_an_inode` (`packages/shm-native/src/setup.rs:671`, `:694`, `:723`) drive the comparison directly; nothing sends an aliased bundle through a socket into `begin_connect` or through raw `attach` |
| setup-connect-honors-its-deadline | A listener whose backlog is full or that never accepts; a signal during the blocking connect | Partial - `connect_honors_the_deadline_when_the_backlog_is_full` (`setup.rs:775`) fills a real backlog and refutes unbounded parking, but its 5 s assertion against a 200 ms deadline does not pin the bound; the `EINTR` re-arm and the pass-through errno arm have no test |
| addon-tokens-never-collide-with-live-entries | A counter just below `u32` wraparound with entries outstanding at the revisited values | Partial - `token_allocation_wraps_and_skips_outstanding_tokens` (`packages/shm-native/src/lib.rs:1246`) constructs exactly that at unit level; no test reaches a production call site, and the per-channel counters wrap in hours on a sustained ring |
| attach-makes-every-received-descriptor-close-on-exec | Descriptors arriving without `FD_CLOEXEC`, then an attach; an `fcntl` failure for the error arm | Partial - `attach_sets_close_on_exec_on_every_descriptor` (`crates/shm-transport/src/backend/ring.rs:3474`) clears the flag and asserts it is set after attach; the `fcntl`-failure arm and its no-mapping effect have no test |
| foreign-slot-state-on-reserve-is-a-fault-not-backpressure | F2 on the next reusable slot with depth not exhausted, and on a published slot before receive | Partial - `foreign_slot_state_on_reserve_is_a_fault_not_backpressure` (`ring.rs:3960`) and `impossible_slot_state_quarantines_the_receiver` (`:4277`) force one state per side on a fresh ring and assert `InvalidSharedState` plus quarantine; the recycled-slot case, the other four states, the `reserve_until` no-parking clause, and a cross-process writer are not constructed |
| failed-publication-wake-leaves-the-slot-published | F1 or F4 closing the peer's data-doorbell end with the consumer marked parked, or F4 quarantine between the stores and the recheck, or F2 moving a cursor between read and exchange | Partial - `failed_publication_wake_leaves_the_slot_published` (`ring.rs:3973`) covers the closed-end arm for slot state, `reservation_len`, and `published`; the raced-quarantine arm asserts its error only, and the cursor-exchange arm is tested on a bare atomic |
| release-leaves-the-consumer-parked-marker-intact | An armed consumer, a successful release, then a publication; F4 for the release and publish on different threads | Partial - `release_leaves_the_consumers_data_wait_armed_for_the_next_publish` (`ring.rs:3737`) asserts the marker survives the release directly and the wake only as a doorbell token observed afterwards on the same thread; no blocked waiter is released and no second thread is involved |
| capacity-wait-unparks-on-exit-and-survives-a-stale-token | A full ring with an expiring deadline; a doorbell byte queued before the park with capacity released from another thread (F4 at the mid-block scale) | Partial - `reserve_until_deadline_leaves_the_capacity_wake_unparked` (`ring.rs:4189`) asserts `parked == 0` after `Deadline`; `stale_capacity_token_after_a_drain_does_not_deadlock_the_next_park` (`:4213`) bounds progress with a second thread; no instruction-scale race and no error-exit arm is constructed |
...
| `shm_queued_writes_exceed_one_per_wake` | The bridge's write queue held two or more entries at one drain of the bridge's own write-queue eventfd (`drain_eventfd`, `crates/host-runtime/src/client.rs:2445-2448`, signalled by `RingWriteSender::try_send` at `:2400-2404`); this is the host-side queue wake, not the transport's socketpair doorbells | Ordinary burst behavior |
