...
describe("Pi eidnara_note", () => {
    ...
    it("compiles surface_condition when the daemon evaluates notes for the project", async () => {
        ...
        expect(text).toBe(`Created conditional note #1.${conditionCompileReplySuffix(expected)}`);
        expect(text).toContain("Observation compile refused: fenced path");
    });

    it("rejects conditional-note writes without a command id when evaluation is unavailable", async () => {
    ...
    it("returns an error when the project identity cannot be resolved", async () => {
        ...
        expect(isError).toBe(true);
        expect(text).toBe("Error: Could not resolve project identity for eidnara_note.");
        expect(requests).toHaveLength(0);
    });
