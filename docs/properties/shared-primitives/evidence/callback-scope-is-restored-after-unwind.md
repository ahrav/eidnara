...
- **Existing evidence:** `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:2901-2921`) panics inside `with_conn`, catches the unwind, then performs a fenced write and a maintenance statement on the same store.
