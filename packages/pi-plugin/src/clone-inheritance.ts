...
export async function handlePiCloneSessionStart(
    ...
    try {
        ...
        if (result.kind === "destination-not-empty") {
            writeLog(
                `[eidnara][pi] clone-inheritance: skipped source=${sourceSessionId} dest=${destinationSessionId} stage=destination-guard reason=destination-not-empty`,
            );
            return result;
        }
        ...
        writeLog(
            `[eidnara][pi] clone-inheritance: migrated compartments=${result.compartmentsCopied} tags=${result.tagsCopied} notes=${result.notesCopied} facts=${result.factsCopied} source=${sourceSessionId} dest=${destinationSessionId} reason=fork`,
        );
        return result;
    } catch (error) {
        writeLog(
            `[eidnara][pi] clone-inheritance: failed source=${sourceSessionId} dest=${destinationSessionId} stage=${stage} error=${error instanceof Error ? error.message : String(error)}; run /ctx-wrapup to rebuild, or re-clone`,
        );
        return null;
    }
}
