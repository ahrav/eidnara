...
- **Primary evidence:** `pin_fence_durability` (`crates/storage/src/lib.rs:572-587`) runs at the start of `with_conn_fenced`; `open_sqlite` (`crates/storage/src/lib.rs:589-694`) calls the same function before the transaction that applies the baseline and claims the fence, so an open on a VFS that cannot switch to WAL fails instead of committing a claim every later fenced write would reject.
- **Existing evidence:** `open_pins_full_synchronous` (`crates/storage/src/lib.rs:1976-1985`), `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:2009-2075`).
