...
- A word-wide `atomic_copy` path for aligned spans, if `to_vec` or `write` shows up in a profile; it needs the peer to also use word-wide accesses on the same bytes.
