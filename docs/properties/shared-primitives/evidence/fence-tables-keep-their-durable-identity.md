...
- **Existing evidence:** `fence_and_version_tables_keep_their_ddl` (`crates/storage/src/lib.rs:2228`) reads `sqlite_schema` after open and one migration and compares the recorded SQL text for both tables and the single fence row; `database_without_fence_table_uses_zero_floor` (`crates/storage/src/lib.rs:2051-2073`) shows what a missing table does.
