//! The harness provides a deterministic embedding engine for LocalEmbeddings protocol tests.
...
pub struct LocalEmbeddingsHost {
    pub info: Discovered,
...
impl LocalEmbeddingsHost {
    pub async fn start(local_embeddings: LocalEmbeddingsComponent) -> Self {
        Self::start_with(local_embeddings, |_| {}).await
    }
    ...
    pub async fn start_with(
        ...
        let composite = StaticComposite::new(
            EchoPrimary,
            local_embeddings,
            super::StubComponent::new("model_execution", "management_surface"),
        )
...
impl Drop for LocalEmbeddingsHost {
    fn drop(&mut self) {
        self.shutdown.cancel();
    }
}

pub async fn open_local_embeddings_route(client: &mut raw_client::RawClient) -> (u16, u32) {
    ...
    loop {
            ...
            .await
        {
            ...
            Err(code) => panic!("local_embeddings route failed to bind: {code}"),
        }
    }
}

pub async fn open_local_embeddings_route_rejection(client: &mut raw_client::RawClient) -> String {
    let deadline = tokio::time::Instant::now() + BUDGET;
    loop {
        match client
            .route_open_target(
            ...
            .await
        {
            Ok(_) => panic!("local_embeddings route unexpectedly bound"),
            Err(code) if code == "module_reloading" && tokio::time::Instant::now() < deadline => {
                tokio::time::sleep(Duration::from_millis(20)).await;
            }
            ...
            Err(code) if code == "module_reloading" => {
                panic!("local_embeddings route still reloading at the rejection deadline")
            }
...
pub fn sha256_hex(text: &str) -> String {
    host_runtime::local_embeddings::protocol::sha256_hex(text.as_bytes())
}
...
pub fn request_key(lane: &LaneInfo, items: &[(String, String)]) -> String {
    let batch_items: Vec<host_runtime::local_embeddings::jobs::BatchItem> = items
        .iter()
        .map(
            |(id, text)| host_runtime::local_embeddings::jobs::BatchItem {
                id: id.clone(),
        ...
        .collect();
    host_runtime::local_embeddings::protocol::canonical_request_key(lane, &batch_items)
}
...
pub fn ready_component(
    ...
    LocalEmbeddingsComponent::ready_with_engine(lane, engine, limits)
        .expect("test LocalEmbeddings limits must be valid")
}
