...
import { insertUserMemory } from "@eidnara/opencode/features/context/user-memory/storage-user-memory";
import {
...
describe("Pi single system-prompt serialization parity", () => {
    it("keeps host identity and Eidnara guidance in one OpenAI-compatible wire message", () => {
        const basePrompt = "You are a helpful coding assistant.";
        const guidance = "## Eidnara\nUse ctx_search when needed.";
        const composedPrompt = composeEidnaraSystemPrompt(basePrompt, guidance);
        ...
        const wireMessages = convertMessages(
            ...
            { systemPrompt: composedPrompt, messages: [] } as Parameters<typeof convertMessages>[1],
            { supportsDeveloperRole: false } as unknown as Parameters<typeof convertMessages>[2],
        );
        const systemMessages = wireMessages.filter((message) => message.role === "system");
        expect(systemMessages).toHaveLength(1);
        expect(systemMessages[0]?.content).toBe(composedPrompt);
    });
});

describe("buildEidnaraBlock v2 system-prompt parity", () => {
    it("keeps Eidnara guidance in the system prompt", () => {
        ...
        try {
            ...
            expect(block).toContain(EIDNARA_GUIDANCE_MARKER);
    ...
    it("includes language guidance only when configured", () => {
        const db = createTestDb();
        try {
            const baseline = buildEidnaraBlock({
                db,
            ...
            const localized = buildEidnaraBlock({
                db,
...
function readA1PrimaryGuidance(): { guidance: string; hash: string } {
    const document = readFileSync(
        join(import.meta.dir, "../../plugin/src/shared/prompt-surface-a1-golden.md"),
        "utf8",
    );
...
describe("Pi prompt-surface guidance epochs", () => {
    it("matches the A1 guidance and hash for no config and explicit full", () => {
        ...
        try {
            ...
            const implicit = buildEidnaraBlock(common);
            const explicit = buildEidnaraBlock({
                ...common,
