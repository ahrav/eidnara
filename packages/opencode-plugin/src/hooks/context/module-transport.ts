...
export interface HostModuleTransportOptions {
...
/**
 * Recognize `HostCallError.kind` through the cross-bundle check, which requires an `Error` with the wire-visible name; validate `kind` at runtime because foreign bundle fields are untyped.
 */
function hostCallErrorKind(error: unknown): HostCallError["kind"] | undefined {
    if (!isHostCallError(error)) return undefined;
...
/** The facade throws `HostCallError` only before any body write. */
...
export class HostModuleTransport {
                ...
                try {
                    ...
                    if (
                        ...
                        throw this.connectionChangedError(
                            "daemon connection changed while awaiting the module response",
                        );
                    }
                    return response;
                } catch (error) {
                    cleanupTicket = cleanupTicketOf(error);
                    const kind = hostCallErrorKind(error);
    ...
    private clientOptions(deadline?: Deadline): HostClientOptions {
    ...
    private async demandManagedReadiness(
        ...
        if (!outcome.ok) {
            const error = new Error(`managed daemon demand failed: ${outcome.reason}`) as Error & {
                code?: string;
            };
        ...
        if (outcome.storage !== "ready") {
            ...
            const error = new Error(
                `managed daemon storage is ${outcome.storage ?? "unknown"}`,
    ...
    private async ensureConnected(
        ...
        const connecting = (async (): Promise<CertifiedConnection> => {
            let candidate: HostClient | null = null;
            try {
                const options = this.clientOptions(deadline);
                candidate = await processHostClient(options);
                if (
                    expectedDaemonId !== undefined &&
                    ...
                    await evictProcessHostClient(options, candidate);
                    candidate.close();
                    throw this.connectionChangedError(
                        "daemon changed after lifecycle compatibility validation",
                    );
                ...
                if (generation !== this.connectionGeneration) {
                    ...
                    await evictProcessHostClient(options, candidate);
                    candidate.close();
                    throw this.connectionChangedError("daemon connection attempt was superseded");
                }
