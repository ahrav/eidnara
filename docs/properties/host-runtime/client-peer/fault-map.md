...
binary, `crates/host-runtime/tests/client.rs`, whose whole subject is this file, which
...
1. **`C0`, running the existing in-crate suite in CI.** A workflow change and
   ...
   `cargo test -p host-runtime --doc` runs at `ci.yml:190`, so the lib **doc** target
...
4. **`C4`, a seeded allocator rather than a fault.** Seeding `Correlations` near
   ...
   `docs/host-wire-protocol.md:654`. Lens A demoted it as the lowest-impact
