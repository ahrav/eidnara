//! This module confines MemoryReviewer-run project-text inspection to literal path, name, and content searches and bounded reads delivered through the broker.
//!
...
//!
//! A captured file becomes exact bytes in the CAS under the MemoryReviewer capture retention class with a finite `retain_until`, one typed local-file observation, and an execution-hold reference charged to the run's reservation. Captures are default-Sensitive and local-only: a repository path proves nothing about provenance. Disclosure then follows the broker's ordinary read path, so every excerpt is policy-checked, render-checked, tagged, charged, and recorded in the disclosed-input union.
...
use kernel::{
    ArtifactErrorKind, ArtifactIngestRequest, CommitIntent, KernelError, KernelStore,
    LocalFileCaptureRequest, MEMORY_REVIEWER_CAPTURE_RETENTION_CLASS, MemoryReviewerHoldBinding,
...
use super::broker::{
    ...
    RenderedBuffer, check_render, check_whole_artifact, hold_refusal,
};
...
mod tests {
    ...
    fn hold_binding() -> kernel::MemoryReviewerHoldBinding {
        kernel::MemoryReviewerHoldBinding {
