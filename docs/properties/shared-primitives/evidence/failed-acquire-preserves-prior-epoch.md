...
- **Primary evidence:** exclusive acquisition unlocks on persistence error (`FileLeaseStore::acquire_above`); `persist_epoch` never truncates and writes canonical digits from most to least significant (`crates/lease/src/lib.rs:375-414,569-577`).
