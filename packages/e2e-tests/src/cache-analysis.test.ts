...
describe("cache-bust oracle", () => {
    ...
    describe("#given a system-prompt drift", () => {
        describe("#when the Eidnara system block changes between passes", () => {
    ...
    describe("#given internal agent request signatures", () => {
        it("rejects title, summary, compaction, and Eidnara-child requests from body.system only", () => {
            const signatures = [
            ...
            for (const signature of signatures) {
                expect(
                    isInternalAgentRequest({
                        body: { system: signature, messages: [EIDNARA_SYSTEM] },
                    }),
                ).toBe(true);
            }
            expect(
