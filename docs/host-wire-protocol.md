...
Version 2 of the context application protocol rides on the daemon's routed request envelope (`method`, `v: 1`, `session_id`, `project_root`) and adds three method literals, one closed outcome vocabulary, and one capability gate. Version 2 adds the `suppress` and `reuse` actions, the `survivors` field, and the `capability_unsupported` terminal to version 1. RP2.8 (#629) Q7 assigns these literals to RP2.7; the harness-side application of an edit is RP2.8.U5 and is outside this section. The literals below MUST NOT be renamed or extended without a new protocol version on this line.
