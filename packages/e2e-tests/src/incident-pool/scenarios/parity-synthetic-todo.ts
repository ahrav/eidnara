...
import { computeSyntheticCallId } from "@eidnara/opencode/hooks/context/todo-view";
...
export const TODO_PARITY_MATRIX: readonly TodoParityMatrixRow[] = [
    ...
    },
    {
        ...
        mode: "rust",
        harness: "rust",
        prerequisites: ["cargo", "eidnara-host"],
        normalizer: "anthropic-tool-use-result",
    },
    {
        interface: "pi-ts",
...
export function isEidnaraRequest(body: RequestBody): boolean {
    return JSON.stringify(body.system ?? "").includes("## Eidnara");
}
...
/* */
export const SYNTHETIC_TODO_CALL_ID_PREFIX = "eidnara_synthetic_todo_";
...
export function emitTodoOnce(
    ...
    adapter.mock.addMatcher((body) => {
        if (probe.executed || !isEidnaraRequest(body)) return null;
...
export async function primeNextTurnAsCacheBust(
    ...
    let emitted = false;
    adapter.mock.addMatcher((body) => {
        if (emitted || !isEidnaraRequest(body)) return null;
        emitted = true;
...
function opencodeAdapter(h: TestHarness, sessionId: string): TodoScriptAdapter {
    return {
        ...
        mainRequests: () =>
            h.mock
                .requests()
                .filter((request) => isEidnaraRequest(request.body)),
        waitForPressure: () =>
            h
                .waitFor(
                    () => {
...
function seedRustLegacyAnchor(h: RustTestHarness, sessionId: string): boolean {
    const path = join(h.env.dataDir, "eidnara", "context", "store.db");
    ...
    try {
        ...
        db.prepare(
            "UPDATE eidnara_cache_state SET meta = ? WHERE session_id = ?",
