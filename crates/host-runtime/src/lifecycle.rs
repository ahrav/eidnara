...
pub const COORDINATION_DIR_NAME: &str = ".eidnara-coordination";
...
/// Renaming or removing `.eidnara-coordination` splits the lifetime fence.
...
/// Supported deployments must not rename `.eidnara-coordination` while a daemon is live.
/// Renaming `.eidnara-coordination` under a live daemon makes the probe report free.
/// descriptor drops.
...
/// The transaction lock is an exclusive cross-process `flock` on `${dataDir}/.eidnara-coordination/transaction.lock`.
