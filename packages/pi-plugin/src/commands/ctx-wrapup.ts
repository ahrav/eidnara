...
} from "@eidnara/opencode/features/context/compartment-lease";
import {
    getCompartments,
    getLastCompartmentEndMessage,
} from "@eidnara/opencode/features/context/compartment-storage";
import {
...
import { resolveExecuteThreshold } from "@eidnara/opencode/hooks/context/event-resolvers";
import {
    hasRunnableCompartmentWindow,
    resolveWrapupProtectedTailBoundary,
} from "@eidnara/opencode/hooks/context/protected-tail-boundary";
import { setRawMessageProvider } from "@eidnara/opencode/hooks/context/read-session-chunk";
import type { SubagentRunner } from "@eidnara/opencode/shared/subagent-runner";
    ...
    pi.registerCommand("ctx-wrapup", {
        ...
        handler: async (args, ctx) => {
            ...
            if (!parsed.ok) {
                sendCtxStatusMessage(pi, {
                    title: "/ctx-wrapup",
                    text: `## Eidnara Wrapup — Invalid Arguments\n\n${parsed.message}`,
                    level: "error",
                });
                return;
            }
            ...
            sendCtxStatusMessage(pi, {
                ...
                text: result,
                level:
                    result.includes("Failed") || result.includes("Partial") ? "error" : "success",
            });
        },
    });
}
    ...
    try {
        ...
        const renewal = setInterval(() => {
            try {
                ...
                console.warn(
                    `[eidnara][pi] /ctx-wrapup marker renewal failed for ${sessionId}: ${err instanceof Error ? err.message : String(err)}`,
                );
            }
        }, 60_000);
        try {
            ...
            for (;;) {
                ...
                if (!leaseResult.ok) {
                    failure = ownershipLost
                        ...
                        : leaseResult.reason === "timeout"
                          ...
                          : "Another Eidnara rebuild started while wrapup was waiting. Run /ctx-wrapup again to continue.";
                    break;
                }
                ...
                const leaseRenewal = setInterval(() => {
                    try {
                        ...
                        console.warn(
                            `[eidnara][pi] /ctx-wrapup compartment lease renewal failed for ${sessionId}: ${err instanceof Error ? err.message : String(err)}`,
                        );
                    }
                }, COMPARTMENT_LEASE_RENEWAL_MS);
                try {
