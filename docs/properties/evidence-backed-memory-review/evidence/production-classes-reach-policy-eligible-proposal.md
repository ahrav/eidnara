...
`crates/daemon/src/curator/coordinator.rs` - `resolve_descriptor` builds
...
`crates/daemon/src/curator/selection.rs` - `MEMORY_CLASSES` and
`PRODUCTION_SELECTION_OPEN`.

`crates/daemon/src/lib.rs` - `scheduled_projects` adds
`CuratorReviewSelection` only when the activation gate is open and
...
`crates/daemon/tests/curator_broker.rs` -
...
`crates/daemon/tests/curator_coordinator.rs` -
...
`crates/daemon/tests/curator_selection.rs` -
