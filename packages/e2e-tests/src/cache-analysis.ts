...
/** HistorySummarizer identity phrase; substring of the production signature above. */
export const HISTORY_SUMMARIZER_SYSTEM_MARKER = "the hippocampus of a long-running coding agent";

/**
 * Classify a provider request body as history_summarizer-originated. Union semantics:
 * the history_summarizer identity marker can appear in the system prompt (any shape —
 * string, block array, or object) OR only in the messages payload
 * (`<new_messages>` is the history_summarizer chunk envelope), so both channels are
 * checked — a system-only predicate misclassifies a marker-in-messages
 * history_summarizer request as the main agent and silently changes what cache/pass
 * assertions measure.
 */
export function isHistorySummarizerRequest(body: Record<string, unknown>): boolean {
    if (JSON.stringify(body.messages ?? "").includes("<new_messages>")) return true;
    const system = body.system;
    if (typeof system === "string") return system.includes(HISTORY_SUMMARIZER_SYSTEM_MARKER);
    return system != null && JSON.stringify(system).includes(HISTORY_SUMMARIZER_SYSTEM_MARKER);
}
