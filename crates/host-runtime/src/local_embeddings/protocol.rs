//! LocalEmbeddings validates four request operations, canonicalizes request keys across languages, and encodes responses.
...
use super::LaneInfo;
use super::LocalEmbeddingsLimits;
        ...
        impl<'de> serde::de::Visitor<'de> for EnvelopeVisitor {
            ...
            fn visit_map<A: serde::de::MapAccess<'de>>(
                            ...
                            match method.as_deref() {
                                ...
                                Some(_) => {
                                    return Err(A::Error::custom(
                                        "unsupported local_embeddings method",
                                    ));
                                }
                            }
                        }
                ...
                match method.as_ref() {
                    ...
                    _ => Err(A::Error::custom("unsupported local_embeddings method")),
                }
            }
        }
...
pub(crate) fn preflight(body: &[u8], binary: bool) -> Result<(), RequestError> {
    if binary {
        return Err(schema("local_embeddings requests are JSON only"));
    }
...
fn decode_typed(
    ...
    match method {
        ...
        _ => Err(schema("unsupported local_embeddings method")),
    }
}
...
pub(crate) fn parse_reservation_bytes(
    body_len: usize,
    limits: &LocalEmbeddingsLimits,
) -> Option<usize> {
...
pub(crate) fn max_vector_body_bytes(dims: usize, limits: &LocalEmbeddingsLimits) -> usize {
...
mod tests {
    ...
    fn the_maximum_reservation_fits_the_scratch_pool_at_every_legal_config() {
        let bound = parse_reservation_bytes(MAX_BODY_BYTES, &LocalEmbeddingsLimits::default())
    ...
    fn reservation_arithmetic_is_checked() {
        ...
        let absurd = LocalEmbeddingsLimits {
