# Part 4a property catalog: the history_summarizer publish path and its validation gate

Scope: the history_summarizer subsystem of `crates/daemon`, five files.
`src/history_summarizer.rs` (4,682 lines) holds the state machine, the publish
projection, and the drive loop. `src/history_summarizer_producer.rs` (2,306) is the
provider transport. `src/history_summarizer_chunk.rs` (2,051) assembles the chunk and
captures the original messages. `src/history_summarizer_validate.rs` (1,869) is the
...
compensates for some of the gate's omissions, `history_summarizer_prompt.rs` as the
escaping counterpart, `config.rs` for the model chain, and the TypeScript
`packages/e2e-tests/src/history_summarizer-eval` lane as the only history_summarizer coverage that
...
`history_summarizer.rs`, `history_summarizer_producer.rs`, `history_summarizer_chunk.rs`,
`history_summarizer_validate.rs`, and `lib.rs` is empty, so every line reference below
...
`history_summarizer-eval-contracts` job is `ci.yml:407-432` (source-catalog line, not present at HEAD) at HEAD and `:415-440` in the
...
- Source: the host repository at `eb6da6109`, catalog `part-4a-history_summarizer`. The records,
...
transaction: the model-generated history_segment rows (`:9458`), the deflated
...
(`history_summarizer.rs:648-653`) on the reasoning stated at `:616-619`, that a surviving
...
model runs (`history_summarizer_chunk.rs:717-727`), carried through the publish request
...
durable full-message and verbose recovery" (`history_summarizer.rs:425-426`). The
...
`history_summarizer.rs:441-443`. Session-level eviction is built never to reclaim the raw
...
reversal path: the suffix revert at `memory-store:9105-9111` deletes history_segments
...
consequential.** `validate_history_summarizer_output` (`history_summarizer_validate.rs:450-641`)
has exactly one production caller, `publish_output_from_awaiting`
(`history_summarizer.rs:1673-1678`), confirmed by search: every other call is inside
`#[cfg(test)]`. It is fail-closed on the history_segment path, with 22 rejecting
...
admitted. The chunk fingerprint (`history_summarizer.rs:151-160`) binds the chunk to the
...
(`history_summarizer_validate.rs:226-238`), `publish_validated_chunk` is a `pub fn` in a
`pub mod` (`history_summarizer.rs:444`, `lib.rs:19`), so a struct literal,
`Default::default()`, or `serde_json::from_str` all construct one, and the store
side does not compensate: the commit point admits `HistorySummarizerPhase::AwaitingProducer`
...
`history_summarizer.rs`, 19 in `history_summarizer_validate.rs`, 19 in `history_summarizer_chunk.rs`, 18 in
`history_summarizer_producer.rs`, and 34 history_summarizer-related tests inside `lib.rs`'s test
...
history_summarizer: a case-insensitive search for `history_summarizer` across all seven binaries
and both support files returns zero matches. Across the whole crate, 926 of 938
tests never execute.

The trap is the workflow name. Two "history_summarizer-eval" workflows exist and one of
them does execute per pull request, which invites the conclusion that the
history_summarizer is covered. It is not, because **those gates test a different
implementation**. The executing job is `history_summarizer-eval-contracts`
(`ci.yml:407-432` (source-catalog line, not present at HEAD) at HEAD), which runs `bun run test:history_summarizer-eval-unit` plus
two `run-history_summarizer-eval.ts` modes, `--lint` and `--mutations`. Its scorer
imports `validateHistorySummarizerOutput`, `validateStoredHistorySegments`,
`shouldDiscardLastHistorySummarizerHistorySegment`, and `HISTORY_SUMMARIZER_BOUNDARY_HEALING_SLACK`
from `packages/plugin/src/hooks/eidnara/history_segment-runner-validation.ts` (source-catalog path, not present at HEAD)
(`scorer.ts:20-26`), and `appendHistorySegments` plus `promoteSessionFactsDurable`
...
target, no `daemon`, and no `history_summarizer_validate.rs` anywhere in it. The
second workflow, `history_summarizer-eval.yml`, is `workflow_dispatch`/`schedule` only
and also runs no Rust target.

That lane's own code excludes the Rust leg explicitly.
`run-test-selection.ts:73-76` says the harness-booting history_summarizer-eval tests are
"TS-mode only: `daemon`'s Rust history_summarizer producer does not promote claims, so
...
(`history_summarizer_validate.rs:1384`), `five_message_narrative_gap_rejects_like_typescript_validator`
...
described in `history_summarizer-eval/README.md` as "the best TS<->Rust validator
...
### Resolved: the history_summarizer is reachable by default
...
config keys `/history_summarizer/module_model`, `/history_summarizer/model`, and their fallback
arrays (`config.rs:390-428`), and an empty chain short-circuits every entry point
(`lib.rs:5020-5030`, `lib.rs:5230-5232`, `history_summarizer.rs:1249-1251`). So a bare
`DaemonConfig::default()` has no models and the history_summarizer is off.

**What settles it is that a completed setup cannot omit a history_summarizer model.** Two
...
- Both shipped setup paths always call it for the history_summarizer role and always carry
  ...
  `/history_summarizer/model` pointer that `config.rs:411` reads: `setup-pi.ts:216-246`
  declares `history_summarizerModel: string` as a required field (`:219`) and writes
  `config.history_summarizer.model` at `:242-246`.
...
`setup-pi.ts:219` types `history_summarizerModel` as a required `string`, so the write at
`:242-246` is unconditional. `setup-opencode.ts:237` types it as
`string | null` and guards the write with `if (options.history_summarizerModel)` at
...
disables it: the history_summarizer model is not a prompt the user can decline, it is a
...
the history_summarizer is off by choice.
...
| [history_summarizer-single-flight-admits-one-publish-per-firing](#history_summarizer-single-flight-admits-one-publish-per-firing) | safety | high |
...
| [hv-no-cross-history_segment-content-distinctness](#hv-no-cross-history_segment-content-distinctness) | safety | high |
| [hv-single-history_segment-skips-lookahead-discard](#hv-single-history_segment-skips-lookahead-discard) | safety | high |
...
(`history_summarizer.rs:1707`) from the publish itself (`memory-store:9360`), which is exactly
...
`publish_history_summarizer_chunk_persists_transcript_inside_cas` and `:17017`
`publish_history_summarizer_chunk_cas_conflict_leaves_no_transcript_row` cover the
...
Guarantee: The history_segment rows, the deflated original messages, the queued side
channels, the raised publication floor, and the history_summarizer phase reset either all
become durable together or none of them do.
Check: `always` - after any outcome of `publish_history_summarizer_chunk`, the observed
store satisfies: `count(history_segments appended by this predicate) > 0` if and only
if `publication_floor_ordinal >= validated.unprocessed_from` and
`meta.history_summarizer.state == Idle` at the published `firing_seq` and a
...
(`publish_history_summarizer_chunk_fails_loud_from_non_publish_state`). Status
`unaudited`.
Impact: A partial commit that appended history_segments without raising the floor, or
...
Reachability: explicit-config-only
Status: active
Exercised: partial - `history_summarizer.rs:4596`
...
history_segment rows, no transcript rows, and no raised floor, and the next load
makes the session refire-eligible rather than stuck.
Check: `always-or-unreached` - if a restart observes
`state in {Firing, Validating, Publishing}`, then no history_segment appended by that
...
(`history_summarizer.rs:1707` versus `memory-store:9360`), that the publish CAS uses the
version that transition wrote (`history_summarizer.rs:1719`), and that
...
- `handle_restart_load` for `AwaitingProducer` with missing producer ids abandons
  (`history_summarizer.rs:630-639`), but `MissingProducerIds` is also a
...
Reachability: explicit-config-only
Status: active
Exercised: partial - `history_summarizer.rs:2323`
...
`publish_history_summarizer_chunk_fails_loud_from_non_publish_state` proves some phases
...
`meta.history_summarizer.state == Publishing`. `always` rather than `unreachable`, because
...
reaches `publish_history_summarizer_chunk` from `AwaitingProducer`.
`publish_output_from_awaiting` always transitions first
(`history_summarizer.rs:1706-1707`), so today the coverage check is on the preconditions:
the store's phase guard admits two phases, and the module has more than one
publish route (`history_summarizer.rs:527-530`, plus two fence implementations at
...
Impact: The documented five-phase machine (`history_summarizer.rs:1-6`) and the
"fail-closed before any database write" claim (`history_summarizer_validate.rs:1-9`) both
...
Guarantee: Every history_segment appended by a publish has, in the same transaction,
a durable deflated copy of the original wire messages for the range it replaces.
Check: `always` - for every appended history_segment sequence `s`, a
`chunk_transcripts` row exists with `history_segment_seq = s` and non-null
`raw_messages_deflate` that inflates to the exact JSON serialized at
`history_summarizer_chunk.rs:717-727`. `always` because this is the invariant that makes
...
`raw_chunk_messages` from `history_summarizer_chunk.rs:717-727` through
`history_summarizer.rs:525` and `memory-store:9472-9481` into `:12687-12713`; confirmed no
...
- `insert_chunk_transcripts_tx` returns early when both payloads are absent
  (`memory-store:12691-12693`) and when the history_segment list is empty
  (`:12679-12681`). The history_summarizer path cannot reach either, but nothing in the
  store's own signature prevents another caller from publishing history_segments with
...
number of publishes.
Check: `always` - after any number of publishes and evictions, every history_segment
...
`count(history_segments)` unchanged. `always` because it constrains every reachable
...
(`history_summarizer.rs:1707`) and the transaction at `memory-store:9360`. The first to commit
...
Confidence: high - [evidence](evidence/history_summarizer-single-flight-admits-one-publish-per-firing.md).
Verified three independent layers: `fire` refuses non-idle
(`history_summarizer.rs:251-253`), the store predicate binds five fields
...
Reachability: explicit-config-only
Status: active
Exercised: not yet - `history_summarizer.rs:3389`
...
`start` calls made.
Check: `always` with per-drive accounting - per call to `run_history_summarizer_firing`,
...
`try_next_model`, the loop `continue`s at `history_summarizer.rs:1318-1321` back to the top
...
attempt lineage rooted at one `run_history_summarizer_firing` call, and the counter must
...
(`history_summarizer_producer.rs:80`), meaning the request may have reached ModelExecution and begun
...
Read `history_summarizer.rs:1290-1329` and confirmed there is no `cancel`, no outcome
...
does `heuristic_decision` (`history_summarizer_producer.rs:412-433`).
Existing check: `history_summarizer.rs:3389`, `:3444`, `:3498`
...
attempt fires under a new `firing_seq` (`history_summarizer.rs:257`) with its own producer
...
(`lib.rs:43047`) drives repeated provider-reported refusals of one chunk across
firings, and `handler_setup_failure_does_not_placeholder_the_chunk` (`:43104`)
...
re-evaluated at `lib.rs:5938`. Each expiry admits one more firing, each
...
(`lib.rs:5933-5939`, with `now` arriving through `HistorySummarizerPrepareContext` at
`:9582`), so expiring the durable field is equivalent to advancing the clock.
The test helper `expire_history_summarizer_backoff` (`lib.rs:42230-42237`) already does
exactly this by committing `Some(now_ms() - 1)`, and
`assert_seeded_phase_recovers_then_refires_after_backoff` (`:42239`) drives a
...
(`history_summarizer.rs:1145-1154`) preserves rather than escalates the cooldown; the
...
for the chunk start (`lib.rs:6309`, `history_summarizer.rs:361-410`); a reattached
run that ends the same way counts too (`lib.rs:5615`). Only the
firing's final error counts: a rejection or provider failure that falls back to
the next model in the chain (`history_summarizer.rs:1959-1964`, `:1996-2001`)
...
working model could summarize. The count is keyed by the model chain as well as
the chunk start (`memory-store/src/lib.rs:541-546`), so a changed chain sends the
bytes to the new models before any placeholder. A re-adopted tail message at or
past the chunk start clears the count (`transform.rs:5596-5607`), since the retried
bytes changed. Assembly varies the
...
model call (`history_summarizer_chunk.rs:572-662`); the placeholder stops before
a tool arc the shrunken chunk end splits, and reaches the result of one that opens
the chunk. A reattachment presents the
frozen range under the same reduced budget (`lib.rs:5488-5504`), so it withdraws
...
Reachability: explicit-config-only
Status: active
Exercised: partial - `history_summarizer.rs:2309` and `:4199` assert the floor equals a
specific value after a publish, and `:2360` and `:3006` assert it stays `None`
after a rejection. No test relates the floor to the appended history_segment ends
generally. Not run in CI.
Guarantee: After any publish, the publication floor is at most one past the
highest appended history_segment end, and it never decreases.
Check: `always` - after every committed publish,
`meta.publication_floor_ordinal <= MAX(end_message) + 1` over all history_segments
...
(`history_summarizer_validate.rs:638`), so the interesting case is a healed batch where
the last emitted history_segment was popped (`:539-556`).
Required faults and enabling state: A configured model chain and an accepted
publish whose last history_segment was discarded by boundary healing, which needs at
least two history_segments and a lookahead distance within `BOUNDARY_HEALING_SLACK`
(`history_summarizer_validate.rs:19`, applied at `:554`).
Confidence: high - [evidence](evidence/publication-floor-never-outruns-appended-coverage.md). Traced
the floor from `history_summarizer_validate.rs:638` through `history_summarizer.rs:1725` and `:523`
to `memory-store:9484-9488`, and confirmed the empty-history_segment rejection at
`history_summarizer_validate.rs:487-491` and the forward-progress check at `:625-635`
together prevent a floor advance with no appended coverage.
Existing check: `history_summarizer.rs:2309`, `:2360`, `:3006`, `:4199`. Status
`unaudited`.
...
fencing (`:1339-1357`) while no history_segment summarizes them. They would still be
...
claim and `history_summarizer.rs:2360` and `:3006` show a rejected run leaves the floor
...
`HistorySummarizerDurableState` (`history_summarizer.rs:353-360`), which lives under
`meta.history_summarizer` and cannot touch the sibling field. The comment at
...
Guarantee: A wrapup drain either advances the maximum history_segment end ordinal on
...
on termination: after `MAX_WRAPUP_REQUEST_BUDGET` (3800 s, `history_summarizer.rs:962`)
...
fence rejection with no cooldown (`history_summarizer.rs:533-547`), are the two shapes
that could spin.
...
- Can a fence rejection with no cooldown (`history_summarizer.rs:533-547`) recur every
...
history_segment's ordinal span without looking at its body again. All three leave
...
Guarantee: A published history_segment set is bound to the specific chunk it
...
`validate_history_summarizer_output` with an output fixture taken from an unrelated
conversation B whose history_segment ranges have been renumbered to be contiguous over
...
(`history_summarizer_validate.rs:5-9`, `:450-455`).
...
Required faults and enabling state: A configured history_summarizer model chain; a producer
that returns a document whose history_segment ranges are contiguous over
...
Existing check: none. The chunk fingerprint at `history_summarizer.rs:1449-1461` and
...
- Is a chunk-derived echo requirement compatible with the byte-identical
  TypeScript oracle the golden test pins (`history_summarizer_validate.rs:1384`)? (needs
  human input)
...
beside a history_segment describe the same message range the model summarized.
...
(`history_summarizer.rs:2369`). **It cannot.** The reattach passes
`range.to_ordinal.saturating_add(1)` to `build_history_summarizer_chunk` as its exclusive
`eligible_end_ordinal` (`lib.rs:4696-4702`), and the builder admits no message at
or beyond that bound: the start scan filters
`message.ordinal < eligible_end_ordinal` (`history_summarizer_chunk.rs:373-375`) and the
...
`history_summarizer.rs:645` and `:1529-1530`. Confidence is raised from medium because the
...
Guarantee: When healing widens a history_segment's ordinal range, the widened range
still contains only ordinals whose content the history_segment's body is entitled to
claim.
Check: `always-or-unreached` - for every accepted history_segment whose `end_message`
...
antecedent is a healed history_segment: `heal_history_segment_gaps` and
`heal_terminal_completed_tool_arc` are optional paths that most publishes never
enter, and the property must hold for each healed history_segment whenever one occurs.
Fault/timing angle: The window is between parse and mapping:
`heal_history_segment_gaps` (`:493-497`) and `heal_terminal_completed_tool_arc`
...
`completed_tool_arcs` populated by `history_summarizer_chunk.rs`, and a model output that
leaves a gap inside one of them.
Confidence: high - [evidence](evidence/hv-heal-extends-range-without-revalidating-content.md).
`heal_history_segment_gaps` sets `history_segments[i - 1].end_message` at `:927-930` and
...
`map_parsed_history_segments_to_chunk` then stamps a NEW `end_message_id` from the
...
Impact: The stored history_segment asserts coverage of raw messages its summary never
...
- Are `tool_only_ranges` and `completed_tool_arcs` themselves derived only from
  module-side classification, with no model influence? `history_summarizer_chunk.rs` builds
...
re-derivation and does not apply to a single-history_segment output, which is exactly
...
history_segment title (`:1367-1375`), never a degenerate one.
Guarantee: The publish path can be reached with a history_segment body that carries
essentially no information about the ordinals it replaces.
Check: `sometimes` - at least one campaign publish reaches the
`to_stored_history_segment` call at `history_summarizer.rs:466`, inside `publish_validated_chunk`
(`:444`), with an accepted history_segment whose `p1.trim().chars().count() == 1` and
...
history_segment, so location coverage is satisfied by any publish at all and proves
...
`to_stored_history_segment` call is `:466`.
Fault/timing angle: None. No interleaving needed.
Required faults and enabling state: A configured history_summarizer model chain and a
producer returning one well-formed history_segment with a one-character `p1`, a
...
Compounded by hv-single-history_segment-skips-lookahead-discard, because a
one-history_segment output also skips the discard-last protection.
Open questions:

- Does the project want a span-relative floor, or is body adequacy deliberately
  delegated to the history_summarizer-eval scorer lane (`ci.yml:415-440` (source-catalog line, not present at HEAD))? (needs human
  input)

### hv-no-cross-history_segment-content-distinctness

Type: safety
Reachability: default-production
Status: active
Exercised: not yet - no test emits two history_segments with identical bodies.
Guarantee: Two history_segments published in one run that cover disjoint ordinal
ranges do not carry byte-identical bodies.
Check: `always` - for every accepted `ValidatedChunk`, no two elements of
`history_segments` share the same `(title, p1, p2, p3, p4)` tuple. `always` because it
is a property of each admitted set, evaluated at every publish.
Fault/timing angle: None.
Required faults and enabling state: A producer returning N contiguous history_segments
whose ranges partition the chunk and whose bodies are copies of one another.
Confidence: high - [evidence](evidence/hv-no-cross-history_segment-content-distinctness.md).
`validate_parsed_history_segments` (`:983-1084`) iterates history_segments and compares
...
covers the k1/k2 lookahead boundary with two history_segments; no test covers the
one-history_segment case.
Guarantee: A history_segment whose terminal boundary was chosen with less than
`BOUNDARY_HEALING_SLACK` ordinals of lookahead is withheld for re-derivation,
regardless of how many history_segments the run produced.
Check: `always-or-unreached` - whenever
`chunk.end_index - last.end_message <= BOUNDARY_HEALING_SLACK`, `in_emergency` is
false, and `force_keep_last_history_segment` is false, the accepted set does not
contain that last history_segment. `always-or-unreached` rather than `always` because
...
history_segment ending at or within two ordinals of `chunk.end_index`, with
`in_emergency` and `force_keep_last_history_segment` both false.
Confidence: high - [evidence](evidence/hv-single-history_segment-skips-lookahead-discard.md). The guard
at `:539` requires `history_segments.len() >= 2`; with one history_segment the whole block
`:539-558` is skipped and `discarded_last` stays false.
`BOUNDARY_HEALING_SLACK = 2` at `:19`, applied at `:554`.
Existing check: `:539-558` for two or more history_segments; tests at `:1633`,
`:1730`, `:1748`, `:1849`.
...
- Is the `>= 2` guard intentional (popping the only history_segment would fail the
...
applies exactly one text transform, `unescape_xml` (`history_summarizer_validate.rs:1148-1154`),
which decodes five entities and strips nothing, and exactly one numeric narrowing,
an unchecked `as i32` on `importance` (`history_summarizer.rs:57`). The compensating
...
characters or line/paragraph separators into durable history_segment rows.
Check: `always` - for every accepted history_segment, no character of `title`,
`content`, or `p1`..`p4` satisfies `char::is_control()` or is
`\u{2028}`/`\u{2029}`. `always` because it must hold for each admitted
history_segment.
Fault/timing angle: None.
Required faults and enabling state: A producer emitting a history_segment whose `p1`
...
own comment as "HistorySummarizer-authored titles are untrusted". Bodies have no
equivalent.
...
Reachability: default-production
Status: active
Exercised: not yet - `history_summarizer_prompt.rs:430-431` tests the escape direction
...
(`decay_render.rs:80-84`, `history_summarizer_prompt.rs:104-108`) escapes `&` first and is
therefore correct; only the inverse is wrong.
Existing check: `history_summarizer_prompt.rs:427-432` asserts the escape functions, never
the inverse.
...
Guarantee: A published history_segment's stored `importance` lies in the documented
1..=100 band that the decay curve consumes, or the output is rejected.
Check: `always` - for every `StoredHistorySegment` produced by
`to_stored_history_segment`, `1 <= importance <= 100`. `always` because the invariant
...
(`:1106-1110`), narrowed by `as i32` at `history_summarizer.rs:57`. Confirmed no clamp in
...
Impact: A history_segment the model marked maximally important is stored with a
...
- Are there stored-history_segment consumers besides `decay_render.rs` that read
...
hand-built input (`history_summarizer.rs:4173`, `:4328`, `:4414`, `:4495`), demonstrating
the bypass is trivially constructible, but no test asserts the production
invariant.
Guarantee: Every set of history_segments that reaches the durable publish transaction
was produced by `validate_history_summarizer_output` from the same output text and the same
pinned chunk.
Check: `always` - every execution of `publish_validated_chunk`
(`history_summarizer.rs:444`) is preceded in the same call chain by a successful
`validate_history_summarizer_output` over the text that produced its `validated` argument.
...
`history_summarizer.rs:444` is `pub fn` inside `pub mod history_summarizer` (`lib.rs:19`), and
`history_summarizer_validate` is also `pub` (`lib.rs:23`). Confirmed by `rg` that the only
production constructions of a `ValidatedChunk` are `history_summarizer_validate.rs:628`
(the gate's own return) and test code at `history_summarizer.rs:4476`.
Existing check: convention only. Both production paths (`history_summarizer.rs:1419`,
...
write model text into durable history_segments with zero validation. This is the
...
Reachability: default-production
Status: active
Exercised: partial - `tierless_history_segments_reject_while_p1_only_output_keeps_soft_fallbacks` (`:1463`)
...
validation rejects every tierless history_segment before publish.
Check: `unreachable` - the `1` arm of the `legacy` expression at `history_summarizer.rs:65`
...
`history_summarizer.rs:60-62` states the claim: "Strict validation makes tierless output
...
`p1`, and `ValidatedHistorySegment.p1` is a clone of the parsed value (`:972`), so the
projection sees the validated field.
Existing check: `history_summarizer.rs:63-67` is itself the defensive derivation; `:1463`
...
`events beyond persisted history_segment count are filtered` (golden case) cover the
...
history_segment is either rejected with the run or reported, never silently discarded.
Check: `always-or-unreached` - for every input side-channel item with
`origin_history_segment_index` or `at_history_segment` outside `1..=persisted_count`, the
...
anchored to a history_segment index above the count that survives discard-last.
...
- **Two freshness inputs at one commit point.**
  ...
  [history_summarizer-single-flight-admits-one-publish-per-firing](#history_summarizer-single-flight-admits-one-publish-per-firing),
...
- **The gate ran, and what happens if it did not.**
  ...
  convention breaks (`history_summarizer.rs:59-66`, whose comment names "a future bypass"
...
- **Nothing ties the words to this conversation.**
  ...
  [hv-no-cross-history_segment-content-distinctness](#hv-no-cross-history_segment-content-distinctness).
...
- **A range that moves after the model spoke.**
  ...
  [publish-fence-rejects-selected-content-drift](#publish-fence-rejects-selected-content-drift),
  [hv-single-history_segment-skips-lookahead-discard](#hv-single-history_segment-skips-lookahead-discard).
  ...
  `force_keep_last_history_segment: false` regardless of the original firing's
  ...
  not run for a single history_segment. No dominance is claimed inside this cluster. It
  is the one place in the part where the records genuinely compose into a scenario
  nobody has constructed: a single-history_segment reattach whose range is recomputed
  ...
  `history_summarizer_chunk.rs:373-375` and `:383-386`) makes a wider range impossible. The
...
- **The floor as a publish detector.**
  ...
  cluster too, because the floor and the history_segment rows being in one transaction
...
- **How many runs, how many refires, and who is told.**
  ...
  Three records whose shared consequence is that a degrading history_summarizer is
