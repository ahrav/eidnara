...
use crate::wire::{FrameId, encode_owned_frame, pure_header_flags, response_flags};
...
async fn charged_error_body(
    ...
    let charge = charge_frame_or_cancel(budget, generation, frame_bytes, deadline, None)
        .await
        .ok_or(())?;
...
async fn emit_frame_with_written(
    ...
    let charge = if body.is_empty() {
        ...
        charge_frame_or_cancel(budget, generation, frame_bytes, deadline, None)
            .await
            .ok_or(())?
    };
...
pub async fn emit_error_terminal(
    ...
    let Ok((body, deadline)) = charged_error_body(budget, generation, code, message, None).await
    else {
        generation.token.cancel();
        return;
    };
    ...
    let (ty, flags, body) = match terminal {
        ...
        Terminal::Error {
            ...
            let Ok((body, deadline)) =
                charged_error_body(budget, generation, &code, &message, retry_after_ms).await
            else {
                generation.token.cancel();
                return true;
            };
...
pub struct StreamSink {
    ...
    pub(crate) generation: Arc<GenerationCore>,
...
impl StreamSink {
    pub(crate) async fn reserve(&self, max_len: usize) -> Result<OutputBuffer, StreamClosed> {
        ...
        let deadline = self.generation.writer.admission_deadline();
        let charge = charge_frame_or_cancel(
            &self.budget,
            &self.generation,
            bytes,
            deadline,
        ...
        if self.cancel.is_cancelled()
            || self.generation.token.is_cancelled()
            || self.settlement.won.load(Ordering::SeqCst)
        {
            return Err(StreamClosed);
        }
...
pub async fn emit_rejection<H: HostHandler>(
...
pub async fn handle_host_shutdown<H: HostHandler>(
    shared: &Arc<HostShared<H>>,
    generation: &Arc<GenerationCore>,
    corr: u64,
) {
    loop {
        ...
        match shared.shutdown_latch.try_own() {
            ...
            crate::lifecycle::LatchDecision::Wait => {
                tokio::select! {
                    biased;
                    () = generation.token.cancelled() => return,
    ...
    let Some(charge) = charge_frame_or_cancel(
        &shared.egress_budget,
        generation,
        frame_bytes,
        deadline,
    ...
    let gen_watch = Arc::clone(generation);
...
pub(crate) async fn emit_authoritative_rejection<H: HostHandler>(
    shared: &Arc<HostShared<H>>,
    generation: &Arc<GenerationCore>,
    id: FrameId,
    code: &'static str,
...
/// Runs the handler's `route_gone` callback once per route under the lifecycle deadline.
/// Returns whether the host is still running afterwards: a deadline overrun trips the fatal
/// latch, and a panic surfaces through the lifecycle join.
async fn run_route_gone<H: HostHandler>(shared: &Arc<HostShared<H>>, handle: RouteHandle) -> bool {
...
pub(crate) async fn finish_route_close<H: HostHandler>(
    shared: &Arc<HostShared<H>>,
    handle: RouteHandle,
) {
    if run_route_gone(shared, handle).await {
...
pub(crate) async fn settle_route_work<H: HostHandler>(
    shared: &Arc<HostShared<H>>,
    handle: RouteHandle,
    decision: CloseDecision,
) -> bool {
...
pub async fn close_generation<H: HostHandler>(
    shared: &Arc<HostShared<H>>,
    generation: &Arc<GenerationCore>,
    ...
    let sweep = shared.registry.begin_close_generation(generation.id);
...
pub async fn force_close_all_routes<H: HostHandler>(shared: &Arc<HostShared<H>>) {
...
/// (protocol §12 step 4).
pub async fn send_connection_goodbye(
    generation: Arc<GenerationCore>,
...
pub fn handle_cancel(generation: &GenerationCore, key: PendingKey) {
    let pending = generation.pending.lock().expect("pending lock");
    if let Some(entry) = pending.get(&key)
        && !entry.settlement.is_settled()
    {
        entry.cancel.cancel();
    }
}
...
#[cfg(test)]
mod tests {
    use super::{Terminal, bounded_terminal_error, error_body_json_into, error_body_len};
