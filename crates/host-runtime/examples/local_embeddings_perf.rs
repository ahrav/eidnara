//! This binary generates hermetic LocalEmbeddings query and batch load.
//!
...
use host_runtime::local_embeddings::jobs::BatchItem;
use host_runtime::local_embeddings::{
    EmbeddingEngine, LOCAL_EMBEDDINGS_MODULE_ID, LaneInfo, LocalEmbeddingsComponent,
...
use perf_measurement::{
    AttemptDisposition, AttemptRecord, DeterministicRng, LocalEmbeddingsMethod,
    LocalEmbeddingsVariant, LogicalDisposition, LogicalRecord, ServiceSample, WindowClass,
};
use tokio::io::{AsyncReadExt, AsyncWriteExt, ReadHalf, WriteHalf};
use tokio::net::UnixStream;
use tokio::sync::{Mutex, oneshot};

const ROOT: &str = "/workspace/local_embeddings-perf";
...
const MODEL: &str = "local_embeddings-perf-tiny";
...
struct Opts {
    variant: LocalEmbeddingsVariant,
    arm: Arm,
    load: Load,
    seconds: u64,
    engine_delay_ms: u64,
...
fn parse_opts_from(args: impl IntoIterator<Item = String>) -> Result<Opts, String> {
    ...
    while let Some(flag) = args.next() {
        ...
        match flag.as_str() {
            "--variant" => variant = Some(LocalEmbeddingsVariant::parse(&value)?),
            "--arm" => arm = Some(value),
    ...
    if requires_build_id(variant) && HOST_BUILD_ID.is_none() {
        return Err(format!(
            "variant {} is defined as pre-change host code, which this binary \
             ...
             pre-change commit with EIDNARA_HOST_PERF_BUILD_ID set, so the emitted \
...
/// `EIDNARA_HOST_PERF_BUILD_ID` identifies the build that produced this binary.
///
...
const HOST_BUILD_ID: Option<&str> = option_env!("EIDNARA_HOST_PERF_BUILD_ID");
...
fn requires_build_id(variant: LocalEmbeddingsVariant) -> bool {
    matches!(
        variant,
        LocalEmbeddingsVariant::Baseline | LocalEmbeddingsVariant::HygieneOnly
    )
}
...
struct PlaceholderModelExecution;

impl CompositeComponent for PlaceholderModelExecution {
    fn manifest(&self) -> ManifestSnapshot {
        ManifestSnapshot {
            module_id: "model_execution".to_owned(),
            module_version: env!("CARGO_PKG_VERSION").to_owned(),
    ...
    async fn bind(&self, _route: RouteHandle, _identity: RouteIdentity) -> BindOutcome {
        BindOutcome::Reject {
            ...
            message: "model_execution is unavailable in the perf host".to_owned(),
        }
    }
...
impl SecondaryComponent for PlaceholderModelExecution {
...
impl HostThread {
    fn start(
        ...
        let composite = StaticComposite::new(PerfPrimary, component, PlaceholderModelExecution)
        ...
        let max_resident_bytes = HostLimits::default()
            .max_resident_bytes
            .checked_add(host_runtime::local_embeddings::bundle::MAX_MODEL_BYTES)
        ...
        let thread = std::thread::Builder::new()
            ...
            .spawn(move || {
                let result = tokio::runtime::Builder::new_multi_thread()
                    ...
                    .and_then(|runtime| {
                        runtime
                            .block_on(host_runtime::run(
                                composite,
                                HostConfig {
                                    data_dir: Some(data_dir),
                                    daemon_ver: "eidnara-host/local_embeddings-perf".to_owned(),
                                    limits: HostLimits {
                                        max_resident_bytes,
                                        ..Default::default()
                                    },
...
impl RoutedWire {
    async fn connect(publication: &std::path::Path, origin: Instant) -> Result<Arc<Self>, String> {
        ...
        let (channel, epoch) = client
            .route_open_target(
                "management_surface",
                LOCAL_EMBEDDINGS_MODULE_ID,
                ROOT,
                "local_embeddings-perf",
                "run",
            )
            .await?;
    ...
    async fn record_call(
        ...
        match result {
            Ok(reply) => {
                ...
                let disposition = if method == LocalEmbeddingsMethod::Result {
            ...
            Err(WireCallError::Timeout {
                ...
                self.attempts.lock().await.push(AttemptRecord {
                    ...
                    attempt_id,
                    method,
                    disposition: if method == LocalEmbeddingsMethod::Result {
                        AttemptDisposition::Poll
                    } else {
                        AttemptDisposition::Timeout
                    },
...
async fn execute_batch(
    ...
    let request_key =
        host_runtime::local_embeddings::protocol::canonical_request_key(&lane(), &items);
    let mut params = constraints();
...
struct Summary {
    ...
    ledger: perf_measurement::LocalEmbeddingsLedgerSummary,
...
> {
    ...
    let mut limits = LocalEmbeddingsLimits {
    ...
    let component = LocalEmbeddingsComponent::ready_with_engine(lane(), engine, limits)
        .map_err(|error| format!("validate LocalEmbeddings limits: {error}"))?;
    ...
    let ledger =
        perf_measurement::validate_local_embeddings_ledgers(&logical_estimates, &attempt_estimates);
    ...
    let latency_residual_ns = SignedSummary::from_unsorted(
        attempt_estimates
            .iter()
            .filter(|attempt| {
                attempt.method == LocalEmbeddingsMethod::Query
    ...
    let summary = Summary {
        kind: "local_embeddings_perf_summary",
...
fn censored_count(ledger: &perf_measurement::LocalEmbeddingsLedgerSummary) -> u64 {
...
fn emit(
    ...
    for record in logical {
        println!(
            ...
            serde_json::to_string(&serde_json::json!({
                "kind": "local_embeddings_perf_logical",
    ...
    for record in attempts {
        println!(
            "{}",
            serde_json::to_string(&serde_json::json!({
                "kind": "local_embeddings_perf_attempt",
                "variant": summary.variant,
                "arm": summary.arm,
                "load": summary.load,
                "seed": summary.seed,
    ...
    for (index, sample) in service_samples.iter().enumerate() {
        println!(
            "{}",
            serde_json::to_string(&serde_json::json!({
                "kind": "local_embeddings_perf_service",
                "variant": summary.variant,
                "arm": summary.arm,
                "load": summary.load,
                "seed": summary.seed,
...
fn main() {
    let opts = match parse_opts() {
        Ok(opts) => opts,
        Err(error) => {
            eprintln!("local_embeddings_perf failed: {error}");
            std::process::exit(1);
        }
    };
        ...
        .build()
    {
        Ok(runtime) => runtime,
        Err(error) => {
            eprintln!("local_embeddings_perf failed: generator runtime: {error}");
            std::process::exit(1);
        }
    };
    ...
    match result {
        Ok((logical, attempts, service_samples, summary)) => {
            if let Err(error) = emit(&logical, &attempts, &service_samples, &summary) {
                eprintln!("local_embeddings_perf failed: emit: {error}");
                std::process::exit(1);
            }
            ...
            {
                eprintln!(
                    "local_embeddings_perf failed: invalid repetition (ledger={}, fatal={}, missed_slots={}, send_lag_max_ns={}, writer_wait_max_ns={}, censored_per_mille={:.3}, poll_max={} vs ceiling {}, overdeadline_writes={}, measured_offered={})",
...
mod tests {
    ...
    fn terminal_rejection_is_not_censoring() {
        let ledger = perf_measurement::validate_local_embeddings_ledgers(
    ...
    fn attempt_latency_is_keyed_by_wire_method() {
        ...
        let attempts = [
            attempt(1, LocalEmbeddingsMethod::Batch, 500),
            attempt(2, LocalEmbeddingsMethod::Result, 10),
            attempt(3, LocalEmbeddingsMethod::Result, 30),
        ];
    ...
    fn terminal_module_restarted_rejection_keeps_ledgers_valid() {
        ...
        let ledger = perf_measurement::validate_local_embeddings_ledgers(&[record], &attempts);
    ...
    fn variant_is_required_and_waiter_combinations_are_typed() {
        ...
        assert_eq!(a.variant, LocalEmbeddingsVariant::APlusC);
