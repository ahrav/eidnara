...
(`packages/shm-native/src/lib.rs:490-491`, exported at `index.ts:526-529`)
...
| `instance_two_incarnations_published_distinct_keys_in_one_data_directory` | Two `InstanceGuard::acquire` calls in one directory produced different key bytes (`instance.rs:263-264`) | The documented behaviour (`docs/host-wire-protocol.md:105`), so observing it is a fact about the code rather than an outcome. Do not compare the bytes in a message; compare fingerprints |
...
added to an `host-runtime` in-crate module is added to a suite no automation executes.
...
1. **S0, executing what already exists.** A workflow change and nothing else:
   `cargo nextest run -p host-runtime --lib` alongside the existing filtered steps
...
6. **A hand-built grant against `activate_client`.** `activate_client` is `pub` and
   ...
   (`packages/shm-native/src/setup.rs:115-124`, `lib.rs:588-594`), which is the
...
8. **A marker seam inside the native addon.** `setup-a-only-an-authenticated-grant-enters-the-native-channel-registry`
   ...
   `packages/shm-native/src/lib.rs`. That is a source change to a shipped
...
9. **A CI ordering change plus a fixture for S6.** Producing
   ...
   (`docs/shm-transport.md:83`) whose classification is already tested.
