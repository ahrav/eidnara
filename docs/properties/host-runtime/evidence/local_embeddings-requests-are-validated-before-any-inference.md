...
Handler order. `handle` (`src/local_embeddings/mod.rs:866-944`) requires a ready lane
...
Preflight. `preflight` (`src/local_embeddings/protocol.rs:553-564`) rejects binary
...
Foreign jobs. `JobTable::parse_job_id` (`src/local_embeddings/jobs.rs:268-282`)
...
Existing checks. All but one are in `tests/local_embeddings_protocol.rs`, a
default-harness binary run by CI (`.github/workflows/ci.yml:118`), and use
`DeterministicEngine` (`tests/support/local_embeddings.rs:40-48`), which counts
`calls` at `:101`:
...
- Sources examined: `src/local_embeddings/mod.rs:866-944`, `:515-640`, `:642-700`,
  `:702-746`; `src/local_embeddings/protocol.rs:553-590`, `:639-768`.
...
- Sources examined: `src/local_embeddings/mod.rs:904-910`, `:651-662`;
  `src/local_embeddings/jobs.rs:305-398`.
