...
use kernel::{
    ArtifactDestination, ArtifactEligibility, ArtifactErrorKind, ArtifactHandle, DecisionRow,
    EligibilityCandidate, EligibilityVerdict, HeldEvidence, KernelError, KernelStore,
    MAX_RUN_BUFFER_BYTES, MEMORY_REVIEWER_CAPTURE_RETENTION_CLASS, MemoryReviewerHold,
...
/// Origin of one MemoryReviewer-visible buffer, from a closed set.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub enum ReferenceExpectation {
    ...
    /// Evidence a MemoryReviewer run captured for itself, protected by its execution hold until its acquisition reference expires.
    TemporaryCapture {
        evidence_id: String,
        artifact_digest: String,
        byte_length: u64,
...
/// Provenance of one MemoryReviewer-visible buffer (Q16): kept in memory beside the rendered bytes, never in any schema.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub struct RunBinding {
    pub hold: MemoryReviewerHoldBinding,
...
pub enum HeldUnder<'a> {
    ...
    Review {
        hold: &'a MemoryReviewerHold,
        binding: &'a MemoryReviewerHoldBinding,
    },
}

impl HeldUnder<'_> {
    pub fn kind(&self) -> MemoryReviewerHoldKind {
        match self {
            Self::Execution(_) => MemoryReviewerHoldKind::Execution,
            Self::Review { .. } => MemoryReviewerHoldKind::Review,
        }
    }
    ...
    pub fn binding(&self) -> &MemoryReviewerHoldBinding {
        match self {
        ...
        let (bytes, rendered, verdict, sensitivity, member, origin_key) = match &expectation {
            ...
            ReferenceExpectation::TemporaryCapture {
                ...
                if held.byte_length != *byte_length
                    || held.retention_class != MEMORY_REVIEWER_CAPTURE_RETENTION_CLASS
    ...
    fn load_range(
        ...
        if loaded {
            ...
            *held = store
                ...
                .pop()
                .ok_or_else(|| refuse(Some(alias), RefusalCode::HoldInvalid))?;
            // Only a MemoryReviewer capture's `retain_until` is an acquisition deadline; on other classes it is a GC retention floor the hold overrides.
            if held.retention_class == MEMORY_REVIEWER_CAPTURE_RETENTION_CLASS
...
pub(crate) fn hold_refusal(error: MemoryReviewerHoldError) -> RefusalCode {
    match error {
        MemoryReviewerHoldError::Refused(
            kernel::MemoryReviewerHoldRefusal::UnavailableEvidence,
        )
        | MemoryReviewerHoldError::Refused(kernel::MemoryReviewerHoldRefusal::NotCovered) => {
            RefusalCode::ExpectationChanged
        }
        MemoryReviewerHoldError::Refused(
            kernel::MemoryReviewerHoldRefusal::ProjectBackingExhausted,
        )
        | MemoryReviewerHoldError::Refused(
        ...
        MemoryReviewerHoldError::Refused(_) => RefusalCode::HoldInvalid,
        MemoryReviewerHoldError::Store(KernelError::Deadline)
        | MemoryReviewerHoldError::Store(_) => RefusalCode::Store,
    }
}
