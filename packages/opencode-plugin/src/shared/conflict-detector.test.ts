...
describe("detectConflicts", () => {
    ...
    // OpenCode also accepts `["pkg@version", { ...options }]` tuple entries, which the
    // detector normalizes to the package string before matching.
    describe("plugin entry matching", () => {
        const none = { dcp: false, omo: false };
        const dcp = { dcp: true, omo: false };
        const omo = { dcp: false, omo: true };
        it.each([
            ["an empty plugin list", [], none],
            ["unrelated plugins", ["@eidnara/opencode@latest", "some-other-plugin"], none],
            ["the canonical @tarquinen/opencode-dcp package", ["@tarquinen/opencode-dcp"], dcp],
            ["canonical DCP with a version suffix", ["@tarquinen/opencode-dcp@latest"], dcp],
            ["canonical DCP with a semver range suffix", ["@tarquinen/opencode-dcp@^3.1.0"], dcp],
            ["a DCP fork under a different package name", ["@some-fork/opencode-dcp-fork"], none],
            [
                "a file:// path that contains 'opencode-dcp'",
                ["file:///home/user/work/opencode-dcp-fork"],
                none,
            ],
            ["the canonical oh-my-opencode package", ["oh-my-opencode"], omo],
            ["the canonical oh-my-openagent package alias", ["oh-my-openagent"], omo],
            [
                "canonical OMO with a version suffix",
                ["oh-my-opencode@3.17.5", "oh-my-openagent@latest"],
                omo,
            ],
            ["oh-my-opencode-slim (issue #43)", ["oh-my-opencode-slim"], none],
            [
                "oh-my-opencode-slim with a version suffix (issue #43)",
                ["oh-my-opencode-slim@latest", "oh-my-opencode-slim@1.0.3"],
                none,
            ],
            [
                "a file:// path containing 'oh-my-opencode' (issue #43)",
                ["file:///home/user/workspace/oh-my-opencode-slim-dev"],
                none,
            ],
            [
                "other OMO forks under different package names",
                [
                    "oh-my-opencode-cli",
            ...
            [
                "canonical OMO installed beside slim",
                ["oh-my-opencode-slim", "oh-my-opencode@latest"],
                omo,
            ],
            [
                "DCP expressed as a tuple (issue #49)",
                ["@eidnara/opencode@latest", ["@tarquinen/opencode-dcp@latest", {}]],
                dcp,
            ],
            ["OMO expressed as a tuple (issue #49)", [["oh-my-opencode@latest", {}]], omo],
            [
                "mixed string and tuple entries with unrelated packages (issue #49)",
                [
                    "oh-my-opencode-slim",
                    [
            ...
            [string, Array<string | [string, unknown]>, { dcp: boolean; omo: boolean }]
        >)("%s", (_title, plugins, expected) => {
            writeProjectConfig(plugins);
            const result = detectConflicts(projectDir);
            expect(result.conflicts.dcpPlugin).toBe(expected.dcp);
            // Without an OMO config, every default-active OMO hook conflicts.
            expect(result.conflicts.omoPreemptiveCompaction).toBe(expected.omo);
    ...
    describe("OPENCODE_DISABLE_PRUNE host semantics (file-based arm)", () => {
        ...
        // The spelling rule is shared with OPENCODE_DISABLE_AUTOCOMPACT.
        it.each([
            ["0", true],
            ["1", false],
        ])("value %j leaves prune conflicting = %p, like the host", (value, conflicts) => {
            writeFileSync(
                join(projectDir, "opencode.json"),
    ...
    describe("resolveCompactionForBoot", () => {
        ...
        // so every non-boolean shape and every client failure falls back to file-based detection.
        it.each([
            ["the compaction block is absent", async () => ({ data: {} })],
            [
                "compaction values are not explicit booleans",
                async () => ({ data: { compaction: { auto: "true", prune: null } } }),
            ],
            [
                "the response data is missing entirely",
                async () => ({}) as { data?: Record<string, unknown> },
            ],
            [
                "the client throws",
                async () => {
            ...
            [string, () => Promise<{ data?: Record<string, unknown> }>]
        >)("returns null when %s (file-based fallback, not host defaults)", async (_title, get) => {
            const result = await resolveCompactionForBoot({ config: { get } });
