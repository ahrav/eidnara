...
Attention focus: what the history_summarizer *says* it promises, and what actually
...
`packages/e2e-tests/src/history_summarizer-eval`, `packages/e2e-tests/history_summarizer-eval`,
...
| --- | --- | --- | --- |
| C1 | "Original wire messages for exact durable full-message and verbose recovery." (`history_summarizer.rs:425`) | The folded conversation stays recoverable verbatim after substitution. | `history_summarizer_chunk.rs:717-727` serializes; `history_summarizer.rs:525` carries; `memory-store/src/lib.rs:9472-9481` inserts. Verified. Owned by lens A `publish-preserves-raw-chunk-messages-atomically`. |
| C2 | "Original wire messages in the compacted interval, serialized for durable ctx_expand." (`history_summarizer_chunk.rs:499`) | The `ctx_expand` tool can serve the pre-fold bytes. | Field populated at `history_summarizer_chunk.rs:717-727`. The `ctx_expand` **read** side is outside this scope and unverified here. |
| C3 | "a publish never mutates cached render state directly" (`history_summarizer.rs:5-6`, restated `:443`); "The transaction intentionally leaves render state (`CoreState`, `coverage_ordinal`, watermarks, and m1 revision) untouched" (`memory-store/src/lib.rs:9345-9350`) | Substitution is additive; no pass loses the original view because of a publish. | Verified by lens A over the whole closure `memory-store:9360-9505`. Lens A lead 4 qualifies it: the two publication fences do take the transform snapshot mutex (`lib.rs:3304`, `:3341`). |
| C4 | "malformed ranges, stale chunks, bad message-id endpoints, and boundary-healing decisions are resolved before any database write is possible" (`history_summarizer_validate.rs:6-9`) | No model text reaches storage without passing the gate. | True of the history_segment write. **False of the phase write**: `history_summarizer.rs:1664` persists `Validating` before validating and `:1693-1701` persists the abandon after. Lens B lead 1. |
| C5 | "Strict validation makes tierless output unreachable, but derive legacy from P1 so a future bypass cannot falsely mark a flat row as v2." (`history_summarizer.rs:59-60`) | Every published row is a v2 tiered row, and a bypass degrades safely. | The defensive derivation exists at `history_summarizer.rs:61-66`. The unreachability half is convention only. See lead L1. **Correction:** lens B cites this comment at `:60-62`; at `HEAD` it is `:59-60`, with the `legacy:` expression at `:61-66`. |
| C6 | "Single-flight is enforced here: any non-idle phase returns `Busy` with the unchanged state." (`history_summarizer.rs:232-233`) | One firing per session at a time. | `history_summarizer.rs:251-253`. Verified. Lens A `history_summarizer-single-flight-admits-one-publish-per-firing`. |
...
| C8 | "if it still observes a publishing row, the transaction did not commit, so the stale single-flight is abandoned" (`history_summarizer.rs:616-619`) | A crash before commit leaves no partial fold and does not wedge the session. | `handle_restart_load` at `history_summarizer.rs:620-655`, phases mapped at `:648-653`. Verified. Lens A `crash-before-publish-commit-refires-without-partial-state`. |
| C9 | "insertion/removal and type/id changes alter the fingerprint, while unrelated metadata drift and same-length content edits do not stale a snapshot" (`history_summarizer.rs:141-143`) | The fingerprint is deliberately blind to same-length content edits; something else must catch those. | `chunk_fingerprint` at `history_summarizer.rs:151-160`. The compensating control is `selected_range_identities` (`memory-store:9413-9425`). The claim is accurate and names its own hole. |
| C10 | "A history_segment must end on an anchorable block so publication cannot mint an impossible coverage boundary." (`history_summarizer_validate.rs:36-37`) | No published boundary names a block that does not exist. | `history_summarizer_validate.rs:958-963`. Verified; test `history_segment_end_must_be_anchorable` (`:1665`). |
| C11 | "make the matching firing immediately idle so the caller never leaves a durable Publishing wedge behind" (`history_summarizer.rs:550-551`) | A losing race never blocks all future firings for the session. | Four differentiated abandon arms at `history_summarizer.rs:533-593`. Verified. |
| C12 | "Authorizing fallback starts a second potentially billable run, so this needs positive proof, not the absence of one known-bad code." (`history_summarizer.rs:1228-1229`) | A second model attempt runs only after the first is proven stopped. | Enforced on the output branch (`history_summarizer.rs:1401`). **Not enforced on the start-failure branch** (`:1290-1329`). Lens A `uncertain-producer-start-authorizes-a-second-billable-run`. |
| C13 | "The id must be unique per (lineage, firing), not per (project, firing)" (`history_summarizer.rs:1004-1005`) | Concurrent subagent lineages cannot cross their producer runs and lose a commit. | `history_summarizer.rs:1013-1035`, full 64-bit FNV-1a rendering justified at `:1034-1035`. Tests `full_lineage_hash_separates_keys_that_collided_at_32_bits` (`:2146`), `producer_session_ids_are_lineage_scoped_under_one_project` (`:2163`). |
| C14 | "an unknown `session.send` outcome is retried at most once with the exact frozen bytes under the same route identity (ModelExecution's session fingerprint makes that recovery idempotent)" (`ARCHITECTURE.md:37`) | An ambiguous send cannot become two billable runs. | `send_frozen_once` (`history_summarizer_producer.rs:929`) plus `replay_frozen_once` (`:949`), fenced at `:999-1000` and reported at `:492`. Holds **within one attempt**. Does not cover the cross-model chain: see lead L3. |
| C15 | "`run.status` handling is closed over the exact wire vocabulary (undocumented statuses fail loud instead of guessing)" (`ARCHITECTURE.md:37`) | An unknown provider status never gets guessed into a publish or an abandon. | Test `run_state_mapping_is_closed_over_known_states` (`history_summarizer_producer.rs:2175`) exists. The production mapping was not read end to end in this pass; recorded as unverified. |
| C16 | "Skips unanchored fact, observation, and primer promotion on the discarded tail to prevent double-storing when the range is re-processed." (`ARCHITECTURE.md:117`) | A re-processed range does not promote its facts twice. | `keep_side_channel` (`history_summarizer_validate.rs:1086-1098`); doc comments at `:145-147` and `:177-179` state the same rule. Tests `:1774`, `:1815`, `:1849`. Verified. |
| C17 | "Guarded by progress (`k≥2`) and emergency-disabled." (`ARCHITECTURE.md:117`) | The weak-lookahead terminal boundary is withheld for re-derivation. | `history_summarizer_validate.rs:539` guards on `history_segments.len() >= 2`, **not** on a lookahead `k`. The lookahead test is separate, at `:554`. The doc's `k≥2` names the wrong quantity. Lens B `hv-single-history_segment-skips-lookahead-discard`. |
| C18 | "The history_summarizer's in-flight snapshot is validated by `computeRawRangeFingerprint`, which hashes **raw content only** (ids, part types, content lengths) — never tag/drop state — so a concurrent drop can't invalidate it." (`ARCHITECTURE.md:88`) | A concurrent tag or drop cannot abort a firing. | The Rust analogue is `chunk_fingerprint` (`history_summarizer.rs:151-160`), which matches the description. But the Rust publish also fences on `selected_range_identities` (`memory-store:9413-9425`), a second freshness input the doc does not mention. See lead L4. |
| C19 | "repeated failures show a `Eidnara — history comparting needs attention` notice" (`README.md:98`); "A warning only appears in `/eidnara-status` after multiple consecutive failures." (`troubleshooting.md:93`) | A persistently failing history_summarizer is visible to the user. | **NOT FOUND for the Rust leg.** `buildHistorySummarizerFailureNotice` (`history_segment-runner-validation.ts:210`) is called only from `history_segment-runner-incremental.ts` (`:208`, `:396`, `:493`, `:546`, `:912`), the TypeScript runner. Nothing in `packages/plugin/src` reads `publish_health_degraded` or `consecutive_publish_failures`. See lead L5. |
| C20 | "Falls back to the draft if the editor call or its validation fails, so it can never regress behavior." (`CONFIGURATION.md:454`, `history_summarizer.two_pass`) | The documented second editor pass is safe by construction. | **NOT FOUND.** No `two_pass`, `history_summarizer_editor`, or `history_summarizer-editor` symbol exists anywhere in `crates/daemon/src`. The `two_pass` identifiers in `selection.rs:872-1229` are an unrelated tool-drop batch concept. See lead L6. |
| C21 | "`history_summarizer_timeout_ms` \| `number` \| `300000` \| Timeout per history_summarizer call (ms)." (`CONFIGURATION.md:170`) | A user-configurable per-call timeout bounds a history_summarizer call. | **NOT FOUND.** `rg history_summarizer_timeout_ms crates/daemon/src` returns nothing. The Rust leg bounds a call by `completion_wait_budget` (660 s) and `wrapup_round_wait_budget` (600 s, `history_summarizer.rs:966`), neither user-configurable. See lead L6. |
| C22 | "There is **no built-in fallback chain** — Eidnara never silently tries models you haven't configured" (`CONFIGURATION.md:354`); "**HistorySummarizer only:** your active session model, as a last resort" (`:359`) | The set of models the history_summarizer may bill against is exactly what the user configured, plus at most the session model. | `model_chain` is built only from user keys at `config.rs:396-420` and deduplicated at `:571`; empty yields `NoModels` (`history_summarizer.rs:1250`, `lib.rs:5021`). The two sentences are in tension with each other; whether the Rust leg appends the session model was not established. Unresolved. |
| C23 | "User observations are stored only when the privacy collection gate is enabled." (`history_summarizer.rs:421`) | Nothing derived from the user's own words is stored without the gate. | `collect_user_memory_candidates` threaded into the publish request; default off at `config.rs:128` per lens B. The store-side honouring of the flag was not read in this pass. Unresolved. |
| C24 | "The substance floor must not block firing." (`history_summarizer_chunk.rs:470-471`, `fold_is_only_reclaim`) | On verbatim-tail profiles, where folding is the only reclaim, a small chunk still fires. | Tests `fold_only_fires_below_substance_floor_without_emergency` (`history_summarizer_chunk.rs:1784`) and `below_budget_refuses_normally_but_fires_in_emergency` (`:1793`). Verified as a claim with a test. |
| C25 | "A history_summarizer publish does NOT bust the cache — between busts every pass is `cache_hit`." (`ARCHITECTURE.md:79`) | Folding never costs the user a cache-priced re-read. | Consistent with C3: the transaction writes no render state. The `daemon` side of the deferral was not traced in this pass. Unresolved, needs the transform lens. |
...
bypass and nothing closes it.** The comment at `history_summarizer.rs:59-60` reads in
...
prose is the doc comment on `to_stored_history_segment` (`:35-37`), which frames the
...
the comment at `history_summarizer.rs:60-62`; at `HEAD` the two comment lines are `:59-60`
...
`if !matches!(meta.history_summarizer.state, HistorySummarizerPhase::Publishing | HistorySummarizerPhase::AwaitingProducer)`,
...
not about validation having run. `history_summarizer.rs:1-6` names `validating` as a phase
the pipeline passes through, and `history_summarizer_validate.rs:6-9` says decisions are
...
"is retried at most once with the exact frozen bytes under the same route
identity (ModelExecution's session fingerprint makes that recovery idempotent)". Both
halves are implemented: `send_frozen_once` (`history_summarizer_producer.rs:929`), one
...
the producer session id embeds `firing_seq` by design (`history_summarizer.rs:1001-1002`),
so a fallback model attempt runs under a *different* session id and ModelExecution's
...
exactly the branch lens A found unprotected (`history_summarizer.rs:1290-1329`). The doc
...
fingerprint matches that description (`history_summarizer.rs:141-143`, `:151-160`), but
...
of `buildHistorySummarizerFailureNotice` are in `history_segment-runner-incremental.ts`, the
...
`history_summarizer.rs:358`. So on the Rust leg a history_summarizer that rejects every output
...
**L6. Two documented `history_summarizer` configuration keys have no Rust
implementation.** `history_summarizer.two_pass` (`CONFIGURATION.md:454`,
`packages/docs/.../configuration.md:100`) and `history_summarizer_timeout_ms`
...
**L7. `HistorySummarizerValidationError`'s justification names a consumer that does not
exist.** `history_summarizer_validate.rs:239-241`: "Validation failures are plain,
...
1. **`ValidatedChunk` is not a proof-carrying token.** Verified at `HEAD`:
   `history_summarizer_validate.rs:226` is
   ...
   module is `pub mod history_summarizer_validate` (`lib.rs:23`), the consumer is
   `pub mod history_summarizer` (`lib.rs:19`), and `publish_validated_chunk` is `pub fn`
   at `history_summarizer.rs:444`. So `Default::default()`, `serde_json::from_str`, or a
...
2. **The three consumer deadline budgets are hand-mirrored across languages.**
   `history_summarizer.rs:951-961` instructs consumers to use `MAX_WRAPUP_REQUEST_BUDGET`
   ...
   deadline for the module's exported history_summarizer::MAX_WRAPUP_REQUEST_BUDGET",
   against the Rust `Duration::from_secs(3_800)` at `history_summarizer.rs:962`. Nothing
   mechanically compares them. Worse, `MAX_EMERGENCY_REQUEST_BUDGET`
   (`history_summarizer.rs:983`, 1500 s) has **no TypeScript mirror at all** that I could
...
3. **`delete_session`'s default is a compatibility shim that production must
   override.** `history_summarizer.rs:789-791`: "Delete the provider session on every
...
4. **The history_summarizer system prompt must not be concatenated into `prompt`.**
   `history_summarizer.rs:894-895`: "Sent via the producer's `system` field, never
...
7. **The tier parser "must never swallow a following tier's opener"**
   (`history_summarizer_validate.rs:1222`, guard at `:1128`). A regex contract stated as
...
| --- | --- | --- | --- |
| `history_summarizer.rs` | **51** (13 sync, 38 tokio) | `:1862-4646` | `:1863` `stored_history_segment_legacy_flag_tracks_p1_presence` / `:4647` `restart_mid_publishing_with_committed_tx_detects_idle` |
| `history_summarizer_validate.rs` | **19** (all sync) | `:1383-1848` | `:1384` `validate_golden_matches_typescript_oracle` / `:1849` `force_keep_last_preserves_final_history_segment_and_side_channels` |
| `history_summarizer_chunk.rs` | **19** (all sync) | `:1299-2044` | `:1300` `chunk_uses_flat_block_ids_and_covers_system_ordinals_without_their_text` / `:2045` `fixture_builder_drives_boundary_chunk_assembly` |
| `history_summarizer_producer.rs` | **18** (2 sync, 16 tokio) | `:1711-2288` | `:1712` `start_opens_expected_identity_and_sends_once` / `:2289` `a_successful_drain_returns_text_and_the_length_cap` |
| `lib.rs`, history_summarizer-related | **34** matching `history_summarizer`, `wrapup`, or `reattach` | within the test module opening at `:15993` | `:16445` `history_summarizer_trigger_token_reuse_matches_retokenized_production_shape` / `:30037` `status_diagnostics_surface_pending_history_summarizer_side_channel_failure` |

**121 in-crate tests** across the four scope files plus the `lib.rs` history_summarizer
set. Clusters:

- **`history_summarizer_validate.rs`, 19 tests. The task's report is confirmed with one
...
- **`history_summarizer_chunk.rs`, 19 tests.** Chunk construction and ordinal coverage
...
- **`history_summarizer_producer.rs`, 18 tests.** The ambiguous-send replay fence, seven
...
- **`lib.rs`, 34 tests.** Dominated by `session.wrapup`, roughly twenty tests
  ...
  shutdown of spawned history_summarizer work (`:16932`, `:17007`, `:17017`).

### Integration and CI status (with workflow line refs)

**Integration tests exercising the history_summarizer path: none found.** A
case-insensitive search for `history_summarizer` across all seven files in
...
(`boundary_counter_durability` 1, `model_execution_roundtrip` 2, `direct_host` 6,
...
on the history_summarizer is therefore an in-crate unit test compiled into the `--lib`
target.
...
`history_summarizer-eval.yml`, `retrieval-benchmark.yml`, `claude-code-review.yml`, and
...
(101 tests total): `:16625` `history_summarizer_publish_failure_counter_accumulates_and_success_state_resets`,
`:16688` `matching_history_summarizer_abandon_fences_predicate_and_update_for_both_backoffs`,
`:16781` `publish_history_summarizer_chunk_rejects_overlapping_history_segment_as_typed_error`,
`:16984` `publish_history_summarizer_chunk_persists_transcript_inside_cas`,
`:17017` `publish_history_summarizer_chunk_cas_conflict_leaves_no_transcript_row`,
`:18221` `publish_history_summarizer_chunk_fails_loud_from_non_publish_state`,
`:18336` `publish_history_summarizer_chunk_rejects_recut_epoch_mismatch_as_conflict`,
...
This is the only history_summarizer coverage that executes on every pull request, so what
it covers matters more than its size.

The gate is one CI job, `history_summarizer-eval-contracts`, at `HEAD` `ci.yml:407-432`
(working tree `:415-440`). Its comment block at `HEAD` `:394-406` says the job
exists because "Nothing invoked test:history_summarizer-eval-unit" before it, and that it
"declares no `needs` so an unrelated failure cannot skip" it. Three steps:

| Step | `HEAD` line | Command | What it checks |
| --- | --- | --- | --- |
| HistorySummarizer eval unit contracts | `ci.yml:426` | `bun run test:history_summarizer-eval-unit` | Every `src/history_summarizer-eval/**/*.test.ts` except `runner.test.ts`, which is excluded as harness-booting (`run-test-selection.ts:59-70`, `:84`). Six files: `contract.test.ts`, `dev-corpus.test.ts`, `mutations.test.ts`, `payload.test.ts`, `promote.test.ts`, `scorer.test.ts`. |
| Freeze lint over the dev corpus | `ci.yml:429` | `run-history_summarizer-eval.ts --lint` | Scenario-schema and freeze conformance of the `history_summarizer-eval/dev` corpus. Corpus hygiene, not module behaviour. |
| Invalid-state mutation battery | `ci.yml:432` | `run-history_summarizer-eval.ts --mutations` | For every scenario, crafted-wrong history_summarizer outputs must go red **at the expected stage** per mutation class. Seven classes with pinned outcomes (`mutations.ts:33-56`): `speculation-promoted` and `rejected-proposal-active` must score `FAIL:false-authoritative`; `wrong-category` and `dropped-gold-fact` must score `FAIL:recall`; `near-miss-perturbation` either; `structural-overlap` must land at `validation-rejected`; `probe-wrong-answer` must fail probe comparison. The battery fails on a stage mismatch, not only on a PASS (`mutations.ts:5-11`). |

**What these gates do not touch: the Rust history_summarizer.** Verified by reading the
scorer's imports. `scorer.ts:20-26` imports `validateHistorySummarizerOutput`,
`validateStoredHistorySegments`, `shouldDiscardLastHistorySummarizerHistorySegment`, and
`HISTORY_SUMMARIZER_BOUNDARY_HEALING_SLACK` from
`packages/plugin/src/hooks/eidnara/history_segment-runner-validation.ts`, and
`:27-28` imports `appendHistorySegments` and `promoteSessionFactsDurable` from the
...
`history_summarizer_validate.rs`.

The lane's own code says so explicitly. `run-test-selection.ts:73-76`: the
harness-booting tests are "TS-mode only: `daemon`'s Rust history_summarizer producer
does not promote claims, so these must never join a rust or pi selection." And
`history_summarizer-eval/README.md` names the untapped opportunity: the frozen corpus's
...
`validate_golden_matches_typescript_oracle` (`history_summarizer_validate.rs:1384`),
...
`history_summarizer-eval.yml` is the live lane; `README.md` records it as
...
`#[cfg(test)]`: `history_summarizer.rs` (`:1821`), `history_summarizer_producer.rs` (`:1486`),
`history_summarizer_chunk.rs` (`:1171`), `history_summarizer_validate.rs` (`:1305`). Zero
...
- `history_summarizer.rs:1492` `RestartAction::ReattachProducer { .. } => unreachable!()`
  — the only `unreachable!`, `panic!`, or `todo!` in production code across the
  four files.
- `history_summarizer_validate.rs:929`
  `.expect("non-empty omitted present ordinals checked above")` — the only
  `.expect` in the four files, inside the gap-healing path.

**Infallible-by-construction unwraps.** 27 in `history_summarizer_validate.rs` and 8 in
`history_summarizer_chunk.rs`, and every one I sampled is a static regex compilation
inside a `OnceLock::get_or_init` (`history_summarizer_validate.rs:1159-1195`,
`history_summarizer_chunk.rs:1133-1168`). None depend on runtime data. Zero unwraps in
`history_summarizer.rs` and `history_summarizer_producer.rs` production paths.
...
- `lib.rs:7065-7069` — the wrapup drain reached the keep watermark unless a
  ...
  states a design choice: "history_segments never shrink, so a failure-free exit has
...
1. **The three quietest things in this scope, and the top one is the whole
   subsystem.** The history_summarizer's 121 in-crate tests, the 7 `memory-store` publish
   ...
   integration test in `crates/daemon/tests/` mentions the history_summarizer. The only
...
2. **Nothing executing anywhere compares the Rust validator to the TypeScript
   ...
   the two golden-fixture drivers in `history_summarizer_chunk.rs` (`:1962`, `:2045`). All
...
3. **The store-side publish transaction's atomicity claim is untested and
   currently untestable.** Seven tests reach `publish_history_summarizer_chunk`, but per
...
4. **Ten of the validation gate's 22 rejecting checks have no test at any
   ...
   `<output>` tags (check 2); history_segment endpoint maps to a chunk line
...
5. **`history_summarizer.rs` has 51 tests and zero production assertions.** The most
...
6. **The user-facing failure signal has no producer and its counter has no
   ...
   (`history_summarizer.rs:358`), and no TypeScript reader of `publish_health_degraded`
...
7. **The three consumer deadline budgets are cross-language constants with one
   ...
   (`history_summarizer.rs:974-982`: a raw array forwarded "at the exact pressure where
...
8. **Two documented configuration keys with no implementation and no test that
   would notice.** `history_summarizer.two_pass` and `history_summarizer_timeout_ms` (L6). A
   configuration-reference conformance check comparing documented `history_summarizer.*`
   keys against `config.rs` parsing would catch both; none exists.
9. **`history_summarizer_chunk.rs`'s two golden fixtures are the only guard on chunk
   assembly, and assembly is what pins the raw messages.** `raw_chunk_messages`
   is built at `history_summarizer_chunk.rs:717-727` and is the sole durable copy of the
...
10. **The `unreachable!()` at `history_summarizer.rs:1492` and the `.expect()` at
    `history_summarizer_validate.rs:929` are the subsystem's only two panic sites and
    ...
    attack.
11. **`docs/AUDIT-KNOWN-ISSUES.md` contains no history_summarizer publish, validation, or
    producer entry.** The file mentions history_summarizer only in passing. None of the
    contract-versus-code gaps in this lens or its two siblings is tracked there.

## Open questions

- Should the `history_summarizer-eval` frozen corpus be reused as a TS-versus-Rust
  validator differential, as its own README proposes? That single change would
  convert the repo's only executing history_summarizer coverage into coverage of the Rust
  ...
  carve-out at `history_summarizer_validate.rs:1391-1399`) become failures or documented
  exceptions. (needs human input)
- Are `history_summarizer.two_pass` and `history_summarizer_timeout_ms` TypeScript-leg-only
...
- Why does the publish transaction admit `AwaitingProducer` (L2)? No comment
  ...
  (needs human input)
- Does `CONFIGURATION.md:359` ("HistorySummarizer only: your active session model, as a
...
- Does the store honour `collect_user_memory_candidates` (C23)? The flag is
  documented at `history_summarizer.rs:421` as the privacy gate for storing text derived
  ...
  unresolved.
- Is `history_summarizer_producer.rs`'s `run.status` mapping genuinely closed over the
