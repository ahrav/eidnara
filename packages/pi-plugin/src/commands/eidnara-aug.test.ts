...
import { registerCtxAugCommand } from "./eidnara-aug";
...
describe("registerCtxAugCommand", () => {
    ...
    it("sends the prompt with context_researcher augmentation when context_researcher returns context", async () => {
        ...
        try {
            ...
            const command = fake.commands.get("eidnara-aug") as {
    ...
    it("surfaces not configured when context_researcher config is absent", async () => {
        const fake = createFakePi();
        ...
        expect(notify).toHaveBeenCalledWith(
            expect.stringContaining("ContextResearcher is not configured"),
            "warning",
        );
        expect(fake.sentMessages).toEqual([]);
    });

    it("sends the original prompt unchanged for an empty context_researcher result", async () => {
        const runner = installRunner({
            ok: true,
    ...
    it("does not send the prompt when the context_researcher run was aborted", async () => {
        const runner = installRunner({
            ok: false,
            reason: "abort",
    ...
    it("sends the original prompt when the context_researcher fails for another reason", async () => {
        const runner = installRunner({
            ok: false,
            reason: "timeout",
    ...
    it("notifies the user when the session directory has no project identity", async () => {
        ...
        const home = mkdtempSync(join(tmpdir(), "eidnara-pi-eidnara-aug-home-"));
        try {
