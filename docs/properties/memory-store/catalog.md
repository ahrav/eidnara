...
Scope: three crates. `crates/memory-store` (21,987 lines across `src/lib.rs`,
`src/claim_mirror.rs`, `src/sqlite_runtime.rs`, and three files under `tests/`),
`crates/context-core` (1,518), and `crates/tokenizer` (85). Production code in
`crates/memory-store/src/lib.rs` ends at line 13,930; line 13,931 is blank and
...
| 19-41 | Imports, including `storage::{open_sqlite, Migration, SqliteStore, StoreError}` at 20 |
...
| 3351-3652 | Error types and conversions, `From<StoreError> for MemoryStoreError` 3554-3558 |
...
`.github/workflows/shm-hardening-optin.yml` (source-catalog path, not present at HEAD), which references no scope crate, so
...
- Source: the host repository at `eb6da6109`, catalog `part-3-store-core`. The records,
...
- The claim mirror is gone from `crates/memory-store` at `dbc4d15b`'s
  ...
  control row stay for the next ticket; with the mirror gone, the only writer
  of the control row's `accepting` state is the authority finish transition,
  which writes it only for a 32-hex context-store id, so a rebuild grant
  (`begin_claim_store_rebuild`, no production caller) now freezes the ledger
  until the next authority cycle. Every
...
(`storage:155-161`) hands out a raw `&Connection` under the store mutex
with no transaction, on 73 production call sites. `SqliteStore::with_conn_fenced`
(`storage:185-233`) is the write path: `TransactionBehavior::Immediate`
...
`storage:265-327` `open_sqlite` is the only place any PRAGMA reaches the
...
is resolved by path from root `Cargo.toml:16`. `MemoryStore::open`
...
`user_version`, and does not read a `format_marker` row.

**`PRAGMA synchronous` is never set anywhere.** Not in `open_sqlite`, not in
`MemoryStore::open`, not anywhere in `crates/memory-store/src`. A content search for
...
`docs/migration-version-lanes.md:50` (source-catalog path, not present at HEAD) promises a "declared synchronous mode" and
...
implement exactly what `docs/migration-version-lanes.md:41-51` (source-catalog path, not present at HEAD) promises, and
...
`storage:230-231` and the caller observing `Ok`. A kill inside `commit()`
...
by `MemoryStore::open`.
Guarantee: The connection `MemoryStore::open` returns runs at a synchronous level the
code chose, not one inherited from the build's compile-time default.
Check: `always` - on a connection obtained from `MemoryStore::open`, read
...
while `docs/migration-version-lanes.md:50` (source-catalog path, not present at HEAD) calls this a "declared synchronous
mode".
Required faults and enabling state: none. Open a store and read the pragma.
Confidence: high - [evidence](evidence/synchronous-level-is-explicitly-declared-not-inherited.md).
Verified by exhaustive content search for `synchronous` across `crates/` and
`storage/src/lib.rs`: the only occurrences are the verifier reads at
`sqlite_runtime.rs:133-138`, its doc comment at 112, and two test lines.
Existing check: `crates/memory-store/tests/sqlite_runtime.rs:192-200` (source-catalog path, not present at HEAD) proves the
verifier rejects `synchronous=OFF`, on a hand-built connection. It never inspects
a `MemoryStore` connection. Status `unaudited`.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/memory-store/tests/sqlite_runtime.rs:139-169` (source-catalog path, not present at HEAD) probes the live
...
Confidence: high - [evidence](evidence/bundled-engine-satisfies-the-declared-wal-reset-precondition.md).
...
- `Cargo.toml:30` says raising `rusqlite` requires bumping `storage` in the
...
rather than used.
Check: `reachable` - the gate call site must execute on the `MemoryStore::open` path.
...
`evaluate_sqlite_runtime_gate` and call `MemoryStore::open`.
Confidence: high - [evidence](evidence/wal-reset-gate-runs-on-the-production-open-path.md). Verified
that `MemoryStore::open` (`lib.rs:4816-4905`) contains no call, and that a content
...
Status `unaudited`.
Impact: `docs/migration-version-lanes.md:41-44` (source-catalog path, not present at HEAD) says "Bun and Node writers probe an
...
- Would wiring the gate in today make `MemoryStore::open` fail outright, given the
...
`MemoryStore::open` produces, so a store whose PRAGMAs did not take effect is refused.
Check: `reachable` - the verifier call site must execute on the `MemoryStore::open`
...
- `storage:287` uses `pragma_update` for `journal_mode`, which discards the
...
Reachability: default-production
Status: active
Exercised: partial - `storage:691-712` `fenced_write_rolls_back_on_error`
forces a closure error after a write and asserts rollback. That is the dependency's
own test, on the dependency's shape, not on any `memory-store` multi-statement writer.
...
Fault/timing angle: the window is the 5 seconds set at `storage:289`.
Because `with_conn_fenced` uses `BEGIN IMMEDIATE` (`storage:191`), the
...
Confidence: medium - [evidence](evidence/busy-timeout-expiry-aborts-cleanly-without-partial-effect.md).
...
Impact: because `StoreError::Backend(e.to_string())` (`storage:229`)
...
Impact: exhaustion returns `MemoryStoreError::Serde` with a prose message
...
`Mutex<Connection>` at `storage:159, 189`; across processes it is prevented
by the file lease at `storage:279-282`. So the fault requires a writer that
bypasses the lease, which is the same enabling state `lib.rs:16697` already
constructs.
Confidence: high - [evidence](evidence/write-predicates-are-re-evaluated-inside-the-write-transaction.md).
...
Reachability: default-production
Status: invalidated
Invalidated: the port to this repository removed the schema-migration ladder, the `schema_version` table, and `sqlite_runtime.rs`; `memory.sqlite` opens through `storage::open_sqlite` against one baseline (`crates/memory-store/baseline.sql`), so no migration runs and the subject is unreachable by any configuration.
Exercised: not yet - no test kills a process mid-migration.
`storage:528` `open_runs_migrations_and_seeds_once` and `:580`
`later_migration_applies_on_top_of_earlier` cover the happy path.
Guarantee: For any migration, either its statements and its
`schema_version` row are both durable, or neither is.
...
Fault/timing angle: the window is `storage:366-383`, between
...
`CREATE TABLE IF NOT EXISTS schema_version` at 341-349 and the
...
Confidence: high - [evidence](evidence/migration-and-its-version-record-commit-together.md). Verified
...
`storage:528`. Both happy-path. Status `unaudited`.
...
resumability. `recorded_cache_version` mapping a recorded `0` to `None` at
...
inventory" promise at `docs/migration-version-lanes.md:11-17` (source-catalog path, not present at HEAD) sits under "Format
...
`schema_version`. Nothing prevents this, because the version lives in an
...
then a reopen through `MemoryStore::open` and a schema-inventory comparison.
Confidence: medium - [evidence](evidence/recorded-schema-version-cannot-disagree-with-the-actual-schema.md).
Verified the version storage and read path at `storage:341-357` and
...
already committed.
Confidence: high - [evidence](evidence/post-migration-open-repair-is-resumable-and-effect-idempotent.md).
...
- The completion flag is a sentinel row in `cache_state` with
...
anything in `memory`, and the only two writers, `replace_claim_mirror_snapshot`
...
transaction.
Existing check: `crates/memory-store/tests/claim_mirror.rs:177-250`
...
- Does the facade retry `claim.mirror.apply` on a lost response, and with
  byte-identical bytes? `daemon/src/lib.rs:10326` is the call site; the retry
...
Confidence: high - [evidence](evidence/mirror-receipt-conflict-rejects-divergent-replay.md). Verified
...
Confidence: high - [evidence](evidence/mirror-project-effect-chain-detects-omission.md). Traced both
...
Confidence: high - [evidence](evidence/mirror-generation-advances-exactly-one-per-touched-project.md).
Read the `increment = i64::from(touched.contains(project_id))` construction and both
mismatch arms.
Existing check: `crates/memory-store/tests/claim_mirror.rs:252-342` and `:528-591`,
status `unaudited`.
...
Confidence: medium - [evidence](evidence/mirror-read-fence-relies-on-generation-advance.md). The coupling
...
Existing check: `crates/memory-store/tests/claim_mirror.rs:377-458`, `:461-479`,
`:482-517`; `crates/memory-store/tests/claim_intent_ledger.rs:288-335`. All status
...
- Is `begin_claim_store_rebuild` intended to be reachable from the host, and if so
  through which facade method? Nothing in `daemon` exposes it. (needs human input)
...
Confidence: high - [evidence](evidence/mirror-clear-without-a-grant-is-never-entered.md).
...
Confidence: high - [evidence](evidence/mirror-accepting-gate-is-skipped-when-control-is-absent.md).
...
`claim_mirror_state.updated_at_ms` is written at `claim_mirror.rs:827` and
...
`Ok`, `claim_intent_controls` afterwards holds the requested `transition_state`.
...
Confidence: high - [evidence](evidence/intent-control-transition-write-is-silently-dropped.md).
Verified `is_lower_hex` requires exactly 32 chars of `[0-9a-f]`
(`context-core/src/claim_operation.rs:173-178`), verified all four call sites pass
...
Confidence: high - [evidence](evidence/intent-identity-is-producer-and-operation-key.md). Read the
primary key, both refusal sites, and `require_claim_intent_binding`'s field list.
Existing check: `crates/memory-store/tests/claim_intent_ledger.rs:133-166`, status
`unaudited`.
...
- Is `producer` authenticated anywhere above this layer? Within `memory-store` it is an
...
both fail.
Confidence: high - [evidence](evidence/intent-terminal-state-is-entered-at-most-once.md). Enumerated all
...
Existing check: `crates/memory-store/tests/claim_intent_ledger.rs:85-131`, `:169-228`,
...
- `(Acknowledged, TerminalRejected)` returns `replayed: true` and writes nothing
  ...
  `context-core/src/claim_operation.rs:368-369` calls `acknowledged` "transport settlement,
...
commits in one `with_conn_fenced` transaction in `memory` (`lib.rs:11037`); the
...
crates.
Confidence: medium - [evidence](evidence/intent-staged-replay-produces-one-context-effect.md). The two
...
Existing check: `crates/memory-store/tests/claim_intent_ledger.rs:337-401` (source-catalog line, not present at HEAD) covers the drain
...
four degrees of freedom are not reachable from `daemon` today. All four records were
...
- Is a subnormal `history_budget` reachable from configuration, or is the budget always
  ...
  `history_budget_tokens` back to its config surface, which is `daemon` territory.
...
Confidence: high - [evidence](evidence/core-decay-tier-ladder-monotone-and-archive-agreement.md). I
...
`crates/daemon/src/decay_render.rs:295`, hardcodes `0.0`; only a direct library call
can supply NaN today.
Existing check: `crates/context-core/src/decay.rs:194` `finite_demotion_at_max_importance`,
...
`crates/daemon/src/decay_render.rs:331-347` becomes the only backstop. If anchors
...
including `crates/memory-store/src/lib.rs:3943` which treats `Applied | Noop` as one class.
Open questions:
...
genuinely undefined.
Confidence: high - [evidence](evidence/core-applicability-heads-order-independence.md). Verified by reading
...
- Can a duplicate stream key occur in a real head list? If it can, the digest is
  ...
  input order. Resolving it needs the head-collection query in `memory-store`, which is a
  sibling lens's file. (unresolved, needs the `memory-store` head-collection query)
...
point in `context-core` that must not execute; there is no transition function to place a
marker in.
...
High confidence that `context-core` does not model the relation: I read every line of
...
overall because whether the *system* enforces legality depends on `memory-store`, which this
lens did not read; the sibling lens confirmed it does, in the single `match` at
`lib.rs:11225-11255`.
Existing check: none in `context-core`. `crates/memory-store/tests/claim_intent_ledger.rs:85-131`,
...
- Is transition legality enforced in `memory-store` SQL, and if so is the guard a `CHECK`, a
...
Confidence: high - [evidence](evidence/core-pass-classifier-destructive-clear-guard.md). I read all eight
...
`test-only`: a repo-wide `rg` for `ClassifierInput|PassPlan|context_core::classify` over
`*.rs` returns hits only inside `crates/context-core/src/lib.rs`; the `classify` symbols found
in `crates/daemon/src/classify.rs` and
`crates/host-runtime/tests/support/perf_measurement.rs:426` are unrelated functions.
Existing check: `crates/context-core/src/lib.rs:176-337` (source-catalog line, not present at HEAD), 14 tests. Status `unaudited`.
...
- Is `classify` dead code awaiting adoption, or has `daemon` diverged with a second
  ...
  no reference at all. (unresolved, needs an `daemon` routing comparison)

## Group G: tokenizer determinism

Two records on `crates/tokenizer` (85 lines), the smallest crate in the part and the
one with the strictest contract: `tokenizer/src/lib.rs:13-19` states that a resume
must produce byte-identical m0, and `crates/daemon/src/tail_hygiene.rs:85` plus the
...
`tokenizer/src/lib.rs:47` builds the encoder exactly once per process, so any
...
`tokenizer/src/lib.rs:61` is order-insensitive and the encoder build is reproducible.
...
`estimate_tokens` for budget fitting, and `tokenizer/src/lib.rs:13-19` states that a
...
(`crates/tokenizer/gen/gen-token-golden.ts:17-19`) and stamps no version anywhere, so the
...
`packages/plugin/package.json:65` (source-catalog path, not present at HEAD) and `packages/pi-plugin/package.json:45` (source-catalog path, not present at HEAD), so a consumer
...
Existing check: `crates/tokenizer/tests/token_golden.rs:26`, `:47`, `:59`, `:64`. All four
compare against the fixture; none validates the fixture's own provenance. Status `unaudited`.
Impact: the golden's stated purpose (`tokenizer/src/lib.rs:16-19`) is faithfulness to the
...
- **One commit boundary, three promises.**
  ...
  `storage:229`, an `Err` returns before `tx.commit()` at `:230-231`, and the
...
- **Three verifiers, zero production callers.**
  ...
  implements exactly what `docs/migration-version-lanes.md:41-51` (source-catalog path, not present at HEAD) promises; both are
...
- **The control row nobody writes.**
  ...
  `claim_intent_controls` is never populated in production, and the three readers of
...
- **Two windows around one durable effect in another database.**
  ...
  kind, and the `context-core` record records that neither the type system nor the wire
  request can express the transition fence at all, since `ClaimIntentAckRequest` carries
  no expected-current-state. Hypothesis: the `context-core` legality record is *dominated*
  ...
  twelve `(kind, state)` pairs against it subsumes the `context-core` claim; the two are kept
...
- **The classifier that nothing calls.**
  ...
  copy of routing logic `daemon` already implements, because a silent divergence between
...
anchored to `crates/memory-store/src/lib.rs` and `crates/context-core/src/redaction.rs` at
...
`MemoryStoreError::Redaction(SecretDetected)` and commits nothing; a detection plus an
...
(`storage:144-152, 170-176`), so the mutex serializes each call but not a
read-then-write sequence.
Required faults and enabling state: a second thread sharing the same `MemoryStore` that
...
Check: `always` - after a call returning `MemoryStoreError::Redaction(SecretDetected)`,
the target table has no row for the write's key and `field_scans` has no row for
