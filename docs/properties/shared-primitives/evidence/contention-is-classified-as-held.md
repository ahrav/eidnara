...
- **Primary evidence:** `LeaseError` distinguishes `Held` from `Io`; exclusive `File::try_lock` and shared `File::try_lock_shared` map `TryLockError::WouldBlock` to `Held` and `TryLockError::Error` to `Io`. PostgreSQL maps a false `pg_try_advisory_lock` result to `Held` and query errors to `Backend` (`commons@89abb40 crates/`commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:347-355`).
