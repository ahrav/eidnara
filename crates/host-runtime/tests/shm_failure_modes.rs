...
const ROLE_ENV: &str = "EIDNARA_SHM_FAILURE_ROLE";
const PUBLICATION_ENV: &str = "EIDNARA_SHM_FAILURE_PUBLICATION";
...
fn shm_role_client() {
    ...
    runtime.block_on(async move {
        if role == "setup" {
            let info = host_runtime::read_connection_file(&publication).expect("discover host");
            ...
            let (_grant, _descriptors) =
                host_runtime::setup_socket::receive_grant(&mut stream, deadline)
                    .await
