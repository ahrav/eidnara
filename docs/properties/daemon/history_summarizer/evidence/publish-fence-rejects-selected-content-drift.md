...
verification" (`history_summarizer.rs:1-6`), which suggests the fingerprint is the guard.
...
`crates/daemon/src/history_summarizer_chunk.rs:698-715` pins one
`HistorySummarizerSelectedMessageIdentity` per non-synthetic message in the chunk range,
...
arming the failure cooldown (`history_summarizer.rs:533-547`), so a retry on a fresh
snapshot is admitted immediately.

### What the fence does not cover

The vector covers only mids inside the pinned range. `history_summarizer.rs:2369`
...
4. The publish commits. The history_segment covering 17 now summarizes the old text,
...
was captured at assembly time (`history_summarizer_chunk.rs:717-727`), so it also holds the
...
`DEFAULT_AWAIT_TIMEOUT` (600 s, `history_summarizer_producer.rs:30`) plus
...
- Sources examined: `crates/memory-store/src/lib.rs:12085-12113`;
  ...
  `crates/daemon/src/history_summarizer_validate.rs:525-556` (terminal boundary and
...
- Findings: an extension past `chunk.end_index` cannot change any pinned identity,
  ...
  the validated terminal boundary: `history_summarizer_validate.rs:529-534` refuses a
  terminal boundary that splits a completed tool arc, and the discard-last healing
  at `:539-556` pops the last history_segment when the lookahead is within
...
- Missing evidence: whether `BOUNDARY_HEALING_SLACK = 2` is sufficient when the
  ...
  `force_keep_last_history_segment: false` (`lib.rs:4767`) interacts badly with an
