...
`packages/shm-native/src/lib.rs:491`, `attach`:
...
`packages/shm-native/src/lib.rs:571`, `connect_setup`:
...
Both are surfaced in TypeScript. `packages/shm-native/index.ts:76-77` declares
...
The shipped consumer uses only one: `packages/plugin/src/shared/host-client/
...
under `packages/plugin/src/shared/host-client/`.
...
(`docs/host-wire-protocol.md:29` and its key-reader paragraph) that is not a
...
- Sources examined: `packages/shm-native/index.ts:76-77`, `:526-534`, and a
  search for `NativeChannel.attach`, `connectSetup` and `attach(` across
  `packages/plugin/src/shared/host-client/` and `packages/shm-native/`.
- Findings: `attach` is declared and wrapped but the only hit for the wrapper
  ...
  shipped path.
- Missing evidence: the search covered `packages/plugin/src/shared/host-client/`
  and `packages/shm-native/`. Other packages and any test files under
