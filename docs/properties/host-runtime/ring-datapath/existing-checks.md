...
`crates/host-runtime/src/ring_transport.rs`, `wire.rs`, `frame_channel.rs`, and
...
`crates/host-runtime/tests/*.rs`, and the ten `TestHost` users by testing each binary
...
The reason is structural rather than an omission. Every `-p host-runtime` invocation
in `ci.yml` carries a `--test <name>` filter, which selects one integration
binary and never builds the lib target. Re-verified at `HEAD`: the 13 `host-runtime`
...
functions and false of doctests. `cargo test -p host-runtime --doc` runs at
...
(`host_runtime::run_with_publish_hook`), against the production installer
...
| Architecture audit | `bun test scripts/check-shm-architecture.test.ts` and `bun run check:shm-architecture` | `:55`, `:58` | the deleted-transport grep gate |
...
`--release`), while `packages/shm-native/index.ts:189-191` rejects a
...
`host-runtime` binary uses is unresolved; it is an open question below, not a finding.
Status unaudited.
...
1. **The entire 14-test semantic contract suite is well-built and unexecuted.**
   ...
   step builds the `host-runtime` lib test target, so it runs only on a developer's
...
3. **Nothing publishes a 64 MiB frame through a real ring, and the geometry
   ...
   (`crates/shm-transport/src/arena.rs:4`) and `MIN_ARENA_BYTES` is defined as
...
- `scripts/check-shm-architecture.test.ts` was not read. It is the gate
  ...
  `crates/host-runtime/tests/`, matches names rather than semantics, and never reads
...
- Does the distributed `host-runtime` artifact ship debug or release? It decides
  ...
  a non-release addon (`packages/shm-native/index.ts:189-191`). Needs the
...
`host-runtime`'s codec. This is consistent with the "no property tooling" finding
