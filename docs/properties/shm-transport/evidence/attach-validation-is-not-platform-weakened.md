...
In `crates/shm-transport/src/backend/ring.rs`, `Mapping::attach` (`:249-274`)
...
again. It deleted the Darwin npm packages (`packages/host-darwin-*`, removed
in `55f47ac64`), removed the macOS CI jobs (`the source repository `ci.yml` workflow` at HEAD
...
- Sources examined: `ring.rs:206-211`, `:213-238`, `:240-259`, `:262-264`,
  ...
  `crates/shm-transport/src`, `packages/shm-native/src`, and
  `crates/host-runtime/src/ring_transport.rs`, which replaced `shm_provider.rs` in
  `ed487e11`; both `attach_ring` helpers as they stood at `9c1eb4d1`;
  `create_test_pair`; `docs/shm-transport.md:117` for the trusted-peer
  boundary; `docs/evidence/shm-traceability-v1.json:48`.
...
- Sources examined: `packages/` listing at HEAD `bdf72f46a`;
  `the source repository `ci.yml` workflow` at HEAD (every `runs-on` is `ubuntu-latest`);
  ...
  `6352f873f` re-added); `docs/shm-transport.md:5`, `:83`.
- Findings: the record's earlier "resolved by deletion" note on the Darwin
  ...
  Linux-x64 glibc support only (`docs/shm-transport.md:5`, `:83`).
