...
- `:12733-12744` selects the victim: oldest by `created_at_ms` then
  `history_segment_seq`, restricted to rows where `raw_messages_deflate IS NULL OR
...
history_segment forever. Because the eviction budget cannot see those bytes, the
...
- Every publish inserts one transcript row per appended history_segment
  (`:12698-12713`), so growth is proportional to history_segments, not to publishes.
- The raw payload size is proportional to the chunk's original message bytes,
  ...
  (`history_summarizer_chunk.rs:670`, `lib.rs:5095`) but unbounded across firings.
...
1. Retention: publish enough on one session that
   ...
   then assert every history_segment sequence that ever had a non-null
...
- Sources examined: `crates/memory-store/src/lib.rs:405-410`, `:12671-12763`;
  ...
  the suffix revert at `:9096-9111`; `crates/daemon/src/history_summarizer.rs:425-426`.
...
- Missing evidence: no design note, plan, or spec covers chunk-transcript
  retention. The scope map established there is no history_summarizer specification outside
  `history_summarizer*.rs` (`scope-map-and-risk-ranking.md:695-700`), and I confirmed
