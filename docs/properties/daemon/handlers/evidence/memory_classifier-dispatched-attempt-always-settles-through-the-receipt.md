...
The N1 specification (issue 306) requires that a MemoryClassifier retry after an unknown
...
outcome. The receipt ledger (`crates/memory-store/src/memory_classifier_ledger.rs`) made
the route's writes checkable; this record states the invariant the route must
keep over that ledger, and replaces
`h4c-memory_classifier-failure-path-ledger-write-is-unchecked`, whose subject (an ignored
...
`handle_memory_classifier_run_task` without a feature or configuration gate. This is
default-production reachability, subject to the route's authority checks.

- `handle_memory_classifier_run_task` validates the request shape (`ClassifyRequest::parse`:
  kernel object ids, a model chain, and a timeout; a
  host-rendered prompt or claim-lane items are refused) and hands
  `MemoryClassifierRuntime::run_memory_classifier_task` the parsed task plus the route facts
  (`MemoryClassifierRoute`, including the kernel project); that shared protocol passes
  the memories authority
  `MODULE` gate, takes the in-process duplicate guard, then judges the
  per-project attempt budget from `count_memory_classifier_attempts` before any receipt
  is written: an exhausted project with no receipt for the command answers
  `memory_classifier_budget_exhausted` and writes nothing; a command the ledger already
  holds proceeds to `begin_memory_classifier_receipt`, so it replays, refuses a changed
  request, or settles under the same rules as below, and only a takeover
  (which would dispatch) is refused over budget.
- The digest in `run_memory_classifier_task` includes the bound root's kernel scope id,
...
- `begin_memory_classifier_receipt` is the first write. The receipt binding has no
  harness field. Each `MemoryClassifierAttemptSpec` records `project_root`, `harness`,
  ...
  `crates/memory-store/src/memory_classifier_ledger.rs` and the `memory_classifier_attempts` table
  in `crates/memory-store/baseline.sql`. `Complete` replays; `DigestConflict`
  and `BindingMismatch` answer `memory_classifier_request_conflict`; `InProgress` goes
  to `resume_memory_classifier_receipt`, all before a new producer is constructed.
...
- `resume_memory_classifier_receipt` selects the newest current-generation attempt that
  is not `NotSent`. Without one, `take_over_undispatched_memory_classifier_receipt` in
  `crates/memory-store/src/memory_classifier_ledger.rs` atomically checks
...
- An ended marker completes the receipt `unknown` through
  ...
  probe the deadline cuts off answer `memory_classifier_outcome_unknown` without writing.
...
- In the chain loop, `connect` runs under the deadline's remaining duration
  ...
  `begin_memory_classifier_attempt` precedes the start call and
  `record_memory_classifier_run_handle` follows a successful start. A fenced handle
...
- `parse_output` rejects length-capped output before `parse_classify_output`
  ...
  dependent on that memory, with `(ModelInference, MemoryClassifierInference)` admission.
  ...
  Its kernel receipt uses producer `memory_classifier.classify`, an operation key built
  ...
  `memory_classifier.classify`, the memory domain, this project's scope), so the query
...
- `classify_write_refusal` reads the route's memories authority again
  ...
  closing it.
- Normal kernel-write failure is recorded as `memory_classifier_kernel_write_failed`;
...
- `attempt_child_session_id` in `crates/daemon/src/classify.rs` hashes the
  ...
  `memory_classifier_attempt_budget_exhausted` once before the receipt is written and
  ...
  `count_memory_classifier_attempts` in `crates/memory-store/src/memory_classifier_ledger.rs`
...
- Takeover, attempt insertion, handle recording, attempt finish, and receipt
  ...
  state in `crates/memory-store/src/memory_classifier_ledger.rs`. Predecessor writes
  after takeover are `Fenced`, as exercised in
  `crates/memory-store/tests/memory_classifier_ledger.rs` by
...
- Between `begin_memory_classifier_receipt` and `begin_memory_classifier_attempt` a crash leaves a
  receipt with no marker; an admitted successor takes over and may dispatch.
- Between `begin_memory_classifier_receipt` and the pool read, a kernel that is still
...
- Between the resume list read and takeover, a predecessor may insert an attempt.
  ...
  (`take_over_undispatched_memory_classifier_receipt` and `begin_memory_classifier_attempt` in
  `crates/memory-store/src/memory_classifier_ledger.rs`). A generation
  containing only `NotSent` attempts remains eligible.
- Between `start` returning and `record_memory_classifier_run_handle` a crash leaves an
  open attempt with no handle; the successor settles `unknown` because the
  dispatch cannot be disproved.
- Between `record_memory_classifier_run_handle` and `finish_memory_classifier_attempt` a crash
...
- A generation change during `start` or `await_output` fences the returning
  ...
  receipt (`run_memory_classifier_task`).
- Between `finish_memory_classifier_attempt` and `complete_memory_classifier_receipt` a crash leaves
...
- The recovery probe shares the request deadline. A runtime that accepts the
  ...
  answers `memory_classifier_outcome_unknown` with no write; the receipt stays open for a
...
- `purge_session` is not under the request deadline. It runs after a terminal
  ...
  (`crates/daemon/src/history_summarizer_producer.rs`, `DEFAULT_REQUEST_TIMEOUT`),
...
1. A classify run dropped while awaiting output, then a retry with `status`
   ...
   `MemoryClassifierHarness::crash_after_dispatch` in `crates/daemon/src/lib.rs` uses
...
4. A receipt stranded `in_progress` by an aborted attempt insert: assert the
   ...
   generation-derived session and a fenced predecessor `finish_memory_classifier_attempt`
   in `memory_classifier_run_task_takes_over_an_undispatched_receipt_and_dispatches_once`.
...
7. The durable attempt count filled to the budget: assert a new command answers
   `memory_classifier_budget_exhausted` with no receipt and no start, and a completed
...
8. A kernel still `Starting` at the pool read: assert `kernel_unavailable`, the
   ...
   (`memory_classifier_run_task_leaves_the_receipt_open_while_the_kernel_is_starting`).
...
11. No current-generation attempt rows, then an open row, then a `NotSent` row:
   ...
   (`crates/memory-store/tests/memory_classifier_ledger.rs`,
...
12. Move the receipt to generation 2 in each of the scripted producer's
   `on_start` and `on_await_output` hooks. Assert `memory_classifier_ledger_fenced`, one
...
13. A memory admitted `Sensitive` in the bound project, shown served at
   ...
   start
   (`memory_classifier_run_task_refuses_a_memory_served_above_normal_sensitivity`).
...
15. Another project's `memory_classification` observation citing the memory
   ...
   (`memory_classifier_run_task_retires_only_the_classification_rows_it_wrote`).
16. Two authority projects hold one root in turn and reuse a session and command
   ...
   (`memory_classifier_run_task_keys_the_kernel_write_by_authority_project_on_a_shared_root`).
17. Authority drained from the producer's `on_start` hook, after the entry
   ...
   (`memory_classifier_run_task_writes_nothing_when_authority_drains_during_the_model_call`).
...
19. A dispatched attempt whose retry finds the runtime accepting `status` and
    ...
    returns `memory_classifier_outcome_unknown` within the deadline, one `status` call,
    no start, and the receipt and attempt still open
    (`memory_classifier_run_task_bounds_the_recovery_probe_by_the_request_deadline`).
20. A fresh command whose factory `connect` never completes: assert it fails
    within `timeout_ms` as `memory_classifier_run_failed`, one connect, no start, no
    attempt row, and the receipt `complete` as `failed`
    (`memory_classifier_run_task_bounds_producer_startup_by_the_request_deadline`).
21. The scheduler bridge asked to run a slot while the kernel is `Starting`:
    ...
    (`memory_classifier_scheduler_bridge_retains_the_slot_while_the_kernel_is_starting`).
22. The authority read armed to fail from the producer's `on_start` hook so the
    ...
    (`memory_classifier_scheduler_bridge_consumes_a_slot_whose_write_time_authority_read_failed`).
23. Two roots bound to one authority project in one session, the second reusing
    ...
    `memory_classifier_request_conflict`, one start, an unchanged kernel tip, and one
    classification
    (`memory_classifier_run_task_does_not_replay_a_receipt_across_roots_of_one_project`).
...
`MemoryClassifierHarness::start(&producer).await` in `crates/daemon/src/lib.rs`.
The main thread's observed results are recorded in
[the catalog](../catalog.md#memory_classifier-dispatched-attempt-always-settles-through-the-receipt).
At `c35a4ad5`: 50 MemoryClassifier tests passed, followed by 50 further runs of those 50
...
is `memory_classifier_run_task_does_not_purge_the_child_session_once_it_is_fenced`; the
...
- Sources examined: the chain loop's `finish_memory_classifier_attempt` and
  `complete_memory_classifier_receipt` order; the history_summarizer's reattach path
   (`crates/daemon/src/history_summarizer.rs`, `reattach_history_summarizer_producer`).
- Findings: the reply to replay is not durable until `complete_memory_classifier_receipt`,
  ...
  The crashed daemon may also have purged the child session. Recovery therefore
   ...
   The history_summarizer's missing-run branch returns `RefireEligible`; MemoryClassifier instead
...
- Sources examined: the ModelExecution supervisor's run status transitions and terminal
  retention (`crates/host-runtime/src/model_execution/supervisor.rs`, `status`,
  `crates/host-runtime/src/model_execution/config.rs`, `TERMINAL_RETENTION`).
...
- Sources examined: `classify_request_timeout`, `classify_attempt_timeout`, and
  the await call in `run_memory_classifier_task`.
...
- Sources examined: `memory_classifier_attempt_budget_exhausted`, its call before
  `begin_memory_classifier_receipt`, and its call at the top of the chain loop.
...
- Sources examined: `resume_memory_classifier_receipt`'s connect, bind, and `status`
  calls; `HistorySummarizerProducer::status` and `DEFAULT_REQUEST_TIMEOUT`
...
- Findings: the producer's `status` call runs under its own 30 s request
  ...
  cut-off probe answers `memory_classifier_outcome_unknown` with no write, the same shape
