...
`derive_history_summarizer_chunk_tokens` (`:45-48`) and
...
| `terse_text_compression.enabled` | `false` (`:75`) | `false` (`:724`) | Yes, both tiers |
| `terse_text_compression.min_chars` | `500` (`:42`, `:77`) | `500`, no range (`:725`) | Yes, silently clamped to `[100, 10000]` (`:607`) |
| `smart_drops` | `false` (`:135`) | `false` (`:752`) | Yes, both tiers (`:467-469`, `:541-543`) |
| `memory_classifier.inject_docs` | `true` (`:132`) | `true` (`:501`) | Yes, both tiers (`:470-475`, `:544-549`) |
| `temporal_awareness` | `true` (`:133`) | `true` (`:650`) | Yes, both tiers (`:476-478`, `:550-555`) |
| `memory_classifier.tasks.review-user-memories.schedule` / `user_memories.enabled` | privacy gate defaults `false` (`:128`) | task default schedule `0 3 * * *`, i.e. on (`:527`) | Yes as a presence test (`:611-621`). The module's default is closed while the documented task default is scheduled |
| `history_summarizer.model`, `history_summarizer.fallback_models` | empty chain (`:121`) | documented, no user-only marker (`:448-449`) | Yes, user tier only. A project-tier value is dropped with **no** warning (`:514-566` has no entry for it) |
| `history_summarizer.module_model`, `history_summarizer.module_fallback_models` | absent | **undocumented** | Yes, user tier only, and when present it replaces the whole chain (`:390-409`) |
| `history_summarizer.context_limit_tokens` | `128000` (`:37`, `:129`) | **undocumented** | Yes (`:464-466`); project tier warns (`:540`) |
...
| `history_summarizer_timeout_ms` | not parsed | `300000` (`:170`) | **No.** `history_summarizer_producer.rs:209-227` has its own `request_timeout`. 4a scope; flagged as a lead only |
...
5. `history_summarizer.model` / `history_summarizer.fallback_models` — documented without a
   user-only marker, and dropped from the project tier with no warning.

Three further keys take effect but are **undocumented**:
`memory.user_profile_budget_tokens`, `history_summarizer.module_model` (plus its fallback
list), and `history_summarizer.context_limit_tokens`, and
...
Two documented keys (`history_summarizer_timeout_ms`, `history_budget_percentage`) have no
...
2. **A malformed config file becomes silence, not an error.**
   ...
   `no_fire: "no_models"`, so the history_summarizer silently stops firing.
...
4. **Every value clamp in `config.rs` is silent.** `:591` (`0.3..=0.95`), `:595`
   ...
   `min_prompt_chars: 0` and `terse_text_compression.min_chars: 0` silently
...
5. **`cfg.model_chain.dedup()` (`config.rs:571`) is adjacent-only.**
   ...
   `history_summarizer.module_fallback_models` repeats the `module_model` value at any
   non-adjacent position keeps a duplicate in the chain, and
   `history_summarizer.rs:1256` iterates that chain as attempts.
...
10. **The project tier can write four leaves the header's allow-list omits.**
    ...
    threshold, and override "trusted memory, auto-search, terse_text_compression, promotion, and
    privacy settings". The project block at `:514-566` also applies
    `memory.injection_budget_tokens` (`:526-528`), `smart_drops` (`:541-543`),
    `memory_classifier.inject_docs` (`:544-549`), and `temporal_awareness` (`:550-555`).
...
17. **`derive_history_summarizer_chunk_tokens` is total at both integer extremes.**
...
Confidence: high — [evidence](../evidence/dec-a-malformed-config-silently-resolves-to-defaults-and-stops-the-history_summarizer.md). `config.rs:261-264` discards both the parse error and the read error. Traced the default `model_chain: Vec::new()` (`:121`) to `lib.rs:5020-5028`, which records `no_fire: "no_models"`.
Existing check: none. Status `unaudited`.
Impact: a typo in the user config silently disables autonomous history_summarizer firing. The only surface is a `no_models` no-fire reason, which points at model configuration rather than at a parse failure.
Open questions:
...
Check: `always` — for every resolved configuration, the `TriggerContext` built at `lib.rs:4962-4963` carries the configured `enabled` and `min_clusters`. `always` because `prepare_history_summarizer_fire` constructs this context on every transform pass that evaluates a trigger.
Fault/timing angle: none.
...
Impact: a user who disables the commit-cluster trigger still gets commit-cluster-driven history_summarizer fires, each of which spends a model call and replaces raw conversation with generated summary text.
Open questions:
...
Reachability: explicit-config-only
Status: active
Exercised: partial — `config.rs:930-970` and `:981-997` assert that `auto_search`, `terse_text_compression`, `inject_docs`, and `temporal_awareness` follow user-then-project tiers, so the behaviour is pinned as intended. No test asserts the header's allow-list as a closed set.
...
Reachability: explicit-config-only
Status: active
Exercised: partial — `config.rs:930-970` exercises `auto_search` and `terse_text_compression` values inside their clamps and asserts the resulting values. No test supplies an out-of-range value and asserts either the clamped result or a warning.
...
Fault/timing angle: none.
Required faults and enabling state: a config with `memory.auto_search.score_threshold: 0.99`, `memory.auto_search.min_prompt_chars: 0`, or `terse_text_compression.min_chars: 50`.
...
Impact: a user tuning auto-search or terse_text_compression sees no effect from a value outside the clamp and no explanation. `min_prompt_chars: 0`, the natural spelling of "hint on every prompt", silently becomes `20`.
Open questions:
...
Fault/timing angle: none.
Required faults and enabling state: a user config with `history_summarizer.module_model: "a"` and `history_summarizer.module_fallback_models: ["b", "a"]`.
Confidence: high — [evidence](../evidence/dec-a-model-chain-dedup-is-adjacent-only.md). `Vec::dedup` removes only consecutive runs, so `["a","b","a"]` is unchanged. `history_summarizer.rs:1256` iterates the chain as ordered attempts, and `:1300`, `:1380`, and `:1443` slice it as a remaining-candidates list.
Existing check: none. Status `unaudited`.
...
- Should `check_history_segment_trigger_with_index` validate `ctx.trigger_budget` the way it validates everything else? It is test-only today, so this is a latent-hazard question, not a defect. (needs human input)
...
Guarantee: `derive_history_summarizer_chunk_tokens` returns a value in `[MIN_HISTORY_SUMMARIZER_CHUNK_TOKENS, MAX_HISTORY_SUMMARIZER_CHUNK_TOKENS]` for every `usize` input, without panicking.
Check: `always` — for every input, the result is in `[8000, 50000]`. `always` because every history_summarizer firing derives the budget from the configured limit.
Fault/timing angle: none.
Required faults and enabling state: `history_summarizer.context_limit_tokens` set to `0` is impossible, because `positive_usize_at` (`config.rs:623-629`) discards it. Reaching the extremes needs a very large configured limit or a direct call.
Confidence: high — [evidence](../evidence/dec-a-derive-history_summarizer-chunk-tokens-is-total-at-both-integer-extremes.md). `config.rs:45-48`. Executed the exact body: `usize::MAX -> 50000` because a float-to-integer `as` cast saturates rather than wrapping, `0 -> 8000`, `1 -> 8000`. Traced the three call sites at `lib.rs:4700`, `:5087`, `:5250`.
Existing check: `config.rs:972-978` `history_summarizer_budget_derivation_clamps_at_both_bounds`. Status `unaudited`.
...
1. **`history_summarizer_timeout_ms` has no consumer in `daemon`.**
   `CONFIGURATION.md:170` documents `300000` as the "Timeout per history_summarizer call
   (ms)". `rg history_summarizer_timeout_ms crates/daemon/src` returns zero matches,
   while `history_summarizer_producer.rs:209-210` and `:226-227` carry their own
   `request_timeout` and `await_timeout` from private constants. The module makes
   the history_summarizer call, so the documented key names behaviour it owns. 4a scope.
...
4. **`history_summarizer.module_model` is the highest-consequence undocumented key.**
   `config.rs:390-409` gives it precedence over `history_summarizer.model` and replaces
...
6. **The code's legacy fallback pointer may not match the documented legacy
   ...
   block" under `memory_classifier`, which would be `/memory_classifier/user_memories`. If the legacy
   key really lived under `memory_classifier`, the fallback never fires for a real legacy
