...
`eidnara-local-embeddings-fingerprint-v1`, so the committed tiny fixture's manifest
...
Pre-image. `canonical_fingerprint` (`local_embeddings/bundle.rs:566-614`) starts the
pre-image with `eidnara-local-embeddings-fingerprint-v1` (`:577`) and appends
...
Python mirror. `tests/fixtures/generate-local_embeddings-tiny.py:115-157` defines
...
Fixture. `tests/fixtures/local_embeddings-tiny/manifest.json` carries `fingerprint`
...
`LocalEmbeddingsComponent::activate` at `local_embeddings/mod.rs:1025-1029`, with the
generation's committed digest passed as `bundle_manifest_sha256`.

Existing checks, verified, in `tests/local_embeddings_bundle.rs`, a default-harness
...
- `fingerprint_binds_initializer_names_to_their_hashes`
  (`local_embeddings/bundle.rs:898-923`) swaps two initializer names while keeping
...
(`local_embeddings/mod.rs:1024`). The generator's mirror must move in the same edit
as the Rust function (`generate-local_embeddings-tiny.py:122-125`); the fixture test
...
- Sources examined: `tests/local_embeddings_bundle.rs:375-389`, `:395-417`;
  `local_embeddings/bundle.rs:262-268`; the fixture manifest.
...
- Sources examined: `tests/local_embeddings_bundle.rs:282-305`;
  `local_embeddings/bundle.rs:230-257`, `:262-268`, `:683-697`.
