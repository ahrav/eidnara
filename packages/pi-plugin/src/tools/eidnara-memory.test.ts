...
function harness(
    ...
    const tool = createEidnaraMemoryTool({
...
describe("Pi eidnara_memory revise", () => {
    ...
    it("renders the same conflict text as OpenCode after a concurrent change", async () => {
        ...
        expect(textOf(piResult)).toBe(
            `Error: ${conflictText} Re-read mem_seeded with eidnara_memory get, then retry.`,
        );
        ...
        const openCode = createEidnaraMemoryTools({
...
describe("Pi eidnara_memory archive and merge", () => {
...
describe("Pi eidnara_memory reads and action gates", () => {
...
describe("Pi eidnara_memory imitated reduced arguments", () => {
...
describe("Pi eidnara_memory memory state table", () => {
    it.each(
        MEMORY_STATE_TABLE,
...
describe("Pi eidnara_memory source", () => {
    it("contains no local-store IDs, embeddings, or mutation-log writes", () => {
        const source = readFileSync(resolve(import.meta.dir, "eidnara-memory.ts"), "utf8");
