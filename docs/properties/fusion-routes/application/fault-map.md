...
| Count eviction | `max_keys = 2` on a live daemon; every receipt in flight; `max_keys` narrowed from 8 to 3 with synthetic `Instant`s in the module tests. | apply-receipt-retention-is-bounded-and-eviction-cannot-authorize-replay |
