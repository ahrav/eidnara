...
replacement. 64 constant situation markers and five forbidden-operation
observers are preserved. Records were authored against the tree of this catalog's introducing commit on
branch `payload-pools/546-owned-pool-transport`; every `file:line` was resolved
by search at authoring time. Read
...
| `private-decode-input-stability` | safety | default-production | `always` | partial |
| `request-conversion-completion-ownership` | safety | default-production | `always` | not yet |
...
| `terminal-credit-follows-storage` | safety | default-production | `always` | not yet |
| `reserved-progress-under-data-exhaustion` | liveness | default-production | `always` | partial |
| `reserved-publication-order` | safety | default-production | `always` | not yet |
...
| `direct-serialization-commit-boundary` | safety | default-production | `always` | partial |
| `terminal-encoding-reserve-bound` | safety | default-production | `always` | partial |
| `send-outcome-no-generic-replay` | safety | default-production | `always` | partial |
| `reclamation-diagnostics-meaning` | safety | default-production | `always` | partial |
...
Status: active
Exercised: partial - `crates/shm-transport/src/backend/ring.rs:1970` copies the four fields out under Miri and shows a later peer rewrite changes nothing the receiver holds; `crates/shm-transport/src/backend/ring.rs:2335` forges every field through the peer handle. No test races a rewrite against the copy itself.
...
Confidence: high - [evidence](evidence/descriptor-private-snapshot.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:1298`; `crates/shm-transport/src/descriptor.rs:222`; `crates/shm-transport/src/backend/ring.rs:1346`.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/backend/ring.rs:2145` publishes past a one-slot ordinary depth after the consumer acknowledges while still holding the payload; `crates/shm-transport/tests/ring.rs:452` does the same across processes.
...
Confidence: high - [evidence](evidence/descriptor-capacity-independent-of-payload.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:1115`; `crates/shm-transport/src/backend/ring.rs:1342`; `crates/shm-transport/src/backend/retained.rs:620`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2145` publishes past a one-slot ordinary depth after the consumer acknowledges while still holding the payload; `crates/shm-transport/tests/ring.rs:452` does the same across processes.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/backend/ring.rs:2413` and `crates/shm-transport/src/backend/ring.rs:2335`; `crates/shm-transport/tests/contract.rs:118` covers the pure validator.
...
Confidence: high - [evidence](evidence/payload-identity-authorizes-reuse.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:1080`; `crates/shm-transport/src/backend/retained.rs:601`; `crates/shm-transport/src/descriptor.rs:222`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2413` and `crates/shm-transport/src/backend/ring.rs:2335`; `crates/shm-transport/tests/contract.rs:118` covers the pure validator.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/backend/ring.rs:2110` holds A for `2*depth+1` reuses of B; `crates/shm-transport/tests/ring.rs:317` repeats it across a process boundary with returns from a worker thread.
...
Confidence: high - [evidence](evidence/released-block-reuse-preserves-held-bytes.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/pool.rs:285`; `crates/shm-transport/src/backend/ring.rs:985`; `crates/shm-transport/src/lease.rs:308`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2110` holds A for `2*depth+1` reuses of B; `crates/shm-transport/tests/ring.rs:317` repeats it across a process boundary with returns from a worker thread.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/backend/ring.rs:2179`, `crates/shm-transport/src/backend/ring.rs:2208`, and `crates/shm-transport/src/backend/ring.rs:2243`.
...
Confidence: high - [evidence](evidence/class-allocation-conservation.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/pool.rs:273`; `crates/shm-transport/src/backend/ring.rs:1133`; `crates/shm-transport/src/backend/ring.rs:533`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2179`, `crates/shm-transport/src/backend/ring.rs:2208`, and `crates/shm-transport/src/backend/ring.rs:2243`.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/lease.rs:563`, `crates/shm-transport/src/lease.rs:592`, and `crates/shm-transport/src/lease.rs:607` run under Miri.
...
Confidence: high - [evidence](evidence/completion-cell-final-owner-once.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/lease.rs:346`; `crates/shm-transport/src/backend/retained.rs:588`; `crates/shm-transport/src/backend/retained.rs:354`.
Existing check: `crates/shm-transport/src/lease.rs:563`, `crates/shm-transport/src/lease.rs:592`, and `crates/shm-transport/src/lease.rs:607` run under Miri.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/backend/ring.rs:2497`, `crates/shm-transport/src/backend/ring.rs:2537`, and both two-process tests in crates/shm-transport/tests/ring.rs.
...
Confidence: high - [evidence](evidence/capacity-wake-progress.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:1024`; `crates/shm-transport/src/backend/retained.rs:620`; `crates/shm-transport/src/backend/ring.rs:75`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2497`, `crates/shm-transport/src/backend/ring.rs:2537`, and both two-process tests in crates/shm-transport/tests/ring.rs.
...
Confidence: high - [evidence](evidence/wake-failure-preserves-published-ownership.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:1210`; `crates/shm-transport/src/backend/retained.rs:624`; `crates/shm-transport/src/backend/retained.rs:648`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2579`; the return-side latch is exercised by `crates/shm-transport/src/backend/ring.rs:2473` whose peer end is closed.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/lease.rs:625` (Miri) and `crates/shm-transport/src/backend/ring.rs:2473`.
...
Confidence: high - [evidence](evidence/retained-mapping-lifetime.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/lease.rs:248`; `crates/shm-transport/src/backend/retained.rs:270`; `crates/host-runtime/src/ring_transport.rs:299`.
Existing check: `crates/shm-transport/src/lease.rs:625` (Miri) and `crates/shm-transport/src/backend/ring.rs:2473`.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/tests/ring.rs:73` and `crates/shm-transport/src/backend/ring.rs:2243`; application vectors stay frozen in `crates/host-runtime/tests/protocol_vectors.rs`.
...
Confidence: high - [evidence](evidence/application-frame-interoperability.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/pool.rs:310`; `crates/shm-transport/src/profile.rs:695`; `crates/shm-transport/src/descriptor.rs:26`.
Existing check: `crates/shm-transport/tests/ring.rs:73` and `crates/shm-transport/src/backend/ring.rs:2243`; application vectors stay frozen in `crates/host-runtime/tests/protocol_vectors.rs`.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/tests/contract.rs:167`, `crates/shm-transport/src/backend/ring.rs:2612`, `crates/shm-transport/tests/profile.rs:261`, and `stale_wire_or_descriptor_schema_is_invalid_identity` in `crates/host-runtime/src/setup_socket.rs`.
...
Confidence: high - [evidence](evidence/sole-identifiers-before-activation.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:170`; `crates/shm-transport/src/backend/ring.rs:704`; `crates/host-runtime/src/ring_transport.rs:918`; `packages/shm-native/src/lib.rs:307`.
Existing check: `crates/shm-transport/tests/contract.rs:167`, `crates/shm-transport/src/backend/ring.rs:2612`, `crates/shm-transport/tests/profile.rs:261`, and `stale_wire_or_descriptor_schema_is_invalid_identity` in `crates/host-runtime/src/setup_socket.rs`.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/backend/ring.rs:2699`, `crates/shm-transport/tests/ring.rs:103`, `crates/shm-transport/src/pool.rs:598`, and `crates/shm-transport/tests/fuzz_corpus.rs:92`.
...
Confidence: high - [evidence](evidence/validated-setup-geometry.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:321`; `crates/shm-transport/src/backend/retained.rs:761`; `crates/shm-transport/src/backend/retained.rs:293`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2699`, `crates/shm-transport/tests/ring.rs:103`, `crates/shm-transport/src/pool.rs:598`, and `crates/shm-transport/tests/fuzz_corpus.rs:92`.
...
Confidence: medium - [evidence](evidence/authentication-transcript-boundary.md). Verified against the tree of this catalog's introducing commit: `packages/shm-native/src/setup.rs:116`; `crates/shm-transport/src/backend/ring.rs:286`.
...
Reachability: default-production
Status: active
Exercised: partial - `crates/shm-transport/src/backend/ring.rs:2335` covers descriptor and header structure; host header validation stays in `validate_inbound_header` tests in `crates/host-runtime/src/frame_channel.rs`.
...
Confidence: high - [evidence](evidence/structural-rejection-before-dispatch.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:1332`; `crates/host-runtime/src/ring_transport.rs:714`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2335` covers descriptor and header structure; host header validation stays in `validate_inbound_header` tests in `crates/host-runtime/src/frame_channel.rs`.
...
Reachability: default-production
Status: active
Exercised: partial - `crates/shm-transport/src/lease.rs:650` and `crates/shm-transport/src/lease.rs:756` under Miri prove same-shape access; cross-process hostile writers are not provable here (recorded limitation).
...
Existing check: `crates/shm-transport/src/lease.rs:650` and `crates/shm-transport/src/lease.rs:756` under Miri prove same-shape access; cross-process hostile writers are not provable here (recorded limitation).
...
Reachability: default-production
Status: active
Exercised: yes - compile-time: the `compile_fail` doctests at the top of crates/shm-transport/src/backend/ring.rs and the `assert_send::<PayloadLease>` in crates/shm-transport/src/lease.rs; runtime: `crates/shm-transport/src/lease.rs:592` and `crates/shm-transport/src/backend/ring.rs:2738`.
...
Confidence: high - [evidence](evidence/owned-lease-thread-boundary.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:25`; `crates/shm-transport/src/backend/retained.rs:356`; `crates/shm-transport/src/lease.rs:21`.
Existing check: compile-time: the `compile_fail` doctests at the top of crates/shm-transport/src/backend/ring.rs and the `assert_send::<PayloadLease>` in crates/shm-transport/src/lease.rs; runtime: `crates/shm-transport/src/lease.rs:592` and `crates/shm-transport/src/backend/ring.rs:2738`.
...
Open questions:
- Handoff: #548 owns the raw-lease `InboundFrame` conversion and the header/body consistency check.
...
Reachability: default-production
Status: active
Exercised: not yet - the request-scoped copy/conversion seam over the owned lease is #548's implementation; the join owner from PR #518 exists in `crates/host-runtime/src/handler.rs` (`RequestCtx::run_blocking`).
...
Check: `always` - a lease moved into blocking work returns only after that work joins; `Cancel` observed mid-copy leaves `outstanding_returns` unchanged until the join.
...
Confidence: low - [evidence](evidence/request-conversion-completion-ownership.md). Verified against the tree of this catalog's introducing commit: `crates/host-runtime/src/handler.rs:606`; PR #518 `c70367615f7e391d7786395b58fb71b4a43bc47e`.
Existing check: the request-scoped copy/conversion seam over the owned lease is #548's implementation; the join owner from PR #518 exists in `crates/host-runtime/src/handler.rs` (`RequestCtx::run_blocking`).
...
Confidence: medium - [evidence](evidence/partial-close-token-conservation.md). Verified against the tree of this catalog's introducing commit: `packages/shm-native/src/lib.rs:409`; `packages/shm-native/src/lib.rs:1653`.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/tests/profile.rs:261` checks the charge equals the created object size; `crates/shm-transport/tests/profile.rs:131` and `process_limits_reject_counts_above_the_resident_byte_ceiling` in crates/host-runtime/src/ring_transport.rs.
...
Confidence: high - [evidence](evidence/complete-capacity-admission.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/profile.rs:166`; `crates/host-runtime/src/ring_transport.rs:70`; `crates/shm-transport/src/pool.rs:495`.
Existing check: `crates/shm-transport/tests/profile.rs:261` checks the charge equals the created object size; `crates/shm-transport/tests/profile.rs:131` and `process_limits_reject_counts_above_the_resident_byte_ceiling` in crates/host-runtime/src/ring_transport.rs.
Impact: An under-charged connection oversubscribes the process ceiling.
Open questions:
- Handoff: #548 for host-controlled private copies, encoding slices, and supporting queues.
...
Reachability: default-production
Status: active
Exercised: not yet - the terminal-credit and dedicated encoding reservation are #548's implementation over the `Inventory::Terminal` class this transport provides.
...
Fault/timing angle: Cancellation and foreign retention of a terminal block.
Required faults and enabling state: A cancelled request whose terminal block is still held by the peer. Markers: marker:`host.terminal_held_after_cancel`.
Confidence: low - [evidence](evidence/terminal-credit-follows-storage.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/pool.rs:40`; `crates/shm-transport/src/backend/ring.rs:973`.
Existing check: the terminal-credit and dedicated encoding reservation are #548's implementation over the `Inventory::Terminal` class this transport provides.
...
Reachability: default-production
Status: active
Exercised: partial - `crates/shm-transport/src/backend/ring.rs:2285` proves control and terminal reservations succeed while ordinary descriptor headroom is exhausted; host and client publication selection belong to #548, #552, and #550.
...
Confidence: high - [evidence](evidence/reserved-progress-under-data-exhaustion.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:971`; `crates/shm-transport/src/pool.rs:31`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2285` proves control and terminal reservations succeed while ordinary descriptor headroom is exhausted; host and client publication selection belong to #548, #552, and #550.
Impact: A draining peer that cannot exchange controls under data backpressure never recovers.
Open questions:
- Handoff: #548, #552, #550 for publication selection.
...
Reachability: default-production
Status: active
Exercised: not yet - eligibility selection (Request correlation order, stream prefix before terminal, drain before Goodbye, no channel-0 Request bypass) is #548's host publisher and #552/#550's client publishers; this layer promises FIFO descriptor consumption only.
...
Confidence: low - [evidence](evidence/reserved-publication-order.md). Verified against the tree of this catalog's introducing commit: docs/payload-pool-protocol.md section 11; `docs/host-wire-protocol.md:314`.
Existing check: eligibility selection (Request correlation order, stream prefix before terminal, drain before Goodbye, no channel-0 Request bypass) is #548's host publisher and #552/#550's client publishers; this layer promises FIFO descriptor consumption only.
...
Reachability: default-production
Status: active
Exercised: partial - `crates/shm-transport/tests/profile.rs:215` covers worker/backing settlement, quarantine, and uncertain retention; `crates/host-runtime/src/ring_transport.rs:344` refunds on a pre-exposure failure. Failure after each individual acquisition is not yet injected.
...
Confidence: medium - [evidence](evidence/partial-setup-reclaims-only-unexposed-resources.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/profile.rs:636`; `crates/host-runtime/src/ring_transport.rs:387`.
...
Open questions:
- Handoff: #548 for per-acquisition failure injection.
...
Reachability: default-production
Status: active
Exercised: yes - `crates/shm-transport/src/backend/ring.rs:2179` and `crates/shm-transport/tests/profile.rs:107`.
...
Confidence: high - [evidence](evidence/bounded-refusal-and-recovery.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:976`; `crates/shm-transport/src/profile.rs:416`.
Existing check: `crates/shm-transport/src/backend/ring.rs:2179` and `crates/shm-transport/tests/profile.rs:107`.
Impact: An unbounded or unrecoverable refusal is a hang the peer cannot diagnose.
Open questions:
- Handoff: #548 for refusal-by-resource diagnostics.
...
Reachability: default-production
Status: active
Exercised: partial - `crates/host-runtime/src/ring_transport.rs:819` serializes through `ReservationWriter` after reservation and commits under the frame deadline; `crates/shm-transport/src/backend/ring.rs:2208` covers abort and short commit.
...
Confidence: medium - [evidence](evidence/direct-serialization-commit-boundary.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/ring.rs:1559`; `crates/host-runtime/src/ring_transport.rs:858`.
Existing check: `crates/host-runtime/src/ring_transport.rs:819` serializes through `ReservationWriter` after reservation and commits under the frame deadline; `crates/shm-transport/src/backend/ring.rs:2208` covers abort and short commit.
Impact: Serializing into class slack or without a reservation would write bytes no descriptor accounts for.
Open questions:
- Handoff: #548 for the unreserved-serializer-runs-zero-times witness.
...
Reachability: default-production
Status: active
Exercised: partial - `crates/shm-transport/tests/contract.rs:187` checks the 32 KiB terminal block holds 25,406 body and 25,427 frame bytes; the serializer-level fit is #548's.
...
Confidence: medium - [evidence](evidence/terminal-encoding-reserve-bound.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/tests/contract.rs:217`; `crates/host-runtime/src/dispatch.rs:71`.
Existing check: `crates/shm-transport/tests/contract.rs:187` checks the 32 KiB terminal block holds 25,406 body and 25,427 frame bytes; the serializer-level fit is #548's.
...
Reachability: default-production
Status: active
Exercised: partial - `crates/host-runtime/src/ring_transport.rs:1029` classifies `Deadline`/`Unreserved` as zero-byte and `Reserved` as unknown; `a_client_send_past_its_frame_deadline_publishes_nothing` in crates/host-runtime/src/ring_transport.rs. Stop/restart witnesses belong to #548, #552, #550.
...
Confidence: medium - [evidence](evidence/send-outcome-no-generic-replay.md). Verified against the tree of this catalog's introducing commit: `crates/host-runtime/src/ring_transport.rs:966`; `crates/shm-transport/src/backend/ring.rs:948`.
Existing check: `crates/host-runtime/src/ring_transport.rs:1029` classifies `Deadline`/`Unreserved` as zero-byte and `Reserved` as unknown; `a_client_send_past_its_frame_deadline_publishes_nothing` in crates/host-runtime/src/ring_transport.rs. Stop/restart witnesses belong to #548, #552, #550.
...
Reachability: default-production
Status: active
Exercised: partial - the transport exposes `outstanding_returns` (`crates/shm-transport/src/backend/retained.rs:440`) and `PoolInventory` (`crates/shm-transport/src/backend/ring.rs:515`); the host's `reclamation.completed` counter still counts generation ends (`crates/host-runtime/src/ring_transport.rs:261`).
Guarantee: Diagnostics report outstanding return obligations, quarantined commitment, and actually released backing as distinct, consistently snapshotted quantities.
...
Confidence: medium - [evidence](evidence/reclamation-diagnostics-meaning.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/src/backend/retained.rs:344`; `crates/host-runtime/src/ring_transport.rs:275`.
Existing check: the transport exposes `outstanding_returns` (`crates/shm-transport/src/backend/retained.rs:440`) and `PoolInventory` (`crates/shm-transport/src/backend/ring.rs:515`); the host's `reclamation.completed` counter still counts generation ends (`crates/host-runtime/src/ring_transport.rs:261`).
...
Reachability: test-only
Status: active
Exercised: partial - `crates/shm-transport/tests/ring.rs:317` records a completed real cross-process exchange at layout 4; the direct-host E2E and native suites still skip on Bun 1.3.14 (`markAsUntransferable` unimplemented) and on Node (`node_detachment_unavailable`), which are recorded limitations, not passes.
...
Confidence: medium - [evidence](evidence/real-process-current-layout-witness.md). Verified against the tree of this catalog's introducing commit: `crates/shm-transport/tests/ring.rs:396`; `packages/e2e-tests/src/rust-runner/hermetic-host.ts:275`.
Existing check: `crates/shm-transport/tests/ring.rs:317` records a completed real cross-process exchange at layout 4; the direct-host E2E and native suites still skip on Bun 1.3.14 (`markAsUntransferable` unimplemented) and on Node (`node_detachment_unavailable`), which are recorded limitations, not passes.
