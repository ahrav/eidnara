...
//! across turns without thrashing the provider prefix cache. Every consumer pins the
//! same golden-vector contract (`cache-stability-golden-vectors.json`, schema_version 3)
//! rather than this crate's internals, so the vectors are byte-stable.
//!
//! ## The one invariant
//!
...
mod tests {
    ...
    fn soft_replaces_by_key_keeps_slot_appends_new() {
        ...
        assert_eq!(
            ...
            "SOFT with new_boundary_id=None leaves the anchor unchanged (the common case: a \
             delta that rides within existing coverage)"
        );
    }
