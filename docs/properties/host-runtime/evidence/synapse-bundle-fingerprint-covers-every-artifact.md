...
changed the pre-image is noticed.
...
- Findings: no test carries the hex literal. The external agreement is that
  the fixture manifest was written by the Python generator and `load_bundle`
  recomputes the same value in Rust; the load succeeding is the
  cross-implementation check. The audit's own Python reproduction confirms
  the value.
- Missing evidence: none for agreement today. If the fixture were
  regenerated by an edited generator alongside an edited Rust function, the
  load would still succeed.
- Conclusion: resolved. Agreement is proven through the fixture load, not
  through a literal in a test.
...
- Missing evidence: a test that mutates a manifest `sha256` field, or a
  struct-level enumeration as described above.
- Conclusion: resolved with the record's own caveat: coverage of every
  artifact by the pre-image rests on reading `:584-612` against the mirror,
  and the audit's reading finds all seven artifact hashes present.
