...
1. The guard at `:512` requires `outcome == SendOutcome::Terminal`. An
   ...
   protocol's own `terminal` definition at `docs/host-wire-protocol.md:62`:
...
3. The absolute deadline at `:458` is checked at `:520` and clamps the sleep at
   `:523`, so the loop terminates. `docs/host-wire-protocol.md:746` requires
...
- Sources examined: `docs/host-wire-protocol.md:658` (the `server_busy` versus
