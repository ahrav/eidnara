...
pub enum InstanceError {
    #[error("eidnara-host only conforms on Unix numeric-IPv4-loopback profiles")]
    UnsupportedPlatform,
    ...
    #[error("another eidnara-host instance holds the lock")]
    AlreadyRunning,
...
/// `crate::lifecycle::LifetimeLock` is acquired before this lock on `.host-coordination`.
...
/// Renaming `.host-coordination` externally splits the lifetime fence.
/// `.host-coordination` must not be renamed externally.
