 ...
 * agent_end cleanup.
 *
 * Pi and OpenCode share `~/.local/share/eidnara/context/context.db`.
 *   ~/.local/share/eidnara/context/context.db
...
import { isCompactionEnabled, isDreamerRunnable } from "@eidnara/opencode/config/agent-disable";
import { migrateEidnaraConfigLocations } from "@eidnara/opencode/config/migrate-config-location";
import type {
    DreamerConfig,
    EidnaraConfig,
    HistorianConfig,
...
} from "@eidnara/opencode/features/context/dreamer/task-config";
import {
    type FailClosedReason,
    formatFailClosedBlockingMessage,
} from "@eidnara/opencode/features/context/fail-closed-block";
import { configureSynapseManagedDemandStart } from "@eidnara/opencode/features/context/memory/embedding-synapse";
import {
    resolveProjectIdentityForSession,
    resolveProjectRootDirectory,
} from "@eidnara/opencode/features/context/memory/project-identity";
import { scheduleIncrementalIndex } from "@eidnara/opencode/features/context/message-index-async";
import { detectOverflow } from "@eidnara/opencode/features/context/overflow-detection";
...
} from "@eidnara/opencode/hooks/context/derive-budgets";
import { resolveCacheTtl } from "@eidnara/opencode/hooks/context/event-resolvers";
import {
...
import {
    ...
    onNoteTrigger,
} from "@eidnara/opencode/hooks/context/note-nudger";
import { preloadTokenizer } from "@eidnara/opencode/hooks/context/read-session-formatting";
import { normalizeTodoStateJson } from "@eidnara/opencode/hooks/context/todo-view";
import { maybeSendUpgradeReminder } from "@eidnara/opencode/hooks/context/upgrade-reminder";
import { beginBootQuietPeriod } from "@eidnara/opencode/plugin/boot-quiet";
import {
...
} from "@eidnara/opencode/shared/announcement";
...
import { setKeepSubagents } from "@eidnara/opencode/shared/keep-subagents";
...
import { setStoragePrivatePermissionEnforcement } from "@eidnara/opencode/shared/storage-permissions";
...
import {
    ...
    EIDNARA_PI_SUBAGENT_ENV,
    PiSubagentRunner,
} from "./subagent-runner";
import {
    buildEidnaraBlock,
    clearPiSystemPromptSession,
    composeEidnaraSystemPrompt,
...
/**
 * Pi native compaction invalidates Eidnara's cached `m[0]` and `m[1]` bytes.
...
export function persistPiMessageEndModelMeta(args: {
    ...
    sessionId: string;
    message: unknown;
    cacheTtlConfig: EidnaraConfig["cache_ttl"];
}): void {
...
// The loader checks shared Eidnara paths before Pi-owned legacy files.
// The loader falls back to Pi-owned legacy files only when no shared Eidnara base exists.
...
function ensureConfigLocationsMigrated(dir: string): void {
    ...
    migrateEidnaraConfigLocations(dir, {
        warn: (m) => warn(m),
        info: (m) => info(m),
    });
}
        ...
        if (
            ...
            if (!meta.cacheAlertSent) {
                ...
                await args.notifyIssue?.(
                    `⚠️ Eidnara: Pi reports a context limit of ${formatTokens(contextLimit)} tokens for ${modelLabel} but you've successfully sent ${formatTokens(safeTokens)} tokens in this session — the reported limit looks wrong. Restart Pi if you suspect this is incorrect.`,
                );
            }
        }
...
export function resolveHistorianFromConfig(config: EidnaraConfig): PiHistorianOptions | undefined {
...
function resolveAutoSearchFromConfig(config: EidnaraConfig): PiAutoSearchHandlerOptions {
...
export function resolveDreamerFromConfig(config: EidnaraConfig): DreamerConfig | undefined {
...
export default async function (pi: ExtensionAPI): Promise<void> {
    ...
    setStoragePrivatePermissionEnforcement(bootConfig.config.storage.enforce_private_permissions);
    setSqlitePragmaConfig({
    ...
    // When fail_closed_blocking is on (the default), Eidnara registers a blocking surface.
    // The blocking surface prevents Eidnara from silently skipping hooks.
    if (!db) {
        ...
        if (early.config.fail_closed_blocking === false || !isCompactionEnabled(early.config)) {
            warn(
                `Eidnara (pi) storage unavailable at ${dbPath}: ${formatFailClosedBlockingMessage(reason)}. ` +
        ...
        registerPiFailClosedSurface(pi, {
            ...
            onRecovered: async (recoveredDb) => {
                ...
                await startPiEidnaraRuntime(pi, recoveredDb, dbPath);
            },
        });
        return;
    }

    await startPiEidnaraRuntime(pi, db, dbPath);
}

/**
 * The fail-closed surface can re-probe and start the runtime without a process restart.
 */
async function startPiEidnaraRuntime(
    pi: ExtensionAPI,
    ...
    if (!compactionOff) {
        try {
            ...
            warn(
                `Eidnara (pi) failed to rehydrate deferred Pi compaction markers: ${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
        ...
        if (hist) {
            ...
            hist.onStatusChange = (ctx) => {
                updateStatusLine(ctx, {
                    db: database,
                    projectIdentity:
                        resolveCurrentProject(ctx, cfg.allow_home_project).projectIdentity ?? "",
                });
            };
        }
    ...
    info(
        bootProjectDeps.historianConfig
            ...
            : "registered historian trigger: DISABLED (set historian.model in eidnara.jsonc)",
    );
    info(
    ...
    const historianCommandDeps = (deps: typeof bootProjectDeps, runner: typeof recompRunner) => ({
        db,
        runner,
    ...
    pi.on("before_agent_start", async (event, ctx) => {
        ...
        // Pi and OpenCode share `getEidnaraStorageDir()/last_announced_version` to suppress duplicate announcements.
        //
        ...
        try {
            if (ctx.hasUI && shouldShowAnnouncement()) {
                ...
                const sections = [
                    `✨ Eidnara v${ANNOUNCEMENT_VERSION} — what's new:`,
                    "",
                    featureText,
                ];
        ...
        try {
            ...
            //
            // The handler clears the refresh signal only after `buildEidnaraBlock` and `processSystemPromptForCache` succeed.
            // `buildEidnaraBlock` or `processSystemPromptForCache` failures preserve the signal for retry on the next prompt.
            ...
            const composedPrompt = composeEidnaraSystemPrompt(event.systemPrompt, block);
            ...
            warn("failed to build eidnara block:", error);
            return;
        }
    });
    ...
    // Eidnara requires background compaction while the main agent continues.
    //
    ...
    pi.on("message_end", async (event, ctx) => {
        ...
        try {
            ...
            log(
                `[eidnara][${sessionId}] overflow detected: reportedLimit=${
                    detection.reportedLimit ?? "?"
