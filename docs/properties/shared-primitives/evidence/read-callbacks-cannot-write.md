...
- **Existing evidence:** `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:1651`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:1778`), `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:2004`).
