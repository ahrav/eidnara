...
The proof construction, `crates/host-runtime/src/auth.rs:140-153`:
...
`docs/host-wire-protocol.md:177` says "Server-nonce freshness is the replay
...
The peer side matches: `packages/shm-native/src/setup.rs:181-182` draws the
...
1. run one complete successful handshake through
   `host_runtime::authenticate_client` and capture both peer-to-host messages verbatim,
...
`crates/host-runtime/tests/support/raw_client.rs:878` already provides
...
- Conclusion: resolved. `client_nonce` is fully peer-controlled and unvalidated
  ...
  (`host-wire-protocol.md:177`) but the host can only enforce its own half.
...
- Sources examined: `auth.rs:326-338`, `packages/shm-native/src/setup.rs:200-205`.
