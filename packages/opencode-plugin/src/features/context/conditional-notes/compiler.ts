...
import { NOTE_CONDITION_COMPILER_AGENT } from "../../../agents/note-condition-compiler";
import {
...
import type {
    ConditionalNoteCapabilityApi,
    ConditionalNoteCapabilityFactory,
} from "./capabilities";
import { NOTE_CONDITION_COMPILER_SYSTEM_PROMPT } from "./compiler-prompt";
import { runCompiledConditionalNoteCheck } from "./sandbox-runner";
import {
    decodeStringLiteral,
    maskSourceSpans,
    type SourceSpanKind,
...
import {
    type ConditionalNoteCapabilityName,
    type ConditionalNoteCheckManifest,
    type ConditionalNoteCheckResult,
    ConditionalNoteNetworkError,
} from "./types";

interface CompileConditionalNoteArgs {
    client: PluginContext["client"];
    parentSessionId: string | undefined;
    sessionDirectory: string | undefined;
    projectIdentity: string;
    note: { id: number; content: string; surfaceCondition: string | null };
    capabilityFactory: ConditionalNoteCapabilityFactory;
    signal: AbortSignal;
    deadline: number;
    model?: string;
...
export interface CompileConditionalNoteSuccess {
    ok: true;
    compiledCheck: string;
    manifest: ConditionalNoteCheckManifest;
    checkCron: string;
    checkHash: string;
    /** Null when a declared URL is unreachable during compilation. */
    dryRun: ConditionalNoteCheckResult | null;
    dryRunNetworkError?: string;
}

export interface CompileConditionalNoteFailure {
    ok: false;
    cancelled: boolean;
    error: string;
}

export type CompileConditionalNoteResult =
    | CompileConditionalNoteSuccess
    | CompileConditionalNoteFailure;
...
const DEADLINE_EXPIRED_ERROR = "conditional-note compile deadline expired";
...
export async function compileConditionalNoteCheck(
    args: CompileConditionalNoteArgs,
): Promise<CompileConditionalNoteResult> {
    if (!args.note.surfaceCondition) {
        return { ok: false, cancelled: false, error: "note has no surface condition" };
    }
    if (args.signal.aborted) {
        return { ok: false, cancelled: true, error: "conditional-note compile cancelled" };
    }
    ...
    const prompt = `Compile this conditional note condition into a sandbox check.
...
Remember: output only the JSON object described by the system prompt.`;
    ...
    try {
        const createResponse = await createChildSession({
            ...
            title: `eidnara-conditional-note-compile-${args.note.id}`,
        ...
        if (!childSessionId) throw new Error("Could not create conditional-note compiler session");
        ...
        const run = await shared.promptSyncWithValidatedOutputRetry(
            args.client,
            {
                ...
                body: {
                    agent: NOTE_CONDITION_COMPILER_AGENT,
                    system: NOTE_CONDITION_COMPILER_SYSTEM_PROMPT,
            ...
            },
            {
                ...
                callContext: "note-condition-compiler",
...
async function validateCompilerOutput(
    ...
    for (const warning of manifestAdvisoryWarnings(compiledCheck, manifest)) {
        log(`[conditional-notes] conditional note #${noteId}: manifest advisory — ${warning}`);
    }
    const bound = await bindDeclaredRequests(compiledCheck, capabilityFactory, signal);
    const dryRun = await runCompiledConditionalNoteCheck({
        compiledCheck,
    ...
    if (!dryRun.cancelled && dryRun.hostNetworkError === true) {
        const escaped = bound
            ...
            .find((message) => dryRun.error === `${ConditionalNoteNetworkError.name}: ${message}`);
...
type BoundResponse =
    ...
    | { ok: false; error: ConditionalNoteNetworkError };

export interface BoundRequests {
    factory: ConditionalNoteCapabilityFactory;
...
export async function bindDeclaredRequests(
    ...
    await Promise.all(
        urls.map(async (url) => {
            try {
                ...
                if (!(error instanceof ConditionalNoteNetworkError)) throw error;
    ...
    return {
        ...
        factory: (runSignal) => {
            const cap: ConditionalNoteCapabilityApi = factory(runSignal);
            return {
...
export function parseCompilerOutput(output: string | null): CompilerResponse {
    if (!output) throw new Error("conditional-note compiler returned no output");
    if (output.length > MAX_COMPILER_OUTPUT_CHARS) {
        throw new Error("conditional-note compiler output exceeds 128 KiB");
    }
    ...
    try {
        ...
        throw new Error("conditional-note compiler returned malformed JSON");
    }
    ...
    if (hasLoneSurrogate(parsed)) {
        throw new Error("conditional-note compiler output contains an unpaired surrogate");
    }
...
export function normalizeManifest(
    manifest: ConditionalNoteCheckManifest,
): ConditionalNoteCheckManifest {
    const capabilities = Array.isArray(manifest.capabilities)
        ? unique(
              manifest.capabilities
                  .slice(0, MAX_MANIFEST_ENTRIES)
                  .filter(
                      (cap): cap is ConditionalNoteCapabilityName =>
                          typeof cap === "string" &&
                          ["readFile", "gitHeadSha", "gitTag", "gitLog", "httpGet"].includes(cap),
                  ),
          )
        : [];
    const normalized: ConditionalNoteCheckManifest = {
        capabilities,
...
export function manifestAdvisoryWarnings(
    code: string,
    manifest: ConditionalNoteCheckManifest,
): string[] {
...
function extractJsonObject(output: string): string {
    ...
    if (start < 0 || end <= start)
        throw new Error("conditional-note compiler returned no JSON object");
    return text.slice(start, end + 1);
}

function capabilityUses(code: string): Set<ConditionalNoteCapabilityName> {
    const uses = new Set<ConditionalNoteCapabilityName>();
    const regex = /\bcap\s*\.\s*(readFile|gitHeadSha|gitTag|gitLog|httpGet)\s*\(/g;
    const codeOnly = maskSourceSpans(code, NON_CODE_SPANS);
    for (const match of codeOnly.matchAll(regex))
        uses.add(match[1] as ConditionalNoteCapabilityName);
    return uses;
}
...
export function normalizeCron(cron: string): string {
    ...
    if (!isValidConditionalNoteCron(normalized)) {
...
/**
 * Accepts the daemon's cron grammar (`parse_cron` in `conditional_note_evaluation.rs`): five numeric
 * fields, each a comma list of `*`, `n`, `lo-hi`, or any of those with `/step`; no names or
 * macros; day-of-week `7` represents Sunday.
 */
export function isValidConditionalNoteCron(expression: string): boolean {
