...
async function temporaryDirectory(prefix = "fs-observer-"): Promise<string> {
...
async function createRepository(): Promise<{ repo: string; firstSha: string }> {
    const repo = await temporaryDirectory("fs-observer-git-");
    await git(repo, "init", "--initial-branch=main");
    await git(repo, "config", "user.name", "FS Observer Test");
    await git(repo, "config", "user.email", "fs-observer@example.invalid");
...
describe("path fence", () => {
    test("refuses every sensitive Eidnara data root", async () => {
        const home = await temporaryDirectory("fs-observer-home-");
