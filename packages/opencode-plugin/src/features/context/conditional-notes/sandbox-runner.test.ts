...
import {
    type ConditionalNoteCapabilityApi,
    createConditionalNoteCapabilities,
} from "./capabilities";
import { runCompiledConditionalNoteCheck } from "./sandbox-runner";
import { ConditionalNoteNetworkError, ConditionalNoteSecurityError } from "./types";

const fakeCap: ConditionalNoteCapabilityApi = {
...
describe("compiled conditional-note QuickJS runner", () => {
    test("runs a check with injected capabilities", async () => {
        const result = await runCompiledConditionalNoteCheck({
    ...
    test("aborts host capabilities on the run timeout and frees the shared lock", async () => {
        await runCompiledConditionalNoteCheck({
            compiledCheck: `function check() { return { met: true }; }`,
            capabilities: fakeCap,
        });
        ...
        const timedOut = (await Promise.race([
            runCompiledConditionalNoteCheck({
                            ...
                            const abort = () =>
                                reject(
                                    new ConditionalNoteNetworkError(
                                        "CONDITIONAL_NOTE_NETWORK: aborted",
                                    ),
                                );
                            if (signal.aborted) {
                                abort();
        ...
        ])) as Awaited<ReturnType<typeof runCompiledConditionalNoteCheck>>;
    ...
    test("a guest that catches the aborted host call cannot report success after the timeout", async () => {
        ...
        const timedOut = await runCompiledConditionalNoteCheck({
        ...
        const cancelled = await runCompiledConditionalNoteCheck({
            compiledCheck: swallowing,
            capabilities: { ...fakeCap, httpGet: () => new Promise(() => {}) },
            signal: controller.signal,
            timeoutMs: 1_000,
    ...
    test("a guest that catches a host refusal cannot report success without the input", async () => {
        ...
        const network = await runCompiledConditionalNoteCheck({
            compiledCheck: swallowing,
            capabilities: {
                ...fakeCap,
                httpGet: async () => {
                    throw new ConditionalNoteNetworkError(
                        "CONDITIONAL_NOTE_NETWORK: transient HTTP 503",
                    );
                },
            },
        });
        expect(network).toEqual({
            ok: false,
            cancelled: false,
            error: "ConditionalNoteNetworkError: CONDITIONAL_NOTE_NETWORK: transient HTTP 503",
            network: true,
        });

        const security = await runCompiledConditionalNoteCheck({
            compiledCheck: swallowing,
            capabilities: {
                ...fakeCap,
                httpGet: async () => {
        ...
        expect(security).toEqual({
            ok: false,
            cancelled: false,
            error: "ConditionalNoteSecurityError: URL resolves to a non-global/internal address",
            network: false,
        });
    });

    test("guest-thrown errors cannot forge the network classification", async () => {
        const forged = await runCompiledConditionalNoteCheck({
            compiledCheck: `function check() { throw new Error("CONDITIONAL_NOTE_NETWORK: fabricated"); }`,
            capabilities: fakeCap,
        });
        expect(forged).toEqual({
            ok: false,
            cancelled: false,
            error: "Error: CONDITIONAL_NOTE_NETWORK: fabricated",
            network: false,
        });

        const renamed = await runCompiledConditionalNoteCheck({
            compiledCheck: `function check() { const e = new Error("x"); e.name = "ConditionalNoteNetworkError"; throw e; }`,
            capabilities: fakeCap,
        });
        expect(renamed.ok).toBe(false);
        if (!renamed.ok) expect(renamed.network).toBe(false);

        const cloned = await runCompiledConditionalNoteCheck({
        ...
        const genuine = await runCompiledConditionalNoteCheck({
    ...
    test("cancelling a queued check settles it before the active run releases the lock", async () => {
        const holdMs = 600;
        const active = runCompiledConditionalNoteCheck({
            compiledCheck: `function check(cap) { cap.httpGet("https://example.test/"); return { met: false }; }`,
            capabilityFactory: (signal) => ({
                ...fakeCap,
                httpGet: () =>
        ...
        const controller = new AbortController();
        const startedAt = Date.now();
        const queued = runCompiledConditionalNoteCheck({
            compiledCheck: `function check() { return { met: true }; }`,
            capabilities: fakeCap,
            signal: controller.signal,
        });
        ...
        const followup = await runCompiledConditionalNoteCheck({
            compiledCheck: `function check() { return { met: true }; }`,
            capabilities: fakeCap,
        });
        expect(followup).toEqual({ ok: true, result: { met: true } });
    ...
    test("external cancellation of a directly supplied capability reports cancelled", async () => {
        ...
        expect(result).toEqual({
            ok: false,
            cancelled: true,
            error: "CONDITIONAL_NOTE_NETWORK: aborted",
            network: false,
        });
    });
    ...
    test("rejects a project FIFO without wedging the shared sandbox lock", async () => {
        const dir = await mkdtemp(path.join(tmpdir(), "eidnara-conditional-note-fifo-"));
        try {
            ...
            const result = await runCompiledConditionalNoteCheck({
                ...
                capabilityFactory: (signal) => ({
                    ...createConditionalNoteCapabilities({ projectRoot: dir, signal }),
                    httpGet: () =>
                        new Promise((_resolve, reject) => {
                            const abort = () =>
                                reject(
    ...
    test("serializes concurrent checks whose host calls suspend (shared-module asyncify safety)", async () => {
        ...
        const slowCap: ConditionalNoteCapabilityApi = {
            ...fakeCap,
            readFile: async (path) => {
        ...
        const results = await Promise.all(
            Array.from({ length: 8 }, () =>
                runCompiledConditionalNoteCheck({ compiledCheck: check, capabilities: slowCap }),
            ),
        );
