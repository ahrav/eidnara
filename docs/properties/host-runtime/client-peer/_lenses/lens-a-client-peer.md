...
Attention focus: what `crates/host-runtime/src/client.rs` assumes about the host, what
...
4. The doc comment "Thread-confined peer endpoint for integration tests" at
   ...
   `"host-test-ring-v1"` (`ring_transport.rs:31`) is likewise a misleading
   name, not a gate.
...
`docs/host-wire-protocol.md:736-743` exactly.
...
per generation (`docs/host-wire-protocol.md:656`) and a new connection is a
new generation.

`docs/host-wire-protocol.md:12` diagrams a client `Recovering` state with
...
`docs/host-wire-protocol.md:656` obliges of a sender.
...
`docs/host-wire-protocol.md:654` says "before another request, sender MUST
...
`close` (`:684`). `docs/host-wire-protocol.md:658` requires finite limits for
...
residue. `docs/host-wire-protocol.md:269` lists what is role-invalid, and
...
length. `docs/host-wire-protocol.md:746` says the opposite: "Data and
...
leaves `retired == false`, contrary to `docs/host-wire-protocol.md:654`. It is
...
Five disagreements against `docs/host-wire-protocol.md`. Per METHOD rule 3
...
`docs/host-wire-protocol.md:62` and the `SendOutcome::NotSent` doc at
...
6. Does any consumer of `host_shutdown`'s `Ok` use it as authority to start a
   ...
   (`crates/daemon/src/bin/eidnara-host.rs`) is outside 2d. (unresolved, needs
   a daemon pass)
...
8. Is `client.rs`'s inline test module executed anywhere? The rescope document
   ...
   never runs an unfiltered `-p host-runtime` or `--lib` invocation. If that holds,
