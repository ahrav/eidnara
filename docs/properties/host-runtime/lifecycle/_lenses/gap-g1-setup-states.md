...
System `the `host` source checkout at `1c193ae0` (merge of
`d90e7811`). Every line reference below was read at that commit.

Scope: the setup state machine in `crates/host-runtime/src/connection.rs`, its
normative source `docs/host-wire-protocol.md` section 7.7 (lines 558-672), and
...
state diagram names eight nodes (`docs/host-wire-protocol.md:653-670`):
...
`docs/host-wire-protocol.md:580` requires malformed negotiation content to
...
- `docs/host-wire-protocol.md:562`: "A first application request, routed
...
`docs/host-wire-protocol.md:621`, exact `unavailable` is the only selection
...
re-upgrade probe that `docs/host-wire-protocol.md:621` reserves for
