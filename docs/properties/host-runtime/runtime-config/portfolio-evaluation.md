...
`the `host` source checkout, `HEAD` = `e447c927` ("refactor(shm):
...
both before `host_runtime::run` at `:632`.
...
- **Any fixture that builds a slow-but-yielding lifecycle callback.** It is the
  ...
  in the sub-part.
- **Any workflow change that runs the `host-runtime` lib target or adds one doctest to a
