...
| --- | --- | --- |
| `ci.yml:122-123` | `shm-crash-recovery`, "Crash, recovery, and soak-smoke suites", ubuntu-latest | `cargo nextest run -p host-runtime --lib --test shm_failure_modes --test shm_soak` |
| `ci.yml:167-168` | `shm-source-build`, "Rust contracts and lifecycle (Linux)", `if: runner.os == 'Linux'` | `cargo nextest run -p host-runtime --test shm_transport --test transport_negotiation` |
| `ci.yml:178` | `shm-source-build`, "Contracts, observer self-tests, and omission proof (macOS)", `if: runner.os == 'macOS'` | `cargo nextest run -p host-runtime --test shm_soak` |
| `ci.yml:179-180` | same macOS step | `cargo test -p host-runtime --lib shm_provider::tests::platform_preflight_is_side_effect_free` |
| `ci.yml:183` | `shm-source-build`, "Rust lease non-escape" | `cargo test -p host-runtime --doc` |
...
`ci.yml:156` is `cargo build -p shm-transport -p host-runtime -p shm-native`,
which compiles the library but no test target, and `ci.yml:157` is
`cargo build -p daemon --bin eidnara-host`, a different crate.

The last row names no binary, but `.config/nextest.toml:18-19` gives profile
`shm-soak` a `default-filter` of
`package(host-runtime) and binary(shm_soak) and test(full_soak_cycles_conserve_resources)`,
...
`ci.yml:161`, `cargo test -p daemon --test lifecycle_cli`, under the step
"Native lifecycle binary contract". Different crate, different target name.
`host-runtime`'s `tests/lifecycle.rs` is named nowhere.
...
`git diff d90e7811..HEAD -- crates/host-runtime .github/workflows scripts` is empty, so
...
- Sources examined: all five workflow files, `.config/nextest.toml`,
  `scripts/test-rust.sh`, `package.json` scripts, `crates/host-runtime/Cargo.toml`,
