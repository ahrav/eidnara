...
The direct component exposes no `thalamus` resolver route. A facade request without an explicit or route-bound session returns the existing typed `session_unresolved` result locally and opens no resolver transport route. Bound OpenCode sessions retain their proven direct path, and a session with transform lineage under another root is refused on this one. A bound session with no transform lineage at all (harness `pi`, which has no transform, or OpenCode in TypeScript transform mode, which never sends one) is the conversation directly: nothing contradicts the route-bound identity, and the plugins bind the harness's own session id.
