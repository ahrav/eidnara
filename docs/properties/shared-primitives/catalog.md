...
Existing check: `distinct_identity_axes_do_not_conflict` covers distinct scope, module, and backend axes; `separator_in_a_key_field_fails_closed_instead_of_aliasing` (`crates/lease/src/lib.rs:1404-1424`) covers the separator in each field; status **unaudited**.
...
Existing check: `superseded_writer_is_fenced_out_after_handover`, `crates/storage/src/lib.rs:2140-2178`, and `superseded_writer_is_rejected_after_reopen`, `commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:1144-1175`; both use synthetic stale stores and remain **unaudited**.
...
Confidence: high - [evidence](evidence/logical-store-has-single-lease-identity.md). High on derivation facts (`crates/storage/src/lib.rs:89-101,588-645`); unknown whether descriptor authority prevents these combinations in deployment.
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:1326-1335`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:1337-1350`) proves sibling files in one directory do not contend; `symlinked_database_paths_contend_or_are_refused_never_aliased` (`:912-965`) proves a directory-symlink alias contends and a file-symlink alias is refused. No test covers descriptor disagreement or hardlink aliases; status **unaudited**.
...
Confidence: high - [evidence](evidence/replacement-fence-is-claimed-before-old-writer-writes.md). High that every returned store has claimed a strictly greater epoch (`crates/storage/src/lib.rs:588-645,728-743`); the stronger acquisition-instant guarantee remains unproved.
Existing check: `open_claims_fence_before_return`, `crates/storage/src/lib.rs:1192-1204`, observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:1211-1247`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced migrations, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1717-1737`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:1740-1804`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:1807-1860`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:1863-1919`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:647-689`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`:802-861`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`:864-908`) and `a_regressed_positive_epoch_fails_closed` (`:911-953`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`:956-1000`), `a_suppressed_epoch_increment_is_rejected` (`:1003-1020`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`:1023-1047`) protect the lease row and its issuance; and `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:1922-2030`) refuses DML, triggers, indexes, views, and renames onto the fence table. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/fence-tables-keep-their-durable-identity.md). `ensure_fence_table` (`crates/storage/src/lib.rs:697-704`) and `run_migrations` (`crates/storage/src/lib.rs:807-815`) emit the DDL; `fence_and_version_tables_keep_their_ddl` (`crates/storage/src/lib.rs:2263`) compares the recorded SQL text and the fence row. Both names are `frozen-durable` in `migration/registry.json`.
...
Confidence: high - [evidence](evidence/migrations-apply-once-per-namespace.md). `run_migrations` (`crates/storage/src/lib.rs:796-892`) reads `MAX(version)` per namespace, sorts, skips `<= current`, and commits SQL and record together; `migrations_seed_once_across_reopen` (`crates/storage/src/lib.rs:1250`), `later_migration_applies_on_top_of_earlier` (`crates/storage/src/lib.rs:1576`), and `independent_namespace_chains_in_one_database` (`crates/storage/src/lib.rs:1605-1636`) cover the three clauses.
...
Confidence: high - [evidence](evidence/read-callbacks-cannot-write.md). `CallbackScope::read_only` (`crates/storage/src/lib.rs:387-391`) sets `query_only` and installs `deny_scope_escapes` (`crates/storage/src/lib.rs:475-493`); `GuardedConn` (`crates/storage/src/lib.rs:256-368`) omits the escapes; `unfenced_connection_rejects_writes` (`crates/storage/src/lib.rs:1680`), `a_read_callback_cannot_clear_the_read_only_guard` (`crates/storage/src/lib.rs:1807`), and `maintenance_runs_through_the_unfenced_path` (`crates/storage/src/lib.rs:2033`) pin the three denials.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:462-467`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1717`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:548-560`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:1706`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:1740`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/store-files-are-owner-only-after-open.md). `open_sqlite` (`crates/storage/src/lib.rs:610-616`) creates the database owner-only and calls `protect_file` on all three paths before WAL setup; `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:1058`) asserts all three modes on reopen and `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`crates/storage/src/lib.rs:1478-1509`) asserts them on a fresh open.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:213-235`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:2049`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:1662`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:785-787`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:746-752`) use checked conversions; `legacy_negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:2110`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:2237`) cover both directions.
