...
//! `EIDNARA_MODEL_EXECUTION_FIXTURE_MODE` routes `main` to fixture behavior.
...
use host_runtime::model_execution::opencode::{
    EIDNARA_MODEL_EXECUTION_CHILD_ENV, OPENCODE_MODEL_EXECUTION_AGENT, OpenCodeBackend,
    OpenCodeRuntime,
};
use host_runtime::model_execution::pi::{
    EIDNARA_PI_SUBAGENT_ENV, MODEL_EXECUTION_MAX_OUTPUT_TOKENS_ENV,
    MODEL_EXECUTION_TEMPERATURE_ENV, PI_MODEL_EXECUTION_EXTENSION_BYTES,
...
use host_runtime::model_execution::supervisor::{SessionKey, Supervisor};
use sha2::{Digest, Sha256};

const FIXTURE_MODE_ENV: &str = "EIDNARA_MODEL_EXECUTION_FIXTURE_MODE";
const OUT_ENV: &str = "EIDNARA_FIXTURE_OUT";
const TRANSCRIPT_FILE_ENV: &str = "EIDNARA_FIXTURE_TRANSCRIPT_FILE";
const STDERR_ENV: &str = "EIDNARA_FIXTURE_STDERR";
const EXIT_ENV: &str = "EIDNARA_FIXTURE_EXIT";
const BEHAVIOR_ENV: &str = "EIDNARA_FIXTURE_BEHAVIOR";
const RETRY_PAUSE_MS_ENV: &str = "EIDNARA_FIXTURE_RETRY_PAUSE_MS";
...
fn state_root() -> StateRoot {
    ...
    StateRoot::resolve(Some(data_dir)).expect("model_execution state root resolves")
}

fn state_root_data_dir() -> PathBuf {
    let root = state_root();
    // `<data_dir>/eidnara/model_execution` -> `<data_dir>`.
    root.path()
        .parent()
...
fn main() {
    ...
    if std::env::var_os(EIDNARA_MODEL_EXECUTION_CHILD_ENV).is_some()
        || std::env::var_os(EIDNARA_PI_SUBAGENT_ENV).is_some()
    {
    ...
    let tests: &[(&str, fn())] = &[
        ...
        (
            "pi_model_execution_hook_owns_generation_controls",
            pi_model_execution_hook_owns_generation_controls,
        ),
        (
...
fn install_fixture_controls() {
    ...
    for credential_name in ["ANTHROPIC_API_KEY", "GEMINI_API_KEY", "OPENAI_API_KEY"] {
        ...
        // SAFETY: this runs in the re-executed fixture child on its main thread before
        // any other thread exists, so no concurrent reader of the environment can
...
mod fixture {
    ...
    fn record_group() {
        ...
        let nonce = std::env::var(GROUP_NONCE_FIXTURE_ENV).unwrap_or_else(|_| {
            host_runtime::model_execution::subprocess::group_registry::new_group_nonce()
                .expect("fixture group nonce")
        });
        let record =
            host_runtime::model_execution::subprocess::group_registry::GroupRecord::record(
                &state_root(),
                leader,
                &nonce,
            )
...
) -> Arc<host_runtime::harness_closure::ValidatedHarnessClosure> {
    static OPENCODE: OnceLock<Arc<host_runtime::harness_closure::ValidatedHarnessClosure>> =
        OnceLock::new();
    static PI: OnceLock<Arc<host_runtime::harness_closure::ValidatedHarnessClosure>> =
        OnceLock::new();
...
fn failed(terminal: &BackendTerminal) -> &host_runtime::model_execution::backend::BackendError {
    match terminal {
...
fn opencode_argv_env_stdin_contract() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "anthropic/claude-test",
            "--agent",
            OPENCODE_MODEL_EXECUTION_AGENT,
            "--format",
            "json",
        ]
    );
    ...
    assert!(
        private_dir
            ...
            .and_then(|n| n.to_str())
            .expect("dir name")
            .starts_with("model_execution-opencode-"),
    ...
    assert_eq!(
        env.get(EIDNARA_MODEL_EXECUTION_CHILD_ENV)
            .map(String::as_str),
        Some("1")
    );
    assert_eq!(
    ...
    assert!(!env.contains_key("EIDNARA_MODULE_ID"));
    assert!(!env.contains_key("EIDNARA_LAUNCH_NONCE"));

    let config: serde_json::Value =
        serde_json::from_str(env.get("OPENCODE_CONFIG_CONTENT").expect("inline config"))
            .expect("config parses");
    let agent = &config["agent"][OPENCODE_MODEL_EXECUTION_AGENT];
...
fn opencode_hostile_project_untouched() {
    ...
    let config: serde_json::Value =
        serde_json::from_str(env.get("OPENCODE_CONFIG_CONTENT").expect("config"))
            .expect("config parses");
    assert!(
        config["agent"][OPENCODE_MODEL_EXECUTION_AGENT]
            .get("prompt")
            .is_none()
    );
}
...
fn pi_argv_privacy_contract() {
    ...
    assert!(extensions[2].ends_with(PI_MODEL_EXECUTION_EXTENSION_FILE));
    ...
    assert_eq!(
        ...
        PI_MODEL_EXECUTION_EXTENSION_BYTES
    );
    ...
    let env = setup.env();
    assert_eq!(
        env.get(EIDNARA_PI_SUBAGENT_ENV).map(String::as_str),
        Some("1")
    );
    assert_eq!(
        env.get(MODEL_EXECUTION_MAX_OUTPUT_TOKENS_ENV)
            .map(String::as_str),
        Some("32000")
    );
    assert_eq!(
        env.get(MODEL_EXECUTION_TEMPERATURE_ENV).map(String::as_str),
        Some("0.25")
    );
    assert_eq!(
...
fn pi_project_pi_resources_ignored() {
    ...
    assert!(extensions[0].ends_with(PI_MODEL_EXECUTION_EXTENSION_FILE));
    assert!(
        !args
            .iter()
    ...
    assert_eq!(
        setup.env().get(EIDNARA_PI_SUBAGENT_ENV).map(String::as_str),
        Some("1")
    );
}
...
fn pi_model_execution_hook_owns_generation_controls() {
...
fn opencode_oversized_inline_config_rejected_before_spawn() {
    ...
    let oversized_system =
        "s".repeat(host_runtime::model_execution::config::MAX_OPENCODE_CONFIG_BYTES + 1);
    let request = request(
        setup.project.path(),
        Harness::OpenCode,
        "anthropic/claude-test",
...
fn model_execution_send_request() -> (SendRequest, Vec<u8>) {
    (
        SendRequest {
            prompt: PROMPT_SENTINEL.to_owned(),
            system: None,
...
fn supervisor_delete_reaps_group() {
    ...
    runtime.block_on(async {
        let (send, body) = model_execution_send_request();
...
fn pi_extension_budget_rejects_oversized_descriptors() {
    ...
    let over_budget = host_runtime::model_execution::config::MAX_PI_PROVIDER_EXTENSIONS + 1;
...
fn private_dir_contract() {
    let dir = PrivateDir::create(&state_root(), "model_execution-test").expect("create");
...
fn env_snapshot_admission_charges_per_entry_overhead() {
    ...
    let string_bytes: usize = flood
        ...
        .sum();
    assert!(
        string_bytes < host_runtime::model_execution::config::MAX_ENV_SNAPSHOT_BYTES,
...
fn oversized_json_structure_rejected_without_capping_prose() {
    ...
    assert!(
        events.iter().any(
            |event| matches!(event, BackendEvent::AssistantText { text, .. } if text == &prose)
        )
    );
}
...
fn crash_orphaned_run_dirs_swept_only_for_dead_owners() {
    use host_runtime::model_execution::subprocess::group_registry::sweep_orphaned_run_dirs;

    // The sweep must retain a directory owned by the running process.
    let mine = PrivateDir::create(&state_root(), "model_execution-test-live").expect("create live");
    let mine_path = mine.path().to_path_buf();

    let root = state_root().run_root().expect("run root");
    let orphan = root.join("model_execution-test-orphan-ffffffffffffffff-2-1-000000000000dead");
        ...
        fs::read_dir(&run_root)
            ...
            .filter(|entry| {
                entry
                    .file_name()
                    .to_string_lossy()
                    .starts_with("model_execution-pi-")
            })
            .count()
    };
...
fn group_registry_sweep_kills_only_dead_owner_groups() {
    ...
    use host_runtime::model_execution::subprocess::group_registry::{
...
fn group_record_mode_forced_under_umask() {
    use host_runtime::model_execution::subprocess::group_registry::{GroupRecord, new_group_nonce};
...
fn sweep_spares_young_record_temps() {
    use host_runtime::model_execution::subprocess::group_registry::sweep_orphaned_groups;
