...
- **Existing evidence:** `run_started_keeps_lineage_resets_episode` (`crates/cache-stability/src/lib.rs:692-714`) mixes one unit of each class and asserts the survivor list and its bytes; `cross_episode_lineage_reproduces_byte_identical` (`crates/cache-stability/tests/golden_vectors.rs:274-322`) drives a lineage vector across a run boundary.
