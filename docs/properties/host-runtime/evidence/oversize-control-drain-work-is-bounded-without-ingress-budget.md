...
- Sources examined: `grep -rn 'record_copy\|copies()' crates/host-runtime/src/`,
...
- Findings: no. `record_copy` has two producers and neither is on the drain
  ...
  path.
- Missing evidence: metrics outside `host-runtime` were not surveyed. A host-level
  byte counter in another crate could see the socket traffic even if this crate
  cannot attribute it.
- Conclusion: resolved within this crate. No signal in `host-runtime` observes drained
