...
fn target() -> host_runtime::RouteTarget {
...
fn identity(session: &str) -> host_runtime::RouteIdentity {
    host_runtime::RouteIdentity {
        project_root: PathBuf::from("/tmp/eidnara-host-ipc-budget"),
...
fn classify(result: &CallResult) -> Outcome {
    match result {
        ...
        Err(error) => match error.outcome() {
            host_runtime::SendOutcome::NotSent => Outcome::WriteFailure,
            host_runtime::SendOutcome::OutcomeUnknown => Outcome::PeerClosed,
            host_runtime::SendOutcome::Terminal => Outcome::ProtocolError,
        },
    }
}
