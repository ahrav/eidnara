...
pub const LIFECYCLE_RECORD_NAME: &str = "host-lifecycle.json";
...
pub const COORDINATION_DIR_NAME: &str = ".host-coordination";
...
/// Keeping coordination outside `eidnara` prevents replacement of that subtree from splitting the locks.
...
pub fn lifecycle_dir_path(data_dir_override: Option<&Path>) -> Result<PathBuf, InstanceError> {
    ...
    let base = run
        .parent()
        .expect("runtime dir always has a eidnara parent")
        .to_path_buf();
...
/// `lifetime.lock` is never renamed and resides outside `eidnara`.
/// Replacing `eidnara` cannot release `lifetime.lock`.
...
/// Renaming or removing `.host-coordination` splits the lifetime fence.
...
/// Supported deployments must not rename `.host-coordination` while a daemon is live.
/// Renaming `.host-coordination` under a live daemon makes the probe report free.
/// descriptor drops.
...
impl InstanceGuard {
    ...
    /// Records the `Stopping` phase and withdraws the connection-file publication, so a
    /// client arriving during shutdown finds no daemon rather than a dying one. A failed
    /// record write does not block the withdrawal.
...
/// The transaction lock is an exclusive cross-process `flock` on `${dataDir}/.host-coordination/transaction.lock`.
/// The transaction lock is never renamed.
///
/// The lock file serializes mutators but does not anchor runtime evidence.
/// Runtime evidence lives in the managed `eidnara/run` directory.
/// Supported code never renames or unlinks `transaction.lock`.
/// Replacing `lifecycle`, `run`, or `eidnara` cannot mint a second transaction owner.
...
/// A release that predates `transaction.lock` serializes transactions on the `${dataDir}/eidnara/lifecycle` directory inode instead.
...
/// `NamespaceAnchor` retains `eidnara` and its existing `lifecycle` and `run` children.
...
mod tests {
    ...
    fn namespace_drift_fails_the_holder_before_a_named_commit() {
        for victim in ["lifecycle", "run", ""] {
            ...
            std::fs::create_dir(eidnara.join("run")).expect("run");
            for dir in [
                eidnara.clone(),
    ...
    /// Replacing the whole `eidnara` subtree after publication isolates old descriptor-owned evidence, but the stable lifetime fence still names a live incarnation.
    #[test]
    fn a_replaced_eidnara_subtree_is_not_reported_stopped_while_the_daemon_lives() {
        let root = temp_root();
        let mut guard = acquire(root.path());
        guard
            .write_lifecycle_record(LifecyclePhase::Running)
        ...
        let eidnara = root.path().join("eidnara");
        std::fs::rename(&eidnara, root.path().join("eidnara-old"))
