...
`packages/shm-native/tests/mechanism.ts:147` is `16_384n`. It is **correct for
...
file declares `GRANT_DESCRIPTOR_DEPTH = 32n` (`:129`) and
`GRANT_MAX_LEASES = 32n` (`:132`). This also confirms the `daf6e244` story
...
- `packages/shm-native/tests/mechanism.ts:129-147` — `GRANT_DESCRIPTOR_DEPTH =
  32n`, `GRANT_MAX_LEASES = 32n`, `GRANT_LAYOUT_OVERHEAD_BYTES = 16_384n`,
  assembled at `:161-168`. Its comment at `:128` states the intent explicitly:
...
- Sources examined: `packages/shm-native/tests/mechanism.ts:110-173` for the
  fixture and its `loadRawAddon` path; `packages/shm-native/src/lib.rs:931-995`
...
- Findings: the fixture is deliberate. `create_test_pair` calls
  ...
  (`lib.rs:933`) where `PROFILE` is the same string the host uses, so the
...
- Findings:
  ...
  - line 64, `packages/shm-native/tests/mechanism.ts:112` now `packages/shm-native/tests/mechanism.ts:147`: At HEAD the constant is 8_192n, which is the depth-8 overhead and agrees with the depth-8 geometry the same file now declares, so it is no longer wrong for depth 8.
  - line 67, `:94` now `:129`: It is 8n at HEAD, not 32n.
  - line 68, `:97` now `:132`: It is 8n at HEAD, not 32n.
  ...
  - line 94, `packages/shm-native/tests/mechanism.ts:94-112` now `packages/shm-native/tests/mechanism.ts:129-147`: The fixture declares depth 8, eight leases, and an 8,192-byte overhead at HEAD, so it belongs to the depth-8 family and no longer contradicts the host.
  - line 96, `:93` now `:128`: The comment names host_test_ring_profile at HEAD, not ring_profile.
  ...
  - line 151, `lib.rs:675-678` now `lib.rs:933`: At HEAD `create_test_pair` calls `host_test_ring_profile()`, so the addon creates depth-8 rings under that name and no longer passes PROFILE into a hardware-profile slot.
