...
Section 2 is regenerated from `createToolRegistry` in `packages/opencode-plugin`; sections 1 and 3 record the guidance and hash baseline the daemon now renders.
...
### ctx_reduce — description ~318 tokens, params ~43 tokens (total ~361)
...
### ctx_note — description ~394 tokens, params ~411 tokens (total ~805)
...
### ctx_memory — description ~167 tokens, params ~728 tokens (total ~895)
...
### ctx_search — description ~127 tokens, params ~162 tokens (total ~289)
