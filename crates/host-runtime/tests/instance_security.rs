...
const UMASK_CHILD_ENV: &str = "EIDNARA_HOST_RESTRICTIVE_UMASK_CHILD";
...
async fn restrictive_umask_subprocess_child() {
    ...
    host_runtime::read_connection_file(host.publication_path())
        .expect("publication stays readable");
...
async fn discovery_validates_the_publication_the_way_a_client_must() {
    ...
    assert_eq!(info.daemon_ver, "eidnara-host/test");
    ...
    std::fs::write(
        &oversized,
        vec![b' '; host_runtime::MAX_CONNECTION_FILE_LEN + 1],
    )
    ...
    assert!(host_runtime::read_connection_file(&oversized).is_err());
...
async fn discovery_requires_numeric_wire_version_two() {
    ...
    for (name, value) in [
        ...
        assert!(
            host_runtime::read_connection_file(&path).is_err(),
...
async fn discovery_rejects_symlink_and_hard_link_publications() {
    ...
    assert!(host_runtime::read_connection_file(&symlink).is_err());
...
async fn a_planted_symlink_at_the_record_name_is_replaced_not_followed() {
    ...
    let data_root = tempfile::tempdir().expect("temp root");
    let run_dir = data_root.path().join("eidnara").join("run");
    std::fs::create_dir_all(&run_dir).expect("create runtime dir");
    std::os::unix::fs::symlink(&victim, run_dir.join(host_runtime::LIFECYCLE_RECORD_NAME))
    ...
    let meta = std::fs::symlink_metadata(run_dir.join(host_runtime::LIFECYCLE_RECORD_NAME))
        .expect("stat record");
...
async fn coordination_locks_are_owner_only_and_survive_teardown() {
    ...
    let coordination = data_root.path().join(host_runtime::COORDINATION_DIR_NAME);
...
async fn startup_refuses_to_overwrite_an_unknown_lifecycle_schema() {
    ...
    assert!(
        matches!(
            refused,
            Err(host_runtime::HostError::Instance(
                host_runtime::InstanceError::UnsupportedStateSchema { .. }
            ))
        ),
