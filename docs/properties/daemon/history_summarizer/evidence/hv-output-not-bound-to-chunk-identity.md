...
Task focus: treat the history_summarizer's producer as a hostile peer. The first question
...
Reading `validate_history_summarizer_output` (`crates/daemon/src/history_summarizer_validate.rs:450-641`)
...
publish request (`history_summarizer.rs:1726-1727`); `validate_history_summarizer_output` never
receives them. Its signature (`:450-455`) takes `text`, `chunk`,
`prior_history_segments`, `options`, and `HistorySummarizerChunk` (`:47-65`) has no text
field at all.
...
rg -ni "nonce|echo_back|session_marker" \
  crates/daemon/src/history_summarizer_prompt.rs \
  crates/daemon/src/history_summarizer_validate.rs
# no matches
```

The prompt does not ask the model to echo anything verifiable.
`history_summarizer_prompt.rs:21` carries `HISTORY_SUMMARIZER_TRANSCRIPT_GUARD`, which tells the
...
`request.observed_chunk_fingerprint` (`history_summarizer.rs:449-461`) and abandons on
mismatch. `chunk_fingerprint` comes from durable firing state via
`publish_predicate` (`history_summarizer.rs:373-388`). So the fingerprint proves *the
...
(`history_summarizer.rs:299-307`) takes the output as `_output_text` and drops it,
...
history_segments about an unrelated frontend refactor.

The document is well formed:

```
<output><history_segments>
<history_segment start="40" end="55" title="Sidebar layout rework" ...><p1>...</p1>...</history_segment>
<history_segment start="56" end="72" title="Icon set migration" ...><p1>...</p1>...</history_segment>
<history_segment start="73" end="89" title="Storybook cleanup" ...><p1>...</p1>...</history_segment>
</history_segments><meta><unprocessed_from>90</unprocessed_from></meta></output>
```
...
- Check 7 `:487-491`: three history_segments. Pass.
...
at most `BOUNDARY_HEALING_SLACK`, so the last history_segment IS popped at `:555` and
`unprocessed_from` becomes 73. That does not save the run: the first two
history_segments still publish with the wrong content.
...
`raw_chunk_messages` (`history_summarizer.rs:435-436`, `:1727`), so the bytes are
...
`history_summarizer.rs:1001-1003` notes that the firing sequence is part of the run id
...
`history_summarizer.rs:1440-1450` also means a single firing can accept output from the
second or third model in the chain, widening the set of producers whose output
must be trusted.

## What a test must construct

A unit test over `validate_history_summarizer_output` directly, which needs no store and
no async runtime:

1. Build a chunk with `chunk(40, 89)` shaped like the helper at `:1350-1366`.
2. Build a document whose history_segment ranges partition 40..=89 contiguously,
...
mark that a firing published a history_segment set whose accepted fields contain no
...
- Sources examined: `history_summarizer_validate.rs:1384-1414` (the golden test and its
  divergence carve-out at `:1391-1399`), `testdata/validate-golden.json`
  (16 cases), `history_summarizer_prompt.rs:72-77` (the vendored system prompt kept
  "byte-identical so both implementations drive the model with the same role
  guidance"), `history_summarizer_prompt.rs:1-4`.
...
- Sources examined: `history_summarizer.rs:444-540` (`publish_validated_chunk`),
  `history_summarizer.rs:38-67` (`to_stored_history_segment`), `decay_render.rs:1-300`,
  `history_summarizer_prompt.rs:218-278` (the session-references block that feeds prior
  history_segments back into the next run).
- Findings: `to_stored_history_segment` copies title, content, and tiers verbatim.
  ...
  history_segments, so a substituted summary becomes the calibration context for the
...
- Conclusion: resolved with answer — no. The substitution is undetected at every
  stage after the gate, and the prior-history_segment feedback loop propagates it.
