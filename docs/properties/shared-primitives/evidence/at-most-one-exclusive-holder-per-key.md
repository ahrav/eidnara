...
- **Primary evidence:** headline contract at `crates/lease/src/lib.rs:2-6`; `FileLeaseStore::lease_path` derives one per-key path and `FileLeaseStore::acquire` calls `File::try_lock`, mapping `TryLockError::WouldBlock` to `Held` and other lock errors to `Io`. PostgreSQL derives one advisory key in `advisory_key` and, in `open_postgres`, treats a false `pg_try_advisory_lock` result as `Held` (the shared-crate source repository at `89abb40`).
...
- **Open-question log:** searched the target crate, both store backends, README, CI, and the pinned external inventory. The external credentials-store blocker is recorded in the [durable consumer inventory](../durable-consumer-inventory.md).
