...
mod tests {
    ...
    fn io_and_json_errors_remain_sources() {
        ...
        assert_eq!(
            ...
            Some("sentinel io".to_owned())
        );

        let json_source = serde_json::from_str::<ConnectionInfo>("{").unwrap_err();
        let json = ConnectionFileError::Json {
            path: PathBuf::from("connection"),
            source: json_source,
        };
    ...
    fn a_fifo_is_rejected_rather_than_blocking_the_open() {
        ...
        let dir = std::env::temp_dir().join(format!("fifo-{}", std::process::id()));
