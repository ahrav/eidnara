...
- **Primary evidence:** `step_soft` (`crates/cache-stability/src/lib.rs:219-252`) advances `boundary_id` inside a let chain guarded by `boundary_match && !self.reconcile_pending`; `reconcile_pending` is not assigned in this arm.
- **Existing evidence:** `soft_does_not_advance_anchor_while_reconcile_pending` (`crates/cache-stability/src/lib.rs:597-634`) reverts, sends a coverage-extending `Soft`, and asserts the anchor holds and the next absent defer still reconciles; `coverage_extending_soft_advances_anchor_keeps_m0_frozen` (`crates/cache-stability/src/lib.rs:527-595`) asserts the positive case and that m0 bytes stay frozen.
