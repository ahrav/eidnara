import { HostClient, type HostClientOptions } from "./client";

interface ProcessClientEntry {
    promise: Promise<HostClient>;
    client?: HostClient;
}
...
function ownerKey(options: HostClientOptions): string {
...
export function processHostClient(options: HostClientOptions): Promise<HostClient> {
    ...
    const created = HostClient.connect(options);
...
export function resetProcessHostClientsForTest(): void {
