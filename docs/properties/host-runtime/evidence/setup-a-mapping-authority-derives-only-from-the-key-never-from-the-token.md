...
`docs/host-wire-protocol.md:563` lists "token mismatch" among the conditions
...
The consequence is already constructed in the test tree. `crates/host-runtime/tests/
shm_failure_modes.rs:44-58`, the `setup` role:

```
let mut stream = tokio::net::UnixStream::connect(&info.setup_socket)...
host_runtime::authenticate_client(&mut stream, &info, BUDGET)...
let (_grant, _descriptors) = host_runtime::setup_socket::receive_grant(&mut stream, deadline)...
...
`receive_grant` is public API: `crates/host-runtime/src/lib.rs:35` declares
`pub mod setup_socket`, and the test reaches it as
`host_runtime::setup_socket::receive_grant`.
...
- Sources examined: `docs/shm-transport.md:38-49`,
  `docs/host-wire-protocol.md:559-564`.
- Findings: `host-shm-transport.md:40-45` numbers the phases explicitly:
