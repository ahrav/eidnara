#!/usr/bin/env python3
"""Generates the committed local_embeddings-tiny test bundle into local_embeddings-tiny/ next to this script.
...
OUT_DIR = Path(__file__).resolve().parent / "local_embeddings-tiny"
...
def build_model() -> onnx.ModelProto:
    ...
    model = helper.make_model(
        ...
        producer_name="local_embeddings-tiny-generator",
    )
    model.ir_version = 8
...
def expected_vector(
    text: str, pooling: str = "mean", output: str = "last_hidden_state"
):
    ids = tokenize(text)
    if not ids:
...
def canonical_fingerprint(
    ...
    Mirrors `canonical_fingerprint` in crates/host-runtime/src/local_embeddings/bundle.rs,
    ...
    lines = [
        "eidnara-local-embeddings-fingerprint-v1",
...
def main() -> None:
    ...
    artifacts = {
        ...
        "tokenizer": {
            "tokenizer": {
                "name": "tokenizer.json",
                "sha256": sha256_file("tokenizer.json"),
            },
    ...
    manifest = {
        ...
        "provenance": {
            "source": "local_embeddings-tiny test generator",
            "production": False,
        },
