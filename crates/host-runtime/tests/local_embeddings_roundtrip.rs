//! These tests exercise a LocalEmbeddings lane over a real authenticated host route.
//! These tests use the `local_embeddings-tiny` bundle and certified FastEmbed backend.
//!
//! Vector-serving tests require a native ONNX Runtime library in `EIDNARA_LOCAL_EMBEDDINGS_TEST_ORT_LIBRARY`.
//! Those tests are `#[ignore]`d so a default `cargo test` reports them as ignored rather than passed.
//! Run them with `cargo test -p host-runtime --test local_embeddings_roundtrip -- --ignored` after setting the variable.
...
use support::local_embeddings::{
    BUDGET, LocalEmbeddingsHost, ROOT, call, open_local_embeddings_route,
};
...
#[tokio::test]
async fn corrupt_bundle_degrades_local_embeddings_and_keeps_context_routable() {
    ...
    let host = LocalEmbeddingsHost::start(component).await;
    ...
    let err = support::local_embeddings::open_local_embeddings_route_rejection(&mut client).await;
    assert_eq!(err, "artifact_invalid");

    // Context requests still complete.
    let (channel, epoch) = client
        .route_open("context", ROOT, "opencode", "s1")
        .await
...
async fn all_four_operations_serve_certified_vectors_over_the_wire() {
    ...
    let items: Vec<(String, String)> = texts
        ...
        .collect();
    let lane_info = host_runtime::local_embeddings::LaneInfo {
        ...
        max_text_bytes: LocalEmbeddingsLimits::default().max_text_bytes,
        provenance: serde_json::Value::Null,
        recommended_rows: 16,
        recommended_token_budget: 8192,
    };
    let params = support::local_embeddings::batch_params(&lane_info, &items);
    ...
    let key = support::local_embeddings::request_key(&lane_info, &items);
