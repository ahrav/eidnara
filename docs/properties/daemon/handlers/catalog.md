...
Reachability: default-production
Status: active
Exercised: yes - `dreamer_run_task_records_an_exhausted_chain_as_a_terminal_failure` (`lib.rs`, `mod tests`) exhausts the model chain, asserts the receipt is `Complete` with a failed terminal kind, and replays the failure without a second producer start; `dreamer_run_task_replays_from_the_receipt_and_refuses_a_changed_request` covers the success replay and the digest conflict. Neither faults the store write itself.
...
Check: `always` - for any `dreamer.run_task` response **that consumed a model attempt**, `lookup_dreamer_receipt` for the request's `(project, producer, operation_key)` returns a `Complete` receipt, and a response that could not be recorded is `dreamer_ledger_failed` or `dreamer_ledger_fenced`, never `dreamer_run_failed`. `always` because the receipt is the retry contract and applies to every terminal outcome, success or failure, that spent a billable call.
Fault/timing angle: The failure path calls `complete_dreamer_receipt(.., DreamerTerminalKind::Failed, ..)` and returns `dreamer_run_failed` only on `Applied`; an `Err` becomes `dreamer_ledger_failed` and a `Fenced` result `dreamer_ledger_fenced`, so a dropped write cannot turn a terminal failure into a replayable command. The success path completes the receipt before deleting the child session. Both writes are row-predicate guarded on key, generation, and `in_progress` state (`memory-store/src/dreamer_ledger.rs`).
...
Existing check: the two receipt tests named under Exercised; no test injects a store fault on the failure-path write.
