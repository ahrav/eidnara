...
`crates/daemon/src/history_summarizer.rs:1666-1704`. Validation is called at
...
Err(err) => {
    ...
    persist_history_summarizer_state(store, session_id,
...
`completion_failure_backoff_at_ms` (`history_summarizer.rs:1145-1154`):
...
`now + HISTORY_SUMMARIZER_FAILURE_BACKOFF_MS` at `lib.rs:4771`, `:5101`, `:5266`, with
`HISTORY_SUMMARIZER_FAILURE_BACKOFF_MS = 60_000` (`history_summarizer.rs:30`). There is no
exponential term and no attempt-count input to the function.

Compare `classified_backoff_at_ms` (`history_summarizer.rs:1156-1166`), which DOES honour a
...
```rust
if loaded.meta.history_summarizer.failure_backoff_at_ms.is_some_and(|backoff_at_ms| now < backoff_at_ms) {
    self.record_no_fire(&store, &parsed.session_id, &loaded, "backoff");
    return PreparedHistorySummarizerAction::Complete(HistorySummarizerDiagnostics { fired: false, ... });
}
```

Once `now >= backoff_at_ms`, the firing decision proceeds. Read the surrounding
`prepare_history_summarizer_fire` region (`lib.rs:4808-5184` per the scope map) for any
...
`history_summarizer.rs:1440-1450`:
...
- `HistorySummarizerDiagnostics.last_failure`, populated at `lib.rs:5313`
  (`diagnostics.last_failure = loaded.meta.history_summarizer.last_failure.clone()`) and
  carried through the prepared-action arms (`lib.rs:4840`, `:4848`, `:5055`,
  `:5147`).
- `history_summarizer_status_summary` (`lib.rs:15464-15478`), which is ordered:

```rust
if state.state != HistorySummarizerPhase::Idle { return format!("fire seq {} {}", ...); }
...
detail is still in durable state and still in `HistorySummarizerDiagnostics`; it is the
...
A user configures a small or heavily-quantised history_summarizer model, or a provider
...
compacts, so the session also never gets the context reduction the history_summarizer
exists to provide.
...
(`prepare_history_summarizer_fire`), so a quiet session retries only when the agent is
...
1. A `MemoryStore` and an injected producer factory. The seam exists:
   `HistorySummarizerProducerFactory` (`lib.rs:3023-3030`) with
...
3. A controllable clock. `set_guidance_now_ms_for_test` exists
   ...
   `completion_now_ms` (`history_summarizer.rs:1637`, a `fn() -> i64`) are the injection
...
4. Drive N firings by advancing past each backoff. Assert the property: after N
   ...
   `HISTORY_SUMMARIZER_FAILURE_BACKOFF_MS` or `publish_health_degraded` is true. Today
...
- Sources examined: `memory-store/src/lib.rs:1517` (the field),
  ...
  asserting a successful publish resets it to 0), `history_summarizer.rs:323`
...
- Missing evidence: whether operators are expected to read
  `history_summarizer_status_summary` (which the no-fire precedence masks) or the JSON block
...
### Q: Is there any escalation or circuit-breaking anywhere on the history_summarizer path that this record missed?

- Sources examined: `history_summarizer.rs:1145-1166` (both backoff functions),
...
- Findings: There IS a permanent-failure concept, signalled by the
  `chain-exhausted-permanent:` prefix (`history_summarizer.rs:32`), and
  ...
  `history_summarizer_producer`), not by validation outcomes. So the machinery for "stop
