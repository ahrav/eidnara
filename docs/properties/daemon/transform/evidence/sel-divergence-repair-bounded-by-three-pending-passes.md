...
history_segment watermark is missing or stale; after this limit, the gap is repaired
...
`ctx.history_summarizer_active || ctx.wrapup_active` (`:3924`). When it holds, the counter
...
(`:2289`), and `history_segment_revision_matches` (`:3913-3918`) means the applied
...
The intended reading is documented at `:3919-3923`: "A non-idle durable history_summarizer
...
ORs in `ctx.history_summarizer_active`, and `transform.rs:601-603` documents that flag
without any duration: "True while this process has a history_summarizer
...
A session's applied-history_segment watermark is stale, so
`history_segment_revision_matches` is false and `divergence_candidate` is `Some` on
every pass. The module holds a history_summarizer lease for this session across a long run:
`prepare_history_summarizer_fire` claims it (`lib.rs:4808-5184`), the producer call goes
out to a language model over ModelExecution, and the lease is held through
...
passes *outside* the window. If the history_summarizer run fails and is retried, the window
...
Window: the union of all history_summarizer and wrapup lease intervals for the session.
`wrapup_active` is bounded at 3,800 seconds (`transform.rs:604-606`).
`history_summarizer_active` is unbounded from the 4b slice's point of view.
...
coverage gap with a stale watermark, take three passes with no history_summarizer or wrapup
...
The uncovered case: hold `ctx.history_summarizer_active` true across N passes for N well
...
is the seam for holding a history_summarizer lease; `history_summarizer_active`
...
- Findings: The 4b slice's test fixtures construct `ProducerContext` with
  `history_summarizer_active` and `wrapup_active` at their defaults. I did not find a test
...
- Conclusion: unresolved, needs a name-level grep of all 280 transform tests for
  `history_summarizer_active`. Recorded as `Exercised: partial` on that basis.

### Q: Is `history_summarizer_active` bounded?
...
- Findings: The doc comment describes the lease's phases but no maximum duration.
  ...
  omission for `history_summarizer_active` look deliberate rather than accidental.
- Missing evidence: `history_summarizer.rs` is sub-part 4a.
