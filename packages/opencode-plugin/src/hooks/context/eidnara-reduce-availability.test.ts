...
import {
    clearEidnaraReduceAvailability,
...
describe("eidnara_reduce availability (OpenCode DB)", () => {
    ...
    it("keeps the frozen fail-open verdict when the database appears after the first read", () => {
        ...
        const beforeDb = resolveEidnaraReduceAvailability(sessionId);
        expect(beforeDb).toEqual({ callable: true, frozen: true });

        writeOpenCodeDbWithFirstUserTools(sessionId, { eidnara_reduce: false });
    ...
    it("breaks a time_created tie by id so the canonical first user message decides", () => {
        ...
        writeOpenCodeDb([
            { id: "msg-b", sessionId, timeCreated: 7, tools: { eidnara_reduce: true } },
            { id: "msg-a", sessionId, timeCreated: 7, tools: { eidnara_reduce: false } },
        ]);
    ...
    it("treats a message row with malformed JSON as a non-match instead of failing open unfrozen", () => {
        ...
        writeOpenCodeDb([
            { id: "msg-2", sessionId, timeCreated: 2, tools: { eidnara_reduce: false } },
        ]);
...
describe("eidnara_reduce process-global registration override (compaction-off #266 S4)", () => {
    // The override applies process-wide; each test resets it so later tests cannot inherit `false`.
    it("when eidnara_reduce is not registered globally, every session resolves callable=false frozen=true", () => {
    ...
    it("the override is specific to eidnara_reduce — todowrite is unaffected", () => {
        setEidnaraReduceRegisteredGlobally(false);
        try {
