...
- `crates/shm-transport/src/backend/ring.rs:2067-2098` is
...
- `ring.rs:2050-2063` initializes a fresh lifecycle page and sets
  ...
  creation onward.
- `packages/shm-native/src/lib.rs:605-608` claims the process-wide grant
...
1. A detach failure or a receive-validation failure sets the flag. The two
   reachable triggers are `packages/shm-native/src/lib.rs:283` and
   `ring.rs:1098`.
