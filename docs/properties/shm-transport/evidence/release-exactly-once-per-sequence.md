...
- `crates/shm-transport/src/backend/ring.rs:1434-1450` `slot_ptr` — the index is
...
- `ring.rs:1190-1210` — the checks that run *before* the CAS: a `consumed` acquire load
  ...
  transition.
- `crates/shm-transport/src/lease.rs:184-192` `release_once` — a second,
...
- Existing checks, corrected and re-anchored at post-#131 HEAD: the
  ...
  `crates/shm-transport/tests/ring.rs:152-175`; `:177-180` is the
...
- Sources examined: `ring.rs:1175-1247` (`release` in full), `:1434-1450`
  ...
  write), `crates/shm-transport/src/lease.rs:184-207`,
  `crates/shm-transport/tests/ring.rs:127-232` (both existing release tests).
