...
import { databaseResetMarkerPath } from "@eidnara/opencode/features/context/storage-format-epoch";
...
export async function runDoctor(options: RunDoctorOptions): Promise<number> {
    ...
    const sharedDbPath = join(getEidnaraStorageDir(), "context.db");
    ...
    const sweepDbPath = join(getEidnaraStorageDir(), "context.db");
