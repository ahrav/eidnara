...
| `crates/daemon/src/packing.rs`, `optional_statements_stop_at_the_first_non_excludable_fault` | A missing or corrupt row is excluded and the next statement runs; an interrupted statement is the fault and no later statement runs. | unaudited | Exercises the combinator over a fixed outcome list, not `prepare_optional` under a deadline. |
...
| `crates/retrieval/tests/packing_grouping.rs`, `selected_bytes_are_never_printed_by_grouping_types`; `crates/daemon/src/packing.rs`, `materialized_bytes_are_never_printed` | `Debug` output of grouping and required-phase types names byte lengths, never selected content. | unaudited | Asserts the crate-level "payloads are never logged" claim; no record owns it in this part or `required/`. |
