//! ModelExecution application protocol: request and event schemas the harness adapters exchange with
//! the daemon, validated with the same strict-decoding rules as the LocalEmbeddings protocol.

use crate::control::check_string;
use crate::local_embeddings::protocol::{
    MapOnly, NoParams, OptionalParams, RequiredParams, depth_exceeds, schema,
};
use serde::Deserialize;

pub use crate::local_embeddings::protocol::RequestError;
...
fn preflight(body: &[u8], binary: bool) -> Result<(), RequestError> {
    if binary {
        return Err(schema("model_execution requests are JSON only"));
    }
...
fn decode_request(body: &[u8]) -> Result<Request, RequestError> {
    ...
    match envelope.0.method.as_ref() {
        ...
        _ => Err(schema("unsupported model_execution method")),
    }
}
...
// ---------------------------------------------------------------------------
// Response and event field names must remain compatible with `HistorySummarizerProducer`'s `unit_*` helpers and `classify_run_state`.
// ---------------------------------------------------------------------------
