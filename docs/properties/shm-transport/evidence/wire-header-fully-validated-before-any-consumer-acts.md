...
At HEAD: The type is `RingClientEndpoint` at HEAD; `send` (`ring_transport.rs:1520`) hands `header.encode()` to `reserve_until_in` and `try_send_bounded` (`:1543`) to `try_reserve_in`, and both commit `body.len()` through `publish` (`:1580-1604`).
