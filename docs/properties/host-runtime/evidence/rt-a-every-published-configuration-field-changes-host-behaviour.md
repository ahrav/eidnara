...
`host_capabilities` has no documentation of its own and no consumer.
...
| `crates/host-runtime/src/config.rs:262` | `Debug` formatting, inside the hand-written impl |
| `crates/daemon/src/bin/eidnara_host/serve.rs:487` | written as `Vec::new()` |
| `crates/daemon/tests/host_adapter.rs:29` | written as `Vec::new()` |
| `crates/daemon/tests/host_adapter.rs:88` | written as `Vec::new()` |
| `crates/daemon/examples/direct_host_fixture.rs:577` | written as `Vec::new()` |
...
`host_capabilities` is rendered in full while `storage` is reduced. In a
...
### Q: is `host_capabilities` referenced by the protocol specification under another name?

- Sources examined: `docs/host-wire-protocol.md`, searched for
  `host_capabilities`, `capabilities`, and the §8.1 initialization steps the
  doc comment cites.
- Findings: no occurrence of `host_capabilities`. The doc comment at
...
- Sources examined: `config.rs:1-6`, which defers CLI and config-file exposure to
  the source daemon-lifecycle work; `config.rs:236-238`, which defers
  `invalidate_on_missed` to the source module-host work; `runtime.rs:3-5`, which defers
  signal wiring to the source module-host work.
- Findings: this crate has an established convention of naming the bead that will
  ...
  examples above. `host_capabilities` carries no such comment and no bead
  reference.
