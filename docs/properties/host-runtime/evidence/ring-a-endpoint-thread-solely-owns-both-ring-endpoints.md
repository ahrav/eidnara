...
The module doc at `crates/host-runtime/src/ring_transport.rs:3-4` states a
...
1. The OS thread is spawned at `:238-240` with the name
   `host-shm-endpoint`. Everything from `:241` to `:277` is inside that
   closure.
...
Neither runs in CI today: every `-p host-runtime` invocation in `ci.yml` carries a
