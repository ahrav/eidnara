...
- **Primary evidence:** `pin_fence_durability` (`crates/storage/src/lib.rs:548-563`) runs at the start of `with_conn_fenced`; `open_sqlite` (`crates/storage/src/lib.rs:565-661`) calls the same function before the transaction that applies the baseline and claims the fence, so an open on a VFS that cannot switch to WAL fails instead of committing a claim every later fenced write would reject.
- **Existing evidence:** `open_pins_full_synchronous` (`crates/storage/src/lib.rs:1639-1648`), `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:1672-1738`).
