...
Status: invalidated
Invalidated: the mandatory-ring refactor (host repo at `907746f7b`) removed this mechanism; no file under `crates/host-runtime/src` holds it at the pinned commit, so the subject is unreachable by any configuration.
...
any configuration.
Status: invalidated
Invalidated: the mandatory-ring refactor (host repo at `907746f7b`, `ed487e11`) removed transport negotiation, the `TransportState` setup machine (`BootstrapTcp`, `CandidateSetup`, `transport_ready`, `handle_negotiate`), and `tests/transport_negotiation.rs`; no file under `crates/host-runtime` holds this mechanism at the pinned commit, so the subject is unreachable by any configuration.
...
unreachable by any configuration.
Status: invalidated
Invalidated: the mandatory-ring refactor (host repo at `907746f7b`, `ed487e11`) removed transport negotiation, TCP fallback reasons, `TransportProviders`, `transport_provider.rs`, and `tests/transport_negotiation.rs`; no file under `crates/host-runtime` holds this mechanism at the pinned commit, so the subject is unreachable by any configuration.
...
Status: active
Exercised: partial - a driver that registers a tampering handler ahead of the hook covers the OpenAI-style, Gemini-style, Bedrock-style, and mixed-spelling payloads plus one unrecognized shape under generation revision 1, and, with the temperature variable removed, an OpenAI-style and a Bedrock-style payload under revision 2; nothing runs the hook inside a real Pi process or covers a non-numeric environment value.
Guarantee: The provider payload Pi sends carries exactly the output-token bound the `session.send` request admitted and, when the request admitted a temperature (every generation revision 1 request; a revision 2 request that names one), exactly that temperature: every recognized output-token spelling present on the payload, `generationConfig.maxOutputTokens`, and `inferenceConfig.maxTokens` are rewritten to the request's `max_output_tokens`; `temperature` follows it when admitted and is left untouched when a revision 2 request omits it, so the model's native decoding policy applies; every unrelated field survives; and a payload with no recognized output-token field or a non-object payload fails the request rather than running uncapped.
Check: `always` - for every payload the hook returns, each recognized output-token field equals the admitted bound; when `EIDNARA_MODEL_EXECUTION_TEMPERATURE` is set, `temperature` equals it, and when it is absent no `temperature` field is written or removed; fields the hook does not own are byte-identical to the input; and a payload with no recognized field throws. The rewrite is an invariant over every provider request, so one `always` covers the conjunction.
Fault/timing angle: An earlier trusted extension leaves a larger limit in a second spelling, or a provider adds a wire family the hook does not recognize; either lets a provider default exceed the caller's budget. Under revision 2 with no admitted temperature, a temperature an earlier handler set survives by design; the hook owns only the output-token bound there.
Required faults and enabling state: A payload touched by an earlier handler; a payload carrying two output-token spellings; a payload with no recognized spelling; a revision 2 request that omits temperature.
...
Impact: A provider request runs with a token budget the caller did not admit, or with a temperature other than the one a request admitted.
Open questions: None.
