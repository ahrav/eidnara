...
(`packages/plugin/src/shared/host-client/shm-frame-channel.ts:295-331`) calls
`decodeHeader` at `:305` and passes the result straight to
`handlers.onFrame` at `:325`. `decodeHeader`
(`packages/plugin/src/shared/host-client/protocol.ts:266-295`) ends in
...
`host-client` must reach `headerViolation`, which today is two of three.
...
- Missing evidence: whether `ShmFrameChannel` is reachable in any shipped
  ...
  (`docs/shm-transport.md:5-7`), and I did not re-derive that for the
  client package.
