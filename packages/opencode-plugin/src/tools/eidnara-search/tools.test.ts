...
import type { EidnaraSearchToolDeps } from "./types";

const toolContext = (sessionID = "ses-search") =>
    ({ sessionID, directory: "/tmp/eidnara-search" }) as never;
...
describe("createEidnaraSearchTools", () => {
    it("routes a pinned session independently of the tool context directory", async () => {
        ...
        const pinnedRoot = "/tmp/eidnara-search-pinned";
        const sessionDirectories = new Map([["ses-search", pinnedRoot]]);
        let metadataReads = 0;
        const tools = createEidnaraSearchTools({
            kernelClient: (route) => {
        ...
        const result = await tools.eidnara_search.execute({ query: "pinned search" }, {
    ...
    it("rejects an over-cap query with the native string error before any work", async () => {
        ...
        const atomResult = await tools.eidnara_search.execute(
    ...
    it("rejects a non-array or unknown sources value instead of searching nothing", async () => {
        const tools = createEidnaraSearchTools(kernelHarness().deps);

        const nonArray = await tools.eidnara_search.execute(
        ...
        const misspelled = await tools.eidnara_search.execute(
    ...
    it('treats sources: ["memory"] like an omitted sources argument', async () => {
        ...
        const tools = createEidnaraSearchTools(harness.deps);
        const explicit = await tools.eidnara_search.execute(
            { query: "offline cache", sources: ["memory"] },
            toolContext(),
        );
        const implicit = await tools.eidnara_search.execute(
            { query: "offline cache" },
            toolContext(),
        );
    ...
    it("renders a rejected-approach memory as an anti-memory warning", async () => {
        ...
        const tools = createEidnaraSearchTools(harness.deps);
        const result = await tools.eidnara_search.execute({ query: "redis cache" }, toolContext());
    ...
    it("rejects a comma-joined id list over the operand bound before any daemon round trip", async () => {
        ...
        const execution = await executeEidnaraSearch(
            harness.deps,
    ...
    it("renders the state text when the memory source is not available", async () => {
        ...
        const tools = createEidnaraSearchTools(harness.deps);
        const memoryOnly = await tools.eidnara_search.execute(
        ...
        const implicit = await tools.eidnara_search.execute({ query: "anything" }, toolContext());
    ...
    it("renders the disabled state from a disabled kernel client", async () => {
        ...
        const tools = createEidnaraSearchTools(harness.deps);
        const result = await tools.eidnara_search.execute({ query: "anything" }, toolContext());
    ...
    it("advertises only the memory source in both the full and light descriptions", () => {
        ...
        for (const description of [
            tools.eidnara_search.description,
            EIDNARA_SEARCH_LIGHT_DESCRIPTION,
        ]) {
...
describe("executeEidnaraSearch", () => {
    it("rejects a blank query as invalid with the same error text the tool returns", async () => {
        const { deps } = kernelHarness();
        const tools = createEidnaraSearchTools(deps);
        const execution = await executeEidnaraSearch(deps, { query: "   " }, toolContext());
        expect(execution.status).toBe("invalid");
        expect(execution.text).toBe("Error: 'query' is required.");
        expect(execution.text).toBe(
            String(await tools.eidnara_search.execute({ query: "   " }, toolContext())),
        );
    });
    ...
    it("keeps direct object-id lookup byte-identical between the tool and the structured helper", async () => {
        ...
        const execution = await executeEidnaraSearch(harness.deps, args, toolContext());
        expect(execution.status).toBe("complete");
        if (execution.status !== "complete") return;
        expect(execution.text).toBe(
            String(await tools.eidnara_search.execute(args, toolContext())),
        );
