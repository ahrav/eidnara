...
- **Existing evidence:** `sqlite_descriptor_golden_json` (`crates/storage-types/src/lib.rs:210-227`) compares against the exact JSON string and round-trips; `postgres_descriptor_golden_json` (`crates/storage-types/src/lib.rs:230-243`) round-trips the other variant; `helpers_reproduce_the_golden_vectors` reserializes eight fixture descriptors.
