...
- `crates/shm-transport/tests/iceoryx.rs` — seven tests, every one
...
dependency, and the suite does execute: `the source repository `ci.yml` workflow:166` under the
Linux guard at `:164` runs `cargo nextest run -p shm-native -p
shm-transport`, which enables the transport crate's default features.
