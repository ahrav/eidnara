...
- The sole production consumer of the field is
  `crates/daemon/src/bin/eidnara-host.rs`. A grep for `.reason` on a probe shows
  exactly one read: `:388`, inside `quarantined_observation` (`:387-395`), which
  compares against `host_runtime::UNSUPPORTED_STATE_SCHEMA_REASON` and returns `None`
  for anything else.
...
- There is no second channel. `crates/host-runtime/Cargo.toml` lists no `tracing`,
...
- Sources examined: every `state: LifecycleState::Wedged` construction in
  ...
  at `:35`; a complete grep for `.reason` in `eidnara-host.rs`; `cmd_probe`
