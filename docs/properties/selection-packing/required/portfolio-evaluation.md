...
`existing-checks.md`, `fault-map.md`, `crates/daemon/src/packing/mod.rs`,
...
2. **Same record: `optional_events() == 0` cannot fail at this base.**
   ...
   push is the private `required()` (`crates/daemon/src/packing/mod.rs:155`).
...
1. **No record that every required request yields exactly one materialized
   ...
   `report.occurrences` (`crates/daemon/src/packing/mod.rs:326`), and
...
3. **No liveness record for phase termination.** All three records are
   ...
   (`crates/daemon/src/packing/mod.rs:240`), and the tested cancellation uses a
...
4. **The production estimator's byte-to-cost mapping is unchecked.**
   ...
   (`crates/daemon/src/packing/mod.rs:103`) before counting, so invalid UTF-8 is
...
2. **Zero is an accepted budget.** `from_budget(Some(0.0))` is `Ok(0)`
   (`crates/daemon/src/packing/mod.rs:419`) and `reserve_required` then refuses
