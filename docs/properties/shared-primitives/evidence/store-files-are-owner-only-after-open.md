...
- **Primary evidence:** `open_sqlite` (`crates/storage/src/lib.rs:581-587`) creates a missing database file with mode `0600` before SQLite opens it, then calls `lease::protect_file` on the database path and its `-wal` and `-shm` siblings before enabling WAL or writing the fence. SQLite gives sidecars the database file's mode, so files created later start owner-only.
- **Existing evidence:** `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:1029`) covers pre-existing permissive files; `new_database_file_is_owner_only_at_creation` (`:938-956`) proves the created database is `0600` under umask `022` with no `chmod`; `fresh_open_creates_owner_only_sidecars_under_a_permissive_umask` (`:1445-1476`) proves a fresh open under umask `022` leaves the database, `-wal`, and `-shm` at `0600`. All Unix only.
