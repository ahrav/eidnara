...
import { createConditionalNoteCapabilities, isSecretDeniedPath } from "./capabilities";
import { ConditionalNoteNetworkError } from "./types";

const execFileAsync = promisify(execFile);

async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
    const dir = await mkdtemp(path.join(tmpdir(), "eidnara-conditional-note-cap-"));
    try {
        return await fn(dir);
    } finally {
...
async function createTaggedRepository(dir: string): Promise<void> {
    ...
    await git(dir, "config", "user.name", "Conditional Note Test");
    await git(dir, "config", "user.email", "conditional-note@example.invalid");
...
describe("conditional-note readFile capability", () => {
    test("keeps leading and trailing whitespace in names and rejects blank input", async () => {
        await withTempDir(async (dir) => {
            ...
            const cap = createConditionalNoteCapabilities({
                projectRoot: dir,
    ...
    test("denies final symlink and parent symlink escapes", async () => {
        await withTempDir(async (dir) => {
            const outside = await mkdtemp(path.join(tmpdir(), "eidnara-conditional-note-outside-"));
            try {
...
describe("conditional-note git capabilities", () => {
    ...
    test("a HEAD ref whose commit object is missing rejects instead of answering", async () => {
        await withTempDir(async (dir) => {
            ...
            await expect(cap.gitHeadSha()).rejects.toBeInstanceOf(ConditionalNoteNetworkError);
            await expect(cap.gitTag()).rejects.toBeInstanceOf(ConditionalNoteNetworkError);
            await expect(cap.gitLog()).rejects.toBeInstanceOf(ConditionalNoteNetworkError);
        });
    });
...
describe("conditional-note secret path denylist", () => {
