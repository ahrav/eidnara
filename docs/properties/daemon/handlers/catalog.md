...
`dreamer_run_task_records_an_exhausted_chain_as_a_terminal_failure` (`:27751`)
...
success replay and the digest conflict with no producer call (`:27184`).
`dreamer_run_task_fails_closed_when_the_failure_record_cannot_be_written`
(`:27621`) installs a `RAISE(ABORT)` trigger on `dreamer_receipts`
`UPDATE OF state`,
...
(`:27677`) faults the attempt row and shows a usable result still completes the
receipt.
`dreamer_run_task_settles_a_missing_run_after_restart_as_unknown_without_redispatch`
(`:27842`) drops the request mid-await, rebinds the route under another harness,
...
`dreamer_run_task_leaves_a_run_the_runtime_still_holds_open` (`:27910`) resumes
with the run still `active` and shows nothing is written or dispatched.
`dreamer_run_task_settles_a_run_the_runtime_reports_ended_as_unknown` (`:27932`)
...
`dreamer_run_task_recovers_a_receipt_stranded_before_any_dispatch` (`:27429`)
...
(`:27979`) reopens a receipt with a `NotSent` attempt and shows the successor
...
`dreamer_run_task_settles_an_open_attempt_without_a_handle_as_unknown` (`:28210`)
...
(`:28246`) faults the receipt completion after a usable answer and shows the
...
(`:28068`) scripts a timed-out await with a late answer queued behind it and
shows the attempt ends `cancelled` with no second read of the run, and counts.
`dreamer_run_task_clamps_the_request_timeout_to_the_host_ceiling` (`:28117`),
...
(`lib.rs:28142`) cover the three bounds; the budget test also shows a changed
...
(`lib.rs:28340`) shows an exhausted budget still settles an open receipt while
refusing a takeover.
`dreamer_run_task_does_not_purge_the_child_session_once_it_is_fenced`
(`lib.rs:27485-27551`) moves the receipt to generation 2 through `on_start` and
...
replays and an `in_progress` one goes to `resume_dreamer_receipt` (`lib.rs:9894`)
...
is recorded right after `start` returns (`lib.rs:9717`); a handle write that does
not land settles the attempt and receipt `unknown` through
`settle_dispatched_attempt_as_unknown` (`lib.rs:13683`), unless the generation
fence rejects settlement. A fenced run-handle write skips purge
(`lib.rs:9723-9734`), matching the fenced await-terminal write
(`lib.rs:9766-9799`); a store error still permits cleanup. If the attempt
terminal cannot be recorded, a usable answer is offered to receipt completion
before unknown settlement (`lib.rs:9771-9786`). The request deadline is clamped
...
`unknown` through `complete_receipt_as_unknown` (`lib.rs:13703`), keeping its own
...
handle settles `unknown` (`lib.rs:9933-9991`). Only the absence of a marker
reaches `take_over_undispatched_dreamer_receipt` (`lib.rs:9913`), whose single
...
(`lib.rs:9771-9799`, `:9846-9876`, `:13714-13723`).
