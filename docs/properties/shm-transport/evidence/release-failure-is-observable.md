...
- `crates/shm-transport/src/lease.rs:201-207` — the drop-path discard:
  ```rust
...
- **Correction to the catalog record.** The catalog describes former `:365` as discarding "a
  ...
  `quarantine`, not `release` (`crates/shm-transport/src/profile.rs:518-521`,
  `:472-480`). So the discarded signal is real, but it is "this record was not in a
  releasable state", not an error value.
- `crates/shm-transport/src/profile.rs:512-515` — `Admission::release(mut self)`
...
- **Where release failure *is* observable.** The host's explicit receive-path
  ...
  `error("receive completion failed")` (`packages/shm-native/src/lib.rs:327-331`),
  which throws through `packages/shm-native/index.ts:494-501` into either
...
5. `run_endpoint` classifies both `Cancelled` and `Overloaded` as **clean** (former
   ...
   `crates/host-runtime/src/ring_transport.rs:276`.
...
(`crates/host-runtime/src/config.rs:221`, `:233`) keeps the endpoint waiting on the
...
- Sources examined: `crates/shm-transport/src/lease.rs:159-207`;
  `packages/shm-native/src/lib.rs:314-334` (`detach_active`) and `:1160-1230`
  (`poll`, with `std::mem::forget(lease)` at `:1208`);
  `packages/shm-native/index.ts:494-507`;
  `packages/plugin/src/shared/host-client/shm-frame-channel.ts:209-230` and
  `:323-370`; former `crates/host-runtime/src/shm_provider.rs:363-371`, former `:546-619`;
  former `crates/host-runtime/src/provider_recovery.rs:137-179`;
  `crates/shm-transport/src/profile.rs:462-470`, `:504-521`.
...
- Missing evidence: no comment at `lease.rs:201-207` states the reasoning. The
  ...
  (`lease.rs:160-162`) carries no doc comment at all. `docs/shm-transport.md` does not cover
...
(`crates/shm-transport/src/lease.rs:201-207`).
...
`crates/host-runtime/src/ring_transport.rs:276`, and `Admission::release`
(`crates/shm-transport/src/profile.rs:512`) takes `self` and returns `()`.
