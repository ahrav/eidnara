...
`crates/host-runtime/src/setup_socket.rs:27-50`, the whole function. The relevant
sequence:
...
directory, resolved as `${dataDir}/eidnara/run`:

- `crates/host-runtime/src/instance.rs:167`: `MANAGED_DIR_NAME` is `"eidnara"`.
- `:171-173`: `managed_dir_path` joins it to the data directory.
- `:177-179`: `runtime_dir_path` joins `"run"`.
- `crates/host-runtime/src/runtime.rs:834`:
  `let setup_socket = cleanup.guard_mut().dir_path().join("setup.sock");`
- `crates/host-runtime/src/runtime.rs:836`: `bind_owner_only(&setup_socket)`.
...
- Sources examined: `crates/host-runtime/src/instance.rs`, re-read line by line after
