...
`crates/shm-transport/src/backend/ring.rs:1363-1367`:
...
- Sources examined: `crates/shm-transport/src/backend/ring.rs:1354-1382`.
...
- Sources examined: grep for `output_from_writer` across the workspace; the
  ...
  `dispatch.rs:545`.
- Findings: the callers are in `daemon`, which is Parts 4a through 4f, not
  this sub-part's scope. I did not audit them.
- Missing evidence: an `daemon` inventory of `output_from_writer` call sites
  and whether each one's `exact_len` is derived from the same code path as its
  serializer.
- Conclusion: unresolved, needs an `daemon` audit. Parts 4c and 4d already
