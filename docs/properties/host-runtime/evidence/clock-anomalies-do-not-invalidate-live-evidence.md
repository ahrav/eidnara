...
- `now_ms` is `crates/host-runtime/src/lifecycle.rs:400-405`. It has both saturating
...
- The window is **60 seconds**, `Duration::from_secs(60)`, configured in `impl
  ...
  `crates/daemon/src/bin/eidnara-host.rs:402`, so 60 s is the only value that
...
- **No monotonic source is used for this comparison.** A grep for `Instant`,
  ...
  (`eidnara-host.rs:408-418`), never for freshness.
...
4. `classify` returns `Wedged`. The CLI's `settle_probe` stops re-probing on a
   non-transitional state (`eidnara-host.rs:415`), so the flip is terminal, and
...
- Sources examined: grep for `Instant`, `monotonic`, `CLOCK_MONO`, and
  `SystemTime` across `lifecycle.rs`; `eidnara-host.rs:46-49` and `:408-418`.
