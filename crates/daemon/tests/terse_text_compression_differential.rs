...
//!
//! `reference` mirrors `crates/daemon/src/terse_text_compression.rs`.
...
fn fragment() -> impl Strategy<Value = String> {
    prop_oneof![
        ...
        Just(
            "the history_summarizer will be summarized because of the history_segment context message"
                .to_string()
        ),
        Just(
        ...
        Just("history_segments history_segments history_segments messages messages messages".to_string(),),
...
fn targeted_case_and_placeholder_edges_match_reference() {
    ...
    assert_eq!(
        compress(cases[0], TerseTextCompressionLevel::Ultra),
        "ALPHA → BETA"
    );
    assert_eq!(
        compress(cases[1], TerseTextCompressionLevel::Ultra),
        cases[1]
    );
    assert_ne!(
        compress(cases[2], TerseTextCompressionLevel::Full),
        cases[2]
    );
    assert_ne!(
        compress(cases[3], TerseTextCompressionLevel::Full),
        cases[3]
    );
}
