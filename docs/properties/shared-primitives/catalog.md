...
Existing check: `superseded_writer_is_fenced_out_after_handover`, `crates/storage/src/lib.rs:2105-2143`, and `superseded_writer_is_rejected_after_reopen`, `primitives@89abb40`; both use synthetic stale stores and remain **unaudited**.
...
Existing check: `distinct_databases_do_not_falsely_contend` (`crates/storage/src/lib.rs:1293-1302`) uses different parent directories; `distinct_databases_in_one_directory_do_not_falsely_contend` (`:1304-1317`) proves sibling files in one directory do not contend; `symlinked_database_paths_contend_or_are_refused_never_aliased` (`:883-936`) proves a directory-symlink alias contends and a file-symlink alias is refused. No test covers descriptor disagreement or hardlink aliases; status **unaudited**.
...
Existing check: `open_claims_fence_before_return`, `crates/storage/src/lib.rs:1163-1175`, observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:1182-1216`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
Existing check: backend tests cover fenced callbacks, fenced migrations, SQLite and PostgreSQL read-only rejection, and both autocommit maintenance paths. `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1684-1704`) leaves later fenced writes authorized; `a_read_callback_cannot_lower_fence_durability` (`:1707-1771`) pins `synchronous=FULL` and a WAL journal; `a_read_callback_cannot_clear_the_read_only_guard` (`:1774-1827`) denies every pragma write; `a_callback_cannot_end_the_fence_checked_transaction` (`:1830-1886`) denies the transaction escape and `a_callback_that_ends_the_transaction_is_rejected` (`primitives@89abb40`) reports it; `a_read_callback_cannot_escape_read_only_mode` (`:773-831`) covers ending and switching a read transaction; `a_negative_epoch_fails_closed` (`:835-879`) and `a_regressed_positive_epoch_fails_closed` (`:882-924`) fail closed on an unreconcilable epoch; `a_callback_cannot_damage_the_lease_row_it_is_fenced_against` (`:927-971`), `a_suppressed_epoch_increment_is_rejected` (`:974-991`), and `open_verifies_the_stored_epoch_matches_the_issued_one` (`:994-1018`) protect the lease row and its issuance; and `a_callback_cannot_damage_the_fence_row_it_is_checked_against` (`crates/storage/src/lib.rs:1889-1997`) refuses DML, triggers, indexes, views, and renames onto the fence table. The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
...
Confidence: high - [evidence](evidence/storage-descriptor-golden-vectors-are-byte-stable.md). Receipt verification compares the fixture bytes to the receipt hash; `helpers_reproduce_the_golden_vectors` (`crates/storage-types/tests/golden_vectors.rs:12-42`) and `golden_vectors_break_slug_collisions` (`crates/storage-types/tests/golden_vectors.rs:45`) assert every derivation. The `eidnara_` prefix and the `eidnara/` path component are frozen identities in the registry.
...
Confidence: high - [evidence](evidence/migrations-apply-once-per-namespace.md). `run_migrations` (`crates/storage/src/lib.rs:767-863`) reads `MAX(version)` per namespace, sorts, skips `<= current`, and commits SQL and record together; `migrations_seed_once_across_reopen` (`crates/storage/src/lib.rs:1219`), `later_migration_applies_on_top_of_earlier` (`crates/storage/src/lib.rs:1543`), and `independent_namespace_chains_in_one_database` (`crates/storage/src/lib.rs:1572-1603`) cover the three clauses.
...
Confidence: high - [evidence](evidence/callback-scope-is-restored-after-unwind.md). `CallbackScope::drop` (`crates/storage/src/lib.rs:433-438`) restores when `release` did not run; `a_panicking_read_does_not_strand_the_connection_read_only` (`crates/storage/src/lib.rs:1684`) exercises the unwind.
...
Confidence: high - [evidence](evidence/protected-transactions-pin-fence-durability.md). `pin_fence_durability` (`crates/storage/src/lib.rs:519-531`) runs before each protected transaction; `open_pins_full_synchronous` (`crates/storage/src/lib.rs:1673`) and `a_read_callback_cannot_lower_fence_durability` (`crates/storage/src/lib.rs:1707`) observe the pinned values. Power loss itself is not injected.
...
Confidence: high - [evidence](evidence/fenced-write-is-atomic.md). `with_conn_fenced` (`crates/storage/src/lib.rs:184-206`) claims and runs inside one transaction and commits only on `Ok`; `fenced_write_rolls_back_on_error` (`crates/storage/src/lib.rs:2016`) checks both the row and the fence.
Existing check: `fenced_write_rolls_back_on_error`, `fenced_write_commits_and_persists` (`crates/storage/src/lib.rs:1629`); audited at U2.
...
Confidence: high - [evidence](evidence/fence-epoch-outside-sqlite-range-fails-closed.md). `decode_fence_epoch` (`crates/storage/src/lib.rs:756-758`) and `fence_epoch_sql_value` (`crates/storage/src/lib.rs:717-723`) use checked conversions; `negative_database_fence_fails_closed` (`crates/storage/src/lib.rs:2075-2102`) and `epoch_above_sqlite_integer_range_fails` (`crates/storage/src/lib.rs:2202`) cover both directions.
