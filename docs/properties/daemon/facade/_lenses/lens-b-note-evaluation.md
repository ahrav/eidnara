# Part 4d Lens B: note evaluation and conditional-note semantics
...
envelope, response assembly, the claim-intent surface, and the `eidnara_note`
...
Primary files read in full: `crates/daemon/src/conditional_note_evaluation.rs`
...
`conditional_note_evaluation.rs`, `lib.rs`, and `memory-store`. The scope map's line
...
So "a note" in this lens means a conditional note: durable content, a
`surface_condition` (the trigger text), and a 20-field compiled-check lifecycle
projection. The projection is `ConditionalNoteLifecycleState`
(`conditional_note_evaluation.rs:281-302`).
...
| --- | --- | --- |
| create, plain | `eidnara_note` write with no condition (`lib.rs:11679-11711`) | `type='session'`, `status='active'` (`memory-store:10152-10157`) |
| create, conditioned | `eidnara_note` write with condition (`lib.rs:11629-11677`) | `type='smart'`, `status='pending'`, condition, compile hints (`memory-store:10192-10199`) |
...
1. **Phase is type-scoped.** `ConditionalNoteEvaluationOutcome`
   (`conditional_note_evaluation.rs:338-344`) wraps a per-phase outcome enum, so a
...
(`conditional_note_evaluation.rs:721-722`), but completion asserts only the phase
...
pre.check_false_since_at.or(Some(now))` at `conditional_note_evaluation.rs:491`, and
...
`conditional_note_selection_snapshot` (`lib.rs:13963-13985`), which defaults a NULL
...
Reduction reads the full `ConditionalNoteLifecycleState`, the phase-scoped outcome,
`note_id`, `now`, and a timezone.

### Outputs

Selection returns `Option<(note_id, phase_name, successor_cycle)>`
(`conditional_note_evaluation.rs:900-949`). Reduction returns `ConditionalNoteReduction`
...
(`conditional_note_evaluation.rs:8-10`). The claim holds for the file: no `unsafe`, no
...
`{note_id}:{hash}` seed (`conditional_note_evaluation.rs:262-274`), pure and
...
golden pins one zone (`testdata/conditional-note-evaluation-golden.json`,
`provenance.timezone = "America/Los_Angeles"`, consumed at
`conditional_note_evaluation.rs:1104-1108`), so the cross-language fixture cannot see
...
`ConditionalNoteSelectionCycle` (`:861-875`) is a per-slot, per-mode cursor, held in
...
1. `conditional_note_evaluation.rs` contains zero `tracing`, `log`, `warn!`,
...
2. `lib.rs:11556-11563` caps five `eidnara_note` string arguments;
...
8. `registration.policy_version` is written at `lib.rs:10964`, incremented at
   ...
   `CONDITIONAL_NOTE_CHECK_POLICY_VERSION` (`conditional_note_evaluation.rs:723`, `:749`,
   `:773`).
...
10. `check_failure_count` is a single column shared by two phases with two
    ...
    (`conditional_note_evaluation.rs:36`) read at `:458`, and
...
12. `next_occurrence` handles the DST direction correctly: it steps epoch
    ...
    panicking beyond chrono's range (`conditional_note_evaluation.rs:195-208`, with
...
Required faults and enabling state: a conditional note with a non-trivial
...
Verified the purity claim at `conditional_note_evaluation.rs:8-10`, the `chrono::Local`
argument at `lib.rs:14244`, the timezone's path into the schedule at
`conditional_note_evaluation.rs:246` and `:439`, and the fixture's pinned
...
Required faults and enabling state: one conditional note with
...
- Does the shipped evaluator worker impose its own inter-poll delay that bounds
  ...
  `packages/plugin/src/features/eidnara/conditional-notes/evaluator-worker.ts`
...
the number of `ConditionalNoteSelectionSnapshot` values built per poll are both
...
`eidnara_note` with a `surface_condition`, and no evaluator draining them. Each
...
close.
Required faults and enabling state: a conditional note in `pending` or `ready`, a
`eidnara_note dismiss`, then a `eidnara_note read` with `filter: "dismissed"` and a
`eidnara_note update` on the same id.
...
1. **`AUDIT-KNOWN-ISSUES.md` A51 contradicts the Rust update path.**
   ...
   that a content edit leaves a conditional note's compiled check intact, on the
   ...
   (`conditional_note_evaluation.rs:1-6`) is scoped narrower than it reads, because
   the update path is outside the fixture's coverage.
2. **The purity claim's scope.** `conditional_note_evaluation.rs:8-10` says "Pure
   ...
   `note-b-reducer-reads-process-local-timezone-for-durable-schedule`.
3. **The cross-language fixture's scope.** `conditional_note_evaluation.rs:4-6` says
   ...
   cases, read from `testdata/conditional-note-evaluation-golden.json`). It does not
...
6. **The snapshot's stated cost model.**
   `conditional_note_evaluation.rs:690-692` says the snapshot "avoids copying the
...
- `conditional_note_evaluation.rs` has no production assertions at all, and the one
...
- The four phase selectors are `pub` in a `pub mod` (`lib.rs:35`) and all four
  clamp `limit` with `limit.max(1)` (`conditional_note_evaluation.rs:729`, `:753`,
  ...
  the selectors be `pub(crate)` like `select_conditional_note_evaluation_cycle`
...
- Is a `wake_owned` veto meant to leave the cursor untouched? It returns before
  ...
  in `packages/plugin/src/features/eidnara/conditional-notes/wake-plane.ts`.
...
- The shipped registrant is the plugin's bridge
  ...
  `:1210`). It returns early unless `memory_classifierRunnable` (`:1024`) and unless the
  `evaluate-conditional-notes` task schedule is non-empty (`:1029`).
- `isMemoryClassifierRunnable` requires the `memory_classifier` block to be *present* and not
  disabled (`packages/plugin/src/config/agent-disable.ts:11-13`).
- The schema does **not** default it: `memory_classifier: MemoryClassifierConfigSchema.optional()`
...
- The shipped setup wizard writes the block unconditionally
  ...
  prompt to yes (`confirm("Enable memory_classifier?", true)`, `:449`). When enabled it
...
worth recording: a hand-authored config with no `memory_classifier` block at all leaves
...
- `boundary.rs` (3,053 lines) was in my brief "only where it bears on note
  ...
  The note lifecycle is entirely in `conditional_note_evaluation.rs`, `lib.rs`, and
