...
pub const MAX_MEMORY_REVIEWER_HELD_BACKING_BYTES_PER_PROJECT: u64 = 64 * 1024 * 1024;
pub const MAX_MEMORY_REVIEWER_HELD_BACKING_BYTES_PER_HOST: u64 = 256 * 1024 * 1024;
...
pub const MAX_MEMORY_REVIEWER_HOLD_REFERENCES: usize = 512;
/// Active MemoryReviewer holds per project and per host; equal to the pending-work bounds of 64 jobs per project and 256 per host, since each pending job owns at most one hold at a time.
pub const MAX_ACTIVE_MEMORY_REVIEWER_HOLDS_PER_PROJECT: usize = 64;
pub const MAX_ACTIVE_MEMORY_REVIEWER_HOLDS_PER_HOST: usize = 256;
...
const PRODUCTION_QUOTA: BackingQuota = BackingQuota {
    project_bytes: MAX_MEMORY_REVIEWER_HELD_BACKING_BYTES_PER_PROJECT,
    host_bytes: MAX_MEMORY_REVIEWER_HELD_BACKING_BYTES_PER_HOST,
};
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MemoryReviewerHoldKind {
    Execution,
    Review,
}

impl MemoryReviewerHoldKind {
    fn pin_kind(self) -> &'static str {
        match self {
            Self::Execution => MEMORY_REVIEWER_EXECUTION_HOLD_KIND,
            Self::Review => MEMORY_REVIEWER_REVIEW_HOLD_KIND,
        }
    }
}
...
pub struct MemoryReviewerHoldBinding {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MemoryReviewerHold {
    pub hold_id: String,
    pub kind: MemoryReviewerHoldKind,
    pub expires_at: i64,
...
pub enum MemoryReviewerHoldRefusal {
    #[error("memory_reviewer hold request is malformed")]
    InvalidRequest,
    #[error("memory_reviewer hold names another kernel incarnation")]
    IncarnationMismatch,
    #[error("an evidence id is unknown, invalidated, or purged")]
    UnavailableEvidence,
    #[error("the hold would carry more than {MAX_MEMORY_REVIEWER_HOLD_REFERENCES} references")]
    TooManyReferences,
    #[error(
        "the project already has {MAX_ACTIVE_MEMORY_REVIEWER_HOLDS_PER_PROJECT} active memory_reviewer holds"
    )]
    ProjectHoldLimit,
    #[error(
        "the host already has {MAX_ACTIVE_MEMORY_REVIEWER_HOLDS_PER_HOST} active memory_reviewer holds"
    )]
    HostHoldLimit,
...
pub enum MemoryReviewerHoldError {
    #[error(transparent)]
    Refused(#[from] MemoryReviewerHoldRefusal),
    #[error(transparent)]
    Store(KernelError),
}

impl From<KernelError> for MemoryReviewerHoldError {
    fn from(error: KernelError) -> Self {
        Self::Store(error)
    }
}

fn sqlite(error: rusqlite::Error) -> MemoryReviewerHoldError {
    MemoryReviewerHoldError::Store(map_sqlite(error))
}

fn corrupt<T>(_: T) -> MemoryReviewerHoldError {
    MemoryReviewerHoldError::Store(KernelError::CorruptCanonicalRow)
}

impl MemoryReviewerHoldBinding {
    fn validate(&self) -> Result<(), MemoryReviewerHoldRefusal> {
        check_digest(&self.project_digest)
            .map_err(|_| MemoryReviewerHoldRefusal::InvalidRequest)?;
        for field in [
            ...
            {
                return Err(MemoryReviewerHoldRefusal::InvalidRequest);
            }
        }
        Ok(())
    }
...
impl KernelStore {
    ...
    pub fn acquire_execution_hold(
        ...
        evidence_ids: &[String],
        expires_at: i64,
    ) -> Result<MemoryReviewerHold, MemoryReviewerHoldError> {
        self.acquire_hold(
    ...
    pub fn extend_execution_hold(
        ...
        let kind = MemoryReviewerHoldKind::Execution;
        binding.validate()?;
        let hold_id = identity(hold_id).map_err(|_| MemoryReviewerHoldRefusal::InvalidRequest)?;
    ...
    pub fn transfer_execution_to_review(
        ...
        execution: &MemoryReviewerHoldBinding,
        ...
        let execution_hold_id =
            identity(execution_hold_id).map_err(|_| MemoryReviewerHoldRefusal::InvalidRequest)?;
        ...
        {
            return Err(MemoryReviewerHoldRefusal::InvalidRequest.into());
        }
        let mut writer = self.lock_writer()?;
        let tx = writer
            .transaction_with_behavior(TransactionBehavior::Immediate)
        ...
        if let Some((review_hold_id, expires_at, _)) =
            live_hold_of(&tx, MemoryReviewerHoldKind::Review, review, now)?
        {
            return admit_totals(
                &tx,
                &review_hold_id,
        ...
        let (result_created_at, payload): (i64, Vec<u8>) = tx
            ...
            .optional()
            .map_err(sqlite)?
            .ok_or(MemoryReviewerHoldRefusal::InvalidRequest)?;
        ...
        let ReviewPayload::Proposal(proposal) = ReviewPayload::decode(&payload)
            .map_err(|_| MemoryReviewerHoldRefusal::InvalidRequest)?
        else {
            return Err(MemoryReviewerHoldRefusal::InvalidRequest.into());
        };
        for reference in &proposal.policy_dependencies.disclosed_inputs {
            if !is_covered(&tx, &execution_hold_id, &reference.evidence_id)? {
                return Err(MemoryReviewerHoldRefusal::NotCovered.into());
            }
        }
        ...
        .map_err(sqlite)?;
        // Only a live MemoryReviewer acquisition reference moves, and only later.
        tx.execute_cached(
        ...
        if !release_capture_pin_in_tx(&tx, &execution_hold_id, now)? {
            return Err(MemoryReviewerHoldRefusal::Released.into());
        }
        tx.commit().map_err(sqlite)?;
        Ok(hold)
    }
    ...
    pub fn lookup_review_hold(
        ...
        binding: &MemoryReviewerHoldBinding,
        now: i64,
    ) -> Result<Option<MemoryReviewerHold>, MemoryReviewerHoldError> {
        binding.validate()?;
    ...
    pub fn release_execution_hold(
        ...
        hold_id: &str,
        binding: &MemoryReviewerHoldBinding,
    ) -> Result<(), MemoryReviewerHoldError> {
        self.release_hold(hold_id, MemoryReviewerHoldKind::Execution, binding)
    }
    ...
    pub fn release_review_hold(
        ...
        self.release_hold(hold_id, MemoryReviewerHoldKind::Review, binding)
    }
    ...
    pub fn validate_held_evidence(
        &self,
        hold_id: &str,
        kind: MemoryReviewerHoldKind,
        binding: &MemoryReviewerHoldBinding,
        evidence_ids: &[String],
        now: i64,
    ) -> Result<Vec<HeldEvidence>, MemoryReviewerHoldError> {
        binding.validate()?;
        if now < 0 || evidence_ids.len() > MAX_MEMORY_REVIEWER_HOLD_REFERENCES {
            return Err(MemoryReviewerHoldRefusal::InvalidRequest.into());
        }
        let hold_id = identity(hold_id).map_err(|_| MemoryReviewerHoldRefusal::InvalidRequest)?;
        let mut reader = self.lock_reader()?;
        ...
        for evidence_id in evidence_ids {
            let evidence_id =
                identity(evidence_id).map_err(|_| MemoryReviewerHoldRefusal::InvalidRequest)?;
    ...
    fn acquire_hold(
        ...
        binding.validate()?;
        if evidence_ids.len() > MAX_MEMORY_REVIEWER_HOLD_REFERENCES {
            return Err(MemoryReviewerHoldRefusal::TooManyReferences.into());
        }
        let mut writer = self.lock_writer()?;
        let tx = writer
            .transaction_with_behavior(TransactionBehavior::Immediate)
        ...
        let transferred: bool = tx
            .query_row_cached(
                ...
                params![MEMORY_REVIEWER_REVIEW_HOLD_KIND, review.owner_id()],
                |row| row.get(0),
            )
            .map_err(sqlite)?;
        if transferred {
    ...
    fn release_hold(
        ...
        let now = current_time_ms();
        let stored =
            load_hold(&tx, &hold_id, kind, binding)?.ok_or(MemoryReviewerHoldRefusal::Missing)?;
        if stored.released {
            return Err(MemoryReviewerHoldRefusal::Released.into());
        }
    ...
    pub(crate) fn check_incarnation(
        ...
        if incarnation != binding.kernel_incarnation {
            return Err(MemoryReviewerHoldRefusal::IncarnationMismatch.into());
        }
        Ok(())
    }
}
...
fn is_covered(
    ...
    hold_id: &str,
    evidence_id: &str,
) -> Result<bool, MemoryReviewerHoldError> {
    tx.query_row_cached(
...
fn live_hold_of(
    ...
    binding: &MemoryReviewerHoldBinding,
    now: i64,
) -> Result<Option<(String, i64, bool)>, MemoryReviewerHoldError> {
    tx.query_row_cached(
...
fn insert_pin(
...
) -> Result<String, MemoryReviewerHoldError> {
    ...
    let project_active: i64 = tx
        ...
        .map_err(sqlite)?;
    if usize::try_from(project_active).map_err(corrupt)?
        >= MAX_ACTIVE_MEMORY_REVIEWER_HOLDS_PER_PROJECT
    {
        return Err(MemoryReviewerHoldRefusal::ProjectHoldLimit.into());
    }
    let host_active: i64 = tx
        ...
        .map_err(sqlite)?;
    if usize::try_from(host_active).map_err(corrupt)? >= MAX_ACTIVE_MEMORY_REVIEWER_HOLDS_PER_HOST {
        return Err(MemoryReviewerHoldRefusal::HostHoldLimit.into());
    }
    let snapshot: i64 = tx
        .query_row_cached(
...
fn precharge(
    ...
    if project_new > 0 {
        ...
        if project_held.saturating_add(project_new) > quota.project_bytes {
            return Err(MemoryReviewerHoldRefusal::ProjectBackingExhausted.into());
        }
    }
    if host_new > 0 {
        let host_held = held_backing(tx, None)?;
        if host_held.saturating_add(host_new) > quota.host_bytes {
            return Err(MemoryReviewerHoldRefusal::HostBackingExhausted.into());
        }
    }
    Ok(facts)
}
...
/// Distinct artifact bytes active MemoryReviewer pins hold, over one project's owner range or the whole host. Purged artifacts no longer occupy backing, so a degraded pin charges only its surviving bytes.
fn held_backing(
    tx: &Transaction<'_>,
    project: Option<(&str, &str)>,
) -> Result<u64, MemoryReviewerHoldError> {
...
fn load_live_evidence(
    tx: &Transaction<'_>,
    evidence_id: &str,
) -> Result<HeldEvidence, MemoryReviewerHoldError> {
    let evidence = tx
        .query_row_cached(
        ...
        .optional()
        .map_err(sqlite)?;
    evidence.ok_or_else(|| MemoryReviewerHoldRefusal::UnavailableEvidence.into())
}
...
fn admit_totals(
    ...
    if references > MAX_MEMORY_REVIEWER_HOLD_REFERENCES {
        return Err(MemoryReviewerHoldRefusal::TooManyReferences.into());
    }
    Ok(MemoryReviewerHold {
...
fn load_hold(
    ...
    kind: MemoryReviewerHoldKind,
    binding: &MemoryReviewerHoldBinding,
) -> Result<Option<StoredHold>, MemoryReviewerHoldError> {
    tx.query_row_cached(
...
pub(crate) fn load_valid_hold(
    ...
    binding: &MemoryReviewerHoldBinding,
    now: i64,
) -> Result<StoredHold, MemoryReviewerHoldError> {
    let stored =
        load_hold(tx, hold_id, kind, binding)?.ok_or(MemoryReviewerHoldRefusal::Missing)?;
    if stored.released {
        return Err(MemoryReviewerHoldRefusal::Released.into());
    ...
    if stored.purge_degraded {
        return Err(MemoryReviewerHoldRefusal::PurgeDegraded.into());
    }
    if now >= stored.expires_at {
        return Err(MemoryReviewerHoldRefusal::Expired.into());
    }
    Ok(stored)
}
