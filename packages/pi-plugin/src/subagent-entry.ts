 ...
 *
 * `--no-session` children receive `ctx_search`; dreamers receive `ctx_memory` only with `--eidnara-dreamer-actions`.
 ...
 *
 * `--eidnara-dreamer-actions` registers `ctx_memory` with the dreamer action surface.
 */
...
import { openDatabase } from "@eidnara/opencode/features/context/storage-db";
import {
...
const SUBAGENT_DREAMER_ACTIONS_FLAG = "eidnara-dreamer-actions";
...
export default function eidnaraSubagentExtension(pi: ExtensionAPI): void {
    ...
    pi.registerFlag(SUBAGENT_DREAMER_ACTIONS_FLAG, {
        description: "Register ctx_memory with dreamer actions for Eidnara subagents.",
        type: "boolean",
        default: false,
    });
    ...
    pi.on("session_start", async () => {
        try {
            ...
            if (!db) {
                throw new Error("storage open failed; refusing to start without Eidnara tools");
            }
            openedDb = db;
            ...
            registerEidnaraTools(pi, {
                ...
                // `--eidnara-dreamer-actions` registers `ctx_memory` with dreamer actions.
