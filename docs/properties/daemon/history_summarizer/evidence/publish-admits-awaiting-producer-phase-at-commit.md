...
`awaiting_producer` and `publishing` (`history_summarizer.rs:1-6`), and the validation
module claims its work happens "before any side effect can publish it"
(`history_summarizer_validate.rs:1-9`). The commit point's phase gate admits two phases,
...
`crates/daemon/src/history_summarizer.rs:1-6`:

> HistorySummarizer writer orchestration: the durable firing state machine
> (idle -> firing -> awaiting_producer -> validating -> publishing) ...

`crates/daemon/src/history_summarizer_validate.rs:1-9`:

> ... validate it against the raw chunk and already-persisted history_segment ranges
...
that constructed a valid predicate and reached `publish_history_summarizer_chunk` from
...
history_segment-set generation (`:9436-9455`). None of them observes that a
validation happened.
...
In-repo callers of `publish_history_summarizer_chunk`:

- `crates/daemon/src/history_summarizer.rs:529`, the direct call.
...
All three receive a `HistorySummarizerPublishRequest` built at `history_summarizer.rs:513-526`,
...
`publish_history_summarizer_chunk_fails_loud_from_non_publish_state` proves some phase is
...
3. Unvalidated model text is appended as history_segment rows and folded on the next
   pass.

The whole risk premise of Part 4a is that `history_summarizer_validate.rs` is the sole gate.
...
1. Store-level: build a durable `AwaitingProducer` row with a matching predicate
   and a valid identity vector, call `publish_history_summarizer_chunk`, and assert it is
...
2. Module-level: assert that the only construction site of
   `HistorySummarizerPublishRequest` is reached from a `Publishing` state, which is a
...
(remove `| HistorySummarizerPhase::AwaitingProducer`), and the test is the regression
...
- Sources examined: `crates/memory-store/src/lib.rs:9389-9396` and its surrounding
  ...
  `publish_history_summarizer_chunk`; `crates/daemon/src/history_summarizer.rs:299-325`
  ...
  (`publish_history_summarizer_chunk_fails_loud_from_non_publish_state`) to see which phase
  that test uses.
- Findings: `:18221`'s test drives an `Idle` state, so it proves only that `Idle`
  ...
  (`history_summarizer.rs:629-647`), but that path still transitions through `Validating`
