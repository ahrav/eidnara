...
- In [transform-capture.ts](../../../../../packages/opencode-plugin/src/hooks/context/transform-capture.ts),
  a lease's `requestCancel` only aborts its controller (`:534-536`);
  `release` is idempotent, subtracts the charge once, and removes the session
  entry (`:537-543`). `admit` on a held session requests cancellation of the
  holder and returns `session_busy` without taking the slot (`:502-508`).
  The owner's `requestCancel` forwards to the held lease (`:549-551`).
- In [rust-mode-transform.ts](../../../../../packages/opencode-plugin/src/hooks/context/rust-mode-transform.ts),
  `clearSession` (`:1540`) and `invalidateWireState` (`:817-820`) request
  cancellation; the lease signal is passed to every transport call
  (`:1307`) and to the ordinal scan budget (`:1168`); `assertCurrentPass`
  converts an aborted signal into a `superseded` decline (`:961`) and runs
  before each page (`:1298`), after the directory and permission awaits
  (`:1087`, `:1096`), and inside every recheck (`:985`); `run` releases only
  in `.finally` after `execute` settles (`:1526`). On success the publication
  block has already transferred the retained state to `wireCaches` and
  `states` (`:1476-1483`) before that release; the transferred state is
...
- Sources examined: the lease operations at `transform-capture.ts:502-551`,
  the wiring at `rust-mode-transform.ts:817-820`, `:961`, `:1168`, `:1298`,
  `:1307`, `:1526`, `:1540`; the witnesses below.
...
- Conclusion (2026-09-13, revision-bound run after merging `origin/main` at
  ...
  markers:
  - `rust-mode-transform.test.ts:2107` "propagates <supersede|clear|
  ...
  - `:2868` "holds the charge through a slow cancellation until the cancelled
