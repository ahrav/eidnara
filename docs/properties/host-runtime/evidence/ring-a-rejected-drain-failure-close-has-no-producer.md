...
declared at `crates/host-runtime/src/frame_channel.rs:33-48` never appeared in the
...
`docs/host-wire-protocol.md:321`'s authoritative-early-terminal guarantee is
...
- Sources examined: `frame_channel.rs:30-48` (the enum and its blanket
  ...
  `docs/host-wire-protocol.md:321` (the guarantee),
  `docs/shm-transport.md:7` ("There is no runtime transport
...
- Findings: the retention argument requires a future transport that reads a byte
  stream, and `docs/shm-transport.md:7` forecloses exactly that,
...
### Q: Does the ring path satisfy `docs/host-wire-protocol.md:321` or merely evade it?

- Sources examined: `docs/host-wire-protocol.md:321` in full;
