...
begin_claim_store_rebuild" (`crates/memory-store/src/claim_mirror.rs:217-220`). An
...
**The grant.** `begin_claim_store_rebuild` (`crates/memory-store/src/lib.rs:11304-11348`)
...
| Location | Kind |
| --- | --- |
| `crates/memory-store/src/lib.rs:11304` | the definition |
| `crates/memory-store/src/claim_mirror.rs:219` | error message text |
| `crates/memory-store/src/claim_mirror.rs:754` | doc comment on `replace_claim_mirror_snapshot` |
| `crates/memory-store/tests/claim_intent_ledger.rs:299`, `:313` | test |
| `crates/memory-store/tests/claim_mirror.rs:331`, `:454`, `:498` | test |

The facade dispatch table exposes only `claim.mirror.replace` and
`claim.mirror.apply` (`crates/daemon/src/lib.rs:10052-10053`). There is no
...
2. Drive the system only through production entry points — the facade dispatch at
   `daemon/src/lib.rs:10052-10053` and the authority lifecycle methods — and
...
4. Complementary assertion that needs no instrumentation: from a seeded mirror,
   ...
   `daemon/src/lib.rs:13844-13860`; confirm the code a caller sees and record it,
...
- Sources examined: `grep` for the symbol across `crates/` and `packages/`;
  `daemon/src/lib.rs:10040-10060` (facade dispatch); `claim_mirror.rs:219`,
...
- Findings: partly. The reseed's `existing` probe at `:770-776` reads
  `claim_mirror_state` with no incarnation predicate, and that table is a
...
- Missing evidence: none for the mechanism. Whether an operator is expected to
  delete `memory` out of band is a deployment question.
- Conclusion: resolved with answer — no. Because `claim_mirror_state` is a
