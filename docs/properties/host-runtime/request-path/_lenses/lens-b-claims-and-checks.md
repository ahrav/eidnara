...
| 10 | the source module-host work will adapt `Handler` onto this boundary | `handler.rs:3` | **Nothing. No `Handler` exists in this crate**; the trait is `HostHandler` (`:558`) |
...
crates/host-runtime/src/handler.rs` returns `cf281ace` (the commit that added
`host-runtime`) and `ef66e349` ("fix(host-runtime): address PR #6 review findings"), and
`git log -S'```compile_fail' -- crates/host-runtime/src/handler.rs` returns
`cf281ace` and `98b7270d` ("fix(host-runtime): resolve retained PR review
...
miscount it as deleted.** The normative document names `Handler` at nine
sites (`:43`, `:292`, `:596`, `:600`, `:626`, `:634`, `:685`, `:800`, `:906`),
and no such type exists in this crate. This is a **forward** reference, not a
stale one: `handler.rs:3` says "the source module-host work will adapt `Handler`
onto this boundary", so the document describes the adapter that is planned
while the code carries the boundary that exists (`HostHandler`, `:558`).
...
`host-runtime` library.** Everything else it has runs only locally.
...
it. Every `-p host-runtime` test invocation in `ci.yml` carries a `--test <name>`
filter, which selects one integration binary and never builds the lib target:
`:132`, `:133`, `:134-135`, `:178-179`, and `:187`. The remaining `host-runtime`
...
`ci.yml:190` runs `cargo test -p host-runtime --doc` under the step name "Rust
lease non-escape".
...
- Is a zero-length `Response` terminal a defect or a supported outcome? A
  ...
  (needs human input)
- Should `docs/host-wire-protocol.md:673` be corrected to describe
