...
Each `git show` below is scoped to `crates/host-runtime/src/connection.rs`.
...
- Sources examined: `crates/host-runtime/src/connection.rs:306-332` (the match),
  `:371-379` (the enum), `:400-414` and every `Peer` site listed above, `:477`
  (the single propagation point), `:175-176` (the token parentage);
  `crates/host-runtime/src/tcp_frame_channel.rs:162`, `:209`, `:283`; `git show` for all
