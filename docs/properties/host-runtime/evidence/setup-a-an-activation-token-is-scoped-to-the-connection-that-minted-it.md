...
`docs/host-wire-protocol.md:561` calls the token "one-use". The code has no
...
Minting, `crates/host-runtime/src/connection.rs`:
...
Checking, `crates/host-runtime/src/setup_socket.rs:261-284`:
...
- Sources examined: a search for `activation_token` across `crates/host-runtime/src/`
  and `packages/shm-native/src/`.
...
- Findings: none exists. `activate_server` holds it as a `&str` parameter and the
  ...
  returns.
- Missing evidence: none.
- Conclusion: resolved. "One-use" in `host-wire-protocol.md:561` is satisfied
