...
//! `EIDNARA_IPC_BUDGET_ROLE=host` selects the process-isolated echo-host child role.
//! `EIDNARA_IPC_BUDGET_MODE=collect` runs one arm attempt and publishes transactional evidence.
//! `EIDNARA_IPC_BUDGET_MODE=plan` prints the deterministic counterbalanced block schedule.
//!   block schedule.
//! `EIDNARA_IPC_BUDGET_MODE=aggregate` verifies, merges, and summarizes a run directory into byte-stable summary data.
//! `EIDNARA_IPC_BUDGET_MODE=finalize-interrupted` moves running manifests to the `interrupted` terminal state.
...
use atomic::{PingPongConfig, run_ping_pong, timed_exchanges};
use evidence::{
    ARM_ATOMIC, ARM_RING_OPEN, ARM_RING_SERIAL, ARM_RING_THROUGHPUT, ArmId, Attempt, BuildId,
    HistogramConfig, HostId, Manifest, State, counterbalanced_schedule,
};
use linux_topology::{AutoSelection, Class, auto_select, effective_affinity, read_topology};
...
fn main() {
    if env_var("EIDNARA_IPC_BUDGET_ROLE").as_deref() == Some("host") {
        run_child_host();
        return;
    }
    match env_var("EIDNARA_IPC_BUDGET_MODE").as_deref() {
        ...
        Some(other) => {
            eprintln!("unknown EIDNARA_IPC_BUDGET_MODE {other:?}");
            std::process::exit(1);
        }
        None => {}
    }
...
fn run_child_host() {
    if let Some(cpu) = env_var("EIDNARA_IPC_BUDGET_HOST_CPU") {
        let cpu: u32 = cpu.parse().expect("EIDNARA_IPC_BUDGET_HOST_CPU");
        linux_topology::pin_current_thread(cpu).expect("host pin");
    }
    let data_dir = PathBuf::from(env_var("EIDNARA_IPC_BUDGET_DATA_DIR").expect("data dir"));
    ...
    runtime.block_on(async move {
        let config = host_runtime::HostConfig {
            ...
            daemon_ver: "eidnara-host/ipc-budget".to_owned(),
            ..Default::default()
        };
        ...
        let host = tokio::spawn(host_runtime::run(
...
impl ChildHost {
    fn spawn(host_cpu: Option<u32>) -> Result<Self, String> {
        ...
        cmd.env("EIDNARA_IPC_BUDGET_ROLE", "host")
            .env("EIDNARA_IPC_BUDGET_DATA_DIR", data_dir.path())
            .env_remove("EIDNARA_IPC_BUDGET_MODE")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit());
        match host_cpu {
            Some(cpu) => cmd.env("EIDNARA_IPC_BUDGET_HOST_CPU", cpu.to_string()),
            None => cmd.env_remove("EIDNARA_IPC_BUDGET_HOST_CPU"),
        };
...
fn read_collect_config() -> Result<CollectConfig, String> {
    let out =
        PathBuf::from(env_var("EIDNARA_IPC_BUDGET_OUT").ok_or("EIDNARA_IPC_BUDGET_OUT unset")?);
    let arm = env_var("EIDNARA_IPC_BUDGET_ARM").ok_or("EIDNARA_IPC_BUDGET_ARM unset")?;
    if ![
        ARM_ATOMIC,
        ARM_RING_SERIAL,
        ARM_RING_OPEN,
    ...
    let class = Class::parse(&env_var("EIDNARA_IPC_BUDGET_CLASS").unwrap_or("same-l3".to_owned()))?;
    let explicit_pair = match env_var("EIDNARA_IPC_BUDGET_PAIR") {
        Some(spec) => {
            let (a, b) = spec
                .split_once(',')
    ...
    env_parse::<u64>("EIDNARA_IPC_BUDGET_RATE", 0)?;
    Ok(CollectConfig {
        out,
        arm,
        class,
        explicit_pair,
        block: env_parse("EIDNARA_IPC_BUDGET_BLOCK", 1)?,
    })
}
...
fn host_id() -> HostId {
    ...
    HostId {
        hostname: env_var("EIDNARA_IPC_BUDGET_HOST_LABEL").unwrap_or_else(|| {
            // are empty.
...
fn build_id() -> BuildId {
    BuildId {
        commit: env_var("EIDNARA_IPC_BUDGET_COMMIT").unwrap_or_else(|| "unknown".to_owned()),
        rustc: env_var("EIDNARA_IPC_BUDGET_RUSTC").unwrap_or_else(|| "unknown".to_owned()),
...
fn base_manifest(cfg: &CollectConfig, pair: Option<(u32, u32)>) -> Manifest {
    Manifest {
        ...
        collection: if cfg.arm == ARM_RING_OPEN {
            let rate =
                env_parse("EIDNARA_IPC_BUDGET_RATE", 0u64).expect("rate validated at config load");
...
fn collect_atomic(attempt: &mut Attempt, pair: (u32, u32)) -> Result<(), String> {
    let cfg = PingPongConfig {
        ...
        warmup_batches: env_parse("EIDNARA_IPC_BUDGET_WARMUP_BATCHES", 50)?,
        batches: env_parse("EIDNARA_IPC_BUDGET_BATCHES", 200)?,
        exchanges_per_batch: env_parse("EIDNARA_IPC_BUDGET_EXCHANGES", 10_000)?,
    };
...
fn collect_ring_serial(attempt: &mut Attempt, pair: (u32, u32)) -> Result<(), String> {
    let cfg = ring::SerialConfig {
        warmup_ops: env_parse("EIDNARA_IPC_BUDGET_WARMUP_OPS", 20_000)?,
        measured_ops: env_parse("EIDNARA_IPC_BUDGET_MEASURED_OPS", 120_000)?,
...
fn collect_ring_open(attempt: &mut Attempt, pair: (u32, u32)) -> Result<(), String> {
    let rate = env_parse("EIDNARA_IPC_BUDGET_RATE", 0u64)?;
    let cfg = ring::OpenLoopConfig {
        rate_per_sec: rate,
        warmup: Duration::from_secs(env_parse("EIDNARA_IPC_BUDGET_WARMUP_SECS", 2)?),
        measure: Duration::from_secs(env_parse("EIDNARA_IPC_BUDGET_MEASURE_SECS", 10)?),
        inflight_cap: env_parse("EIDNARA_IPC_BUDGET_INFLIGHT_CAP", 1024)?,
        histogram: HistogramConfig::default(),
    };
    let mut host = ring_arm_setup(attempt, pair)?;
...
fn collect_ring_throughput(attempt: &mut Attempt, pair: (u32, u32)) -> Result<(), String> {
    let cfg = ring::ThroughputConfig {
        depth: env_parse("EIDNARA_IPC_BUDGET_DEPTH", 32)?,
        warmup: Duration::from_secs(env_parse("EIDNARA_IPC_BUDGET_WARMUP_SECS", 2)?),
        measure: Duration::from_secs(env_parse("EIDNARA_IPC_BUDGET_MEASURE_SECS", 10)?),
    };
    let mut host = ring_arm_setup(attempt, pair)?;
...
fn run_record_plan() -> i32 {
    let out = PathBuf::from(match env_var("EIDNARA_IPC_BUDGET_OUT") {
        Some(out) => out,
        None => {
            eprintln!("EIDNARA_IPC_BUDGET_OUT unset");
            return 1;
        }
    });
    let plan = (|| -> Result<serde_json::Value, String> {
        let blocks = env_parse("EIDNARA_IPC_BUDGET_BLOCKS", 10u32)?;
        let rates: Vec<u64> = env_var("EIDNARA_IPC_BUDGET_RATES")
            .unwrap_or_else(|| DEFAULT_RATES.to_owned())
            .split_whitespace()
            .map(|token| {
                token
                    .parse()
                    .map_err(|_| format!("EIDNARA_IPC_BUDGET_RATES: malformed rate {token:?}"))
            })
            .collect::<Result<_, _>>()?;
        Ok(serde_json::json!({
...
fn aggregate(run_dir: &Path) -> Result<String, String> {
    ...
    if unfinalized > 0 {
        return Err(format!(
            "{unfinalized} attempt(s) hold only a running manifest; run \
             EIDNARA_IPC_BUDGET_MODE=finalize-interrupted over this directory first"
        ));
    }
    ...
    if !plan_path.is_file() {
        return Err(format!(
            "{}: run-plan.json is missing; rerun EIDNARA_IPC_BUDGET_MODE=record-plan with this \
