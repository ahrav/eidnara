//! The History Summarizer's reserve, stage, activate handoff of accepted facts to MemoryReviewer review (KTD3).
//!
...
use memory_store::{
    ExtractionFailure, HistorySummarizerDurableState, MemoryReviewerNonadmissionCode,
    MemoryReviewerReservation, MemoryStore,
};
...
pub enum Handoff {
    ...
    Nonadmission(MemoryReviewerNonadmissionCode),
...
pub struct PreparedActivation {
    ...
    pub input: MemoryReviewerJobInput,
...
pub enum HandoffError {
    #[error("memory_reviewer reservation: {0}")]
    Reserve(#[from] MemoryReviewerJobError),
...
pub fn expired_activation(
    ...
    let job = store
        .lookup_memory_reviewer_job(project, &reservation.causal_identity)
        .map_err(MemoryReviewerJobError::Store)?
        .ok_or(MemoryReviewerJobError::Refused(
...
fn reserved_row(
    ...
    let causal_identity = inputs
        .causal_identity()
        .map_err(MemoryReviewerJobError::Refused)?;
    let held = request
        .firing
        ...
        .map(|held| {
            request
                .store
                .lookup_memory_reviewer_job(request.project, &held.causal_identity)
                .map_err(MemoryReviewerJobError::Store)
        })
        .transpose()?
        .flatten();
    if let Some(job) = held {
        return Ok(match job.state {
            MemoryReviewerJobState::Reserved if job.producer == *producer => {
    ...
    let existing = match request.store.reserve_memory_reviewer_job(
        ...
        Err(MemoryReviewerJobError::Store(_)) => {
            return Ok(ReservedRow::Done(Handoff::Nonadmission(
...
fn adopt(
    ...
    match job.state {
        ...
        MemoryReviewerJobState::Reserved if job.producer.producer == producer.producer => {
            match request.store.rebind_reserved_memory_reviewer_job(
                request.project,
                &job.causal_identity,
                producer,
                request.now_ms,
...
pub fn reserve_and_stage(
    target: &HandoffTarget,
    request: &HandoffRequest<'_>,
    persist: impl FnOnce(&MemoryReviewerReservation) -> Result<u64, HistorySummarizerStateError>,
    ...
    let row_version = persist(&MemoryReviewerReservation {
