...
actually sits. Tracing every line that touches history_segment text put the floor at
one non-whitespace character.

## Evidence trail

Three content-touching sites exist in the whole module. There are no others in
`crates/daemon/src/history_summarizer_validate.rs:1-1304`.
...
1. `:297-303` — the title. `capture_string(attr_title_regex(), attrs)` matched
   ...
   `continue`, a silent drop of the history_segment, not a rejection.
2. `:308-331` — tier presence. `extract_tier(inner, 0)` is filtered by
   ...
   one-character `p1` produces a history_segment with `p1 == p2 == p3 == "."`.
3. `:1000-1008` — the only content REJECT in the module:

```rust
match history_segment.p1.as_deref() {
    Some(p1) if !p1.trim().is_empty() => {}
    _ => { return Some(format!("history_segment {} is missing the tiered ...", index + 1)); }
}
```
...
- No maximum either. The only size defence is the producer-side truncation flag
  `output.length_capped`, refused at `history_summarizer.rs:1666-1671`, which detects a
...
`to_stored_history_segment` at `history_summarizer.rs:38-67` clones `title`, `content`, and
...
<history_segment start="1" end="500" title="Work" episode_type="feature" importance="50"><p1>.</p1></history_segment>
</history_segments><meta><unprocessed_from>501</unprocessed_from></meta></output>
```

Gate walk: check 1 passes (single root); check 7 passes (one history_segment);
...
single history_segment starts at the first present ordinal); check 18 falls through
to `:1063` and `501 == 500 + 1` passes; check 22 passes.

Discard-last does NOT fire: `:539` requires `history_segments.len() >= 2` and there
is one. So `discarded_last` stays false and the history_segment publishes.
...
`chunk_transcript` and `raw_chunk_messages` (`history_summarizer.rs:1726-1727`), the
...
session grows large enough to trigger the history_summarizer.

Two amplifiers, both verified:

- The single-history_segment shape also escapes the lookahead protection
  (`:539`), so the weakest content AND the weakest boundary coincide. See
  `hv-single-history_segment-skips-lookahead-discard.md`.
- Because `keep_side_channel` returns `true` for unanchored items when
  ...
  while the history_segment says nothing.
...
3. Assert `Ok`, and assert `validated.history_segments[0].p1.as_deref() == Some(".")`
...
### Q: Does the project want a span-relative floor, or is body adequacy delegated to the history_summarizer-eval lane?

- Sources examined: `.github/workflows/ci.yml:402-440` (source-catalog line, not present at HEAD) (the
  `history_summarizer-eval-contracts` job and its comment block),
  `.github/workflows/history_summarizer-eval.yml:1-70` (source-catalog path, not present at HEAD) (the live lane's header),
  `history_summarizer_validate.rs:1-9` (the module's own statement of what it validates).
...
- Conclusion: needs human input. The architecture plausibly delegates adequacy to
  ...
  (`history_summarizer-eval.yml:12-26`), so nothing blocks a degenerate output at publish
  time on a real session.

### Q: Is the raw conversation genuinely recoverable after a degenerate publish?

- Sources examined: `history_summarizer.rs:428-437` (the `ValidatedPublishRequest` fields
  and their doc comments), `history_summarizer.rs:1726-1727` (the call site),
  `history_summarizer.rs:1-6` (the module header's claim that publish writes surface only
