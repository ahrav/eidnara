...
- Discovery at U3 against the host repo at `39e8230`. The crate is a port of the `ai-tokenizer` claude encoding. Its contract is
...
Existing check: `bom_before_newline_is_preserved` and, for the rank half, `encode_ordinary_matches_ai_tokenizer_ids`; unaudited. Together they compose the guarantee: the first pins the piece shape (three standalone tokens for `"x\u{feff}\n"`) against the crate's own single-character encodings, and the second pins ids 57538, 92, and 203 against the oracle-produced golden on cases that carry them, so a BOM-stripping regression fails the first and a BOM-rank change fails the second. What remains partial is that no single test asserts the literal `[92, 57538, 203]` against the asset, so the two halves are joined by argument rather than by one assertion. The first test's oracle is circular for the piece shape only: a renumbered `EF BB BF` row is already caught by `encode_ordinary_matches_ai_tokenizer_ids` on the three BOM golden cases, where id 57538 is pinned against the oracle.
