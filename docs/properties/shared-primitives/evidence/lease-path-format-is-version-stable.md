...
- **Cross-crate evidence:** PostgreSQL imports public `fnv1a` and hashes public `LeaseKey::identity` at `commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:11`,54-59`.
...
- **New evidence:** `lease_path_vectors_are_version_stable` (`crates/lease/src/lib.rs:1206-1260`) pins eight keys against digests computed by an FNV-1a-64 implementation outside the crate (the source's own `51a7eaa424b9fd8f` vector reproduced independently first): the two production keys `("magic-context-kernel", "sqlite", "core")` → `1a0ede79732fcf81` and `("magic-context", "sqlite", "mc_cache")` → `3af1f17c55068a4d`, empty fields, non-ASCII fields, a `U+001F` inside a field, and a 300-byte field. It also acquires the kernel key and asserts the directory then contains exactly `1a0ede79732fcf81.lease`.
