...
| --- | --- | --- |
| T1 vocabulary or pre-tokenizer drift | A rank, token byte sequence, or pattern change that keeps counts equal but changes ids | **Yes** - the committed golden was produced by the oracle, not the crate, so any drift fails `encode_ordinary_matches_ai_tokenizer_ids` |
...
| --- | --- | --- |
| tokenizer-encoding-matches-the-independent-oracle | T1, with a golden the crate did not produce | Yes - 36 oracle-produced cases |
