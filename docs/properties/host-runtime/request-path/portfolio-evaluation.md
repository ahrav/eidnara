...
`the source repository `ci.yml` workflow:105-200`. One correction to the brief: it cites the
...
`cargo nextest run -p host-runtime --test client --test lifecycle` under
...
`GenerationCore`" is refuted by a `grep -n 'GenerationCore {' crates/host-runtime/src/`.
...
- Any workflow change that names one of the six subject binaries, or runs the
  `host-runtime` lib target. `tests/dispatch.rs` alone holds 20 tests that are the real
