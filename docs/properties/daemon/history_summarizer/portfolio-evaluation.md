...
Applied in `catalog.md`, replacing the "Unresolved: is the history_summarizer reachable by
default?" subsection with "Resolved: the history_summarizer is reachable by default", and
...
- Both setup paths call it unconditionally for the history_summarizer role and carry the
  ...
  `/history_summarizer/model` pointer `config.rs:411` reads (`setup-pi.ts:242-246`).
...
no step turns it off, because the history_summarizer model is a required answer mid-setup
...
`setup-pi.ts:219` declares `history_summarizerModel: string`, so its write at `:242-246` is
unconditional and the guarantee is in the signature. `setup-opencode.ts:237`
declares `string | null` and guards the write with `if (options.history_summarizerModel)`
...
wrong twice over. `history_summarizer.rs:1738` is inside `abandon_current_state`'s
signature, and the secondary citation `:471-475` is the events projection. The
only production `to_stored_history_segment` call is `history_summarizer.rs:466`, inside
...
call sits inside a `.map` over every accepted history_segment, which is exactly why
...
| `hv-heal-extends-range-without-revalidating-content` | Only a history_segment whose `end_message` was healed |
| `hv-single-history_segment-skips-lookahead-discard` | Only a chunk whose narrative ends within `BOUNDARY_HEALING_SLACK` of the chunk end |
...
`#[tokio::test(...)]` attributes, and all five are right: 51 in `history_summarizer.rs`, 19
in `history_summarizer_validate.rs`, 19 in `history_summarizer_chunk.rs`, 18 in
`history_summarizer_producer.rs`, 34 in `lib.rs`. The total is not. 51 + 19 + 19 + 18 + 34
is 141.
...
(`history_summarizer.rs:1318-1321`) back to `:1256` and calls `fire` again (`:1265-1274`),
...
The check is now stated per `run_history_summarizer_firing` call, summing `attempted` and
...
(`history_summarizer.rs:257`)". The record contradicted itself across two adjacent fields,
...
after `append_history_segments_tx` (`:9457-9471`), `insert_chunk_transcripts_tx`
(`:9472-9481`), and `enqueue_history_summarizer_side_channels_tx` (`:9482`) have already
...
sit after `append_history_segments_tx`. That needs a targeted read of each post-write
return.
...
- **Backoff expiry.** The cooldown gate compares the durable
  ...
  with `now` arriving through `HistorySummarizerPrepareContext` into
  `prepare_history_summarizer_fire` (`:4808-4821`) rather than being read inside the gate.
  The helper `expire_history_summarizer_backoff` (`:29784-29791`) expires it by committing
  `Some(now_ms() - 1)`, and
...
so (`history_summarizer_validate.rs:5-9`) and the signature confirms it, taking only `text`,
`chunk`, `prior_history_segments`, and `options` with no store, clock, filesystem, or
...
- `history_summarizer-single-flight-admits-one-publish-per-firing` was routed to a
...
`validate_history_summarizer_output` with an output fixture from unrelated conversation B
whose history_segment ranges are renumbered contiguous over
...
(`history_summarizer_chunk.rs:373-375`) and the body loop `continue`s on
...
1. **Whether this catalog is a risk-selected slice or owes representative coverage
   ...
   about 13.5k lines across five files (`history_summarizer.rs` 4,682,
   `history_summarizer_producer.rs` 2,306, `history_summarizer_chunk.rs` 2,051,
   `history_summarizer_validate.rs` 1,869, plus two `lib.rs` regions), and the records
