...
| release-leaves-the-consumer-parked-marker-intact | An armed consumer, a release, then a publication; F4 for the release and publish on different threads | **Yes** in-process - `release_leaves_the_consumers_data_wait_armed_for_the_next_publish` (`ring.rs:3737`) asserts the marker survives the release and the next publish wakes the consumer; single-threaded |
