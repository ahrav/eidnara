...
| --- | --- | --- |
| [export-fixed-s-exactly-once](catalog.md#export-fixed-s-exactly-once) | Multiple pages, mutations between pages, class boundary, historical invalidation, in-place domain-name remediation. | Fixture ledger `E(S)`, exact key/byte comparison or abort when required S bytes are unavailable; `search_projection_export_writes_between_pages`, `search_projection_export_operator_remediation_during_snapshot`. |
| [export-retention-fence-covers-read](catalog.md#export-retention-fence-covers-read) | Prune under valid fence and lost source/history coverage during build. | Durable consumer/fence witness, required-source availability, selector exclusion, cleanup outcome; `search_projection_export_prune_with_fence`, `search_projection_export_retention_lost_mid_build`. |
| [export-predecode-bounds](catalog.md#export-predecode-bounds) | Over-cap row, residual page-byte edge, exact boundary, JSON expansion. | Size admission before materialization, decoder entries, separately observed live decoded heap; `search_projection_export_oversize_row_present`, `search_projection_export_page_budget_edge`. |
| [catchup-complete-commit-prefix](catalog.md#catchup-complete-commit-prefix) | Split commit, published retained rows, duplicate delivery, empty commit, `operator_remediation`. | Complete canonical transaction ledger, per-ordinal application, durable prefix; `search_projection_catchup_split_commit`, `search_projection_catchup_published_retained`, `search_projection_catchup_empty_commit`, `search_projection_export_operator_remediation_during_snapshot`. |
| [ack-follows-local-release](catalog.md#ack-follows-local-release) | Crash after local commit but before ack, contended kernel writer, lost ack response. | Ownership event order and independent durable reads; `search_projection_ack_local_commit_interrupted`, `search_projection_ack_writer_contended`, `search_projection_ack_response_lost`. |
| [replacement-selects-complete-compatible-state](catalog.md#replacement-selects-complete-compatible-state) | Reader during old/new switch, partial staging, selector failure/restart. | One selected identity per read, `O(T)`, compatible tuple, consumer-release order; `search_projection_replacement_switch_interrupted`. |
| [rebuild-after-pruning-converges](catalog.md#rebuild-after-pruning-converges) | Search deletion after confirmed historical pruning, finite concurrent work then quiet window. | Independent canonical `O(T)`, stable T, approved recovery/work bounds; `search_projection_rebuild_missing_db_after_prune`, `search_projection_rebuild_quiet_window_admitted`. |
| [catchup-and-authorized-recovery-converge](catalog.md#catchup-and-authorized-recovery-converge) | Healthy finite normal backlog plus a separate explicitly authorized Disabled recovery with every gate accepted. | Per-admitted-episode Current/coverage/checkpoint observation within RP2.9 bounds; `search_projection_healthy_lagged_target_admitted`, `search_projection_authorized_recovery_current_observed`. |
| [disable-preserves-consumer-obligations](catalog.md#disable-preserves-consumer-obligations) | Lagging disable, last/non-last removal, outstanding deletion barrier, restart. | Consumer rows, audit records, serving outcomes per surface; `search_projection_disable_pending_consumer`, `search_projection_disable_last_consumer`, `search_projection_abandon_request_with_barrier`. |
| [recovery-preserves-canonical-authority](catalog.md#recovery-preserves-canonical-authority) | Stale valid projection, each identity mismatch, canonical unavailability, authorized remediation. | Independent canonical mutation ledger and final verdicts; `search_projection_recovery_stale_authority`, `search_projection_recovery_contract_mismatch`, `search_projection_recovery_authority_unavailable`. |
...
| --- | --- |
| `search_projection_export_writes_between_pages` | One export has returned a nonfinal page, and a canonical mutation affecting its source domain commits before a later page request at the same S. |
| `search_projection_export_operator_remediation_during_snapshot` | An export is active at S when a domain-name `operator_remediation` commits after S with unchanged source revision, before its catch-up target T is fixed. The fixture records whether approved export mapping consumes that field. |
| `search_projection_export_prune_with_fence` | A durable bootstrap registration exists, unread export input remains, and an outbox prune is attempted before export completion. |
| `search_projection_export_retention_lost_mid_build` | An incomplete candidate exists and the harness invalidates its retention witness or removes required source bytes before the next export/catch-up/publication decision. |
| `search_projection_export_oversize_row_present` | The fixture ledger has a row larger than the declared standalone cap and export requests the page whose next key is that row. |
| `search_projection_export_page_budget_edge` | A page has admitted input, and its next row fits alone but exceeds remaining row/byte capacity. |
| `search_projection_catchup_split_commit` | A commit with multiple ordinals after S exceeds the declared batch cut; the first partial batch is delivered and remaining ordinals are available. |
| `search_projection_catchup_published_retained` | An event after S is marked published, remains stored above the consumer checkpoint, and the consumer requests catch-up through its commit. |
| `search_projection_catchup_empty_commit` | A commit-log entry with zero outbox rows lies after S and at or before requested T. |
| `search_projection_ack_local_commit_interrupted` | The observer records a batch's successful local COMMIT, then the process is terminated before its kernel ack is attempted. |
| `search_projection_ack_writer_contended` | Another operation holds the kernel writer when a locally committed batch attempts its ack. |
| `search_projection_ack_response_lost` | Durable kernel ack COMMIT is observed and the harness loses its response before the consumer observes completion. |
| `search_projection_replacement_switch_interrupted` | A selected old projection and a staged candidate exist, a read is requested during switching, and execution is interrupted at a declared selector boundary. |
| `search_projection_rebuild_missing_db_after_prune` | Independent reads prove pre-S outbox history was pruned, all local search state is removed, and retained canonical source remains nonempty when recovery starts. |
| `search_projection_rebuild_quiet_window_admitted` | After a fault/load phase, an episode starts with a fixed finite T, retained inputs within approved caps, available dependencies, and injection/new-write pressure stopped for the approved window. |
| `search_projection_healthy_lagged_target_admitted` | An ordinary CatchingUp episode is admitted with local complete prefix below finite T, healthy stores/workers, retained inputs, all prerequisite/gate acceptances, approved RP2.9 caps/bound, and injection/new writes stopped for the interval. |
| `search_projection_authorized_recovery_current_observed` | A previously Disabled controller has recorded explicit recovery authorization and all prerequisite/gate acceptances, admitted a healthy finite-target episode, and is subsequently observed Current through that episode's T. This positive outcome is observed independently of the deadline assertion. |
| `search_projection_disable_pending_consumer` | Disable is requested with retrieval's checkpoint below the kernel pre-operation tip. |
| `search_projection_disable_last_consumer` | A safely caught-up retrieval consumer is the only registered consumer when its deregistration is requested. |
| `search_projection_abandon_request_with_barrier` | An authorized abandonment request names a consumer recorded by an incomplete deletion barrier. |
| `search_projection_recovery_stale_authority` | A valid old projection names an occurrence subsequently invalidated or revised canonically before a recovery/egress attempt. |
| `search_projection_recovery_contract_mismatch` | A persisted projection tuple differs from the active tuple on a selected schema, tokenizer, model, policy, or identity dimension when opened. |
| `search_projection_recovery_authority_unavailable` | Recovery or egress is attempted while the canonical authority read is deliberately failed. |

This table is the single definition site for all 23 markers, including
`search_projection_ack_local_commit_interrupted` and `search_projection_ack_response_lost`. The projection
