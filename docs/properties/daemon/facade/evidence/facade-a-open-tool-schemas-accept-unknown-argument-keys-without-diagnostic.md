...
- `:15790-15924` `eidnara_memory_schema`, root `"additionalProperties": true` at
  `:15846`. Two nested objects are closed: `mutation_token` at `:15793` and the
  anti-memory arm at `:15829`.
- `:15926-15948` `eidnara_search_schema`, `"additionalProperties": true` at
  `:15929`.
- `:15948-15958` `ctx_expand_schema`, `"additionalProperties": true` at
  `:15950`.
- `:15960-15975` `eidnara_note_schema`, `"additionalProperties": true` at `:15963`.
...
- `:25632-25641` — for every advertised tool except `eidnara_reduce`:
...
`{"action":"write","content":"...","importance":90}` on `eidnara_note` receives
...
The `eidnara_reduce` exception makes the inconsistency concrete. Its advertised
...
(`config.rs:124`) means the `eidnara_memory` gate at `lib.rs:10608-10610` is open by
default too.

## What a test must construct

1. Bind a facade route and install a store.
2. For each of `eidnara_memory`, `eidnara_search`, `ctx_expand`, `eidnara_note`, call the
...
3. A stronger form: derive the read set mechanically. For each tool, take the
   ...
   test would currently fail on `eidnara_note` (`memory_project` is declared and
   read, `command_id` is read and undeclared) and on `eidnara_reduce`, which is the
   useful signal.
...
### Q: `eidnara_reduce`'s advertised schema is closed yet the handler accepts `command_id` and the reduced envelope. Which side is the contract?

- Sources examined: `prompt_surface.rs:188-205`, the whole `eidnara_reduce` `Tool`
  ...
  manifest test's explicit `eidnara_reduce` exemption; `lib.rs:14419-14435`, the
...
- Findings: the comment at `prompt_surface.rs:195-196` says "This exact
  ...
  and one of them, `command_id`, it reads only to ignore: `handle_eidnara_reduce_facade`
...
- Missing evidence: what "Thalamus" validates the advertised shape against, and
  whether an authorization layer outside this repository rejects a `eidnara_reduce`
...
- Conclusion: needs human input. If the advertised closed shape is enforced
  upstream, then `command_id` on `eidnara_reduce` is unreachable in production and
