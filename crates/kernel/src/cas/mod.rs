...
/// `retention_class` of evidence a MemoryReviewer run captured for itself; an ingest request for such a row must carry a finite `retain_until`, and a new reference commits only while that deadline is still ahead of the store clock. An identical request whose receipt already committed replays regardless.
pub const MEMORY_REVIEWER_CAPTURE_RETENTION_CLASS: &str = "memory_reviewer_capture";
