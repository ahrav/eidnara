...
- **Primary evidence:** `wait_until_handled` (`packages/shm-native/src/scheduling.rs`) blocks on the control eventfd and returns only once the pending flag is cleared, so a wake that arrives while a callback is still pending does not release the waiter. `retry_interrupted` loops on `EINTR` and returns `None` once the closing flag is set, so an interrupted wait is never reported as readiness.
...
- **Failure scenario:** a wake delivered before acknowledgement runs two callbacks for one readiness, or an `EINTR` surfaced as a wake makes the JavaScript side poll an empty ring.
