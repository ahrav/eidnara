//! The wire shape matches JSON `HistorySummarizerProducer` sends and parses.
...
use host_runtime::model_execution::protocol::{self, Request};
use host_runtime::model_execution::subprocess::EnvSnapshot;
use host_runtime::model_execution::{ModelExecutionComponent, config};
use host_runtime::{BindOutcome, CompositeComponent, RouteHandle, RouteIdentity};

use support::model_execution::{
    ScriptedBackend, call, drain_subscribe, open_model_execution_route, send_call, send_params,
    start_model_execution_host,
};
...
fn each_valid_operation_decodes_its_exact_schema() {
    ...
    assert_eq!(
        protocol::parse_request(
            &body(serde_json::json!({
                "method": "run.status",
                "params": { "run_id": "model_execution-abc-1" },
            })),
            false,
        )
        .expect("status decodes"),
        Request::Status {
            run_id: "model_execution-abc-1".to_owned()
        }
    );
    assert_eq!(
        protocol::parse_request(
...
fn error_unit_stays_within_terminal_headroom_after_json_escaping() {
    use host_runtime::model_execution::backend::{BackendError, ErrorClass};

    // Admission headroom must accommodate the worst-case encoding of two diagnostic fields plus the `run_started` unit that shares the reserved slice.
...
async fn health_reports_unavailable_when_no_harness_can_run() {
    ...
    assert_eq!(
        ...
        (host_runtime::HealthStatus::Ok, "ready".into()),
        "one runnable harness keeps ModelExecution ready"
    );
    assert_eq!(
...
async fn credential_snapshot_must_match_before_backend_spawn() {
    ...
    let (bad_ch, bad_ep) = open_model_execution_route(&mut client, "opencode", "bad").await;
    let rejected = call(
        &mut client,
        bad_ch,
        bad_ep,
...
async fn five_operation_round_trip_matches_the_consumed_wire_shapes() {
    ...
    let host = start_model_execution_host(component).await;
    let mut client = host.client().await;
    let (command_ch, command_ep) =
        open_model_execution_route(&mut client, "opencode", "hist:1").await;
    let (sub_ch, sub_ep) = open_model_execution_route(&mut client, "opencode", "hist:1").await;
    ...
    assert_eq!(
        ...
        serde_json::json!([{ "type": "text", "text": "history_summarizer output" }])
    );
    ...
    // A cancel of an already-terminal run is idempotent: it acknowledges without rewriting the committed `completed` state.
    let status = call(
        &mut client,
        command_ch,
        command_ep,
...
async fn malformed_requests_over_the_host_create_no_run_state() {
    ...
    let host = start_model_execution_host(component).await;
    let mut client = host.client().await;
    let (channel, epoch) = open_model_execution_route(&mut client, "pi", "s1").await;
