...
- **Existing evidence:** `soft_does_not_advance_anchor_while_reconcile_pending` (`crates/cache-stability/src/lib.rs:572-609`) reverts, sends a coverage-extending `Soft`, and asserts the anchor holds and the next absent defer still reconciles; `coverage_extending_soft_advances_anchor_keeps_m0_frozen` (`crates/cache-stability/src/lib.rs:502-570`) asserts the positive case and that m0 bytes stay frozen.
