...
| The association's `created_commit_seq` was not compared with its row's, the one `AssociationRow::decode` check the read still lacked | gap | fixed: refused as `CorruptRow`; `an_association_created_in_another_commit_than_its_row_is_refused` |
| The incarnation check compared the projection identity with a caller-supplied string and captured the target only after the row read, so a restore during the row read, or a caller naming the wrong kernel, went undetected | gap | fixed: the kernel's own `database_incarnation_id` is compared, and the incarnation is captured before and after the row read as the daemon's `checked_target` does; `a_projection_from_another_kernel_incarnation_is_refused_before_any_read` |
| `classify_live_claims` accepted no budget, so reader contention or a long facts read could not be cancelled or bounded | gap | fixed: `budget: &EvalBudget` drives target capture, identity read, and `claim_facts_at_within_budget`; `an_exhausted_budget_is_refused_before_the_projection_is_read` |
| The `TooManyClaims` bound test faulted the tip read to show the bound ran before any kernel read; the incarnation bracket now reads the tip first | refinement | the bound is enforced before the facts read; the test faults `object_registry` so the facts read fails while tip and identity reads succeed |
| A kernel restored from an older backup of the same database keeps its identity and generation, so rows the projection applied past the restored tip classified `Retracted` instead of the read refusing two histories | gap | fixed: the projection checkpoint is read with the identity and a target behind it is `KernelBehindProjection`, as `daemon::coverage` refuses it; `a_kernel_tip_behind_the_projection_checkpoint_is_refused` |
| The budgeted facts read polled cancellation only after every requested object, so many short indexed reads could outrun an interrupt | refinement | fixed: `limit.check()` per object, as the `commit_read` loops do |
| A kernel restored from an older backup that then accepts divergent commits past the projection checkpoint passes the identity, generation, and tip checks | bias | kept: the checkpoint's `hold_id` is the durable binding to one kernel history, and the daemon validates it with `source_hold_status_within_budget` under its consumer binding (`Missing` on a restored kernel) as `search_replacement` does before its reads; the classifier has no consumer binding and no production caller yet |
| A projection row whose `payload_id` or source triple was rewritten to another valid value while its identity tuple stayed intact is not compared with the canonical `ClaimOccurrence` | bias | kept: the classifier derives state from identity; payload and provenance agreement is the payload read path's check and applies to every occurrence class, not to claims alone |
