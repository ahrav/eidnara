...
`docs/migration-version-lanes.md:9-20` (source-catalog path, not present at HEAD) defines format identity as six things
...
- `storage/src/lib.rs:341-349` creates
  `schema_version(namespace, version, applied_at_unix,
  PRIMARY KEY (namespace, version))`.
- `:375-380` inserts one row per applied migration.
- `crates/memory-store/src/lib.rs:401` `const NS: &str = "memory"` is this
  crate's namespace.
- `lib.rs:5348-5358` `module_store_schema_version` reads
  `SELECT COALESCE(MAX(version), 0) FROM schema_version WHERE
...
- `PRAGMA user_version` is never set. `docs/migration-version-lanes.md:13` (source-catalog path, not present at HEAD) lists
...
- `PRAGMA application_id` is never set on `store.db`. `sqlite_runtime.rs:11-12`
  defines `KERNEL_APPLICATION_ID = 0x4549_444E`; the only code that *applies* it is
  TypeScript, at `packages/cli/src/commands/doctor-repair-db.ts:355` (source-catalog path, not present at HEAD).
- `format_marker` (`sqlite_runtime.rs:17`) is never created by Rust. The only
  references outside the constant and its test are TypeScript:
  `packages/cli/src/commands/doctor-repair-db.test.ts:321` (source-catalog path, not present at HEAD) and
  `packages/plugin/src/features/eidnara/memory/fixtures/claim-operations-crash-worker.ts:49` (source-catalog path, not present at HEAD).
...
- Neither has a production caller. A content search returns definitions plus
  `crates/memory-store/tests/sqlite_runtime.rs:87` (source-catalog path, not present at HEAD) and `:94, :102`.

The version-reading guard:

- `lib.rs:1346-1366` `recorded_cache_version`. `:1348-1355` checks whether
  `schema_version` exists in `main.sqlite_schema` and returns `None`
...
**2. Recorded zero read as pristine.** A store whose `schema_version`
contains `("memory", 0)` is mapped to `None` by `lib.rs:1364`.
`refuse_pre_cutover_store` then falls through to `Ok(())` (`:1383`) and
`run_migrations` computes `current = 0` (`storage:351-357`) and applies
the bootstrap. If any bootstrap object already exists, the first
`CREATE TABLE cache_state` (`lib.rs:435`) fails with the raw DDL-collision
...
(`storage:363-365`). So a `store.db` written by a binary shipping
...
**4. Post-migration repairs are not version-recorded.** `MemoryStore::open` runs
...
- Case 2 needs a store seeded with a literal `0` version row, which no code path
  in this crate produces (`storage:378` inserts `m.version`, which is
  57). It would come from an external writer or a historical binary.
- Case 4's window is between the migration commit
  (`storage:381`) and the repair flag commit (`lib.rs:5105-5112`).
...
1. `MemoryStore::open` a temp store, assert `module_store_schema_version() == 57`.
2. Drop the store, open a raw `rusqlite::Connection` on the same path, and
   `DROP TABLE tags` (or any bootstrap object).
3. `MemoryStore::open` again and assert a diagnosable refusal.
...
1. Seed `schema_version` with `("memory", 58)` on a store whose
   schema is otherwise version 57.
2. `MemoryStore::open` and assert a refusal or an explicit forward-compatibility
   decision.

Fails today: the open succeeds silently.

For case 2, seed `("memory", 0)` alongside an existing `cache_state` table
and assert the error names the family rather than the DDL collision.

## Investigation log

### Q: Is there a plan to wire `compute_schema_manifest_digest` into `MemoryStore::open`?

- Sources examined: `sqlite_runtime.rs:1-6`, which names
  `packages/plugin/src/features/eidnara/fixtures/direct-format-vocabulary-v1.json` (source-catalog path, not present at HEAD)
  as "the cross-runtime source of truth"; `docs/migration-version-lanes.md:18-20` (source-catalog path, not present at HEAD)
...
- Findings: the vocabulary is deliberately shared between runtimes, and the
  ...
  registry in `crates/memory-store/src`. `MIGRATIONS` is one flat SQL string, not a
