...
import { registerEidnaraMemoryMarkCommand } from "./eidnara-memory-mark";
...
function harness() {
    ...
    registerEidnaraMemoryMarkCommand(pi, { kernelClient: resolver.kernelClient });
    const run = async (args: string, hasUI = true, replaceSessionOnConfirm = false) => {
        const command = fake.commands.get("eidnara-memory-mark") as {
...
describe("Pi /eidnara-memory-mark", () => {
