...
`crates/daemon/src/history_summarizer_validate.rs:1054-1074`. The other declared
...
1. Regex, no bound: `history_summarizer_validate.rs:1193-1196`
...
1. Capture into `u64`: `:306` calls `capture_u64(attr_importance_regex(), attrs)`,
...
2. Stored on the parsed struct as `Option<u64>`: `:134`, assigned at `:327` and
   `:344`.

3. Copied to the validated struct unchanged: `:976` `importance: history_segment.importance`.
   `ValidatedHistorySegment.importance` is `Option<u64>` (`:220`).

4. Never validated. Read all of `validate_parsed_history_segments` (`:983-1084`) and
   `validate_history_summarizer_output` (`:450-641`): `importance` appears in neither. No
   range check, no clamp, no reject.

5. Truncating narrow at the publish projection, `history_summarizer.rs:57`:
...
1. Stored as-is. `StoredHistorySegment.importance` is `i32`
...
2. Clamped only at render, `decay_render.rs:269-272`:

```rust
let importance = history_segments[original_index]
    .importance
    .unwrap_or(50)
    .clamp(1, 100);
```
...
to 50 when absent"; `history_summarizer_prompt.rs:17` defines
...
The model emits an otherwise perfect history_segment for the most consequential
stretch of the session and marks it maximally important, but formats the number
wrongly, for example as a scaled integer:

```
<history_segment start="1" end="20" title="Root cause of the outage" episode_type="incident" importance="4294967295"><p1>...</p1></history_segment>
```

Gate: no importance check exists, so every one of the 22 rejecting checks passes
on ordinals and tiers alone. The history_segment publishes.

Projection: `4294967295u64 as i32` is `-1`. The durable row holds
`importance = -1`.

Render: `(-1).clamp(1, 100)` is `1`. The history_segment the model called maximally
...
1. Gate level, direct unit test. Chunk `1..=20`, one history_segment with
   `importance="4294967295"`, valid tiers and `<unprocessed_from>21</unprocessed_from>`.
   Assert the property: `validated.history_segments[0].importance` is within `1..=100`
   or the call returned `Err`. Today it returns `Ok` with `Some(4294967295)`,
   which is the finding.
2. Projection level. Call `to_stored_history_segment` with
...
### Q: Are there stored-history_segment consumers besides decay_render that read importance without clamping?
...
- Sources examined: `rg -n "importance" crates/memory-store/src/lib.rs` (hits at
  ...
  `history_summarizer_prompt.rs:127-138`.
- Findings: The store hits are schema, struct field, column lists in SELECT and
  ...
  that ordering is documented for MEMORY rows rather than history_segment rows.
- Missing evidence: whether the memory trim ordering at `:9877-9881` ever sees a
  value derived from a history_segment's `importance`. Facts promoted from the
  history_summarizer carry their own category and content
  (`history_summarizer_validate.rs:140-150`) with no importance field, so the two look
  independent, but the promotion projection in `history_summarizer.rs` was not fully
  traced in this pass.
- Conclusion: unresolved, needs a sweep of `memory-store` history_segment and memory
...
- Sources examined: `testdata/validate-golden.json` (all 16 cases inspected for
  their importance values), `history_summarizer_validate.rs:1367-1375` (the test helper,
  which hardcodes `importance="50"`), `history_summarizer.rs:1912` and `:4008-4009`,
