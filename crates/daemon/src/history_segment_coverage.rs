...
///
/// Discrete memories, sequenced history_segments, and additive profile entries use m1 corrections instead of HARD folds.
...
pub struct M0ContentEpoch {
    ...
    /// `history_segment_render_epoch` coordinates one HARD fold across serializer profiles when history_segment history bytes change.
    /// An empty `history_segment_render_epoch` denotes epoch zero and is omitted from the folded identity.
    pub history_segment_render_epoch: String,
...
pub fn fold_m0_content_epoch(base_render_config: &str, epoch: &M0ContentEpoch) -> String {
    ...
    if !epoch.history_segment_render_epoch.is_empty() {
        parts.push(part("cre", &epoch.history_segment_render_epoch));
    }
...
mod tests {
    ...
    /// A single history_segment, a contiguous run, and a run with a sparse
    /// coordinate gap (retired ordinals are absent from store data) all anchor
    /// coverage on the last history_segment.
    #[test]
    fn valid_sets_report_the_last_history_segment_as_coverage() {
        let cases: [(Vec<StoredHistorySegment>, i64, u64, &str); 3] = [
    ...
    fn m0_content_epoch_folds_legibly_and_deterministically() {
        ...
        let epoch = M0ContentEpoch {
            ...
            history_segment_render_epoch: String::new(),
        ...
        assert!(
            !folded.contains("cre:"),
            "history_segment epoch zero must not add a component"
        );
        assert!(
            !folded.contains("mpe:"),
        ...
        let mut history_segment_render_epoch = epoch.clone();
        history_segment_render_epoch.history_segment_render_epoch = "cre1".into();
        let history_segment_render_folded =
            fold_m0_content_epoch(base, &history_segment_render_epoch);
        assert!(history_segment_render_folded.contains("cre:4:cre1"));
        assert_ne!(folded, history_segment_render_folded);
