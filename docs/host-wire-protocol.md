...
| TypeScript frames waiting for pool capacity | 64 frames, each holding its byte charge |
...
Both managed clients publish without waiting on the pool. A frame whose inventory is exhausted waits in admission order and the client parks on the host's capacity doorbell, so a host consumption or block return resumes publication without inbound data or a timer. A pure-header `Pong` publishes from the control reserve past waiting data; `Cancel` and `Goodbye` publish after the frames queued ahead of them. A waiting frame that reaches its deadline is dropped unpublished and settles `not_sent`. A frame past the TypeScript queue bound is refused as `not_sent` with code `ring_full`. A binary unary response past either retained-response ceiling is released unread and fails with `retained_response_limit`; the host's work completed, so the outcome is a terminal, not a retry.
