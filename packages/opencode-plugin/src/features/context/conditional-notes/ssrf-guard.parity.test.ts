...
import { validateConditionalNoteHttpUrl } from "./ssrf-guard";
...
describe("conditional-note SSRF guard runtime parity", () => {
    test("Bun and Node classify public/private DNS the same way", async () => {
        await supportedNodeVersion();
        const bunAllowed = await validateConditionalNoteHttpUrl("https://example.test/", {
        ...
        const bunBlocked = await validateConditionalNoteHttpUrl("https://example.test/", {
            signal: new AbortController().signal,
            resolver: {
                lookup: async () => [
        ...
        try {
            ...
            await writeFile(
                script,
                `import { validateConditionalNoteHttpUrl } from ${JSON.stringify(`file://${bundledPath}`)};
const signal = new AbortController().signal;
const allowed = await validateConditionalNoteHttpUrl('https://example.test/', { signal, resolver: { lookup: async () => [{ address: '93.184.216.34', family: 4 }] } }).then(() => true, () => false);
const blocked = await validateConditionalNoteHttpUrl('https://example.test/', { signal, resolver: { lookup: async () => [{ address: '93.184.216.34', family: 4 }, { address: '10.0.0.1', family: 4 }] } }).then(() => false, () => true);
