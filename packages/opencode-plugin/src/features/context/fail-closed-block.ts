/**
 * The fail-closed gate blocks user-enabled sessions when Eidnara cannot operate because of a schema fence or storage open/migration failure.
 *
...
/**
 * Eidnara hidden-child jobs bypass blocking because blocking them stalls recovery and background maintenance.
 */
function isEidnaraHiddenAgentName(agent: string): boolean {
    if (
        agent === "sidekick" ||
...
export function formatFailClosedBlockingMessage(reason: FailClosedReason): string {
    if (reason.kind === "format_refusal") {
        ...
        return [
            `Eidnara cannot operate: the shared database is not the supported direct claims format (${reason.family})${detail}. No data was changed.`,
            `To abandon this database family and start fresh, run '${FAIL_CLOSED_DOCTOR_COMMAND} reset-db'.`,
        ].join(" ");
    }
    if (reason.kind === "schema_fence") {
        return [
            `Eidnara cannot operate: this Eidnara build is older than the database; upgrade/restart this harness (upstream migration lane v${reason.persistedVersion}, build supports through v${reason.supportedVersion}).`,
            `Recovery: ${FAIL_CLOSED_DOCTOR_COMMAND}`,
        ].join(" ");
    }
    const cause = reason.cause.trim().length > 0 ? reason.cause.trim() : "unknown storage error";
    return [
        `Eidnara cannot operate: persistent storage failed (${cause}).`,
...
/**
 * Primary user sessions are never exempt; internal OpenCode agents, Eidnara hidden children, and Pi subagent processes are exempt.
 */
export function shouldBypassFailClosedBlock(input: {
    ...
    if (isEidnaraHiddenAgentName(agent)) return true;
    return false;
}
