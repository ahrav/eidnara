...
use memory_store::memory_reviewer_ledger::{
    MEMORY_REVIEWER_MAX_ATTEMPTS, MEMORY_REVIEWER_TASK_LEASE_MS, MemoryReviewerAttemptTerminal,
    MemoryReviewerBeginOutcome, MemoryReviewerLedgerRefusal,
};
...
fn intent(key: &str) -> CommitIntent {
    CommitIntent {
        producer: "memory_reviewer-disclosure-test".to_string(),
...
fn open_job(
    ...
    let MemoryReviewerBeginOutcome::Begun(_) = ledger
...
/// A Kernel store holding one commit message published through the byte-verifying Git publisher (the only producer of remote-eligible evidence a MemoryReviewer fixture may use), a Memory Store with a ready job under a live claim and receipt, and a remote-destination broker that has disclosed the commit.
struct Fixture {
...
impl Fixture {
    fn open() -> Self {
        ...
        let ledger = MemoryStore::open(&MemoryStore::test_descriptor(
            ledger_dir.path(),
            "eidnara-memory_reviewer-disclosure-test",
        ))
        .unwrap();
        let registration = {
...
) -> Result<daemon::memory_reviewer::disclosure::Disclosed, DisclosureRefusal> {
...
async fn nothing_is_sent_without_approval_under_cancellation_or_when_the_marker_cannot_commit() {
    ...
    assert_eq!(
        outcome,
        DisclosureRefusal::Ledger(MemoryReviewerLedgerRefusal::ClaimInvalid)
    );
    let observed = server.await.unwrap();
...
async fn a_lapsed_attempt_whose_terminal_cannot_be_recorded_reports_it() {
    ...
    storage::after_commit_for_test(move |_| {
        ...
        rusqlite::Connection::open(ledger_path)
            .unwrap()
            .execute(
                "UPDATE memory_reviewer_receipts SET claim_id = 'claim-successor'
...
async fn provider_failure_and_a_mismatched_model_end_the_attempt_without_a_second_send() {
    ...
    let allowance = usize::try_from(MEMORY_REVIEWER_MAX_ATTEMPTS).unwrap();
...
async fn attempt_with_sender(
    fixture: &Fixture,
    sender: &daemon::memory_reviewer::model_request::Sender,
    broker: &EvidenceBroker,
    prepared: &PreparedBody,
    now: &(dyn Fn() -> i64 + Sync),
) -> Result<daemon::memory_reviewer::disclosure::Disclosed, DisclosureRefusal> {
...
async fn the_attempt_is_charged_under_the_holds_job_while_another_job_is_live() {
    ...
    assert!(
        fixture
            .ledger
            .list_memory_reviewer_attempts(PROJECT, &other_identity)
            .unwrap()
            .is_empty(),
...
async fn the_committed_attempt_deadline_bounds_the_response_wait() {
    ...
    let now = fixture.now + MEMORY_REVIEWER_TASK_LEASE_MS - 700;
    let mut peer = Peer::start().await;
    let server = peer.serve(
        Box::new(|_, _| {
            Box::pin(async {
