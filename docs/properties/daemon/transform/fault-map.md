...
| canonical-read-staleness-is-distinguishable-from-emptiness | A HARD pass whose canonical read is withheld: kernel phase not `Ready`, an `outbox_lag`/`read_visible` error, or a registered outbox consumer past a lag threshold (`serving.rs:58-63`); the unit test at `transform.rs:13258` injects the `KernelOutcome` directly; the integration test registers a consumer and publishes 10,000 outbox positions past its checkpoint on a real store | **Yes** - both forms run in CI |
