...
is an arbitrary `HostHandler` implementation (`handler.rs:558`), and the
...
| **C2** a direct-output serializer that mismatches its declared length | A handler calling `RequestCtx::output_from_writer` with an `exact_len` its serializer does not satisfy, so `reservation.commit(body_len)` rejects on `cursor != body_len` (`shm-transport/src/backend/ring.rs:1719-1721`, `ProducerError::Underfill`) | **Yes, and no injection seam is needed, which is the surprising part.** The mismatch is authored in the handler, and the failure occurs inside `publish_direct` (`ring_transport.rs:1280-1292`) on the endpoint thread, long after `settle` returned at `dispatch.rs:460`. What a test *cannot* do is observe the host attributing the loss, because `Publisher::try_publish` (`ring_transport.rs:1263-1265`) returns `Err` without running the `written` hook (`:1272-1274`) and without touching the settlement. That absence is the record, not an obstacle to it |
...
is `crates/daemon/src/bin/eidnara_host/serve.rs` (composite at `:575`,
`host_runtime::run` at `:632`), and `RouteClass::Reserved` is live because
`model_execution/mod.rs:164-177` declares it; see the reachability resolution in
...
| `req_publication_failed_after_a_recorded_settlement` | `Publisher::try_publish` returned `Err` (`ring_transport.rs:1263-1265`) for a correlation whose `won` was already `true` | Legal: the two events are unordered by construction, and the record's whole point is that nothing relates them. Does **not** assert what the client then observed |
...
1. **`C0`, running the existing suites in CI.** A workflow change and nothing
   else: add an unfiltered `-p host-runtime` invocation, or name the six binaries whose
...
(`StaticComposite::new`) and `:632` (`host_runtime::run`), plus the fact that
