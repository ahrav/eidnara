...
**1. `now` is sampled before the lock.** `crates/host-runtime/src/connection.rs:504-506`:
...
**Where `completed_at` comes from.** `crates/host-runtime/src/tcp_frame_channel.rs:366-378` captures it
...
(`crates/daemon/src/bin/eidnara-host.rs:437-438`). Two workers is enough for the read task and the
...
`liveness: Some(..)` in `crates/host-runtime/src/` is `config.rs:664`, inside the `#[cfg(test)]` module
that begins at `:469`. Nothing in `crates/daemon/` configures it. So no shipped configuration in
...
- Sources examined: `crates/host-runtime/src/connection.rs:1-20`; `grep -n "Instant"` across that file;
...
- Sources examined: all six `pings.lock()` sites (`connection.rs:505`, `:1357`, `:1371`, `:1381`,
  ...
  `crates/daemon/src/bin/eidnara-host.rs:437-438`.
...
- Sources examined: `crates/host-runtime/src/connection.rs:56-74` (the full `PingProbe` rationale);
  `:512-526`; `:1429-1446`.
