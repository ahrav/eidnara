...
import { EIDNARA_MEMORY_RESPONSE_BUDGET_BYTES } from "./constants";
import {
    EIDNARA_MEMORY_ACTOR,
    type EidnaraMemoryWriteIdentity,
...
function identityFor(toolCallId: string): EidnaraMemoryWriteIdentity {
...
function rawArgs(value: Record<string, unknown>): EidnaraMemoryArgs {
    return value as unknown as EidnaraMemoryArgs;
}
...
describe("executeEidnaraMemory", () => {
    ...
    test("a create replay recovered from the row reports the creating commit as knownAsOf", async () => {
        const { kernel, client } = harness();
        const args: EidnaraMemoryArgs = { category: "REJECTED_APPROACH", antiMemory: ANTI_MEMORY };
        try {
    ...
    test("get bounds the echoed missing ids to the response budget", async () => {
        ...
        expect(Buffer.byteLength(text, "utf8")).toBeLessThan(
            EIDNARA_MEMORY_RESPONSE_BUDGET_BYTES + 512,
        );
        expect(reply.memories).toEqual([]);
...
describe("executeEidnaraMemory get expiry", () => {
...
describe("executeEidnaraMemory get on a truncated read", () => {
