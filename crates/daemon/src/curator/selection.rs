...
/// Gates scheduling of selection over [`MEMORY_CLASSES`]; the scheduler adds no selection task while this is `false`. Opening requires the owner decision and positive witness recorded under "Production selection gate" in `docs/curator-operations.md`.
