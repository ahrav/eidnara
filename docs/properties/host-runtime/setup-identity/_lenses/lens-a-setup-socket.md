...
`crates/host-runtime/src/auth.rs` (1,112), `crates/host-runtime/src/connection_file.rs`
(471), the credential-minting and publication parts of
`crates/host-runtime/src/instance.rs`, and `packages/shm-native/src/setup.rs` (433)
...
`client.rs:346-369` (2b and 2d), `packages/shm-native/src/lib.rs:491-629`.
...
`${dataDir}/eidnara/run/setup.sock` (`runtime.rs:834`, path from
...
descriptors. `crates/host-runtime/tests/shm_failure_modes.rs:44-58` constructs exactly
...
5. The socket path is fully predictable: `${dataDir}/eidnara/run/setup.sock`,
...
15. `lib.rs:491` `attach` remains a published napi export taking caller-supplied
    ...
    (`packages/shm-native/index.ts:526-529`, `:531-534`); only `connectSetup`
    is used by the shipped frame channel
    (`packages/plugin/src/shared/host-client/shm-frame-channel.ts:77`).
...
1. **Descriptor transfer precedes validation, and the doc says so.**
   `docs/shm-transport.md:40-45` orders setup as authenticate, admit
   ...
   **not** a disagreement. What the docs never state is the consequence, that a
   ...
   descriptors, which `host-wire-protocol.md:563` reads as if it prevented
   access. Recorded as
   `setup-a-mapping-authority-derives-only-from-the-key-never-from-the-token`.
2. **"One-use activation token."** `host-wire-protocol.md:561` says setup
...
3. **The published socket path in the doc's example does not match the code.**
   `host-wire-protocol.md:76` shows `"setup_socket":
   "/run/user/1000/eidnara/host-runtime.sock"`. The code produces
   ...
   `${dataDir}/eidnara/run/connection.json`, so the example contradicts
...
4. **Host-side profile acceptance has no host-side enforcement.**
   `host-wire-protocol.md:563` says "Setup accepts only the release's current
...
5. **Nonce non-reuse is asserted as a MUST and is only probabilistic.**
   `host-wire-protocol.md:177` says both nonces MUST NOT be reused within or
...
6. **The two-second budgets are stated as coupled and are configured
   independently.** `host-wire-protocol.md:747` states the client's whole-
...
1. Should the setup socket use `SO_PEERCRED` to bind the peer's uid, given that
   ...
   claims at `host-wire-protocol.md:29`. (needs human input)
...
3. Where are the two bootstrap tests that `auth.rs:390-392` names as the
   ...
   `crates/host-runtime/tests/` in this pass. (unresolved, needs a repository-wide
   search)
...
5. Part 1's `native-boundary-not-weaker-than-its-wrapper` was written before
   `packages/shm-native/src/setup.rs` existed. Its `Impact` line says the
...
7. The re-scope document's open question at
   ...
   `packages/shm-native/src/setup.rs` belongs to Part 1 or to 2c. This lens
