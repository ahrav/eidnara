//! The MemoryClassifier receipt ledger. It stores one durable receipt for each
...
pub use context_core::canonical_json::{
    MEMORY_CLASSIFIER_REQUEST_ENCODING_VERSION, compute_memory_classifier_request_digest,
};

/// The project-scoped request identity a receipt is keyed by.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MemoryClassifierReceiptKey<'a> {
    pub project: &'a str,
    pub producer: &'a str,
...
pub struct MemoryClassifierReceiptBinding {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MemoryClassifierTerminalKind {
    Complete,
    Failed,
    Cancelled,
    ...
    /// Only attempt rows end this way; `count_memory_classifier_attempts` excludes them.
    NotSent,
}

impl MemoryClassifierTerminalKind {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MemoryClassifierReceiptState {
    InProgress {
        generation: u64,
    },
    Complete {
        generation: u64,
        terminal_kind: MemoryClassifierTerminalKind,
        result_json: String,
    },
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MemoryClassifierReceipt {
    pub binding: MemoryClassifierReceiptBinding,
    pub state: MemoryClassifierReceiptState,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MemoryClassifierBeginOutcome {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MemoryClassifierTransition {
    Applied,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MemoryClassifierAttemptSpec<'a> {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MemoryClassifierAttempt {
    pub generation: u64,
    pub attempt_index: u32,
    pub model: String,
    pub prompt_template_version: u32,
    ...
    pub terminal_kind: Option<MemoryClassifierTerminalKind>,
...
fn receipt_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<MemoryClassifierReceipt> {
    ...
    let state = match (state.as_str(), terminal_kind, result_json) {
        ("in_progress", None, None) => MemoryClassifierReceiptState::InProgress { generation },
        ("complete", Some(kind), Some(result_json)) => MemoryClassifierReceiptState::Complete {
            generation,
    ...
    Ok(MemoryClassifierReceipt {
        binding: MemoryClassifierReceiptBinding {
...
fn parse_terminal_kind(
    column: usize,
    kind: String,
) -> rusqlite::Result<MemoryClassifierTerminalKind> {
    MemoryClassifierTerminalKind::parse(&kind).ok_or_else(|| {
...
fn validate_key(key: MemoryClassifierReceiptKey<'_>) -> Result<(), MemoryStoreError> {
    if key.project.is_empty() {
        return Err(MemoryStoreError::Serde(
            "memory_classifier receipt project must not be empty".to_string(),
        ));
    }
    for (name, value) in [
        ...
        if value.is_empty() || value.len() > 256 {
            return Err(MemoryStoreError::Serde(format!(
                "memory_classifier receipt {name} must be 1 to 256 bytes"
            )));
        }
    }
    Ok(())
...
fn validate_binding(binding: &MemoryClassifierReceiptBinding) -> Result<(), MemoryStoreError> {
    if !is_lower_hex(&binding.request_digest, 64) {
        return Err(MemoryStoreError::Serde(
            "memory_classifier receipt request digest must be 64 lowercase hex characters"
                .to_string(),
        ));
    }
...
fn receipt_write(key: MemoryClassifierReceiptKey<'_>) -> PreparedWrite {
    let mut write = PreparedWrite::new(DurableWriteFamily::CommandLedgers);
    write.domain_owner(
        "project",
        key.project,
        active_scan_owner_key(&["memory_classifier", key.producer, key.operation_key]),
    );
    write
}

fn disposition(changed: usize) -> WriteDisposition<MemoryClassifierTransition> {
    if changed == 0 {
        WriteDisposition::Replay(MemoryClassifierTransition::Fenced)
    } else {
        WriteDisposition::Applied(MemoryClassifierTransition::Applied)
    }
}
...
impl MemoryStore {
    ...
    pub fn begin_memory_classifier_receipt(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
        binding: &MemoryClassifierReceiptBinding,
        now_ms: i64,
    ) -> Result<MemoryClassifierBeginOutcome, MemoryStoreError> {
        validate_key(key)?;
        ...
        write.execute(&self.inner, |coordinated| {
            ...
            let existing = tx
                .query_row(
                    &format!(
                        "SELECT {RECEIPT_COLUMNS} FROM memory_classifier_receipts
            ...
            if let Some(existing) = existing {
                let outcome = if existing.binding.request_digest != binding.request_digest {
                    MemoryClassifierBeginOutcome::DigestConflict {
                    ...
                    != binding.database_incarnation_id
                {
                    ...
                    match existing.state {
                        MemoryClassifierReceiptState::InProgress { generation } => {
                            MemoryClassifierBeginOutcome::InProgress { generation }
                        }
            ...
            tx.execute(
                ...
                params![
                    ...
                    MEMORY_CLASSIFIER_REQUEST_ENCODING_VERSION,
            ...
            Ok(WriteDisposition::Applied(
                MemoryClassifierBeginOutcome::Begun { generation: 1 },
            ))
        })
    }
    ...
    pub fn take_over_memory_classifier_receipt(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
        predecessor_generation: u64,
        now_ms: i64,
    ) -> Result<MemoryClassifierTransition, MemoryStoreError> {
        ...
        self.guarded_transition(
            key,
            |_| Ok(()),
            "UPDATE memory_classifier_receipts SET generation = ?4, updated_at_ms = ?5
              WHERE project = ?1 AND producer = ?2 AND operation_key = ?3
    ...
    pub fn take_over_undispatched_memory_classifier_receipt(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
        predecessor_generation: u64,
        now_ms: i64,
        ...
        self.guarded_transition(
            ...
            "UPDATE memory_classifier_receipts SET generation = ?4, updated_at_ms = ?5
              WHERE project = ?1 AND producer = ?2 AND operation_key = ?3
                AND state = 'in_progress' AND generation = ?6
                AND NOT EXISTS (
                    SELECT 1 FROM memory_classifier_attempts a
    ...
    pub fn begin_memory_classifier_attempt(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
        generation: u64,
        spec: &MemoryClassifierAttemptSpec<'_>,
        now_ms: i64,
    ) -> Result<MemoryClassifierTransition, MemoryStoreError> {
        ...
        self.guarded_transition(
            ...
            "INSERT INTO memory_classifier_attempts (
             ...
             SELECT project, producer, operation_key, generation, ?4, ?5, ?6, ?7, ?8, ?9, ?10, ?12, ?13
               FROM memory_classifier_receipts
              WHERE project = ?1 AND producer = ?2 AND operation_key = ?3
    ...
    pub fn record_memory_classifier_run_handle(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
        generation: u64,
        attempt_index: u32,
        ...
        self.guarded_transition(
            ...
            "UPDATE memory_classifier_attempts SET run_handle = ?6
              WHERE project = ?1 AND producer = ?2 AND operation_key = ?3
                AND generation = ?4 AND attempt_index = ?5
                AND terminal_kind IS NULL AND run_handle IS NULL
                AND EXISTS (
                    SELECT 1 FROM memory_classifier_receipts r
    ...
    pub fn finish_memory_classifier_attempt(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
        generation: u64,
        attempt_index: u32,
        ...
        self.guarded_transition(
            key,
            |_| Ok(()),
            "UPDATE memory_classifier_attempts SET terminal_kind = ?6, terminal_at_ms = ?7
              WHERE project = ?1 AND producer = ?2 AND operation_key = ?3
    ...
    pub fn complete_memory_classifier_receipt(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
        generation: u64,
        terminal_kind: MemoryClassifierTerminalKind,
        ...
        write.execute(&self.inner, |coordinated| {
            let changed = coordinated.tx().execute(
                "UPDATE memory_classifier_receipts
    ...
    pub fn release_memory_classifier_attempt_session(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
        generation: u64,
        attempt_index: u32,
        ...
        self.guarded_transition(
            key,
            |_| Ok(()),
            "UPDATE memory_classifier_attempts SET session_released_at_ms = ?6
              WHERE project = ?1 AND producer = ?2 AND operation_key = ?3
                AND generation = ?4 AND attempt_index = ?5
    ...
    pub fn lookup_memory_classifier_receipt(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
    ) -> Result<Option<MemoryClassifierReceipt>, MemoryStoreError> {
        validate_key(key)?;
        self.inner
            .with_conn(|conn| {
                conn.query_row(
    ...
    pub fn list_memory_classifier_attempts(
        &self,
        key: MemoryClassifierReceiptKey<'_>,
    ) -> Result<Vec<MemoryClassifierAttempt>, MemoryStoreError> {
        validate_key(key)?;
        self.inner
            .with_conn(|conn| {
                let mut statement = conn.prepare_cached(
                    "SELECT generation, attempt_index, model, prompt_template_version,
                       ...
                       FROM memory_classifier_attempts
                      WHERE project = ?1 AND producer = ?2 AND operation_key = ?3
    ...
    pub fn count_memory_classifier_attempts(
        &self,
        project: &str,
        since_ms: i64,
        ...
        self.inner
            .with_conn(|conn| {
                conn.query_row(
                    "SELECT COUNT(*) FROM memory_classifier_attempts
                      WHERE project = ?1 AND dispatched_at_ms >= ?2
                        AND (terminal_kind IS NULL OR terminal_kind != ?3)",
                    params![
                        project,
                        since_ms,
                        MemoryClassifierTerminalKind::NotSent.as_str()
                    ],
                    |row| row.get::<_, i64>(0),
                )
            })
...
/// Digest over the effect-defining inputs of one MemoryClassifier request. Canonical
...
pub fn memory_classifier_request_digest(inputs: &Value) -> Result<String, MemoryStoreError> {
    compute_memory_classifier_request_digest(inputs)
