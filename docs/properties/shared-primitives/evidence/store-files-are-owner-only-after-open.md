...
- **Primary evidence:** `open_sqlite` (`crates/storage/src/lib.rs:612-615`) calls `lease::protect_file` on the database path and its `-wal` and `-shm` siblings after enabling WAL.
- **Existing evidence:** `reopening_a_permissive_store_protects_the_database_and_its_wal` (`crates/storage/src/lib.rs:829`), Unix only.
