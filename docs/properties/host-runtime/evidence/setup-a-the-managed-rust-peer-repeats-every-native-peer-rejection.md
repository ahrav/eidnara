...
`packages/shm-native/src/setup.rs`, and the task asked whether that asymmetry
...
`packages/shm-native/src/setup.rs:95-161`, `connect`:
...
`packages/shm-native/src/lib.rs:571-629`, `connect_setup`, adds two more:
...
`crates/host-runtime/src/setup_socket.rs:288-331`, `activate_client`:
...
`crates/host-runtime/src/ring_transport.rs:636-656`,
...
`lib.rs:539-549` and `:585-588` and nowhere in `crates/host-runtime/src`. So the managed
...
For each row, drive both `packages/shm-native`'s `connect_setup` and the managed
...
- Sources examined: the Part 1 record, `lib.rs:491-568`, `packages/shm-native/index.ts:41-48`.
...
- Sources examined: `setup_socket.rs:52-59`, `:61-62`,
  `packages/shm-native/src/setup.rs:42-43`, `:60-61`,
