//! This crate provides a scriptable linked handler and a real-loopback host launcher for `host-runtime` conformance tests.
...
) -> Result<host_runtime::OutputBuffer, host_runtime::StreamClosed> {
    let mut body = ctx.reserve_output(256).await?;
    serde_json::to_writer(&mut body, value).map_err(|_| host_runtime::StreamClosed)?;
    Ok(body)
}
...
struct Inner {
    ...
    /// The `blocking_hold` closure waits on this gate from its blocking thread.
    blocking_gate: Arc<(Mutex<bool>, Condvar)>,
...
struct CountedCharge {
    _charge: host_runtime::wire::ByteCharge,
    released: Arc<AtomicUsize>,
}

impl Drop for CountedCharge {
    fn drop(&mut self) {
...
impl TestHandler {
    ...
    /// How many `blocking_hold` closures have started on their thread.
    pub fn blocking_started(&self) -> usize {
    ...
    pub fn release_blocking(&self) {
        let (lock, ready) = &*self.inner.blocking_gate;
        *lock.lock().expect("blocking gate lock") = true;
        ready.notify_all();
    }
...
impl HostHandler for TestHandler {
    ...
    async fn initialize(&self, _init: host_runtime::HostInit) -> Result<(), InitError> {
            ...
            "blocking_hold" | "blocking_detached" | "blocking_panic" => {
                let Some(charge) = ctx.try_reserve_resident(1024) else {
                    return RequestOutcome::error(
                        "scratch_exhausted",
                        "the test handler could not reserve its held charge",
                    );
                };
                let charge = CountedCharge {
                    _charge: charge,
                    released: Arc::clone(&self.inner.blocking_charge_released),
                };
                if mode == "blocking_panic" {
                    let outcome = ctx
                        .run_blocking(move || -> Vec<u8> {
                            let _charge = charge;
                            panic!("{}", String::from_utf8_lossy(CANARY_BODY))
                        })
                        .await;
                    return match outcome {
                        Ok(body) => response_from_slice(&ctx, &body, false).await,
...
impl TestHost {
    ...
    pub async fn start_with_publish_hook(hook: host_runtime::ring_transport::PublishHook) -> Self {
    ...
    async fn try_start_with_publish_hook(
        handler: TestHandler,
        tweak: impl FnOnce(&mut HostConfig),
        publish_hook: Option<host_runtime::ring_transport::PublishHook>,
    ) -> Result<Self, HostError> {
        ...
        let join = tokio::spawn(async move {
            host_runtime::run_with_publish_hook(run_handler, config, run_shutdown, publish_hook)
                .await
        });
...
impl CompositeTestHost {
    ...
    /// `try_start` returns `host_runtime::run`'s error when the run task ends before publication.
    pub async fn try_start<H: HostHandler>(
        handler: H,
        tweak: impl FnOnce(&mut HostConfig),
    ) -> Result<Self, HostError> {
        let data_root = tempfile::tempdir().expect("temp data root");
    ...
    pub async fn start<H: HostHandler>(handler: H, tweak: impl FnOnce(&mut HostConfig)) -> Self {
