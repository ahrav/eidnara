//! Smoke host that publishes a LocalEmbeddings lane and waits for a signal.
//!
//! Usage:
//!
//! `local_embeddings_host <data-dir> <bundle-dir|-> [<ort-library> <ort-library-sha256>]`
//!
...
impl CompositeComponent for PlaceholderModelExecution {
    ...
    async fn bind(&self, _route: RouteHandle, _identity: RouteIdentity) -> BindOutcome {
        BindOutcome::Reject {
            ...
            message: "model_execution is unavailable in this smoke host".to_owned(),
        }
    }
    ...
    async fn handle(&self, _ctx: RequestCtx) -> RequestOutcome {
        RequestOutcome::error(
            "internal_error",
            "unreachable: model_execution binds are rejected",
        )
    }
...
async fn main() {
    ...
    let usage = "usage: local_embeddings_host <data-dir> <bundle-dir|-> [<ort-library> <ort-library-sha256>]";
    ...
    let local_embeddings_config = (bundle_dir != "-").then(|| LocalEmbeddingsConfig {
    ...
    let local_embeddings = LocalEmbeddingsComponent::new(local_embeddings_config);
    let composite = StaticComposite::new(EchoPrimary, local_embeddings, PlaceholderModelExecution)
    ...
    let config = HostConfig {
        ...
        daemon_ver: "eidnara-host/local_embeddings-smoke".to_owned(),
        limits: HostLimits {
