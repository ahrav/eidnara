...
`docs/shm-transport.md:79` makes a release-gate claim in terms of
...
`docs/shm-transport.md:68` lists "completed reclamation count" as a
...
**Where the charge is released.** `crates/host-runtime/src/ring_transport.rs:276`,
...
because every `-p host-runtime` invocation in `ci.yml` filters to an integration
binary.
