...
`docs/host-wire-protocol.md` section 12 makes possible: its step 6 notes that on
...
- Sources examined: the claim at `client.rs:575`;
  `docs/host-wire-protocol.md` section 12's graceful shutdown ordering, whose
...
- Sources examined: `grep` for `host_shutdown` across `crates`, which finds the
  definition and the `eidnara-host` binary as the plausible consumer.
- Findings: `crates/daemon/src/bin/eidnara-host.rs` is described by its own manifest
  as "the production lifecycle/serve executable"
  (`crates/daemon/Cargo.toml:18-19`) and contains an `authenticate` helper at
...
- Conclusion: unresolved, needs a daemon pass. Sub-part 2d's scope is `client.rs`
