...
- **Scope:** sqlite/file backend only. PostgreSQL uses a server-side session advisory lock across processes and machines, not a filesystem lock; the module doc states the release rule and `open_postgres` takes the lock (`commons@89abb40 crates/cortexkit-store-postgres/src/lib.rs:7-9,355-389`).
