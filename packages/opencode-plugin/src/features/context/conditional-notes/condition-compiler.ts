...
import { resolveAndFenceProviderPath } from "@eidnara/fs-observer/path-fence";
import type { AtomicPredicate, ProviderConfig } from "@eidnara/fs-observer/provider";
import { validateProviderConfig } from "@eidnara/fs-observer/provider";

export const FS_OBSERVER_PROVIDER = "local-fs";

export type ConditionCompileResult =
    | {
          status: "compiled";
          provider: typeof FS_OBSERVER_PROVIDER;
...
export function conditionCompileReplySuffix(result: ConditionCompileResult): string {
    ...
    if (result.status === "refused") {
        return `\n- Observation compile refused: ${result.reason}`;
    }
    return "";
}
...
function unsafeGrammarReason(value: string): string | null {
    ...
    if (quote !== null) return "unbalanced quote; leaving condition memory_classifier-evaluated";
    if (/\bcontains\s+no(?:\s|$)/i.test(value)) {
        return "ambiguous negation after contains; leaving condition memory_classifier-evaluated";
    }
    ...
    if (temporalSuffix.test(value)) {
        return "temporal suffix after contains; leaving condition memory_classifier-evaluated";
    }
    return null;
}
