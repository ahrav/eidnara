...
| `packing.optional.excluded_optional_presented` | daemon | `sometimes` | packing-coverage-is-a-per-identity-partition | `optional_faults_are_excluded_with_a_reason_and_never_refuse_the_preparation`: a duplicate, an unpersisted, a kernel-excluded, and a stale request. |
