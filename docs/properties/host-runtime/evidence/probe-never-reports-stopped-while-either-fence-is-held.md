...
- `crates/host-runtime/src/lifecycle.rs` contains exactly two production
...
- Acquisition order is the source of the single-fence window.
  ...
  `crates/host-runtime/src/instance.rs:245`, resolves the runtime directory at
...
`.host-coordination` under a live daemon is declared out of contract there
...
`eidnara` subtree and assert the verdict is `Wedged`, which
