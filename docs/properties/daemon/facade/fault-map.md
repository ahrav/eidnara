...
| note-b-completion-applies-only-under-the-claimed-revision-and-state-version | An outstanding claim on a note, plus a concurrent facade mutation of that note. **No injected fault is needed** | 1 pending note · 3 (claim, interleaved `ctx_note` mutation, complete) · yes · present | **Yes.** Three requests with the mutation interleaved between claim and completion. The two direct `task_lease::fence_task_claims_tx` call sites are in the shared `update_note_cas_tx` and `dismiss_note_tx` helpers (`crates/memory-store/src/lib.rs:14992` and `crates/memory-store/src/lib.rs:15072`). The module side asserts only the phase, which is the gap. `smart_note_revision_matrix_normative_matches_memory_store` covers the matrix, not the interleaving |
...
| `SMART_NOTE_COMPLETION_ARRIVED_AGAINST_A_MUTATED_NOTE` | An applied completion whose claim revision was taken before a concurrent facade mutation | Legal input, and the fence at `memory-store/src/lib.rs:13569-13573` exists for it. The precondition of the revision-fence record |
