...
- **Execution profile:** Single branch `hardening/unsafe-surface`, one PR. Units land as separate commits in dependency order. Every commit keeps `cargo test --workspace` green. U1's attributes on the four crates whose `lib.rs` the wave U2 receipt pins (`cache-stability`, `lease`, `storage-types`, `storage`) land through the wave process instead, so the receipt's property records and `modules_hash` are regenerated against an audited tree rather than refreshed in place; `tokenizer` is unpinned and takes its `forbid` here.
...
- R7. `crates/storage-types`, `crates/cache-stability`, and `crates/tokenizer` carry `#![forbid(unsafe_code)]`. `crates/storage` and `crates/lease` carry `#![deny(unsafe_code)]` with `#[allow(unsafe_code)]` on exactly the items that need it: the test-only `umask`/`mkfifo` helpers in `storage`, and the two `geteuid` reads plus the two Windows file-information blocks (`FileIdentity::of_file` and `link_count`) in `lease`. `forbid` cannot coexist with an inner `allow`, and neither `umask`, `mkfifo`, nor `geteuid` has a std equivalent. No `cfg_attr` forbid is used; the same attributes apply on every target.

**Verification**

- R8. `LeaseSpan`, `copy_out`, `copy_in`, `ReceiveLease`, and the shared-memory accessor module have tests that run under `cargo +nightly miri test` without an unsupported-operation error or data-race report, including a test where a second thread stores into the bytes a `LeaseSpan` is reading. Every span access is a relaxed atomic load through `AtomicU8::from_ptr` or `AtomicU64::from_ptr` at the width `AccessShape` fixes for that byte, so the read side is atomic under the Rust abstract machine (KTD4).
...
- `crates/lease` production unsafe stays: two Unix `geteuid` reads and the two Windows `GetFileInformationByHandle` blocks (`FileIdentity::of_file` and `link_count`), each under `#[allow(unsafe_code)]` with its existing safety comment (R7).
...
- KTD4. **`LeaseSpan::new` stays `unsafe fn`; every span access is a relaxed atomic at the width `AccessShape` fixes per byte.** `new` gains a `# Safety` section naming two obligations: valid for reads and writes for `'lease` (`from_ptr` requires both even for a load), and no `&[u8]` or `&mut [u8]` formed over the range by this process while a `LeaseSpan` exists. `AccessShape::of(base, len)` splits a range from its absolute address into single bytes up to the first 8-aligned address, whole `u64`s, then single bytes; `read_byte`, `checksum`, and the two copy routines (`copy_out` from the mapping into a `&mut [u8]`, `copy_in` from a `&[u8]` into the mapping; together they replace the `[u8; 8]` volatile copy) all follow that shape, using `AtomicU8::from_ptr` and `AtomicU64::from_ptr` only on the shared-memory side because a slice-derived pointer cannot promise write validity; the ordinary-memory side uses plain slice access. Because the shape depends only on address and length, any two parties touching the same range use the same width for every byte, which is what makes a racing pair of relaxed atomics a stale read rather than the mixed-size data race Miri reports for a byte store against a word load. A per-byte-only draft of this unit measured 1.8x to 3.9x slower than `origin/main` at the call sites (LLVM never merges atomic accesses, so it emitted one byte per instruction); the shipped shape measures faster than `origin/main` in every cell. Call-site numbers are in the evidence file. The remaining assumption, recorded there, is that an honest peer never writes a span while this side holds a lease or reservation on it, which the slot state machine already requires. Rationale: it turns a documented gap into a Miri-checked property without giving up the word-wide copy.
- KTD5. **`sys` module owns every remaining libc call.** `crates/shm-transport/src/backend/sys.rs` exports `pub(crate)` safe fns, one per libc call: `memfd_create`, `ftruncate`, `fchmod`, `add_seals`, `get_seals`, `fstat` (returns a small struct of `st_mode`, `st_uid`, `st_size`), `geteuid`, `mmap_shared(fd, len)`, `mmap_anonymous(len)` (test-only), `munmap`, `madvise_remove`, `mincore`, `page_size`, `send_token`, `recv_tokens`, `poll_readable(fd, timeout_ms)`, `set_cloexec`, `socket_type`. Each returns `io::Result` or a plain value, takes `BorrowedFd`, and holds one unsafe block. Where a call needs a zeroed out-param plus the call plus `assume_init` (`fstat`) or a call plus `OwnedFd::from_raw_fd` (`memfd_create`), the block holds those steps together with one comment. `mmap` returns a mapping pointer, not a descriptor: `mmap_shared` and `mmap_anonymous` return the pointer with its length, and `munmap` releases the pair. Rationale: the remaining unsafe is enumerable by reading one file.
...
flowchart TB
  subgraph shm["crates/shm-transport (deny undocumented_unsafe_blocks)"]
    ...
    lease["lease.rs :: LeaseSpan\nnew, copy_out, copy_in (unsafe fn); AccessShape-width atomic loads and stores\n~5 blocks"]
...
- **Verification:** `cargo clippy --workspace --all-targets -- -D warnings` passes; `rg -A1 '#\[allow\(unsafe_code\)\]' crates/storage crates/lease` lists exactly the items named above, the line after each attribute naming the item that owns it.
...
- **Approach:**
  ...
  6. `write_reservation` and `lease_span` keep their arena pointer math but route through `Mapping::arena_ptr(offset, len) -> Result<*mut u8>` which owns the bounds check; each keeps one unsafe call into `LeaseSpan::new`/`copy_in`.
...
- **Approach:**
  1. Introduce `AccessShape` and route `LeaseSpan::read_byte`, `checksum`, `copy_out(*mut u8, &mut [u8])`, and `copy_in(&[u8], *mut u8)` through it, atomics on the mapping side only (KTD4). Update `# Safety` on `LeaseSpan::new` to two obligations.
  ...
  4. Add a test where a spawned thread stores through `copy_in` into an `AtomicU64` array while the main thread constructs a `LeaseSpan` over the same range and calls `read_byte`, `checksum`, `copy_to`; assert no panic and that each observed byte is one of the two written values. The writer must cover the identical range (same base and length) so both sides derive the same `AccessShape`; a byte-wide writer, or one whose overlapping range starts or ends elsewhere, is a mixed-size race Miri reports.
...
- **Patterns to follow:** the existing volatile-copy alignment test on `origin/main`, rewritten as `copy_in_then_copy_out_round_trips_at_every_alignment_and_length`.
- **Test scenarios:**
  ...
  - `copy_in_then_copy_out_round_trips_at_every_alignment_and_length` passes for every alignment of source and shared destination; `access_shape_partitions_the_range_on_aligned_words` and `read_byte_agrees_with_copy_to_at_every_alignment` pin the partition.
...
- **Approach:**
  ...
  2. Write `// SAFETY:` for the remaining blocks; `# Safety` sections for `LeaseSpan::new` (KTD4), `copy_out`, and `copy_in`.
...
- **Approach:**
  1. Add a step installing `valgrind` via `apt-get` and running `cargo +1.98 test -p shm-transport --test ring` with `CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_RUNNER` set to the valgrind command in KTD9 and `EIDNARA_SHM_SKIP_TWO_PROCESS=1`.
  2. In `tests/ring.rs`, make the two-process exchange test return early when `EIDNARA_SHM_SKIP_TWO_PROCESS` is set.
  3. Add a step installing a dated nightly (`nightly-2026-07-27`) with `miri` and running the U5 Miri command with default `MIRIFLAGS`; the step greps its own log for `[1-9][0-9]* passed` because `cargo miri test` exits zero when its filter matches nothing.
...
| Miri | `cargo +nightly-2026-07-27 miri test -p shm-transport --lib -- lease:: backend::ring::miri` | U5 | pass with at least one test executed, no unsupported-operation error |
| valgrind | `CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_RUNNER="valgrind --tool=memcheck --leak-check=full --errors-for-leak-kinds=definite --trace-children=yes --error-exitcode=1" EIDNARA_SHM_SKIP_TWO_PROCESS=1 cargo +1.98 test -p shm-transport --test ring` | U7 | exit 0 |
| Fuzz workspace | `cargo +1.98 check --manifest-path crates/shm-transport/fuzz/Cargo.toml --locked --bins` | U2, U4 | exit 0 |
| Unsafe count | `gawk '/^mod tests \{$/{exit} /\yunsafe\y/{n++} END{print n+0}' crates/shm-transport/src/backend/ring.rs` (`\y` is a gawk word boundary, so the command names `gawk`; counts only lines before the top-level test module) | U3 | below 40 |
