...
`crates/daemon/src/history_summarizer_chunk.rs:611-790` builds the chunk, the transcript,
the raw payload, the fingerprint, and the identity vector from one projection, in
one call, before the producer runs. `AssembledHistorySummarizerFiring` carries all of them
(`:765-790`), and `HistorySummarizerFiringTask` carries that struct through to
`run_history_summarizer_firing` (`lib.rs:5165-5183`, `:5363-5371`). The bytes the model sees
and the bytes stored beside the history_segment come from the same observation.
...
- `:4692-4695` reads `chunk_range` from the durable state; without it the arm gives
  up with `recovering`.
- `:4696-4702` calls `build_history_summarizer_chunk` with the current request's
...
- `:4703-4709` reloads `prior_history_segments` from the store.
...
- `:4754-4757` passes the rebuilt `observed`, `chunk.chunk`, `chunk.text`, and
  `raw_chunk_messages` into `HistorySummarizerReattachRequest`.

`crates/daemon/src/history_summarizer.rs:1468-1619`:
...
- `publish_validated_chunk` compares the durable `predicate.chunk_fingerprint`
  against the rebuilt observation (`history_summarizer.rs:448-460`), so an insertion,
...
- The store's identity fence compares each pinned mid's block identities against
  ...
  message rejects.
- `history_summarizer.rs:2942` `reattach_equal_length_identity_drift_rejects_before_publish`
...
- The rebuilt chunk's `end_index` is not compared against the durable
  ...
  `to_ordinal + 1`, which should cap it, but `build_history_summarizer_chunk` also applies a
...
- `history_summarizer.rs:2369` `tail_identity_extension_during_await_still_publishes`
...
- The validate options differ from the fresh path: `in_emergency: false`
  (`lib.rs:4762`), `force_keep_last_history_segment: false` (`:4767`), and
  `sequence_offset: prior_history_segments.len() as u64 + 1` (`:4761`) rather than
  `MAX(sequence) + 1` (`history_summarizer_chunk.rs:758-763`).
...
4. If the rebuilt `end_index` came out lower than 40, for example because the token
   ...
   summarized, and the history_segments validated against a narrower chunk.
...
- `build_history_summarizer_chunk`'s determinism given the same messages, start, budget, and
...
1. Determinism baseline: assemble a firing, then rebuild with
   `build_history_summarizer_chunk(messages, live, from, budget, to + 1)` and assert
...
- Sources examined: `crates/daemon/src/lib.rs:4696-4725`;
  `crates/daemon/src/history_summarizer_chunk.rs:352-455` (`build_history_summarizer_chunk`'s
  signature and body start), `:611-790` (the fresh assembly for comparison),
  `:840-856` (`end_placeholder`); `crates/daemon/src/history_summarizer.rs:645`
...
- Findings: the exclusive end at `lib.rs:4701` caps the rebuild at the durable
  ...
  state, and `build_history_summarizer_chunk` truncates to that budget. A contraction would
  change the fingerprint, which would reject at `history_summarizer.rs:448-460`, so the
  observable outcome would be a rejected reattach rather than a mismatched publish.
- Missing evidence: I did not read `build_history_summarizer_chunk`'s body in full, only its
...
- Sources examined: `crates/daemon/src/lib.rs:4760-4768` versus
  `crates/daemon/src/history_summarizer_chunk.rs:777-784` and
  `crates/daemon/src/lib.rs:5261-5262` with `:5276`;
  `crates/daemon/src/history_summarizer_validate.rs:538-556` (discard-last healing gated
  on `in_emergency` and `force_keep_last_history_segment`).
- Findings: with `in_emergency: false` and `force_keep_last_history_segment: false`, the
  ...
  a history_segment the original attempt would have kept, narrowing the published range
