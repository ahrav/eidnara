...
import { updateSessionMeta } from "@eidnara/opencode/features/context/storage";
import {
    getOverflowState,
...
import { consumeDeferredHistoryRefresh, consumeDeferredMaterialization } from "../context-handler";
import { parseWrapupArgs, type RegisterCtxWrapupDeps, runPiWrapup } from "./ctx-wrapup";
...
describe("Pi /ctx-wrapup", () => {
    ...
    it("stops on no progress and releases the durable marker", async () => {
        ...
        try {
            ...
            expect(result).toContain("## Eidnara Wrapup — Partial");
    ...
    it("signals deferred history and materialization after a wrapup publish", async () => {
        ...
        try {
            ...
            expect(result).toContain("## Eidnara Wrapup");
    ...
    it("leaves the pending Pi marker queued until the next consuming context pass", async () => {
        ...
        try {
            ...
            expect(result).toContain("## Eidnara Wrapup");
            expect(result).not.toContain("## Eidnara Wrapup — Partial");
