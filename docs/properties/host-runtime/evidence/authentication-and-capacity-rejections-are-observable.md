...
- **The crate has no logging or metrics dependency.**
  `crates/host-runtime/Cargo.toml:13-25` lists `serde`, `hmac`, `subtle`, `serde_json`,
  `sha2`, `fastembed`, `ort`, `tokio`, `tokio-util`, `rustix`, `getrandom`, and
  `shm-transport`. Dev-dependencies at `:27-35` add nothing relevant. A grep for
  `tracing` and `log::` across `crates/host-runtime/src/` returns zero matches, so there
  is no channel to carry a rejection record even if one were emitted.
- The only stderr writer in production code is
  `crates/host-runtime/src/panic_boundary.rs:43`, an `eprintln!` of a fixed redacted
...
- The other in-crate observability channel is `FatalCell`
  (`crates/host-runtime/src/runtime.rs:65-88`), a first-failure latch carrying a
...
- A fourth, pre-authentication site behaves the same way:
  `crates/host-runtime/src/runtime.rs:1012-1015`, handshake-permit exhaustion, is
...
- **The peer address is never captured, though it is obtainable at site 1.**
  ...
  across `crates/host-runtime/src/` returns zero matches, so no path recovers it. But
