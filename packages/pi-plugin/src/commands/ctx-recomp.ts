...
import { COMPARTMENT_STRUCTURAL_SYSTEM_PROMPT } from "@eidnara/opencode/hooks/context/compartment-prompt";
import { executeContextRecompWithResult } from "@eidnara/opencode/hooks/context/compartment-runner";
import {
    type PartialRecompRange,
    snapRangeToCompartments,
} from "@eidnara/opencode/hooks/context/compartment-runner-partial-recomp";
import type { RawMessageProvider } from "@eidnara/opencode/hooks/context/read-session-chunk";
...
export function registerCtxRecompCommand(pi: ExtensionAPI, deps: RegisterCtxRecompDeps): void {
    pi.registerCommand("ctx-recomp", {
        description: "Rebuild Eidnara compartments from raw Pi session history",
        handler: async (args, ctx) => {
            ...
            if (parsed.kind === "error") {
                sendCtxStatusMessage(pi, {
                    title: "/ctx-recomp",
                    text: `## Eidnara Recomp — Invalid Arguments\n\n${parsed.message}`,
                    level: "error",
                });
                return;
            }
            ...
            const argsKey =
                parsed.kind === "partial" ? `${parsed.range.start}-${parsed.range.end}` : "";
            const now = Date.now();
