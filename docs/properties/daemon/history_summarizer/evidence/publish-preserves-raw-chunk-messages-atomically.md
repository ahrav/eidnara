...
frames the history_summarizer as "the only path that irreversibly substitutes unverified
...
Capture, before the model runs, in `crates/daemon/src/history_summarizer_chunk.rs`:
...
- `:765-790` returns them on `AssembledHistorySummarizerFiring` (fields at `:771` and
  `:773`).

Carriage, in `crates/daemon/src/history_summarizer.rs`:
...
- `:9472-9481` calls `insert_chunk_transcripts_tx` when either payload is
  ...
  append just used.
- `:12679-12681` returns early when the history_segment list is empty.
...
- `:12698-12714` inserts one row per history_segment, `INSERT OR REPLACE`, carrying
  both payloads.
...
revert at `:9106`, which deletes transcripts and history_segments for the same
reverted suffix inside one transaction (`:9105-9111`).

## Failure scenario

If the raw copy were absent for a published history_segment, the folded range would
...
inside the same transaction as the history_segment append.

Two dependencies matter:

- The history_segment list must be non-empty, or `insert_chunk_transcripts_tx`
  returns early (`:12679-12681`). Validation refuses an empty set
  (`history_summarizer_validate.rs:487-491`), so the history_summarizer path cannot reach it.
- Both payloads must not be absent, or `:12691-12693` returns early. The
  history_summarizer path always supplies both.
...
1. Happy path: a configured model chain, one accepted publish, then inflate
   ...
   against the JSON that `history_summarizer_chunk.rs:717-727` produced. The assembled
...
4. Negative: a compression failure on the raw payload aborts the publish with no
   history_segment appended. Hard to construct without a seam, since deflate does not
   fail on valid UTF-8 input in practice.

## Investigation log

### Q: `insert_chunk_transcripts_tx` returns early when both payloads are absent and when the history_segment list is empty. Can another caller publish history_segments with no recoverable original?

- Sources examined: `crates/memory-store/src/lib.rs:1766-1782`
  (`HistorySummarizerPublishRequest`, both payload fields are `Option`), `:9472-9481`,
  `:12671-12716`; every in-repo caller of `publish_history_summarizer_chunk`
  (`crates/daemon/src/history_summarizer.rs:529` and the two publication fences at
  `crates/daemon/src/lib.rs:3310` and `:3347`).
- Findings: exactly one production construction site of the request exists,
  `history_summarizer.rs:513-526`, and it always supplies both payloads. The `Option`
...
- Conclusion: unresolved, needs a decision on whether the store should require a
  raw payload when appending history_segments, rather than relying on the single
  module caller to supply one. The property as written is about the history_summarizer
