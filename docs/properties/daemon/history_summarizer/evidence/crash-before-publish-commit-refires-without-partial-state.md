...
`crates/daemon/src/history_summarizer.rs`, in order:
...
2. `:1264-1278` load, `fire`, persist. This writes the phase `Firing`, the
   ...
   `history_segment_set_generation` (`:255-275`).
...
Steps 2 through 5 are four separate `persist_history_summarizer_state` calls, each of which
is its own `store.commit` (`history_summarizer.rs:391-403`). The chunk text, the transcript,
the raw messages, and the validated history_segments exist only in memory until step 6.

### What recovery does with each phase

`crates/daemon/src/history_summarizer.rs:620-655`:
...
`abandon_with_detail` (`history_summarizer.rs:348-361`) rebuilds the state from
`HistorySummarizerDurableState::default()`, preserving only `firing_seq`, the new backoff,
...
row version (`persist_history_summarizer_state` commits at `history_summarizer.rs:402`), so the CAS
...
`history_summarizer.rs:1497-1513` deliberately does **not** abandon when
...
session (`history_summarizer.rs:251-253`), and the session would never fold again. Context
...
leaves a run that ModelExecution may have completed. The reattach path is what recovers it,
and it redrains from the start rather than re-sending
(`history_summarizer_producer.rs:864-878`, test at `history_summarizer.rs:2881`).
...
- `persist_history_summarizer_state` short-circuits when the meta is unchanged
  (`history_summarizer.rs:399-401`), returning the current row version without committing.
...
1. Simulated restart, which already exists: `history_summarizer.rs:4596`
...
2. Stale-resume rejection: drive to `Publishing`, capture the predicate and row
   ...
   captured values. Assert `CasConflict` or `StateMismatch` and no history_segment
...
- Sources examined: `crates/daemon/src/history_summarizer.rs:629-647` (the recovery
  ...
  state), `:391-403` (`persist_history_summarizer_state`).
