...
- `crates/shm-transport/src/backend/ring.rs:126-137` declares
...
- `packages/shm-native/src/lib.rs:283` and `:290` show the addon raising
...
2. The local side now treats the direction as terminal. Charges are retained
   rather than returned, per `docs/shm-transport.md:79`.
...
`crates/shm-transport/tests/ring.rs:240`
...
- Sources examined: `ring.rs:126-137` (page layout), `:719-727` (`Ring`
  ...
  quarantine call sites at `packages/shm-native/src/lib.rs:283`, `:290`,
  `:319`, `:324`, `:345`, `:352`, `:381-382`, `:418-419`, and
  `docs/shm-transport.md:79`, `:21`.
...
### Q: Does the `docs/shm-transport.md:116` non-guarantee about malicious peers extend to control pages, or only to payload bytes?

- Sources examined: `docs/shm-transport.md:116` in full.
