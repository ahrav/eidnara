...
import { registerCtxRecompCommand } from "./eidnara-recomp";
import { registerCtxStatusCommand } from "./eidnara-status";
...
describe("Pi /eidnara-flush", () => {
    it("routes session.flush through the module client and reports the armed result", async () => {
        ...
        const [entry] = await run("eidnara-flush");
...
describe("Pi /eidnara-status", () => {
    ...
    it("resolves project settings and the daemon route from the same invoking cwd", async () => {
        ...
        await run("eidnara-status", "", { cwd: "/tmp/project-a" });
        await run("eidnara-status", "", { cwd: "/tmp/project-b" });
    ...
    it("opens the dialog with a daemon answer and routes the read through the cwd root", async () => {
        ...
        const entries = await run("eidnara-status", "", { hasUI: true, ui });
    ...
    it("reports a status failure as text instead of opening a dialog of zero counts", async () => {
        ...
        registerCtxStatusCommand(pi, statusDeps(module.client));
        const { ui, opened } = dialogUi();
        const [entry] = await run("eidnara-status", "", { hasUI: true, ui });
...
describe("Pi /eidnara-recomp", () => {
    ...
    for (const [disposition, expected, level] of cases) {
        it(`maps the ${disposition} disposition`, async () => {
            ...
            const [entry] = await run("eidnara-recomp");
    ...
    it("refuses a message range or malformed arguments without calling the daemon or touching the footer", async () => {
        ...
        const module = fakeModuleClient(() => ({}));
        registerCtxRecompCommand(pi, { moduleClient: module.client });
        const [ranged] = await run("eidnara-recomp", "1-40");
        ...
        const [, malformed] = await run("eidnara-recomp", "bogus");
