...
| `scripts/eidnara-migration/check.ts`, `check.test.ts` | receipt, registry, waiver, and catalog validation; `fixture` registry kind with `byte-stable`, `generator`, and `external-record` roles; identity classes `frozen-durable`, `external-protocol`, `third-party`; registry verification rejects migration-machinery tokens (`schema_migrations`, `MIGRATIONS`, `LATEST_MIGRATION_VERSION`, `BOOTSTRAP_MIGRATION_VERSION`, `ensureColumn`, `Migration {`, `fn migrate`, `run_migrations`) and unregistered persistent literals in every file Cargo compiles into a crate's library, binaries, or build script (every `.rs` outside the `tests/`, `benches/`, and `examples/` directories and outside any `[[test]]`, `[[bench]]`, or `[[example]]` path the manifest names, so a `[lib]` or `[[bin]]` outside `src/` is scanned) and under `packages/*/src`, with `#[cfg(test)]` items, variants, and fields blanked out rather than files skipped by a test-like name and the attribute recognized only outside comments and string literals; a `[lib]`, `[[bin]]`, or build script the manifest names is scanned wherever it lives; a check anchor must be a test the build can run (not `#[ignore]`, not under a `cfg` predicate false in every build, with several attributes on one line each read) and a Rust anchor is read with comments and string literals blanked and with the `cfg` predicates of every enclosing item and inner attribute evaluated; a `[lib]`, `[[bin]]`, or build script the manifest names is scanned as Rust whatever its extension; every TypeScript file under `packages` is scanned, not only `src`; TypeScript regex literals are lexed past; a TypeScript anchor must be a `test`, `it`, or `describe` call outside comments and strings and not `.skip` or `.todo`, nor nested in a skipped suite; a property-impact's provenance entries and carried-forward records must name the receipt's pinned source at its pinned commit; receipt source blobs must belong to the pinned commit's own tree (`rev-list --objects <commit>^{tree}`), not merely to its history | `bun test scripts/eidnara-migration` |
