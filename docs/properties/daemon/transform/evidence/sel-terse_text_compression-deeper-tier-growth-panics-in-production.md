...
Reading `new_terse_text_compression_units` for the eligibility ladder
(`sel-terse_text_compression-eligibility-ladder-deterministic-over-frozen-basis`) surfaced a bare
...
`terse_text_compression_payload(core, &block_id)` (`:6353`), the currently frozen `cav:*` unit for
that block. `compressed` is `crate::terse_text_compression::compress(&source, level)` (`:6360`),
where `source` is the block's persisted original bytes,
`String::from_utf8(row.source_bytes.clone())` (`:6338-6339`), and `level` is
`terse_text_compression_level(target_depth)` (`:6359`), one of `Lite`, `Full`, `Ultra` for depths
1, 2, 3 (`:6274-6281`).
...
That means the assertion is a claim about `terse_text_compression.rs`'s level ladder, not about
...
reaches this point. `terse_text_compression.rs` is 651 lines with 40 inline tests and belongs to
...
units.push(terse_text_compression_unit(&block_id, target_depth, payload));
```
...
- **Config default.** `TerseTextCompressionConfig::default()` is
  `{ enabled: false, min_size: DEFAULT_TERSE_TEXT_COMPRESSION_MIN_SIZE }` (`config.rs:73-79`,
  `enabled: false` at `:76`), and `DaemonConfig::default()` takes it (`:126`).
- **Request default.** `TransformRequest.terse_text_compression_enabled` is
  `#[serde(default)] pub terse_text_compression_enabled: bool` (`transform.rs:729-731`), so an
...
- **Shipped OpenCode path.**
  ...
  `terse_text_compression_enabled: !sessionMeta.isSubagent && deps.terse_text_compressionTextCompression?.enabled === true`,
  so it is false unless the user's config says exactly `true`.
- **Shipped Claude Code path.** `apply_claude_code_config_controls`
  (`lib.rs:173-193`) copies `config.terse_text_compression.enabled` into the request at `:186`,
  which is the same user config leaf.
- **Documented default.** `CONFIGURATION.md:724` (source-catalog path, not present at HEAD) gives
  `terse_text_compression.enabled` as `boolean`, default `false`.
...
A user sets `terse_text_compression.enabled: true`. A session accumulates a long
...
`terse_text_compression_target_depth` returns 3 (`:6285-6297`), the depth guard passes, and
`compress(source, Ultra)` comes back longer than the frozen `Full` bytes.

`apply_once` panics inside `new_terse_text_compression_units` with
`terse_text_compression deeper tier grew frozen payload for <block_id>`. The pass does not commit.
...
None in the concurrency sense. The dependency is entirely on `terse_text_compression.rs`'s level
ladder and on the candidate's position in the sorted list, which is a function of
the tag window.

## What a test must construct

Two layers. At the `terse_text_compression.rs` layer, a property test over the three levels
...
already builds a `core` with a pre-existing terse_text_compression unit and calls
`new_terse_text_compression_units(&core, &request, &tags, &live, None, true, 1)`. What is missing
...
### Q: Can `terse_text_compression::compress` at a deeper level produce more bytes?

- Sources examined: `terse_text_compression_level` (`transform.rs:6274-6281`) for the depth-to-level
...
- Missing evidence: `terse_text_compression.rs` was not read; it is 651 lines and belongs to 4e.
...
- Conclusion: unresolved, needs a read of `terse_text_compression.rs`'s level ladder. The record's
