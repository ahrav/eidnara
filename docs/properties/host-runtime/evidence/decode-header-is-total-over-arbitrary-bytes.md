...
exactly one directory, `crates/shm-transport/fuzz`, whose three targets are
`frame_descriptor.rs`, `provider_grant.rs` and `provider_sample.rs`. All three
are transport decoders and none reaches `host-runtime`'s `decode_header`.
...
- Sources examined: `wire.rs:292-297` (`header_len_for_version`), `:306-368`
  ...
  (`PROTOCOL_VERSION`, `HEADER_LEN`), `docs/host-wire-protocol.md:222-234`
