...
`docs/migration-version-lanes.md:47-51` (source-catalog path, not present at HEAD) states that "Application connections
...
- `crates/memory-store/src/sqlite_runtime.rs:113-140` (source-catalog path, not present at HEAD)
...
`storage:287` activates WAL with
...
concurrent-reader assumption stated at `storage:285-286` no longer
holds. The `-wal` and `-shm` permission tightening at `storage:317-320`
...
The same shape applies to `foreign_keys`: `storage:291` sets it, but
...
confirms it, and the migration SQL depends on it. `crates/memory-store/src/lib.rs:1291-1294`
declares `FOREIGN KEY (database_incarnation_id, project_id) REFERENCES
claim_mirror_projects(...) ON DELETE CASCADE`; with foreign keys off, that
...
2. `MemoryStore::open` a temp-dir descriptor.
...
needs no special mount: after `MemoryStore::open`, read `PRAGMA journal_mode` through
...
- Sources examined: `storage:287-288`, which maps the result with
  `.map_err(|e| StoreError::Backend(e.to_string()))?`; the contrast with
  `crates/memory-store/tests/sqlite_runtime.rs:178-181` (source-catalog path, not present at HEAD), which deliberately uses
...
- Sources examined: content searches for `PRAGMA` across `crates/`; the four
  verifier reads in `sqlite_runtime.rs`; `crates/memory-store/src/lib.rs:5453`,
