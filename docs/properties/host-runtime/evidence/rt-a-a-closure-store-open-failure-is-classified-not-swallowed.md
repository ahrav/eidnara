...
`crates/host-runtime/src`. Finding the real constructor led to two call sites that
discard the error.
...
Scope note: `serve.rs` is in `crates/daemon`, outside this sub-part's declared
...
`docs/research/dry-audit-2026-08-29/host-and-crates.md:254` note establishes
...
- Findings: no `#[cfg(test)]` block. The file ends at `hex` (`:1114-1122`). All
  coverage is external: `crates/host-runtime/tests/harness_closure.rs` (10 `open` calls,
