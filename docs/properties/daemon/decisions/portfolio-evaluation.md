...
`enabled` and `min_clusters`. `CONFIGURATION.md:237-238` (source-catalog path, not present at HEAD) documents `enabled: true`
...
1. **Whether 4f's boundary is the crate or the product, because the same fact is a
   ...
   `CONFIGURATION.md` (source-catalog path, not present at HEAD) against `config.rs` alone or against `config.rs` plus the
