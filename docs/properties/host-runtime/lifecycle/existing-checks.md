...
Every claim-bearing check for `crates/host-runtime/src/{lifecycle,generation,connection,frame_read,panic_boundary}.rs`
at `d90e7811`.
...
Verified against `the source repository workflows directory `. `host-runtime` has 26 integration test
binaries. CI names four things:

| Workflow line | Invocation |
| --- | --- |
| `ci.yml:122` | `cargo nextest run -p host-runtime --lib --test shm_failure_modes --test shm_soak` |
| `ci.yml:167` | `cargo nextest run -p host-runtime --test shm_transport --test transport_negotiation` (Linux) |
| `ci.yml:178` | `cargo nextest run -p host-runtime --test shm_soak` (macOS) |
| `ci.yml:179` | `cargo test -p host-runtime --lib` with a single filter (macOS) |
| `ci.yml:183` | `cargo test -p host-runtime --doc` |
...
workflow is `-p daemon --test lifecycle_cli`, a different crate and file.
...
macOS, and a prior commit records that the `host-runtime` library test target did not
...
`crates/host-runtime/tests/lifecycle.rs` and `crates/daemon/tests/lifecycle_cli.rs`.
...
18. `docs/AUDIT-KNOWN-ISSUES.md` contains **no** `host-runtime` lifecycle, generation,
