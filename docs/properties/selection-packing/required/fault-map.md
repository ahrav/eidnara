...
| `EvalBudget` expired or cancelled before the phase, or cancelled inside the estimator | `EvalBudget::new` with a past deadline; `EvalBudget::cancel`, including from a `CostEstimator`. | `packing.required.budget_exhausted_before_optional` | packing-required-phase-precedes-optional-work |
| Malformed floating budget | `ClaudeTokens::from_budget` with NaN, negative, fractional, infinite, above-2^53, or absent values; the legacy trim with the same values. | `packing.budget.malformed_budget_presented` | packing-budget-is-an-integer-never-clamped |
