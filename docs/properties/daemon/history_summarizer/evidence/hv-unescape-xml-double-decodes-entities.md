...
`crates/daemon/src/history_summarizer_validate.rs:1148-1154`:
...
- `history_summarizer_prompt.rs:104-108`: identical body.
- `history_summarizer_prompt.rs:96-102` (`escape_xml_attr`) also escapes `&` first, then
  `"`, `'`, `<`, `>`.
...
`history_summarizer_prompt.rs:427-432`:
...
`history_summarizer_validate.rs:1305-1868` for entity strings; none appears.
...
A session about XML or HTML templating. The history_summarizer summarises:

```
<history_segment start="1" end="30" title="Escaping rules" ...><p1>Template output must use &amp;lt; and &amp;gt; for angle brackets.</p1></history_segment>
```

Gate: `extract_tier` (`:1112-1134`) returns
`unescape_xml(body.trim())`, which yields
`Template output must use < and > for angle brackets.` The history_segment publishes
with that text.
...
2. `escape_xml_content` exists in two places (`decay_render.rs:80`,
   `history_summarizer_prompt.rs:104`) with identical bodies. The round-trip test should
...
history_segment whose `p1` is `&amp;lt;`, then assert the stored `p1` is `&lt;` rather
than `<`.
...
- Sources examined: `history_summarizer_validate.rs:261-263` (the doc comment stating the
  ...
  `history_summarizer_validate.rs:816-820` (an explicit note where Rust deliberately
...
- Sources examined: `history_summarizer.rs:38-67` (`to_stored_history_segment`),
  `decay_render.rs:80-84`, `:235`, `:245`, `history_summarizer_prompt.rs:239-263`.
- Findings: The projection clones verbatim; the renderer re-escapes; the prompt
  ...
  `history_summarizer.rs:1727`, which is the input, not the model's output).
- Missing evidence: none needed.
- Conclusion: resolved with answer — fully silent. Because the model's raw output
  text is not stored alongside the parsed history_segments, there is no artifact
