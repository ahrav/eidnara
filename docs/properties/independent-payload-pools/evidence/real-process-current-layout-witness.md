...
- Missing evidence: the Rust client's cross-process run is still owed; a native/TypeScript daemon-level witness on a runtime that reports the detachment capability remains a recorded gap; the capability witness records artifact and runtime identity for every skip.
