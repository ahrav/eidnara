...
- In [rust-mode-transform.ts](../../../../../packages/opencode-plugin/src/hooks/context/rust-mode-transform.ts),
  ...
  (`:1514-1522`); a decline logs at warn for `pass_count` and debug for
  `session_busy` (`:1517`) and returns a resolved promise without touching
  ...
  (`PassDeclined` default at `:735`, `:1496-1503`). Ordinal annotation and
  scan pages charge through the same lease (`:1158-1170`), and candidate
  slots are charged at `CANDIDATE_SLOT_BYTES` (8) each (`:710`, `:1441`)
...
- The lease releases in `.finally` after `execute` settles and before
  `deliverTransformNotes` (`:1526-1527`). By then the publication block has
  ...
  `wireCaches` owner (`:122`, `:789`, `:1482`) and to `states` (`:1478`).
...
- In [transform-capture.ts](../../../../../packages/opencode-plugin/src/hooks/context/transform-capture.ts),
  ...
  of a retained string or symbol description (`:151-162`), one slot per
  descriptor read (`:228-234`), and the declared array length before element
  reads (`:277-278`), so the byte budget also bounds traversal work.
  `TransformCaptureAdmission` (`:479-552`) defaults to 64 passes and 64 MiB
  and keeps one lease per session. Neither its constructor (`:482-487`) nor
...
- Sources examined: `run` at `rust-mode-transform.ts:1514-1522`, the charge
  sites at `:979-1009`, `:1158-1170`, `:1441`, `buildNativeCandidate` at
  `:578-626`, `hostArrayReplacementRejection` at
  `transform-capture.ts:439-455`, `TransformCaptureAdmission` at `:479-552`,
...
- Conclusion (2026-09-13, revision-bound run after merging `origin/main` at
  ...
  markers:
  - `rust-mode-transform.test.ts:1845` "admits a full pass over 1,000
    realistic messages under the default budget with room for a second
    session". Marker: `jsonBytes > 2 * 1024 * 1024` (`:1876`);
    `peakCharge < admission.remainingBytes / 2` (`:1897`); `chargedBytes` 0
    after.
  ...
  - `:2750` "keeps every pass within the global count limit and declines
  ...
  - `:3046` "shares default admission across factories and cancels the
    earlier session owner". Marker: `defaultTransformCaptureAdmission`
    counters held across factories until the first settles.
  - `:2243` (8 cases). Marker: `heldBytes > MODULE_ORDINAL_PAGE_SIZE *
    ORDINAL_ENTRY_RETAINED_BYTES` at the scan yield (`:2282`).
  - `:2025`, `:2156`, `:1961`: blocker leases leave exact remainders
    (`remainingBytes - candidateLength * 8 + offset` at `:1992`) and the
  ...
  - `transform-capture.test.ts:889` "keeps counts and charges equal to a
    ...
    (removing `owner.leases.delete` at `transform-capture.ts:542`) is recorded
    as reported, not re-run.
  - `transform-capture.test.ts:971`, `:998`, `:1020`, `:1036`, `:1046`,
    `:1070`, `:1147`: default limits, invalid charges, per-session lease,
