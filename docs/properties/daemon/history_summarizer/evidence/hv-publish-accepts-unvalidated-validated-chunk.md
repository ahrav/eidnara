...
`crates/daemon/src/history_summarizer_validate.rs:225-238`:
...
construction routes exist that bypass `validate_history_summarizer_output`: struct literal,
`Default::default()`, and deserialising attacker-influenced JSON.
`ValidatedHistorySegment` (`:202-223`) is likewise all-`pub` with `Deserialize`.

### The consumer is public and takes it on faith

`history_summarizer.rs:444`:
...
`ValidatedPublishRequest.validated` is `&'a ValidatedChunk` (`history_summarizer.rs:420`).
The function's first act is a fingerprint comparison (`:449-461`), which binds the
CHUNK to the FIRING, then it projects history_segments (`:463-467`) with no check that
the `validated` argument came from the gate.

Both modules are public: `lib.rs:19` `pub mod history_summarizer;` and `lib.rs:23`
`pub mod history_summarizer_validate;`. So this is reachable from any crate depending on
...
rg -n "ValidatedChunk\s*\{" crates/daemon/src/*.rs
  history_summarizer_validate.rs:227   (the definition)
  history_summarizer_validate.rs:628   (the gate's own Ok return)
  history_summarizer.rs:4476           (test code)
```

```
rg -n "publish_validated_chunk" crates/daemon/src/history_summarizer.rs
  :444   definition
...
callers are `history_summarizer.rs:1419` (reattach) and `:1592` (drive). So both production
...
hypothetical or awkward. `history_summarizer.rs:4476-4489` builds a literal `ValidatedChunk`
with a hand-written `ValidatedHistorySegment`, and `:4337` and `:4423` pass
...
`history_summarizer.rs:60-62`, inside `to_stored_history_segment`:
...
`publication_floor_ordinal: validated.unprocessed_from` (`history_summarizer.rs:1725`).

### What a bypass would skip

All 22 rejecting checks. Most consequentially, the ones that protect durable
coverage: endpoint existence in the pinned chunk (`history_summarizer_validate.rs:941-957`),
...
editable by anything with store access, the re-driven publish writes history_segments
that no gate ever saw. `publish_validated_chunk` compares the chunk fingerprint
(`:449-461`) and would still catch a stale CHUNK, but it cannot catch history_segment
content or ranges that were never validated against that chunk.

Consequence: arbitrary ranges and content in durable history_segments, plus a
...
honestly from `p1` (`history_summarizer.rs:63-67`), and its consequence when it fires is
...
1. A structural test that fails when a new caller appears. The cheapest honest
   ...
   `crates/daemon/src/history_summarizer.rs`, count occurrences of
...
2. A coverage marker at the gate, per METHOD.md's rule to assert independent
   ...
   publish transaction was entered, and that `validate_history_summarizer_output` returned
   ...
   `history_summarizer.rs:381`, and the durable `firing_seq` the gate's caller holds), so the
...
- Sources examined: `history_summarizer_validate.rs:226-238` (the derives and field
  visibility), `:202-223` (`ValidatedHistorySegment`, same shape), `history_summarizer.rs:4337`,
  `:4423`, `:4476-4489` (the three test construction sites),
  `history_summarizer_chunk.rs:1419-1428` (a test that builds one via the gate).
- Findings: A private zero-sized witness field would break all three test
  ...
  (`history_summarizer.rs:4337`, `:4423`). It would also break `Deserialize`, and whether
...
- Sources examined: `history_summarizer.rs:1641-1745` (`publish_output_from_awaiting` in
...
- Findings: Every path from a producer output to a history_segment write passes through
...
- Conclusion: resolved with answer — yes, fail-closed at HEAD on the history_segment
