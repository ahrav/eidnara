...
use host_runtime::{HostError, HostHandler};

use support::raw_client::{
    self, FLAGS_INTERACTIVE, FLAGS_PURE_HEADER, TY_GOODBYE, TY_PING, TY_PONG, TY_REQUEST,
    TY_RESPONSE,
};
use support::{BindPolicy, Event, LINKED_MODULE_ID, TestHandler, TestHost, mode_body};
...
impl HostHandler for DelayedRouteGoneHandler {
    ...
    async fn bind(
        ...
        target: host_runtime::RouteTarget,
        identity: host_runtime::RouteIdentity,
    ...
    async fn route_gone(&self, route: host_runtime::RouteHandle) {
...
async fn publication_waits_for_initialization_to_return() {
    ...
    let task =
        tokio::spawn(async move { host_runtime::run(run_handler, config, run_shutdown).await });
...
async fn initialization_timeout_aborts_and_joins_the_callback() {
    ...
    let config = host_runtime::HostConfig {
        data_dir: Some(data_root.path().to_path_buf()),
        timing: host_runtime::HostTiming {
            lifecycle_callback_deadline: Duration::from_millis(50),
            ..Default::default()
        },
        ..Default::default()
    };
    let result = host_runtime::run(
        handler.clone(),
        config,
    ...
    assert!(matches!(
        result,
        Err(host_runtime::HostError::InitFailed(_))
    ));
    handler.release_init();
...
async fn transport_serves_while_the_handler_reports_degraded_storage() {
    ...
    .await;
    host.handler
        .set_health(host_runtime::HealthStatus::Degraded, Some("store opening"));
...
async fn ping_and_consumer_correlations_do_not_cross_settle() {
    let host = TestHost::start_with(|config| {
        config.liveness = Some(host_runtime::LivenessPolicy {
...
async fn a_hung_route_gone_callback_is_fatal_rather_than_falsely_graceful() {
    ...
    assert!(
        matches!(
            err,
            host_runtime::HostError::LifecycleFatal(_)
                | host_runtime::HostError::ShutdownDeadlineExpired
        ),
...
async fn a_panicking_route_gone_callback_is_fatal() {
    ...
    assert!(
        matches!(
            result,
            Err(host_runtime::HostError::LifecycleFatal(_))
                | Err(host_runtime::HostError::ShutdownDeadlineExpired)
        ),
...
async fn lock_release_follows_route_gone_and_handler_drop() {
    ...
    .await;
    assert!(
        matches!(blocked, Err(host_runtime::HostError::Instance(_))),
    ...
    assert!(
        matches!(
            blocked_during_drop,
            Err(host_runtime::HostError::Instance(_))
        ),
...
async fn delayed_lifecycle_callback_runs_shutdown_once_before_successor_starts() {
    ...
    let run_shutdown = shutdown.clone();
    let task =
        tokio::spawn(async move { host_runtime::run(runtime_handler, config, run_shutdown).await });
    let publication = support::connection_file(data_root.path());
    let deadline = tokio::time::Instant::now() + BUDGET;
    while !publication.exists() {
...
async fn a_publication_failure_runs_the_shutdown_callback_before_lock_release() {
    ...
    let host_dir = data_root.path().join("eidnara").join("run");
...
async fn host_shutdown_commits_after_the_full_response_and_stops_gracefully() {
    let host = TestHost::start().await;
    let publication = host.publication_path();
    let record = host.runtime_dir().join(host_runtime::LIFECYCLE_RECORD_NAME);
...
async fn probe_observes_running_then_stopped_across_an_incarnation() {
    ...
    let observed =
        host_runtime::probe_lifecycle(Some(&data_root), &host_runtime::ProbeFreshness::default())
            .expect("probe while running");
    assert_eq!(observed.state, host_runtime::LifecycleState::Running);
    ...
    assert_eq!(observed.state, host_runtime::LifecycleState::Stopped);
    assert!(observed.instance_lock_free);
    assert!(
...
/// Replacing the whole managed `eidnara` subtree while a host serves leaves
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn a_replaced_eidnara_subtree_cannot_admit_an_overlapping_incarnation() {
    let data_root = tempfile::tempdir().expect("temp root");
    let host = TestHost::try_start_with(TestHandler::new(), {
        let path = data_root.path().to_path_buf();
        move |config| config.data_dir = Some(path)
    ...
    let eidnara = data_root.path().join("eidnara");
    std::fs::rename(&eidnara, data_root.path().join("eidnara-old"))
        .expect("replace the managed subtree");

    let root = data_root.path().to_path_buf();
    let observed = tokio::task::spawn_blocking({
        let root = root.clone();
        move || host_runtime::probe_lifecycle(Some(&root), &host_runtime::ProbeFreshness::default())
    })
    .await
    .expect("probe task")
    .expect("probe");
    assert_eq!(
        observed.state,
        host_runtime::LifecycleState::Wedged,
    ...
    assert!(
        matches!(
            blocked,
            Err(HostError::Instance(
                host_runtime::InstanceError::AlreadyRunning
            ))
        ),
...
async fn successive_incarnations_lock_the_same_coordination_inodes() {
    ...
    let coordination = data_root
        .path()
        .join(host_runtime::COORDINATION_DIR_NAME)
        .join(host_runtime::LIFETIME_LOCK_NAME);
