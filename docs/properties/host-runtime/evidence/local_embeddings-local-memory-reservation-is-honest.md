...
- `crates/host-runtime/src/local_embeddings/jobs.rs` defines the runtime input charge in
...
- `crates/host-runtime/src/local_embeddings/bundle.rs` computes
...
- `crates/host-runtime/src/local_embeddings/mod.rs` builds the local byte budget from
...
- `declared_local_input_capacity` maps the same invalid arithmetic to
  `u64::MAX`, not zero.
- `LocalEmbeddingsComponent::resources` saturating-adds the retained-result cap, so the
...
- `crates/host-runtime/examples/local_embeddings_host.rs` sums the actual composite
  resource declarations when it derives the host resident-byte budget.
- `crates/host-runtime/examples/local_embeddings_perf.rs` adds the model and retained
