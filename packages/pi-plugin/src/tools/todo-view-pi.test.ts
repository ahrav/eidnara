...
function commandCtx(sessionId = "ses-test", ui = makeUi().ui): ExtensionCommandContext {
    return {
        hasUI: true,
        ui,
...
describe("todowrite tool rendering", () => {
    it("renders glyph lines while preserving the exact JSON text result", async () => {
        ...
        if (!renderResult || !renderCall) throw new Error("todowrite render hooks missing");
        ...
        expect(renderCall({ todos } as never, identityTheme, {} as never).render(80)[0]).toContain(
            "Todos — 3 active",
        );
    });
    ...
    it("evicts the oldest cached tool-call todos after the cap", () => {
        ...
        expect(
            renderCall({}, identityTheme, { toolCallId: "call-1" } as never).render(80)[0],
...
describe("/todos command", () => {
    ...
    it("caps each /todos section with a +N more tail", async () => {
        ...
        const { ui, notifyCalls } = makeUi();
        await commands.get("todos")?.handler("", commandCtx("ses-command-capped", ui));
        expect(notifyCalls).toHaveLength(1);
        const [text, level] = notifyCalls[0] as [string, string];
        expect(level).toBe("info");
        expect(text).toContain("── Pending ──");
...
describe("TodoOverlay lifecycle", () => {
    it("registers once and refreshes with requestRender", () => {
        setTodoSnapshot("ses-overlay", [{ id: "1", content: "Plan", status: "pending" }]);
        ...
        expect(setWidgetCalls[0][0]).toBe("eidnara-todos");
    ...
    it("auto-hides on empty snapshots", () => {
        ...
        expect(setWidgetCalls[1]).toEqual(["eidnara-todos", undefined]);
    });
    ...
    it("hides completed tasks from previous turns after showing them once", () => {
        setTodoSnapshot("ses-overlay", [{ id: "1", content: "Done", status: "completed" }]);
        const overlay = new TodoOverlay();
        const { ui, setWidgetCalls } = makeUi();
        overlay.setUICtx("ses-overlay", ui);
        overlay.update("ses-overlay");
    ...
    it("seeds from session_meta.last_todo_state and clears on session switch", async () => {
        ...
        expect(setWidgetCalls).toHaveLength(1);
        await handlers.get("session_before_switch")?.({ type: "session_before_switch" }, ctx);
        expect(getTodoSnapshot("ses-seeded").todos).toEqual([]);
        expect(setWidgetCalls[setWidgetCalls.length - 1]).toEqual(["eidnara-todos", undefined]);
        expect(
            renderCall({}, identityTheme, {
