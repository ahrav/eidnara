...
`crates/host-runtime/src/setup_socket.rs`, `auth.rs`, `instance.rs`,
`connection_file.rs`, and `packages/shm-native/src/setup.rs` as the peer half.
...
| --- | --- | --- | --- | --- |
| `crates/host-runtime/src/instance.rs` | `mod tests`, from `:889` | 535 | **22** | no |
| `crates/host-runtime/src/setup_socket.rs` | `mod tests`, `:441-826` | 386 (47% of the file) | **12** | no |
| `crates/host-runtime/src/auth.rs` | `mod tests`, from `:633` | 480 | **11** | no |
| `crates/host-runtime/src/connection_file.rs` | `mod tests`, `:337-471` | 135 | **4** | no |
...
- Every `-p host-runtime` test invocation in `ci.yml` carries a `--test <name>` filter.
  A `--test` filter selects one integration binary and does not build the lib
  target, so no `host-runtime` in-crate unit test is compiled in CI, let alone run. The
  invocations are at `:132`, `:133`, `:134`, `:178-179`, `:187` and `:190`; none is
  an unfiltered `cargo nextest run -p host-runtime` or a `--lib` run.
- `ci.yml:177` runs `cargo nextest run -p shm-native -p shm-transport`
  **unfiltered** on Linux, and `:184` runs `cargo nextest run -p shm-native`
...
file at `HEAD`. `ci.yml:190` runs `cargo test -p host-runtime --doc` under the step name
"Rust lease non-escape", so it executes doctests that exist elsewhere in the crate
and covers nothing in this scope. **The `host-runtime` side of this sub-part therefore
...
### `packages/shm-native/src/setup.rs`, 2 tests, and they are the only two that run
...
- `:401-432` `auth_proofs_match_committed_wire_vectors` asserts the same 32-byte
  literal arrays as `docs/host-wire-protocol.md:180` and `:182` that the host's
...
- `tests/activation.rs` holds the startup-order claim, four tests at `:144`,
  ...
  normative seven-step order at `docs/host-wire-protocol.md:586-593`.
...
`host_runtime::setup_socket::activate_client` at `:329` and connects at `:353` and
...
| --- | --- | --- | --- |
| Plugin shared-memory contracts | `bun test packages/plugin/src/shared/host-client` | `:211` | `connection-file.test.ts` (27 tests) is the CI-executed twin of the connection-file reader contract; also `credential-fingerprint.test.ts` (3), `deadline.test.ts` (15), `owner.test.ts` (4), `shm-frame-channel.test.ts` (15) including the terminal-class mappings at `:49-57` |
| Native behaviour (Bun) | `bun run --cwd packages/shm-native test:bun` | `:196` | `tests/suite.test.ts` through `capability.ts`, `mechanism.ts`, `runtime.ts`. Runs under `EIDNARA_SHM_NATIVE_CLAIMED_TARGET: "1"` (`:197`) |
| Native behaviour (Node 24) | `bun run --cwd packages/shm-native test:node` | `:203` | same suite through `tests/runtime.ts`, also with the claimed-target flag (`:202`) |
...
| Plugin shared-memory lifetime (Node 24) | `bun run --cwd packages/plugin test:context-shm:node` | `:214` | native lease and channel lifetime across the runtime boundary |
| Architecture audit | `bun test scripts/check-shm-architecture.test.ts`, `bun run check:shm-architecture` | `:55`, `:58` | the forbidden-name gate, scanning `packages/plugin/src/shared/host-client` and `packages/shm-native/src` among its five roots |

Two more `host-client` gates run outside `shm-source-build`:
`host-client-interop` (`ci.yml:442`) runs the Node 24 smoke at `:461`, and
`ci.yml:216-217` typechecks the plugin.

**`EIDNARA_SHM_NATIVE_CLAIMED_TARGET` inverts the mechanism gate from tolerant to
...
`packages/shm-native/shm_native.node` and asserts its absence. The removal
...
(`:353`), `instance.rs` (`:889`), `packages/shm-native/src/setup.rs` (`:377`).
...
| Cluster | Sites | Label |
| --- | --- | --- |
| Infallible slice-to-array | 2 | `setup_socket.rs:400` and `packages/shm-native/src/setup.rs:290`, both `"four-byte prefix"`, each guarded by a preceding read that fills four bytes |
| HMAC key acceptance | 2 | `auth.rs:147` `"HMAC accepts keys of any length"` and `packages/shm-native/src/setup.rs:229` `"HMAC accepts every key length"` |
...
| peer | `packages/shm-native/src/setup.rs:200` | the expected server proof against the presented one |
| peer | `packages/shm-native/src/setup.rs:201` | the expected daemon id against the presented one |
...
(`packages/shm-native/src/setup.rs:202`) is a plain string comparison, which
`docs/host-wire-protocol.md:190-192` covers by classifying version binding as
...
| `packages/shm-native/src/setup.rs:165`, `:171` | the peer's `goodbye`, same shape |
...
`packages/shm-native/src/setup.rs:164`) that no configuration reaches.
...
(`TimedOut`), at `packages/shm-native/src/setup.rs:359-375`. So nine host-side
distinctions reach the peer as at most three. `NativeStartupError`
(`packages/shm-native/index.ts:34-39`) carries a nine-member closed reason set
...
Verified at `HEAD`: `packages/plugin/src/shared/host-client/auth.test.ts` does
...
(`packages/shm-native/src/setup.rs:401-432`) asserts the same literal vectors,
...
`packages/shm-native/src/setup.rs` names a deleted file or mechanism.
`docs/shm-transport.md` and `docs/host-wire-protocol.md` were audited
...
(`packages/shm-native/src/setup.rs:401-432`) assert the same 64 literal bytes
from `docs/host-wire-protocol.md:180` and `:182`. **Only the peer's runs.** The
...
another: the prose formula at `docs/host-wire-protocol.md:186-190`, the host's
`compute_proof` (exported at `lib.rs:42`), and the peer's `proof` at
`packages/shm-native/src/setup.rs:222-235` with its own copies of the two domain
...
`ring_transport.rs:31`, `packages/shm-native/src/lib.rs:27`, and
`packages/shm-native/index.ts:8`.

The architecture gate that could have caught D1 and D2 does not look at
documentation. `scripts/check-shm-architecture.ts:7-23` lists five source
roots, one file and six manifests, and no documentation path appears; it also skips
`.test.ts` (`:48`) and never walks `crates/host-runtime/tests/`. So both findings above
...
`packages/shm-native/index.ts:151-187` (`packageAddonPath`) implements manifest
...
`bun run --cwd packages/shm-native build:source` before every native and plugin
...
`docs/shm-transport.md:83` promises "before loading" has its
...
- **The key is a bearer capability with no narrowing.**
  `docs/host-wire-protocol.md:26` states that possession grants every
...
- **Host-side profile acceptance.** `docs/host-wire-protocol.md:563` says
...
- **Nonce non-reuse as a MUST.** `docs/host-wire-protocol.md:177` says both
...
- **The two-second budgets stated as coupled.**
  `docs/host-wire-protocol.md:747` states the client's whole-handshake deadline
...
- **The macOS size-seal gap, and its check is absent by design.**
  `docs/shm-transport.md:85` states that macOS does not provide the Linux
  ...
  `shm-transport --test contract --test fuzz_corpus`, and `host-runtime --test client
  --test lifecycle`; none is a resize test.
- **The published socket path in the doc's example does not match the code.**
  `docs/host-wire-protocol.md:76` shows
  `"setup_socket": "/run/user/1000/eidnara/host-runtime.sock"`. The code produces
  `${dataDir}/eidnara/run/setup.sock` (`instance.rs:167`, `:177-179`,
  `runtime.rs:834`), and the same document's `:70` says clients read
  `${dataDir}/eidnara/run/connection.json`, so the example contradicts its
...
- **Whether `packages/shm-native/src/setup.rs` belongs to Part 1 or to 2c is
