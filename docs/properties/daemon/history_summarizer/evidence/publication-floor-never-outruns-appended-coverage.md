...
in the same commit that appends history_segments. Asking what happens if those two
...
- `:487-491` rejects a parsed output with no history_segments:
...
- `:539-556` discard-last healing: when not in emergency, not forcing keep, and
  there are at least two history_segments, if `lookahead_distance <=
  BOUNDARY_HEALING_SLACK` (`:19`, value 2) and popping would not split a completed
  tool arc, the last history_segment is popped and `discarded_last` is set.
- `:625-635` forward-progress check: `offset` is the last prior history_segment's end
...
- `:629` `let last_new_end = history_segments.last().map(|c| c.end_message).unwrap_or(0);`
...
- `:523` copied onto `HistorySummarizerPublishRequest`.

`crates/memory-store/src/lib.rs`:

- `:1776` the field on `HistorySummarizerPublishRequest`.
- `:9484-9488`:
...
The history_segments passed to `append_history_segments_tx` are the same post-pop list that
produced `last_new_end`: `history_summarizer.rs:462-467` maps
`request.validated.history_segments`, and `publish_output_from_awaiting` passes
...
(`history_summarizer_chunk.rs:629-642` starts past `MAX(end_message)`), a later publish's
...
- `history_summarizer_validate.rs:487-491`, empty history_segments after parsing.
- `history_summarizer_validate.rs:625-635`, no forward progress.
- `memory-store:12614-12616`, `append_history_segments_tx` treats an empty list as
...
side-channel items when the history_segment list is empty.
...
fencing (`:1349`), while no history_segment summarized them. They would still be served
...
require an accepted publish with zero history_segments. Three guards prevent it, and
...
- The discard-last pop must happen before `last_new_end` is computed. It does:
  ...
  outrun coverage by exactly the popped history_segment's span. That is the concrete
  regression this record guards.
- `history_summarizer_chunk.rs:629-642` must keep starting the next chunk past
...
1. The general invariant, cheap: after any accepted publish, assert
   ...
   history_segments, and that it did not decrease.
2. The healed case, which is the one that would catch a reordering regression: a
   validated output with at least two history_segments where the last one's end is
   within `BOUNDARY_HEALING_SLACK` of `chunk.end_index`, so the pop fires. Assert
   the floor equals the **surviving** last history_segment's end plus one, not the
   popped one's. `history_summarizer.rs:2309` and `:4199` already assert specific floor
...
3. The emergency and force-keep cases, where the pop is suppressed
   (`history_summarizer_validate.rs:538`), so the floor equals the emitted last end.
4. The rejection cases: `history_summarizer.rs:2360` and `:3006` already assert the floor
   stays `None` after a rejected publish.

## Investigation log

No open questions. The derivation was traced end to end at `HEAD` across
`history_summarizer_validate.rs:638`, `history_summarizer.rs:1725` and `:523`, and
