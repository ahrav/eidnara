...
`host_runtime::run` is `crates/daemon/src/bin/eidnara_host/serve.rs:582-593`:
...
let config = HostConfig {
    ...
    init,
    limits: host_runtime::HostLimits { max_resident_bytes: ..., ..Default::default() },
    ..HostConfig::default()
};
```
...
The specification permits the absence. `docs/host-wire-protocol.md:681`: "A
...
sends a frame, and `docs/host-wire-protocol.md:294` states that "waiting for
...
- Sources examined: repository-wide search for `host_runtime::run`,
...
- Findings: `serve.rs` is the only binary caller. `run_with_publish_hook` is
  ...
  `daemon/examples/direct_host_fixture.rs` is an example, not a shipped
  binary.
