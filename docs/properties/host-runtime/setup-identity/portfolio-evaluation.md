...
read-only from `the `host` source checkout at that commit. Every line
...
(`crates/host-runtime/src/frame_channel.rs:685-694`), and `run_endpoint` returns when
`queue.recv()` yields `None` (`crates/host-runtime/src/ring_transport.rs:437-440`),
...
export (`packages/shm-native/src/lib.rs:490-491`) surfaced to JavaScript as
...
corrections to `packages/shm-native/src/lib.rs` references in two records, and
...
- Any workflow change that runs the `host-runtime` lib target or names
