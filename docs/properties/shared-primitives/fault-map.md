...
| A symlink on the lease path owned by another user | `at-most-one-exclusive-holder-per-key`; refused (`a_lease_path_through_a_foreign_owned_symlink_is_refused`) |
