...
import {
    ...
    isSessionReconciled,
} from "../../features/context/message-index-async";
import {
    closeDatabase,
...
import { getWindowReportsPath } from "../../features/context/window-report-ledger";
...
describe("createEventHandler", () => {
    ...
    it("flags our own hidden children by the eidnara- title prefix", async () => {
        ...
        // Historian, dreamer, sidekick, and migration-title children are eidnara-context children.
        await handler({
            event: {
                type: "session.created",
                properties: {
        ...
        await handler({
            event: {
                type: "session.created",
                properties: {
                    info: { id: "ses-root2", parentID: "", title: "eidnara-compartment" },
                },
            },
        });
        ...
        // Sessions with an empty parentID are not children and are never flagged, even when their title matches an eidnara child.
