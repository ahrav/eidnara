...
The allocator is one line. `crates/host-runtime/src/connection.rs:1399`:
...
`docs/host-wire-protocol.md:750` exempts `Pong` from the watermark because it "reference[s]
existing identities".
...
No in-tree production caller sets it; the only `liveness: Some(..)` in `crates/host-runtime/src/` is at
...
- Sources examined: `crates/host-runtime/src/connection.rs:1399`, `:1412-1418`;
  `crates/host-runtime/src/wire.rs:517-523` (`FrameId::control`), `:571-596` (`encode_owned_frame`).
...
- Sources examined: `grep -rn "next_ping_corr"` across `crates/host-runtime/src/`.
