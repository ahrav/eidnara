...
`../commons/crates/storage` (789 lines, absolute path
`/local/home/ahrav/scratch/commons/crates/storage/src/lib.rs`). That
file is read-only context for this lens: it is the durability contract, and
`memory-store` only consumes it. Citations to it are marked `storage:NNN`.

`crates/context-core` (1,518 lines) and `crates/tokenizer` (85 lines) contain no
...
| 59-399 | Wire and block value types: `HarnessMeta` 59, `WireMessage` 86, `WireBlock` 192, `BlockKind` 268, `ToolOutput` 313, `OutputKind` 330, `ResultBlock` 357, `OpaqueBlock` 372, `MediaBlock` 381, `MediaKind` 391 |
| 401-430 | Constants and `current_time_ms`: `NS = "memory"` 401, `NO_ROW = -1` 404, size caps 405-419, `current_time_ms` 425 |
...
`storage:265-327` `open_sqlite` is the only place any PRAGMA is applied
to the real database file. It sets exactly three:

- `storage:287` `journal_mode = WAL`
- `storage:289` `busy_timeout(Duration::from_secs(5))`
- `storage:291` `foreign_keys = ON`

Then `storage:317-320` tightens file mode on the main file, `-wal`, and
`-shm`, deliberately after the WAL pragma so the siblings exist.

`synchronous` is never set. Not in `open_sqlite`, not in `MemoryStore::open`, not
anywhere in `crates/memory-store/src`. A content search for `PRAGMA` across
...
or read a `format_marker` row.

### Transaction primitives in use

Three, and only three.

- `SqliteStore::with_conn` (`storage:155-161`) — raw `&Connection`
  under the store mutex, no transaction. 73 call sites in production
  `lib.rs`.
- `SqliteStore::with_conn_fenced` (`storage:185-233`) — the write path.
  `storage:191` uses `TransactionBehavior::Immediate`, so the write lock
  is taken at `BEGIN`, not deferred to the first write. It lazily creates
  `fence` (194-199), reads the stored epoch (203-209), rejects a
...
1. `lib.rs:5069-5114` `repair_note_artifacts_v51`. Reads a completion flag
   ...
   (5105-5112). The flag row is a sentinel written into `cache_state` with
...
The runner is `storage:336-385` `run_migrations`. Structure:
...
- 362-383 per migration above `current`: `conn.transaction()` (366, DEFERRED),
  ...
  schema_version` (375-380), `tx.commit()` (381).
...
The schema version is stored in `schema_version`, keyed by
...
`MAX(version)` for namespace `memory`.

`refuse_pre_cutover_store` (`lib.rs:1375-1385`) calls
`recorded_cache_version` (1346-1366), which first checks whether
`schema_version` exists in `main.sqlite_schema` (1348-1358), then
reads `MAX(version)` (1359-1363) and maps `0` to `None` via
`filter(|v| *v > 0)`. A recorded version strictly below 57 is refused with
`MemoryStoreError::PreCutoverModuleStore`. `None` — a fresh file, or one predating
...
- Writer-writer conflict is prevented at two layers before SQLite sees it: the
  ...
  (`storage:279-282`, rejecting a second live writer with
  `StoreError::Lease`), and the process-local `Mutex<Connection>` taken by
  `with_conn` and `with_conn_fenced` (`storage:159, 189`). Within one
...
- When busy does surface, the code path is: `rusqlite::Error` →
  `StoreError::Backend(e.to_string())` (`storage:192, 199, 209, 226,
  229, 231`) → `MemoryStoreError::Store` (`lib.rs:3554-3558`). The error **code is
...
**Journal mode.** WAL, set at `storage:287`. Verified as an expectation
...
`with_conn_fenced` means `tx.commit()` succeeded (`storage:230-231`),
...
**Foreign keys.** `ON`, set at `storage:291`. The migration SQL relies
on it: `claim_mirror_claims` declares `FOREIGN KEY ... ON DELETE CASCADE`
...
its name finds two call sites, both in `crates/memory-store/tests/sqlite_runtime.rs`
...
Required faults and enabling state: `SIGKILL` to the writer between commit and acknowledgement, then reopen through `MemoryStore::open`. To separate process crash from power loss the test needs a second variant that loses the page cache, which a user-space test cannot do; that variant needs `dm-flakey` or equivalent.
Confidence: high on the transaction shape, low on the power-loss half — [evidence](../evidence/acknowledged-commit-survives-process-crash.md). Verified `with_conn_fenced` commits at `storage:230-231` and that no `synchronous` pragma exists in either crate.
...
Reachability: default-production
Status: active
Exercised: not yet — no test reads `PRAGMA synchronous` from a connection produced by `MemoryStore::open`.
Guarantee: The connection `MemoryStore::open` returns runs at a synchronous level the code chose, not one inherited from the build's compile-time default.
Check: `always` — on a connection from `MemoryStore::open`, `PRAGMA synchronous` equals a value some line of code set. `always` because the level governs every commit on that connection, so it must hold at every observation, not merely once at open.
...
Confidence: high — [evidence](../evidence/synchronous-level-is-explicitly-declared-not-inherited.md). Verified by exhaustive content search for `synchronous` across `crates/` and `storage/src/lib.rs`: the only occurrences are the verifier reads at `sqlite_runtime.rs:133-138`, its doc comment at 112, and two test lines.
...
Reachability: default-production
Status: active
Exercised: yes — `crates/memory-store/tests/sqlite_runtime.rs:139-169` probes the live engine and asserts the *failing* branch, expecting `"SQLite 3.46.0 predates the WAL-reset fix in 3.47.1"`.
...
Check: `reachable` — the gate call site must execute on the `MemoryStore::open` path. `reachable` because this is location coverage: the question is whether a specific code point is entered at all, and today it is not.
Fault/timing angle: none. The check either runs at open or does not.
Required faults and enabling state: none. Instrument `evaluate_sqlite_runtime_gate` and call `MemoryStore::open`.
Confidence: high — [evidence](../evidence/wal-reset-gate-runs-on-the-production-open-path.md). Verified that `MemoryStore::open` (`lib.rs:4816-4905`) contains no call, and that a content search for `evaluate_sqlite_runtime_gate` and `probe_sqlite_engine_identity_off_path` across the repository finds definitions in `sqlite_runtime.rs:45, 92` and call sites only in `crates/memory-store/tests/sqlite_runtime.rs`.
...
Guarantee: `verify_sqlite_connection_contract` runs against the connection `MemoryStore::open` produces, so a store whose PRAGMAs did not take effect is refused.
Check: `reachable` — the verifier call site must execute on the `MemoryStore::open` path with the store's own connection. `reachable` because it is location coverage for a specific code point that must be entered.
...
Confidence: high — [evidence](../evidence/connection-contract-is-verified-on-the-production-connection.md). Verified `MemoryStore::open` has no call, and the only call sites are `crates/memory-store/tests/sqlite_runtime.rs:183, 189, 194`.
...
Reachability: default-production
Status: active
Exercised: not yet — no test kills a process mid-migration. `storage:528` `open_runs_migrations_and_seeds_once` and `:580` `later_migration_applies_on_top_of_earlier` cover the happy path.
Guarantee: For any migration, either its statements and its `schema_version` row are both durable, or neither is.
...
Fault/timing angle: the window is `storage:366-383`, between `conn.transaction()` and `tx.commit()`. A kill anywhere inside rolls the whole thing back and the migration re-runs. Two structural notes narrow the guarantee: the `CREATE TABLE IF NOT EXISTS schema_version` at 341-349 and the `MAX(version)` read at 351-357 are outside any transaction, so a crash between them and the first migration transaction leaves an empty version table, which is indistinguishable from a fresh database; and `conn.transaction()` at 366 is DEFERRED, so the write lock is taken at the first statement of the migration rather than at `BEGIN`.
...
Fault/timing angle: three ways they can diverge. First, a crash between the migration commit and the post-migration repairs at `lib.rs:4902-4903`, which alter data rather than schema but are part of what version 57 is supposed to have produced. Second, an out-of-band writer that alters the schema without touching `schema_version`; nothing prevents this because the version lives in an ordinary table, not in `PRAGMA user_version`. Third, `recorded_cache_version` mapping a recorded `0` to `None` at `lib.rs:1364`, so a database with a version row of literal `0` is treated as pristine.
Required faults and enabling state: for the out-of-band case, a raw connection that drops a table and reopens. For the third case, a store seeded with `(memory, 0)`.
Confidence: medium — [evidence](../evidence/recorded-schema-version-cannot-disagree-with-the-actual-schema.md). Verified the version storage and read path at `storage:341-357` and `lib.rs:5348-5358`. Did not build the full declared-object inventory from the 878-line bootstrap.
...
1. **The Rust runtime probe is not implemented.**
   ...
   `MemoryStore::open` (`lib.rs:4816-4905`) does not call them.
...
2. **Application connections do not verify anything.**
   ...
   only from `crates/memory-store/tests/sqlite_runtime.rs:183, 189, 194`.
...
5. **The format identity is not applied to `store.db`.**
   ...
   immutable `format_marker` row, a 32-hex incarnation ID, a manifest digest,
   and an exact `main.sqlite_schema` inventory. For `store.db` none of these are
   set or checked: identity is the `schema_version` table's
   `MAX(version)` for namespace `memory` (`storage:351-357`,
...
6. **"Bootstrapped under `BEGIN IMMEDIATE`" is true for writes, not for
   ...
   `with_conn_fenced` does use `IMMEDIATE` (`storage:191`), but the
   migration runner uses `conn.transaction()`, which is DEFERRED
   (`storage:366`), and the version-table creation and current-version
   read precede any transaction (`storage:341-357`).
...
7. **"Migration version ranges are not supported production inputs" is enforced
   ...
   (`storage:363-365`). So an older binary opens a newer database
...
- What is `PRAGMA synchronous` on a connection returned by `MemoryStore::open`? The
...
- What is `PRAGMA wal_autocheckpoint`? Inferred as the 1000-page default, since
  nothing sets it.
- Does `pragma_update(None, "journal_mode", "WAL")` at `storage:287`
...
- Is the `rusqlite` 0.32 to WAL-reset-safe bump tracked? `Cargo.toml:30`
  says it must be coordinated with `storage`.
- Should the gate and the connection-contract verifier be wired into
  `MemoryStore::open` before the engine bump? Wiring them today would presumably
  make every open fail.
- Should `store.db` adopt the `application_id` / `user_version` /
  `format_marker` identity the doc describes and the TypeScript side already
