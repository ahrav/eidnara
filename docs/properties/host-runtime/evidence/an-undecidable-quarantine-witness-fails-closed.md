...
(`crates/host-runtime/src/lifecycle.rs:250-257`) and then the manifest-side gate
(`crates/host-runtime/src/generation.rs:941-968`) shows two gates answering the same
question - "can I prove this schema is known?" - with opposite defaults.

## Evidence trail

The repaired gate is `quarantined_record_present`
(`crates/host-runtime/src/lifecycle.rs:257-284`). It splits absence from
...
(`crates/host-runtime/src/generation.rs:941-968`). Its four early returns are:
...
(`crates/host-runtime/src/instance.rs:852-865`) also errors when the accumulated
...
- Sources examined: `generation.rs:941-968`, `:976-1016`, `:1259-1307`,
  `:364-384`; `crates/host-runtime/src/instance.rs:852-865`;
  `crates/host-runtime/src/lifecycle.rs:250-284`; the one existing test at
